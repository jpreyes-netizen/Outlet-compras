...
function RepoView({prods,cart,setCart,go,config,params,paramsABCD}){
  const[q,setQ]=useState("");const[fc,setFc]=useState("T");const[ft,setFt]=useState("T")
  const tipos=[...new Set(prods.map(p=>p.tipo_producto).filter(Boolean))].sort()
  const fil=prods.filter(i=>(fc==="T"||i.clasif_abcd===fc)&&(ft==="T"||i.tipo_producto===ft)&&(!q||i.producto?.toLowerCase().includes(q.toLowerCase())||i.sku?.toLowerCase().includes(q.toLowerCase())))
  ...
  return<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
      <div><div style={{fontSize:22,fontWeight:800,color:"#1C1C1E",letterSpacing:"-0.02em"}}>Reposición</div><div style={{fontSize:13,color:"#8E8E93"}}>{prods.length} productos · Umbral quiebre: {config.umbral_quiebre_pct||30}%</div></div>
      {cL>0&&<Bt v="pri" onClick={go} ic="➕">OC ({cL})</Bt>}
    </div>

    {cL>0&&<Cd ac="#007AFF" s={{marginBottom:12,background:"#007AFF08"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{fontSize:13,fontWeight:600,color:"#007AFF"}}>{cL} SKUs seleccionados</div><div style={{fontSize:11,color:"#8E8E93"}}>Total: {fmt(tC)}</div></div>
        <Bt sm v="gry" onClick={()=>setCart({})}>Vaciar</Bt>
      </div>
    </Cd>}

    <div style={{display:"flex",gap:6,marginBottom:10}}>
      <input placeholder="🔍 Buscar producto o SKU..." value={q} onChange={e=>setQ(e.target.value)} style={{...css.input,flex:1}}/>
      <select value={fc} onChange={e=>setFc(e.target.value)} style={{...css.select,width:80}}>
        <option value="T">ABCD</option>{["A","B","C","D"].map(c=><option key={c} value={c}>{c}</option>)}
      </select>
      <select value={ft} onChange={e=>setFt(e.target.value)} style={{...css.select,width:120}}>
        <option value="T">Todos</option>{tipos.map(t=><option key={t} value={t}>{t}</option>)}
      </select>
    </div>

    {fil.map(it=>{const inC=!!cart[it.sku];const crit=(it.dias_cobertura||999)<15&&it.clasif_abcd==="A";const cp=inC?cart[it.sku].cp:(it.reposicion_necesaria||0)
      return<div key={it.sku} onClick={()=>tog(it)} style={{...css.card,marginBottom:6,cursor:"pointer",background:inC?"#007AFF08":"#fff",borderColor:inC?"#007AFF30":"rgba(0,0,0,0.04)",transition:"all 0.2s"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"start",gap:8}}>
          <div style={{display:"flex",gap:10,alignItems:"start",flex:1}}>
            <input type="checkbox" checked={inC} onChange={()=>{}} style={{marginTop:4,accentColor:"#007AFF"}}/>
            <div style={{flex:1,minWidth:0}}>
              <div style={{display:"flex",alignItems:"center",gap:6,marginBottom:2}}>
                <Bd c={CL[it.clasif_abcd]?.c} bg={CL[it.clasif_abcd]?.bg}>{it.clasif_abcd}</Bd>
                {crit&&<Bd c="#FF3B30" bg="#FF3B3015">⚠ URGENTE</Bd>}
              </div>
              <div style={{fontSize:14,fontWeight:600,color:"#1C1C1E"}}>{it.producto}</div>
              <div style={{fontSize:11,color:"#8E8E93"}}>{it.tipo_producto} · {it.sku}</div>
              <div style={{display:"flex",gap:12,marginTop:4,fontSize:12}}>
                <span style={{color:"#8E8E93"}}>Stock: <strong style={{color:"#1C1C1E"}}>{fN(it.stock_actual)}</strong></span>
                {it.stock_transito>0&&<span style={{color:"#007AFF"}}>Tránsito: <strong>{fN(it.stock_transito)}</strong></span>}
                <span style={{color:crit?"#FF3B30":"#8E8E93"}}>Cob: <strong>{Math.round(it.dias_cobertura||0)}d</strong></span>
              </div>
            </div>
          </div>
          <div style={{textAlign:"right",flexShrink:0}}>
            {inC?<div><input type="number" value={cp} onChange={e=>{e.stopPropagation();setQty(it.sku,e.target.value)}} onClick={e=>e.stopPropagation()} style={{width:80,textAlign:"center",fontSize:14,fontWeight:700,padding:"6px",border:"2px solid #007AFF",borderRadius:10,color:"#007AFF"}}/><div style={{fontSize:10,color:"#8E8E93",marginTop:2}}>Sugerido: {fN(it.reposicion_necesaria)}</div></div>
            :<div><div style={{fontSize:15,fontWeight:700,color:"#1C1C1E"}}>{fN(it.reposicion_necesaria)}</div><div style={{fontSize:10,color:"#8E8E93"}}>{fmt(it.costo_reposicion)}</div></div>}
          </div>
        </div>
      </div>
    })}
