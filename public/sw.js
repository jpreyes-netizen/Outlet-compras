    ...
    self.clients.matchAll({ type: 'window', includeUncontrolled: true }).then(lista => {
      for (const c of lista) {
        if (c.url.indexOf(self.location.origin) === 0 && 'focus' in c) {
          // OJO: la app ya abierta se navega con un hash (#go/...). Cambiar solo
          // el hash NO recarga la SPA, y deeplink.js parsea una unica vez al
          // cargar el bundle. Por eso se avisa por postMessage y el cliente
          // decide: fija el hash y recarga. Sin esto la notificacion abre la
          // app pero deja al usuario donde estaba.
          try { c.postMessage({ type: 'DEEPLINK', url: destino }) } catch (e) {}
          return c.focus()
        }
      }
      // Sin ventana abierta: openWindow carga el bundle de cero y deeplink.js
      // lee el hash normalmente.
