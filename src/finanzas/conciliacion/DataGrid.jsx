...
import { Maximize2, Minimize2, Download, Filter as FilterIcon, X, Loader2 } from 'lucide-react'
...
export function DataGrid({
  ...
  // Si el juego de columnas cambia (misma instancia reutilizada para otra tabla),
  // hay que re-sincronizar el orden visual: si no, sobreviven solo las columnas
  // que existían en ambas y el resto desaparece de la grilla.
  const firmaCols = columns.map(c => c.key).join('|')
  useEffect(() => {
    const nuevas = columns.map(c => c.key)
    setOrdenCols(prev => {
      // mismas columnas (quizá reordenadas a mano): se respeta el orden del usuario
      if (prev.length === nuevas.length && nuevas.every(k => prev.includes(k))) return prev
      return nuevas
    })
    setFiltros(f => Object.fromEntries(Object.entries(f).filter(([k]) => nuevas.includes(k))))
    setOrden(o => (o.key && !nuevas.includes(o.key) ? { key: null, dir: 'asc' } : o))
...
// Celda editable: dropdown inline. col.editable = { options:[{value,label}], value(row), onSave(row, nuevoValor), disabled(row)? }
function CeldaEditable({ row, col }) {
  const ed = col.editable
  const [guardando, setGuardando] = useState(false)
  const actual = ed.value ? ed.value(row) : row[col.key]
  const disabled = ed.disabled ? ed.disabled(row) : false

  if (disabled) {
    return <span style={{ fontSize: 11, color: '#CBD5E1' }}>{col.render ? col.render(row) : (actual ?? '—')}</span>
  }
  ...
  async function onChange(e) {
    ...
    setGuardando(true)
    try { await ed.onSave(row, nuevo) }
    finally { setGuardando(false) }
  }

  return (
    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 4 }}>
      <select value={actual ?? ''} onChange={onChange} disabled={guardando}
        style={{ fontSize: 11, padding: '3px 6px', borderRadius: 6, border: '1px solid #CBD5E1', background: guardando ? '#F1F5F9' : '#fff', color: '#334155', cursor: 'pointer', maxWidth: 150 }}>
        {(!actual || !ed.options.some(o => o.value === actual)) && <option value="">— sin clasificar —</option>}
        {ed.options.map(o => <option key={o.value} value={o.value}>{o.label}</option>)}
      </select>
      {guardando && <Loader2 size={11} style={{ color: '#94A3B8' }} />}
    </span>
  )
}
