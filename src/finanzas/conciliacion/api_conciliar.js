...
export async function fetchMovimientos(filtros) {
  // Lee la vista enmascarada: glosas de socios llegan ya protegidas desde
  // Postgres para roles no directivos, con montos intactos (cuadratura al peso).
  // total_aplicado viene agregado por la vista (sin embed conciliaciones).
  let q = supabase
    .from('v_movimientos_conciliacion')
    .select('id, cartola_id, fecha, tipo, monto, descripcion, referencia, estado, subcuenta_id, ceco_id, total_aplicado, enmascarado')
  ...
  let mapped = (data ?? []).map(r => {
    const aplicado = Number(r.total_aplicado) || 0
    ...
    return {
      ...
      estado: r.estado, total_aplicado: aplicado, enmascarado: !!r.enmascarado,
      subcuenta_id: r.subcuenta_id ?? null, ceco_id: r.ceco_id ?? null,
  ...
  // Adjuntar la clasificación CONTABLE (plan de cuentas) desde movimientos_bancarios.subcuenta_id.
  // Es distinta de la categoría de flujo: sirve para el EERR. ~99% de los movimientos la tienen.
  {
    const subIds = [...new Set(mapped.map(m => m.subcuenta_id).filter(Boolean))]
    if (subIds.length) {
      const { data: subs } = await supabase.from('subcuentas')
        .select('id, codigo, nombre').in('id', subIds)
      const subMap = new Map((subs ?? []).map(s => [s.id, s]))
      mapped.forEach(m => {
        const s = m.subcuenta_id ? subMap.get(m.subcuenta_id) : null
        m.subcuenta_codigo = s?.codigo ?? null
        m.subcuenta_nombre = s?.nombre ?? null
      })
    }
  }
...
export async function fetchCargosCandidatos({ factura, texto }) {
  ...
  async function buscar(patron) {
    const { data: rows, error } = await supabase.from('v_movimientos_conciliacion')
      .select('id, fecha, monto, descripcion, tipo, estado, total_aplicado')
      .eq('tipo', 'CARGO')
      .eq('estado', 'clasificado')
  ...
  let cargos = data.map(m => {
    const aplicado = Number(m.total_aplicado) || 0
...
// ============ Bolsa de no-conciliables (v0.0.38) ============
// Facturas que no tienen contrapartida bancaria directa: comisiones que se
// descuentan del abono, facturas pagadas por el agente vía provisión, etc.
// Se marcan conciliable_banco=false con un motivo tipificado → salen del
// pendiente pero quedan trazadas y filtrables por motivo.

export async function fetchMotivosNoConciliable() {
  const { data, error } = await supabase.from('motivos_no_conciliable')
    .select('motivo, label, descripcion, color_hex, orden').eq('activo', true).order('orden')
  if (error) throw error
  return data ?? []
}

export async function marcarNoConciliableMasivo(facturaIds, motivo, nota = null) {
  const { data, error } = await supabase.rpc('fn_marcar_no_conciliable', {
    p_factura_ids: facturaIds, p_motivo: motivo, p_nota: nota,
  })
  if (error) throw error
  return data  // cantidad de facturas afectadas
}

export async function revertirNoConciliableMasivo(facturaIds) {
  const { data, error } = await supabase.rpc('fn_revertir_no_conciliable', {
...
// ============ Reembolso de provisión (abono contra la provisión) ============
// El agente reembolsa el excedente vía ABONO en la cartola. Se concilia contra
// la provisión como monto_aplicado NEGATIVO → reduce el "pagado neto" y la
// cuadratura (pagado neto − rendido) cierra. v_provisiones_estado ya suma con
// signo, así que un aplicado negativo resta correcto.

// Abonos de un RUT (agente) aún no conciliados a ninguna provisión
export async function fetchAbonosAgente(agenteRut) {
  const cuerpo = String(agenteRut ?? '').replace(/[.\-]/g, '').replace(/[0-9kK]$/, '')
  if (!cuerpo) return []
  const { data, error } = await supabase.from('movimientos_bancarios')
    .select('id, fecha, monto, descripcion, estado, conciliaciones(id, provision_id, monto_aplicado)')
    .eq('tipo', 'ABONO')
    .ilike('descripcion', `%${cuerpo}%`)
    .order('fecha', { ascending: false }).limit(50)
  if (error) throw error
  // Excluir abonos que ya tienen una conciliación con provision_id
  return (data ?? []).filter(m => !(m.conciliaciones ?? []).some(c => c.provision_id))
}

export async function registrarReembolso({ movimientoId, provisionId, monto, observaciones }) {
  const { data: sess } = await supabase.auth.getSession()
  const userId = sess.session?.user?.id ?? null
  // monto_aplicado NEGATIVO: el reembolso reduce el pagado neto de la provisión
  const row = {
    movimiento_id: movimientoId, tipo_respaldo: 'provision_aduana',
    provision_id: provisionId, monto_aplicado: -Math.abs(monto),
    observaciones: `[REEMBOLSO] ${observaciones ?? ''}`.trim(), created_by: userId,
  }
  const { error } = await supabase.from('conciliaciones').insert(row)
  if (error) throw error
  // El abono queda marcado como conciliado (procesado)
  await supabase.from('movimientos_bancarios').update({ estado: 'conciliado' }).eq('id', movimientoId)
}

// ============ Clasificación masiva desde banco (v0.0.39 Pieza 2) ============
// Clasifica movimientos a una categoría de flujo escribiendo en clasif_pagos_rut
// por el cuerpo del RUT. Como es por RUT, arrastra todos los del mismo RUT.

export async function clasificarMovimientosMasivo(movimientoIds, categoria, nota = null) {
  const { data, error } = await supabase.rpc('fn_clasificar_movimientos_masivo', {
    p_movimiento_ids: movimientoIds, p_categoria: categoria, p_nota: nota,
  })
  if (error) throw error
  // La función devuelve TABLE → data es array con 1 fila { ruts_afectados, movimientos_cubiertos }
