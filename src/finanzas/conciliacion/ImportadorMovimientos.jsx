...
import { Upload, Loader2, TrendingUp, TrendingDown, CheckCircle2, AlertTriangle, AlertCircle, Info } from 'lucide-react'
...
export function ImportadorMovimientos({ onImportado }) {
  ...
  // ── Parse + análisis de duplicados contra BD + duplicados internos ──
  async function parsear(file) {
    ...
    try {
      ...
      const { data: existentes } = await supabase.from('movimientos_bancarios')
        .select('fecha, monto, descripcion').in('fecha', fechas).limit(5000)
      const setExist = new Set((existentes ?? []).map(e => `${e.fecha}|${e.monto}|${e.descripcion}`))

      // Detectar duplicados INTERNOS dentro del Excel (mismo fecha+monto+descripcion)
      // Asignar grupo_dup_id: si dos movs comparten key, comparten id de grupo
      const conteoInterno = new Map()
      for (const m of movimientos) {
        const key = `${m.fecha}|${m.monto}|${m.descripcion}`
        conteoInterno.set(key, (conteoInterno.get(key) ?? 0) + 1)
      }

      // Marcar cada movimiento: en_bd (ya existe en BD) | dup_interno (repetido dentro del Excel)
      let groupCounter = 0
      const grupoIds = new Map()
      const movsMarcados = movimientos.map((m, idx) => {
        const key = `${m.fecha}|${m.monto}|${m.descripcion}`
        const en_bd = setExist.has(key)
        const veces_excel = conteoInterno.get(key)
        const dup_interno = veces_excel > 1
        if (dup_interno && !grupoIds.has(key)) grupoIds.set(key, ++groupCounter)
        return {
          ...m,
          idx_excel: idx,                  // posición original en el Excel
          es_duplicado: en_bd,             // ya está en BD
          dup_interno,                     // hay otro idéntico en el mismo Excel
          grupo_dup_id: dup_interno ? grupoIds.get(key) : null,
        }
      })

      const nEnBD       = movsMarcados.filter(m => m.es_duplicado).length
      const nDupInterno = movsMarcados.filter(m => m.dup_interno).length
      const nNuevos     = movsMarcados.filter(m => !m.es_duplicado).length

      setPreview({
        movimientos: movsMarcados,
        metadata,
        file: file.name,
        analisis: { nEnBD, nDupInterno, nNuevos, tieneConflicto: nEnBD > 0 || nDupInterno > 0 },
        importarDupsInternos: 'todos',  // 'todos' (incluye duplicados internos) | 'solo_primero' (solo el primero de cada grupo)
      })
      // Si hay duplicados internos, mostrar esa pestaña automáticamente
      if (nDupInterno > 0) setTabVista('dup_interno')
      else if (nEnBD > 0) setTabVista('duplicados')
    } catch (e) {
  ...
  async function confirmar() {
    ...
    try {
      ...
      // Evita reinsertar movimientos aunque vengan de una cartola distinta
      const fechas = [...new Set(movimientos.map(m => m.fecha))]
      const { data: existentes } = await supabase.from('movimientos_bancarios')
      ...
      // 2a) Filtrar los que ya están en BD
      let candidatos = movimientos.filter(m => !setExist.has(`${m.fecha}|${m.monto}|${m.descripcion}`))
      const duplicadosEnBD = movimientos.length - candidatos.length

      // 2b) Si el usuario eligió "solo_primero", quitar duplicados internos manteniendo el primero
      const modoDup = preview.importarDupsInternos ?? 'todos'
      let dupInternosOmitidos = 0
      if (modoDup === 'solo_primero') {
        const seen = new Set()
        candidatos = candidatos.filter(m => {
          const key = `${m.fecha}|${m.monto}|${m.descripcion}`
          if (seen.has(key)) { dupInternosOmitidos++; return false }
          seen.add(key); return true
        })
      }

      // 3) Insertar en lotes de 500 — usar upsert con ignoreDuplicates por si acaso
      ...
      for (let i = 0; i < candidatos.length; i += 500) {
        ...
        // upsert con ignoreDuplicates: si el índice rechaza alguno, lo ignora silenciosamente en vez de explotar
        const { data, error } = await supabase.from('movimientos_bancarios')
          .upsert(lote, { onConflict: 'fecha,monto,descripcion', ignoreDuplicates: true })
          .select('id')
      ...
      const duplicados = duplicadosEnBD + dupInternosOmitidos + conflictosIndice
  ...
  const movsFiltrados = useMemo(() => {
    ...
    if (tabVista === 'dup_interno') return preview.movimientos.filter(m => m.dup_interno).sort((a, b) => a.grupo_dup_id - b.grupo_dup_id || a.idx_excel - b.idx_excel)
      ...
      {preview && metricas && (
        <>
            ...
            <div style={{
              ...
              <div style={{ flex: 1 }}>
                <div style={{ fontSize: 13, fontWeight: 700, color: '#78350F', marginBottom: 4 }}>
                  {preview.analisis.nDuplicados} movimiento{preview.analisis.nDuplicados > 1 ? 's' : ''} ya existen en la base de datos
                </div>
                <div style={{ fontSize: 12, color: '#92400E', lineHeight: 1.5 }}>
                  Misma fecha, monto y descripción detectados. Se importarán solo los <strong>{preview.analisis.nNuevos} nuevos</strong> — los duplicados serán ignorados automáticamente.
                  Revisa la pestaña <strong>"Duplicados ({preview.analisis.nDuplicados})"</strong> para ver el detalle.
                </div>
              </div>
            </div>
          )}
          ...
          <div style={{ ...cardSt, padding: 0, overflow: 'hidden' }}>
            ...
            <div style={{ padding: '12px 16px', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 2, background: '#F1F5F9', borderRadius: 8, padding: 3 }}>
                {[
                  ...
                  ...(preview.analisis?.nEnBD > 0
                    ? [{ k: 'duplicados', l: `⚠ En BD (${preview.analisis.nEnBD})`, warn: true }]
                    : []),
                  ...(preview.analisis?.nDupInterno > 0
                    ? [{ k: 'dup_interno', l: `⚠ Duplicados internos (${preview.analisis.nDupInterno})`, warn: true }]
                    : []),
                ].map(({ k, l, warn }) => (
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={confirmar} disabled={loading} style={{ ...btnSt('#1E40AF'), opacity: loading ? 0.6 : 1 }}>
                  ...
                  {(() => {
                    const enBD = preview.analisis?.nEnBD ?? 0
                    const dupInt = preview.analisis?.nDupInterno ?? 0
                    const modo = preview.importarDupsInternos ?? 'todos'
                    const aOmitir = modo === 'solo_primero' ? (preview.movimientos.filter(m => m.dup_interno).length - new Set(preview.movimientos.filter(m => m.dup_interno).map(m => m.grupo_dup_id)).size) : 0
                    const aImportar = preview.movimientos.length - enBD - aOmitir
                    if (enBD > 0 || aOmitir > 0) return `Importar ${aImportar} (${enBD} en BD${aOmitir > 0 ? ` + ${aOmitir} dup internos omitidos` : ''})`
                    return `Importar ${preview.movimientos.length} movimientos`
                  })()}
                </button>
              </div>
            </div>
            ...
            {/* Selector de acción para duplicados internos */}
            {tabVista === 'dup_interno' && preview.analisis?.nDupInterno > 0 && (
              <div style={{ padding: '10px 16px', background: '#FEF3C7', borderBottom: '1px solid #FCD34D', display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 14, flexWrap: 'wrap' }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 11, color: '#92400E' }}>
                  <AlertCircle size={14} />
                  <span>
                    Se detectaron <strong>{preview.analisis.nDupInterno}</strong> movimientos con misma fecha, monto y descripción. Pueden ser pagos legítimos repetidos o errores del banco.
                  </span>
                </div>
                <div style={{ display: 'flex', gap: 4, background: '#fff', borderRadius: 7, padding: 2, border: '1px solid #FCD34D' }}>
                  {[
                    { k: 'todos',         l: 'Importar todos' },
                    { k: 'solo_primero',  l: 'Solo el primero de cada grupo' },
                  ].map(({ k, l }) => (
                    <button key={k} onClick={() => setPreview(p => ({ ...p, importarDupsInternos: k }))} style={{
                      padding: '4px 10px', borderRadius: 5, border: 'none', fontSize: 11, fontWeight: 600, cursor: 'pointer',
                      background: (preview.importarDupsInternos ?? 'todos') === k ? '#D97706' : 'transparent',
                      color: (preview.importarDupsInternos ?? 'todos') === k ? '#fff' : '#92400E',
                    }}>{l}</button>
                  ))}
                </div>
              </div>
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                ...
                <tbody>
                  {movsFiltrados.slice(0, 150).map((m, i) => {
                    ...
                    const bgRow = m.es_duplicado ? '#FFFBEB' : (m.dup_interno ? '#FEF2F2' : 'transparent')
                    const bgHover = m.es_duplicado ? '#FEF3C7' : (m.dup_interno ? '#FEE2E2' : '#F8FAFC')
                    return (
                      <tr key={i}
                        ...
                        <td style={TD}>
                          {fmtFecha(m.fecha)}
                          {m.dup_interno && (
                            <div style={{ fontSize: 9, color: '#B91C1C', marginTop: 2, fontWeight: 700 }}>
                              Grupo #{m.grupo_dup_id}
                            </div>
                          )}
                        </td>
                        <td style={TD}>
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
                            ...
                            {m.dup_interno && !m.es_duplicado && (
                              <span style={{ fontSize: 9, fontWeight: 700, padding: '1px 5px', borderRadius: 4, background: '#FECACA', color: '#B91C1C' }}>
                                REPETIDO
                              </span>
                            )}
                          </div>
