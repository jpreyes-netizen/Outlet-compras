...
export function ImportadorMovimientos({ onImportado }) {
  ...
  async function parsear(file) {
    ...
    try {
      ...
      // Marcar cada movimiento: nuevo | duplicado
      const movsMarcados = movimientos.map(m => ({
        ...m,
        es_duplicado: setExist.has(`${m.fecha}|${m.monto}|${m.descripcion}`),
      }))

      const nDuplicados = movsMarcados.filter(m => m.es_duplicado).length
      const nNuevos     = movsMarcados.length - nDuplicados

      setPreview({
        movimientos: movsMarcados,
        metadata,
        file: file.name,
        analisis: { nDuplicados, nNuevos, tieneConflicto: nDuplicados > 0 },
      })
      // Si hay duplicados, mostrar la pestaña de duplicados automáticamente
      if (nDuplicados > 0) setTabVista('duplicados')
    } catch (e) {
  ...
  async function confirmar() {
    ...
    try {
      ...
      const nuevos = movimientos.filter(m => !setExist.has(`${m.fecha}|${m.monto}|${m.descripcion}`))
      const duplicados = movimientos.length - nuevos.length

      // 3) Insertar en lotes de 500
      let insertados = 0
      for (let i = 0; i < nuevos.length; i += 500) {
        const lote = nuevos.slice(i, i + 500).map(m => {
          ...
          return {
            ...
            estado: 'pendiente',
            mes_cartola: mes,
            mes_nominal: mes,  // por defecto igual al mes_cartola; el usuario puede ajustar
          }
        })
        const { error } = await supabase.from('movimientos_bancarios').insert(lote)
            ...
            <div style={{ padding: '12px 16px', borderBottom: '1px solid #F1F5F9', display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 10 }}>
              ...
              <div style={{ display: 'flex', gap: 2, background: '#F1F5F9', borderRadius: 8, padding: 3 }}>
                {[
                  ...
                  ...(preview.analisis?.nDuplicados > 0
                    ? [{ k: 'duplicados', l: `⚠ Duplicados (${preview.analisis.nDuplicados})`, warn: true }]
                    : []),
                ].map(({ k, l, warn }) => (
              ...
              <div style={{ display: 'flex', gap: 8 }}>
                ...
                <button onClick={confirmar} disabled={loading} style={{ ...btnSt('#1E40AF'), opacity: loading ? 0.6 : 1 }}>
                  ...
                  {preview.analisis?.nDuplicados > 0
                    ? `Importar ${preview.analisis.nNuevos} nuevos (${preview.analisis.nDuplicados} duplicados ignorados)`
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                ...
                <tbody>
                  {movsFiltrados.slice(0, 150).map((m, i) => {
                    ...
                    return (
                      <tr key={i}
                        style={{ borderTop: '1px solid #F1F5F9', background: m.es_duplicado ? '#FFFBEB' : 'transparent' }}
                        onMouseEnter={e => e.currentTarget.style.background = m.es_duplicado ? '#FEF3C7' : '#F8FAFC'}
                        onMouseLeave={e => e.currentTarget.style.background = m.es_duplicado ? '#FFFBEB' : 'transparent'}>
