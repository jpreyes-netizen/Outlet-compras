...
import { Search, Loader2, Check, Sparkles, FileText, AlertCircle, RefreshCw } from 'lucide-react'
import { supabase } from '../../supabase'
import { fetchFacturas, fetchSugerenciasDeFactura, fetchCargosCandidatos, vincularRespaldo } from './api_conciliar'
...
// ════════════════════════════════════════════════════════════════════════
export function ConciliarDesdeFacturasFlow() {
  const [filtros, setFiltros] = useState({ desde: null, hasta: null, estado: 'sin_pagar', texto: '' })
  ...
  return (
    <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 16, height: 'calc(100vh - 260px)', minHeight: 520 }}>
...
function FacturasPanel({ facturas, loading, selId, onSelect, filtros, onFiltrosChange, onReload }) {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', overflow: 'hidden', borderRadius: 14, border: '1px solid #E2E8F0', background: '#fff', height: '100%' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto' }}>
        <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
          ...
          <tbody>
            {loading && <tr><td colSpan={5} style={{ padding: 40, textAlign: 'center', color: '#94A3B8' }}><Loader2 size={16} /></td></tr>}
            {!loading && facturas.length === 0 && <tr><td colSpan={5} style={{ padding: 40, textAlign: 'center', color: '#94A3B8', fontSize: 12 }}>No hay facturas que coincidan.</td></tr>}
            {!loading && facturas.map(f => {
              const st = EST_FACT[f.estado_factura] ?? EST_FACT.sin_pagar
              ...
              return (
                <tr key={f.id} onClick={() => onSelect(f.id)}
                  ...
                  <td style={TD}>
                    <span style={{ display: 'inline-block', padding: '2px 8px', borderRadius: 99, fontSize: 10, fontWeight: 600, background: st.pill.bg, color: st.pill.c }}>{st.label}</span>
                  </td>
                </tr>
              )
            })}
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', border: '1px solid #E2E8F0', borderRadius: 14, background: '#fff', overflow: 'hidden' }}>
      ...
      <div style={{ flex: 1, overflowY: 'auto', padding: '0 12px' }}>
        ...
        {!loadingCargos && cargos.length === 0 && <div style={{ padding: 30, textAlign: 'center', color: '#94A3B8', fontSize: 12 }}>Sin pagos candidatos. Busca por la descripción del banco.</div>}
