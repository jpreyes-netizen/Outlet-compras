...
import { Search, Loader2, Check, Sparkles, FileText, AlertCircle, RefreshCw, Archive, Undo2, X, Table2 } from 'lucide-react'
...
function FacturasPanel({ facturas, loading, selId, onSelect, filtros, onFiltrosChange, onReload, marcadas, onToggleMarcada, onToggleTodas, motivos, onLimpiarMarcadas, onAfterBulk }) {
  ...
  const columnasGrid = useMemo(() => [
    { key: 'fecha_emision', label: 'Fecha', width: 100, value: r => r.fecha_emision, render: r => fmtFecha(r.fecha_emision) },
    { key: 'folio', label: 'Folio', width: 100 },
    { key: 'razon_social', label: 'Proveedor', width: 240 },
    { key: 'rut_proveedor', label: 'RUT', width: 120 },
    { key: 'monto_total', label: 'Monto total', align: 'right', width: 120, value: r => r.monto_total ?? 0, render: r => fmtCLP(r.monto_total), exportValue: r => r.monto_total ?? 0 },
    { key: 'saldo', label: 'Saldo', align: 'right', width: 120, value: r => r.saldo ?? 0, render: r => fmtCLP(r.saldo), exportValue: r => r.saldo ?? 0 },
    { key: 'estado_factura', label: 'Estado', width: 110, value: r => r.conciliable_banco === false ? 'no aplica' : (r.estado_factura || ''), render: r => r.conciliable_banco === false ? (r.motivo_no_conciliable || 'No aplica') : (EST_FACT[r.estado_factura]?.label ?? r.estado_factura) },
  ], [])
      ...
      <div style={{ borderBottom: '1px solid #E2E8F0', padding: 14 }}>
        <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', marginBottom: 10 }}>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 6 }}>
            <button onClick={() => setVistaExcel(true)} title="Vista Excel"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 10px', borderRadius: 8, border: '1px solid #E2E8F0', background: '#fff', fontSize: 11, fontWeight: 600, color: '#475569', cursor: 'pointer' }}>
        ...
        {vistaExcel && (
          <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(15,23,42,0.4)', padding: 16 }}>
            <div style={{ height: '100%', position: 'relative' }}>
              ...
              <DataGrid
                title="Libro de compras — Vista Excel"
                exportName="facturas_compra"
                columns={columnasGrid}
                rows={facturas}
                getRowId={r => r.id}
                selectedId={selId}
                onRowClick={f => { onSelect(f.id); setVistaExcel(false) }}
                loading={loading}
                emptyText="No hay facturas que coincidan con los filtros."
              />
            </div>
          </div>
        )}
