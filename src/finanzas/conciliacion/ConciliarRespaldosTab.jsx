...
import { ConciliarDesdeFacturasFlow } from './ConciliarDesdeFacturas'

export function ConciliarRespaldosTab() {
  const [filtros, setFiltros] = useState({ estado: 'todos', desde: null, hasta: null, texto: '', soloCargo: true, soloSugerencia: false, soloConciliables: true, proveedor: '', categoria: 'todas', montoMin: '', montoMax: '', diasMin: '', diasMax: '' })
  const [movimientos, setMovimientos] = useState([])
  const [loading, setLoading] = useState(true)
  const [selectedId, setSelectedId] = useState(null)
  const [multiSel, setMultiSel] = useState([])
  const [modoConc, setModoConc] = useState('factura')   // 'factura' (default) | 'banco'
  ...
  return (
    <div style={ampliado ? {
      position: 'fixed', inset: 0, zIndex: 9999, background: '#F9FAFB',
      padding: 10, overflow: 'auto', display: 'flex', flexDirection: 'column', gap: 6,
    } : { display: 'flex', flexDirection: 'column', gap: 6 }}>
      {/* Barra ejecutiva única: sync BSALE + toggle modo + ampliar (todo en una fila densa) */}
      <div style={{
        ...
        {/* Toggle modo */}
        <div style={{ display: 'flex', gap: 3, padding: 2, background: '#F3F4F6', borderRadius: 7 }}>
          {[
            { k: 'factura', l: 'Desde facturas', c: '#7C3AED', desc: 'Parte del libro de compras' },
            { k: 'banco', l: 'Desde banco', c: '#15803D', desc: 'Parte de la cartola' },
          ].map(t => (
            <button key={t.k} onClick={() => setModoConc(t.k)} title={t.desc} style={{
              padding: '4px 12px', fontSize: 12, fontWeight: 600,
              border: 'none', cursor: 'pointer', borderRadius: 5,
              background: modoConc === t.k ? '#fff' : 'transparent',
              color: modoConc === t.k ? t.c : '#6B7280',
              boxShadow: modoConc === t.k ? '0 1px 2px rgba(0,0,0,0.1)' : 'none',
            }}>{t.l}</button>
          ))}
        </div>
        ...
        <button onClick={sincronizarBsale} disabled={syncing}
          ...
          {syncing ? '⏳ Sincronizando…' : '🔄 Sincronizar BSALE'}
        </button>
        {ultimaSync && (
          <span style={{ fontSize: 10, color: '#9CA3AF' }}>
            {ultimaSync.docs_traidos ?? 0} docs · <span style={{ color: ultimaSync.estado === 'ok' ? '#15803D' : '#DC2626', fontWeight: 600 }}>{ultimaSync.estado}</span> · {new Date(ultimaSync.created_at).toLocaleDateString('es-CL')}
          </span>
        )}

        {/* Ampliar a la derecha */}
        <button onClick={() => setAmpliado(a => !a)} title={ampliado ? 'Restaurar' : 'Ampliar a pantalla completa'}
          style={{
            marginLeft: 'auto', display: 'inline-flex', alignItems: 'center', gap: 5,
            padding: '5px 12px', borderRadius: 6, border: '1px solid #E2E8F0',
            background: '#fff', color: '#475569', fontSize: 12, fontWeight: 600, cursor: 'pointer',
          }}>
          {ampliado ? '✕ Restaurar' : '⛶ Ampliar'}
        </button>
      </div>

      {modoConc === 'factura' ? (
        <ConciliarDesdeFacturasFlow ampliado={ampliado} />
      ) : (
        <div style={{ display: 'grid', gridTemplateColumns: '1.1fr 1fr', gap: 10, height: ampliado ? 'calc(100vh - 90px)' : 'calc(100vh - 175px)', minHeight: 540 }}>
          <MovimientosPanel
            ...
            multiSel={multiSel} onMultiSelChange={setMultiSel} onAfterClasificar={refrescar}
          />
