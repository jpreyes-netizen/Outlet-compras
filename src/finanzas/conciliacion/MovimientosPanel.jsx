...
import { Search, RefreshCw, Loader2, Sparkles, Tag, Table2 } from 'lucide-react'
...
// Chip de métrica compacto (una línea, estilo ejecutivo SAP)
function Chip({ label, value, color }) {
  return (
    <span style={{ display: 'inline-flex', alignItems: 'baseline', gap: 4, padding: '2px 8px', borderRadius: 6, background: (color || '#64748B') + '12', whiteSpace: 'nowrap' }}>
      <span style={{ fontSize: 9, fontWeight: 600, textTransform: 'uppercase', letterSpacing: '0.04em', color: '#94A3B8' }}>{label}</span>
      <span style={{ fontSize: 13, fontWeight: 700, color: color ?? '#334155' }}>{value}</span>
    </span>
  )
}
...
export function MovimientosPanel({ movimientos, loading, selectedId, onSelect, filtros, onFiltrosChange, onReload, onAutoMatch, autoRunning, multiSel = [], onMultiSelChange, onAfterClasificar }) {
  ...
  // Columnas para la Vista Excel (DataGrid): ordenar/filtrar/exportar/redimensionar
  const columnasGrid = useMemo(() => [
    { key: 'fecha', label: 'Fecha', width: 100, value: r => r.fecha, render: r => fmtFecha(r.fecha) },
    { key: 'tipo', label: 'Tipo', width: 80 },
    { key: 'monto', label: 'Monto', align: 'right', width: 120, value: r => Math.abs(r.monto), render: r => fmtCLP(r.monto), exportValue: r => r.monto },
    { key: 'descripcion', label: 'Descripción', width: 280 },
    { key: 'categoria_label', label: 'Categoría flujo', width: 160,
      value: r => r.categoria || '',
      render: r => r.categoria_label || r.categoria || '—',
      editable: {
        options: categorias.map(c => ({ value: c.categoria, label: c.label })),
        value: r => r.categoria || '',
        // Sin RUT en la glosa no se puede clasificar por RUT → deshabilitado
        disabled: r => !/^0*\d{7,8}[0-9kK]/.test(r.descripcion || ''),
        onSave: async (r, nuevaCat) => {
          const cat = categorias.find(c => c.categoria === nuevaCat)
          const tid = toast.loading(`Clasificando como ${cat?.label ?? nuevaCat}…`)
          try {
            const res = await clasificarMovimientosMasivo([r.movimiento_id], nuevaCat)
            toast.success(`${res.ruts_afectados} RUT · ${res.movimientos_cubiertos} movs`, { id: tid })
            onAfterClasificar?.()
          } catch (e) { toast.error('Error: ' + (e?.message ?? '?'), { id: tid }) }
        },
      },
    },
    { key: 'subcuenta', label: 'Cuenta contable', width: 200, value: r => r.subcuenta_codigo || '', render: r => r.subcuenta_codigo ? `${r.subcuenta_codigo} · ${r.subcuenta_nombre}` : '—', exportValue: r => r.subcuenta_codigo ? `${r.subcuenta_codigo} ${r.subcuenta_nombre}` : '' },
    { key: 'estado_conciliacion', label: 'Estado', width: 110 },
    { key: 'saldo_pendiente', label: 'Saldo pend.', align: 'right', width: 120, value: r => Math.max(0, r.saldo_pendiente), render: r => fmtCLP(Math.max(0, r.saldo_pendiente)), exportValue: r => Math.max(0, r.saldo_pendiente) },
  ], [categorias, onAfterClasificar])
      ...
      {vistaExcel && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 100, background: 'rgba(15,23,42,0.4)', padding: 16 }}>
          <div style={{ height: '100%', position: 'relative' }}>
            <button onClick={() => setVistaExcel(false)}
              style={{ position: 'absolute', top: -8, right: -8, zIndex: 101, width: 32, height: 32, borderRadius: 99, background: '#1E293B', color: '#fff', border: 'none', cursor: 'pointer', fontSize: 16, boxShadow: '0 2px 8px rgba(0,0,0,0.3)' }}>×</button>
            <DataGrid
              title="Movimientos bancarios — Vista Excel"
              exportName="movimientos_banco"
              columns={columnasGrid}
              rows={movimientos}
              getRowId={r => r.movimiento_id}
              selectedId={selectedId}
              onRowClick={m => { onSelect(m); setVistaExcel(false) }}
              loading={loading}
              emptyText="No hay movimientos que coincidan con los filtros."
            />
          </div>
        </div>
      )}

      {/* Header ejecutivo denso: título + chips de métricas + acciones en una fila */}
      <div style={{ borderBottom: '1px solid #E8EDF2', padding: '8px 12px' }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 8, flexWrap: 'wrap' }}>
          <span style={{ fontSize: 12, fontWeight: 700, color: '#1E293B', whiteSpace: 'nowrap' }}>Movimientos</span>
          {/* Chips de métricas en línea */}
          <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
            <Chip label="Total" value={totales.total} color="#475569" />
            <Chip label="Compl." value={totales.completo} color="#16A34A" />
            <Chip label="Parc." value={totales.parcial} color="#0284C7" />
            <Chip label="Sin conc." value={totales.sin} color="#D97706" />
          </div>
          {/* Avance inline */}
          <div style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 120 }}>
            <div style={{ flex: 1, height: 5, background: '#F1F5F9', borderRadius: 99, overflow: 'hidden', minWidth: 60 }}>
              <div style={{ height: '100%', width: `${totales.pct}%`, background: 'linear-gradient(to right, #34D399, #10B981)', borderRadius: 99 }} />
            </div>
            <span style={{ fontSize: 10, fontWeight: 600, color: '#334155' }}>{totales.pct}%</span>
          </div>
          <div style={{ display: 'flex', gap: 5, marginLeft: 'auto' }}>
            <button onClick={() => setVistaExcel(true)} title="Vista Excel: ordenar, filtrar, exportar"
              style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 9px', borderRadius: 7, border: '1px solid #E2E8F0', background: '#fff', fontSize: 11, fontWeight: 600, color: '#475569', cursor: 'pointer' }}>
              <Table2 size={12} /> Excel
            </button>
            <button onClick={generarSugerencias} disabled={generando || loading}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 4, padding: '4px 9px', borderRadius: 7, border: 'none', background: 'linear-gradient(to bottom, #A855F7, #7C3AED)', fontSize: 11, fontWeight: 600, color: '#fff', cursor: 'pointer', opacity: generando || loading ? 0.5 : 1 }}>
              {generando ? <Loader2 size={11} style={{ animation: 'spin 1s linear infinite' }} /> : <Sparkles size={11} />} {generando ? 'Analizando…' : 'Sugerencias'}
            </button>
            <button onClick={onReload} disabled={loading}
              style={{ width: 26, height: 26, borderRadius: 99, background: '#F1F5F9', border: 'none', cursor: 'pointer', display: 'flex', alignItems: 'center', justifyContent: 'center', color: '#475569', opacity: loading ? 0.5 : 1 }}>
