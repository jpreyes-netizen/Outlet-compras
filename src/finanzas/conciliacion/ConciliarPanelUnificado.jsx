...
import { Search, Loader2, Check, Sparkles, Link2, AlertCircle, FileText, Package, X, Trash2, Plus, ClipboardList, Undo2 } from 'lucide-react'
...
import {
  ...
  fetchAbonosAgente, registrarReembolso,
} from './api_conciliar'
...
function RendicionModal({ provision, onClose }) {
  ...
  const [reembolsando, setReembolsando] = useState(false)   // panel de reembolso abierto
  ...
  const pagado = Number(provision.monto_pagado) || 0   // ya es NETO (reembolsos restan en la vista)
  const cuadra = Math.abs(pagado - totalRendido) < 1
  ...
  async function abrirReembolso() {
    setReembolsando(true); setAboLoading(true)
    try { setAbonos(await fetchAbonosAgente(provision.agente_rut)) } catch { setAbonos([]) } finally { setAboLoading(false) }
  }
  async function aplicarReembolso(abono) {
    try {
      await registrarReembolso({ movimientoId: abono.id, provisionId: provision.id, monto: abono.monto, observaciones: `Reembolso provisión ${provision.folio_agencia ?? provision.oc_id ?? ''}`.trim() })
      toast.success(`Reembolso de ${fmtCLP(abono.monto)} aplicado`); setReembolsando(false); onClose()
    } catch (e) { toast.error('Error: ' + (e?.message ?? '?')) }
  }
        ...
        <div style={{ maxHeight: '75vh', overflowY: 'auto', padding: 16 }}>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 8, marginBottom: 8 }}>
            <div style={{ background: '#F8FAFC', borderRadius: 8, padding: 8, textAlign: 'center' }}>
              <div style={{ fontSize: 10, textTransform: 'uppercase', color: '#94A3B8' }}>Pagado neto</div>
            ...
            <div style={{ background: cuadra ? '#F0FDF4' : '#FFFBEB', borderRadius: 8, padding: 8, textAlign: 'center', border: `1px solid ${cuadra ? '#BBF7D0' : '#FDE68A'}` }}>
              <div style={{ fontSize: 10, textTransform: 'uppercase', color: '#94A3B8' }}>Saldo cta. cte.</div>
              <div style={{ fontSize: 13, fontWeight: 700, color: cuadra ? '#16A34A' : '#D97706' }}>{fmtCLP(pagado - totalRendido)}</div>
            </div>
          </div>
          <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 8, marginBottom: 12 }}>
            <span style={{ fontSize: 11, fontWeight: 600, color: cuadra ? '#15803D' : '#92400E' }}>
              {cuadra ? '✓ La provisión cuadra (pagado neto = rendido)' : `Falta rendir/reembolsar ${fmtCLP(pagado - totalRendido)}`}
            </span>
            <button onClick={abrirReembolso}
              style={{ display: 'inline-flex', alignItems: 'center', gap: 5, padding: '5px 12px', borderRadius: 7, border: '1px solid #BAE6FD', background: '#F0F9FF', color: '#0284C7', fontSize: 11, fontWeight: 600, cursor: 'pointer', whiteSpace: 'nowrap' }}>
              <Undo2 size={12} /> Registrar reembolso
            </button>
          </div>

          {reembolsando && (
            <div style={{ marginBottom: 12, border: '1px solid #BAE6FD', background: '#F8FBFF', borderRadius: 10, padding: 10 }}>
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 6 }}>
                <div style={{ fontSize: 12, fontWeight: 600, color: '#0369A1' }}>Abonos de {provision.agente_nombre} sin aplicar — elegí el reembolso</div>
                <button onClick={() => setReembolsando(false)} style={{ background: 'none', border: 'none', cursor: 'pointer', color: '#94A3B8', fontSize: 14 }}>×</button>
              </div>
              {aboLoading && <div style={{ textAlign: 'center', padding: '10px 0' }}><Loader2 size={13} style={{ display: 'inline-block', color: '#94A3B8' }} /></div>}
              {!aboLoading && abonos.length === 0 && <div style={{ fontSize: 11, color: '#94A3B8', textAlign: 'center', padding: '8px 0' }}>No hay abonos pendientes de este agente en la cartola.</div>}
              {!aboLoading && abonos.map(a => (
                <div key={a.id} onClick={() => aplicarReembolso(a)}
                  style={{ display: 'flex', justifyContent: 'space-between', gap: 10, padding: '6px 8px', borderRadius: 6, fontSize: 12, cursor: 'pointer', background: '#fff', border: '1px solid #E2E8F0', marginBottom: 4 }}>
                  <span style={{ fontFamily: 'monospace' }}>{a.fecha}</span>
                  <span style={{ overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap', flex: 1, color: '#64748B' }}>{a.descripcion}</span>
                  <span style={{ fontFamily: 'monospace', fontWeight: 600, color: '#16A34A' }}>+{fmtCLP(a.monto)}</span>
                </div>
              ))}
            </div>
          )}
