...
// ════════════════════════════════════════════════════════════════════════
function DetalleFacturas({ facturaIds, facturasMap }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 2, marginTop: 4 }}>
      {(facturaIds ?? []).map(id => {
        const f = facturasMap[id]
        if (!f) return <div key={id} style={{ fontSize: 11, color: '#94A3B8' }}>Folio ? (factura no encontrada)</div>
        return (
          <div key={id} style={{ fontSize: 11, color: '#475569' }}>
            Folio <strong>{f.folio}</strong> · {fFecha(f.fecha_emision)} · {f.razon_social}
          </div>
        )
      })}
    </div>
...
export function ConciliarCombosTab() {
  ...
  const [facturasMap, setFacturasMap] = useState({})  // id -> {folio, fecha_emision, razon_social}
  const [aceptando, setAceptando] = useState(null)

  const cargar = useCallback(async () => {
    setLoading(true)
    try {
      const { data, error } = await supabase.from('v_combos_sugeridos').select('*')
      if (error) throw error
      const lista = data ?? []
      setRows(lista)

      // Recolectar todos los factura_ids únicos involucrados y traer su detalle
      const idsSet = new Set()
      lista.forEach(r => (r.factura_ids ?? []).forEach(id => idsSet.add(id)))
      const ids = [...idsSet]
      if (ids.length) {
        const { data: facs } = await supabase.from('libro_compras')
          .select('id, folio, fecha_emision, razon_social').in('id', ids)
        const map = {}
              ...
              return (
                <div key={c.combo_id} style={{
                  ...
                  <div style={{ flex: 1 }}>
                    ...
                    <DetalleFacturas facturaIds={c.factura_ids} facturasMap={facturasMap} />
                  </div>
                    ...
                    {grupo.map(c => (
                      <div key={c.combo_id} style={{
                        ...
                        <div style={{ flex: 1, fontSize: 12, color: '#374151' }}>
                          <div>{c.n_facturas} factura{c.n_facturas > 1 ? 's' : ''} · {fmtCLP(c.monto_facturas)}
                          {c.diferencia > 0 && <span style={{ color: '#94A3B8' }}> (dif. {fmtCLP(c.diferencia)})</span>}</div>
                          <DetalleFacturas facturaIds={c.factura_ids} facturasMap={facturasMap} />
                        </div>
                        <button onClick={() => aceptar(c)} disabled={aceptando === c.combo_id} style={{
