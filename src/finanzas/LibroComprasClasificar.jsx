...
export function LibroComprasClasificar({ cu }) {
  ...
  const cargar = useCallback(async () => {
    setCargando(true)
    try {
      const [{ data: f, error }, { data: c }] = await Promise.all([
                ...
                {agrupar ? grupos.map(g => {
                  ...
                  return (
                    <>
                      ...
                      {g.facturas.map(f => <Fila key={f.id} f={f} sel={sel} setSel={setSel} clasificar={clasificar} procesando={procesando} SelCuenta={SelCuenta} indent />)}
                    </>
                  )
                }) : visibles.map(f => <Fila key={f.id} f={f} sel={sel} setSel={setSel} clasificar={clasificar} procesando={procesando} SelCuenta={SelCuenta} />)}
              </tbody>
            </table>
          </div>
        )}
...
function Fila({ f, sel, setSel, clasificar, procesando, SelCuenta, indent }) {
