...
export function LibroComprasClasificar({ cu }) {
  ...
  const colsProv = useMemo(() => [
    ...
    { key: 'saldo_abierto', label: 'Saldo abierto', width: 110, align: 'right', value: p => Number(p.saldo_abierto) || 0,
      render: p => Number(p.saldo_abierto) > 1 ? <span style={MONO}>{fmt(p.saldo_abierto)}</span> : '', exportValue: p => Number(p.saldo_abierto) || 0 },
  ...
  async function abrirDte(f) {
    ...
    try {
      ...
      if (error) throw error
      if (data?.url_pdf) { win.location = data.url_pdf }
      else { win.close(); toast.info(`${TIPO_DOC(f.codigo_sii)} ${f.folio}: sin documento digital (no vino de BSALE)`) }
    } catch (e) { win.close(); toast.error(e.message) }
  }
  ...
  const colsFact = useMemo(() => [
    ...
    { key: 'monto_total', label: 'Total', width: 100, align: 'right', value: f => Number(f.monto_total) || 0,
      render: f => <span style={{ ...MONO, fontWeight: 600 }}>{fmt(f.monto_total)}</span>, exportValue: f => Number(f.monto_total) || 0 },
    ...
    { key: 'ceco_nombre', label: 'Centro de costo', width: 130, value: f => f.ceco_nombre ?? (cecoNom.get(String(f.ceco_id ?? '')) ?? ''),
      render: f => <span style={{ fontSize: 11, color: SLATE }}>{f.ceco_nombre ?? cecoNom.get(String(f.ceco_id ?? '')) ?? '—'}</span> },
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', flexWrap: 'wrap' }}>
        ...
        <div style={{ flex: 1, minWidth: 280, background: '#F0F4FF', border: '1px solid #C7D2FE', borderRadius: 8, padding: '7px 12px', fontSize: 11.5, color: '#1E3A8A', lineHeight: 1.5 }}>
          {esProv
            ...
            : <><b>Excepciones por factura.</b> Cambiar la cuenta aquí afecta solo a esa factura; la regla del proveedor se administra en el nivel Proveedores. Vencimiento = emisión + plazo del proveedor.</>}
        </div>
      </div>
      ...
      {esProv ? (
        <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
          ...
          <Kpi label="Saldo abierto" valor={fmt(kpiP.abierto)} detalle="facturas por pagar o parciales" />
