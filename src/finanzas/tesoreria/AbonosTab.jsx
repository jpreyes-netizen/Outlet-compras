...
import {
  ...
  fetchFrescuraLedger, buscarClienteBsale, fetchCuentaClientes,
} from './api'
...
export function AbonosTab({ usuario }) {
  ...
  const colsCuenta = useMemo(() => [
    ...
    {
      key: 'saldo_a_favor', label: 'Saldo a favor', align: 'right', width: 115,
      render: r => (
        <strong style={{ color: Number(r.saldo_a_favor) > 1000 ? C.verde : Number(r.saldo_a_favor) < -1000 ? C.rojo : C.gris }}>
          {fmt(r.saldo_a_favor)}
        </strong>
      ),
    },
    {
      key: 'estado', label: 'Estado', width: 190,
      render: r => {
        const cfg = r.estado === 'saldo a favor del cliente' ? { c: C.verde, b: C.verdeBg }
          : r.estado === 'imputado de más · revisar' ? { c: C.rojo, b: C.rojoBg }
          : { c: C.gris, b: C.grisBg }
        return <Chip texto={r.estado} color={cfg.c} bg={cfg.b} />
      },
    },
    {
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {seccion === 'clientes' && (
        <>
          <div style={{ ...cardSt, padding: '12px 16px', display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'center' }}>
            <div style={{ fontSize: 12, color: C.texto, flex: 1, minWidth: 260, lineHeight: 1.5 }}>
              <strong>Cuenta corriente de cada cliente.</strong> Lo que entró, lo que se imputó y
              lo que queda a favor. Un saldo negativo significa que se imputó más crédito del que
              entró: revisar. La columna <em>Sin mover</em> señala a quién contactar para que
              retire o para emitirle una nota de crédito.
            </div>
            <div style={{ textAlign: 'right' }}>
              <div style={{ fontSize: 20, fontWeight: 700, color: C.verde }}>
                {fmt(cuentas.filter(c => Number(c.saldo_a_favor) > 1000).reduce((a, c) => a + Number(c.saldo_a_favor), 0))}
              </div>
              <div style={{ fontSize: 10, color: C.gris }}>
                a favor en {cuentas.filter(c => Number(c.saldo_a_favor) > 1000).length} clientes
              </div>
              {cuentas.some(c => Number(c.saldo_a_favor) < -1000) && (
                <div style={{ fontSize: 10, color: C.rojo, marginTop: 3, fontWeight: 600 }}>
                  {cuentas.filter(c => Number(c.saldo_a_favor) < -1000).length} con saldo negativo
                </div>
              )}
            </div>
          </div>
