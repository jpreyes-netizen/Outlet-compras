...
import {
  ...
  fetchPuenteVenta, fetchConsistenciaPaneles,
} from './api'
...
export function AbonosTab({ usuario }) {
  ...
  const cargar = useCallback(async () => {
    setCargando(true)
    try {
      // allSettled en vez de catch(() => []): un fetch caído (timeout/RLS) debe
      // avisarse, no mostrarse como $0 silencioso — eso es un dato falso.
      const fuentes = [
        ['panel de control', fetchControlConciliacion(anio, sucursalSel), setControl],
        ['flujo por sucursal', fetchAbonosPasivo(), setPasivo],
        ['movimientos', fetchAbonosTrazabilidad({ anio, mes, sucursal_id: sucursalSel, movimiento: filtroMov || null }), setMovs],
        ['cola por conciliar', fetchAbonosPorConciliar({ sucursal_id: sucursalSel }), setCola],
        ['notas de crédito', fetchNcControl({ anio, mes, sucursal_id: sucursalSel, efecto: filtroEfecto || null }), setNcs],
        ['cuentas por cliente', fetchCuentaClientes({}), setCuentas],
        ['puente de venta', fetchPuenteVenta(anio, sucursalSel), setPuente],
        ['consistencia entre paneles', (() => {
          if (mes) {
            const mm = String(mes).padStart(2, '0')
            const fin = String(new Date(anio, mes, 0).getDate()).padStart(2, '0')
            return fetchConsistenciaPaneles(`${anio}-${mm}-01`, `${anio}-${mm}-${fin}`)
          }
          return fetchConsistenciaPaneles(`${anio}-01-01`, `${anio}-12-31`)
        })(), setConsistencia],
      ]
      const res = await Promise.allSettled(fuentes.map(f => f[1]))
      const caidas = []
      res.forEach((r, i) => {
        if (r.status === 'fulfilled') fuentes[i][2](r.value ?? [])
        else { fuentes[i][2]([]); caidas.push(fuentes[i][0]); console.error('AbonosTab —', fuentes[i][0], r.reason) }
      })
      if (caidas.length) {
        toast.error(`No se pudo cargar: ${caidas.join(', ')}. Los $0 de esas secciones NO son reales.`, { duration: 8000 })
      }
  ...
  // Venta bruta − NC = neta, misma fuente y misma cifra que el tab Cuadratura
  const puenteMes = useMemo(() => {
    const filas = mes ? puente.filter(p => new Date(p.mes + 'T00:00:00').getMonth() + 1 === mes) : puente
    const s = k => filas.reduce((a, r) => a + Number(r[k] ?? 0), 0)
    return { venta_bruta: s('venta_bruta'), nc_rebaja: s('nc_rebaja'), venta_neta: s('venta_neta'), hay: filas.length > 0 }
  }, [puente, mes])
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {frescura.length > 0 && (
        <div style={{ fontSize: 10, color: C.gris, display: 'flex', gap: 14, flexWrap: 'wrap', marginTop: -8 }}>
          ...
          {control[0]?.refrescado_en && (() => {
            const h = (Date.now() - new Date(control[0].refrescado_en).getTime()) / 3600000
            return (
              <span style={{ color: h > 1.5 ? C.naranja : C.gris }}>
                panel: calculado {h < 1 ? 'hace minutos' : h < 24 ? `hace ${Math.round(h)} h` : `hace ${Math.round(h / 24)} d`}
              </span>
            )
          })()}
        </div>
      ...
      {seccion === 'resumen' && (
        <>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10 }}>
            {puenteMes.hay ? (
              <Kpi label="Venta neta del período" valor={fmt(puenteMes.venta_neta)}
                sub={`bruta ${fmtC(puenteMes.venta_bruta)} − NC ${fmtC(puenteMes.nc_rebaja)} · igual que Cuadratura`} />
            ) : (
              <Kpi label="Venta bruta del período" valor={fmt(resumen.venta)}
                sub="sin NC descontadas (puente no disponible)" />
            )}
            <Kpi label="Recaudación" valor={fmt(resumen.recaudacion)} color={C.verde}
              sub={`Desvío ${pct1(resumen.desvio, resumen.venta)}% vs venta bruta`} />
          ...
          {/* Consistencia con el tab Cuadratura y análisis */}
          {consistencia.length > 0 && (() => {
            const malos = consistencia.filter(c => !c.ok)
            return (
              <div style={{ ...cardSt, padding: '14px 16px', border: malos.length ? `1px solid #FECACA` : undefined }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 10, flexWrap: 'wrap', gap: 6 }}>
                  <span style={{ fontSize: 11, fontWeight: 600, color: C.gris, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
                    Consistencia con Cuadratura y análisis
                  </span>
                  <span style={{ fontSize: 11, fontWeight: 700, color: malos.length ? C.rojo : C.verde }}>
                    {malos.length ? `⚠ ${malos.length} concepto(s) no calzan` : '✓ Ambos paneles dicen lo mismo'}
                  </span>
                </div>
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 12 }}>
                  <thead><tr style={{ background: '#F9FAFB' }}>
                    {['Concepto', 'Este panel', 'Cuadratura', 'Diferencia'].map((h, i) => (
                      <th key={h} style={{ padding: '6px 10px', fontSize: 10, fontWeight: 600, color: C.gris, textAlign: i === 0 ? 'left' : 'right', borderBottom: '1px solid #E5E7EB' }}>{h}</th>
                    ))}
                  </tr></thead>
                  <tbody>
                    {consistencia.map(c => (
                      <tr key={c.concepto} style={{ borderBottom: '1px solid #F1F5F9', background: c.ok ? 'transparent' : C.rojoBg }}>
                        <td style={{ padding: '6px 10px' }}>{c.ok ? '✓' : '✗'} {c.concepto}</td>
                        <td style={{ padding: '6px 10px', textAlign: 'right', fontFamily: 'ui-monospace, monospace' }}>{fmt(c.panel_abonos)}</td>
                        <td style={{ padding: '6px 10px', textAlign: 'right', fontFamily: 'ui-monospace, monospace' }}>{fmt(c.panel_cuadratura)}</td>
                        <td style={{ padding: '6px 10px', textAlign: 'right', fontWeight: 700, color: c.ok ? C.verde : C.rojo }}>
                ...
                <div style={{ fontSize: 10, color: C.gris, marginTop: 8, lineHeight: 1.45 }}>
                  Compara las definiciones de este panel (medios 7/3, return_id) contra las de Cuadratura
                  (naturaleza del medio de pago) sobre los mismos pagos, a nivel empresa (sin filtro de sucursal).
                  Si una fila no calza, probablemente BSALE agregó o cambió un medio de pago: avisa a JP antes de
                  confiar en cualquiera de los dos paneles.
                </div>
              </div>
            )
          })()}
