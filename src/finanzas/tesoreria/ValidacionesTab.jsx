...
const MOTIVOS_RECHAZO = [
  ...
  'Pago duplicado: el mismo abono ya fue usado en otra venta',
  'Otro',
]
...
export function ValidacionesTab({ usuario, cu }) {
  ...
  /* Deep link ?val=VAL-000123: la notificacion apunta a UNA solicitud
     especifica; al llegar los datos se abre su detalle de inmediato. */
  const valPendienteRef = useRef(
    deepLink?.app === 'finanzas' && deepLink?.modulo === 'tesoreria' ? (deepLink.params?.val || null) : null
  )
  useEffect(() => {
    if (!valPendienteRef.current || rows.length === 0) return
    const objetivo = rows.find(x => x.id === valPendienteRef.current)
    if (objetivo) { setSel(objetivo); valPendienteRef.current = null }
  }, [rows])
  ...
  const abiertas = useMemo(
    () => rows.filter(r => r.estado === 'pendiente' || r.estado === 'en_revision')
              // Las alertas de duplicado suben al tope: son las que no pueden esperar
              .sort((a, b) => (b.alerta_duplicado ? 1 : 0) - (a.alerta_duplicado ? 1 : 0)
  ...
  const kpis = useMemo(() => {
    ...
    const prom = res.length
      ...
      : 0
    return {
      alertas: abiertas.filter(r => r.alerta_duplicado).length,
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center', marginBottom: 12 }}>
        ...
        {kpis.alertas > 0 && <Kpi l="⚠ Duplicados" v={kpis.alertas} c="#DC2626" destacar />}
                ...
                {historico.map(r => {
                  ...
                  return (
                    <tr key={r.id} style={{ cursor: 'pointer' }} onClick={() => setSel(r)}>
                      ...
                      <td style={{ ...td, textAlign: 'center' }}>{r.alerta_duplicado ? '⚠️' : ''}</td>
...
function Tarjeta({ r, onClick }) {
  ...
  return (
    <div onClick={onClick} style={{
      ...
      background: r.alerta_duplicado ? '#FFF7F7' : '#fff',
      border: '1px solid ' + (r.alerta_duplicado ? '#FCA5A5' : e.bd),
      borderLeft: '4px solid ' + (r.alerta_duplicado ? '#DC2626' : e.c),
      borderRadius: 10, cursor: 'pointer', boxShadow: '0 1px 2px rgba(0,0,0,.04)',
    }}>
      <div style={{ fontSize: 20 }}>{r.alerta_duplicado ? '\u26A0\uFE0F' : m.ic}</div>
      ...
      {r.alerta_duplicado && (
        <span style={{ ...pill, color: '#fff', background: '#DC2626', border: '1px solid #DC2626' }}>
          DUPLICADO
        </span>
      )}
...
/* Bloque de alerta de comprobante reutilizado.
   Carga las coincidencias con fn_val_duplicados, que recalcula en vivo en vez
   de confiar en el arreglo congelado del registro. */
function AlertaDuplicado({ r }) {
  ...
  useEffect(() => {
    let cancel = false
    setCargando(true)
    supabase.rpc('fn_val_duplicados', { p_id: r.id })
      .then(({ data }) => { if (!cancel) setDups(data || []) })
      .catch(() => {})
      .finally(() => { if (!cancel) setCargando(false) })
    return () => { cancel = true }
  }, [r.id])

  // Nada que mostrar solo si la busqueda EN VIVO no encontro coincidencias.
  // No se confia en r.alerta_duplicado: la marca puede faltar (carrera de
  // inserciones simultaneas) o estar obsoleta (la gemela ya fue rechazada).
  if (!cargando && dups.length === 0) return null
  if (cargando && !r.alerta_duplicado) return null   // sin marca, no parpadear mientras carga

  const factores = r.medio === 'webpay_link' ? 'fecha y monto' : 'fecha, monto y banco de origen'

  return (
    <div style={{
      border: '2px solid #DC2626', background: '#FFF1F2', borderRadius: 10,
      padding: 14, marginBottom: 16,
    }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
        <span style={{ fontSize: 20 }}>{'\u26A0\uFE0F'}</span>
        <div style={{ fontSize: 13.5, fontWeight: 800, color: '#991B1B' }}>
          Posible comprobante reutilizado
        </div>
      </div>
      <div style={{ fontSize: 12, color: '#7F1D1D', lineHeight: 1.5, marginBottom: 10 }}>
        Coincide en <b>{factores}</b> con {dups.length === 1 ? 'otra solicitud' : 'otras solicitudes'}.
        Puede ser el mismo abono presentado dos veces, o dos pagos reales del mismo monto.
        Confirma en el banco que existan <b>dos abonos distintos</b> antes de aprobar.
      </div>

      {cargando ? (
        <div style={{ fontSize: 11.5, color: '#9CA3AF' }}>Buscando coincidencias…</div>
      ) : (
        <div style={{ display: 'grid', gap: 6 }}>
          {dups.map(d => {
            const e2 = EST[d.estado] || EST.pendiente
            return (
              <div key={d.id} style={{
                ...
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
                  <span style={{ fontFamily: 'monospace', fontWeight: 700 }}>{d.id}</span>
                  <span style={{ ...pill, color: e2.c, background: e2.bg, border: '1px solid ' + e2.bd }}>{e2.l}</span>
                  <span style={{ color: '#6B7280' }}>{(d.sucursal_id || '').replace('suc-', '')}</span>
                </div>
                <div style={{ color: '#4B5563', marginTop: 3 }}>
                  {fmt(d.monto)} · {d.cliente_nombre || 'sin cliente'}
                  {d.cliente_rut ? ' (' + d.cliente_rut + ')' : ''}
                  {d.banco_origen ? ' · ' + d.banco_origen : ''}
                  {d.doc_referencia ? ' · ' + d.doc_referencia : ''}
                  {d.solicitante_nombre ? ' · pidió ' + d.solicitante_nombre : ''}
                </div>
              </div>
            )
          })}
