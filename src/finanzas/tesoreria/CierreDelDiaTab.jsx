...
export function CierreDelDiaTab({ usuario }) {
  ...
  async function cargarTransferencias(fechaP, sucursalId) {
    ...
    try {
      ...
      console.warn('[cargarTransferencias]', e.message)
    } finally { setLoadingTransf(false) }
  }

  async function conciliarTransferencia(transf) {
    setConciliandoId(transf.id)
    try {
      const desde = new Date(transf.fecha)
      desde.setDate(desde.getDate() - 2)
      const hasta = new Date(transf.fecha)
      hasta.setDate(hasta.getDate() + 2)
      const desdeStr = desde.toISOString().split('T')[0]
      const hastaStr = hasta.toISOString().split('T')[0]

      // IDs ya conciliados para excluir
      const { data: yaConc } = await supabase
        .from('cartola_conciliacion')
        .select('cartola_id')
        .eq('tabla_origen', 'abonos_transferencia')
      const idsExcluir = (yaConc ?? []).map(r => r.cartola_id)

      let q = supabase
        .from('cartola_bancaria')
        .select('id, fecha, monto, descripcion')
        .eq('monto', transf.monto)
        .eq('cargo_abono', 'abono')
        .gte('fecha', desdeStr)
        .lte('fecha', hastaStr)
      if (idsExcluir.length > 0) q = q.not('id', 'in', `(${idsExcluir.map(id => `"${id}"`).join(',')})`)

      const { data: matches, error } = await q
      if (error) throw error

      if (!matches || matches.length === 0) {
        toast.error(`Sin match en cartola para ${fmt(transf.monto)}`)
        return
      }

      const match = matches.sort((a, b) =>
        Math.abs(new Date(a.fecha) - new Date(transf.fecha)) -
        Math.abs(new Date(b.fecha) - new Date(transf.fecha))
      )[0]
      ...
      const { error: errConc } = await supabase
        ...
        .insert({
          cartola_id: match.id,
          tabla_origen: 'abonos_transferencia',
          registro_id: transf.id,
          monto_aplicado: transf.monto,
          metodo_match: 'automatico_monto_fecha',
          score: 90,
          descripcion_libre: `${transf.tipo_documento} N°${transf.bsale_doc_numero} — ${transf.vendedor_nombre}`,
      ...
      setTransferencias(prev => prev.map(t => t.id === transf.id ? { ...t, estado: 'conciliado' } : t))
      toast.success(`Conciliado con movimiento del ${match.fecha}`)
    } catch (e) {
      toast.error(e instanceof Error ? e.message : 'Error al conciliar')
    } finally { setConciliandoId(null) }
  }
  ...
  const cargarCierres = useCallback(async () => {
    ...
    try {
      ...
      // Excluir Página Web (bsale_vendedor_id=33) de sucursales físicas
      if (sucursalSel !== 'suc-web') q = q.neq('bsale_vendedor_id', 33)
  ...
  const filas = useMemo(() => {
    const recaudadoresBsale = (bsaleData?.por_recaudador ?? [])
      .filter(bv => sucursalSel === 'suc-web' || String(bv.bsale_user_id) !== '33')
    const result = []
  ...
  function abrirPanel(fila) {
    ...
    if (fila.cierre) {
      ...
      if (Number(fila.cierre.transferencia ?? 0) > 0) {
        cargarTransferencias(fecha, sucursalSel)
      }
    } else {
            ...
            <div style={{ flex: 1, padding: '16px 20px', overflowY: 'auto' }}>
              ...
              {esAdmin && panelVendedor.cierre && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  {/* Transferencias BSALE */}
                  {Number(panelVendedor.cierre?.transferencia ?? 0) > 0 && (
                    <div>
                      <div style={{ fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 8, display: 'flex', alignItems: 'center', gap: 6 }}>
                        Transferencias BSALE
                        <span style={{ background: '#DBEAFE', color: '#1D4ED8', fontSize: 10, padding: '1px 6px', borderRadius: 4 }}>
                          {fmt(Number(panelVendedor.cierre.transferencia))}
                        </span>
                      </div>
                      {loadingTransf
                        ? <div style={{ textAlign: 'center', padding: 12 }}><Loader2 size={16} style={{ color: '#9CA3AF' }} /></div>
                        : transferencias.length === 0
                          ? <div style={{ fontSize: 12, color: '#9CA3AF', padding: '8px 0' }}>Sin transferencias registradas para esta fecha</div>
                          : (
                            <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
                              {transferencias.map(t => (
                                <div key={t.id} style={{
                                  background: t.estado === 'conciliado' ? '#F0FDF4' : '#FFFBEB',
                                  border: `1px solid ${t.estado === 'conciliado' ? '#BBF7D0' : '#FDE68A'}`,
                                  borderRadius: 8, padding: '8px 10px',
                                  display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8
                                }}>
                                  <div style={{ minWidth: 0 }}>
                                    <div style={{ fontSize: 11, fontWeight: 600, color: '#111827' }}>
                                      {t.tipo_documento} N°{t.bsale_doc_numero}
                                    </div>
                                    <div style={{ fontSize: 10, color: '#6B7280', marginTop: 1 }}>{t.vendedor_nombre}</div>
                                  </div>
                                  <div style={{ display: 'flex', alignItems: 'center', gap: 6, flexShrink: 0 }}>
                                    <span style={{ fontSize: 12, fontWeight: 700 }}>{fmt(t.monto)}</span>
                                    {t.estado === 'conciliado'
                                      ? <span style={{ fontSize: 10, color: '#16A34A', background: '#DCFCE7', padding: '2px 6px', borderRadius: 4 }}>✓ Conciliado</span>
                                      : (
                                        <button
                                          onClick={() => conciliarTransferencia(t)}
                                          disabled={conciliandoId === t.id}
                                          style={{ fontSize: 10, color: '#1D4ED8', background: '#DBEAFE', border: 'none', padding: '3px 8px', borderRadius: 4, cursor: 'pointer', display: 'flex', alignItems: 'center', gap: 3 }}>
                                          {conciliandoId === t.id ? <Loader2 size={10} /> : null}
                                          Conciliar
                                        </button>
                                      )
                                    }
