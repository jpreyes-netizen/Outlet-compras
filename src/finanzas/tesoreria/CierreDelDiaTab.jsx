...
import {
  ...
  fetchPuenteBsale, fetchFrescuraLedger, fetchHistorialCierre,
} from './api'
...
// ── Huella del cierre ─────────────────────────────────────────────────────
// Quién digitó, quién corroboró y quién editó, con fecha y hora. Los sellos los
// graba la base (trigger tr_cierres_huella) según el usuario conectado: el navegador
// no puede escribir el nombre de otra persona. El historial (tr_cierres_historial)
// guarda cada dato cambiado con su valor anterior y nuevo, y no se puede editar.
const ETIQUETA_CAMPO = {
  efectivo: 'Efectivo', t_credito: 'Crédito', t_debito: 'Débito', webpay: 'Webpay', transferencia: 'Transferencias',
  m_pago: 'Mercado Pago', abono_cliente: 'Abono cliente', canje: 'Canje', p_clay: 'Puntos Clay', cheque: 'Cheque',
  retiros_efectivo: 'Retiros de efectivo', estado: 'Estado', observaciones_vendedor: 'Observación del cajero',
  observaciones_admin: 'Observación de tesorería', venta_bsale_api: 'Venta BSALE', caja_esperada_api: 'Caja esperada',
  base_comparacion: 'Base de comparación', vendedor_id: 'Cajero', bsale_vendedor_id: 'Cajero BSALE',
}
const etiquetaCampo = c => c?.endsWith('_corrob')
  ? `${ETIQUETA_CAMPO[c.replace(/_corrob$/, '')] ?? c} (corroborado)` : (ETIQUETA_CAMPO[c] ?? c)
const ACCION_HIST = {
  declarar:   { l: 'Declaró',   c: '#1D4ED8' }, corroborar: { l: 'Corroboró', c: '#047857' },
  modificar:  { l: 'Modificó',  c: '#B45309' }, anular:     { l: 'Anuló',     c: '#B91C1C' },
  eliminar:   { l: 'Eliminó',   c: '#B91C1C' }, sistema:    { l: 'Sistema',   c: '#6B7280' },
}
const fmtFechaHora = iso => iso ? new Date(iso).toLocaleString('es-CL', { day: '2-digit', month: '2-digit', hour: '2-digit', minute: '2-digit' }) : '—'
const CAMPOS_MONTO = ['efectivo','t_credito','t_debito','webpay','transferencia','m_pago','abono_cliente','canje','p_clay','cheque',
  'retiros_efectivo','venta_bsale_api','caja_esperada_api']
const esMonto = c => !!c && (c.endsWith('_corrob') || CAMPOS_MONTO.includes(c))
const valorLegible = (campo, v) => v == null || v === '' ? '—' : (esMonto(campo) && !isNaN(Number(v)) ? fmt(Number(v)) : String(v))

function HuellaCierre({ cierre }) {
  const [abierto, setAbierto] = useState(false)
  const [hist, setHist] = useState(null)
  useEffect(() => { setHist(null) }, [cierre?.id])
  useEffect(() => {
    if (!abierto || hist || !cierre?.id) return
    fetchHistorialCierre(cierre.id).then(setHist).catch(() => setHist([]))
  }, [abierto, cierre?.id, hist])
  if (!cierre) return null

  const mismaPersona = cierre.declarado_por && cierre.admin_id && cierre.declarado_por === cierre.admin_id
    && cierre.estado !== 'declarado' && cierre.estado !== 'anulado'
  const fila = (etiqueta, nombre, fechaIso, color) => (
    <div style={{ display: 'flex', justifyContent: 'space-between', gap: 10, fontSize: 11, padding: '3px 0' }}>
      ...
      <span style={{ textAlign: 'right' }}>
        <strong style={{ color: color ?? '#111827' }}>{nombre ?? '—'}</strong>
        {fechaIso && <span style={{ color: '#9CA3AF', marginLeft: 6 }}>{fmtFechaHora(fechaIso)}</span>}
      </span>
    </div>
  )
  return (
    <div style={{ border: '1px solid #E5E7EB', borderRadius: 8, padding: '9px 12px', background: '#FAFAFA' }}>
      <div style={{ fontSize: 10, fontWeight: 700, color: '#6B7280', textTransform: 'uppercase', letterSpacing: '0.05em', marginBottom: 4 }}>
        Huella del cierre
      </div>
      {cierre.vendedor_nombre && fila('Caja de', cierre.vendedor_nombre, null)}
      {fila('Declarado por', cierre.firmado_por_nombre, cierre.declarado_at, '#1D4ED8')}
      {cierre.estado !== 'declarado' && cierre.estado !== 'anulado' &&
        fila('Corroborado por', cierre.corroborado_por_nombre, cierre.corroborado_at, '#047857')}
      {cierre.editado_at && fila('Última edición', cierre.editado_por_nombre, cierre.editado_at, '#B45309')}
      {mismaPersona && (
        <div style={{ marginTop: 6, fontSize: 10.5, color: '#92400E', background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 6, padding: '5px 8px' }}>
          La misma persona declaró y corroboró este cierre: no hubo revisión de un segundo responsable.
        </div>
      )}
      <button type="button" onClick={() => setAbierto(a => !a)}
        style={{ marginTop: 6, background: 'none', border: 'none', padding: 0, cursor: 'pointer', fontSize: 11, color: '#1F4E79', fontWeight: 600 }}>
        {abierto ? '▾ Ocultar historial de cambios' : '▸ Ver historial de cambios'}
      </button>
      {abierto && (
        <div style={{ marginTop: 6, maxHeight: 260, overflowY: 'auto', borderTop: '1px solid #E5E7EB', paddingTop: 6 }}>
          {hist == null && <div style={{ fontSize: 11, color: '#9CA3AF' }}>Cargando…</div>}
          {hist?.length === 0 && (
            <div style={{ fontSize: 11, color: '#9CA3AF' }}>
              Sin cambios registrados. El historial detallado rige desde el 23-09-2026; los cierres anteriores muestran solo los sellos de arriba.
            </div>
          )}
          {hist?.map(h => {
            const a = ACCION_HIST[h.accion] ?? { l: h.accion, c: '#374151' }
            return (
              <div key={h.id} style={{ fontSize: 11, padding: '4px 0', borderBottom: '1px dashed #F1F5F9' }}>
                <div style={{ display: 'flex', justifyContent: 'space-between', gap: 8 }}>
                  <span><strong style={{ color: a.c }}>{a.l}</strong> · {h.actor_nombre}</span>
                  <span style={{ color: '#9CA3AF', whiteSpace: 'nowrap' }}>{fmtFechaHora(h.registrado_at)}</span>
                </div>
                {h.campo && (
                  <div style={{ color: '#374151', marginTop: 1 }}>
                    {etiquetaCampo(h.campo)}: <span style={{ color: '#9CA3AF', textDecoration: 'line-through' }}>{valorLegible(h.campo, h.valor_anterior)}</span>
                    {' → '}<strong>{valorLegible(h.campo, h.valor_nuevo)}</strong>
                  </div>
                )}
                {!h.campo && h.accion === 'declarar' && <div style={{ color: '#6B7280', marginTop: 1 }}>Registro inicial del cierre</div>}
              </div>
            )
          })}
        </div>
            ...
            <div style={{ flex: 1, padding: '16px 20px', overflowY: 'auto', minHeight: 0 }}>
              ...
              {(esAdmin || puedeCorroborar) && panelVendedor.cierre && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  {/* Huella: quién declaró, quién corroboró, quién editó */}
                  <HuellaCierre cierre={panelVendedor.cierre} />
