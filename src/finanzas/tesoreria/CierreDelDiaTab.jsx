...
function PanelDeclaracion({ vendedorBsale, cierre, sucursalId, fecha, usuario, umbrales, onGuardado }) {
  ...
  async function guardar() {
    ...
    try {
      ...
      const payload = {
        ...
        vendedor_id: vendedorUsuario.id,                  // ← dueño de la venta
        bsale_vendedor_id: vendedorBsale.bsale_user_id,   // ← clave de cruce con BSALE
        firmado_por_usuario_id: usuario.id,               // ← auditoría: quien firmó
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {vendedorBsale?.cruces && Object.keys(vendedorBsale.cruces).length > 0 && (
        <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8, padding: '10px 12px' }}>
          ...
          {Object.entries(vendedorBsale.cruces).map(([id, info]) => (
            <div key={id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#78350F', padding: '2px 0' }}>
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  {panelVendedor.bsaleUser && (
                    <>
                      ...
                      {panelVendedor.bsaleUser.cruces && Object.keys(panelVendedor.bsaleUser.cruces).length > 0 && (
                        <div>
                          <div style={{ fontSize: 12, fontWeight: 600, color: '#374151', marginBottom: 6, display: 'flex', alignItems: 'center', gap: 6 }}>
                            ...
                            <span title="Ventas asignadas a otro vendedor, pero esta persona las cobró físicamente"
                              ...
                              ↔ {Object.keys(panelVendedor.bsaleUser.cruces).length}
                            </span>
                          </div>
                          <div style={{ background: '#FFFBEB', borderRadius: 8, padding: '8px 12px', border: '1px solid #FDE68A' }}>
                            {Object.entries(panelVendedor.bsaleUser.cruces).map(([id, info]) => (
                              <div key={id} style={{ display: 'flex', justifyContent: 'space-between', padding: '3px 0', fontSize: 12 }}>
