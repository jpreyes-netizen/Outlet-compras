...
function PanelDeclaracion({ vendedorBsale, cierre, sucursalId, fecha, usuario, umbrales, onGuardado }) {
  ...
  const recaudBsale = vendedorBsale?.recaudacion ?? null
      ...
      {vendedorBsale && (
        <div style={{ background: 'linear-gradient(135deg, #1e3a5f 0%, #1a2f4a 100%)', borderRadius: 10, padding: '14px 16px', color: '#fff' }}>
          <div style={{ fontSize: 11, opacity: 0.7, marginBottom: 8, textTransform: 'uppercase', letterSpacing: '0.05em' }}>
            Recaudación BSALE hoy
          </div>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end' }}>
            <div>
              <div style={{ fontSize: 24, fontWeight: 700 }}>{fmt(vendedorBsale.recaudacion)}</div>
              <div style={{ fontSize: 11, opacity: 0.6, marginTop: 2 }}>
                Venta: {fmt(vendedorBsale.venta)} · {vendedorBsale.docs_venta} docs
                {vendedorBsale.nc > 0 && ` · NC: -${fmt(vendedorBsale.nc)}`}
              </div>
            </div>
            {/* Medios de pago BSALE */}
            <div style={{ display: 'flex', flexDirection: 'column', gap: 2, alignItems: 'flex-end' }}>
              {Object.entries(vendedorBsale.medios ?? {})
                .sort(([, a], [, b]) => (b as number) - (a as number))
                .slice(0, 4)
                .map(([medio, amt]) => (
                  <div key={medio} style={{ fontSize: 10, opacity: 0.8 }}>
                    {medio.split(' ').slice(0, 2).join(' ')}: {fmt(amt as number)}
                  </div>
                ))}
            </div>
          </div>
...
export function CierreDelDiaTab({ usuario }) {
  ...
  // Cruzar vendedores BSALE con cierres declarados
  const filas = useMemo(() => {
    const vendedoresBsale = bsaleData?.por_vendedor ?? []
    const result = []

    // Vendedores con actividad BSALE hoy
    for (const bv of vendedoresBsale) {
      const cierre = cierres.find(c => {
        // Si el vendedor tiene bsale_user_id mapeado en usuarios → match directo
        // Sino → intentar match por nombre
    ...
    // Cierres sin actividad BSALE (vendedor declaró pero no hay docs en BSALE ese día)
  ...
  // KPIs resumen
  const totalBsale = bsaleData?.total_recaudacion ?? null
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(160px, 1fr))', gap: 10 }}>
        {[
          { label: 'Recaudación BSALE', value: totalBsale, loading: loadingBsale, color: '#1e3a5f' },
            ...
            {Object.entries(bsaleData.medios_global)
              ...
              .map(([medio, amt]) => (
                <div key={medio} style={{ background: '#F3F4F6', borderRadius: 6, padding: '4px 10px', fontSize: 12 }}>
                  ...
                  <span style={{ fontWeight: 600, color: '#111827' }}>{fmt(amt as number)}</span>
                </div>
              ))}
          </div>
        </div>
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#F9FAFB' }}>
                {['Vendedor', 'Recaudación BSALE', 'Venta BSALE', 'Declarado', 'Brecha', 'Estado', ''].map(h => (
                  <th key={h} style={{
                    padding: '10px 14px', fontSize: 11, fontWeight: 600, color: '#6B7280',
                    textAlign: ['Recaudación BSALE', 'Venta BSALE', 'Declarado', 'Brecha'].includes(h) ? 'right' : 'left',
            ...
            <tbody>
              ...
              {!loadingBsale && !loadingCierres && filas.map((fila, i) => {
                ...
                const recaud = bsaleUser?.recaudacion ?? null
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  {panelVendedor.bsaleUser && (
                    <div style={{ background: 'linear-gradient(135deg, #1e3a5f 0%, #1a2f4a 100%)', borderRadius: 10, padding: '12px 14px', color: '#fff' }}>
                      <div style={{ fontSize: 10, opacity: 0.7, marginBottom: 6, textTransform: 'uppercase', letterSpacing: '0.05em' }}>Recaudación BSALE</div>
                      <div style={{ fontSize: 20, fontWeight: 700 }}>{fmt(panelVendedor.bsaleUser.recaudacion)}</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6, marginTop: 8 }}>
                        {Object.entries(panelVendedor.bsaleUser.medios ?? {}).map(([medio, amt]) => (
