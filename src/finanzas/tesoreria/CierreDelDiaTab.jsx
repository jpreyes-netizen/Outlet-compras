...
export function CierreDelDiaTab({ usuario }) {
  ...
  console.log('[CierreDelDia] usuario:', usuario, 'esAdmin:', esAdmin, 'PUEDE_VER_TODAS:', PUEDE_VER_TODAS)
  ...
  function abrirPanel(fila) {
    ...
    console.log('[abrirPanel] fila:', fila)
    console.log('[abrirPanel] cierre:', fila.cierre)
    console.log('[abrirPanel] cierre.estado:', fila.cierre?.estado)
    if (fila.cierre) {
      const v = {}
      for (const med of MEDIOS) v[`${med.key}_corrob`] = Number(fila.cierre[`${med.key}_corrob`] ?? fila.cierre[med.key] ?? 0)
      setValoresCorrob(v)
      console.log('[abrirPanel] valoresCorrob seteado:', v)
