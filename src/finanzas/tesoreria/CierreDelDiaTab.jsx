      ...
      {panelVendedor && (
        <div style={{ position: 'fixed', inset: 0, zIndex: 50, display: 'flex', alignItems: 'stretch' }}>
          ...
          <aside style={{
            ...
            {/* Footer panel */}
            {console.log('[FOOTER RENDER] panelVendedor:', !!panelVendedor, 'cierre:', !!panelVendedor?.cierre)}
            <div style={{ padding: '12px 20px', borderTop: '2px solid red', display: 'flex', justifyContent: 'flex-end', gap: 8, flexShrink: 0, background: 'yellow', minHeight: 60 }}>
              <button onClick={() => setPanelVendedor(null)} style={btnOutlineSt}>Cerrar</button>
              <button style={{ background: '#DC2626', color: '#fff', padding: '8px 16px', border: 'none', borderRadius: 6 }}>
                DEBUG
              </button>
            </div>
          </aside>
