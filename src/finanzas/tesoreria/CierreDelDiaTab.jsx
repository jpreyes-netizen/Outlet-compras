...
export function CierreDelDiaTab({ usuario }) {
  ...
  async function handleCorrob() {
    ...
    try {
      const updated = await corroborarCierre({
      ...
      if (updated.descuadre_registrado) {
        ...
        toast.success(`Cierre corroborado · Descuadre efectivo ${dif < 0 ? 'faltante' : 'sobrante'} de ${fmt(Math.abs(dif))} pasó a cobranza`)
      } else {
        toast.success('Cierre corroborado · efectivo cuadra · pendiente conciliación cartolas')
      }
  ...
  const difEfectivo = Number(efectivoContado ?? 0) - efectivoDeclarado
  const obsRequeridaCorrob = difEfectivo !== 0 && obsAdmin.trim() === ''
            ...
            <div style={{ flex: 1, padding: '16px 20px', overflowY: 'auto' }}>
              ...
              {esAdmin && panelVendedor.cierre && (
                <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
                  ...
                  {/* Corroboración admin — SOLO EFECTIVO (los demás se concilian con cartolas) */}
                    ...
                    <div>
                      ...
                      <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8, padding: '10px 12px', marginBottom: 12 }}>
                        <div style={{ fontSize: 11, color: '#92400E', lineHeight: 1.5 }}>
                          💵 Cuenta el efectivo físico que entrega el vendedor. Si hay diferencia, queda registrada para cobranza.
                          <br />
                          💳 🔗 🏦 Los medios electrónicos (Crédito, Débito, Webpay, Transferencia) se concilian automáticamente con las cartolas Getnet/Webpay/banco.
                        </div>
                      </div>

                      <div style={{ background: '#F9FAFB', borderRadius: 8, padding: '12px 14px', display: 'flex', flexDirection: 'column', gap: 10 }}>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12 }}>
                          <span style={{ color: '#6B7280' }}>Efectivo declarado</span>
                          <span style={{ fontWeight: 600 }}>{fmt(efectivoDeclarado)}</span>
                        </div>
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', alignItems: 'center', gap: 8 }}>
                          <label style={{ ...labelSt, marginBottom: 0, fontSize: 12, fontWeight: 600, color: '#111827' }}>
                            Efectivo contado
                          </label>
                          <MoneyInput value={efectivoContado} onChange={setEfectivoContado} />
                        </div>
                        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', fontSize: 12, borderTop: '1px solid #E5E7EB', paddingTop: 8 }}>
                          <span style={{ color: '#6B7280' }}>Diferencia</span>
                          <span style={{
                            fontWeight: 700, fontSize: 13,
                            color: difEfectivo === 0 ? '#16A34A' : '#DC2626'
                          }}>
                        ...
                        {difEfectivo !== 0 && (
                          <div style={{ fontSize: 11, color: '#DC2626', fontWeight: 500 }}>
                            ⚠ Pasará a cobranza como {difEfectivo < 0 ? 'faltante del vendedor' : 'sobrante en caja'}
                          </div>
                        )}
                      </div>
                      ...
                      <div style={{ marginTop: 10 }}>
                        <label style={labelSt}>
                          Observaciones admin {difEfectivo !== 0 && <span style={{ color: '#DC2626' }}>*</span>}
                        </label>
                        <textarea value={obsAdmin} onChange={e => setObsAdmin(e.target.value)} rows={2}
                          placeholder={difEfectivo !== 0 ? 'Obligatorio: explica el descuadre de efectivo' : 'Opcional'}
