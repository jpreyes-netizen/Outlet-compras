...
function PanelDeclaracion({ vendedorBsale, cierre, sucursalId, fecha, usuario, umbrales, onGuardado, vendedorReal, rigeCaja, esperado, onSync, syncing }) {
  ...
  const cajaOperativa = rigeCaja && !!esp
  const sinLedger = rigeCaja && !esp
  const abonoAplicadoLedger = esp ? N(esp.abono_aplicado) : null
  // Frescura: si el último pago del ledger es muy viejo, el esperado puede estar incompleto
  const ledgerHorasAtras = esp?.ultimo_pago_at ? (Date.now() - new Date(esp.ultimo_pago_at).getTime()) / 3600000 : null
  const ledgerViejo = ledgerHorasAtras != null && ledgerHorasAtras > 2
  ...
  const brecha = rigeCaja ? (cajaEsperada != null ? cajaDeclarada - cajaEsperada : null) : brechaVenta
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {cajaOperativa && ledgerViejo && (
        <div style={{ background: '#FFFBEB', border: '1px solid #FDE68A', borderRadius: 8, padding: '8px 12px', fontSize: 11, color: '#92400E', display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
          <span>El último pago del ledger es de hace {Math.round(ledgerHorasAtras)} h. Si hubo ventas después, actualiza antes de firmar.</span>
            ...
            <div key={med.key}>
              ...
              {esp && (
                <Esperado valor={esperadoPorMedio[med.key]} declarado={valores[med.key]}
                  onUsar={!esReadOnly && med.key !== 'efectivo' ? (v => setValores(p => ({ ...p, [med.key]: v }))) : null} />
              )}
            </div>
          ))}
        </div>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
            {MEDIOS_OTROS.map(med => (
              <div key={med.key}>
                <label style={{ ...labelSt, marginBottom: 3 }}>{med.label}</label>
                <MoneyInput disabled={esReadOnly} value={valores[med.key]} onChange={n => setValores(p => ({ ...p, [med.key]: n }))} />
                {esp && esperadoPorMedio[med.key] != null && (
                  <Esperado valor={esperadoPorMedio[med.key]} declarado={valores[med.key]}
                    onUsar={!esReadOnly ? (v => setValores(p2 => ({ ...p2, [med.key]: v }))) : null} />
                )}
              </div>
            ))}
          </div>
...
export function CierreDelDiaTab({ usuario }) {
  ...
  // Esperado por cajero desde el ledger. Para HOY (o al forzar) trae primero los últimos
  // pagos de BSALE; para días anteriores el cron diario ya dejó el ledger completo.
  const cargarEsperados = useCallback(async (forzarSync = false) => {
    ...
    try {
      ...
      if (forzarSync || fecha === todayISO()) {
        setSyncing(true)
        await syncPagosIncremental(5)
        setEsperados(await fetchCierresEsperadosDia(fecha, sucursalSel))
      }
    } catch (e) {
      console.warn('[cargarEsperados]', e?.message)
    } finally { setLoadingEsp(false); setSyncing(false) }
  }, [fecha, sucursalSel])
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead>
              <tr style={{ background: '#F9FAFB' }}>
                {(rigeCaja
                  ? ['Recaudador', 'Venta contable', 'Caja esperada', 'Caja declarada', 'Brecha caja', 'Estado', '']
                  : ['Recaudador', 'Venta BSALE', 'Declarado', 'Brecha', 'Estado', '']
                ).map(h => (
                  <th key={h} style={{
                  ...
                  <tr key={i}
                    ...
                    {rigeCaja && (
                      <td style={{ padding: '10px 14px', textAlign: 'right', fontWeight: 600, fontSize: 13, color: '#166534' }}>
                        {cajaEsp != null ? fmt(cajaEsp) : (loadingEsp ? <Loader2 size={12} style={{ color: '#9CA3AF' }} /> : <span style={{ color: '#D1D5DB' }}>—</span>)}
                      </td>
                    )}
