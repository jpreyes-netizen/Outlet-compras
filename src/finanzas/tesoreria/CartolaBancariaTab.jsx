...
async function fetchRegistrosMes(tipo, desde, hasta, filtroTexto) {
  ...
  if (tipo === 'getnet') {
    const { data } = await supabase.from('getnet_abonos')
      .select('id, num_abono, fecha_abono, local_getnet, sucursal_id, valor, cant_ventas, estado_abono, info_abono')
      .gte('fecha_abono', desde).lte('fecha_abono', hasta)
      .order('fecha_abono', { ascending: false }).limit(200)
    return (data ?? [])
      .filter(r => !ft || String(r.num_abono||'').includes(ft) || String(r.valor||'').includes(ft) || (r.local_getnet||'').toLowerCase().includes(ft) || (r.sucursal_id||'').toLowerCase().includes(ft))
      .map(r => ({ id: r.id, _tabla: 'getnet_abonos', _monto: r.valor, _fecha: r.fecha_abono,
        _label: `Abono #${r.num_abono}`,
        _cols: [
          { l: 'Fecha abono',  v: r.fecha_abono },
          { l: 'Num. abono',   v: r.num_abono },
          { l: 'Local',        v: r.local_getnet },
          { l: 'Sucursal',     v: r.sucursal_id },
          { l: 'N° ventas',    v: r.cant_ventas },
          { l: 'Estado',       v: r.estado_abono },
          { l: 'Monto',        v: fmt(r.valor), bold: true },
        ]
      }))
  }
  ...
  if (tipo === 'webpay') {
    const { data } = await supabase.from('webpay_transacciones')
      .select('id, fecha_venta_date, fecha_abono, monto_afecto, cod_autorizacion, tipo_tarjeta, tipo_normalizado, tipo_cuota, n_cuotas, n_boleta')
      .gte('fecha_venta_date', desde).lte('fecha_venta_date', hasta)
      .eq('tipo_movimiento', 'Venta')
      .order('fecha_venta_date', { ascending: false }).limit(200)
    return (data ?? [])
      .filter(r => !ft || String(r.cod_autorizacion||'').toLowerCase().includes(ft) || String(r.monto_afecto||'').includes(ft) || (r.tipo_tarjeta||'').toLowerCase().includes(ft) || (r.n_boleta||'').includes(ft))
      .map(r => ({ id: r.id, _tabla: 'webpay_transacciones', _monto: r.monto_afecto, _fecha: r.fecha_venta_date,
        _label: `Aut. ${r.cod_autorizacion}`,
        _cols: [
          { l: 'Fecha venta',  v: r.fecha_venta_date },
          { l: 'Fecha abono',  v: r.fecha_abono ?? '—' },
          { l: 'Cód. aut.',    v: r.cod_autorizacion },
          { l: 'Tarjeta',      v: r.tipo_tarjeta },
          { l: 'Tipo',         v: r.tipo_normalizado },
          { l: 'Cuotas',       v: r.n_cuotas === 'S/C' ? 'Sin cuotas' : `${r.n_cuotas} · ${r.tipo_cuota||''}` },
          { l: 'N° boleta',    v: r.n_boleta ?? '—' },
          { l: 'Monto',        v: fmt(r.monto_afecto), bold: true },
        ]
      }))
  }
  ...
  if (tipo === 'transferencia_cliente') {
    const { data } = await supabase.from('cierres_caja')
      .select('id, fecha, sucursal_id, transferencia, transferencia_corrob, estado')
      .gte('fecha', desde).lte('fecha', hasta)
      .gt('transferencia_corrob', 0)
      .order('fecha', { ascending: false }).limit(200)
    return (data ?? [])
      .filter(r => !ft || String(r.transferencia_corrob||'').includes(ft) || (r.sucursal_id||'').toLowerCase().includes(ft))
      .map(r => ({ id: r.id, _tabla: 'cierres_caja', _monto: r.transferencia_corrob, _fecha: r.fecha,
        _label: `Cierre ${r.fecha}`,
        _cols: [
          { l: 'Fecha',            v: r.fecha },
          { l: 'Sucursal',         v: r.sucursal_id },
          { l: 'Estado cierre',    v: r.estado },
          { l: 'Decl. transf.',    v: fmt(r.transferencia ?? 0) },
          { l: 'Corrob. transf.',  v: fmt(r.transferencia_corrob), bold: true },
        ]
      }))
  }
      ...
      <div style={{ background:'#fff', borderRadius:14, padding:0, maxWidth:660, width:'100%', maxHeight:'92vh', display:'flex', flexDirection:'column', boxShadow:'0 16px 48px rgba(0,0,0,0.25)' }}>
        ...
        <div style={{ padding:'18px 24px 12px', borderBottom:'1px solid #F3F4F6' }}>
          ...
          <div style={{ fontSize:12, color:'#6B7280', padding:'8px 12px', background:'#F9FAFB', borderRadius:8, display:'flex', gap:12, flexWrap:'wrap' }}>
            ...
            {fechaGetnet && <span style={{ color:'#4F46E5', fontWeight:600 }}>→ venta {fechaGetnet}</span>}
          </div>
