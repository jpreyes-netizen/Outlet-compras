...
// Extrae fecha desde descripción cartola: "Abono Ventas GETNET 29/01/26 CIERR" → "2026-01-29"
...
async function buscarCandidatos(cartola, busquedaManual = null) {
  ...
  if (tipo === 'GETNET' || tipo === 'getnet') {
    let q = supabase.from('getnet_abonos')
      .select('id, fecha_abono, valor, num_abono, sucursal_id')
      .gte('fecha_abono', desdeStr).lte('fecha_abono', hastaStr)
    if (busquedaManual?.monto) q = q.eq('valor', busquedaManual.monto)
    const { data } = await q
    for (const r of data ?? []) {
      const s = scoreMatch(cartola, { monto: r.valor, fecha: r.fecha_abono, nombre: '', rut: '', medio: 'getnet' })
      candidatos.push({ ...r, _tabla: 'getnet_abonos', _score: s,
        _label: `Abono Getnet #${r.num_abono} · ${r.sucursal_id ?? ''}`,
        _monto: r.valor, _fecha: r.fecha_abono })
    }
  }

  if (tipo === 'WEBPAY' || tipo === 'webpay') {
    const { data } = await supabase.from('webpay_transacciones')
      .select('id, fecha_abono, monto_afecto, cod_autorizacion')
      .gte('fecha_abono', desdeStr).lte('fecha_abono', hastaStr)
    for (const r of data ?? []) {
      const s = scoreMatch(cartola, { monto: r.monto_afecto, fecha: r.fecha_abono, nombre: '', rut: '', medio: 'webpay' })
      candidatos.push({ ...r, _tabla: 'webpay_transacciones', _score: s,
        _label: `Webpay aut.${r.cod_autorizacion}`,
        _monto: r.monto_afecto, _fecha: r.fecha_abono })
    }
  }
  ...
  if (tipo === 'TRANSFERENCIA' || tipo === 'transferencia_cliente') {
    const { data } = await supabase.from('cierres_caja')
      .select('id, fecha, sucursal_id, transferencia_corrob')
      .gte('fecha', desdeStr).lte('fecha', hastaStr)
      .gt('transferencia_corrob', 0)
    for (const r of data ?? []) {
      const s = scoreMatch(cartola, { monto: r.transferencia_corrob, fecha: r.fecha, nombre: '', rut: '', medio: 'transferencia' })
      candidatos.push({ ...r, _tabla: 'cierres_caja', _score: s,
        _label: `Cierre ${r.fecha} · ${r.sucursal_id}`,
        _monto: r.transferencia_corrob, _fecha: r.fecha })
    }
  }
...
// ── Modal Conciliar ────────────────────────────────────────────────────────
function ModalConciliar({ movimiento, conciliacionExistente, onClose, onSaved }) {
  ...
  const necesitaMatch = !['prestamo_socio','reembolso','otro'].includes(tipo)
  const fechaGetnet   = extraerFechaDesdeDesc(movimiento.descripcion)

  function lanzarBusqueda(params = {}) {
    if (!necesitaMatch || estado !== 'conciliado') return
    setBuscando(true); setCandidatos([]); setSeleccion(null)
    const manual = busqFecha || busqMonto || params.fecha || params.monto
      ? { fecha: params.fecha ?? busqFecha ?? null, monto: params.monto ? Number(params.monto) : busqMonto ? Number(busqMonto) : null }
      : null
    buscarCandidatos({ ...movimiento, tipo_confirmado: tipo }, manual)
      .then(setCandidatos)
      .catch(e => console.error(e))
      .finally(() => setBuscando(false))
  }

  useEffect(() => { lanzarBusqueda() }, [tipo, estado])

  async function desvincular() {
    if (!window.confirm('¿Deseas desvincular esta conciliación?')) return
    setSaving(true)
    try {
      await supabase.from('cartola_conciliacion').delete().eq('cartola_id', movimiento.id)
      await supabase.from('cartola_bancaria').update({ estado: 'pendiente', tipo_confirmado: null }).eq('id', movimiento.id)
      toast.success('Conciliación desvinculada')
      onSaved()
      ...
      if (estado === 'conciliado') {
        ...
        if (seleccion) {
          await supabase.from('cartola_conciliacion').insert({
            ...
            score: seleccion._score, metodo_match: 'manual', observaciones: obs || null,
          })
        } else if (!necesitaMatch) {
          await supabase.from('cartola_conciliacion').insert({
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.5)' }}
      ...
      <div style={{ background:'#fff', borderRadius:14, padding:24, maxWidth:580, width:'100%', maxHeight:'90vh', overflowY:'auto', boxShadow:'0 12px 40px rgba(0,0,0,0.2)' }}>

        <div style={{ fontSize:15, fontWeight:700, marginBottom:4 }}>Conciliar movimiento</div>
        <div style={{ fontSize:12, color:'#6B7280', marginBottom:16, padding:'8px 12px', background:'#F9FAFB', borderRadius:8 }}>
          <span style={{ fontWeight:600 }}>{movimiento.fecha}</span> ·{' '}
          <span style={{ fontWeight:700, color:'#0F766E' }}>{fmt(movimiento.monto)}</span> ·{' '}
          {movimiento.descripcion}
          {fechaGetnet && <span style={{ marginLeft:8, color:'#4F46E5', fontWeight:600 }}>→ venta {fechaGetnet}</span>}
        </div>

        {/* Conciliación existente */}
        {conciliacionExistente && (
          <div style={{ padding:'10px 12px', background:'#DCFCE7', borderRadius:8, fontSize:12, color:'#166534', marginBottom:12,
            display:'flex', justifyContent:'space-between', alignItems:'center' }}>
            <span>✓ Vinculado: <strong>{conciliacionExistente.tabla_origen}</strong> · reg. {conciliacionExistente.registro_id.slice(0,8)}...</span>
            <button onClick={desvincular} style={{ fontSize:11, color:'#DC2626', background:'none', border:'none', cursor:'pointer', fontWeight:600 }}>
              Desvincular
            </button>
          </div>
        )}
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
          <div style={{ display:'grid', gridTemplateColumns:'1fr 1fr', gap:10 }}>
            <div>
              ...
              <select style={selectSt} value={tipo} onChange={e => { setTipo(e.target.value); setSeleccion(null) }}>
          ...
          {/* Sección candidatos */}
          {estado === 'conciliado' && necesitaMatch && (
            <div>
              <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginBottom:6 }}>
                <label style={labelSt}>Vincular con registro origen</label>
                {fechaGetnet && tipo.toLowerCase().includes('getnet') && (
                  <span style={{ fontSize:10, color:'#4F46E5', fontWeight:600 }}>Buscando fecha {fechaGetnet}</span>
                )}
              </div>

              {/* Búsqueda manual */}
              <div style={{ display:'flex', gap:6, marginBottom:8 }}>
                <input style={{ ...inputSt, flex:1 }} placeholder="Fecha exacta (YYYY-MM-DD)"
                  value={busqFecha} onChange={e => setBusqFecha(e.target.value)} />
                <input style={{ ...inputSt, flex:1 }} placeholder="Monto exacto"
                  value={busqMonto} onChange={e => setBusqMonto(e.target.value)} />
                <button onClick={() => lanzarBusqueda()}
                  style={{ ...btnSt('#4F46E5'), padding:'6px 14px', fontSize:12, whiteSpace:'nowrap' }}>
                  Buscar
                </button>
              </div>
              ...
              {buscando ? (
                <div style={{ padding:'12px', textAlign:'center', color:'#9CA3AF', fontSize:12 }}>
                  <Loader2 size={14} style={{ display:'inline-block' }} /> Buscando...
                </div>
              ) : candidatos.length === 0 ? (
                <div style={{ padding:'12px', background:'#FEF3C7', borderRadius:8, fontSize:12, color:'#92400E' }}>
                  Sin candidatos. Usa el buscador manual o concilia sin vinculación.
                </div>
              ) : (
                <div style={{ display:'flex', flexDirection:'column', gap:5 }}>
                  {candidatos.map((c, i) => (
                    <div key={i} onClick={() => setSeleccion(seleccion?.id === c.id ? null : c)}
                      style={{ padding:'10px 12px', borderRadius:8, cursor:'pointer',
                        border:`2px solid ${seleccion?.id === c.id ? '#0F766E' : '#E5E7EB'}`,
                        background: seleccion?.id === c.id ? '#F0FDFA' : '#FAFAFA',
                        display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                      <div>
                        <div style={{ fontSize:12, fontWeight:600 }}>{c._label}</div>
                        <div style={{ fontSize:11, color:'#6B7280', marginTop:1 }}>
                          {fmt(c._monto)} · {c._fecha}
                          {Math.abs(c._monto - movimiento.monto) > 5 && (
                            <span style={{ color:'#DC2626', marginLeft:6 }}>Δ {fmt(Math.abs(c._monto - movimiento.monto))}</span>
                          )}
                        </div>
                      </div>
                      <div style={{ display:'flex', alignItems:'center', gap:6 }}>
                        <Estrellas score={c._score} />
                        <span style={{ fontSize:10, color:'#6B7280' }}>{c._score}pts</span>
                        {seleccion?.id === c.id && <Check size={14} color="#0F766E" />}
                      </div>
                    </div>
                  ))}
                  <div style={{ fontSize:10, color:'#9CA3AF' }}>★★★ ≥80 · ★★ ≥50 · ★ &lt;50 — clic para seleccionar</div>
                </div>
              )}
            </div>
          )}
          ...
          {estado === 'conciliado' && !necesitaMatch && (
            <div style={{ padding:'10px 12px', background:'#EFF6FF', borderRadius:8, fontSize:12, color:'#1E40AF' }}>
              Se concilia sin vinculación a registro específico. Agrega una observación descriptiva.
            </div>
          )}
          ...
          <div>
            ...
            <input style={inputSt} value={obs} onChange={e => setObs(e.target.value)}
              placeholder={!necesitaMatch ? 'Ej: Préstamo JP Reyes Feb 2026...' : 'Notas adicionales...'} />
          </div>
        </div>

        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:20 }}>
          <div style={{ fontSize:11, color:'#9CA3AF' }}>
            {estado === 'conciliado' && necesitaMatch && !seleccion && candidatos.length > 0 &&
              '⚠ Sin selección — conciliado sin vinculación'}
          </div>
          <div style={{ display:'flex', gap:8 }}>
