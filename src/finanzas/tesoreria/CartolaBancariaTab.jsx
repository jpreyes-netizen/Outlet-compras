...
function scoreMatch(cartola, candidato) {
  ...
  // Nombre (simple — contiene)
  const nc = (cartola.descripcion || '').toUpperCase()
  const nn = (candidato.nombre || '').toUpperCase().split(' ')[0] // primer apellido
  if (nn.length > 3 && nc.includes(nn))                score += 25

  // Fecha — cartola debe ser ANTES o igual al registro origen
  const dc = new Date(cartola.fecha)
  const dd = new Date(candidato.fecha)
  const diasDiff = Math.round((dd - dc) / 86400000) // positivo = cartola antes que doc
  if (diasDiff >= 0 && diasDiff < 7)                   score += 10 + 15
  else if (diasDiff >= 0 && diasDiff < 30)             score += 5  + 15
  else if (diasDiff >= -3)                             score += 5   // cartola 3 días después

  // Medio de pago
  const med = (candidato.medio || '').toUpperCase()
  if (med.includes('TRANSF') || med.includes('TRANSFERENCIA')) score += 20

  return Math.min(score, 100)
}

async function buscarCandidatos(cartola) {
  const tipo = cartola.tipo_confirmado ?? cartola.tipo_auto
  const fecha = cartola.fecha
  const desde = new Date(fecha); desde.setDate(desde.getDate() - 5)
  ...
  if (tipo === 'GETNET' || tipo === 'getnet') {
    const { data } = await supabase.from('getnet_abonos')
      .select('id, fecha_abono, valor, num_abono')
    ...
    for (const r of data ?? []) {
      ...
      if (s >= 30) candidatos.push({ ...r, _tabla: 'getnet_abonos', _score: s, _label: `Abono Getnet #${r.num_abono}`, _monto: r.valor, _fecha: r.fecha_abono })
    }
  }

  if (tipo === 'WEBPAY' || tipo === 'webpay') {
    const { data } = await supabase.from('webpay_transacciones')
      .select('id, fecha_abono, monto_afecto, numero_unico, cod_autorizacion')
      .gte('fecha_abono', desdeStr).lte('fecha_abono', hastaStr)
    for (const r of data ?? []) {
      const s = scoreMatch(cartola, { monto: r.monto_afecto, fecha: r.fecha_abono, nombre: '', rut: '', medio: 'webpay' })
      if (s >= 30) candidatos.push({ ...r, _tabla: 'webpay_transacciones', _score: s, _label: `Webpay ${r.cod_autorizacion}`, _monto: r.monto_afecto, _fecha: r.fecha_abono })
    }
  }
  ...
  if (tipo === 'DEPOSITO' || tipo === 'deposito_efectivo') {
    ...
    for (const r of data ?? []) {
      ...
      if (s >= 30) candidatos.push({ ...r, _tabla: 'depositos_efectivo', _score: s, _label: `Depósito ${r.fecha_deposito} · ${r.sucursal_id}`, _monto: r.monto_depositado, _fecha: r.fecha_deposito })
    }
  }

  if (tipo === 'TRANSFERENCIA' || tipo === 'transferencia_cliente') {
    const { data } = await supabase.from('cierres_caja')
      .select('id, fecha, sucursal_id, transferencia_corrob, vendedor_id')
    ...
    for (const r of data ?? []) {
      ...
      if (s >= 30) candidatos.push({ ...r, _tabla: 'cierres_caja', _score: s, _label: `Cierre ${r.fecha} · ${r.sucursal_id}`, _monto: r.transferencia_corrob, _fecha: r.fecha })
    }
  }

  return candidatos.sort((a,b) => b._score - a._score).slice(0, 8)
}
...
// ── Modal Conciliar ────────────────────────────────────────────────────────
function ModalConciliar({ movimiento, onClose, onSaved }) {
  ...
  useEffect(() => {
    ...
    setBuscando(true)
    buscarCandidatos({ ...movimiento, tipo_confirmado: tipo })
      .then(setCandidatos)
  ...
  async function guardar() {
    setSaving(true)
    try {
      // 1) Actualizar cartola_bancaria
      const { error: e1 } = await supabase.from('cartola_bancaria')
        .update({ tipo_confirmado: tipo, observaciones: obs || null, estado })
        .eq('id', movimiento.id)
      if (e1) throw e1

      // 2) Si hay selección, insertar en cartola_conciliacion
      if (seleccion && estado === 'conciliado') {
        // Borrar conciliación previa si existe
        await supabase.from('cartola_conciliacion').delete().eq('cartola_id', movimiento.id)
        const { error: e2 } = await supabase.from('cartola_conciliacion').insert({
          cartola_id:       movimiento.id,
          tabla_origen:     seleccion._tabla,
          registro_id:      String(seleccion.id),
          monto_aplicado:   movimiento.monto,
          score:            seleccion._score,
          metodo_match:     'manual',
          observaciones:    obs || null,
        })
        if (e2) throw e2
      } else if (estado === 'conciliado' && !necesitaMatch) {
        // Préstamo/reembolso — conciliar libre sin vinculación
        await supabase.from('cartola_conciliacion').delete().eq('cartola_id', movimiento.id)
  ...
  return (
    <div style={{ position:'fixed', inset:0, zIndex:9999, display:'flex', alignItems:'center', justifyContent:'center', background:'rgba(0,0,0,0.5)' }}
      ...
      <div style={{ background:'#fff', borderRadius:14, padding:24, maxWidth:560, width:'100%', maxHeight:'90vh', overflowY:'auto', boxShadow:'0 12px 40px rgba(0,0,0,0.2)' }}>
        ...
        <div style={{ display:'flex', flexDirection:'column', gap:12 }}>
            ...
            <div>
              ...
              {buscando ? (
                <div style={{ padding:'12px', textAlign:'center', color:'#9CA3AF', fontSize:12 }}>
                  <Loader2 size={14} style={{ display:'inline-block' }} /> Buscando candidatos...
                </div>
              ) : candidatos.length === 0 ? (
                <div style={{ padding:'12px', background:'#FEF3C7', borderRadius:8, fontSize:12, color:'#92400E' }}>
                  No se encontraron candidatos. Puedes conciliar sin vinculación o revisar el tipo.
                </div>
              ) : (
                <div style={{ display:'flex', flexDirection:'column', gap:6 }}>
                  {candidatos.map((c, i) => (
                    <div key={i} onClick={() => setSeleccion(seleccion?.id === c.id ? null : c)}
                      style={{ padding:'10px 12px', borderRadius:8, cursor:'pointer', border:`2px solid ${seleccion?.id === c.id ? '#0F766E' : '#E5E7EB'}`,
                        background: seleccion?.id === c.id ? '#F0FDFA' : '#FAFAFA',
                        display:'flex', justifyContent:'space-between', alignItems:'center' }}>
                      <div>
                        <div style={{ fontSize:12, fontWeight:600, color:'#111827' }}>{c._label}</div>
                        <div style={{ fontSize:11, color:'#6B7280', marginTop:2 }}>
                          {fmt(c._monto)} · {c._fecha}
                          {Math.abs(c._monto - movimiento.monto) > 0 && (
                            <span style={{ color:'#DC2626', marginLeft:6 }}>
                              Δ {fmt(Math.abs(c._monto - movimiento.monto))}
                            </span>
                          )}
                        </div>
                      </div>
                  ...
                  <div style={{ fontSize:11, color:'#9CA3AF', marginTop:2 }}>
                    ★★★ ≥80pts · ★★ ≥50pts · ★ &lt;50pts — Haz clic para seleccionar
                  </div>
                </div>
              )}
            </div>
          ...
          {/* Libre — préstamo/reembolso */}
          {estado === 'conciliado' && !necesitaMatch && (
            <div style={{ padding:'10px 12px', background:'#EFF6FF', borderRadius:8, fontSize:12, color:'#1E40AF' }}>
              Este tipo se concilia sin vinculación a un registro específico. Agrega una observación descriptiva.
            </div>
          )}
          ...
          <div>
            ...
            <input style={inputSt} value={obs} onChange={e => setObs(e.target.value)}
              placeholder={!necesitaMatch ? 'Ej: Préstamo JP Reyes Feb 2026, Reembolso proveedor XYZ...' : 'Notas adicionales...'} />
          </div>
        </div>
        ...
        <div style={{ display:'flex', justifyContent:'space-between', alignItems:'center', marginTop:20 }}>
          <div style={{ fontSize:11, color:'#9CA3AF' }}>
            {estado === 'conciliado' && necesitaMatch && !seleccion && candidatos.length > 0 &&
              '⚠ Sin selección — se guardará como conciliado sin vinculación'}
          </div>
          <div style={{ display:'flex', gap:8 }}>
