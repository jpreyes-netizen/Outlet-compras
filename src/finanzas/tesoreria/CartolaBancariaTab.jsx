...
async function fetchRegistrosMes(tipo, desde, hasta, filtroTexto) {
  ...
  if (tipo === 'getnet') {
    const { data } = await supabase.from('getnet_abonos')
      .select('id, fecha_abono, valor, num_abono, sucursal_id, cant_ventas')
      .gte('fecha_abono', desde).lte('fecha_abono', hasta)
      .order('fecha_abono', { ascending: false }).limit(200)
    return (data ?? [])
      .filter(r => !ft || String(r.num_abono).includes(ft) || String(r.valor).includes(ft) || (r.sucursal_id||'').toLowerCase().includes(ft))
      .map(r => ({ id: r.id, _tabla:'getnet_abonos', _monto: r.valor, _fecha: r.fecha_abono,
        _label: `Abono #${r.num_abono}`, _sub: `${r.sucursal_id ?? ''} · ${r.cant_ventas} ventas` }))
  }

  if (tipo === 'webpay') {
    const { data } = await supabase.from('webpay_transacciones')
      .select('id, fecha_venta_date, monto_afecto, cod_autorizacion, tipo_tarjeta')
      .gte('fecha_venta_date', desde).lte('fecha_venta_date', hasta)
      .eq('tipo_movimiento', 'Venta')
      .order('fecha_venta_date', { ascending: false }).limit(200)
    return (data ?? [])
      .filter(r => !ft || String(r.cod_autorizacion||'').toLowerCase().includes(ft) || String(r.monto_afecto).includes(ft))
      .map(r => ({ id: r.id, _tabla:'webpay_transacciones', _monto: r.monto_afecto, _fecha: r.fecha_venta_date,
        _label: `Aut. ${r.cod_autorizacion}`, _sub: r.tipo_tarjeta ?? '' }))
  }

  if (tipo === 'deposito_efectivo') {
    const { data } = await supabase.from('depositos_efectivo')
      .select('id, fecha, monto_depositado, sucursal_id, estado, observaciones')
      .gte('fecha', desde).lte('fecha', hasta)
      .order('fecha', { ascending: false }).limit(200)
    return (data ?? [])
      .filter(r => !ft || String(r.monto_depositado).includes(ft) || (r.sucursal_id||'').toLowerCase().includes(ft) || (r.observaciones||'').toLowerCase().includes(ft))
      .map(r => ({ id: r.id, _tabla:'depositos_efectivo', _monto: r.monto_depositado, _fecha: r.fecha,
        _label: `Depósito ${r.fecha}`, _sub: `${r.sucursal_id ?? ''} · ${r.estado ?? ''}${r.observaciones ? ' · '+r.observaciones.slice(0,30) : ''}` }))
  }
  ...
  if (tipo === 'transferencia_cliente') {
    ...
    return (data ?? [])
      ...
      .map(r => ({ id: r.id, _tabla:'cierres_caja', _monto: r.transferencia_corrob, _fecha: r.fecha,
        _label: `Cierre ${r.fecha}`, _sub: r.sucursal_id ?? '' }))
  }
                  ...
                  return (
                    <div key={r.id} onClick={() => setSeleccion(sel ? null : r)}
                      ...
                      <div style={{ minWidth:0 }}>
                        <div style={{ fontSize:12, fontWeight:600, color:'#111827' }}>{r._label}</div>
                      ...
                      <div style={{ display:'flex', alignItems:'center', gap:10, flexShrink:0, marginLeft:12 }}>
                        <div style={{ textAlign:'right' }}>
                          <div style={{ fontSize:12, fontWeight:700 }}>{fmt(r._monto)}</div>
                          {diffM > 5 && <div style={{ fontSize:10, color:'#DC2626' }}>Δ {fmt(diffM)}</div>}
                        </div>
                        <div style={{ textAlign:'center', minWidth:48 }}>
                          {sc > 0 && <><Estrellas score={sc} /><div style={{ fontSize:9, color:'#9CA3AF' }}>{sc}pts</div></>}
                        </div>
                        {sel && <Check size={16} color="#0F766E" />}
                      </div>
                    </div>
                  )
                })
