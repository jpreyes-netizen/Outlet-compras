...
import { fetchSucursales, fetchUmbrales, fetchKpisMes, fetchCierres } from './api'
...
export function AnalisisTab({ usuario }) {
  ...
  const resumenPrev = useMemo(() => ({
    totalDecl: cierresMesPrev.reduce((s, c) => s + Number(c.total_declarado ?? 0), 0),
    totalCorr: cierresMesPrev.reduce((s, c) => s + Number(c.total_corroborado ?? 0), 0),
    n: cierresMesPrev.length,
  }), [cierresMesPrev])

  const funnel = useMemo(() => ({
    vendido: kpis?.ventasBsale ?? 0,
    cobrado: resumen.totalCorr || resumen.totalDecl,
    depositado: depositosBancarios.total,
    conciliado: depositosBancarios.conciliado,
  }), [kpis, resumen, depositosBancarios])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{
        ...
        color: '#fff', display: 'grid', gridTemplateColumns: '1fr auto auto auto auto', gap: 14, alignItems: 'center'
      }}>
        <div>
