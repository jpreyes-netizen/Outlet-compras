...
export function AnalisisTab({ usuario }) {
  ...
  async function handleSync(forzarTodo = false) {
    ...
    try {
      let result
      if (forzarTodo) {
        // Sincronización mes completo - usa edge function "mes" (mucho más rápido)
        result = await sincronizarMesCompleto({
        ...
        // Solo faltantes - usa modo combos paralelo
        result = await sincronizarCacheFaltante({
          anio, mes, sucursal_id: sucursalEf,
          onProgress: (p) => setSyncProgress(p)
        })
        if (result.total === 0) {
