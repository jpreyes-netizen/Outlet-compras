...
import { fetchSucursales, fetchUmbrales, fetchKpisMes, fetchCierres, sincronizarYAuditarBsaleMes } from './api'
...
export function AnalisisTab({ usuario }) {
  ...
  const [gapsCobertura, setGapsCobertura] = useState([]) // días con venta BSALE sin cierre
  ...
  async function handleSync() {
    ...
    setSyncing(true)
    setGapsCobertura([])
    setSyncProgress({ actual: 0, total: 0, ok: 0, errores: 0, gaps: 0 })
    try {
      const result = await sincronizarYAuditarBsaleMes({
        anio, mes, sucursal_id: sucursalEf,
        onProgress: (p) => setSyncProgress(p)
      })
      setGapsCobertura(result.gaps ?? [])
      const nGaps = result.gaps?.length ?? 0
      setSyncProgress({
        actual: result.total, total: result.total,
        ok: result.ok, errores: result.errores, gaps: nGaps,
        mensaje: nGaps > 0
          ? `⚠ ${nGaps} día(s) con venta sin cierre detectados`
          : '✓ Sin gaps — todo declarado'
      })
      await cargar()
    } catch (e) {
      console.error('[handleSync]', e)
      setSyncProgress({ actual: 0, total: 0, ok: 0, errores: 1, gaps: 0, mensaje: 'Error: ' + (e.message ?? 'desconocido') })
    } finally {
      setSyncing(false)
  ...
  const funnel = useMemo(() => {
    // vendido: suma TODAS las filas (cada vendedor declara su propia venta)
    // usa venta_bsale_api cuando existe, sino total_declarado
    const vendido = cierres.reduce((s, c) => {
  ...
  const coberturaResumen = useMemo(() => {
    ...
    const ventaNoDeclarada = gapsCobertura.reduce((s, g) => s + g.venta_bsale, 0)
    const porSucursal = {}
    gapsCobertura.forEach(g => {
      if (!porSucursal[g.sucursal_id]) porSucursal[g.sucursal_id] = { dias: 0, venta: 0 }
      porSucursal[g.sucursal_id].dias += 1
      porSucursal[g.sucursal_id].venta += g.venta_bsale
    })
    return { ventaNoDeclarada, porSucursal, gaps: gapsCobertura }
  }, [gapsCobertura])
            ...
            <div style={{
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 12 }}>
                ...
                <span style={{ fontSize: 13, fontWeight: 700, color: '#EA580C' }}>
                  🚨 {coberturaResumen.gaps.length} día(s) con venta BSALE sin cierre declarado
                </span>
                <span style={{ marginLeft: 'auto', fontSize: 13, fontWeight: 800, color: '#EA580C' }}>
                  {fmt(coberturaResumen.ventaNoDeclarada)} sin declarar
                </span>
              </div>
              ...
              <div style={{ display: 'flex', gap: 10, marginBottom: 12, flexWrap: 'wrap' }}>
                {Object.entries(coberturaResumen.porSucursal).map(([suc, v]) => {
                  ...
                  return (
                    <div key={suc} style={{ background: '#FEE2E2', borderRadius: 8, padding: '8px 14px', fontSize: 12 }}>
                      <div style={{ fontWeight: 700, color: C.rojo }}>{nombre}</div>
                      <div style={{ color: C.rojo, fontSize: 11 }}>{v.dias} día(s) · {fmtCompact(v.venta)}</div>
                    </div>
                  )
                })}
              </div>
                ...
                <table style={{ width: '100%', borderCollapse: 'collapse', fontSize: 11 }}>
                  <thead>
                    <tr style={{ background: '#FFEDD5' }}>
                      ...
                      <th style={{ ...TH, textAlign: 'right', color: '#EA580C' }}>Venta BSALE</th>
                      <th style={{ ...TH, color: '#EA580C' }}>Medios detectados</th>
                      <th style={{ ...TH, color: '#EA580C' }}>Acción requerida</th>
                    </tr>
                  </thead>
                  <tbody>
                    {coberturaResumen.gaps.sort((a, b) => b.venta_bsale - a.venta_bsale).map((g, i) => {
                      const nombre = sucursales.find(s => s.id === g.sucursal_id)?.nombre ?? g.sucursal_id
                      const mediosStr = Object.entries(g.medios)
                        .filter(([, v]) => v > 0)
                        .map(([k, v]) => `${k}: ${fmtCompact(v)}`)
                        .join(' · ')
                      return (
                        <tr key={i} style={{ borderTop: '1px solid #FED7AA', background: i % 2 === 0 ? '#FFFBF7' : '#FFF7ED' }}>
                          <td style={{ ...TD, fontWeight: 600, color: C.rojo }}>{g.fecha}</td>
                          <td style={{ ...TD, fontWeight: 600 }}>{nombre}</td>
                          <td style={{ ...TD, textAlign: 'right', fontWeight: 800, color: C.rojo }}>{fmt(g.venta_bsale)}</td>
                          <td style={{ ...TD, fontSize: 10, color: C.textTer }}>{mediosStr || '—'}</td>
                          <td style={{ ...TD }}>
                            <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 10, background: C.rojoLight, color: C.rojo }}>
                              Exigir cierre al vendedor
                            </span>
                          </td>
                        </tr>
                      )
                  ...
                  <tfoot>
                    <tr style={{ background: '#FFEDD5', borderTop: '2px solid #FED7AA' }}>
                      <td colSpan={2} style={{ ...TD, fontWeight: 700, color: '#EA580C' }}>Total no declarado</td>
                      <td style={{ ...TD, textAlign: 'right', fontWeight: 800, color: C.rojo }}>{fmt(coberturaResumen.ventaNoDeclarada)}</td>
                      <td colSpan={2} style={{ ...TD, fontSize: 11, color: C.textTer }}>
                        Venta total estimada: <strong style={{ color: C.text }}>{fmt(ventaTotalEstimada)}</strong>
                        {' '}vs declarado: <strong>{fmt(resumen.totalDecl)}</strong>
                      </td>
                    </tr>
                  </tfoot>
                </table>
              ...
              <div style={{ marginTop: 10, fontSize: 11, color: '#92400E' }}>
                <strong>Nota:</strong> Estos días tienen venta registrada en BSALE pero ningún vendedor declaró cierre. La venta existe y fue cobrada — no está capturada en el sistema de tesorería.
              </div>
            </div>
          )}
