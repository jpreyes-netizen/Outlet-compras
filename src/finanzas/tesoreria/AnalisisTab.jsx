...
export function AnalisisTab({ usuario }) {
  ...
  const cargar = async () => {
    ...
    try {
      ...
      const cartolaPromise = supabase.from('cartola_bancaria')
        ...
        .eq('cargo_abono', 'A')
        .gt('monto', 0)
        .neq('tipo_auto', 'OTRO')  // excluir créditos bancarios, transferencias internas
        .then(r => r.data ?? [])
  ...
  const funnel = useMemo(() => {
    // vendido: suma venta_bsale_api por vendedor cuando existe, sino total_declarado
    const vendidoDeclarado = cierres.reduce((s, c) => {
      return s + Number(c.venta_bsale_api ?? c.total_declarado ?? 0)
    }, 0)
    // Agregar gaps detectados (sin cierre + sub-declarado) para estimación real
    const gapTotal = gapsCobertura.reduce((s, g) => s + Number(g.diferencia ?? 0), 0)
    const vendidoReal = vendidoDeclarado + gapTotal
  ...
  }, [cierres, resumen, depositosBancarios, gapsCobertura])
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, animation: 'fadeIn 0.3s ease' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 12 }}>
            <KpiHero
              ...
              sub={funnel.gapTotal > 0
                ? `Declarado: ${fmtCompact(funnel.vendidoDeclarado)} + gaps: ${fmtCompact(funnel.gapTotal)}`
                : `${resumen.n} cierres · sincronizado`}
