...
import {
  ...
  sincronizarRango, sincronizarDia, sincronizarMesCompleto,
  fetchEmbudoCuadratura
} from './api'
...
// ═══════════════════ CASH FUNNEL ═══════════════════
function CashFunnel({ vendido, cobrado, depositado, conciliado, cajaEsperada, cajaDeclarada, puente = [], sinCorroborar = 0 }) {
  // Dos naturalezas distintas en una sola cadena:
  //   Vendido es DEVENGADO (lo facturado). Caja esperada es PERCIBIDO (lo que entró).
  //   Entre ambos va el puente, que NO es fuga: es desfase contable.
  //   Desde caja esperada en adelante todo es percibido y ahí sí, cada caída es fuga real.
  const conPuente = cajaEsperada != null && puente.length > 0
  const etapas = conPuente
    ? [
        { l: 'Caja esperada (BSALE)', v: cajaEsperada, color: C.cyan, ic: <Banknote size={14} />, base: 'percibido' },
        { l: 'Caja declarada (Cierres)', v: cajaDeclarada, color: C.azul2, ic: <CheckCircle2 size={14} />, base: 'percibido' },
  ...
  const max = Math.max(vendido, ...etapas.map(e => e.v), 1)

  const Barra = ({ e, prev, mostrarFuga = true }) => {
    const w = (e.v / max * 100).toFixed(1)
    const ratio = prev != null ? (e.v / prev * 100) : null
    const fuga = prev != null ? prev - e.v : 0
    return (
      <div>
          ...
          <div style={{ display: 'flex', alignItems: 'center', gap: 8 }}>
            {ratio != null && (
              <span style={{
                ...
                background: ratio >= 99 ? C.verdeLight : ratio >= 95 ? C.naranjaLight : C.rojoLight,
                color: ratio >= 99 ? C.verde : ratio >= 95 ? C.naranja : C.rojo
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      {/* Devengado */}
      <Barra e={{ l: 'Vendido (BSALE)', v: vendido, color: C.azul, ic: <DollarSign size={14} /> }} prev={null} />

      {/* Puente: por qué la venta no es la caja. Sin fuga, porque no se pierde nada aquí. */}
      {conPuente && (
        <div style={{ background: '#F9FAFB', borderRadius: 8, padding: '8px 12px', border: '1px dashed #E5E7EB' }}>
          <div style={{ fontSize: 10, fontWeight: 700, color: C.gris, textTransform: 'uppercase', letterSpacing: '0.04em', marginBottom: 5 }}>
            Puente: de lo facturado a lo recaudado
          </div>
          {puente.map(x => (
            <div key={x.l} title={x.tip} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, padding: '1px 0' }}>
              <span style={{ color: C.gris }}>{x.l}</span>
              <span style={{ fontFamily: 'monospace', fontWeight: 600, color: Number(x.v) > 0 ? C.verde : C.text }}>
                {Number(x.v) > 0 ? '+' : ''}{fmt(Number(x.v))}
              </span>
            </div>
          ))}
          <div style={{ fontSize: 10, color: C.gris, marginTop: 5, borderTop: '1px solid #E5E7EB', paddingTop: 4 }}>
            Estas partidas explican la diferencia entre venta y caja. No son pérdida: son desfase contable.
          </div>
        </div>
      )}
      ...
      {/* Percibido */}
      {etapas.map((e, i) => (
        <Barra key={e.l} e={e} prev={i === 0 ? (conPuente ? null : vendido) : etapas[i - 1].v} />
      ))}

      {conPuente && sinCorroborar > 0 && (
        <div style={{ fontSize: 10, color: C.naranja, textAlign: 'right' }}>
          {sinCorroborar} cierre(s) del período aún sin corroborar por tesorería
        </div>
      )}
    </div>
  )
...
// ═══════════════════ RESUMEN POR MES (modo anual) ═══════════════════
function ResumenPorMes({ cierres, anio }) {
  const meses = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic']
  const porMes = Array.from({ length: 12 }, (_, i) => ({
    mes: i + 1, label: meses[i],
    cuadra: 0, tolerable: 0, descuadre: 0, declarado: 0, anulado: 0,
    monto: 0, n: 0
  }))
  for (const c of cierres) {
    if (!c.fecha) continue
    const mesIdx = Number(c.fecha.slice(5, 7)) - 1
    if (mesIdx < 0 || mesIdx > 11) continue
    porMes[mesIdx][c.estado] = (porMes[mesIdx][c.estado] ?? 0) + 1
    porMes[mesIdx].monto += Number(c.total_declarado ?? 0)
    porMes[mesIdx].n += 1
  }
  return (
    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(6, 1fr)', gap: 8 }}>
      {porMes.map(m => {
        const max = Math.max(m.cuadra, m.tolerable, m.descuadre, m.declarado)
        let estadoDom = 'declarado'
        if (m.descuadre === max && max > 0) estadoDom = 'descuadre'
        else if (m.tolerable === max && max > 0) estadoDom = 'tolerable'
        else if (m.cuadra === max && max > 0) estadoDom = 'cuadra'
        const cfg = ESTADO[estadoDom]
        return (
          <div key={m.mes} style={{
            background: m.n === 0 ? '#F9FAFB' : cfg.bg,
            border: `1.5px solid ${m.n === 0 ? '#E5E7EB' : cfg.c}`,
            borderRadius: 8, padding: 10, display: 'flex', flexDirection: 'column', gap: 4
          }}>
            <div style={{ fontSize: 11, fontWeight: 800, color: m.n === 0 ? C.textTer : cfg.c }}>{m.label}</div>
            <div style={{ fontSize: 14, fontWeight: 800, color: C.text }}>{formatCLP(m.monto)}</div>
            <div style={{ fontSize: 10, color: C.textTer }}>{m.n} cierres</div>
            {m.n > 0 && (
              <div style={{ display: 'flex', gap: 3, marginTop: 2, fontSize: 9 }}>
                {m.cuadra > 0 && <span style={{ color: C.verde, fontWeight: 700 }}>✓{m.cuadra}</span>}
                {m.tolerable > 0 && <span style={{ color: C.naranja, fontWeight: 700 }}>⚠{m.tolerable}</span>}
                {m.descuadre > 0 && <span style={{ color: C.rojo, fontWeight: 700 }}>✗{m.descuadre}</span>}
                {m.declarado > 0 && <span style={{ color: C.azul2, fontWeight: 700 }}>·{m.declarado}</span>}
              </div>
            )}
          </div>
        )
...
export function AnalisisTab({ usuario }) {
  ...
  const [embudo, setEmbudo] = useState(null)   // puente devengado → percibido (v_cuadratura_embudo)
  ...
  // Modo anual: mes === 0 significa "Año completo"
  const modoAnual = mes === 0
  const mesPrev = modoAnual ? 0 : (mes === 1 ? 12 : mes - 1)
  const anioPrev = modoAnual ? anio - 1 : (mes === 1 ? anio - 1 : anio)
  ...
  const cargar = async () => {
    setLoading(true)
    try {
      // Fechas según modo (mes o año completo)
      let desde, hasta, desdePrev, hastaPrev
      if (modoAnual) {
        desde = `${anio}-01-01`
        hasta = `${anio}-12-31`
        desdePrev = `${anioPrev}-01-01`
        hastaPrev = `${anioPrev}-12-31`
      } else {
        desde = `${anio}-${String(mes).padStart(2, '0')}-01`
        const fin = new Date(anio, mes, 0)
        hasta = `${anio}-${String(mes).padStart(2, '0')}-${String(fin.getDate()).padStart(2, '0')}`
        desdePrev = `${anioPrev}-${String(mesPrev).padStart(2, '0')}-01`
        const finPrev = new Date(anioPrev, mesPrev, 0)
        hastaPrev = `${anioPrev}-${String(mesPrev).padStart(2, '0')}-${String(finPrev.getDate()).padStart(2, '0')}`
      }
      ...
      const [k, c, kPrev, cPrev, cartola, conciliaciones, vbCache, auditoria, emb] = await Promise.all([
        ...
        // Puente contable devengado → percibido (ledger de pagos BSALE)
        fetchEmbudoCuadratura({ desde, hasta, sucursal_id: sucursalEf }).catch(() => null),
      ])
  ...
  async function handleSync(forzarTodo = false) {
    ...
    try {
      // Ambos modos (faltantes o forzar todo) usan bsale-ventas-dia día por día
      // Más estable que bsale-ventas-mes para meses con mucha data
  ...
  // Sync rápido: usa bsale-ventas-mes (1 llamada por mes, sin reintentos)
  async function handleSyncRapido() {
    if (syncing) return
    if (modoAnual) {
      alert('Para año completo usa "🔄 Todo" (mes por mes). El sync rápido es por un solo mes.')
      return
    }
    setSyncing(true)
    setSyncProgress({ actual: 0, total: 1, ok: 0, errores: 0, mensaje: 'Sincronizando mes en BSALE...' })
    try {
      const result = await sincronizarMesCompleto({
        anio, mes, sucursal_id: sucursalEf,
        onProgress: (p) => setSyncProgress(p)
      })
      setSyncProgress({
        actual: 1, total: 1, ok: result.ok, errores: result.errores,
        mensaje: result.errores > 0
          ? `Error: ${result.error ?? 'desconocido'} — usa "🔄 Todo" como respaldo`
          : `✓ Mes sincronizado · Total BSALE: ${formatCLP(result.total_mes ?? 0)}`
      })
      await cargar()
    } catch (e) {
      console.error('[handleSyncRapido]', e)
      setSyncProgress({ actual: 0, total: 1, ok: 0, errores: 1, mensaje: 'Error: ' + (e.message ?? 'desconocido') })
    } finally {
      setSyncing(false)
      setTimeout(() => setSyncProgress(null), 6000)
    }
  ...
  async function handleSyncCustom() {
    ...
    try {
      ...
      let mensaje
      if (result.total === 0) mensaje = '⚠ Sin días para sincronizar en el rango'
      else if (result.errores > 0) {
        const detalle = (result.fallidos ?? []).slice(0, 3).map(f => `${f.fecha}/${f.sucursal_id}`).join(', ')
        mensaje = `⚠ ${result.errores} combo(s) fallido(s) tras 3 reintentos: ${detalle}${result.errores > 3 ? '...' : ''}`
      }
      else mensaje = `✓ ${result.ok} de ${result.total} sincronizado(s) correctamente`
      setSyncProgress({
        actual: result.total, total: result.total,
        ok: result.ok, errores: result.errores,
        mensaje
  ...
  const funnel = useMemo(() => {
    ...
    // Puente devengado → percibido. Sin él, "Vendido vs Cobrado" compara la venta
    // facturada contra la plata recaudada, que son cosas distintas: el abono aplicado
    // se cobró en un período anterior y la venta a crédito se cobra después. Esa
    // diferencia aparecía rotulada como "perdido en cobro" cuando era desfase contable.
    const puente = embudo ? [
      { l: 'Abonos aplicados', v: embudo.p_abono_aplicado, tip: 'Anticipos del cliente usados como pago: la plata entró en un período anterior' },
      { l: 'Venta a crédito (CxC)', v: embudo.p_credito_cxc, tip: 'Queda por cobrar' },
      { l: 'Excepciones administrativas', v: embudo.p_excepciones, tip: 'Autorizaciones especiales' },
      { l: 'Anticipos recibidos', v: embudo.p_anticipos_recibidos, tip: 'Plata que entró hoy por ventas futuras: es caja, no es venta' },
      { l: 'Devoluciones en dinero', v: embudo.p_devoluciones_dinero, tip: 'Salió del cajón sin documento de venta' },
      { l: 'Cobros de otros períodos', v: embudo.p_cobros_otros_periodos, tip: 'Facturas a crédito de días anteriores cobradas en este período' },
    ].filter(x => Math.round(Number(x.v ?? 0)) !== 0) : []
    ...
    return {
      ...
      cajaEsperada: embudo?.caja_esperada ?? null,
      cajaDeclarada: embudo?.caja_declarada ?? null,
      puente,
      fugaRecaudacion: embudo ? embudo.caja_declarada - embudo.caja_esperada : null,
      sinCorroborar: embudo?.n_sin_corroborar ?? 0,
    }
  }, [cierres, resumen, depositosBancarios, ventasBsaleCache, embudo])
  ...
  const serieDia = useMemo(() => {
    ...
    cierres.forEach(c => {
      const d = c.fecha
      if (!d) return
      // En modo anual agrupar por mes; en modo mes por día
      const key = modoAnual ? d.slice(0, 7) : d
      map[key] = (map[key] ?? 0) + Number(c.total_declarado ?? 0)
    })
    const meses3 = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic']
    return Object.entries(map).sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => ({
      l: modoAnual ? meses3[Number(k.slice(5, 7)) - 1] : k.slice(8, 10),
      v, fecha: k
    }))
  }, [cierres, modoAnual])
  ...
  const serieDiaPrev = useMemo(() => {
    ...
    const meses3 = ['Ene', 'Feb', 'Mar', 'Abr', 'May', 'Jun', 'Jul', 'Ago', 'Sep', 'Oct', 'Nov', 'Dic']
    return Object.entries(map).sort(([a], [b]) => a.localeCompare(b)).map(([k, v]) => ({
      l: modoAnual ? meses3[Number(k.slice(5, 7)) - 1] : k.slice(8, 10), v
    }))
  }, [cierresMesPrev, modoAnual])
  ...
  const alertas = useMemo(() => {
    ...
    if (funnel.fugaRecaudacion != null) {
      if (Math.abs(funnel.fugaRecaudacion) > (funnel.cajaEsperada || 1) * 0.005)
        arr.push({ sev: funnel.fugaRecaudacion < 0 ? 'alta' : 'media', titulo: 'Fuga de recaudación', desc: funnel.fugaRecaudacion < 0 ? 'La caja declarada es menor que lo registrado en BSALE' : 'La caja declarada supera lo registrado en BSALE', valor: fmtCompact(Math.abs(funnel.fugaRecaudacion)), ic: <AlertCircle size={16} /> })
    } else if (brechaVsBsale > 0) {
      arr.push({ sev: 'media', titulo: 'Brecha BSALE vs Cierres', desc: 'Diferencia entre facturado y declarado', valor: fmtCompact(brechaVsBsale), ic: <AlertCircle size={16} /> })
    }
      ...
      <div style={{
        ...
        <div>
          ...
          <div style={{ fontSize: 18, fontWeight: 800, marginTop: 2 }}>
            {modoAnual ? `Año ${anio}` : `${meses[mes - 1]} ${anio}`} · {nombreSucursal}
          </div>
        </div>
        ...
        <select value={mes} onChange={e => setMes(Number(e.target.value))}
          style={{ background: 'rgba(255,255,255,0.15)', border: 'none', color: '#fff', padding: '7px 10px', borderRadius: 7, fontSize: 12, fontWeight: 600, cursor: 'pointer' }}>
          <option value={0} style={{ color: C.text, fontWeight: 800 }}>📅 Año completo</option>
        ...
        <button onClick={() => handleSync(true)} disabled={syncing || loading} title="Sincronizar mes día por día (más lento pero confiable con reintentos)"
          style={{
        ...
        <button onClick={handleSyncRapido} disabled={syncing || loading} title="Sincronizar mes en 1 llamada (~30-60 seg, sin reintentos)"
          style={{
            background: 'rgba(255,213,0,0.25)', border: '1px solid rgba(255,213,0,0.5)', color: '#fff',
            padding: '7px 10px', borderRadius: 7, cursor: syncing ? 'wait' : 'pointer',
          ...
          <div style={{ display: 'grid', gridTemplateColumns: '1.4fr 1fr', gap: 12 }}>
            <div style={cardSt}>
              ...
              <div style={{ marginTop: 14, padding: '10px 14px', background: '#F9FAFB', borderRadius: 8, fontSize: 11, color: C.textTer, lineHeight: 1.5 }}>
                <strong style={{ color: C.text }}>Lectura:</strong>{' '}
                {funnel.cajaEsperada != null ? (
                  <>De cada $100 que la caja <strong>debía</strong> recaudar, los cajeros declararon $<strong>{pct(funnel.cajaDeclarada, funnel.cajaEsperada)}</strong>, llegaron al banco $<strong>{pct(funnel.depositado, funnel.cajaEsperada)}</strong> y están conciliados $<strong>{pct(funnel.conciliado, funnel.cajaEsperada)}</strong>. La diferencia entre lo facturado y lo que la caja debía recaudar está explicada en el puente y no es pérdida.</>
                ) : (
                  <>De cada $100 facturado en BSALE, $<strong>{pct(funnel.cobrado, funnel.vendido)}</strong> se cobraron, $<strong>{pct(funnel.depositado, funnel.vendido)}</strong> llegaron al banco, y $<strong>{pct(funnel.conciliado, funnel.vendido)}</strong> están conciliados con su origen.</>
                )}
              </div>
            </div>
      ...
      {!loading && vista === 'cashflow' && (
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, animation: 'fadeIn 0.3s ease' }}>
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 12 }}>
            {funnel.fugaRecaudacion != null ? (
              <KpiHero
                label="Fuga de recaudación"
                valor={fmt(funnel.fugaRecaudacion)}
                sub={funnel.fugaRecaudacion < 0
                  ? `Falta respecto de lo que BSALE registró · ${pct1(Math.abs(funnel.fugaRecaudacion), funnel.cajaEsperada)}%`
                  : funnel.fugaRecaudacion > 0 ? 'Declarado por sobre lo registrado en BSALE' : 'La caja declarada calza con BSALE'}
                color={Math.abs(funnel.fugaRecaudacion) > (funnel.cajaEsperada || 1) * 0.005 ? C.rojo : C.verde} />
            ) : (
              <KpiHero label="Brecha venta → cobro" valor={fmt(funnel.vendido - funnel.cobrado)} sub="Sin ledger de pagos para el período" color={C.gris} />
            )}
            <KpiHero
              label="Brecha caja → banco"
              valor={fmt((funnel.cajaDeclarada ?? funnel.cobrado) - funnel.depositado)}
              sub="Pendiente de depósito o liquidación"
              color={((funnel.cajaDeclarada ?? funnel.cobrado) - funnel.depositado) > (funnel.cajaDeclarada ?? funnel.cobrado) * 0.1 ? C.rojo : C.naranja} />
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 14, animation: 'fadeIn 0.3s ease' }}>
          ...
          <div style={cardSt}>
            <div style={{ fontSize: 13, fontWeight: 700, color: C.text, marginBottom: 14 }}>
              {modoAnual ? `Cuadratura por mes — Año ${anio}` : `Calendario — ${meses[mes - 1]} ${anio}`}
            </div>
            {modoAnual ? (
              <ResumenPorMes cierres={cierres} anio={anio} />
            ) : (
