...
export function AnalisisTab({ usuario }) {
  ...
  const [subTab,                setSubTab]                = useState('resumen') // resumen | analisis
  ...
  // Cargar análisis mensual desde view
  useEffect(() => {
    if (subTab !== 'analisis') return
    setLoadingAnalisis(true)
    supabase.from('v_analisis_medios_pago').select('*')
      .then(({ data }) => setAnalisisMensual(data || []))
  ...
  return (
    <div style={{ fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif" }}>
      ...
      <div style={{ display: 'flex', gap: 2, marginBottom: 16, borderBottom: '1px solid rgba(0,0,0,0.06)' }}>
        {[
          { k: 'resumen',  l: '📊 Resumen cierres' },
          { k: 'analisis', l: '🔁 Declarado · Corroborado · Depositado' },
        ].map(t => (
          <button key={t.k} onClick={() => setSubTab(t.k)} style={{
            padding: '8px 16px', fontSize: 13, fontWeight: 600,
            background: 'none', border: 'none', cursor: 'pointer',
            color: subTab === t.k ? AZUL : GRIS,
            borderBottom: subTab === t.k ? `2px solid ${AZUL}` : '2px solid transparent',
            whiteSpace: 'nowrap',
          }}>{t.l}</button>
        ))}
      </div>

      {/* ── SUB-TAB: ANÁLISIS MENSUAL ── */}
      {subTab === 'analisis' && (() => {
        const MEDIOS_INFO = {
          efectivo:       { l: '💵 Efectivo',        c: VERDE    },
          credito_getnet: { l: '💳 Crédito Getnet',  c: AZUL     },
          debito_getnet:  { l: '💳 Débito/Prepago',  c: '#2E6DA4' },
          transferencia:  { l: '🏦 Transferencia',   c: '#0891B2' },
          otros:          { l: '📋 Otros medios',    c: GRIS     },
        }

        // Filtros del análisis mensual
        const mesesDisp = [...new Set(analisisMensual.map(r => r.mes?.slice(0, 7)).filter(Boolean))].sort().reverse()
        const [aMes, setAMes] = [filMes, setFilMes]
        const [aSuc, setASuc] = [filSuc, setFilSuc]

        const filtrado = analisisMensual.filter(r => {
          const enMes = aMes ? r.mes?.startsWith(aMes) : true
          const enSuc = aSuc !== 'todas' ? r.sucursal_id === aSuc : true
          return enMes && enSuc
        })
        ...
        const totales = {}
        filtrado.forEach(r => {
          if (!totales[r.medio]) totales[r.medio] = { decl: 0, corrob: 0, dep: 0 }
          totales[r.medio].decl   += parseFloat(r.declarado   || 0)
          totales[r.medio].corrob += parseFloat(r.corroborado || 0)
          totales[r.medio].dep    += parseFloat(r.depositado  || 0)
        })

        // Detalle por mes+sucursal
        const porMes = {}
        filtrado.forEach(r => {
          const k = `${r.mes?.slice(0,7)}__${r.sucursal_id}`
          if (!porMes[k]) porMes[k] = { mes: r.mes?.slice(0,7), suc: r.sucursal_nombre || r.sucursal_id, rows: [] }
          porMes[k].rows.push(r)
        })
        const detalles = Object.values(porMes).sort((a, b) => b.mes?.localeCompare(a.mes))
        ...
        const difColor = d => Math.abs(d) < 5000 ? VERDE : Math.abs(d) < 50000 ? NARANJA : ROJO

        return (
          <div>
            {/* Filtros */}
            <div style={{ display: 'flex', gap: 10, marginBottom: 16, flexWrap: 'wrap', alignItems: 'center' }}>
              <select value={aMes} onChange={e => setAMes(e.target.value)} style={selSt}>
                <option value="">Todos los meses</option>
                {mesesDisp.map(m => <option key={m} value={m}>{m}</option>)}
              </select>
              <select value={aSuc} onChange={e => setASuc(e.target.value)} style={selSt}>
            ...
            {loadingAnalisis
              ? <div style={{ textAlign: 'center', padding: 40, color: GRIS }}>Cargando...</div>
              : (
              <>
                {/* Resumen por medio */}
                    ...
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      ...
                      <tbody>
                        {Object.entries(totales).length === 0
                          ? <tr><td colSpan={6} style={{ ...TD, textAlign: 'center', padding: '24px 0', color: GRIS }}>Sin datos. Verifica que la view v_analisis_medios_pago esté creada en Supabase.</td></tr>
                          : Object.entries(totales).map(([medio, v]) => {
                            const info = MEDIOS_INFO[medio] || { l: medio, c: GRIS }
                            const dC = v.corrob - v.decl
                            const dD = v.dep - v.corrob
                            return (
                              <tr key={medio} style={{ borderTop: '1px solid #F9FAFB' }}>
                                <td style={{ ...TD, fontWeight: 600, color: info.c }}>{info.l}</td>
                                <td style={{ ...TD, textAlign: 'right' }}>{fmt(v.decl)}</td>
                                <td style={{ ...TD, textAlign: 'right' }}>{fmt(v.corrob)}</td>
                                <td style={{ ...TD, textAlign: 'right', fontWeight: 700, color: difColor(dC) }}>{dC >= 0 ? '+' : ''}{fmt(dC)}</td>
                                <td style={{ ...TD, textAlign: 'right' }}>{v.dep > 0 ? fmt(v.dep) : <span style={{ color: GRIS }}>—</span>}</td>
                                <td style={{ ...TD, textAlign: 'right', fontWeight: 700, color: v.dep > 0 ? difColor(dD) : GRIS }}>{v.dep > 0 ? (dD >= 0 ? '+' : '') + fmt(dD) : '—'}</td>
                              </tr>
                            )
                          })
                        }
                ...
                <div style={{ ...cardSt, padding: 0, overflow: 'hidden' }}>
                  <div style={{ padding: '12px 20px', borderBottom: '1px solid #F3F4F6', fontSize: 13, fontWeight: 700, color: AZUL }}>
                    Detalle mensual por sucursal y medio
                  </div>
                  <div style={{ overflowX: 'auto' }}>
                    <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                      <thead>
                      ...
                      <tbody>
                        {detalles.length === 0
                          ? <tr><td colSpan={8} style={{ ...TD, textAlign: 'center', padding: '24px 0', color: GRIS }}>Sin datos</td></tr>
                          : detalles.flatMap(fila =>
                            fila.rows.map(r => {
                              const info = MEDIOS_INFO[r.medio] || { l: r.medio, c: GRIS }
                              const decl   = parseFloat(r.declarado   || 0)
                              const corrob = parseFloat(r.corroborado || 0)
                              const dep    = parseFloat(r.depositado  || 0)
                              const dC = corrob - decl
                              const dD = dep - corrob
                              return (
                                <tr key={`${r.mes}-${r.sucursal_id}-${r.medio}`} style={{ borderTop: '1px solid #F9FAFB' }}>
                                  <td style={TD}>{fila.mes}</td>
                                  <td style={TD}>{fila.suc}</td>
                                  <td style={{ ...TD, fontWeight: 600, color: info.c }}>{info.l}</td>
                                  <td style={{ ...TD, textAlign: 'right' }}>{fmt(decl)}</td>
                                  <td style={{ ...TD, textAlign: 'right' }}>{fmt(corrob)}</td>
                                  <td style={{ ...TD, textAlign: 'right', fontWeight: 700, color: difColor(dC) }}>{dC >= 0 ? '+' : ''}{fmt(dC)}</td>
                                  <td style={{ ...TD, textAlign: 'right' }}>{dep > 0 ? fmt(dep) : <span style={{ color: GRIS }}>—</span>}</td>
                                  <td style={{ ...TD, textAlign: 'right', fontWeight: 700, color: dep > 0 ? difColor(dD) : GRIS }}>{dep > 0 ? (dD >= 0 ? '+' : '') + fmt(dD) : '—'}</td>
                                </tr>
                              )
                            })
                          )
      ...
      {/* ── SUB-TAB: RESUMEN CIERRES (contenido original) ── */}
      {subTab === 'resumen' && <>
