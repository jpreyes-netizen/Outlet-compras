  ...
  async function parsearArchivo(file) {
    ...
    try {
      ...
      for (let i = 0; i < Math.min(5, raw.length); i++) {
        if (raw[i].some(c => /fecha/i.test(String(c)))) { headerRow = i; break }
      }
      ...
      const toNum = v => { const n = parseFloat(String(v).replace(/[^0-9.-]/g,'')); return isNaN(n)?0:Math.round(n) }
      const toFecha = v => {
        if (!v) return null
        // Número Excel serial
        if (!isNaN(Number(v))) {
      ...
      for (let i = headerRow+1; i < raw.length; i++) {
        ...
        const moneda = iMoneda >= 0 ? toNum(row[iMoneda]) : 0
...
function SeccionMedio({ config, sucursalEf, desde, hasta, puedeEscribir, sucursales }) {
  ...
  const matchDiario = useMemo(() => {
    ...
    for (const r of recaudacion) {
      ...
      mapRec[key] = (mapRec[key] ?? 0) + Number(r[config.campoRec] ?? 0)
    }
    const mapDep = {}
    ...
    return Array.from(allKeys).map(key => {
      ...
      const rec = mapRec[key] ?? 0
      const dep = mapDep[key] ?? 0
      return { fecha, sucursal_id: sucId, rec, dep, diff: dep - rec }
    }).sort((a, b) => b.fecha.localeCompare(a.fecha))
  }, [recaudacion, abonos])

  const pendienteDepositar = matchDiario.filter(r => r.rec > 0 && r.dep === 0).reduce((s, r) => s + r.rec, 0)
          ...
          {verMatch && (
            <div style={{ overflowX:'auto' }}>
              <table style={{ width:'100%', borderCollapse:'collapse', fontSize:13 }}>
                ...
                <tbody>
                  {matchDiario.map((r, i) => {
                    const esPend = r.rec > 0 && r.dep === 0
                    const esOk   = !esPend && Math.abs(r.diff) < 5000
                    const estado = esPend?'Sin depositar':esOk?'Cuadra':r.diff<0?'Falta':'Excedente'
                    const bg     = esPend?'#FEF3C7':esOk?'#DCFCE7':r.diff<0?'#FEE2E2':'#EFF6FF'
                    const clr    = esPend?'#D97706':esOk?'#16A34A':r.diff<0?'#DC2626':'#1E40AF'
                    return (
                      <tr key={i} style={{ borderTop:'1px solid #F3F4F6', background: esPend?'#FFFBEB':'transparent' }}
                        onMouseEnter={e=>e.currentTarget.style.background='#F9FAFB'}
                        onMouseLeave={e=>e.currentTarget.style.background=esPend?'#FFFBEB':'transparent'}>
                        <td style={TD}>{r.fecha}</td>
                        <td style={TD}>{sucMap[r.sucursal_id] ?? r.sucursal_id ?? '—'}</td>
                        <td style={{ ...TD, textAlign:'right' }}>{r.rec>0?fmt(r.rec):<span style={{color:'#D1D5DB'}}>—</span>}</td>
...
export function DepositosAbonosTab({ usuario }) {
  ...
  const TABS = [
    ...
    { k: 'importar_ef',      l: '📤 Importar Efectivo' },
    { k: 'debito',           l: 'Débito Getnet'      },
    { k: 'credito',          l: 'Crédito Getnet'     },
    { k: 'importar_getnet',  l: '📤 Importar Getnet'  },
    { k: 'webpay',           l: 'Webpay'             },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
      {/* Filtros */}
      {tabActivo !== 'importar_getnet' && tabActivo !== 'importar_ef' && (
        <div style={cardSt}>
      ...
      {/* Contenido */}
      {tabActivo === 'importar_getnet' && <ImportadorGetnet />}
      {tabActivo === 'importar_ef'     && <ImportadorEfectivo sucursales={sucursales} />}
      {tabActivo !== 'importar_getnet' && tabActivo !== 'importar_ef' && configs[tabActivo] && (
        <SeccionMedio
          key={tabActivo}
