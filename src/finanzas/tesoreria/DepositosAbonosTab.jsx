...
async function fetchGetnetAgrupado({ desde, hasta, sucursal_id, tipo_pago }) {
  let q = supabase.from('getnet_detalle_abonos')
    .select('fecha_venta, sucursal_id, valor_cuota, monto_abono')
    .gte('fecha_venta', desde).lte('fecha_venta', hasta)
    .eq('tipo_normalizado', tipo_pago)
    .not('tipo_movimiento', 'ilike', '%Devol%')
    .limit(20000)
  if (sucursal_id) q = q.eq('sucursal_id', sucursal_id)
  const { data, error } = await q
  if (error) throw error
  ...
  for (const r of data ?? []) {
    ...
    if (!map[key]) map[key] = { fecha: r.fecha_venta, sucursal_id: r.sucursal_id, total_abono: 0, n: 0 }
    map[key].total_abono += Number(r.valor_cuota ?? 0)  // bruto — comisión se resta en etapa Recaudado
    map[key].n++
  }
  ...
  useEffect(() => {
    ...
    Promise.all([
      ...
      supabase.from('getnet_detalle_abonos')
        .select('fecha_venta, sucursal_id, valor_cuota, tipo_normalizado')
        .gte('fecha_venta', desde).lte('fecha_venta', hasta)
        .not('tipo_movimiento', 'ilike', '%Devol%')
        .limit(20000),
      ...
      for (const g of getnet ?? []) {
        ...
        // Usar valor_cuota (bruto) — la comisión se resta en etapa Recaudado
      ...
      const result = Array.from(sucIds).map(sucId => {
        ...
        return { sucId, ...c, debito_neto: g.debito_bruto, credito_neto: g.credito_bruto, corrTotal, getTotal, brecha: getTotal - corrTotal }
      })
      setRows(result)
