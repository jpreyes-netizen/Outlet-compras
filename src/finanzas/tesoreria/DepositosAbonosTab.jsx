...
function MontoEditable({ valor, onGuardar }) {
  const [editando, setEditando] = useState(false)
  const [text, setText] = useState(formatCLP(valor))
  const [guardando, setGuardando] = useState(false)
  useEffect(() => { if (!editando) setText(formatCLP(valor)) }, [valor, editando])

  async function confirmar() {
    const nuevo = parseCLP(text)
    setEditando(false)
    if (nuevo === valor || isNaN(nuevo)) { setText(formatCLP(valor)); return }
    setGuardando(true)
  ...
  if (!editando) {
    return (
      <span onClick={() => { setEditando(true); setText(String(valor)) }}
        style={{ cursor: 'pointer', borderBottom: '1px dashed #9CA3AF', paddingBottom: 1 }}
        title="Click para editar">
        {guardando ? '...' : formatCLP(valor)}
      </span>
    )
  }
  return (
    <input
      type="text" inputMode="numeric" autoFocus value={text}
      onChange={e => setText(e.target.value)}
      onBlur={confirmar}
      onKeyDown={e => { if (e.key === 'Enter') e.target.blur(); if (e.key === 'Escape') { setEditando(false); setText(formatCLP(valor)) } }}
      style={{ width: 120, textAlign: 'right', fontWeight: 600, fontSize: 13, padding: '4px 8px', border: '1px solid #1F4E79', borderRadius: 4, background: '#fff' }}
    />
  )
}
                ...
                return (
                  <tr key={r.id ?? i} style={{ borderTop: '1px solid #F3F4F6' }}>
                    ...
                    <td style={{ ...TD, textAlign: 'right', fontWeight: 600 }}>
                      {puedeEscribir && r.id ? (
                        <MontoEditable
                          valor={Number(r[montoKey] ?? 0)}
                          onGuardar={async (nuevoMonto) => {
                            try {
                              const tabla = tipo
                              const { error } = await supabase.from(tabla).update({ [montoKey]: nuevoMonto }).eq('id', r.id)
                              if (error) throw error
