...
import { fetchCierres, fetchKpisMes, fetchSucursales, sincronizarBsaleMes } from './api'
...
export function CuadraturaTab({ usuario }) {
  ...
  const [syncProgress, setSyncProgress] = useState(null) // { actual, total, ok, errores }
  ...
  async function handleSync() {
    ...
    try {
      ...
      if (result.total === 0) {
        toast.success('Todos los cierres ya tienen datos BSALE')
      } else {
        toast.success(`Sincronizados ${result.ok} de ${result.total} días${result.errores > 0 ? ` (${result.errores} errores)` : ''}`)
        setLoadingCierres(true)
        setLoadingKpis(true)
        fetchCierres({ sucursal_id: sucursalEf, fecha_desde: desde, fecha_hasta: hasta })
          .then(setCierres).catch(e => toast.error(e.message)).finally(() => setLoadingCierres(false))
      ...
      <div style={cardSt}>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(130px, 1fr))', gap: 12, alignItems: 'flex-end' }}>
          ...
          <div style={{ display: 'flex', alignItems: 'flex-end', gap: 8 }}>
            <button onClick={exportarCSV} disabled={cierres.length === 0} style={{ ...btnOutlineSt, flex: 1, justifyContent: 'center', opacity: cierres.length === 0 ? 0.5 : 1 }}>
              <Download size={13} /> CSV
            </button>
            <button onClick={handleSync} disabled={syncing} style={{ ...btnOutlineSt, flex: 1, justifyContent: 'center', gap: 6, borderColor: '#1D4ED8', color: '#1D4ED8', opacity: syncing ? 0.7 : 1 }}>
              {syncing ? <Loader2 size={13} style={{ animation: 'spin 1s linear infinite' }} /> : '↻'}
              {syncing && syncProgress ? `${syncProgress.actual}/${syncProgress.total}` : 'Sync BSALE'}
            </button>
          </div>
        </div>
        {syncing && syncProgress && (
          <div style={{ marginTop: 10 }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', fontSize: 11, color: '#6B7280', marginBottom: 4 }}>
              <span>Sincronizando con BSALE…</span>
              <span>{syncProgress.actual} de {syncProgress.total} días · {syncProgress.ok} ok{syncProgress.errores > 0 ? ` · ${syncProgress.errores} errores` : ''}</span>
            </div>
            <div style={{ height: 4, background: '#E5E7EB', borderRadius: 2, overflow: 'hidden' }}>
              <div style={{ height: '100%', background: '#1D4ED8', borderRadius: 2, width: `${Math.round((syncProgress.actual / syncProgress.total) * 100)}%`, transition: 'width 0.2s' }} />
            </div>
          </div>
        )}
      </div>
