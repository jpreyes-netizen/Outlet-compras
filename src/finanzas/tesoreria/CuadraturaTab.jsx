          ...
          <div style={{ background: '#F9FAFB', borderRadius: 8, padding: '10px 14px' }}>
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {analisis.declarados.map(c => (
                <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#6B7280' }}>
                  <span>{c.fecha} · {c.sucursal_nombre ?? '—'} · {c.vendedor_nombre ?? c.bsale_vendedor_id ?? 'Sin nombre'}</span>
                  <span style={{ fontWeight: 600, color: '#D97706' }}>{fmt(c.total_declarado)}</span>
                </div>
              ))}
            </div>
          </div>
        ...
        {analisis.descuadres.length > 0 && (
          <div style={{ background: '#FEF2F2', borderRadius: 8, padding: '10px 14px', border: '1px solid #FECACA' }}>
            <div style={{ fontSize: 11, fontWeight: 600, color: '#DC2626', marginBottom: 6 }}>
              Cierres con descuadre real
            </div>
            <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
              {analisis.descuadres.map(c => (
                <div key={c.id} style={{ display: 'flex', justifyContent: 'space-between', fontSize: 12, color: '#6B7280' }}>
                  <span>{c.fecha} · {c.sucursal_nombre ?? '—'} · {c.vendedor_nombre ?? c.bsale_vendedor_id ?? 'Sin nombre'}</span>
                  <span style={{ fontWeight: 600, color: '#DC2626' }}>{fmt(Number(c.diferencia ?? 0))}</span>
                </div>
              ))}
            </div>
          </div>
