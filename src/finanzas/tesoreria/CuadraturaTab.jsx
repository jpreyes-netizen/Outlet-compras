...
import { CierreCuadraturaTab } from './CierreCuadraturaTab'
...
const Linea = ({ l, v, neg, pos, bold, sub }) => (
  <div style={{ display: 'flex', justifyContent: 'space-between', gap: 12, padding: '6px 0', borderBottom: bold ? undefined : '1px solid #F3F4F6', alignItems: 'baseline' }}>
    ...
    <div style={{ fontSize: bold ? 16 : 13, fontWeight: 700, fontFamily: 'ui-monospace, monospace', color: neg ? ROJO : pos ? VERDE : bold ? NAVY : INK }}>{neg ? '−' : pos ? '+' : ''}{fmt(Math.abs(v))}</div>
  </div>
)
...
export function CuadraturaTab({ usuario }) {
  ...
  const vb = (k, l) => (
    <button key={k} onClick={() => setVista(k)} style={{ fontSize: 12, fontWeight: 600, padding: '5px 14px', borderRadius: 6, cursor: 'pointer',
      border: `1px solid ${vista === k ? NAVY : BORDE}`, background: vista === k ? NAVY : '#fff', color: vista === k ? '#fff' : INK }}>{l}</button>
  )
  const BarraVista = (
    <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
      {vb('bandeja', 'Qué revisar hoy')}
      {vb('cuadratura', 'Cuadratura del período')}
      {vb('cierre', 'Cierre diario y trazabilidad')}
      {vb('diagnostico', 'Diagnóstico de causas')}
      {vb('explorador', 'Explorador')}
    </div>
  )

  if (vista === 'explorador') return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>{BarraVista}<ExploradorTab /></div>
  )
  if (vista === 'diagnostico') return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>{BarraVista}<DiagnosticoTab /></div>
  )
  if (vista === 'cierre') return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>{BarraVista}<CierreCuadraturaTab /></div>
  )
  if (vista === 'bandeja') return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>{BarraVista}<BandejaTab /></div>
  )
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <Seccion paso="1" titulo="Qué se vendió y cuánto tenía que llegar a la caja"
        ...
        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 24 }}>
          ...
          <div>
            <Linea l="Pagado con saldo a favor o nota de crédito" sub="el cliente usó un crédito que ya tenía: hoy no entra plata" v={p1.pagado_con_saldo} neg />
            <Linea l="Venta a crédito (se cobra después)" v={p1.venta_a_credito} neg />
            {Number(p1.credito_generado || 0) !== 0 && (
              <Linea l="Crédito nacido de notas de crédito" sub="la NC rebajó la venta, pero esa plata ya había entrado" v={p1.credito_generado} pos />
            )}
            {Number(p1.abonos_recibidos || 0) !== 0 && (
              <Linea l="Abonos recibidos sin documento" sub="el cliente pagó por adelantado: es caja, no es venta" v={p1.abonos_recibidos} pos />
            )}
            {Number(p1.devoluciones_dinero || 0) !== 0 && (
              <Linea l="Devoluciones en dinero" sub="salió efectivo del cajón sin venta que lo respalde" v={p1.devoluciones_dinero} neg />
            )}
            {Number(p1.calce_periodos || 0) !== 0 && (
              <Linea l="Cobros de documentos de otros períodos"
                sub="facturas a crédito cobradas ahora y ventas de este mes pagadas después"
                v={Math.abs(p1.calce_periodos)} neg={Number(p1.calce_periodos) < 0} pos={Number(p1.calce_periodos) > 0} />
            )}
            <div style={{ marginTop: 8 }}><Linea l="Debió entrar a la caja" v={p1.debio_entrar} bold /></div>
          </div>
        </div>
        <div style={{ fontSize: 11, color: SLATE, marginTop: 10, lineHeight: 1.5, borderTop: `1px solid ${BORDE}`, paddingTop: 8 }}>
          Las líneas de la derecha explican por qué lo vendido y lo recaudado no son la misma cifra, y suman
          exactamente al total. No son fugas: un abono aplicado se cobró en otro período, y una nota de crédito
          rebaja la venta sin que la plata haya salido. La fuga real aparece en el paso 2, cuando un peso que
          debió entrar no se puede probar contra Getnet, la cartola o un depósito.
          El detalle cliente a cliente de los abonos, créditos y NC de estas líneas vive en el tab
          <b> Abonos de clientes</b>, cuyo panel de Resumen verifica al peso la consistencia con estas mismas cifras.
        </div>
      </Seccion>
