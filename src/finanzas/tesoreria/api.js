...
export async function corroborarCierre(p) {
  ...
  // 5. Generar/actualizar línea de depósito efectivo automática
  // Consolida el efectivo de todos los cierres corroborados del mismo día×sucursal
  await upsertDepositoEfectivoAutoCierre({
    fecha: fecha ?? cierre.fecha,
    sucursal_id: sucursal_id ?? cierre.sucursal_id,
  }).catch(() => {}) // no bloquear si columnas no existen aún
