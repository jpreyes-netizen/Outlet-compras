...
export async function fetchUmbrales() {
  const { data, error } = await supabase.from('parametros_sistema').select('clave, valor').in('clave', ['tesoreria_umbral_cuadra', 'tesoreria_umbral_tolerable', 'tesoreria_materialidad_cobranza'])
  ...
  return {
    ...
    // Bajo este monto la diferencia se reconoce como diferencia de caja del período
    // y no genera cuenta por cobrar contra el trabajador.
    materialidad: map.get('tesoreria_materialidad_cobranza') ?? map.get('tesoreria_umbral_cuadra') ?? UMBRALES_DEFAULT.cuadra,
  }
}
...
// ── F2 · Base percibida (caja física) ───────────────────────────────────────
// El arqueo compara contra el ledger de pagos BSALE (bsale_pagos), no contra
// documentos. Cut-off en config_sistema.teso_caja_fisica_desde; antes rige la
// base venta contable. Ver F2_Control_Caja_Fisica.md.

const CFG_TESO_DEFAULT = { cajaFisicaDesde: '2026-09-21', tolerancia: 1000 }
let _cfgTesoCache = null, _cfgTesoAt = 0

export async function fetchConfigTesoreria() {
  if (_cfgTesoCache && Date.now() - _cfgTesoAt < 5 * 60 * 1000) return _cfgTesoCache
  try {
    const { data, error } = await supabase.from('config_sistema').select('clave, valor')
      .in('clave', ['teso_caja_fisica_desde', 'teso_tolerancia_brecha'])
    if (error) return CFG_TESO_DEFAULT
    const map = Object.fromEntries((data ?? []).map(r => [r.clave, r.valor]))
    const tol = Number(map.teso_tolerancia_brecha)
    _cfgTesoCache = {
      cajaFisicaDesde: /^\d{4}-\d{2}-\d{2}$/.test(map.teso_caja_fisica_desde ?? '') ? map.teso_caja_fisica_desde : CFG_TESO_DEFAULT.cajaFisicaDesde,
      tolerancia: Number.isFinite(tol) ? tol : CFG_TESO_DEFAULT.tolerancia,
    }
    _cfgTesoAt = Date.now()
    return _cfgTesoCache
  } catch { return CFG_TESO_DEFAULT }
}

// Trae los últimos pagos de BSALE al ledger (incremental, 1–3 llamadas API, ~2 s).
// Se invoca al abrir el cierre del día para que el esperado esté fresco.
// Nunca lanza: si falla, el ledger queda como lo dejó el cron (cada 30 min).
// cubrirDesde: fecha mínima que el sync debe re-leer (v6.2). Permite reparar un día
// pasado desde la pantalla, no solo hoy.
export async function syncPagosIncremental(maxPaginas = 30, cubrirDesde = null) {
  try {
    const { data: { session } } = await supabase.auth.getSession()
    const headers = { 'Content-Type': 'application/json' }
    if (session?.access_token) headers['Authorization'] = `Bearer ${session.access_token}`
    const body = { max_paginas: maxPaginas }
    if (cubrirDesde) body.cubrir_desde = cubrirDesde
    const res = await fetch(
      `${import.meta.env.VITE_SUPABASE_URL}/functions/v1/bsale-sync-pagos`,
      { method: 'POST', headers, body: JSON.stringify(body) }
    )
    if (!res.ok) return null
    return await res.json()
  } catch (e) {
    console.warn('[syncPagosIncremental]', e.message)
    return null
  }
}

// Caja esperada de un cajero (fecha × sucursal × recaudador BSALE) desde v_cierre_esperado
export async function fetchCierreEsperado(fecha, sucursal_id, bsale_user_id) {
  if (!fecha || !sucursal_id || bsale_user_id == null) return null
  const filas = await fetchCierresEsperadosDia(fecha, sucursal_id)
  return filas.find(f => String(f.bsale_user_id) === String(bsale_user_id)) ?? null
}

// Todos los cajeros con actividad de caja del día × sucursal.
// Vía RPC: la función filtra ANTES de agregar (44 ms). Consultar la vista directamente
// materializa las CTEs y recorre el ledger completo (~870 ms y creciendo).
export async function fetchCierresEsperadosDia(fecha, sucursal_id) {
  if (!fecha || !sucursal_id) return []
  const { data, error } = await supabase.rpc('fn_cierre_esperado', {
    p_fecha: fecha, p_sucursal: normalizarSucursalId(sucursal_id),
  })
  if (error) throw error
  return data ?? []
}

// Retiros de efectivo intra-día (traslado a caja fuerte / depósito). El detalle vive en
// cierres_retiros; cierres_caja.retiros_efectivo lo mantiene un trigger — nunca escribirlo.
export async function fetchRetiros(cierre_id) {
  if (!cierre_id) return []
  const { data, error } = await supabase.from('cierres_retiros').select('*').eq('cierre_id', cierre_id).order('hora')
  if (error) throw error
  return data ?? []
}

export async function crearRetiro({ cierre_id, fecha, sucursal_id, monto, hora, destino, comprobante, deposito_id, nota }) {
  const uid = (await supabase.auth.getSession()).data.session?.user.id
  if (!cierre_id) throw new Error('El retiro requiere un cierre guardado')
  const m = Number(monto)
  if (!Number.isFinite(m) || m <= 0) throw new Error('Monto de retiro inválido')
  const row = {
    cierre_id, fecha, sucursal_id: normalizarSucursalId(sucursal_id), monto: m,
    hora: hora ?? new Date().toISOString(),
    destino: ['caja_fuerte', 'deposito', 'otro'].includes(destino) ? destino : 'caja_fuerte',
    comprobante: comprobante?.trim() || null,
    deposito_id: deposito_id ?? null,
    registrado_por: uid ?? null,
    nota: nota?.trim() || null,
  }
  const { data, error } = await supabase.from('cierres_retiros').insert(row).select('*').single()
  if (error) throw error
  return data
}

export async function eliminarRetiro(id) {
  const { error } = await supabase.from('cierres_retiros').delete().eq('id', id)
  if (error) throw error
}

// Embudo de cuadratura con el puente devengado → percibido (v_cuadratura_embudo).
// Devuelve el agregado del período: la venta contable, las partidas que explican por qué
// la caja difiere de la venta, y la caja esperada resultante.
export async function fetchEmbudoCuadratura({ desde, hasta, sucursal_id }) {
  let q = supabase.from('v_cuadratura_embudo')
    .select('vendido, p_abono_aplicado, p_credito_cxc, p_excepciones, p_anticipos_recibidos, p_devoluciones_dinero, p_cobros_otros_periodos, caja_esperada, caja_declarada, efectivo_esperado, efectivo_declarado, efectivo_depositado, n_cierres, n_sin_corroborar')
    .gte('fecha', desde).lte('fecha', hasta)
  const suc = normalizarSucursalId(sucursal_id)
  if (suc) q = q.eq('sucursal_id', suc)
  const { data, error } = await q
  if (error) throw error
  const cero = {
    vendido: 0, p_abono_aplicado: 0, p_credito_cxc: 0, p_excepciones: 0, p_anticipos_recibidos: 0,
    p_devoluciones_dinero: 0, p_cobros_otros_periodos: 0, caja_esperada: 0, caja_declarada: 0,
    efectivo_esperado: 0, efectivo_declarado: 0, efectivo_depositado: 0, n_cierres: 0, n_sin_corroborar: 0,
  }
  return (data ?? []).reduce((acc, r) => {
    for (const k of Object.keys(cero)) acc[k] += Number(r[k] ?? 0)
    return acc
  }, { ...cero })
}

// ── Sub-tab Abonos ─────────────────────────────────────────────────────────
// Lee las vistas de control: libro mayor de abonos, trazabilidad por transacción,
// tipología de notas de crédito y panel mensual de conciliación.

function rangoMes(anio, mes) {
  if (!mes) return { desde: `${anio}-01-01`, hasta: `${anio}-12-31` }
  const fin = new Date(anio, mes, 0).getDate()
  const mm = String(mes).padStart(2, '0')
  return { desde: `${anio}-${mm}-01`, hasta: `${anio}-${mm}-${String(fin).padStart(2, '0')}` }
}

// Panel mensual: % conciliado, pendientes por tramo, NC sin explicar
export async function fetchControlConciliacion(anio, sucursal_id) {
  // mv_control_conciliacion: espejo materializado de v_control_conciliacion (27s de cálculo),
  // refrescado por cron 'abonos-control-refresh' 10 min después de cada sync de pagos.
  // Incluye columna refrescado_en con el momento del último cálculo.
  let q = supabase.from('mv_control_conciliacion').select('*')
    .gte('mes', `${anio}-01-01`).lte('mes', `${anio}-12-31`).order('mes')
  const suc = normalizarSucursalId(sucursal_id)
  if (suc) q = q.eq('sucursal_id', suc)
  const { data, error } = await q
  if (error) throw error
...
// Puente de venta: bruta − NC = neta, desde la MISMA fuente (ventas_bsale_dia)
// que usa el tab Cuadratura y análisis, para que ambos paneles digan lo mismo
// cuando dicen "venta"
export async function fetchPuenteVenta(anio, sucursal_id) {
  let q = supabase.from('v_tes_puente_venta').select('*')
    .gte('mes', `${anio}-01-01`).lte('mes', `${anio}-12-31`)
  const suc = normalizarSucursalId(sucursal_id)
  if (suc) q = q.eq('sucursal_id', suc)
  const { data, error } = await q
  if (error) throw error
...
// Chequeo de consistencia Abonos ↔ Cuadratura: compara las dos definiciones de
// imputaciones, entrada por NC, devoluciones y venta neta. Deben calzar al peso;
// si BSALE agrega un medio de pago nuevo, esto detecta el drift antes que nadie.
export async function fetchConsistenciaPaneles(desde, hasta) {
  const { data, error } = await supabase.rpc('fn_tes_consistencia', { p_desde: desde, p_hasta: hasta })
  if (error) throw error
  return data ?? []
}

// Puente Cierre del día ↔ cierre de caja de BSALE: descompone la caja del ledger
// en (docs del día en espejo / docs fuera del espejo) y expone abono aplicado,
// NC crédito y devoluciones para explicar por qué BSALE muestra otros números
export async function fetchPuenteBsale(fecha, sucursal_id) {
  const { data, error } = await supabase.rpc('fn_cierre_puente_bsale', { p_fecha: fecha, p_sucursal: sucursal_id })
  if (error) throw error
  return Array.isArray(data) ? (data[0] ?? null) : data
}

// Frescura del espejo de pagos: sin esto la pantalla no dice contra qué momento compara
export async function fetchFrescuraLedger() {
  const { data, error } = await supabase.from('bsale_sync_control')
    .select('recurso, ultima_corrida, estado').in('recurso', ['payments', 'returns', 'clients'])
  if (error) return []
  return data ?? []
}

// Buscar cliente por nombre o RUT para el panel de asignación
export async function buscarClienteBsale(texto) {
  const t = (texto ?? '').trim()
  if (t.length < 3) return []
  const esRut = /^[0-9.\-kK]+$/.test(t)
  const patron = esRut ? `%${t.replace(/[^0-9kK]/gi, '')}%` : `%${t}%`
  const { data, error } = await supabase.from('bsale_clientes')
    .select('bsale_id, nombre, rut, email')
    .or(esRut ? `rut.ilike.${patron}` : `nombre.ilike.${patron},empresa.ilike.${patron}`)
    .limit(20)
  if (error) return []
  return data ?? []
}

// Libro mayor con estado de conciliación por transacción
export async function fetchAbonosTrazabilidad({ anio, mes, sucursal_id, movimiento, estado }) {
  const { desde, hasta } = rangoMes(anio, mes)
  let q = supabase.from('v_abonos_trazabilidad').select('*')
    .gte('fecha', desde).lte('fecha', hasta).order('fecha', { ascending: false }).limit(3000)
  const suc = normalizarSucursalId(sucursal_id)
  if (suc) q = q.eq('sucursal_id', suc)
  if (movimiento) q = q.eq('movimiento', movimiento)
  if (estado) q = q.like('estado_conciliacion', `${estado}%`)
  const { data, error } = await q
  if (error) throw error
  return data ?? []
}

// Cola de trabajo: entradas en plata sin cliente, con candidatos
export async function fetchAbonosPorConciliar({ sucursal_id }) {
  let q = supabase.from('v_abonos_sin_identificar').select('*').order('monto', { ascending: false })
  const suc = normalizarSucursalId(sucursal_id)
  if (suc) q = q.eq('sucursal_id', suc)
  const { data, error } = await q
  if (error) throw error
...
// Candidatos de una entrada concreta (para el panel de asignación)
export async function fetchCandidatosAbono(pago_id) {
  const { data, error } = await supabase.from('v_abonos_match_sugerido').select('*')
    .eq('entrada_id', pago_id).order('dias')
  if (error) throw error
  return data ?? []
}

// Asignar cliente a una entrada en plata. Queda trazado: quién, cuándo y con qué confianza.
export async function asignarClienteAbono({ bsale_pago_id, cliente_id, cliente_nombre, confianza, nota }) {
  const uid = (await supabase.auth.getSession()).data.session?.user.id
  const { data, error } = await supabase.from('abonos_asignacion').upsert({
    bsale_pago_id, cliente_id: Number(cliente_id),
    cliente_nombre: cliente_nombre?.trim() || null,
    origen: 'manual',
    confianza: ['alta', 'media', 'baja'].includes(confianza) ? confianza : 'media',
    asignado_por: uid ?? null, asignado_at: new Date().toISOString(),
    nota: nota?.trim() || null,
  }, { onConflict: 'bsale_pago_id' }).select('*').single()
  if (error) throw error
  return data
}

export async function quitarAsignacionAbono(bsale_pago_id) {
  const { error } = await supabase.from('abonos_asignacion').delete().eq('bsale_pago_id', bsale_pago_id)
  if (error) throw error
}

// Cuenta corriente por cliente: el seguimiento que pidió Claudia
export async function fetchCuentaClientes({ estado, minSaldo, soloRevisar } = {}) {
  let q = supabase.from('v_abonos_cuenta_cliente').select('*').order('saldo_a_favor', { ascending: false }).limit(2000)
  if (estado) q = q.eq('estado', estado)
  if (soloRevisar) q = q.like('estado', '%revisar%')
  if (minSaldo != null) q = q.gte('saldo_a_favor', minSaldo)
  const { data, error } = await q
  if (error) throw error
  return data ?? []
}

// Ciclo de vida del crédito nacido de una NC: vigente, consumido, devuelto a la
// tarjeta o sin rastro. Cruza el libro contra el saldo oficial de BSALE.
export async function fetchCreditoDestino({ anio, mes, sucursal_id, destino }) {
  const { desde, hasta } = rangoMes(anio, mes)
  let q = supabase.from('v_credito_destino').select('*')
    .gte('fecha', desde).lte('fecha', hasta).order('monto', { ascending: false }).limit(3000)
  const suc = normalizarSucursalId(sucursal_id)
  if (suc) q = q.eq('sucursal_id', suc)
...
// Notas de crédito con su efecto monetario real y el caso de postventa que las originó
export async function fetchNcControl({ anio, mes, sucursal_id, efecto }) {
  const { desde, hasta } = rangoMes(anio, mes)
  let q = supabase.from('v_nc_control').select('*')
    .gte('fecha', desde).lte('fecha', hasta).order('fecha', { ascending: false }).limit(3000)
  const suc = normalizarSucursalId(sucursal_id)
  if (suc) q = q.eq('sucursal_id', suc)
  if (efecto) q = q.eq('efecto', efecto)
  const { data, error } = await q
  if (error) throw error
  return data ?? []
}

// Pasivo por sucursal (flujo del período, no el saldo histórico)
export async function fetchAbonosPasivo() {
  const { data, error } = await supabase.from('v_abonos_pasivo_sucursal').select('*').order('sucursal_id')
  if (error) throw error
  return data ?? []
}
...
export async function declararCierre(p) {
  ...
  if (error) {
    // Índice único uq_cierre_vendedor_dia (fecha × sucursal × recaudador, no anulados)
    if (error.code === '23505') throw new Error('Ya existe un cierre de este recaudador para el día. Ábrelo desde la lista en vez de firmar uno nuevo.')
    throw error
  }
  return data
}
...
function buildRow(p, uid) {
  return {
    ...
    // F2: snapshot del esperado según ledger de pagos y criterio de comparación usado
    caja_esperada_api: p.caja_esperada_api ?? null,
    base_comparacion: p.base_comparacion === 'caja' ? 'caja' : 'venta',
...
export async function corroborarCierre(p) {
  ...
  const difTotalSigned  = totalCorrob - Number(cierre.total_declarado ?? 0)   // + sobra · − falta
  const difTotal        = Math.abs(difTotalSigned)

  // Clasificación del ARQUEO (lo que contó el tesorero vs lo que declaró el cajero).
  // Es un eje distinto de la brecha de caja (declarado vs BSALE): aquí se mide si el
  // recuento coincide, allá si se declaró todo lo recaudado.
  // La severidad la manda el TOTAL, no el efectivo. Principio: hay diferencia de caja
  // solo si el total contado difiere del declarado. Si el total calza y solo cambió la
  // columna (efectivo → débito, por ejemplo), no falta plata: es una reclasificación
  // entre medios, que importa para la conciliación Getnet y como tema de capacitación,
  // pero no es un faltante.
  // Auditado el 20-09-2026 sobre 2.296 cierres del año: los dos "faltantes" mayores
  // ($2.074.550 y $1.229.296) eran reclasificaciones con total intacto, y estaban
  // generando cobranzas contra personas. La regla anterior clasificaba por el efectivo.
  const estado = difTotal <= umbrales.cuadra
    ? 'cuadra'
  ...
  // Cobranza: SOLO cuando falta o sobra plata de verdad, es decir cuando el TOTAL
  // corroborado difiere del declarado y esa diferencia supera la materialidad.
  //   · difTotalSigned = 0 y difEfectivo != 0 → reclasificación entre medios: no hay
  //     faltante, no se cobra. (Antes se cobraba: $3,3M imputados a dos personas por
  //     montos que estaban en débito, no perdidos.)
  //   · |difTotalSigned| <= materialidad → diferencia de caja del período (redondeo
  //     legal del efectivo). Se registra para trazabilidad como 'absorbido', sin
  //     entrar a la cola de cobro: administrar $30 cuesta más que $30.
  const esReclasificacion = Math.abs(difTotalSigned) <= 0 && Math.abs(difEfectivo) > 0
  if (Math.abs(difTotalSigned) > 0) {
    const bajoMaterialidad = Math.abs(difTotalSigned) <= umbrales.materialidad
    await supabase.from('descuadres_cobranza').insert({
      ...
      monto:               difTotalSigned,
      medio:               Math.abs(difEfectivo) >= Math.abs(difTotalSigned) ? 'efectivo' : 'otros',
      motivo:              difTotalSigned < 0 ? 'Falta efectivo en caja' : 'Sobra efectivo en caja',
      estado_cobro:        bajoMaterialidad ? 'absorbido' : 'pendiente',
      observaciones_admin: [
        p.observaciones_admin ?? null,
        bajoMaterialidad ? `[ABSORBIDA] Bajo materialidad ($${umbrales.materialidad}): diferencia de caja del período, no se cobra.` : null,
      ].filter(Boolean).join(' ') || null,
      registrado_por:      uid,
      resuelto_at:         bajoMaterialidad ? new Date().toISOString() : null,
    }).then(() => {}).catch(() => {})
  }
  ...
  return {
    ...data,
    descuadre_registrado: Math.abs(difTotalSigned) > umbrales.materialidad,
    diferencia_efectivo: difEfectivo,
    diferencia_total: difTotalSigned,
    reclasificacion_medios: esReclasificacion,
  }
}

// ── Edición administrativa de cierres firmados/corroborados ─────────────────
// Requiere capability fin.teso.cierre.editar (validada en UI).
// - Actualiza montos declarados y, si el cierre está corroborado, también los _corrob
// - Reclasifica el estado con los umbrales vigentes
// - Reemplaza el descuadre PENDIENTE del cierre (los ya cobrados no se tocan)
// - Recalcula la línea auto de depósito del día×sucursal
// - Traza: editado_por / editado_at + nota automática en observaciones_admin
export async function editarCierreAdmin(p) {
  const uid = (await supabase.auth.getSession()).data.session?.user.id
  if (!uid) throw new Error('Sesión no encontrada')
  if (!p.id) throw new Error('Falta id del cierre')

  const [{ data: cierre, error: errC }, umbrales] = await Promise.all([
    supabase.from('cierres_caja').select('*').eq('id', p.id).single(),
    fetchUmbrales(),
  ])
  if (errC) throw errC
  if (cierre.estado === 'anulado') throw new Error('No se puede editar un cierre anulado')

  const KEYS = ['efectivo','t_credito','t_debito','webpay','transferencia','m_pago','abono_cliente','canje','p_clay','cheque']
  const eraCorroborado = ['cuadra', 'tolerable', 'descuadre'].includes(cierre.estado)

  const decl = {}, corr = {}
  for (const k of KEYS) {
    decl[k] = Number(p[k] ?? cierre[k] ?? 0)
    corr[`${k}_corrob`] = Number(p[`${k}_corrob`] ?? cierre[`${k}_corrob`] ?? 0)
  }

  // Nota de auditoría con el delta exacto de lo que cambió
  const cambios = []
  for (const k of KEYS) {
    const d0 = Number(cierre[k] ?? 0)
    if (d0 !== decl[k]) cambios.push(`${k} ${d0}→${decl[k]}`)
    if (eraCorroborado) {
      const c0 = Number(cierre[`${k}_corrob`] ?? 0)
      if (c0 !== corr[`${k}_corrob`]) cambios.push(`${k}_corrob ${c0}→${corr[`${k}_corrob`]}`)
    }
  }
  const nota = `[EDITADO ${new Date().toISOString().slice(0, 10)}] ${cambios.length ? cambios.join(', ') : 'sin cambio de montos'}`
  const obsBase = p.observaciones_admin !== undefined ? p.observaciones_admin : (cierre.observaciones_admin ?? null)
  const observaciones_admin = obsBase ? `${obsBase} · ${nota}` : nota

  const update = { ...decl, observaciones_admin, editado_por: uid, editado_at: new Date().toISOString() }

  let difEfectivo = 0
  if (eraCorroborado) {
    Object.assign(update, corr)
    difEfectivo = corr.efectivo_corrob - decl.efectivo
    const totalCorrob = KEYS.reduce((s, k) => s + corr[`${k}_corrob`], 0)
    const totalDecl   = KEYS.reduce((s, k) => s + decl[k], 0)
    const difTotal    = Math.abs(totalCorrob - totalDecl)
    update.estado = difEfectivo !== 0
      ? 'descuadre'
      : difTotal <= umbrales.cuadra ? 'cuadra'
      : difTotal <= umbrales.tolerable ? 'tolerable'
      : 'descuadre'
  }
  ...
  let descuadre_nota = null
  if (eraCorroborado) {
    // Reemplazar el descuadre pendiente; los ya cobrados son historia contable y no se tocan
    const { data: descs } = await supabase.from('descuadres_cobranza')
      .select('id, estado_cobro').eq('cierre_id', p.id)
    const pendientes = (descs ?? []).filter(d => d.estado_cobro === 'pendiente').map(d => d.id)
    const cobrados   = (descs ?? []).filter(d => d.estado_cobro !== 'pendiente').length

    if (pendientes.length > 0) {
      const { error: errDel } = await supabase.from('descuadres_cobranza').delete().in('id', pendientes)
      if (errDel) throw new Error('Cierre editado pero falló la limpieza del descuadre previo: ' + errDel.message)
    }
    if (difEfectivo !== 0) {
      const { error: errIns } = await supabase.from('descuadres_cobranza').insert({
        cierre_id:           p.id,
        ...
        observaciones_admin: `Generado en edición admin. ${nota}`,
        registrado_por:      uid,
      })
      if (errIns) throw new Error('Cierre editado pero falló el registro del descuadre: ' + errIns.message)
    }
    if (cobrados > 0) descuadre_nota = 'Este cierre tiene descuadres ya cobrados que no fueron modificados.'

    await upsertDepositoEfectivoAutoCierre({ fecha: cierre.fecha, sucursal_id: cierre.sucursal_id })
      .catch(e => console.error('[upsertDeposito] error:', e?.message))
  }

  return { ...data, descuadre_nota }
}
...
export async function upsertDepositoEfectivoAutoCierre({ fecha, sucursal_id }) {
  ...
  // F2: el efectivo que salió del cajón a caja fuerte durante el día también debe
  // depositarse. Los retiros con destino 'deposito' ya están en un comprobante y se
  // descuentan abajo junto con los depósitos confirmados.
  const { data: retiros } = await supabase.from('cierres_retiros')
    .select('cierre_id, monto, destino')
    .in('cierre_id', cierreIds)
    .neq('destino', 'deposito')
  const retirosPendientes = (retiros ?? []).reduce((s, r) => s + Number(r.monto ?? 0), 0)

  const total = cierres.reduce((s, c) => s + Number(c.efectivo_corrob ?? c.efectivo ?? 0), 0) + retirosPendientes
  ...
  // 3. No existe línea abierta → crear nueva por el SALDO pendiente
  // A2: si ya hay depósitos confirmados del día (con comprobante), hay que
  // descontarlos. Sin esto, una corroboración o edición posterior al comprobante
  // creaba una segunda línea con el total completo → doble conteo.
  const { data: confirmados } = await supabase.from('depositos_efectivo')
    .select('monto_depositado')
    .eq('fecha', fecha)
    .eq('sucursal_id', sucursal_id)
    .not('comprobante_url', 'is', null)

  const yaDepositado = (confirmados ?? []).reduce((s, d) => s + Number(d.monto_depositado ?? 0), 0)
  const pendiente = total - yaDepositado
...
// Historial inmutable de un cierre (trigger tr_cierres_historial): quién registró cada dato
export async function fetchHistorialCierre(cierreId) {
  if (!cierreId) return []
  const { data, error } = await supabase.from('cierres_caja_historial')
    .select('id, accion, campo, valor_anterior, valor_nuevo, actor_nombre, actor_tipo, registrado_at')
    .eq('cierre_id', cierreId).order('registrado_at', { ascending: true }).order('id', { ascending: true })
  if (error) throw error
  return data ?? []
}
...
export async function sincronizarYAuditarBsaleMes({ anio, mes, sucursal_id, onProgress }) {
  ...
  const SUCURSALES_FISICAS = ['suc-mp', 'suc-lg', 'suc-la', 'suc-web']
