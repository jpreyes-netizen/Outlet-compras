...
export async function corroborarCierre(p) {
  ...
  // Se clasifica por MAGNITUD contra los umbrales. La versión anterior mandaba a
  // 'descuadre' cualquier diferencia de efectivo distinta de cero, así que $30 de
  // sencillo quedaba igual que un faltante de $2.000.000: 379 de 422 descuadres del
  // año eran monedas y sepultaban los 14 casos graves.
  // Ojo: la cobranza de más abajo se sigue generando por CUALQUIER diferencia de
  // efectivo (difEfectivo !== 0). No se deja de registrar un solo peso; lo que cambia
  // es cómo se rotula la severidad.
  const peorDif = Math.max(Math.abs(difEfectivo), difTotal)
  const estado = peorDif <= umbrales.cuadra
    ? 'cuadra'
    : peorDif <= umbrales.tolerable
      ? 'tolerable'
      : 'descuadre'
