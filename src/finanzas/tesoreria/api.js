...
export async function sincronizarYAuditarBsaleMes({ anio, mes, sucursal_id, onProgress }) {
  ...
  // Generar todos los días L-S del mes (cerrado domingos)
  const dias = []
  ...
  // Traer cierres existentes del mes en una sola query
  let q = supabase.from('cierres_caja')
    .select('fecha, sucursal_id')
    .gte('fecha', desde).lte('fecha', hasta)
    .neq('estado', 'anulado')
  ...
  const cierresSet = new Set(
    (cierresExistentes ?? []).map(c => `${c.fecha}|${c.sucursal_id}`)
  )
  ...
  for (let i = 0; i < combos.length; i++) {
    ...
    try {
      ...
      if (res.ok) {
        const json = await res.json()
        const ventaBsale = json.total_venta ?? 0
        const tieneCierre = cierresSet.has(`${fecha}|${suc}`)
        if (ventaBsale > 0 && !tieneCierre) {
          gaps.push({ fecha, sucursal_id: suc, venta_bsale: ventaBsale, medios: json.medios_global ?? {} })
        }
        ok++
      } else { errores++ }
    } catch { errores++ }
