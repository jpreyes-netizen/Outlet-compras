      ...
      <div style={{
        ...
        {kpi.explicacion && (
          <div style={{ padding: '14px 22px', borderBottom: '1px solid #F3F4F6', background: '#fff' }}>
            ...
            {kpi.benchmarkFuente && (
              <div style={{ fontSize: 11, color: '#1E40AF', background: '#EFF6FF', border: '1px solid #BFDBFE', borderRadius: 6, padding: '8px 10px', marginTop: 8, lineHeight: 1.45 }}>
                <b>Referencia de mercado:</b> {kpi.benchmarkFuente}
              </div>
            )}
          </div>
        )}
...
async function cargarTopMovimientos(codigosEerr, anio, mesHasta) {
  ...
  // Tope EXCLUSIVO = primer día del mes siguiente a mesHasta.
  // Antes se armaba con '-31' fijo y Postgres rechazaba abr/jun/sep/nov y feb.
  const mH = Number(mesHasta)
  const yEndEx = mH >= 12 ? (Number(anio) + 1) + '-01-01' : anio + '-' + String(mH + 1).padStart(2, '0') + '-01'
