...
/* ─── Costo + Gastos vs Venta mensual (barras triples) ─── */
export function GraficoGastoVsVenta({ valores, mesHasta }) {
  const data = useMemo(() => {
    if (!valores) return []
    const get = c => valores.get(c) ?? new Array(12).fill(0)
    const codigosGastoOp = [
    ...
    return Array.from({ length: 12 }, (_, i) => {
      ...
      const costo = !futuro ? (get('COSTO_NETO')[i] || 0) : null
      const gastoOp = !futuro ? codigosGastoOp.reduce((s, c) => s + (get(c)[i] || 0), 0) : null
      const resultado = (venta !== null && costo !== null && gastoOp !== null) ? (venta - costo - gastoOp) : null
      return { mes: MESES[i], venta, costo, gastoOp, resultado }
    })
  }, [valores, mesHasta])
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 10, padding: '14px 16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: '#6B7280', marginBottom: 4, letterSpacing: '0.02em' }}>
        VENTA vs COSTO vs GASTOS OPERATIVOS
      </div>
      <div style={{ fontSize: 10, color: '#9CA3AF', marginBottom: 10 }}>
        Costo de ventas (rojo claro) va al margen bruto. Gastos operativos (rojo oscuro) van después. Línea verde = resultado operacional.
      </div>
      <div style={{ height: 260 }}>
        <ResponsiveContainer width="100%" height="100%">
          <ComposedChart data={data} margin={{ top: 10, right: 20, bottom: 5, left: 0 }}>
            ...
            <Bar dataKey="costo" fill="#FB923C" name="Costo de Ventas" />
            <Bar dataKey="gastoOp" fill="#DC2626" name="Gastos Operativos" />
...
/* ─── Composición de gastos operativos (donut + leyenda, SIN costo de ventas) ─── */
export function GraficoComposicionGastos({ valores, mesHasta, lineasPorCodigo }) {
  const data = useMemo(() => {
    ...
    // SOLO gastos operativos. COSTO_NETO se muestra en gráfico aparte (Gasto vs Venta).
    const codigos = [
      'REM_OPERACION','REM_VENTA','REM_ADMIN','REM_SOCIOS',
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 10, padding: '14px 16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: '#6B7280', marginBottom: 4, letterSpacing: '0.02em' }}>
        COMPOSICIÓN DE GASTOS OPERATIVOS (acumulado)
      </div>
      <div style={{ fontSize: 10, color: '#9CA3AF', marginBottom: 10 }}>
        Top 8 líneas + resto agrupado. Excluye costo de ventas. Total: {formato.clp(total)}
      </div>
...
export function GraficoTopGastos({ valores, mesHasta, lineasPorCodigo }) {
  const { data, codigos } = useMemo(() => {
    ...
    const todos = [
      ...
      'FINIQUITOS','OTROS_GASTOS_ADMIN','COMISION_GETNET',
    ]
  ...
  return (
    <div style={{ background: '#fff', borderRadius: 10, padding: '14px 16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
      <div style={{ fontSize: 12, fontWeight: 600, color: '#6B7280', marginBottom: 4, letterSpacing: '0.02em' }}>
        TENDENCIA TOP 5 GASTOS OPERATIVOS
      </div>
      <div style={{ fontSize: 10, color: '#9CA3AF', marginBottom: 10 }}>
        Mes a mes, excluyendo costo de ventas. Si una línea sube fuerte, conviene drillear esa partida.
      </div>
