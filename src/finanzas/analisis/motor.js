...
export const LINEAS_VARIABLES = new Set([
  ...
  'FINIQUITOS','OTROS_GASTOS_ADMIN','TRANSPORTE_VIATICOS'
])
...
export function calcularKPIs({ valores, mesHasta, dotacionMes, lineasPorCodigo }) {
  ...
  // 3. Margen Neto (CONTABLE — resultado operacional menos intereses. IVA NO es gasto, no se resta)
  const interesesAcum = sumHasta(get('INTERES_CREDITOS'), m)
  const resNetoContable = resOp - interesesAcum
  ...
  kpis.push({
    ...
    explicacion: 'Rentabilidad contable final: resultado operacional menos intereses de créditos. NO descuenta el IVA (es un pasivo neutro que se paga al SII, no un gasto) ni las compras de inventario (inversión en activo, no gasto del período).',
    formula: '(Resultado Operacional − Intereses) / Venta Neta',
    composicion: [
    ...
    evolucionPct: Array.from({ length: 12 }, (_, i) => {
      ...
      if (!v) return null
      const rn = get('RESULTADO_OPERACIONAL')[i] - get('INTERES_CREDITOS')[i]
      return (rn / v) * 100
    }),
  })
...
export function calcularMetricasGestion({ valores, mesHasta, lineasPorCodigo, saldoCajaEstimado }) {
  ...
  const resOp = sumHasta(get('RESULTADO_OPERACIONAL'), m)
  const interesesAcum = sumHasta(get('INTERES_CREDITOS'), m)
  // Resultado neto CONTABLE: resultado operacional menos intereses.
  // IVA NO se resta (pasivo neutro al SII). Compras de inventario tampoco (inversión en activo).
  const resNetoContable = resOp - interesesAcum
...
export function construirResumenParaLLM({ kpis, alertas, metricas, anio, mesHasta }) {
  ...
  txt += '- Resultado Neto Contable: ' + fmtCLP(metricas.resNetoContable) + ' (' + (metricas.margenNetoContablePct || 0).toFixed(1) + '% de VN). Resultado operacional menos intereses. El IVA y las compras de inventario no se descuentan acá.\n'
