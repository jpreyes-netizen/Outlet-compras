const ESTILO_SEVERIDAD = {
  critica: { bg: '#FEF2F2', border: '#FECACA', icon: '🔴', label: 'CRÍTICA', color: '#B91C1C' },
  atencion: { bg: '#FFFBEB', border: '#FDE68A', icon: '🟡', label: 'ATENCIÓN', color: '#B45309' },
  positiva: { bg: '#ECFDF5', border: '#A7F3D0', icon: '🟢', label: 'POSITIVA', color: '#047857' },
}

function FilaAlerta({ alerta, onClick }) {
  ...
  return (
    <div
      ...
      onMouseEnter={clickable ? (e) => { e.currentTarget.style.transform = 'translateX(2px)' } : undefined}
      onMouseLeave={clickable ? (e) => { e.currentTarget.style.transform = 'translateX(0)' } : undefined}
    >
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 3, flexWrap: 'wrap' }}>
          ...
          {alerta.codigo && (
            <span style={{ fontSize: 9, fontWeight: 600, color: '#6B7280', background: '#fff', padding: '1px 6px', borderRadius: 3, fontFamily: 'monospace' }}>
              {alerta.codigo}
            </span>
          )}
        </div>
        ...
        {clickable && (
          <div style={{ fontSize: 10, color: s.color, marginTop: 4, fontStyle: 'italic' }}>
            Click para investigar →
          </div>
        )}
      </div>
    </div>
...
export function AlertasInteligentes({ alertas, titulo = 'Alertas inteligentes', maxItems, onClickAlerta }) {
  if (!alertas || alertas.length === 0) {
    return (
      <div style={{ background: '#fff', borderRadius: 10, padding: '14px 16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)', marginBottom: 14 }}>
        <div style={{ fontSize: 13, fontWeight: 600, color: '#6B7280' }}>{titulo}</div>
        <div style={{ fontSize: 12, color: '#9CA3AF', marginTop: 6 }}>Sin hallazgos relevantes para este período.</div>
      </div>
    )
  }
    ...
    <div style={{ marginBottom: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 12, marginBottom: 8, flexWrap: 'wrap' }}>
        ...
        {maxItems && alertas.length > maxItems && (
          <span style={{ fontSize: 11, color: '#6B7280', marginLeft: 'auto' }}>Mostrando {maxItems} de {alertas.length}</span>
        )}
      </div>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 6 }}>
        {mostrar.map((a, i) => <FilaAlerta key={i} alerta={a} onClick={onClickAlerta ? () => onClickAlerta(a) : null} />)}
      </div>
    </div>
  )
}
