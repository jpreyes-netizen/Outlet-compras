...
async function fetchEerr(anio) {
  ...
  const [lineasR, mapeoR, ventasR, movsR, comprasR, ajustesR, subcuentasR, userR] = await Promise.all([
    ...
    supabase.from('ventas_bsale').select('fecha, total_venta').gte('fecha', yStart).lte('fecha', yEnd),
  ...
  if (ventasR.error) throw new Error('ventas_bsale: ' + ventasR.error.message)
