...
function PanelMetricasGestion({ metricas }) {
  const flujoNeg = (metricas.flujoEconomico || 0) < 0
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      {/* Contabilidad vs Caja — la distinción crítica */}
      <div style={{ background: '#fff', borderRadius: 10, padding: '14px 16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <div style={{ fontSize: 12, fontWeight: 600, color: '#6B7280', marginBottom: 4, letterSpacing: '0.02em' }}>RESULTADO CONTABLE vs FLUJO DE CAJA</div>
        <div style={{ fontSize: 10, color: '#9CA3AF', marginBottom: 10 }}>
          Dos miradas distintas: cuánto ganas (contable) vs cuánta caja queda tras reinvertir en inventario (económico).
        </div>
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 10 }}>
          <MetricaBox titulo="Resultado Neto Contable"
            valor={formato.clp(metricas.resNetoContable) + ' (' + (metricas.margenNetoContablePct || 0).toFixed(1) + '%)'}
            detalle="Resultado operacional − intereses − impuestos. Esta es tu rentabilidad real. NO descuenta compra de inventario."
            color={(metricas.resNetoContable || 0) >= 0 ? '#047857' : '#DC2626'} />
          <MetricaBox titulo="Compras de Inventario"
            valor={formato.clp(metricas.comprasInventario)}
            detalle="Plata invertida en stock (importaciones + reposición). Es inversión en activo, NO gasto del período."
            color="#B45309" />
          <MetricaBox titulo="Flujo Económico"
            valor={formato.clp(metricas.flujoEconomico) + ' (' + (metricas.flujoEconomicoPct || 0).toFixed(1) + '%)'}
            detalle={flujoNeg
              ? 'Negativo = estás reinvirtiendo en stock más de lo que generas este período. Normal en crecimiento/importación, NO es pérdida.'
              : 'Positivo = generas más caja de la que reinviertes en inventario.'}
            color={flujoNeg ? '#B45309' : '#047857'} />
        </div>
      </div>

      {/* Métricas de gestión operativa */}
      <div style={{ background: '#fff', borderRadius: 10, padding: '14px 16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        ...
        <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(220px, 1fr))', gap: 10 }}>
          ...
          <MetricaBox titulo="Burn Rate Diario"
            ...
            detalle="Egreso diario total de caja (incluye costo, gastos operativos y compra de inventario)."
            color="#6B7280" />
