...
export function FinAnalisis({ cu, isMobile }) {
  ...
  useEffect(() => {
    ...
    Promise.all([fetchValoresEerr(anio), fetchDotacion(anio), fetchPresupuesto(anio)])
      .then(([d, dot, pres]) => { setData(d); setDotacion(dot); setPresupuesto(pres); setLoading(false) })
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <div style={{ borderRadius: 8, border: '1px solid #FCD34D', background: '#FEF3C7', padding: '10px 14px', fontSize: 12, color: '#92400E', display: 'flex', alignItems: 'flex-start', gap: 10 }}>
        ...
        <div style={{ flex: 1, lineHeight: 1.5 }}>
          <b>Aviso:</b> los KPIs y alertas dependen de <code style={{ background: '#FDE68A', padding: '0 4px', borderRadius: 3 }}>ventas_bsale_dia</code> que actualmente subreporta vs BSALE. Conclusiones cualitativas válidas; valores absolutos sujetos a ajuste cuando se reconcilie el pipeline.
        </div>
      </div>
