...
function EerrDevengo() {
  ...
  useEffect(() => {
    (async () => {
      try {
        const [{ data: m }, { data: d }, { data: s }] = await Promise.all([
        ...
        setMensual(m ?? []); setDetalle(d ?? []); setPorSucursal(s ?? [])
      } catch (e) { toast.error('Error: ' + e.message) }
      finally { setCargando(false) }
    })()
  }, [])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {[{ k: 'consolidado', l: 'Consolidado' }, { k: 'sucursal', l: 'Por sucursal (contribución)' }].map(v => (
