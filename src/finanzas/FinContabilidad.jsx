...
import { Comprobantes, PorClasificar, Tributario, Glosario, CuadraturaMotores } from './FinContabilidadOps'
    ...
    (async () => {
      try {
        const [{ data: pc }, { data: bal }] = await Promise.all([
          supabase.from('plan_cuentas').select('codigo, nombre, nombre_contador, nivel, tipo_eeff, naturaleza, acepta_movimientos, origen, activa, descripcion_uso').order('codigo').limit(1000),
...
function EerrDevengo() {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {[{ k: 'formal', l: 'EERR formal (NIC 1)' }, { k: 'consolidado', l: 'Resumen mensual' }, { k: 'sucursal', l: 'Por sucursal' }, { k: 'flujo', l: 'Flujo de caja (NIC 7)' }].map(v => (
      ...
      {vista === 'formal' ? (
        <Panel titulo={`Estado de resultados — ${perSuc || periodosSuc[0] || ''}`} sub="Presentación por función (NIC 1) desde el libro mayor · EBITDA como métrica de gestión"
          acciones={<BtnExport onClick={() => exportar(formal.map(f => ({ Período: f.periodo, Línea: f.linea, Monto: f.monto })), 'eerr_formal', 'EERR')} />}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <tbody>
              {formal.filter(f => f.periodo === (perSuc || periodosSuc[0])).map(f => (
                <tr key={f.orden} style={{ background: f.nivel === 'total' ? '#EEF2FF' : f.nivel === 'subtotal' ? FONDO : '#fff' }}>
                  <td style={{ ...TD, fontWeight: f.nivel !== 'detalle' ? 700 : 400, color: f.nivel !== 'detalle' ? NAVY : INK,
                    paddingLeft: f.nivel === 'detalle' ? 24 : 8 }}>{f.linea}</td>
                  <td style={{ ...TDNUM, fontWeight: f.nivel !== 'detalle' ? 700 : 400,
                    color: f.nivel !== 'detalle' ? (f.monto >= 0 ? VERDE : ROJO) : INK, width: 160 }}>{fmtSigno(f.monto)}</td>
                </tr>
              ))}
            </tbody>
          </table>
          <div style={{ padding: '8px 12px', fontSize: 10.5, color: SLATE, borderTop: `1px solid ${BORDE}` }}>
            EBITDA = resultado operacional antes de depreciación. Provisión de renta al 27% sobre RAI mensual positivo (recalculada si el resultado cambia). Estimación de gestión: la RLI tributaria definitiva la determina el contador.
          </div>
        </Panel>
      ) : vista === 'flujo' ? (
        ...
        <Panel titulo="Contribución por sucursal" sub="Ingresos netos, costo directo, dotación y mermas atribuidos por tienda — gastos centrales y financieros no incluidos"
          acciones={<BtnExport onClick={() => exportar(filasSuc.map(f => ({
            ...
            Mermas: f.mermas, Contribución: f.contribucion, '% Contribución': f.contribucion_pct,
