...
import { Comprobantes, PorClasificar, Tributario, Glosario, CuadraturaMotores, Auditoria } from './FinContabilidadOps'
import { LibroBancoDetalle, LibroCompras, LibroVentas } from './FinLibros'
...
import { ControlVentasTab } from './ControlVentasTab'
...
const BtnExport = ({ onClick, filas, titulo, sub }) => (
  <span style={{ display: 'inline-flex', gap: 6 }}>
    <button onClick={onClick} style={{ ...INPUT, cursor: 'pointer', fontWeight: 600, color: NAVY }}>Excel</button>
    {filas?.length ? <button onClick={() => exportarPDF({ titulo: titulo || 'Informe', sub, filas })}
      style={{ ...INPUT, cursor: 'pointer', fontWeight: 600, color: NAVY }}>PDF</button> : null}
  </span>
)
...
function BalanceComprobacion() {
  ...
  const [formato, setFormato] = useState('saldos')
  const [cargando, setCargando] = useState(true)
  const [det, setDet] = useState(null)
  function verCuenta(f) {
    abrirFuente(setDet, {
      titulo: `${f.codigo} · ${f.nombre}`, sub: 'Libro mayor de la cuenta',
      query: supabase.from('v_libro_mayor').select('fecha, asiento, glosa_asiento, glosa_linea, debe, haber, saldo_acumulado').eq('plan_cuenta_codigo', f.codigo).order('fecha'),
    })
  }

  useEffect(() => {
    supabase.from('v_balance_8_columnas').select('*').limit(1000).then(({ data }) => setOcho(data ?? []))
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      <div style={{ display: 'flex', gap: 6 }}>
        {[{ k: 'saldos', l: 'Comprobación y saldos' }, { k: 'ocho', l: 'Balance tributario (8 columnas)' }].map(v => (
          <button key={v.k} onClick={() => setFormato(v.k)} style={{ fontSize: 12, fontWeight: 600, padding: '5px 12px', borderRadius: 6, cursor: 'pointer',
            border: `1px solid ${formato === v.k ? NAVY : BORDE}`, background: formato === v.k ? NAVY : '#fff', color: formato === v.k ? '#fff' : INK }}>{v.l}</button>
        ))}
      </div>
      {formato === 'ocho' && (
        <Panel titulo="Balance tributario de 8 columnas" sub="Sumas · Saldos · Inventario · Resultado — formato clásico para el contador"
          acciones={<BtnExport titulo="Balance tributario de 8 columnas" sub={`Ejercicio ${new Date().getFullYear()}`}
            filas={ocho.map(f => ({ Código: f.codigo, Cuenta: f.nombre, 'Débitos': f.debitos, 'Créditos': f.creditos, 'S. Deudor': f.saldo_deudor, 'S. Acreedor': f.saldo_acreedor, Activo: f.activo, Pasivo: f.pasivo, 'Pérdida': f.perdida, Ganancia: f.ganancia }))}
            onClick={() => exportar(ocho.map(f => ({ Código: f.codigo, Cuenta: f.nombre, Débitos: f.debitos, Créditos: f.creditos, 'Saldo Deudor': f.saldo_deudor, 'Saldo Acreedor': f.saldo_acreedor, Activo: f.activo, Pasivo: f.pasivo, Pérdida: f.perdida, Ganancia: f.ganancia })), 'balance_8_columnas', '8 columnas')} />}>
          <div style={{ maxHeight: '62vh', overflow: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr>
                <th style={TH}>Cuenta</th>
                <th style={{ ...TH, textAlign: 'right' }}>Débitos</th><th style={{ ...TH, textAlign: 'right' }}>Créditos</th>
                <th style={{ ...TH, textAlign: 'right' }}>S. Deudor</th><th style={{ ...TH, textAlign: 'right' }}>S. Acreedor</th>
                <th style={{ ...TH, textAlign: 'right' }}>Activo</th><th style={{ ...TH, textAlign: 'right' }}>Pasivo</th>
                <th style={{ ...TH, textAlign: 'right' }}>Pérdida</th><th style={{ ...TH, textAlign: 'right' }}>Ganancia</th>
              </tr></thead>
              <tbody>
                {ocho.map(f => (
                  <tr key={f.codigo} onClick={() => verCuenta(f)} style={{ cursor: 'pointer' }} title="Clic: libro mayor de la cuenta">
                    <td style={{ ...TD, whiteSpace: 'normal' }}><span style={{ fontFamily: 'ui-monospace, monospace', color: SLATE }}>{f.codigo}</span> {f.nombre}</td>
                    <td style={TDNUM}>{fmt(f.debitos)}</td><td style={TDNUM}>{fmt(f.creditos)}</td>
                    <td style={TDNUM}>{f.saldo_deudor ? fmt(f.saldo_deudor) : ''}</td><td style={TDNUM}>{f.saldo_acreedor ? fmt(f.saldo_acreedor) : ''}</td>
                    <td style={TDNUM}>{f.activo ? fmt(f.activo) : ''}</td><td style={TDNUM}>{f.pasivo ? fmt(f.pasivo) : ''}</td>
                    <td style={TDNUM}>{f.perdida ? fmt(f.perdida) : ''}</td><td style={TDNUM}>{f.ganancia ? fmt(f.ganancia) : ''}</td>
                  </tr>
                ))}
                <tr style={{ background: '#EEF2FF' }}>
                  <td style={{ ...TD, fontWeight: 700, color: NAVY }}>TOTALES</td>
                  {['debitos', 'creditos', 'saldo_deudor', 'saldo_acreedor', 'activo', 'pasivo', 'perdida', 'ganancia'].map(k => (
                    <td key={k} style={{ ...TDNUM, fontWeight: 700, color: NAVY }}>{fmt(ocho.reduce((a, f) => a + Number(f[k] || 0), 0))}</td>
                  ))}
                </tr>
                <tr>
                  <td colSpan={5} style={{ ...TD, textAlign: 'right', fontWeight: 700, color: NAVY }}>Resultado del ejercicio (Ganancia − Pérdida = Activo − Pasivo)</td>
                  <td colSpan={4} style={{ ...TDNUM, fontWeight: 700, color: VERDE }}>{fmt(ocho.reduce((a, f) => a + Number(f.ganancia || 0) - Number(f.perdida || 0), 0))}</td>
                </tr>
              </tbody>
            </table>
          </div>
      ...
      {formato === 'saldos' && <Panel titulo="Balance de comprobación y saldos" sub={`${filas.length} cuentas con movimiento`}
        acciones={<BtnExport filas={filas.map(f => ({ Código: f.codigo, Cuenta: f.nombre, Debe: f.debe, Haber: f.haber, Saldo: f.saldo }))}
          titulo="Balance de comprobación y saldos" sub={`${filas.length} cuentas con movimiento`}
          onClick={() => exportar(filas.map(f => ({
...
function EerrDevengo({ cu }) {
  ...
  const [det, setDet] = useState(null)
  function verSucursal(f) {
    abrirFuente(setDet, {
      titulo: `${f.ceco_nombre} · ${f.periodo}`, sub: 'Gastos directos por línea (facturas con centro de costo + pagos banco sin factura)',
      query: supabase.from('v_eerr_sucursal_gastos').select('linea_nombre, fuente, monto').eq('ceco', f.ceco).eq('periodo', f.periodo).order('monto', { ascending: false }),
    })
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {vista === 'formal' ? (
        ...
        <Panel titulo="Contribución por sucursal" sub="Ingresos, costo, dotación, mermas y gastos directos (facturas con centro de costo) por tienda — asigná el CeCo de arriendos y servicios en Conciliación → Imputar"
          acciones={<BtnExport titulo="Contribución por sucursal" sub={perSuc || periodosSuc[0]}
            filas={filasSuc.map(f => ({ Centro: f.ceco_nombre, Ingresos: f.ingresos, Costo: f.costo_ventas, 'MB %': f.margen_bruto_pct, Dotación: f.remuneraciones, Mermas: f.mermas, 'Gastos dir.': f.gastos_directos, Contribución: f.contribucion }))}
            onClick={() => exportar(filasSuc.map(f => ({
          ...
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            ...
            <tbody>
              {filasSuc.map(f => (
                <tr key={f.ceco} onClick={() => verSucursal(f)} style={{ opacity: f.tipo === 'central' ? 0.75 : 1, cursor: 'pointer' }} title="Clic: gastos directos por línea de este centro">
...
function KardexPmp() {
  ...
  const cargar = useCallback(async () => {
    const { data } = await supabase.from('v_inv_pmp_resumen').select('*')
    setRes(data ?? [])
  }, [])
  useEffect(() => { cargar() }, [cargar])
  async function recalcular() {
    if (!window.confirm('Recalcular el kardex PMP de todos los SKUs desde el primer snapshot? Tarda unos segundos.')) return
    setProc(true)
    const { data, error } = await supabase.rpc('fn_calcular_kardex_pmp', { p_sku: null })
    setProc(false)
    if (error) { toast.error(error.message); return }
    toast.success(`Kardex recalculado · ${data.filas} filas SKU×mes`); cargar()
  }
  function verMes(r, modo) {
    const base = supabase.from('v_inv_pmp_detalle').select('sku, producto, entradas_qty, costo_entrada, pmp, estandar, desvio_pct, vendidas, cv_pmp, cv_estandar, impacto, fuente_costo, stock_teorico, stock_bsale, dif_unidades').eq('periodo', r.periodo)
    if (modo === 'impacto') abrirFuente(setDet, { titulo: `Kardex ${r.periodo} — SKUs por impacto PMP vs estándar`, sub: 'Impacto = costo estándar − costo PMP (positivo: el estándar sobreestimaba)', query: base.gt('vendidas', 0).order('impacto', { ascending: false }) })
    else abrirFuente(setDet, { titulo: `Kardex ${r.periodo} — stock BSALE sin recepción en el ERP`, sub: 'Unidades en BSALE que el kardex no explica con recepciones (productos ≥ $500)', query: base.gt('dif_unidades', 0).gte('estandar', 500).order('dif_unidades', { ascending: false }) })
  }
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      <Panel titulo="Kardex a costo promedio ponderado (PMP) — análisis" sub="Desde el primer snapshot de stock (jun-2026). La contabilidad sigue a costo estándar hasta que la cobertura y el control de recepciones lo justifiquen."
        acciones={<><BtnExport filas={res} titulo="Kardex PMP — resumen mensual" onClick={() => exportar(res, 'kardex_pmp_resumen', 'PMP')} />
          <button onClick={recalcular} disabled={proc} style={{ ...INPUT, cursor: 'pointer', fontWeight: 600, color: NAVY }}>{proc ? 'Calculando…' : 'Recalcular'}</button></>}>
        <table style={{ width: '100%', borderCollapse: 'collapse' }}>
          <thead><tr>
            <th style={TH}>Mes</th><th style={{ ...TH, textAlign: 'right' }}>Venta neta</th>
            <th style={{ ...TH, textAlign: 'right' }}>Costo estándar</th><th style={{ ...TH, textAlign: 'right' }}>Costo PMP</th><th style={{ ...TH, textAlign: 'right' }}>Diferencia</th>
            <th style={{ ...TH, textAlign: 'right' }}>Margen std</th><th style={{ ...TH, textAlign: 'right' }}>Margen PMP</th>
            <th style={{ ...TH, textAlign: 'right' }}>Cobertura PMP</th><th style={{ ...TH, textAlign: 'right' }}>Sin recepción</th>
          </tr></thead>
          <tbody>
            {res.map(r => (
              <tr key={r.periodo}>
                <td style={{ ...TD, fontWeight: 700 }}>{r.periodo}</td>
                <td style={TDNUM}>{fmt(r.venta_neta)}</td>
                <td style={TDNUM}>{fmt(r.costo_ventas_estandar)}</td>
                <td onClick={() => verMes(r, 'impacto')} style={{ ...TDNUM, fontWeight: 600, cursor: 'pointer', textDecoration: 'underline dotted #C7D2FE' }} title="Clic: SKUs por impacto">{fmt(r.costo_ventas_pmp)}</td>
                <td style={{ ...TDNUM, color: r.diferencia >= 0 ? VERDE : ROJO }}>{fmt(r.diferencia)}</td>
                <td style={TDNUM}>{r.margen_estandar_pct}%</td>
                <td style={{ ...TDNUM, fontWeight: 700, color: NAVY }}>{r.margen_pmp_pct}%</td>
                <td style={{ ...TDNUM, color: r.cobertura_pmp_pct >= 80 ? VERDE : AMBAR }}>{r.cobertura_pmp_pct}% · {r.skus_con_pmp}/{r.skus_vendidos}</td>
                <td onClick={() => verMes(r, 'recepcion')} style={{ ...TDNUM, color: ROJO, cursor: 'pointer', textDecoration: 'underline dotted #C7D2FE' }} title="Clic: SKUs en BSALE sin recepción">
                  {fmt(r.unidades_producto_sin_recepcion)} u · {fmt(r.valor_sin_recepcion)}</td>
              </tr>
            ))}
          </tbody>
        </table>
        <div style={{ fontSize: 11, color: SLATE, marginTop: 8, lineHeight: 1.5 }}>
          <b>Cómo leerlo.</b> PMP = (stock inicial × PMP anterior + entradas × costo landed) / unidades. Entradas: recepciones con costo de OC, importaciones a TC y factor landed del costeo propio.
          <b>Cobertura</b>: % del costo de ventas calculado con PMP real; el resto usa estándar porque el SKU no tiene entradas costeadas ni costo en el snapshot.
          <b>Sin recepción</b>: mercadería que existe en BSALE pero nunca pasó por Recepción en el ERP — hallazgo de control interno.
        </div>
      </Panel>
      <FuenteDrawer det={det} onClose={() => setDet(null)} />
    </div>
...
function CierreEjercicio({ cu }) {
  const [chk, setChk] = useState([])
  const [sim, setSim] = useState(null)
  const [proc, setProc] = useState(false)
  const anio = new Date().getFullYear()
  const cargar = useCallback(async () => {
    const { data } = await supabase.from('v_cierre_ejercicio_checklist').select('*').order('orden')
    setChk(data ?? [])
  }, [])
  useEffect(() => { cargar() }, [cargar])
  ...
  async function simular() {
    setProc(true)
    const { data, error } = await supabase.rpc('fn_cierre_ejercicio', { p_anio: anio, p_usuario: cu?.id ?? 'ui', p_simular: true })
    setProc(false)
    if (error) { toast.error(error.message); return }
    setSim(data)
  }
  async function ejecutar() {
    if (!window.confirm(`Cerrar contablemente el ejercicio ${anio}?\n\nSe saldan todas las cuentas de resultado contra Resultado del ejercicio, y éste se traspasa a Resultados acumulados. Queda reversible mientras no se abra ${anio + 1}.`)) return
    setProc(true)
    const { data, error } = await supabase.rpc('fn_cierre_ejercicio', { p_anio: anio, p_usuario: cu?.id ?? 'ui', p_simular: false })
    setProc(false)
    if (error) { toast.error(error.message); return }
    toast.success(`Ejercicio ${anio} cerrado · resultado ${fmt(data.resultado_del_ejercicio)}`)
    cargar(); setSim(null)
  }
  async function abrirSiguiente() {
    if (!window.confirm(`Generar el asiento de apertura del ejercicio ${anio + 1} con los saldos de balance al 31-12-${anio}?`)) return
    setProc(true)
    const { data, error } = await supabase.rpc('fn_apertura_ejercicio', { p_anio: anio + 1, p_usuario: cu?.id ?? 'ui', p_simular: false })
    setProc(false)
    if (error) { toast.error(error.message); return }
    toast.success(`Apertura ${anio + 1} creada con ${data.cuentas} cuentas`)
    cargar()
  }

  const todoOk = chk.length > 0 && chk.every(c => c.ok)
  return (
    <Panel titulo={`Cierre del ejercicio ${anio}`} sub="Salda las cuentas de resultado contra patrimonio y habilita la apertura del año siguiente — el hito contable anual">
      <table style={{ width: '100%', borderCollapse: 'collapse', marginBottom: 12 }}>
        <tbody>
          {chk.map(c => (
            <tr key={c.orden}>
              <td style={{ ...TD, width: 26, color: c.ok ? VERDE : ROJO, fontWeight: 700 }}>{c.ok ? '✓' : '·'}</td>
              <td style={{ ...TD, fontWeight: 600 }}>{c.requisito}</td>
              <td style={{ ...TD, color: c.ok ? VERDE : AMBAR, fontWeight: 600 }}>{c.estado}</td>
            </tr>
          ))}
        </tbody>
      </table>
      <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', alignItems: 'center' }}>
        <button onClick={simular} disabled={proc} style={{ ...INPUT, cursor: 'pointer', fontWeight: 600, color: NAVY }}>Simular cierre</button>
        <button onClick={ejecutar} disabled={proc || !todoOk} style={{ ...INPUT, cursor: todoOk ? 'pointer' : 'not-allowed', fontWeight: 600, background: todoOk ? NAVY : '#F3F4F6', color: todoOk ? '#fff' : SLATE, border: 'none' }}>Ejecutar cierre {anio}</button>
        <button onClick={abrirSiguiente} disabled={proc} style={{ ...INPUT, cursor: 'pointer', fontWeight: 600, color: NAVY }}>Apertura {anio + 1}</button>
        {!todoOk && <span style={{ fontSize: 11.5, color: SLATE }}>Completá el checklist para habilitar el cierre. La simulación funciona igual y no escribe nada.</span>}
      </div>
      {sim && (
        <div style={{ marginTop: 12, background: FONDO, border: `1px solid ${BORDE}`, borderRadius: 8, padding: 12 }}>
          <div style={{ fontSize: 13, fontWeight: 700, color: NAVY, marginBottom: 6 }}>
            Simulación · {sim.cuentas_a_saldar} cuentas de resultado · Resultado del ejercicio {fmt(sim.resultado_del_ejercicio)}
          </div>
          <div style={{ maxHeight: 260, overflow: 'auto' }}>
            <table style={{ width: '100%', borderCollapse: 'collapse' }}>
              <thead><tr><th style={TH}>Cuenta</th><th style={TH}>Nombre</th><th style={{ ...TH, textAlign: 'right' }}>Saldo a saldar</th></tr></thead>
              <tbody>
                {(sim.detalle ?? []).map(d => (
                  <tr key={d.cuenta}>
                    <td style={{ ...TD, fontFamily: 'ui-monospace, monospace', color: SLATE }}>{d.cuenta}</td>
                    <td style={TD}>{d.nombre}</td>
                    <td style={TDNUM}>{fmt(d.saldo)}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          ...
          <div style={{ fontSize: 11, color: SLATE, marginTop: 6 }}>
            Contrapartida: cuenta 2630102 Resultado del ejercicio, que luego se traspasa a 2630101 Resultados acumulados.
          </div>
        </div>
      )}
    </Panel>
...
export function FinContabilidad({ cu }) {
  ...
  const GRUPOS = [
    { g: 'Estados', tabs: [
      ...
      { k: 'control', l: 'Control vs BSALE' },
      { k: 'eerrdev', l: 'EERR y EBITDA' },
      { k: 'kardex', l: 'Kardex PMP' },
    ]},
    { g: 'Libros', tabs: [
      { k: 'diario', l: 'Diario' },
      { k: 'mayor', l: 'Mayor' },
    ...
    { g: 'Gobierno', tabs: [
      { k: 'cierre', l: 'Control y cierre' },
      { k: 'centro_control', l: 'Centro de control' },
      { k: 'kpis_auditoria', l: 'KPIs y auditoría' },
      { k: 'plan', l: 'Plan de cuentas' },
      { k: 'ejercicio', l: 'Cierre de ejercicio' },
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
      ...
      {tab === 'banco' && <LibroBancoDetalle />}
      {tab === 'compras' && <LibroCompras />}
      {tab === 'ventas' && <LibroVentas />}
      {tab === 'centro_control' && <CentroControlTab />}
      {tab === 'kpis_auditoria' && <FinKpisAuditoria />}
      {tab === 'plan' && <PlanCuentas />}
      {tab === 'comprobantes' && <Comprobantes cu={cu} />}
      {tab === 'porclasificar' && <PorClasificar cu={cu} />}
      {tab === 'tributario' && <Tributario />}
      {tab === 'cierre' && <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}><CuadraturaMotores /><CierrePeriodos cu={cu} /><ControlContable /></div>}
      {tab === 'glosario' && <Glosario />}
      {tab === 'control' && <ControlVentasTab />}
      {tab === 'kardex' && <KardexPmp />}
      {tab === 'ejercicio' && <CierreEjercicio cu={cu} />}
