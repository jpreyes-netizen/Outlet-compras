...
export function GmEditorMovimiento({ show, onClose, mov, cu, fondos, categorias, onGuardado }) {
  ...
  const guardarCambios = async () => {
    ...
    try {
      ...
      const cambioMontos = montoNuevo !== form.monto_original
        || tipoNuevo !== form.tipo_original
        || form.fecha !== mov?.fecha
      ...
      // Si cambió monto, tipo o fecha → recalcular la cadena completa.
      // NUNCA ajustar saldo_actual con deltas: desincroniza los saldo_post.
      if (cambioMontos && form.afecta_saldo) {
  ...
  const validar = async () => {
    ...
    try {
      ...
      // Si venía de rechazado, vuelve a afectar saldo → recalcular cadena
      if (form.estado === 'rechazado' && !form.afecta_saldo) {
        const { error: e2 } = await supabase.from('gm_movimientos')
          .update({ afecta_saldo: true }).eq('id', form.id)
        if (e2) throw e2
        const r = await recalcularCadena(form.fondo_id)
        if (!r.ok) throw new Error('Recálculo de saldos falló: ' + r.error)
      }
  ...
  const rechazar = async () => {
    ...
    setSaving(true)
    try {
      // Marcar rechazado primero, luego recalcular la cadena completa
      ...
      toast.success('Movimiento rechazado — saldos recalculados')
      onGuardado && onGuardado()
      onClose()
    } catch (e) {
      console.error(e)
