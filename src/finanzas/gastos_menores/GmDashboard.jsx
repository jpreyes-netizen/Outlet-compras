...
export function GmDashboard({ cu, isMobile }) {
    ...
    try {
      ...
      const [{ data: fs }, { data: ms }, { data: msAnt }] = await Promise.all([
        ...
        supabase.from('gm_movimientos')
          .select('id, fondo_id, fecha, tipo, monto, categoria_id, descripcion, proveedor, responsable_nombre, url_respaldo, archivo_storage, gm_categorias(nombre)')
  ...
  const kpis = useMemo(() => {
    const R = RANGOS[rango]
    const gastos = movsFiltrados.filter(m => m.tipo === 'gasto')
  ...
  return (
    <div>
      ...
      <div style={{
        ...
        <div style={{ display: "flex", gap: 4, background: "#F2F2F7", borderRadius: 8, padding: 3 }}>
          {[{k:'todos',l:'Todos'},{k:'sucursal',l:'Sucursales'},{k:'direccion',l:'Direcciones'}].map(t => (
