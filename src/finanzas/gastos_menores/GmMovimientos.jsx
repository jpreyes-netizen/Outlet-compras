...
export function GmMovimientos({ cu, isMobile }) {
  ...
  const subirArchivoNuevo = async (file) => {
    ...
    try {
      ...
      const path = `${fondo.sucursal_id}/${tempId}.${ext}`
      const { error } = await supabase.storage.from('gastos-menores').upload(path, file, { upsert: false })
                  ...
                  return (
                    <tr key={m.id}
                      ...
                      <td style={td} onClick={e => e.stopPropagation()}>
                        {m.url_respaldo || m.archivo_storage ? (
