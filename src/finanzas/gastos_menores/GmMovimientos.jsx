...
import { recalcularCadena } from './gm_saldos'
...
export function GmMovimientos({ cu, isMobile }) {
  ...
  const cargar = async () => {
    ...
    try {
      const [{ data: fs }, { data: cs }, { data: ms }] = await Promise.all([
        supabase.from('gm_fondos').select('id, sucursal_id, custodio_id, monto_asignado, saldo_actual, saldo_inicial, estado').eq('estado', 'activo'),
        ...
        supabase.from('gm_movimientos')
          ...
          // El desempate por 'id' es OBLIGATORIO y debe coincidir con el orden
          // usado al recalcular la cadena (fecha, created_at, id). Sin él, los
          // movimientos migrados en batch (mismo created_at) se muestran en
          // orden arbitrario y la columna Saldo aparenta no encadenar.
  ...
  const guardar = async () => {
    ...
    try {
      ...
      if (esRetroactivo) {
        // Recalcular la cadena completa con el helper compartido
        // (respeta saldo_inicial, orden canónico y afecta_saldo=false)
        const r = await recalcularCadena(form.fondo_id)
        if (!r.ok) throw new Error('Recálculo de saldos falló: ' + r.error)
        saldoFinal = r.saldoFinal
        toast.info(`Fecha retroactiva — ${r.actualizados} saldos recalculados`, { duration: 3000 })
      } else {
        // Fecha al día: flujo rápido, solo actualizar el saldo del fondo
        if (saldoFinal < 0) {
