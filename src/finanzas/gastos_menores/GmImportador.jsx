...
const TIPOS_DOC_VALIDOS = ['Boleta', 'Factura', 'Ticket', 'Guía', 'Comprobante', 'Otro', 'Ingreso de dinero']
...
const normalizarTipoDoc = (s) => {
  ...
  if (n.startsWith('compro'))  return 'Comprobante'
  return 'Otro'
}
  ...
  const onArchivo = async (file) => {
    ...
    try {
      ...
      const colIdx = {
        ...
        tipo_doc:     header.findIndex(h => h.includes('tipo doc') || h.includes('tipo de doc')),
      ...
      for (let i = 1; i < rows.length; i++) {
        ...
        if (monto == null || monto === 0) { errs.push({ fila: i + 1, msg: `Monto inválido (${r[colIdx.monto]})` }); continue }

        // Si es ingreso con monto negativo, sigue siendo ingreso (registra retiro)
        // Si es gasto con monto negativo, lo rechazamos (no tiene sentido lógico)
        if (tipo === 'gasto' && monto < 0) {
          errs.push({ fila: i + 1, msg: `Gasto con monto negativo (${monto})` }); continue
        }
