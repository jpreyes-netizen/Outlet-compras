...
const DOMINIOS = [
  { k: 'vision', l: 'Inicio', Icono: LayoutDashboard, hojas: [
    { k: 'inicio', l: 'Inicio · qué hacer hoy', cap: 'fin.dashboard' },
    { k: 'dashboard', l: 'Dashboard ejecutivo', cap: 'fin.dashboard' },
    { k: 'analisis', l: 'Análisis ejecutivo', cap: 'fin.presupuesto' },
    { k: 'pmo_gestion', l: 'Gestión · tareas y reuniones', cap: null },  // PMO embebido: sin cap → todo usuario con acceso a Finanzas; ve solo lo suyo
  ]},
...
export function FinanzasApp({ cu, setAppActual }) {
  const [tab, setTab] = useState(() => {
    try {
      // Deep link (#go/finanzas/<modulo>/<tab>) manda sobre lo guardado
      if (deepLink?.app === 'finanzas' && deepLink.modulo) {
        const existe = DOMINIOS.some(d => d.hojas.some(h => h.k === deepLink.modulo))
        if (existe) return deepLink.modulo
      }
  ...
  const pasaCap = t => !t.cap ? (capsLoaded || cu?.rol === 'admin') : capsLoaded
  ...
  const contenido = (
    <>
      {tabValido === "inicio" && <FinInicio cu={cu} setTab={setTab} />}
      ...
      {tabValido === "analisis" && <AnalisisEjecutivo cu={cu} />}
      {tabValido === "flujocaja" && <FlujoCajaTab />}
      {tabValido === "gastos" && <GastosShell cu={cu} isMobile={isMobile} />}
      {tabValido === "pmo_gestion" && <PmoGestion cu={cu} area="finanzas" />}
    </>
  )
