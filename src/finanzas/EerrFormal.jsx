...
export function EerrFormal() {
  const [lineas, setLineas] = useState([])
  const [paralelo, setParalelo] = useState([])
  const [modo, setModo] = useState('devengo')
    ...
    (async () => {
      try {
        const [{ data: l }, { data: p, error }] = await Promise.all([
          supabase.from('eerr_lineas').select('codigo, nombre, seccion, tipo_costo, orden, es_subtotal, activo_devengo').eq('activo_devengo', true).order('orden'),
  ...
  const datos = useMemo(() => {
    ...
    paralelo.forEach(r => {
      ...
      d[r.codigo] = d[r.codigo] || new Array(12).fill(0)
      d[r.codigo][i] += Number(modo === 'devengo' ? r.devengo : modo === 'caja' ? r.caja : r.diferencia)
    })
    return d
  }, [paralelo, modo])

  const filas = useMemo(() => construir(lineas, datos), [lineas, datos])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 10 }}>
      <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap' }}>
        {[{ k: 'devengo', l: 'Devengo (contabilidad)' }, { k: 'caja', l: 'Caja (gestión)' }, { k: 'paralelo', l: 'Paralelo: devengo − caja' }].map(v => (
        ...
        <button onClick={exportar} style={{ marginLeft: 'auto', fontSize: 12, padding: '5px 10px', borderRadius: 6, border: `1px solid ${BORDE}`, background: '#fff', cursor: 'pointer', fontWeight: 600, color: NAVY }}>Exportar</button>
      </div>
            ...
            {filas.map(f => {
              ...
              return (
                <>
                  ...
                  <tr key={f.codigo} style={{ background: rowBg }} title={modo === 'paralelo' ? explic[f.codigo] : undefined}>
                    <td style={{ ...TD, fontWeight: esSub ? 700 : 500, color: esSub ? NAVY : esInfo ? SLATE : INK, position: 'sticky', left: 0, background: rowBg, zIndex: 1, paddingLeft: esSub ? 8 : 18 }}>{f.nombre}</td>
                    ...
                    {f.valores.map((v, i) => (
                      <td key={i} style={{ ...NUM, fontWeight: esSub ? 700 : 400, color: esSub ? color : v < 0 && !esInfo && f.nivel === 'detalle' ? '#7F1D1D' : color }}>{fmt(v)}</td>
                    ))}
      ...
      <div style={{ fontSize: 10.5, color: SLATE, lineHeight: 1.5 }}>
        Fijo/variable y orden de líneas vienen del maestro <b>eerr_lineas</b> — el mismo que usa Gestión → EERR Gestión (caja). Cambiar el maestro cambia ambos.
