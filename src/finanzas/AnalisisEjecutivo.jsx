...
export function AnalisisEjecutivo({ cu }) {
  ...
  // Los textos de centro de costo que usa Contaline en la liquidación ('La Granja',
  // 'Maipu', 'Casa Matriz'...) se traducen a ceco vía ceco_alias. Es el mismo mapeo
  // que usan v_eerr_sucursal y v_indicadores_sucursal, así que el drill-down muestra
  // exactamente las filas que componen el número.
  useEffect(() => {
    supabase.from('ceco_alias').select('alias, ceco_id')
      .then(({ data }) => setAlias(data ?? []), () => setAlias([]))
  }, [])
  const textosDe = (ceco) => alias.filter(a => a.ceco_id === ceco).map(a => a.alias)
      ...
      {vista === 'gastosuc' && (
        <>
          <Panel titulo={`Indicadores por tienda · ${rotulo}`} sub="Cada gasto medido contra su propia venta: así se comparan tiendas de distinto tamaño"
                  ...
                  {indVista.map(i => {
                    ...
                    return (
                      <tr key={i.ceco}>
                        ...
                        <td onClick={() => abrirFuente(setDet, { titulo: `Remuneraciones · ${i.ceco_nombre} · ${rotulo}`, sub: `Fuente: rrhh_liquidaciones · centro de costo de la nómina del período · el EERR reparte el total de REM_* en proporción a estos haberes`, query: esAnual ? supabase.from('rrhh_liquidaciones').select('periodo, nombre, cargo, centro_costo_texto, total_haberes').in('centro_costo_texto', textosDe(i.ceco)).order('periodo').order('total_haberes', { ascending: false }) : supabase.from('rrhh_liquidaciones').select('nombre, cargo, centro_costo_texto, total_haberes').eq('periodo', mes).in('centro_costo_texto', textosDe(i.ceco)).order('total_haberes', { ascending: false }) })} style={{ ...NUM, ...CLICK }}>{fmt(i.remuneraciones)}</td>
                        ...
                        <td onClick={() => abrirFuente(setDet, { titulo: `Dotación · ${i.ceco_nombre} · ${rotulo}`, sub: `Fuente: rrhh_liquidaciones · quien recibió liquidación en el período, NO el maestro de empleados`, query: esAnual ? supabase.from('rrhh_liquidaciones').select('periodo, nombre, cargo, centro_costo_texto, total_haberes').in('centro_costo_texto', textosDe(i.ceco)).order('periodo').order('nombre') : supabase.from('rrhh_liquidaciones').select('nombre, cargo, centro_costo_texto, total_haberes').eq('periodo', mes).in('centro_costo_texto', textosDe(i.ceco)).order('nombre') })} style={{ ...NUM, ...CLICK }}>{i.trabajadores ?? '—'}</td>
                        <td onClick={() => abrirFuente(setDet, { titulo: `Venta × trabajador · ${i.ceco_nombre} · ${rotulo}`, sub: `${fmt(i.ingresos)} de venta ÷ ${i.trabajadores ?? 0} trabajadores de la nómina del período`, query: esAnual ? supabase.from('rrhh_liquidaciones').select('periodo, nombre, cargo, centro_costo_texto, total_haberes').in('centro_costo_texto', textosDe(i.ceco)).order('periodo').order('nombre') : supabase.from('rrhh_liquidaciones').select('nombre, cargo, centro_costo_texto, total_haberes').eq('periodo', mes).in('centro_costo_texto', textosDe(i.ceco)).order('nombre') })} style={{ ...NUM, ...CLICK }}>{i.venta_por_trabajador ? fmt(i.venta_por_trabajador) : '—'}</td>
            ...
            <div style={{ fontSize: 10.5, color: SLATE, marginTop: 8, paddingTop: 8, borderTop: `1px solid ${BORDE}`, lineHeight: 1.65 }}>
              <b style={{ color: NAVY }}>Fuente de cada columna</b> · todas vía <code>v_indicadores_sucursal</code><br />
              <b>Venta y margen</b>: <code>bsale_ventas_control</code>, el control oficial de BSALE del período.<br />
              <b>Remuneración</b>: total REM_* del EERR repartido entre centros en proporción a los haberes de <code>rrhh_liquidaciones</code>, usando el centro de costo que traía la nómina de ese mes.<br />
              <b>Dotación</b>: cuántas personas recibieron liquidación en el período, con el centro de costo de esa misma nómina. No se usa el maestro de empleados, que sólo refleja la asignación de hoy y no tiene historia.<br />
              <b>Arriendo y gasto</b>: <code>v_eerr_sucursal_gastos</code>, documentos imputados al centro.<br />
              <b>Mermas y ajuste de inventario</b>: <code>v_merma_clasificada</code> sobre <code>bsale_consumo</code>, la misma fuente que contabiliza el motor de salidas de inventario.
            </div>
