...
export function AnalisisEjecutivo({ cu }) {
  ...
  useEffect(() => {
    Promise.all([
      ...
      supabase.from('v_alertas_variacion').select('*').like('periodo', `${anio}%`),
      supabase.from('v_indicadores_sucursal').select('*').like('periodo', `${anio}%`),
      supabase.from('v_sucursal_detalle_linea').select('*').like('periodo', `${anio}%`).limit(20000),
      ...
      setAlertas(j.data ?? []); setIndSuc(k.data ?? []); setSucLinea(l.data ?? [])
    })
  }, [anio])
  ...
  const cecosMes = useMemo(() => [...new Set(sucLinea.filter(x => x.periodo === mes).map(x => x.ceco_nombre).filter(Boolean))], [sucLinea, mes])
  const lineasSuc = useMemo(() => [...new Set(sucLinea.filter(x => x.periodo === mes && x.grupo !== 'resultado').map(x => x.linea))].sort(), [sucLinea, mes])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {vista === 'gastosuc' && (
        <>
          <Panel titulo={`Indicadores por tienda · ${mes}`} sub="Cada gasto medido contra su propia venta: así se comparan tiendas de distinto tamaño"
            acciones={<Ex filas={indSuc.filter(i => i.periodo === mes)} nombre={`indicadores_tienda_${mes}`} titulo={`Indicadores por tienda ${mes}`} />}>
            <div style={{ overflow: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {indSuc.filter(i => i.periodo === mes).map(i => {
                    const ref = indSuc.filter(x => x.periodo === mes && Number(x.ingresos) > 0)
                    ...
                    return (
                      <tr key={i.ceco}>
                        <td style={{ ...TD, fontWeight: 600 }}>{i.ceco_nombre}</td>
                        ...
                        <td onClick={() => abrirFuente(setDet, { titulo: `Remuneraciones · ${i.ceco_nombre} · ${mes}`, sub: 'Liquidaciones y honorarios del centro', query: supabase.from('v_rrhh_master').select('trabajador, cargo, glosa_nombre, monto').eq('periodo', mes).eq('centro_costo_nombre', i.ceco_nombre).order('monto', { ascending: false }) })} style={{ ...NUM, ...CLICK }}>{fmt(i.remuneraciones)}</td>
                        ...
                        <td onClick={() => abrirFuente(setDet, { titulo: `Arriendo · ${i.ceco_nombre} · ${mes}`, sub: 'Facturas y pagos de arriendo del centro', query: supabase.from('v_eerr_sucursal_gastos').select('periodo, linea_nombre, fuente, monto').eq('ceco', i.ceco).eq('periodo', mes).eq('linea', 'ARRIENDO') })} style={{ ...NUM, ...CLICK }}>{fmt(i.arriendo)}</td>
            ...
            <div style={{ fontSize: 11, color: SLATE, marginTop: 6 }}>Verde: mejor ratio del mes. Rojo: más de 1,5× (remuneración) o 2× (arriendo) el mejor. Clic en los montos abre el detalle del centro.</div>
          </Panel>

          <Panel titulo={`Gasto por línea y tienda · ${mes}`} sub="Cada línea del EERR abierta por centro de costo — clic para ver los documentos"
            acciones={<Ex filas={sucLinea.filter(x => x.periodo === mes)} nombre={`gasto_linea_tienda_${mes}`} titulo={`Gasto por línea y tienda ${mes}`} />}>
            <div style={{ overflow: 'auto' }}>
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                ...
                <tbody>
                  {lineasSuc.map(ln => {
                    const fila = cecosMes.map(c => sucLinea.filter(x => x.periodo === mes && x.ceco_nombre === c && x.linea === ln).reduce((s2, x) => s2 + Number(x.monto || 0), 0))
                    ...
                    return (
                      <tr key={ln}>
                        ...
                        {fila.map((v, i2) => (
                          <td key={i2} onClick={() => v !== 0 && abrirFuente(setDet, { titulo: `${ln} · ${cecosMes[i2]} · ${mes}`, sub: 'Documentos que componen el gasto', query: supabase.from('v_eerr_sucursal_gastos').select('periodo, linea_nombre, fuente, monto').eq('periodo', mes).eq('ceco', (indSuc.find(z => z.ceco_nombre === cecosMes[i2]) || {}).ceco || '').order('monto', { ascending: false }) })}
