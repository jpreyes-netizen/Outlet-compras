...
const VERSION = 'v0.0.68 · mes/año + gasto fino'
...
export function AnalisisEjecutivo({ cu }) {
  ...
  useEffect(() => {
    const FUENTES = [
      ['v_informe_resultado_mensual', setMensual], ['v_eerr_desglose_mes', setDesglose], ['v_estructura_costo', setEstructura],
      ['v_diagnostico_ejecutivo', setDiag], ['v_salud_financiera', setSalud], ['v_punto_equilibrio', null],
      ['v_eerr_sucursal_prorrateado', setSuc], ['v_cobertura_datos', setCobertura], ['v_cobertura_resumen', setCobResumen],
      ['v_alertas_variacion', setAlertas], ['v_indicadores_sucursal', setIndSuc], ['v_sucursal_detalle_linea', setSucLinea],
      ['v_sucursal_anual', setSucAnual],
    ]
    Promise.allSettled([
      ...
      supabase.from('v_punto_equilibrio').select('*'),
      ...
      supabase.from('v_sucursal_anual').select('*'),
    ]).then(res => {
      const errs = []
      res.forEach((r, idx) => {
        const [nombre, setter] = FUENTES[idx]
        if (r.status === 'rejected') { errs.push(`${nombre}: ${r.reason?.message ?? 'sin respuesta'}`); return }
        const { data, error } = r.value ?? {}
        if (error) { errs.push(`${nombre}: ${error.message}`); return }
        if (nombre === 'v_punto_equilibrio') setEquilibrio((data ?? [])[0] ?? null)
        else if (setter) setter(data ?? [])
      })
      setErrores(errs)
    })
  }, [anio])
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
        ...
        <span style={{ fontSize: 11, color: SLATE, marginLeft: 'auto' }}>
          {mensual.length ? `${cerrados.length} meses cerrados · ${indSuc.length} filas de indicadores · ${sucLinea.length} de gasto por tienda` : 'cargando…'}
        </span>
      </div>
      {errores.length > 0 && (
      ...
      {vista === 'sucursal' && (
        <Panel titulo={`Resultado por sucursal · ${rotulo}`} sub="CD Maipú y Casa Matriz no venden pero gastan: con prorrateo su costo se reparte según participación en ventas"
          ...
          <div style={{ fontSize: 11, color: SLATE, marginTop: 6 }}>
            * Centros sin ingreso propio. En la visión prorrateada la suma del resultado final de las tiendas es exactamente el resultado del EERR{esAnual ? ' acumulado' : ' del mes'}.
          </div>
