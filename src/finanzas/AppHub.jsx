...
const APPS_CATALOG = {
  ...
  logistica: {
    l: "Logística", desc: "WMS — reposición, recepción, inventarios y layout de bodega",
    ic: "ti-truck", c1: "#FF9500", c2: "#cc7700",
    bg: "#fff5e6", bgBadge: "#ffe8c2", txt: "#cc7700",
    tabs: ["Reposición","Recepción","Inventario","Layout","BI"]
  },
  comercial: {
    l: "Comercial", desc: "Metas de venta diaria y seguimiento de cotizaciones",
    ic: "ti-chart-bar", c1: "#5856D6", c2: "#3d3ba3",
    bg: "#f3f3fc", bgBadge: "#e5e4f7", txt: "#3d3ba3",
    tabs: ["Metas", "Cotizaciones"]
  },
  agente: {
    l: "Agente IA", desc: "Pregunta sobre compras, finanzas y negocio",
    ic: "ti-robot", c1: "#6366F1", c2: "#4338CA",
    bg: "#f4f4fd", bgBadge: "#e6e6fb", txt: "#4338CA",
    tabs: ["Chat"]
  },
  proyectos: {
    l: "Proyectos", desc: "Proyectos, tareas y control de gestión",
    ic: "ti-checklist", c1: "#3b4a7a", c2: "#16213e",
    bg: "#f4f5f9", bgBadge: "#e6e8f2", txt: "#16213e",
    tabs: ["Panel","Proyectos","Carta Gantt","Derivación"]
  },
  procesos: {
    l: "Procesos", desc: "Matriz de procesos, SOP oficiales y flujogramas",
    ic: "ti-compass", c1: "#3aa99a", c2: "#0f6e62",
    bg: "#f2faf8", bgBadge: "#dcf2ee", txt: "#0f6e62",
    tabs: ["Dashboard","Matriz","Comités","Repositorio"]
  }
}

function appsLegado(rol) {
  if (rol === "admin" || rol === "dir_general") return ["compras","finanzas","postventa","rrhh","admin","inventario","agente","proyectos","comercial","procesos"]
...
export function AppHub({ cu, onSelect, onLogout }) {
  ...
  useEffect(() => {
    ...
    const cargarKPIs = async () => {
      ...
      // ─── PROCESOS ───
      try {
        const { data: prc } = await supabase.from('v_prc_dashboard').select('*')
        const d = (prc || [])[0] || {}
        k.procesos = {
          total: d.total_procesos ?? null,
          enRiesgo: d.en_riesgo ?? null,
          avance: d.avance_ponderado ?? null,
          implementados: d.implementados ?? null
        }
      } catch (e) { k.procesos = {} }
  ...
  const kpisByApp = {
    ...
    procesos: [
      { v: kpis.procesos?.avance != null ? kpis.procesos.avance + '%' : '—', l: 'Avance', highlight: true },
      { v: kpis.procesos?.enRiesgo != null ? fmtNum(kpis.procesos.enRiesgo) : '—', l: 'En riesgo', danger: kpis.procesos?.enRiesgo > 0 },
      { v: kpis.procesos?.total != null ? `${kpis.procesos.implementados ?? 0}/${kpis.procesos.total}` : '—', l: 'Implementados' }
    ]
  }
  ...
  const badgeForApp = (k) => {
    ...
    if (k === 'procesos') {
      const r2 = kpis.procesos?.enRiesgo
      if (r2 > 0) return { txt: `${r2} en riesgo`, dot: true }
      if (kpis.procesos?.total != null) return { txt: 'Al día', dot: true }
      return { txt: 'Matriz', dot: false }
    }
    return null
  }
