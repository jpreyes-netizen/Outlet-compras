...
export function InformeEjecutivo() {
  ...
  function fuenteMensual(m, col) {
    const map = { venta: ['VENTA_NETA'], costo: ['COSTO_NETO'], remuneraciones: ['REM_OPERACION', 'REM_VENTA', 'REM_ADMIN', 'REM_SOCIOS'], arriendo: ['ARRIENDO'], mermas: ['MERMAS'], depreciacion: ['DEPRECIACION'], interes: ['INTERES_CREDITOS'], impuesto: ['IMPUESTO_RENTA'] }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      <Panel titulo={`Resultado mes a mes ${anio}`} sub="Clic en cualquier cifra abre los asientos que la componen"
        acciones={<Ex filas={mensual.map(m => ({ Mes: m.periodo, Venta: m.venta, Costo: m.costo, 'MB %': m.margen_bruto_pct, Remuneraciones: m.remuneraciones, Arriendo: m.arriendo, Mermas: m.mermas, 'Otros gastos': m.otros_gastos, EBITDA: m.ebitda, Depreciación: m.depreciacion, Interés: m.interes, Impuesto: m.impuesto, Resultado: m.resultado }))} nombre={`resultado_mensual_${anio}`} titulo={`Resultado mes a mes ${anio}`} sub="Devengo contable" />}>
        <div style={{ overflow: 'auto' }}>
          <table style={{ width: '100%', borderCollapse: 'collapse' }}>
            <thead><tr>
              <th style={TH}>Mes</th>{['Venta', 'Costo', 'MB %', 'Remuneraciones', 'Arriendo', 'Mermas', 'Otros gastos', 'EBITDA', 'Deprec.', 'Interés', 'Impuesto', 'Resultado', 'Neto %'].map(h => <th key={h} style={{ ...TH, textAlign: 'right' }}>{h}</th>)}
            </tr></thead>
            <tbody>
              {mensual.map(m => (
                <tr key={m.periodo} style={{ background: m.periodo === mes ? '#F0F4FF' : m.mes_en_curso ? '#FFFBEB' : undefined }}>
                  ...
                  <td onClick={() => fuenteMensual(m, 'mermas')} style={{ ...NUM, ...CLICK, color: Number(m.mermas) > 30000000 ? ROJO : INK }}>{fmt(m.mermas)}</td>
                  <td style={NUM}>{fmt(m.otros_gastos)}</td>
                  <td style={{ ...NUM, fontWeight: 600 }}>{fmt(m.ebitda)}</td>
                  ...
                  <td onClick={() => fuenteMensual(m, 'impuesto')} style={{ ...NUM, ...CLICK }}>{fmt(m.impuesto)}</td>
                  <td style={{ ...NUM, fontWeight: 700, color: colorRes(m.resultado) }}>{fmt(m.resultado)}</td>
                  <td style={{ ...NUM, color: colorRes(m.resultado) }}>{m.margen_neto_pct ?? '—'}%</td>
                </tr>
              ))}
              <tr style={{ background: '#EEF2FF' }}>
                ...
                {['remuneraciones', 'arriendo', 'mermas', 'otros_gastos', 'ebitda', 'depreciacion', 'interes', 'impuesto', 'resultado'].map(k => <td key={k} style={{ ...NUM, fontWeight: 700, color: k === 'resultado' ? colorRes(ytdRes) : NAVY }}>{fmt(cerrados.reduce((s, m) => s + Number(m[k]), 0))}</td>)}
                <td style={{ ...NUM, fontWeight: 700, color: colorRes(ytdRes) }}>{ytdVenta ? (100 * ytdRes / ytdVenta).toFixed(1) : '—'}%</td>
              </tr>
            </tbody>
          </table>
        </div>
        <div style={{ fontSize: 11, color: SLATE, marginTop: 6 }}>"Otros gastos" es el residuo que hace cuadrar la fila con el resultado oficial (servicios, TI, marketing, comisiones, cuentas básicas, etc.); su detalle está en el EERR completo. El mes en curso ⌛ no tiene costo, mermas ni depreciación: no es comparable.</div>
      </Panel>
