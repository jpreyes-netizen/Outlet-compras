...
import { AbonosTab } from './tesoreria/AbonosTab'
import { ValidacionesTab } from './tesoreria/ValidacionesTab'
import { deepLink } from '../core/deeplink'
import { ManualDrawer } from './tesoreria/ManualDrawer'

// RBAC-4: sub-tabs vinculados a capabilities
const ALL_TABS = [
  { k: 'cuadratura', l: 'Cuadratura y análisis', cap: 'fin.teso.analisis' },
  ...
  { k: 'abonos',    l: 'Abonos de clientes', cap: 'fin.teso.abonos' },
  { k: 'validaciones', l: 'Validación de pagos', cap: 'fin.teso.validaciones' },
]

export function FinTesoreria({ cu, isMobile, rol }) {
  const [tab, setTab] = useState(() => {
    // Deep link: #go/finanzas/tesoreria/<subtab>
    if (deepLink?.app === 'finanzas' && deepLink.modulo === 'tesoreria' && deepLink.tab) {
      if (ALL_TABS.some(x => x.k === deepLink.tab)) return deepLink.tab
    }
    return 'cierre'
  })
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 2, marginBottom: 16, borderBottom: '1px solid rgba(0,0,0,0.06)', overflowX: 'auto', alignItems: 'center' }}>
        {TABS.map(t => (
        ...
        {/* Botón Manual — siempre visible para cualquier usuario logueado */}
        <button onClick={() => setShowManual(true)}
          title="Manual de operación del módulo Tesorería"
          style={{
            marginLeft: 'auto',
      ...
      {tab === 'abonos'    && <AbonosTab          usuario={usuario} />}
      {tab === 'validaciones' && <ValidacionesTab usuario={usuario} cu={cu} />}

      {showManual && <ManualDrawer onClose={() => setShowManual(false)} />}
    </div>
  )
}
