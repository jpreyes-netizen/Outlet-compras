...
export function ClasificarPanel({ movimiento, cuentas, subcuentas, cecos, respaldoTipos, sugerencia, onClose, onSaved }) {
  ...
  async function handleSave() {
    ...
    try {
      ...
      // Si la sugerencia trae un caso PV y el tipo de respaldo es caso_postventa, vincular el caso
      if (sugerencia?.caso_postventa_id && tipoRespaldo === 'caso_postventa') {
        updateData.caso_postventa_id = sugerencia.caso_postventa_id
      }
      const { error } = await supabase.from('movimientos_bancarios').update(updateData).eq('id', movimiento.id)
    ...
    <div style={{ position: 'fixed', inset: 0, zIndex: 100, display: 'flex', pointerEvents: 'none' }}>
      ...
      <aside style={{
        ...
        <div style={{ flex: 1, overflowY: 'auto', padding: '16px 20px', display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          {/* Tarjeta especial: caso postventa detectado */}
          {sugerencia && sugerencia.fuente === 'caso_postventa' && (
            <div style={{
              borderRadius: 10,
              ...
              <div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 8 }}>
                ...
                <span style={{ fontSize: 10, fontWeight: 700, textTransform: 'uppercase', letterSpacing: '0.06em', color: '#5B21B6' }}>
                  Caso Postventa Detectado · {sugerencia.score ?? 99}% confianza
                </span>
              </div>
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, fontSize: 12, marginBottom: 8 }}>
                <div>
                  <div style={{ fontSize: 10, color: '#7C3AED', fontWeight: 600, marginBottom: 2 }}>CASO</div>
                  <div style={{ fontWeight: 700, color: '#111827' }}>{sugerencia.caso_postventa_numero}</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: '#7C3AED', fontWeight: 600, marginBottom: 2 }}>NC BSALE</div>
                  <div style={{ fontWeight: 700, color: '#111827' }}>{sugerencia.caso_postventa_nc || '—'}</div>
                </div>
                <div style={{ gridColumn: 'span 2' }}>
                  <div style={{ fontSize: 10, color: '#7C3AED', fontWeight: 600, marginBottom: 2 }}>CLIENTE</div>
                  <div style={{ fontWeight: 600, color: '#111827' }}>{sugerencia.caso_postventa_cliente}</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: '#7C3AED', fontWeight: 600, marginBottom: 2 }}>MONTO NC</div>
                  <div style={{ fontWeight: 700, color: '#111827', fontFamily: 'monospace' }}>{formatCLP(sugerencia.caso_postventa_monto || 0)}</div>
                </div>
                <div>
                  <div style={{ fontSize: 10, color: '#7C3AED', fontWeight: 600, marginBottom: 2 }}>RUT</div>
                  <div style={{ fontWeight: 600, color: '#111827', fontFamily: 'monospace' }}>{sugerencia.rut_extraido}</div>
                </div>
              </div>
              <div style={{ padding: '7px 10px', background: '#EDE9FE', borderRadius: 6, fontSize: 11, color: '#6D28D9' }}>
                Selecciona la subcuenta de devolución a clientes. El caso quedará vinculado al movimiento.
              </div>
            </div>
          )}

          {/* Sugerencia IA — con botón aplicar (oculta si es caso postventa, que tiene su tarjeta propia) */}
          {sugerencia && sugerencia.fuente !== 'caso_postventa' && (
            <div style={{
              borderRadius: 10,
