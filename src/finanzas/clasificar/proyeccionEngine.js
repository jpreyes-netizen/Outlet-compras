...
export async function cargarDatosProyeccion(anio) {
  ...
  // Santander: saldo del último movimiento del banco — refleja TODO, incluidos los traspasos a Global66.
  // Global66: saldo real en CLP al costo, manual (cuenta USD; no se reconstruye acá).
  let santanderReal = 0
  const { data: ultMov } = await supabase
    .from('movimientos_bancarios').select('saldo, fecha')
    .not('saldo', 'is', null).order('fecha', { ascending: false }).order('id', { ascending: false }).limit(1)
  if (ultMov?.[0]?.saldo != null) santanderReal = Number(ultMov[0].saldo) || 0
