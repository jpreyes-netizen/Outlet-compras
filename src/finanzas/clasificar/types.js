...
const RUT_REGEX_NODASH = /(?<![\d])(0?\d{8,9})(?![\d])/
...
export function extraerRut(desc) {
  ...
  if (m3) {
    ...
    if (raw.length >= 7 && raw.length <= 9) return `${raw.slice(0,-1)}-${raw.slice(-1).toUpperCase()}`
  }
  return null
}
...
export function calcularScoreSugerencia({ fuente, tipo_regla, aciertos = 0, consistencia }) {
  ...
  // Pago a empleado: match perfecto RUT + empleado activo con subcuenta de pago configurada
  if (fuente === 'empleado_sueldo') return { score: 97, nivel: 'alto', razon: 'Pago a empleado (match RUT con rrhh_empleados)' }
