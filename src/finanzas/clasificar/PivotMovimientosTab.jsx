  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        // Subcuentas que pertenecen a cuentas madre tipo 'traspaso' (transferencias entre
        // cuentas propias, ej. Santander → Global66). NO son gasto económico:
        // se excluyen del Analítico y del árbol, pero SÍ cuentan en Cuadratura banco.
        const cmTraspaso = new Set(cuentas.filter(c => c.tipo === 'traspaso').map(c => c.id))
        const subTraspaso = new Set(subcuentas.filter(s => cmTraspaso.has(s.cuenta_madre_id)).map(s => s.id))
        ...
        if (tipo === 'COMPARATIVO') {
          ...
          for (const m of movs) {
            // En vista ANALÍTICO los traspasos entre cuentas propias se excluyen (no son gasto/ingreso económico).
            // En CUADRATURA sí cuentan: son salida/entrada real de la cuenta bancaria.
            if (!modoCuadratura && m.subcuenta_id && subTraspaso.has(m.subcuenta_id)) continue
        ...
        for (const m of movs) {
          ...
          if (cm.tipo === 'traspaso') continue  // traspasos entre cuentas propias: fuera del análisis económico
