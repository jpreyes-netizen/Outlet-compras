...
export function ClasificarTab() {
  ...
  // Nuevos filtros (fila 2)
  const [filtroMesCalendar, setFiltroMesCalendar] = useState('todos')  // mes de la fecha del movimiento (1-12, año actual implícito)
  const [filtroMesCartola, setFiltroMesCartola] = useState('todos')    // mes_cartola del registro (1-12)
  const [filtroTipo, setFiltroTipo] = useState('todos')                // CARGO / ABONO / todos
  const [filtroMontoDesde, setFiltroMontoDesde] = useState('')         // input numérico
  const [filtroMontoHasta, setFiltroMontoHasta] = useState('')         // input numérico
  const [filtroMontoCategoria, setFiltroMontoCategoria] = useState('todos')  // <10k, 10k-100k, 100k-1M, >1M
  const [filtroCuentaMadre, setFiltroCuentaMadre] = useState('todos')  // id de cuenta madre
  const [filtroSubcuenta, setFiltroSubcuenta] = useState('todos')      // id de subcuenta
  ...
  // Si cambia la cuenta madre seleccionada, resetear el filtro de subcuenta
  // (las subcuentas se filtran por cuenta madre, una de otra madre no aplicaría)
  useEffect(() => { setFiltroSubcuenta('todos') }, [filtroCuentaMadre])
  ...
  const [casosPV, setCasosPV] = useState([])  // Casos postventa nc_transfer recientes (devoluciones a clientes)
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        //      EXCEPCIÓN: merchant tokens (prefijo MERCHANT:) son inequívocos, se permiten con 1 palabra
        ...
        for (const [pat, subs] of conteo) {
          // Filtro 1: patrón ambiguo si tiene <2 palabras (excepto merchant tokens)
          if (!pat.startsWith('MERCHANT:') && pat.split(' ').filter(Boolean).length < 2) continue
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      try {
        ...
        if (filtroMesCartola !== 'todos') q = q.eq('mes_cartola', Number(filtroMesCartola))
        if (filtroTipo !== 'todos') q = q.eq('tipo', filtroTipo)
        if (filtroSubcuenta !== 'todos') q = q.eq('subcuenta_id', filtroSubcuenta)

        // Filtro mes calendar (fecha del movimiento). Se traduce a rango fecha
        if (filtroMesCalendar !== 'todos') {
          const mes = Number(filtroMesCalendar)
        ...
        // Filtro monto: rango personalizado tiene prioridad sobre categoría
        // El monto en BD es signed (negativo = CARGO), pero el filtro es siempre sobre |monto|
        const montoDesde = filtroMontoDesde ? Number(filtroMontoDesde) : null
        const montoHasta = filtroMontoHasta ? Number(filtroMontoHasta) : null
        if (montoDesde !== null && !isNaN(montoDesde) && montoDesde > 0) {
          // |monto| >= montoDesde  ⟺  monto >= desde OR monto <= -desde
          q = q.or(`monto.gte.${montoDesde},monto.lte.${-montoDesde}`)
        }
        if (montoHasta !== null && !isNaN(montoHasta) && montoHasta > 0) {
          // |monto| <= montoHasta  ⟺  monto BETWEEN -hasta AND hasta
          q = q.gte('monto', -montoHasta).lte('monto', montoHasta)
        }
        // Categoría de monto (solo aplica si no hay rango personalizado)
        if (!montoDesde && !montoHasta && filtroMontoCategoria !== 'todos') {
          const rangos = {
            'menos10k': [null, 10000],
          ...
          const [min, max] = rangos[filtroMontoCategoria] || [null, null]
          if (min !== null) q = q.or(`monto.gte.${min},monto.lte.${-min}`)
          if (max !== null) q = q.gte('monto', -max).lte('monto', max)
        }
    ...
    return () => { cancelled = true }
  }, [filtroCartola, filtroEstado, filtroDesde, filtroHasta, busqueda, soloPendientes, filtroMesNominal, filtroMesCalendar, filtroMesCartola, filtroTipo, filtroMontoDesde, filtroMontoHasta, filtroMontoCategoria, filtroSubcuenta, page, pageSize, reloadKey])
  ...
  // ─── Cargar casos PV nc_transfer recientes (últimos 90 días) ───
  // Se cruza con movimientos bancarios para sugerir devolución a clientes
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      try {
        ...
        const { data: f3s } = await supabase
          ...
          .select('caso_id, tipo_resolucion, monto, rut_titular, nombre_titular, bsale_doc_numero, fecha_resolucion')
          .eq('tipo_resolucion', 'nc_transfer')
          .gte('fecha_resolucion', hace90)
          .order('fecha_resolucion', { ascending: false })
        if (cancelled || !f3s?.length) { if (!cancelled) setCasosPV([]); return }
        const casoIds = [...new Set(f3s.map(f => f.caso_id))]
        const { data: casos } = await supabase
          .from('casos_postventa')
          .select('id, numero, cliente_rut, cliente_nombre, estado')
          .in('id', casoIds)
        const casoMap = new Map((casos ?? []).map(c => [c.id, c]))
        const enriched = f3s.map(f => ({ ...f, caso: casoMap.get(f.caso_id) || null })).filter(x => x.caso)
        if (!cancelled) setCasosPV(enriched)
      } catch (e) { /* silencioso, sugerencia es opcional */ }
    })()
    return () => { cancelled = true }
  }, [reloadKey])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // Index de casos PV por RUT normalizado (para match rápido en FUENTE 0)
      const normRut = r => (r || '').replace(/[.-]/g, '').toUpperCase()
      const pvPorRut = new Map()
      for (const f3 of casosPV) {
        const rut = normRut(f3.rut_titular || f3.caso?.cliente_rut || '')
        if (!rut) continue
        if (!pvPorRut.has(rut)) pvPorRut.set(rut, [])
        pvPorRut.get(rut).push(f3)
      }
      ...
      for (const m of pendientes) {
        ...
        // FUENTE 0: caso postventa (nc_transfer) — la más confiable cuando aplica
        // Match: CARGO + RUT en descripción + monto exacto (±$1) + fecha ≤30 días después de resolución
        if (m.tipo === 'CARGO' && rut) {
          const rutNorm = normRut(rut)
          const candidatos = pvPorRut.get(rutNorm) || []
          const montoMov = Math.abs(Number(m.monto) || 0)
          const transferMatch = candidatos.find(f3 => {
            const montoF3 = Number(f3.monto) || 0
            if (Math.abs(montoF3 - montoMov) > 1) return false
            const fechaF3 = new Date(f3.fecha_resolucion)
            const fechaMov = new Date(m.fecha)
            const diffDias = (fechaMov - fechaF3) / 86400000
            return diffDias >= -1 && diffDias <= 30
          })
          if (transferMatch) {
            // Solo asignar subcuenta si el caso ya tiene un mov previo clasificado (aprender)
            // Si no, sugerimos el caso pero dejamos subcuenta_id null para que el usuario elija
            found = {
              subcuenta_id: null,
              subcuenta_nombre: '— Selecciona subcuenta para devolución —',
              ceco_id: null,
              tipo_respaldo: 'caso_postventa',
              fuente: 'caso_postventa',
              regla_id: null,
              rut_extraido: rut,
              tipo_regla: null,
              aciertos: 0,
              caso_postventa_id: transferMatch.caso_id,
              caso_postventa_numero: transferMatch.caso?.numero,
              caso_postventa_cliente: transferMatch.caso?.cliente_nombre || transferMatch.nombre_titular,
              caso_postventa_nc: transferMatch.bsale_doc_numero,
              caso_postventa_monto: transferMatch.monto,
              caso_postventa_fecha: transferMatch.fecha_resolucion,
            }
          }
        }
    ...
    return () => { cancelled = true }
  }, [movs, reglas, subcuentas, patronesHistoricos, casosPV])
  ...
  async function aceptarSugerencia(m) {
    ...
    // Si la sugerencia es caso_postventa sin subcuenta, abrir panel para que usuario elija
    if (s.fuente === 'caso_postventa' && !s.subcuenta_id) {
      setEditing(m)
      return
    }
    const { data: sess } = await supabase.auth.getSession()
    const userId = sess.session?.user?.id ?? null
    const updateData = { subcuenta_id: s.subcuenta_id, ceco_id: s.ceco_id, tipo_respaldo: s.tipo_respaldo, estado: 'clasificado', clasif_ia: true, clasificado_por: userId, clasificado_at: new Date().toISOString() }
    if (s.caso_postventa_id) updateData.caso_postventa_id = s.caso_postventa_id
    const { error } = await supabase.from('movimientos_bancarios').update(updateData).eq('id', m.id)
  ...
  async function autoClasificarMasivo(umbral = 85) {
    ...
    for (const m of movs) {
      ...
      if (s && s.score >= umbral) {
        // Caso postventa sin subcuenta_id NO entra al masivo (requiere elección manual del usuario)
        if (s.fuente === 'caso_postventa' && !s.subcuenta_id) continue
        candidatos.push({ mov: m, sug: s })
      }
    }
    ...
    try {
      ...
      for (let i = 0; i < candidatos.length; i += 10) {
        ...
        const results = await Promise.all(lote.map(({ mov, sug }) => {
          ...
          if (sug.caso_postventa_id) upd.caso_postventa_id = sug.caso_postventa_id
          return supabase.from('movimientos_bancarios').update(upd).eq('id', mov.id)
        }))
  ...
  const [descargandoCartola, setDescargandoCartola] = useState(false)

  async function descargarCartolaAnual() {
    setDescargandoCartola(true)
    try {
      const anio = new Date().getFullYear()
      // Lee la vista enmascarada: el export hereda la confidencialidad de socios
      // (glosas protegidas para roles no directivos), con montos intactos.
      const { data, error } = await supabase.from('v_movimientos_conciliacion')
        .select('fecha, tipo, descripcion, referencia, monto, saldo, estado, mes_cartola, mes_nominal, subcuenta_id, ceco_id, total_aplicado')
        .gte('fecha', `${anio}-01-01`)
        .order('fecha', { ascending: true })
        .order('id', { ascending: true })
        .limit(20000)
      if (error) throw error
      const cecoById = new Map(cecos.map(c => [c.id, c]))
      const filas = (data ?? []).map(m => ({
        ...
        Saldo: m.saldo != null ? Number(m.saldo) : '',
        'Mes cartola': m.mes_cartola ?? '',
        'Mes nominal': m.mes_nominal ?? '',
        Estado: m.estado ?? '',
        Subcuenta: m.subcuenta_id ? (subById.get(m.subcuenta_id)?.nombre ?? '') : '',
        'Centro de costo': m.ceco_id ? (cecoById.get(m.ceco_id)?.nombre ?? '') : '',
        Conciliado: (Number(m.total_aplicado) || 0) > 0 ? 'Sí' : 'No',
      }))
      if (!filas.length) { toast.info(`Sin movimientos registrados en ${anio}`); return }
      const wb = XLSX.utils.book_new()
      XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(filas), `Cartola ${anio}`)
      XLSX.writeFile(wb, `cartola_${anio}_completa.xlsx`)
      toast.success(`Cartola ${anio}: ${filas.length} movimientos descargados`)
    } catch (e) { toast.error(e instanceof Error ? e.message : 'Error descargando cartola') }
    finally { setDescargandoCartola(false) }
  }
  ...
  async function aceptarSugerenciasSeleccionadas() {
    const candidatos = []
    const omitidosPV = []  // casos PV sin subcuenta — requieren elección manual
    for (const id of selected) {
      const m = movs.find(x => x.id === id)
      const s = sugerencias.get(id)
      if (m && s && m.estado === 'pendiente') {
        if (s.fuente === 'caso_postventa' && !s.subcuenta_id) { omitidosPV.push(m); continue }
        candidatos.push({ m, s })
      }
    }
    if (candidatos.length === 0) {
      if (omitidosPV.length > 0) toast.warning(`${omitidosPV.length} caso${omitidosPV.length > 1 ? 's' : ''} de postventa requiere${omitidosPV.length > 1 ? 'n' : ''} elegir subcuenta manualmente`)
      else toast.info('Ningún movimiento seleccionado tiene sugerencia IA')
      return
    }
    const msgConfirm = `¿Aceptar sugerencias IA para ${candidatos.length} movimiento${candidatos.length > 1 ? 's' : ''}?\n\nSe clasificarán automáticamente según la sugerencia de cada uno.` +
      (omitidosPV.length > 0 ? `\n\n⚠️ ${omitidosPV.length} caso${omitidosPV.length > 1 ? 's' : ''} de postventa se omite${omitidosPV.length > 1 ? 'n' : ''} (requiere${omitidosPV.length > 1 ? 'n' : ''} elegir subcuenta manualmente).` : '')
    ...
    try {
      ...
      for (let i = 0; i < candidatos.length; i += 10) {
        ...
        const results = await Promise.all(lote.map(({ m, s }) => {
          ...
          if (s.caso_postventa_id) upd.caso_postventa_id = s.caso_postventa_id
          return supabase.from('movimientos_bancarios').update(upd).eq('id', m.id)
        }))
        for (let j = 0; j < results.length; j++) {
          if (results[j].error) err++
          else {
      ...
      <div style={{ background: '#fff', borderRadius: 10, padding: '14px 16px', boxShadow: '0 1px 4px rgba(0,0,0,0.06)' }}>
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'flex-end' }}>
          ...
          <div style={{ marginLeft: 'auto', display: 'flex', gap: 8, alignItems: 'center' }}>
            <button type="button" onClick={descargarCartolaAnual} disabled={descargandoCartola} title="Descarga todos los movimientos bancarios registrados del año en curso" style={{ display: 'inline-flex', alignItems: 'center', gap: 6, padding: '6px 12px', borderRadius: 6, border: '1px solid #D1D5DB', background: '#fff', fontSize: 12, fontWeight: 500, color: '#374151', cursor: 'pointer', opacity: descargandoCartola ? 0.6 : 1 }}>
              {descargandoCartola ? <Loader2 size={13} /> : <Download size={13} />} Cartola año
            </button>
        ...
        {/* ─── SEGUNDA FILA DE FILTROS (filtros avanzados) ─── */}
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'flex-end', marginTop: 10, paddingTop: 10, borderTop: '1px dashed #E5E7EB' }}>
          {/* Tipo (CARGO/ABONO) */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <label style={{ fontSize: 11, color: '#6B7280' }}>Tipo</label>
            <select style={selectSt} value={filtroTipo} onChange={e => { setPage(0); setFiltroTipo(e.target.value) }}>
              <option value="todos">Todos</option>
          ...
          {/* Mes calendar (mes de la fecha) */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <label style={{ fontSize: 11, color: '#6B7280' }}>Mes calendar</label>
            <select style={selectSt} value={filtroMesCalendar} onChange={e => { setPage(0); setFiltroMesCalendar(e.target.value) }}>
              <option value="todos">Todos</option>
              {MESES_CORTOS.map((m, i) => <option key={i+1} value={String(i+1)}>{i+1} — {m}</option>)}
            </select>
          </div>
          ...
          {/* Mes cartola */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <label style={{ fontSize: 11, color: '#6B7280' }}>Mes cartola</label>
            <select style={selectSt} value={filtroMesCartola} onChange={e => { setPage(0); setFiltroMesCartola(e.target.value) }}>
              <option value="todos">Todos</option>
              {MESES_CORTOS.map((m, i) => <option key={i+1} value={String(i+1)}>{i+1} — {m}</option>)}
            </select>
          </div>

          {/* Monto: rango o categoría */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <label style={{ fontSize: 11, color: '#6B7280' }}>Monto desde</label>
            <input type="number" style={{ ...inputSt, width: 100 }} placeholder="$ min" value={filtroMontoDesde} onChange={e => { setPage(0); setFiltroMontoDesde(e.target.value); if (e.target.value) setFiltroMontoCategoria('todos') }} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <label style={{ fontSize: 11, color: '#6B7280' }}>Monto hasta</label>
            <input type="number" style={{ ...inputSt, width: 100 }} placeholder="$ max" value={filtroMontoHasta} onChange={e => { setPage(0); setFiltroMontoHasta(e.target.value); if (e.target.value) setFiltroMontoCategoria('todos') }} />
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <label style={{ fontSize: 11, color: '#6B7280' }}>O categoría</label>
            <select style={selectSt} value={filtroMontoCategoria} disabled={!!(filtroMontoDesde || filtroMontoHasta)} onChange={e => { setPage(0); setFiltroMontoCategoria(e.target.value) }}>
              <option value="todos">Todos</option>
              <option value="menos10k">Menos de $10k</option>
              <option value="10k-100k">$10k - $100k</option>
              <option value="100k-1m">$100k - $1M</option>
              <option value="mas1m">Más de $1M</option>
            </select>
          </div>

          {/* Clasificación: cuenta madre → subcuenta */}
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <label style={{ fontSize: 11, color: '#6B7280' }}>Cuenta madre</label>
            <select style={selectSt} value={filtroCuentaMadre} onChange={e => { setPage(0); setFiltroCuentaMadre(e.target.value) }}>
              <option value="todos">Todas</option>
              {cuentas.map(c => <option key={c.id} value={c.id}>{c.codigo ? `${c.codigo} · ` : ''}{c.nombre}</option>)}
            </select>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 3 }}>
            <label style={{ fontSize: 11, color: '#6B7280' }}>Subcuenta</label>
            <select style={selectSt} value={filtroSubcuenta} disabled={filtroCuentaMadre === 'todos'} onChange={e => { setPage(0); setFiltroSubcuenta(e.target.value) }}>
              <option value="todos">Todas</option>
              {subcuentas.filter(s => filtroCuentaMadre !== 'todos' && s.cuenta_madre_id === filtroCuentaMadre).map(s => (
          ...
          {(filtroTipo !== 'todos' || filtroMesCalendar !== 'todos' || filtroMesCartola !== 'todos' || filtroMontoDesde || filtroMontoHasta || filtroMontoCategoria !== 'todos' || filtroCuentaMadre !== 'todos' || filtroSubcuenta !== 'todos') && (
            <button type="button"
              onClick={() => {
                setPage(0)
                setFiltroTipo('todos')
                setFiltroMesCalendar('todos')
                setFiltroMesCartola('todos')
                setFiltroMontoDesde('')
                setFiltroMontoHasta('')
                setFiltroMontoCategoria('todos')
                setFiltroCuentaMadre('todos')
                setFiltroSubcuenta('todos')
              }}
              style={{ padding: '6px 12px', borderRadius: 6, border: '1px solid #D1D5DB', background: '#fff', fontSize: 11, fontWeight: 500, color: '#6B7280', cursor: 'pointer', display: 'inline-flex', alignItems: 'center', gap: 4 }}>
              <X size={11} /> Limpiar avanzados
            </button>
          )}
        </div>
      </div>
                  ...
                  <tr key={m.id} onClick={() => setEditing(m)}
                    ...
                    <td style={TD}>
                      {m.estado === 'clasificado' || m.estado === 'conciliado'
                        ...
                        : sug
                          ? (() => {
                              // Badge especial para sugerencias de caso postventa
                              if (sug.fuente === 'caso_postventa') {
                                return (
                                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }} title={`Caso ${sug.caso_postventa_numero} · NC ${sug.caso_postventa_nc || '—'} · ${sug.caso_postventa_cliente}`}>
                                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2, padding: '0px 5px', borderRadius: 3, fontSize: 9, fontWeight: 700, background: '#EDE9FE', color: '#5B21B6', border: '1px solid #C4B5FD' }}>
                                      💜 Postventa {sug.caso_postventa_numero}
                                    </span>
                                    <span style={{ fontSize: 8, fontWeight: 700, padding: '0px 4px', borderRadius: 99, background: '#DCFCE7', color: '#166534', border: '1px solid #86EFAC', fontFamily: 'monospace' }}>
                                      {sug.score}%
                                    </span>
                                  </span>
                                )
