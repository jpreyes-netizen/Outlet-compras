...
export function ClasificarTab() {
  ...
  const [empleados, setEmpleados] = useState([])  // Empleados activos con RUT + subcuenta_pago_default (pagos de sueldo)
  ...
  // ─── Cargar empleados activos con RUT + subcuenta_pago_default ───
  // Se cruza con movimientos bancarios para sugerir pago de sueldo (RUT en descripción)
  useEffect(() => {
    let cancelled = false
    ;(async () => {
      try {
        const { data: emps } = await supabase
          .from('rrhh_empleados')
          .select('cod_contaline, rut, nombre, cuenta_madre_codigo, centro_costo_codigo, subcuenta_pago_default')
          .eq('activo', true)
          .not('rut', 'is', null)
          .not('subcuenta_pago_default', 'is', null)
        if (!cancelled) setEmpleados(emps || [])
      } catch (e) { /* silencioso, sugerencia es opcional */ }
    })()
    return () => { cancelled = true }
  }, [reloadKey])
  ...
  useEffect(() => {
    ...
    ;(async () => {
      ...
      // Index de empleados activos por RUT normalizado (para match rápido en FUENTE 0.5)
      const empPorRut = new Map()
      for (const e of empleados) {
        const rut = normRut(e.rut || '')
        if (!rut || !e.subcuenta_pago_default) continue
        empPorRut.set(rut, e)
      }
      ...
      for (const m of pendientes) {
        ...
        // FUENTE 0.5: pago a empleado — match por RUT con rrhh_empleados
        // Match: CARGO + RUT en descripción coincide con empleado activo + tiene subcuenta_pago_default
        if (!found && m.tipo === 'CARGO' && rut) {
          const rutNorm = normRut(rut)
          const emp = empPorRut.get(rutNorm)
          if (emp && subById.has(emp.subcuenta_pago_default)) {
            const sub = subById.get(emp.subcuenta_pago_default)
            found = {
              subcuenta_id: emp.subcuenta_pago_default,
              subcuenta_nombre: sub.nombre,
              ceco_id: emp.centro_costo_codigo || null,
              tipo_respaldo: 'liquidacion_sueldo',
              fuente: 'empleado_sueldo',
              regla_id: null,
              rut_extraido: rut,
              tipo_regla: null,
              aciertos: 0,
              empleado_nombre: emp.nombre,
              empleado_cod: emp.cod_contaline,
            }
          }
        }
    ...
    return () => { cancelled = true }
  }, [movs, reglas, subcuentas, patronesHistoricos, casosPV, empleados])
                      ...
                      {m.estado === 'clasificado' || m.estado === 'conciliado'
                        ...
                        : sug
                          ? (() => {
                              ...
                              // Badge especial para pago a empleado
                              if (sug.fuente === 'empleado_sueldo') {
                                return (
                                  <span style={{ display: 'inline-flex', alignItems: 'center', gap: 3 }} title={`Pago a empleado #${sug.empleado_cod} · ${sug.empleado_nombre} → ${sug.subcuenta_nombre}`}>
                                    <span style={{ display: 'inline-flex', alignItems: 'center', gap: 2, padding: '0px 5px', borderRadius: 3, fontSize: 9, fontWeight: 700, background: '#DBEAFE', color: '#1E3A8A', border: '1px solid #93C5FD' }}>
                                      🧑 Sueldo · {sug.subcuenta_nombre}
                                    </span>
                                    <span style={{ fontSize: 8, fontWeight: 700, padding: '0px 4px', borderRadius: 99, background: '#DCFCE7', color: '#166534', border: '1px solid #86EFAC', fontFamily: 'monospace' }}>
                                      {sug.score}%
                                    </span>
