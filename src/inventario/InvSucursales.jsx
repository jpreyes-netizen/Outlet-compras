...
export function InvSucursales({ data, accent, isMobile }) {
  ...
  return (
    <div>
      ...
      {ranking.length >= 2 && (
        <div style={card}>
          <SectionTitle>📊 Lectura comparativa</SectionTitle>
          <Lectura icon="🏆" titulo={ranking[0].s + ":"} color="#34C759">
            es la sucursal más eficiente con GMROI <b>{ranking[0].gmroi.toFixed(2)}</b> y rotación {ranking[0].rotValorizada.toFixed(1)}x. Su inventario es el que mejor convierte capital en margen.
          </Lectura>
          {ranking.length >= 2 && ranking[ranking.length - 1].gmroi < ranking[0].gmroi * 0.8 && (
            <Lectura icon="⚠️" titulo={ranking[ranking.length - 1].s + ":"} color="#FF9500">
              tiene el GMROI más bajo (<b>{ranking[ranking.length - 1].gmroi.toFixed(2)}</b>). Su mix de inventario rinde menos — revisar dead stock local y si hay sobre-abastecimiento de productos lentos.
            </Lectura>
          )}
          {kSuc["Maipu"].venta < kSuc["La Granja"].venta * 0.1 && (
            <Lectura icon="📍" titulo="Maipú:" color="#FF9500">
              concentra <b>{fmtMM(kSuc["Maipu"].valorInventario)}</b> de inventario con baja venta directa. Funciona como centro de distribución (CD) que abastece a las otras sucursales — su GMROI no es comparable con un punto de venta maduro.
            </Lectura>
          )}
        </div>
      )}
