...
export function planTienda(an, sucursal) {
  ...
  for (const x of an.items) {
    ...
    if (necesita) {
      // cantidad para llevar hasta el objetivo (reorden + cobertura)
      ...
      const ventaMes = (d.vtaDia || 0) * 30 * (() => {
        // precio unitario aproximado desde la venta del período
        return d.qtyVend > 0 ? d.netoVend / d.qtyVend : 0
      })()

      // ¿cuánto puede ceder Maipú sin quedar bajo su propio reorden?
      ...
      if (dispCD > 0) {
        const transferir = Math.min(cantidad, dispCD)
        pedirAMaipu.push({ ...base, transferir, stockCD: cd.stockUnid })
        // si Maipú no cubre todo, el resto va a compra
        const resto = cantidad - transferir
        if (resto > 0) {
          solicitarCompra.push({ ...base, comprar: resto, stockCD: cd.stockUnid, motivo: "Maipú no cubre todo" })
        }
      } else {
        solicitarCompra.push({
          ...base, comprar: cantidad, stockCD: cd?.stockUnid || 0,
          motivo: (cd && cd.stockUnid > 0) ? "Maipú debe reservar su stock" : "Maipú sin stock",
        })
      }
    }
  ...
  const resumen = {
    ...
    margenProtegido: pedirAMaipu.reduce((s, t) => s + t.margenMes, 0) + solicitarCompra.reduce((s, t) => s + t.margenMes, 0),
  }

  return { sucursal, pedirAMaipu, solicitarCompra, liquidar, porTipo, resumen }
}
