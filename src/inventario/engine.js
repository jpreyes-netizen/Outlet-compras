...
export function sugerencias(analisis, estac) {
  ...
  // Helper: ¿el SKU está descontinuado? (sin stock en NINGUNA sucursal de la red)
  // Si lo está, no tiene sentido sugerir comprarlo ni reponerlo — el negocio
  // ya decidió no comercializarlo. Se excluye de TODAS las recomendaciones.
  const stockRed = (x) => Object.values(x.porSuc || {}).reduce((s, p) => s + (p?.stockUnid || 0), 0)
  const vivo = (x) => stockRed(x) > 0

  // A) Quiebres de clase A/B → comprar YA (priorizado por MARGEN en riesgo)
  const quiebreUrgente = it
    .filter((x) => vivo(x) && x.estado === "Quiebre" && (x.clase === "A" || x.clase === "B"))
...
export function planTienda(an, sucursal) {
  ...
  for (const x of an.items) {
    ...
    // ── DESCONTINUADO: si el SKU no existe en NINGUNA sucursal del stock actual,
    //    es un producto que la empresa decidió descontinuar (típicamente códigos
    //    largos generados al vuelo en POS o productos retirados del catálogo BSALE).
    //    No tiene sentido sugerir "comprar" ni "pedir a Maipú" — el negocio ya
    //    decidió no comercializarlo. Se excluye de las acciones de reposición.
    const stockTotalRed = Object.values(x.porSuc || {}).reduce((s, p) => s + (p?.stockUnid || 0), 0)
    const descontinuado = stockTotalRed === 0
    if (descontinuado) continue
