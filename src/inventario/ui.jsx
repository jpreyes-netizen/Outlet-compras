...
/* Tarjeta de "qué significa" — para interpretación de jefes de tienda */
export function MetricaExplicada({ icon, titulo, valor, color, queEs, queHacer, isMobile }) {
  return (
    <div style={{ background: "#fff", borderRadius: 14, padding: 16, flex: "1 1 240px", minWidth: 0, borderTop: "3px solid " + color }}>
      <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 6 }}>
        <span style={{ fontSize: 20 }}>{icon}</span>
        <div style={{ flex: 1 }}>
          <div style={{ fontSize: 12, color: "#8E8E93", fontWeight: 600 }}>{titulo}</div>
          <div style={{ fontSize: 22, fontWeight: 800, color, lineHeight: 1.1 }}>{valor}</div>
        </div>
      </div>
      <div style={{ fontSize: 11.5, color: "#636366", lineHeight: 1.5, marginBottom: 5 }}>{queEs}</div>
      {queHacer && <div style={{ fontSize: 11.5, color: color, fontWeight: 600, lineHeight: 1.5, background: color + "12", borderRadius: 8, padding: "6px 9px" }}>👉 {queHacer}</div>}
    </div>
  )
}

/* Semáforo simple verde/amarillo/rojo */
export function Semaforo({ estado }) {
  const c = estado === "ok" ? "#34C759" : estado === "alerta" ? "#FF9500" : "#FF3B30"
  const txt = estado === "ok" ? "Bien" : estado === "alerta" ? "Atención" : "Crítico"
  return <span style={{ display: "inline-flex", alignItems: "center", gap: 4, fontSize: 11, fontWeight: 700, color: c }}><span style={{ width: 8, height: 8, borderRadius: "50%", background: c }} />{txt}</span>
}
