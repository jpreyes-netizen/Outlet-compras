...
export function InvMiTienda({ data, sucursal, accent, isMobile, onPickSucursal }) {
  ...
  const [abierto, setAbierto] = useState(() => {
    ...
    return { pedir: true, comprar: false, liquidar: false }  // solo el primero abierto por defecto
  })
  useEffect(() => {
    try { const s = localStorage.getItem(keyAcc); setAbierto(s ? JSON.parse(s) : { pedir: true, comprar: false, liquidar: false }) } catch {}
  }, [sucursal])
  ...
  return (
    <div>
      ...
      <div style={{ display: "flex", gap: 6, marginBottom: 14 }}>
        {[["plan", "📋 Plan de hoy", resumen.nPedir + resumen.nComprar + resumen.nLiquidar],
