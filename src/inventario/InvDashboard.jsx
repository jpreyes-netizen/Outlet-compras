...
export function InvDashboard({ data, accent, isMobile, irA }) {
  const { k, an } = data
  ...
  return (
    <div>
      ...
      <div style={{ ...card, background: "linear-gradient(135deg,#1a1a2e,#16213e)", color: "#fff" }}>
        ...
        {quiebresA.length > 0 && (
          <div style={{ marginTop: 12, padding: "10px 14px", background: "rgba(255,255,255,0.08)", borderRadius: 10, fontSize: 13, display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
            <span><b>🎯 Tu acción #1 hoy:</b> repón los {topUrgentes.length} productos estrella agotados (~{fN(reponerUnidades)} unidades).</span>
            <button onClick={() => irA?.("decision")} style={{ padding: "7px 14px", borderRadius: 9, border: "none", background: "#fff", color: "#1a1a2e", fontWeight: 700, fontSize: 12.5, cursor: "pointer", whiteSpace: "nowrap" }}>Ir a Decisión →</button>
          </div>
        )}
      </div>
      ...
      <div style={card}>
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
          ...
          <div style={{ display: "flex", gap: 6 }}>
            <button onClick={() => irA?.("mitienda")} style={{ padding: "7px 13px", borderRadius: 9, border: "1.5px solid " + accent, background: "#fff", color: accent, fontWeight: 700, fontSize: 12, cursor: "pointer" }}>Plan por tienda →</button>
            <button onClick={() => irA?.("datos")} style={{ padding: "7px 13px", borderRadius: 9, border: "none", background: "#F2F2F7", color: "#636366", fontWeight: 700, fontSize: 12, cursor: "pointer" }}>Ver sábana →</button>
          </div>
        </div>
