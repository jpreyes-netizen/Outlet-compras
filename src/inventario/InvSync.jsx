...
export function InvSync({ accent = ACCENT, isMobile }) {
  ...
  const ejecutarSync = async () => {
    ...
    try {
      ...
      // Ventas: una llamada por tipo de documento por mes (evita timeout en meses densos)
      // Tipos: 39=boleta, 41=boleta no afecta, 33=factura, 34=factura no afecta, 61=NC
      const TIPOS_DOC = [
      ...
      if (tipo !== "stock_only") {
        ...
        addProg(`📅 Procesando ${periodos.length} meses × ${TIPOS_DOC.length} tipos = ${periodos.length * TIPOS_DOC.length} llamadas...`)

        for (const periodo of periodos) {
          let docsMes = 0, filasMes = 0, errMes = 0
          for (const td of TIPOS_DOC) {
            try {
              const r = await llamarEdge(session, {
                tipo: "ventas_only",
