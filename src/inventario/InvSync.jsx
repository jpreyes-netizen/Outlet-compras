...
export function InvSync({ accent = ACCENT, isMobile }) {
  ...
  const llamarEdge = async (session, payload) => {
    const r = await fetch(
  ...
  const ejecutarSync = async () => {
    ...
    try {
      ...
      if (tipo !== "ventas_only") {
        ...
        const r = await llamarEdge(session, { tipo: "stock_only" })
        if (r.ok) {
      ...
      // Ventas: una llamada por mes (evita timeout de 150s)
      if (tipo !== "stock_only") {
        const periodos = generarPeriodos(desde, hasta)
        addProg(`📅 Procesando ${periodos.length} meses de ventas uno por uno...`)

        for (const periodo of periodos) {
          addProg(`  ⏳ ${fmtPeriodo(periodo)}...`)
          try {
            const r = await llamarEdge(session, { tipo: "ventas_only", desde: periodo, hasta: periodo })
            if (r.ok) {
              ...
              totales.duracion_seg += r.duracion_seg || 0
              addProg(`  ✅ ${fmtPeriodo(periodo)}: ${fN(r.docs)} docs · ${fN(r.filas_ventas)} filas (${fmtSeg(r.duracion_seg)})`)
            } else {
              addProg(`  ❌ ${fmtPeriodo(periodo)}: ${r.error}`)
              errores++
            }
          } catch (e) {
            addProg(`  ❌ ${fmtPeriodo(periodo)}: ${e.message}`)
            errores++
          }
        }
      }
