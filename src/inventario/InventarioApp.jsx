  ...
  return (
    <div style={{ fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif", margin:0, padding: isMobile?"0 10px calc(90px + env(safe-area-inset-bottom))":"0 20px calc(80px + env(safe-area-inset-bottom))", background:"#F2F2F7", minHeight:"100vh", fontSize:14 }}>
      ...
      <div className="inv-fade" style={{ display: cargandoSB && !data ? "none" : "block" }}>
        ...
        {(() => {
          if (!dataGlobal || tab === "sync") return null
          const sinNombre = dataGlobal.an.items.filter(x => x.producto === x.sku).length
          if (sinNombre === 0) return null
          return (
            <div style={{ display:"flex", alignItems:"center", justifyContent:"space-between", gap:10, background:"#FF950012", border:"1px solid #FF950040", borderRadius:12, padding:"9px 14px", marginBottom:10, fontSize:12.5 }}>
              <span>🏷️ <b>{sinNombre}</b> productos aparecen con su código porque no tienen nombre registrado.</span>
              <button onClick={() => setTab("sync")} style={{ padding:"6px 12px", borderRadius:8, border:"none", background:"#FF9500", color:"#fff", fontWeight:700, fontSize:12, cursor:"pointer", whiteSpace:"nowrap" }}>Sincronizar nombres →</button>
            </div>
          )
        })()}
