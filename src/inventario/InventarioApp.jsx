...
import {
  ...
  filtrarPorTipo, tiposDeProducto, filtrarPorClase, transferenciasCD
} from './engine'
...
const TABS = [
  ...
  { k: "transfer",   l: "Transferencias CD", ic: "🚚" },
  ...
  useEffect(() => {
    const cargarDesdeSupabase = async () => {
      ...
      try {
        ...
        const prodMap = {}
        ;(prods || []).forEach(p => {
          if (p.sku != null) prodMap[String(p.sku)] = {
            producto: p.producto || "",
        ...
        // Helper: busca metadata de un SKU, probando variantes del código
        const enrich = (sku) => prodMap[String(sku)] || prodMap[String(sku).trim()] || { producto: "", tipo: "", costo: 0 }
  ...
  return (
    <div style={{ fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif", margin:0, padding: isMobile?"0 10px calc(90px + env(safe-area-inset-bottom))":"0 20px calc(80px + env(safe-area-inset-bottom))", background:"#F2F2F7", minHeight:"100vh", fontSize:14 }}>
      ...
      <div style={{ position:"sticky", top:0, zIndex:50, background:"rgba(242,242,247,0.92)", backdropFilter:"blur(20px)", padding: isMobile?"10px 0 0":"14px 0 0", marginBottom:12, borderBottom:"1px solid rgba(0,0,0,0.06)" }}>
        ...
        {data && tab !== "sync" && tab !== "mitienda" && tiposDisponibles.length > 1 && (
        ...
        {data && tab !== "sync" && tab !== "mitienda" && distABCD && (
      ...
      <div className="inv-fade" style={{ display: cargandoSB && !data ? "none" : "block" }}>
        ...
        {tab === "mitienda"   && data && <InvMiTienda       data={dataGlobal} sucursal={sucursalMiTienda} onPickSucursal={setSucursalFiltro} accent={ACCENT} isMobile={isMobile} />}
        {tab === "transfer"   && data && <InvTransferencias data={dataTransfer} destinoFiltro={sucursalFiltro} accent={ACCENT} isMobile={isMobile} />}
...
function EmptyTab({ tab, onUpload, cargando, accent, setTab }) {
  const INFO = {
    ...
    mitienda:   { ic:"🏬", l:"Mi Tienda",       desc:"Centro de mando: qué pedir a Maipú, qué comprar y qué liquidar en tu sucursal." },
    sucursal:   { ic:"🏬", l:"Por Sucursal",    desc:"KPIs comparativos y detalle de stock por cada sucursal." },
    transfer:   { ic:"🚚", l:"Transferencias CD", desc:"Productos que Maipú puede transferir a las sucursales que los necesitan." },
