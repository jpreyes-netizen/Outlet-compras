...
const MAP_SUCURSAL_ID = {
  "suc-lg":    "La Granja",
  "suc-la":    "Los Angeles",
  "suc-maipu": "Maipu",      // Tienda Maipú — punto de venta (abrió 07-ago-2026)
  "suc-mp":    "CD Maipu",   // Centro de distribución — NO vende
  "com-lg":    "La Granja",
  "ops-lg":    "La Granja",
}
// Scope canónico del usuario para la Torre de Control, que trabaja con
// sucursal_id y sí distingue CD de tienda.
const SCOPE_CANONICO = { "suc-lg":"suc-lg", "suc-la":"suc-la", "suc-maipu":"suc-maipu",
...
export function InventarioApp({ cu, setAppActual }) {
  ...
  const [scopeCanonico, setScopeCanonico] = useState(null) // sucursal_id para la Torre
  ...
  useEffect(() => {
    ...
    const detectar = async () => {
      try {
        ...
        setScopeCanonico(SCOPE_CANONICO[s] || null)
  ...
  return (
    <div style={{ fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif", margin:0, padding: isMobile?"0 10px calc(90px + env(safe-area-inset-bottom))":"0 20px calc(80px + env(safe-area-inset-bottom))", background:"#F2F2F7", minHeight:"100vh", fontSize:14 }}>
      ...
      <div style={{ position:"sticky", top:0, zIndex:50, background:"rgba(242,242,247,0.92)", backdropFilter:"blur(20px)", padding: isMobile?"10px 0 0":"14px 0 0", marginBottom:12, borderBottom:"1px solid rgba(0,0,0,0.06)" }}>
        ...
        <div style={{ display:"flex", gap:4, overflowX:"auto", paddingBottom:10 }}>
          {TABS.filter(t => {
            ...
            const bloqueado = !tieneData && !["sync","dashboard","torre","ayuda"].includes(t.k)
            return (
      ...
      <div className="inv-fade" style={{ display: cargandoSB && !data ? "none" : "block" }}>
        ...
        {/* Torre de Control — no depende del Excel: lee las vistas analíticas */}
        {tab === "torre" && <InvTorre scopeUsuario={scopeCanonico} isMobile={isMobile} />}
