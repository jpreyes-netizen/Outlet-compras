...
// ── Normalizador de centros de costo (parche app Gm) ─────────────────────────
// La app Gm (Gastos Menores / Caja chica) usa IDs adicionales en
// usuario_acceso.sucursal_id que NO existen en la tabla sucursales:
//   - com-lg, ops-lg  → sub-sucursales de La Granja
//   - dir-adm, dir-com, dir-neg, dir-ops → direcciones (no son sucursales físicas)
//
// Estos IDs son válidos SOLO para Gm. El resto de los módulos (Tesorería,
// Cierres, etc.) necesitan la sucursal física real. Esta función traduce
// el ID crudo al ID de sucursal física correspondiente.
//
// Para preservar el ID crudo (uso exclusivo de Gm), pasar { raw: true } a
// userScope / userScopeSync.
function normalizarSucursal(rawId) {
  if (!rawId) return rawId
  // com-XX / ops-XX → suc-XX (parche La Granja: com-lg/ops-lg → suc-lg)
  const m = rawId.match(/^(com|ops)-(.+)$/)
  if (m) return 'suc-' + m[2]
  // dir-XXX no es una sucursal física → sin filtro de sucursal
  if (rawId.startsWith('dir-')) return null
  // Ya es suc-XX u otro ID válido → devolver tal cual
  return rawId
}
...
// opts.raw=true → devuelve el ID crudo de usuario_acceso (solo usar en app Gm)
export async function userScope(usuario, appCodigo, capabilityId, opts = {}) {
  ...
  if (scope === 'sucursal') {
    ...
    const raw = cached.sucursal_id || usuario?.sucursal_id || null
    return opts.raw ? raw : normalizarSucursal(raw)
  }
...
// opts.raw=true → devuelve el ID crudo de usuario_acceso (solo usar en app Gm)
export function userScopeSync(usuario, appCodigo, capabilityId, opts = {}) {
