...
export function AdminUsuarios({ cu, isMobile }) {
  ...
  const verComo = async (u) => {
    ...
    if (!confirm(`¿Generar un link para entrar como ${u.nombre}?\n\nEs de un solo uso, expira en 60 min y queda registrado en la auditoría. Ábrelo en una ventana de incógnito: si lo abres en este navegador reemplaza tu sesión.`)) return
    ...
    if (res.ok && res.link) {
      let copiado = false
      try { await navigator.clipboard.writeText(res.link); copiado = true } catch (e) { }
      if (!copiado) window.prompt('Copia este link y ábrelo en una ventana de incógnito:', res.link)
      setMensaje({ tipo: 'ok', txt: 'Link para ' + u.nombre + (copiado ? ' copiado al portapapeles' : ' generado') + '. Ábrelo en incógnito. Expira en 60 min.' })
    } else {
