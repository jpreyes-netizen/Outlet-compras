...
// admin-actions v17 exige el JWT de SESIÓN del usuario: la anon key ya no sirve.
// Siempre devuelve un objeto { ok, error?, ... }, también ante error de red o HTTP.
async function adminAction(accion, payload = {}) {
  try {
    const { data } = await supabase.auth.getSession()
    const token = data?.session?.access_token
    if (!token) return { ok: false, codigo: 'sin_sesion', error: 'Tu sesión expiró. Vuelve a iniciar sesión.' }
    ...
    let j
    try { j = JSON.parse(raw) } catch (e) { j = { ok: false, error: 'Respuesta inesperada del servidor (HTTP ' + res.status + ')' } }
    if (!res.ok && j.ok !== false) j = { ...j, ok: false }
    return j
  } catch (e) {
    return { ok: false, codigo: 'red', error: (e && e.message) || 'Error de red' }
  }
}
...
export function AdminUsuarios({ cu, isMobile }) {
  ...
  const [linkVerComo, setLinkVerComo] = useState(null) // { nombre, correo, link }
  const [credenciales, setCredenciales] = useState(null) // { nombre, correo, password }
  const [creandoCuenta, setCreandoCuenta] = useState(null)
    ...
    try {
      ...
      if (editando) {
        ...
        if (form.correo_original && correoNuevo !== form.correo_original.toLowerCase()) {
          ...
          const resEmail = await adminAction('set_email', { usuario_id: editando, auth_uid: form.auth_uid, email: correoNuevo })
          if (!resEmail.ok) {
        ...
        if (error) throw error
        // v17: crear también la cuenta de acceso (Auth) con clave temporal
        if (payload.activo) {
          const rc = await crearCuenta({ id: nuevoId, nombre: payload.nombre, correo: payload.correo }, true)
          if (!rc) setMensaje({ tipo: "error", txt: "Ficha creada, pero la cuenta de acceso no. Usa el botón 'Crear cuenta' en la fila del usuario." })
          else setMensaje({ tipo: "ok", txt: "Usuario y cuenta de acceso creados. Asigna sus accesos en el tab 'Accesos'." })
        } else {
          setMensaje({ tipo: "ok", txt: "Usuario creado (inactivo, sin cuenta de acceso). Asigna sus accesos en el tab 'Accesos'." })
        }
      }
      await cargar()
      setShowForm(false)
  ...
  // Crea (o vincula) la cuenta de acceso de una ficha que no la tiene y muestra la clave temporal.
  // silencioso=true: no pisa el mensaje general (lo maneja quien llama). Devuelve true si quedó OK.
  const crearCuenta = async (u, silencioso = false) => {
    setCreandoCuenta(u.id)
    try {
      let res = await adminAction('vincular_auth', { usuario_id: u.id })
      if (!res.ok && res.codigo === 'auth_existe') {
        const ult = res.auth?.ultimo_ingreso ? new Date(res.auth.ultimo_ingreso).toLocaleDateString('es-CL') : 'nunca'
        if (!confirm(`Ya existe una cuenta de acceso para ${u.correo} sin ficha asociada (último ingreso: ${ult}).\n\n¿Vincularla a ${u.nombre}? Se le asignará una clave temporal nueva.`)) return false
        res = await adminAction('vincular_auth', { usuario_id: u.id, usar_cuenta_existente: true })
      }
      if (!res.ok) {
        if (!silencioso) setMensaje({ tipo: 'error', txt: res.error || 'No se pudo crear la cuenta de acceso' })
        return false
      }
      setCredenciales({ nombre: u.nombre, correo: res.email || u.correo, password: res.password || null })
      if (!silencioso) {
        await cargar()
        setMensaje({ tipo: 'ok', txt: res.mensaje || 'Cuenta de acceso creada' })
      }
      return true
    } finally {
      setCreandoCuenta(null)
    }
  }
  ...
  const toggleActivo = async (u) => {
    ...
    // v17: la baja bloquea también la cuenta de acceso y corta los accesos a las apps
    // (antes solo cambiaba la ficha y la sesión seguía sirviendo contra la API).
    const texto = u.activo
      ? `¿Desactivar a ${u.nombre}?\n\nSe bloquea su cuenta de acceso y se desactivan todos sus accesos a las apps.`
      : `¿Reactivar a ${u.nombre}?\n\nSe desbloquea su cuenta y se restauran los accesos que tenía al darse de baja (si la baja quedó registrada en este panel).`
    if (!confirm(texto)) return
    const res = u.activo
      ? await adminAction('desactivar', { usuario_id: u.id })
      : await adminAction('reactivar', { usuario_id: u.id, restaurar_accesos: true })
    if (!res.ok) {
      setMensaje({ tipo: "error", txt: res.error || "No se pudo completar la operación" })
      return
    }
    await cargar()
    const adv = (res.advertencias || []).length ? ' · ' + res.advertencias.join(' · ') : ''
    setMensaje({ tipo: adv ? "error" : "ok", txt: (res.mensaje || `Usuario ${u.activo ? 'desactivado' : 'reactivado'}`) + adv })
  }

  // Limpieza única: bloquea la cuenta de acceso de fichas ya inactivas y corta sus accesos.
  // Primero muestra la lista (dry_run) y pide confirmación.
  const sincronizarBajas = async () => {
    setSincronizando(true)
    try {
      const prev = await adminAction('sync_bajas', { dry_run: true })
      if (!prev.ok) { setMensaje({ tipo: 'error', txt: prev.error || 'No se pudo revisar' }); return }
      const lista = prev.a_bloquear || []
      const omitidas = prev.omitidas || []
      if (lista.length === 0 && !prev.accesos_a_desactivar) {
        setMensaje({ tipo: 'ok', txt: 'No hay cuentas de bajas pendientes de bloquear' + (omitidas.length ? ` (${omitidas.length} omitida(s) por cuenta compartida)` : '') })
        return
      }
      const det = lista.map(x => '· ' + x.nombre + (x.correo ? ' (' + x.correo + ')' : '')).join('\n')
      const om = omitidas.map(x => '· ' + x.nombre + ': ' + x.motivo).join('\n')
      if (!confirm(`Se bloquearán ${lista.length} cuenta(s) de fichas inactivas y se desactivarán ${prev.accesos_a_desactivar || 0} acceso(s):\n\n${det || '(ninguna cuenta; solo accesos)'}${om ? '\n\nSe omiten:\n' + om : ''}\n\n¿Continuar?`)) return
      const res = await adminAction('sync_bajas', {})
      if (!res.ok) { setMensaje({ tipo: 'error', txt: res.error || 'Error al bloquear' }); return }
      await cargar()
      setMensaje({ tipo: res.errores ? 'error' : 'ok', txt: `Bloqueadas ${res.bloqueadas} cuenta(s) · ${res.accesos_desactivados} acceso(s) desactivados` + (res.errores ? ` · ${res.errores} con error` : '') })
    } finally {
  ...
  const ejecutarReset = async () => {
    ...
    const res = await adminAction('set_password', { usuario_id: resetUser.id, auth_uid: resetUser.auth_uid, password: resetPass })
  ...
  const verComo = async (u) => {
    ...
    const res = await adminAction('impersonate', { usuario_id: u.id, redirect_to: window.location.origin })
    setImpersonando(null)
    if (res.ok && res.link) {
      // Se muestra en un modal: copiar dentro de un clic es lo único confiable en el navegador
      setLinkVerComo({ nombre: u.nombre, correo: res.email || u.correo, link: res.link })
    } else {
    ...
    <div>
      ...
      <div style={{ background: "var(--bg-surface)", borderRadius: 12, padding: 12, marginBottom: 12, border: "1px solid rgba(0,0,0,0.04)" }}>
        <div style={{ display: "flex", flexWrap: "wrap", gap: 8, alignItems: "center" }}>
          ...
          {cu?.rol === 'admin' && (
            <button onClick={sincronizarBajas} disabled={sincronizando} title="Bloquea la cuenta de acceso de las fichas inactivas y desactiva sus accesos" style={{ padding: "9px 14px", borderRadius: 10, background: "var(--danger-bg)", color: "var(--danger)", border: "none", fontSize: 13, fontWeight: 600, cursor: sincronizando ? "default" : "pointer", opacity: sincronizando ? 0.6 : 1 }}>
              {sincronizando ? "Revisando..." : "Bloquear cuentas de bajas"}
            </button>
          )}
                  ...
                  <tr key={u.id} style={{ borderBottom: "1px solid var(--bg-hover)", opacity: u.activo ? 1 : 0.5 }}>
                    ...
                    <td style={{ padding: "10px 14px", textAlign: "right" }}>
                      <div style={{ display: "inline-flex", gap: 4 }}>
                        ...
                        <button onClick={() => crearCuenta(u)} disabled={creandoCuenta === u.id} title="Esta ficha no tiene cuenta de acceso: crearla con clave temporal" style={{ padding: "6px 10px", borderRadius: 8, background: "var(--warning-bg)", color: "var(--warning-text)", border: "none", cursor: creandoCuenta === u.id ? "default" : "pointer", fontSize: 12, fontWeight: 600, opacity: creandoCuenta === u.id ? 0.5 : 1 }}>{creandoCuenta === u.id ? "Creando..." : "Crear cuenta"}</button>
                        ) : null}
      ...
      {/* MODAL CREDENCIALES (cuenta recién creada) */}
      {credenciales && (
        <ModalCredenciales data={credenciales} onClose={() => setCredenciales(null)} />
      )}

      {/* MODAL VER COMO */}
      {linkVerComo && (
        <ModalVerComo data={linkVerComo} onClose={() => setLinkVerComo(null)} />
      )}
...
function ModalCredenciales({ data, onClose }) {
  const [copiado, setCopiado] = useState(false)
  const texto = `Acceso ERP Outlet de Puertas\nhttps://outletdepuertas-erp.netlify.app\nUsuario: ${data.correo}\nClave temporal: ${data.password}`
  const copiar = async () => {
    let ok = false
    try { await navigator.clipboard.writeText(texto); ok = true } catch (e) { }
    if (!ok) { const el = document.getElementById('txt-credenciales'); if (el) { el.select(); try { ok = document.execCommand('copy') } catch (e) { } } }
    setCopiado(ok)
    if (!ok) alert('No se pudo copiar automáticamente. Selecciona el texto y cópialo con Ctrl+C.')
  }
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 210, padding: 20 }}>
      <div style={{ background: 'var(--bg-surface)', borderRadius: 20, padding: 28, width: '100%', maxWidth: 460 }}>
        <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-primary)' }}>Cuenta de acceso creada</div>
        <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2, marginBottom: 14 }}>{data.nombre}</div>
        {data.password ? (<>
          <div style={{ padding: 12, background: 'var(--warning-bg)', borderRadius: 10, marginBottom: 12, fontSize: 12, color: 'var(--warning-text)' }}>
            Esta clave se muestra una sola vez. Entrégala por WhatsApp o en persona; el usuario puede cambiarla desde el AppHub.
          </div>
          <textarea id="txt-credenciales" readOnly value={texto} rows={4} onFocus={e => e.target.select()}
            style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1px solid var(--border-2)', fontSize: 13, fontFamily: 'monospace', boxSizing: 'border-box', marginBottom: 16, resize: 'none', color: 'var(--text-primary)', background: 'var(--bg-hover)' }} />
        </>) : (
          <div style={{ fontSize: 13, color: 'var(--text-secondary)', marginBottom: 16 }}>Cuenta vinculada: {data.correo}. Usa 🔑 para definirle una clave.</div>
        )}
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          {data.password && <button onClick={copiar} style={{ padding: '10px 16px', borderRadius: 10, border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', background: copiado ? 'var(--success)' : 'var(--accent)', color: 'var(--bg-surface)' }}>{copiado ? 'Copiado ✓' : 'Copiar datos'}</button>}
          <button onClick={onClose} style={{ padding: '10px 16px', borderRadius: 10, border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer', background: 'var(--bg-hover)', color: 'var(--text-secondary)' }}>Listo</button>
        </div>
      </div>
    </div>
  )
...
function ModalVerComo({ data, onClose }) {
  const [copiado, setCopiado] = useState(false)
  const copiar = async (e) => {
    let ok = false
    try { await navigator.clipboard.writeText(data.link); ok = true } catch (err) { }
    if (!ok) {
      // Respaldo: seleccionar el texto del campo y copiar con execCommand
      const el = document.getElementById('link-ver-como')
      if (el) { el.select(); try { ok = document.execCommand('copy') } catch (err) { } }
    }
    setCopiado(ok)
    if (!ok) alert('No se pudo copiar automáticamente. Selecciona el link del recuadro y cópialo con Ctrl+C.')
  }
  const abrirAqui = () => {
    if (!confirm(`Se cerrará TU sesión en este navegador y entrarás como ${data.nombre}.\n\nPara volver a tu usuario tendrás que cerrar sesión e ingresar de nuevo. ¿Continuar?`)) return
    window.location.href = data.link
  }
  const btn = { padding: '10px 16px', borderRadius: 10, border: 'none', fontSize: 13, fontWeight: 600, cursor: 'pointer' }
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.4)', backdropFilter: 'blur(8px)', display: 'flex', alignItems: 'center', justifyContent: 'center', zIndex: 200, padding: 20 }}>
      <div onClick={e => e.stopPropagation()} style={{ background: 'var(--bg-surface)', borderRadius: 20, padding: 28, width: '100%', maxWidth: 520 }}>
        <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', marginBottom: 16 }}>
          <div>
            <div style={{ fontSize: 18, fontWeight: 700, color: 'var(--text-primary)' }}>Entrar como {data.nombre}</div>
            <div style={{ fontSize: 12, color: 'var(--text-muted)', marginTop: 2 }}>{data.correo} · link de un solo uso, expira en 60 min · queda en la auditoría</div>
          </div>
          <button onClick={onClose} style={{ width: 32, height: 32, borderRadius: 16, background: 'var(--bg-hover)', border: 'none', cursor: 'pointer', fontSize: 14 }}>×</button>
        </div>
        <div style={{ fontSize: 13, color: 'var(--text-secondary)', lineHeight: 1.6, marginBottom: 12 }}>
          1. Copia el link.<br />
          2. Abre una ventana de incógnito (Ctrl+Shift+N en Chrome).<br />
          3. Pega el link en la barra de direcciones. Tu sesión aquí no se toca.
        </div>
        <input
          id="link-ver-como"
          readOnly
          ...
          style={{ width: '100%', padding: '10px 12px', borderRadius: 10, border: '1px solid var(--border-2)', fontSize: 12, fontFamily: 'monospace', boxSizing: 'border-box', marginBottom: 16, color: 'var(--text-primary)', background: 'var(--bg-hover)' }}
        />
        <div style={{ display: 'flex', gap: 8, justifyContent: 'space-between', flexWrap: 'wrap' }}>
          <button onClick={abrirAqui} style={{ ...btn, background: 'var(--bg-hover)', color: 'var(--text-secondary)' }}>Abrir en esta ventana</button>
          <button onClick={copiar} style={{ ...btn, background: copiado ? 'var(--success)' : 'var(--accent)', color: 'var(--bg-surface)' }}>
