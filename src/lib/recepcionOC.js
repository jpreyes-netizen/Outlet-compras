...
export async function registrarRecepcionEnCompras({ ocId, tipoDoc, numeroDoc, fecha, items, cu, rolLabel, notas, archivoUrl = null, archivoNombre = null, logRecId = null, resumenInforme = null }) {
  ...
  const { error: eF } = await supabase.from('firmas').insert({
    ...
    accion: `${labelTipo} #${(numeroDoc || 'S/N').trim()} registrada desde Logística: ${totalCant} uds · ${items.length} SKU(s)` + (resumenInforme ? ` — ${resumenInforme}` : ''),
