...
//                       fase_actual (Importación=11 / Nacional=6 → paso 'Cierre'), fecha_real_recepcion,
...
export async function fetchArribosOC() {
  ...
  return ocs.map(oc => {
    ...
    return {
      ...
      pct: totalPedido > 0 ? Math.min(totalPendiente > 0 ? 99 : 100, Math.round(totalRecibido / totalPedido * 100)) : 0,
...
// ── Informe de discrepancias vs OC (llegó / faltante / exceso) ───────────────
// items: [{sku, producto, cantidad}] de ESTA recepción, ANTES de insertarla en
// Compras (el acumulado previo no la incluye). Estado por línea según el saldo
// GLOBAL de la OC tras esta entrega: completo (0) / faltante (>0) / exceso (<0).
// Nunca lanza: ante error devuelve null (el informe es mejor-esfuerzo, la
// recepción no se bloquea por él).
export async function generarInformeOC(ocId, items) {
  try {
    const { data: ocItems, error: e1 } = await supabase.from('oc_items')
      .select('sku,producto,cantidad_pedida').eq('oc_id', ocId)
    if (e1 || !ocItems?.length) return null
    const acc = await fetchAcumuladoOC(ocId)
    const ahora = {}
    ;(items || []).forEach(it => { ahora[it.sku] = (ahora[it.sku] || 0) + Number(it.cantidad || 0) })

    const lineas = ocItems.map(i => {
      const pedido   = Number(i.cantidad_pedida || 0)
      const previo   = acc[i.sku] || 0
      const recAhora = ahora[i.sku] || 0
      const saldo    = pedido - previo - recAhora
      return {
        sku: i.sku, producto: i.producto || i.sku,
        pedido, recibido_previo: previo, recibido_ahora: recAhora, saldo,
        estado: saldo < 0 ? 'exceso' : saldo === 0 ? 'completo' : 'faltante',
      }
    })
    const fueraOC = (items || []).filter(it => !ocItems.some(i => i.sku === it.sku))
      .map(it => ({ sku: it.sku, producto: it.producto || it.sku, cantidad: Number(it.cantidad || 0) }))

    const completas = lineas.filter(l => l.estado === 'completo').length
    const faltantes = lineas.filter(l => l.saldo > 0)
    const excesos   = lineas.filter(l => l.saldo < 0)
    const udsFaltan = faltantes.reduce((s, l) => s + l.saldo, 0)
    const udsExceso = excesos.reduce((s, l) => s - l.saldo, 0)

    const partes = [`${completas}/${lineas.length} líneas completas`]
    if (faltantes.length) partes.push(`faltan ${udsFaltan} uds (${faltantes.length} SKU)`)
    if (excesos.length)   partes.push(`exceso ${udsExceso} uds (${excesos.length} SKU)`)
    if (fueraOC.length)   partes.push(`${fueraOC.length} SKU fuera de OC`)

    return {
      oc_id: ocId, generado_at: new Date().toISOString(),
      lineas, fuera_oc: fueraOC,
      resumen: {
        total_lineas: lineas.length, completas,
        faltantes: faltantes.length, excesos: excesos.length,
        uds_faltantes: udsFaltan, uds_exceso: udsExceso,
        texto: partes.join(' · '),
      },
    }
  } catch (e) { console.error('[generarInformeOC]', e); return null }
}

// ── Espejo completo hacia Compras ───────────────────────────────────────────
// items: [{sku, producto, cantidad}] · Devuelve {pct, estado}
export async function registrarRecepcionEnCompras({ ocId, tipoDoc, numeroDoc, fecha, items, cu, rolLabel, notas, archivoUrl = null, archivoNombre = null, logRecId = null, resumenInforme = null, permitirDiferencias = false }) {
  ...
  // Revalidación defensiva. Con permitirDiferencias (el usuario YA confirmó el
  // excedente/faltante en la UI) no se lanza: los SKU ajenos a la OC quedan
  // fuera del espejo (registrados en la nota) y los excesos pasan tal cual.
  const porSku = {}
  ;(ocItems || []).forEach(i => { porSku[i.sku] = i })
  const fueraOC = []
  for (const it of items) {
    const ref = porSku[it.sku]
    if (!ref) {
      if (!permitirDiferencias) throw new Error(`${it.sku} no pertenece a la OC ${ocId}`)
      fueraOC.push(it); continue
    }
    const pend = Number(ref.cantidad_pedida || 0) - (acc[it.sku] || 0)
    if (Number(it.cantidad || 0) > pend && !permitirDiferencias)
      throw new Error(`${it.sku}: recibiendo ${it.cantidad}, pendiente ${pend}`)
  }
  const itemsOC = items.filter(it => porSku[it.sku])
  // Nada que espejar: TODOS los SKU son ajenos a esta OC. La recepción queda
  // en Logística; en Compras no se crea una recepción vacía.
  if (!itemsOC.length) return { sinCoincidencias: true, fueraOC: fueraOC.length, estado: 'sin coincidencias con la OC', pct: null }

  const notaFuera = fueraOC.length
    ? ` · [Fuera de OC, no espejado: ${fueraOC.map(f => `${f.sku}×${f.cantidad}`).join(', ')}]`
    : ''
  const tipo = mapTipoDocACompras(tipoDoc)
  const totalCant = itemsOC.reduce((s, i) => s + Number(i.cantidad || 0), 0)

  // ── Anti-duplicado: si la analista ya registró este documento en la OC,
  //    NO se crea una gemela — se vincula la existente y se sale. Evita
  //    duplicar cantidades en el acumulado de la OC.
  const escRx = (t) => String(t||'').trim().replace(/[.*+?^${}()|[\]\\]/g, '\\$&')
  const nd = (numeroDoc||'').trim()
  if (nd) {
    const { data: prevs } = await supabase.from('recepciones')
      .select('id, numero_doc, log_recepcion_id, archivo_url').eq('oc_id', ocId)
    const dup = (prevs||[]).find(p => {
      const pd = (p.numero_doc||'').trim(); if (!pd) return false
      return new RegExp(`(^|[^0-9])${escRx(nd)}([^0-9]|$)`).test(pd)
          || new RegExp(`(^|[^0-9])${escRx(pd)}([^0-9]|$)`).test(nd)
    })
    if (dup) {
      const patch = {}
      if (logRecId && !dup.log_recepcion_id) patch.log_recepcion_id = logRecId
      if (archivoUrl && !dup.archivo_url) { patch.archivo_url = archivoUrl; patch.archivo_nombre = archivoNombre }
      if (Object.keys(patch).length) await supabase.from('recepciones').update(patch).eq('id', dup.id)
      return { yaRegistrada: true, numeroExistente: dup.numero_doc, estado: 'ya registrada', pct: null }
    }
  }
  ...
  const { error: eR } = await supabase.from('recepciones').insert({
    ...
    archivo_url: archivoUrl, archivo_nombre: archivoNombre, log_recepcion_id: logRecId,
    notas: (notas ? `[Logística] ${notas}` : '[Logística] Recepción registrada desde el módulo Logística') + notaFuera,
    registrado_por: cu?.id || null,
  })
  if (eR) throw eR

  // 2 · Detalle por SKU
  const rows = itemsOC.map(it => ({
  ...
  const { error: eF } = await supabase.from('firmas').insert({
    ...
    accion: `${labelTipo} #${(numeroDoc || 'S/N').trim()} registrada desde Logística: ${totalCant} uds · ${itemsOC.length} SKU(s)` + (fueraOC.length ? ` (+${fueraOC.length} SKU fuera de OC, no espejados)` : '') + (resumenInforme ? ` — ${resumenInforme}` : ''),
  ...
  const nuevoAcc = { ...acc }
  itemsOC.forEach(it => { nuevoAcc[it.sku] = (nuevoAcc[it.sku] || 0) + Number(it.cantidad || 0) })
  ...
  let up = null
  // fase_actual apunta al paso EN CURSO. Recibida ⇒ queda pendiente 'Cierre':
  // FN[6]='Cierre' (7 fases) · FI[11]='Cierre' (12 fases). El 13 anterior se
  // salía del arreglo y pintaba la barra completa: una OC recibida se veía cerrada.
  if (pct >= 100) up = { estado: 'Recibida OK', fase_actual: esImp ? 11 : 6, fecha_real_recepcion: fecha, recepcion_cuadrada: true, recepcion_forzada: false }
  else if (pct > 0) up = { estado: 'Recibida parcial', fase_actual: esImp ? 11 : 6, fecha_real_recepcion: oc.fecha_real_recepcion || fecha, recepcion_cuadrada: false }
  if (up) {
