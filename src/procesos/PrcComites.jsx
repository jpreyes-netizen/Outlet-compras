...
import { PrcArranque } from './PrcArranque'
...
export function PrcComites({ matriz, cat, cu, onAbrir, toast, onIrConfig, onRecargar }) {
  const [vista, setVista] = useState(() => { try { const v = localStorage.getItem('prc_vista_comite'); return (v === 'arranque' || VISTAS.some(x => x.k === v)) ? v : 'calendario' } catch { return 'calendario' } })
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
      ...
      {vista !== 'arranque' && (() => { const inc = (cat.comites || []).filter(c => (c.integrantes || []).length < 3 || !c.secretario)
        return inc.length > 0 ? (
          <div style={{ padding: '9px 12px', borderRadius: 9, background: 'var(--warning-bg)', color: 'var(--warning-text)', fontSize: 12.5, borderLeft: '3px solid var(--warning)', display: 'flex', gap: 9, alignItems: 'center', flexWrap: 'wrap' }}>
            <span style={{ flex: 1, minWidth: 240 }}><b>{inc.length} comité(s) con reglamento incompleto</b> (sin secretario o con menos de 3 integrantes): {inc.map(c => c.codigo).join(' · ')}. Sin integrantes no hay quórum ni convocatoria sugerida.</span>
            <Bt sm onClick={() => cambiarVista('arranque')}>🏛️ Constituirlos ahora →</Bt>
            {onIrConfig && <Bt sm v="sec" onClick={onIrConfig}>Uno por uno en Config</Bt>}
          </div>
        ) : null })()}

      {vista === 'arranque' && <PrcArranque cat={cat} matriz={matriz} cu={cu} toast={toast}
        onCerrar={() => { cambiarVista('calendario'); onRecargar?.() }}
        onListo={d => { if (d === 'calendario') { cambiarVista('calendario'); onRecargar?.() } }} />}
