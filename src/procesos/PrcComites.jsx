...
import { Cd, Bt, Bd, Sheet, Vacio, Tabs, css, hoy, fFecha, uid, puedeEditar, SEMAFORO, Ayuda } from './prcUI'
...
const VISTAS = [
  { k: 'calendario', l: 'Calendario de sesiones', ic: '📅' },
  { k: 'agenda', l: 'Agenda y acuerdos', ic: '🤝' }
]

export function PrcComites({ matriz, cat, cu, onAbrir, toast }) {
  const [vista, setVista] = useState(() => { try { return localStorage.getItem('prc_vista_comite') || 'calendario' } catch { return 'calendario' } })
  ...
  const cargar = useCallback(async () => {
    setLoading(true)
    const [a, d, s2] = await Promise.all([
      ...
      supabase.from('v_prc_sesiones').select('id, comite_codigo, fecha, tema, estado').order('fecha', { ascending: false })
    ])
    setAgenda(a.data || []); setDocs(d.data || []); setSesiones(s2.data || []); setLoading(false)
  }, [])
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
      <div style={{ borderBottom: '1px solid var(--border-1)' }}>
        <Tabs sm tabs={VISTAS} val={vista} onChange={cambiarVista} />
      </div>

      {vista === 'calendario' && (
        <PrcCalendario matriz={matriz} cat={cat} cu={cu} onAbrir={onAbrir} toast={toast} />
      )}

      {vista === 'agenda' && (<>
      <Ayuda k="comites" titulo="Para qué sirve esta vista">
        Cada comité tiene procesos asignados y una agenda. Antes de la reunión, mira <b>Pendiente de decisión</b>:
        son los SOP que están esperando aprobación y las revisiones semestrales vencidas de ese comité. Durante la
        reunión, usa <b>Registrar acuerdo</b> para dejar cada decisión con responsable y fecha de compromiso. Los
        acuerdos vencidos sin cerrar se marcan en rojo, y si el acuerdo va asociado a un proceso también queda en su
        bitácora.
      </Ayuda>
      ...
      {c && (
        <Cd>
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'flex-start' }}>
            ...
            <Bt dis={!editable} title="Deja por escrito una decisión del comité con responsable y fecha de compromiso"
              onClick={() => { setForm({ fecha: hoy(), tipo: 'SEGUIMIENTO', responsable: cu?.nombre }); setSheet(true) }}>
              Registrar acuerdo
            </Bt>
          </div>
        </Cd>
      ...
      <Sheet open={sheet} onClose={() => setSheet(false)} title={`Registrar acuerdo · ${c?.nombre || ''}`}>
        <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
          ...
          <Campo l="Sesión del calendario (opcional)">
            <select style={{ ...css.input, cursor: 'pointer' }} value={form.sesion_id || ''}
              onChange={e => {
                const sx = sesionesComite.find(x => x.id === e.target.value)
                setForm({ ...form, sesion_id: e.target.value, fecha: sx ? sx.fecha : form.fecha })
              }}>
              <option value="">Sin sesión asociada</option>
              {sesionesComite.map(sx => (
                <option key={sx.id} value={sx.id}>{fFecha(sx.fecha)}{sx.tema ? ' · ' + sx.tema : ''}</option>
              ))}
            </select>
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: 3 }}>
              Si eliges una sesión, el acuerdo aparece dentro de ella en el calendario y toma su fecha.
            </div>
          </Campo>
