...
export function PrcComites({ matriz, cat, cu, onAbrir, toast, onIrConfig }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
      ...
      {(() => { const inc = (cat.comites || []).filter(c => (c.integrantes || []).length < 3 || !c.secretario)
        return inc.length > 0 ? (
          <div style={{ padding: '9px 12px', borderRadius: 9, background: 'var(--warning-bg)', color: 'var(--warning-text)', fontSize: 12.5, borderLeft: '3px solid var(--warning)', display: 'flex', gap: 9, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            {onIrConfig && <Bt sm v="sec" onClick={onIrConfig}>Completar en Config →</Bt>}
          </div>
        ) : null })()}
