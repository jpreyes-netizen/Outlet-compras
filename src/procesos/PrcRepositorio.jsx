...
import { Cd, Bt, Bd, Sheet, Markdown, Vacio, css, fFecha, descargar, Ayuda } from './prcUI'
...
export function PrcRepositorio({ matriz, cu, onAbrir, toast }) {
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
      <Ayuda k="repo" titulo="Qué es el repositorio">
        Todos los documentos de todos los procesos, con su historial de versiones. Es la fuente única: si alguien
        pregunta "cuál es el procedimiento oficial de postventa", la respuesta es la fila marcada <b>vigente</b>.
        Los dos contadores de la derecha son alarmas: <b>vigentes sin aprobador</b> son documentos publicados sin
        firma —no deberían existir— y <b>revisión vencida</b> son SOP que pasaron su plazo de 6 meses sin revisarse.
        El botón <b>Ver</b> abre el documento y la flecha ↓ lo descarga.
      </Ayuda>
              ...
              return (
                <tr key={d.id} style={{ background: d.es_vigente ? 'var(--bg-surface)' : 'transparent' }}>
                  ...
                  <td style={{ ...css.td, whiteSpace: 'nowrap' }}>
                    {d.contenido_md && <Bt v="ghost" sm title="Ver el documento completo" onClick={() => setVer(d)}>Ver</Bt>}
                    {d.contenido_md && <Bt v="ghost" sm title="Descargar el documento" onClick={() => descargar(d.nombre_archivo, d.contenido_md, 'text/markdown;charset=utf-8')}>↓</Bt>}
