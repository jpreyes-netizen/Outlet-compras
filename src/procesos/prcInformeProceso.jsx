...
const LEYENDA = `<div class="ley">
  <span><svg width="15" height="15" viewBox="-8 -8 16 16"><circle r="6.5" fill="#D5E8D4" stroke="#82B366" stroke-width="1.6"/><path d="M -1.9 -2.8 L 3 0 L -1.9 2.8 Z" fill="#1F5B24"/></svg>Inicio</span>
  <span><svg width="15" height="15" viewBox="-8 -8 16 16"><circle r="6.3" fill="#F8CECC" stroke="#B85450" stroke-width="2.4"/><circle r="2.3" fill="#7C2B25"/></svg>Fin</span>
  <span><svg width="16" height="16" viewBox="-9 -9 18 18"><polygon points="0,-8 8,0 0,8 -8,0" fill="#FFF2CC" stroke="#D6B656" stroke-width="1.5"/><path d="M -2.2 -2.2 L 2.2 2.2 M 2.2 -2.2 L -2.2 2.2" stroke="#B8860B" stroke-width="1.6" stroke-linecap="round"/></svg>Decisión (SÍ / NO)</span>
  <span><svg width="14" height="15" viewBox="-7 -8 14 16"><path d="M -5 -6.5 h 6.2 l 3.8 3.8 v 9.2 h -10 z" fill="#DAE8FC" stroke="#6C8EBF" stroke-width="1.2"/><path d="M 1.2 -6.5 v 3.8 h 3.8" fill="none" stroke="#6C8EBF" stroke-width="1.2"/></svg>Genera documento</span>
  <span><svg width="15" height="15" viewBox="-8 -8 16 16"><circle r="7" fill="#fff" stroke="#B85450" stroke-width="1.1"/><path d="M 0 -4.2 L 3.2 -2.9 V 0.4 Q 3.2 3 0 4.3 Q -3.2 3 -3.2 0.4 V -2.9 Z" fill="#B85450"/></svg>Control crítico</span>
  <span><svg width="15" height="15" viewBox="-8 -8 16 16"><circle r="7" fill="#fff" stroke="#CBD5E1" stroke-width="1.1"/><circle r="3.1" fill="none" stroke="#475569" stroke-width="2" stroke-dasharray="2 1.6"/><circle r="1" fill="#475569"/></svg>Usa sistema</span>
  <span><svg width="15" height="15" viewBox="-8 -8 16 16"><circle r="7" fill="#fff" stroke="#CBD5E1" stroke-width="1.1"/><circle r="3.9" fill="none" stroke="#475569" stroke-width="1.2"/><path d="M 0 -2.2 V 0 L 1.8 1.3" fill="none" stroke="#475569" stroke-width="1.2" stroke-linecap="round"/></svg>Con plazo</span>
  <span><svg width="15" height="15" viewBox="-8 -8 16 16"><circle r="7" fill="#fff" stroke="#CBD5E1" stroke-width="1.1"/><circle cx="1.6" cy="-1.4" r="1.6" fill="none" stroke="#475569" stroke-width="1"/><path d="M -0.4 4 q 2 -3.2 4 0" fill="none" stroke="#475569" stroke-width="1"/><circle cx="-1.7" cy="-1.9" r="1.8" fill="#fff" stroke="#475569" stroke-width="1.1"/><path d="M -4.1 3.9 q 2.4 -3.7 4.8 0" fill="#fff" stroke="#475569" stroke-width="1.1"/></svg>Participan otros</span>
  <span><i style="border:0;border-top:2px dashed #D79B00;height:0;width:22px"></i>Retorno o salto de la decisión</span>
  <span><svg width="15" height="15" viewBox="-8 -8 16 16"><circle r="6.8" fill="#F8FAFC" stroke="#475569" stroke-width="1.2"/><path d="M -3.4 -1.4 h 2.4 v -1.4 l 3.4 2.8 -3.4 2.8 v -1.4 h -2.4 z" fill="#475569"/></svg>Viene de / continúa en (otra página)</span>
...
export function anexoFlujoHTML({ p, fs, pasos = [], errores = [], version, fecha }) {
  ...
  const matriz = `<table class="t mx">
    ...
    <tr class="docs"><td><b>Documentos del flujo</b></td>${fs.map(f => { const ds = pasosFase(f).filter(x => x.documento)
      if (!ds.length) return '<td class="c"></td>'
      const total = norm.filter(x => x.documento).length
      return `<td class="c">${total > 8 ? `<span class="d">${ds.length} doc.</span><span class="p">${ds.slice(0, 2).map(x => x._num).join(', ')}${ds.length > 2 ? '…' : ''}</span>` : ds.map(x => `<span class="d">${esc(x.documento)} <span class="p">(${x._num})</span></span>`).join('')}</td>` }).join('')}<td class="c tot">${norm.filter(x => x.documento).length}</td><td></td><td></td></tr>
  </table>`
  ...
  const iUltima = fs.reduce((acc, f, i) => (pasosFase(f).length ? i : acc), -1)
  const paginas = [pag0]
  fs.forEach((f, fi) => {
    ...
    const descCorta = (() => { const v = String(f.descripcion || 'Sin descripción registrada.'); return v.length > 230 ? v.slice(0, 229).replace(/[\s,.;:]+\S*$/, '') + '…' : v })()
    ...
    tramos.forEach((t, k) => {
      ...
      const altoFila = x => {
        let n = Math.max(lineas(x.accion, hayDec ? 42 : 66), lineas(x.responsable, 20), lineas((x.participantes || []).join(', '), 16), lineas(x.sistema, 14), lineas(x.documento, 16), lineas(x.control_tiempo, 13))
        if (x.es_decision) n = Math.max(n, 2 + lineas((x.rama_si || 'SÍ') + '000000000000000000000000000', 34) + lineas((x.rama_no || 'NO') + '000000000000000000000000000', 34))
        return 3.2 + n * 4
      }
      const altoTabla = 8 + t.reduce((acc, x) => acc + altoFila(x), 0)
      const txtErr = k === tramos.length - 1 ? errs.map(e => `${e.error} ${e.prevencion || ''}`).join(' · ') : ''
      const altoErr = txtErr ? 4 + lineas(txtErr, 130) * 4 : 0
      // presupuesto por página: 192 mm útiles − cabeceras − la cabecera de la fase (según
      // el largo de su descripción) − reserva para el pie del documento en la última página
      const altoCab = 12 + lineas(descCorta, 95) * 3.6
      const extraPie = (fi === iUltima && k === tramos.length - 1) ? 12 : 0
      const presup = 168 - altoCab - extraPie
      // en páginas apretadas los errores frecuentes se omiten (van completos en la sección 3)
      const mostrarErr = !!txtErr && (presup - altoTabla - altoErr) >= 40
      const cap = Math.max(34, Math.min(120, Math.round(presup - altoTabla - (mostrarErr ? altoErr : 0) - (llegan.length ? 5 : 0))))
      ...
      const fija = altoTabla + altoCab <= 150
      paginas.push(`<div class="pag-flujo${fija ? (extraPie ? ' pag-fija pag-ult' : ' pag-fija') : ''}">
        ...
        <div class="fase-cab" style="border-left-color:${col.tono}">
          ...
          <div class="fase-txt"><div class="fase-nom">${esc(f.nombre)}</div><div class="fase-desc">${esc(descCorta)}</div></div>
          <div class="fase-meta">
        ...
        ${tabla}
        ${mostrarErr ? `<p class="small fase-err"><b>Errores frecuentes en esta fase:</b> ${errs.map(e => `${esc(e.error)}${e.prevencion ? ` <span class="muted">(prevención: ${esc(e.prevencion)})</span>` : ''}`).join(' · ')}</p>` : ''}
      </div>`)
    })
  })
  ...
  const css = `<style>
    ...
    @media print {
      .pag-fija { height: 184mm; display: flex; flex-direction: column; overflow: hidden; }
      .pag-fija.pag-ult { height: 172mm; }   /* deja espacio al pie del documento en la última página */
      .pag-fija .tramo-svg { flex: 1 1 auto; min-height: 0; display: flex; align-items: center; justify-content: center; margin-bottom: 1.5mm; }
      .pag-fija .tramo-svg svg { max-height: 100%; }
      .pag-fija table.tp, .pag-fija .fase-cab, .pag-fija .fase-nav, .pag-fija .fase-err, .pag-fija .anx-cab, .pag-fija .fase-llegan { flex: 0 0 auto; }
    }
  ...
  secciones.push(`<h2>4. Flujograma, documentos y responsables</h2>
    ${anexo.paginas
      ? `<div class="caja"><b>El flujograma completo va en el Anexo A</b>, en ${anexo.paginas} páginas horizontales al final de este informe: <b>A.0</b> es el mapa del proceso (cadena de ${fs.length} fases, matriz de quién hace qué entre ${carrilesN} responsables y tabla de las ${norm.filter(x => x.es_decision).length} decisiones con sus destinos); luego hay <b>una página por fase</b> con su diagrama swimlane, el detalle de cada paso (responsable, participantes, sistema, documento, control) y los retornos que entran o salen de ella.</div>
         <p class="small muted">Cómo leer el diagrama (simbología BPMN): cada carril horizontal es un responsable; las tareas son rectángulos redondeados numerados fase.paso, con insignias en el borde (escudo = control crítico, engranaje = usa sistema, reloj = con plazo, personas = participan otros roles) y en celeste si generan documento; el rombo con × es una decisión con salidas SÍ/NO; el círculo verde con señal de partida es el inicio y el círculo rojo grueso el fin; la flecha naranja discontinua es un retorno o salto de una decisión (si el destino está en otra página, un rótulo indica el paso); los círculos con flecha son eventos de enlace que conectan las páginas ("viene de / continúa en").</p>`
