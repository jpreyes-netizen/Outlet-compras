          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 7 }}>
            {comitesIncompletos.length > 0 && (
              <div style={{ display: 'flex', gap: 9, alignItems: 'center', flexWrap: 'wrap', fontSize: 12.5 }}>
                ...
                <Bt sm onClick={() => onIrComites('arranque')}>🏛️ Constituirlos ahora →</Bt>
              </div>
            )}
            ...
            {misEncargos.map(e => {
              ...
              return (
                <Cd key={e.id} accent={e.vencido ? 'var(--danger)' : 'var(--accent)'}>
                  ...
                  {!e.equipo_confirmado_at && (
                    <div style={{ fontSize: 12, padding: '7px 10px', borderRadius: 8, marginBottom: 8, lineHeight: 1.55,
                      background: 'var(--warning-bg)', color: 'var(--warning-text)' }}>
                      <b>{soyLider ? 'Te toca conformar el equipo.' : 'Equipo por conformar.'}</b>{' '}
                      {soyLider
                        ? 'Convoca a quienes lo integrarán, revisa que no choque con sus otros comités y registra la nómina (paso 1.3). Sin eso la fase 1 no cierra.'
                        : `${e.lider} aún no registra la nómina del comité de trabajo.`}
                    </div>
                  )}
                  {g && (
                    <>
      ...
      {nada && (
        <Cd>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(240px, 1fr))', gap: 10 }}>
            {[
              ['1 · Constituye los comités', 'Integrantes (impar ≥ 3), secretario, quórum y ritmo — el asistente los propone.', 'Comités → Asistente de arranque', () => onIrComites('arranque')],
              ['2 · Agenda el período', 'Las sesiones de cada comité, de una sola vez, según su ritmo.', 'Comités → Calendario', () => onIrComites('calendario')],
