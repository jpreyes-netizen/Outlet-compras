...
export function PrcFicha({ id, cu, cat, matriz, deps, onCerrar, onAbrir, toast }) {
  ...
  const guardarAvance = async () => {
    ...
    setSheet(null); toast('Avance registrado'); cargar(); onCerrar(true)
  }
  ...
  const guardarCapac = async () => {
    ...
    setSheet(null); toast(`Capacitación registrada con ${lista.length} asistentes`); cargar(); onCerrar(true)
  }
  ...
  const guardarMedicion = async () => {
    ...
    setSheet(null); toast('Medición registrada'); cargar(); onCerrar(true)
  }
  ...
  const guardarHito = async () => {
    ...
    setSheet(null); toast('Hito registrado'); cargar()
  }
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
      ...
      {!loading && d && sub === 'sop' && (
        <PrcSOP proceso={p} cu={cu} matriz={matriz} comites={cat.comites}
          docs={d.docs} firmas={d.firmas} onRecargar={() => { cargar(); onCerrar(true) }} toast={toast}
          bundle={{
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
          <Cd style={{ padding: 13, display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
            ...
            <div style={{ marginLeft: 'auto' }}>
              <Bt sm dis={!editable} onClick={abrirAvance}>Registrar avance</Bt>
            </div>
          </Cd>
