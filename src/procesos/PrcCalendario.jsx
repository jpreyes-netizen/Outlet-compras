...
import {
  ...
  css, hoy, uid, fFecha, puedeEditar, descargar
} from './prcUI'
...
export function PrcCalendario({ matriz, cat, cu, onAbrir, toast, onAbrirSesion }) {
  ...
  const [plan, setPlan] = useState({})              // generar calendario del período
  ...
  /* ── generar el calendario del período según la periodicidad del comité ── */
  const sumarMes = (fecha, n) => { const [y, m, d] = fecha.split('-').map(Number); const ult = new Date(Date.UTC(y, m - 1 + n + 1, 0)).getUTCDate(); return iso(y, m - 1 + n, Math.min(d, ult)) }
  const fechasPlan = useMemo(() => {
    if (!plan.comite_codigo || !plan.desde || !plan.hasta) return []
    const c = cat.comites.find(x => x.codigo === plan.comite_codigo)
    const per = c?.periodicidad || 'SEMANAL'
    const ya = new Set(ses.filter(s => s.comite_codigo === plan.comite_codigo && s.estado !== 'ANULADA').map(s => s.fecha))
    const out = []
    let f = plan.desde, i = 0
    while (f <= plan.hasta && out.length < 30 && i < 60) {
      if (!ya.has(f)) out.push(f)
      f = per === 'MENSUAL' ? sumarMes(plan.desde, ++i) : per === 'TRIMESTRAL' ? sumarMes(plan.desde, (++i) * 3) : sumarDias(f, per === 'QUINCENAL' ? 14 : 7)
      if (per === 'SEMANAL' || per === 'QUINCENAL') i++
    }
    return out
  }, [plan, cat.comites, ses])

  const abrirPlan = () => {
    const c = cat.comites.find(x => x.codigo === filtro) || cat.comites[0]
    setPlan({ comite_codigo: c?.codigo || '', desde: hoy(), hasta: sumarDias(hoy(), 90), hora_inicio: '09:00', hora_fin: '10:30', lugar: '' })
    setSheet('plan')
  }
  const guardarPlan = async () => {
    if (!plan.comite_codigo) return toast('Elige el comité.', 'err')
    if (!fechasPlan.length) return toast('No hay fechas nuevas que agendar en ese rango (puede que ya estén todas).', 'err')
    setBusy(true)
    const filas = fechasPlan.map(f => ({
      id: `SES-${plan.comite_codigo}-${f.replace(/-/g, '')}-${uid().slice(-4)}`,
      comite_codigo: plan.comite_codigo, fecha: f, hora_inicio: plan.hora_inicio || null, hora_fin: plan.hora_fin || null,
      lugar: plan.lugar || null, estado: 'PLANIFICADA', creada_por: cu?.nombre || '—'
    }))
    const { error } = await supabase.from('prc_sesiones_comite').insert(filas)
    setBusy(false)
    if (error) return toast('No se pudo generar el calendario: ' + error.message, 'err')
    setSheet(null)
    toast(`${filas.length} sesiones agendadas para ${comiteNombre(plan.comite_codigo)} (${fFecha(fechasPlan[0])} → ${fFecha(fechasPlan[fechasPlan.length - 1])})`)
    cargar(true)
  }

  /* ── invitación de agenda (.ics) de la sesión seleccionada ── */
  const bajarIcs = () => {
    const dt = sel.fecha.replace(/-/g, '')
    const hi = (sel.hora_inicio || '09:00').replace(':', '') + '00'
    const hf = (sel.hora_fin || '10:30').replace(':', '') + '00'
    const lineas = ['BEGIN:VCALENDAR', 'VERSION:2.0', 'PRODID:-//ERP Outlet//Procesos//ES', 'BEGIN:VEVENT',
      `UID:${sel.id}@erp-outlet`, `DTSTART:${dt}T${hi}`, `DTEND:${dt}T${hf}`,
      `SUMMARY:${comiteNombre(sel.comite_codigo)}${sel.numero != null ? ' · Sesión N° ' + sel.numero : ''}`,
      sel.lugar ? `LOCATION:${sel.lugar}` : null,
      `DESCRIPTION:${(sel.tema || 'Sesión ordinaria')} · agendada en el ERP Outlet (módulo Procesos)`,
      'BEGIN:VALARM', 'TRIGGER:-PT48H', 'ACTION:DISPLAY', 'DESCRIPTION:Convocatoria (48 h)', 'END:VALARM',
      'END:VEVENT', 'END:VCALENDAR'].filter(Boolean)
    descargar(`Sesion_${sel.comite_codigo}_${sel.fecha}.ics`, lineas.join('\r\n'), 'text/calendar;charset=utf-8')
    toast('Invitación descargada: ábrela para agregarla a tu agenda o adjúntala al correo de convocatoria.')
  }
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
        ...
        <div style={{ display: 'flex', gap: 9, alignItems: 'center', flexWrap: 'wrap' }}>
          ...
          {editable && (
            <span style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
              <Bt v="sec" sm onClick={abrirPlan}
                title="Agenda de una vez todas las sesiones del período según la periodicidad del comité (semanal, quincenal, mensual)">📆 Generar calendario</Bt>
              <Bt sm onClick={() => abrirSesion(null, hoy())}
        ...
        <Cd accent={EST_SESION[sel.estado]?.c}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: 13 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', alignItems: 'flex-start' }}>
              ...
              <Bt v="sec" sm onClick={bajarIcs} title="Descarga la invitación .ics para la agenda (Google/Outlook), con recordatorio de convocatoria a las 48 h">🗓️ .ics</Bt>
      ...
      {/* ── sheet: generar calendario del período ── */}
      <Sheet open={sheet === 'plan'} onClose={() => setSheet(null)} title="Generar calendario del período">
        <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
          <Campo l="Comité" obligatorio hint="Usa su periodicidad configurada (Config → Comités → Reglamento).">
            <select style={{ ...css.input, cursor: 'pointer' }} value={plan.comite_codigo || ''} onChange={e => setPlan({ ...plan, comite_codigo: e.target.value })}>
              <option value="">Elige el comité</option>
              {cat.comites.map(c => <option key={c.codigo} value={c.codigo}>{c.nombre} · {String(c.periodicidad || 'SEMANAL').toLowerCase()}</option>)}
            </select>
          </Campo>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9 }}>
            <Campo l="Primera sesión" obligatorio hint="Elige el día de la semana en que sesiona: las demás siguen ese patrón.">
              <input type="date" style={css.input} value={plan.desde || ''} onChange={e => setPlan({ ...plan, desde: e.target.value })} /></Campo>
            <Campo l="Hasta" obligatorio><input type="date" style={css.input} value={plan.hasta || ''} onChange={e => setPlan({ ...plan, hasta: e.target.value })} /></Campo>
          </div>
          <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1.4fr', gap: 9 }}>
            <Campo l="Hora inicio"><input type="time" style={css.input} value={plan.hora_inicio || ''} onChange={e => setPlan({ ...plan, hora_inicio: e.target.value })} /></Campo>
            <Campo l="Hora término"><input type="time" style={css.input} value={plan.hora_fin || ''} onChange={e => setPlan({ ...plan, hora_fin: e.target.value })} /></Campo>
            <Campo l="Lugar"><input style={css.input} placeholder="Sala / Meet" value={plan.lugar || ''} onChange={e => setPlan({ ...plan, lugar: e.target.value })} /></Campo>
          </div>
          <Hint>
            {plan.comite_codigo && fechasPlan.length
              ? `Se agendarán ${fechasPlan.length} sesiones: ${fechasPlan.slice(0, 5).map(fFecha).join(' · ')}${fechasPlan.length > 5 ? ' …' : ''}. Los días que ya tienen sesión de este comité se saltan.`
              : 'Elige comité y rango para previsualizar las fechas.'}
          </Hint>
          <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
            <Bt v="sec" onClick={() => setSheet(null)}>Cancelar</Bt>
            <Bt dis={busy || !fechasPlan.length} onClick={guardarPlan}>{busy ? 'Agendando…' : `Agendar ${fechasPlan.length || ''} sesiones`}</Bt>
          </div>
        </div>
      </Sheet>
