...
import {
  Cd, Bt, Bd, Ayuda, Hint, Campo, BtIc, BtEliminar, Sucio, Vacio, Chips,
  css, uid, puedeEditar
} from './prcUI'
...
export function PrcEditor({ proceso, d, cat, cu, onGuardado, toast }) {
  ...
  const fichaDe = (pr) => ({
    objetivo: pr.objetivo || '', alcance: pr.alcance || '', regla_critica: pr.regla_critica || '',
    dueno_cargo: pr.dueno_cargo || '', dueno_persona: pr.dueno_persona || '',
    dueno_provisional: !!pr.dueno_provisional, comite_codigo: pr.comite_codigo || '',
    impacto: pr.impacto, urgencia: pr.urgencia, onda: pr.onda || '',
    fecha_objetivo_vigente: pr.fecha_objetivo_vigente || '', observaciones: pr.observaciones || '',
    meses_revision: pr.meses_revision || 6, sistemas: pr.sistemas || []
  })
  const copia = arr => (arr || []).map(x => ({ ...x }))

  const reset = () => {
    setFicha(fichaDe(proceso))
    setPrincipios(copia(d.principios))
    setRoles(copia(d.roles))
    setTransicion(copia(d.transicion))
    setKpis(copia(d.kpis))
    setFases(copia(d.fases))
    setPasos(copia(d.pasos))
    setErrores(copia(d.errores))
  }

  // Al abrir otro proceso se recarga todo. Al refrescar el MISMO proceso (por
  // ejemplo después de guardar una sección) se adopta lo que llega de la base
  // solo en las secciones que no tengan cambios sin guardar: escribir en la
  // sección 6 y guardar la 5 no debe borrar lo que estabas escribiendo.
  const ultimo = useRef(null)
  useEffect(() => {
    const primeraVez = ultimo.current === null || ultimo.current.id !== proceso.id
    if (primeraVez) {
      reset()
      ultimo.current = { id: proceso.id, proceso, d }
      return
    }
    const ant = ultimo.current
    const igual = (a, b) => JSON.stringify(a) === JSON.stringify(b)
    // ficha: se adopta si no la tocaste desde el último refresco
    if (igual(ficha, fichaDe(ant.proceso))) setFicha(fichaDe(proceso))
    const secciones = [
      ['principios', principios, setPrincipios], ['roles', roles, setRoles],
      ['transicion', transicion, setTransicion], ['kpis', kpis, setKpis],
      ['fases', fases, setFases], ['pasos', pasos, setPasos], ['errores', errores, setErrores]
    ]
    secciones.forEach(([k, actual, set]) => {
      if (igual(actual, ant.d[k] || [])) set(copia(d[k]))
    })
    ultimo.current = { id: proceso.id, proceso, d }
  ...
  // sistemas: los del catálogo se marcan con botones; el resto son texto libre
  const codigosCatalogo = useMemo(() => new Set((cat.sistemas || []).map(s => s.codigo)), [cat.sistemas])
  const sistemasLibres = useMemo(() => (ficha.sistemas || []).filter(x => !codigosCatalogo.has(x)), [ficha.sistemas, codigosCatalogo])
  // sugerencias de responsable: roles declarados + los ya usados en fases y pasos
  const sugRoles = useMemo(() => {
    const s = new Set()
    roles.forEach(r => r.rol && s.add(r.rol))
    fases.forEach(f => {
      if (f.responsable_principal) s.add(f.responsable_principal)
      ;(f.responsables_apoyo || []).forEach(x => s.add(x))
    })
    pasos.forEach(p => {
      if (p.responsable) s.add(p.responsable)
      ;(p.participantes || []).forEach(x => s.add(x))
    })
    return [...s].sort()
  }, [roles, fases, pasos])
  // pasos numerados F.N para elegir el destino de una decisión
  const opcionesPasos = useMemo(() => {
    const out = []
    fases.forEach((f, fi) => {
      pasos.filter(x => x.fase_id === f.id).forEach((x, i) => {
        out.push({ id: x.id, label: `${fi + 1}.${i + 1} — ${String(x.accion || 'sin texto').slice(0, 58)}` })
      })
    })
    return out
  }, [fases, pasos])
  ...
  const sucioFicha = dif(ficha, fichaDe(proceso))
  ...
  const guardarFlujo = async () => {
    ...
    // 2. Pasos, renumerados dentro de su fase. Los ids nuevos se resuelven ANTES
    // para poder traducir los destinos de las decisiones que apuntan a ellos.
    const mapaPasoId = {}
    pasos.forEach(p => {
      const fid = mapaId[p.fase_id] || p.fase_id
      mapaPasoId[p.id] = String(p.id).startsWith('NUEVO-') ? `${fid}-${uid()}` : p.id
    })
    const destino = v => (v && mapaPasoId[v]) ? mapaPasoId[v] : null
    const filasPasos = []
      ...
      pasos.filter(p => mapaId[p.fase_id] === f.id || p.fase_id === f.id)
        .forEach((p, i) => {
          ...
          filasPasos.push({
            ...
            rama_si_destino: p.es_decision ? destino(p.rama_si_destino) : null,
            rama_no_destino: p.es_decision ? destino(p.rama_no_destino) : null,
  ...
  const agregarFase = () => {
    const id = 'NUEVO-' + uid()
    setFases(p => [...p, { id, proceso_id: proceso.id, nombre: '', responsable_principal: '', responsables_apoyo: [], descripcion: '', color: '#334155', orden: p.length + 1 }])
    setFaseAbierta(id)
  }
  const agregarPaso = (faseId) => setPasos(p => [...p, {
    id: 'NUEVO-' + uid(), fase_id: faseId, proceso_id: proceso.id, accion: '', responsable: '',
    sistema: '', control_tiempo: '', participantes: [], es_control_critico: false, es_decision: false,
    rama_si: '', rama_no: '', rama_si_destino: '', rama_no_destino: '', documento: '', documento_url: ''
  }])
    ...
    <div style={{ display: 'flex', flexDirection: 'column', gap: 13 }}>
      ...
      <Seccion n="1" titulo="Identificación y prioridad" editable={editable}
        ...
        <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
          ...
          <Campo l="Sistemas que usa el proceso" hint="Marca los del catálogo y, si el sistema no está en la lista, escríbelo abajo. Aparecen como etiquetas en la ficha.">
            ...
            <div style={{ marginTop: 10, paddingTop: 10, borderTop: '1px dashed var(--border-2)' }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: 'var(--text-muted)', textTransform: 'uppercase', letterSpacing: .3, marginBottom: 5 }}>
                Otros sistemas o herramientas (escritos a mano)
              </div>
              <Chips editable={editable} valores={sistemasLibres}
                sugerencias={['Excel', 'Google Sheets', 'WhatsApp', 'Correo', 'Planilla en papel', 'Drive', 'Trello']}
                ph="Ej: planilla de bodega en Excel — escribe y Enter"
                vacio="Si el proceso se apoya en una planilla, un cuaderno o una herramienta que no está en el catálogo, escríbela acá."
                onChange={v => setFicha(f => ({ ...f, sistemas: [...(f.sistemas || []).filter(x => codigosCatalogo.has(x)), ...v] }))} />
            </div>
          </Campo>
      ...
      <Seccion n="5" titulo="Fases, pasos y errores frecuentes" editable={editable}
        ...
        {vistaFlujo === 'diseno' && (
          <PrcDisenador proceso={proceso} fases={fases} pasos={pasos}
            ...
            listaRolesId={listaRoles} sistemasId={'sis-' + proceso.id} sugRoles={sugRoles} />
        )}
              ...
              {abierta && (
                <div style={{ padding: 13 }}>
                  <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(190px, 1fr))', gap: 10, marginBottom: 12 }}>
                    <Campo l="Responsable principal" hint="El cargo que lidera la etapa y responde por ella.">
                  ...
                  <div style={{ marginBottom: 14 }}>
                    <Campo l="Otros responsables de la etapa" hint="Los cargos que también responden por esta etapa junto al principal. Escribe uno y presiona Enter.">
                      <Chips editable={editable} valores={f.responsables_apoyo || []} sugerencias={sugRoles}
                        ph="Ej: Director de Negocios — escribe y Enter"
                        vacio="Si la etapa la sostiene más de un cargo, agrégalos acá."
                        onChange={v => setFases(p => p.map((x, j) => j === fi ? { ...x, responsables_apoyo: v } : x))} />
                    </Campo>
                  </div>
                  ...
                  {lista.map((p, i) => (
                    <div key={p.id} style={{
                      ...
                      <div style={{ marginTop: 8 }}>
                        <Campo l="Quién más participa en el paso" hint="Cargos que intervienen sin ser el responsable. El responsable de arriba es el que define el carril del flujograma.">
                          <Chips editable={editable} valores={p.participantes || []} sugerencias={sugRoles}
                            ph="Ej: Jefe de Tienda — escribe y Enter" vacio="Opcional. Útil cuando el paso se hace entre dos cargos."
                            onChange={v => setPasoCampo(p.id, 'participantes', v)} />
                        </Campo>
                      </div>
                      <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
                        <Campo l="Documento del paso" hint="El documento que este paso genera o exige. Sale con símbolo de documento en el flujograma.">
                          <input disabled={!editable} placeholder="Ej: Acta de sesión, Ficha de diagnóstico" value={p.documento || ''}
                            onChange={e => setPasoCampo(p.id, 'documento', e.target.value)}
                            style={{ ...css.input, fontSize: 12.5, padding: '6px 9px' }} />
                        </Campo>
                        <Campo l="Enlace al estándar del documento" hint="URL de la plantilla o formato oficial (Drive, repositorio).">
                          <input disabled={!editable} placeholder="https://…" value={p.documento_url || ''}
                            onChange={e => setPasoCampo(p.id, 'documento_url', e.target.value)}
                            style={{ ...css.input, fontSize: 12.5, padding: '6px 9px' }} />
                        </Campo>
                      </div>
                      {p.es_decision && (
                        <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 8, marginTop: 8 }}>
                          <Campo l="Si SÍ → (texto de la rama)" hint="Qué pasa cuando la respuesta es afirmativa.">
                            <input disabled={!editable} value={p.rama_si || ''} onChange={e => setPasoCampo(p.id, 'rama_si', e.target.value)}
                              style={{ ...css.input, fontSize: 12.5, padding: '6px 9px' }} />
                          </Campo>
                          <Campo l="Si SÍ, el flujo continúa en" hint="Por defecto sigue al paso siguiente; elige otro para saltar.">
                            <select disabled={!editable} value={p.rama_si_destino || ''}
                              onChange={e => setPasoCampo(p.id, 'rama_si_destino', e.target.value)}
                              style={{ ...css.input, fontSize: 12.5, padding: '6px 9px', cursor: 'pointer' }}>
                              <option value="">Paso siguiente (normal)</option>
                              {opcionesPasos.filter(o => o.id !== p.id).map(o => <option key={o.id} value={o.id}>{o.label}</option>)}
                            </select>
                          </Campo>
                          <Campo l="Si NO → (texto de la rama)" hint="La ruta alternativa o de excepción.">
                            <input disabled={!editable} value={p.rama_no || ''} onChange={e => setPasoCampo(p.id, 'rama_no', e.target.value)}
                              style={{ ...css.input, fontSize: 12.5, padding: '6px 9px' }} />
                          </Campo>
                          <Campo l="Si NO, el flujo vuelve o salta a" hint="Elige el paso al que se devuelve el flujo; el flujograma dibuja la flecha de retorno.">
                            <select disabled={!editable} value={p.rama_no_destino || ''}
                              onChange={e => setPasoCampo(p.id, 'rama_no_destino', e.target.value)}
                              style={{ ...css.input, fontSize: 12.5, padding: '6px 9px', cursor: 'pointer' }}>
                              <option value="">Sin flecha (solo texto)</option>
                              {opcionesPasos.filter(o => o.id !== p.id).map(o => <option key={o.id} value={o.id}>{o.label}</option>)}
                            </select>
                          </Campo>
                        </div>
