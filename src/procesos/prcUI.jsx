...
/** Panel colapsable "¿qué hago acá?". Se recuerda abierto/cerrado por clave. */
export function Ayuda({ titulo = '¿Qué hago en esta pantalla?', children, k, abierta = true }) {
  const key = k ? 'prc_ayuda_' + k : null
  const [open, setOpen] = useState(() => {
    if (!key) return abierta
    try { const v = localStorage.getItem(key); return v === null ? abierta : v === '1' } catch { return abierta }
  })
  const tog = () => {
    const v = !open
    setOpen(v)
    if (key) { try { localStorage.setItem(key, v ? '1' : '0') } catch {} }
  }
  return (
    <div style={{
      background: 'var(--accent-bg)', border: '1px solid var(--accent-bg)',
      borderLeft: '3px solid var(--accent)', borderRadius: '0 12px 12px 0', overflow: 'hidden'
    }}>
      <button onClick={tog} style={{
        width: '100%', display: 'flex', alignItems: 'center', gap: 8, padding: '10px 14px',
        background: 'transparent', border: 'none', cursor: 'pointer', textAlign: 'left',
        color: 'var(--accent-text)', fontSize: 12.5, fontWeight: 700, minHeight: 0
      }}>
        <span style={{ fontSize: 14 }}>💡</span>
        <span style={{ flex: 1 }}>{titulo}</span>
        <span style={{ fontSize: 11, opacity: .7 }}>{open ? 'ocultar' : 'mostrar'}</span>
      </button>
      {open && (
        <div style={{ padding: '0 14px 12px 36px', fontSize: 12.5, lineHeight: 1.65, color: 'var(--accent-text)' }}>
          {children}
        </div>
      )}
    </div>
...
/** Texto de ayuda bajo un campo o botón. */
export function Hint({ children, style }) {
  return <div style={{ fontSize: 11, color: 'var(--text-muted)', lineHeight: 1.5, marginTop: 3, ...style }}>{children}</div>
}

/** Campo de formulario con etiqueta y ayuda. */
export function Campo({ l, hint, children, obligatorio }) {
  return (
    <div>
      <label style={{ fontSize: 12, fontWeight: 600, color: 'var(--text-secondary)', display: 'block', marginBottom: 4 }}>
        {l}{obligatorio && <span style={{ color: 'var(--danger)' }}> *</span>}
      </label>
      {children}
      {hint && <Hint>{hint}</Hint>}
    </div>
...
/** Botón con leyenda visible debajo. Para acciones que necesitan explicación. */
export function BtGuia({ children, leyenda, onClick, v = 'pri', dis, ancho }) {
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 3, width: ancho }}>
      <Bt v={v} dis={dis} onClick={onClick} style={{ width: '100%' }}>{children}</Bt>
      {leyenda && <div style={{ fontSize: 10.5, color: 'var(--text-muted)', lineHeight: 1.4, textAlign: 'center' }}>{leyenda}</div>}
    </div>
  )
}

/** Botón chico de icono con tooltip obligatorio. */
export function BtIc({ ic, title, onClick, dis, color }) {
  return (
    <button title={title} aria-label={title} onClick={dis ? undefined : onClick} disabled={dis} style={{
      width: 26, height: 26, minHeight: 26, borderRadius: 7, cursor: dis ? 'not-allowed' : 'pointer',
      border: '1px solid var(--border-2)', background: 'var(--bg-surface)',
      color: color || 'var(--text-secondary)', fontSize: 12, lineHeight: 1,
      opacity: dis ? .35 : 1, padding: 0, display: 'inline-flex', alignItems: 'center', justifyContent: 'center'
    }}>{ic}</button>
  )
}

/** Confirmación en dos clics, sin window.confirm (que bloquea el iframe). */
export function BtEliminar({ onConfirm, title = 'Eliminar', label = '🗑' }) {
  const [armado, setArmado] = useState(false)
  if (!armado) return <BtIc ic={label} title={title} color="var(--danger)" onClick={() => setArmado(true)} />
  return (
    <span style={{ display: 'inline-flex', gap: 4, alignItems: 'center' }}>
      <Bt v="dan" sm onClick={() => { setArmado(false); onConfirm() }}>Sí, eliminar</Bt>
      <Bt v="ghost" sm onClick={() => setArmado(false)}>No</Bt>
    </span>
  )
}

/** Indicador de cambios sin guardar. */
export function Sucio({ hay }) {
  if (!hay) return null
  return <Bd c="var(--warning)">● cambios sin guardar</Bd>
}

/**
 * Lista editable de valores libres, mostrados como etiquetas quitables.
 * Sirve para varios responsables, participantes o sistemas escritos a mano.
 *   valores      array de strings
 *   onChange     recibe el array nuevo
 *   sugerencias  array de strings que se ofrecen al escribir (datalist)
 *   ph           placeholder del campo de escritura
 *   color        color de las etiquetas
 */
export function Chips({ valores, onChange, sugerencias = [], ph = 'Escribe y presiona Enter', editable = true, color = 'var(--accent)', vacio = 'Ninguno todavía.' }) {
  const [txt, setTxt] = useState('')
  const lista = Array.isArray(valores) ? valores : []
  const listaId = 'chips-' + useMemo(() => Math.random().toString(36).slice(2, 8), [])

  const agregar = (raw) => {
    // acepta pegar varios separados por coma o punto y coma
    const nuevos = String(raw).split(/[,;]/).map(s => s.trim()).filter(Boolean)
      .filter(v => !lista.some(x => x.toLowerCase() === v.toLowerCase()))
    if (nuevos.length) onChange([...lista, ...nuevos])
    setTxt('')
  }
  const quitar = v => onChange(lista.filter(x => x !== v))

  return (
    <div>
      {(lista.length > 0 || !editable) && (
        <div style={{ display: 'flex', gap: 5, flexWrap: 'wrap', marginBottom: editable ? 6 : 0 }}>
          {lista.length === 0 && <span style={{ fontSize: 11.5, color: 'var(--text-muted)' }}>—</span>}
          {lista.map(v => (
            <span key={v} style={{
              display: 'inline-flex', alignItems: 'center', gap: 5, padding: '4px 6px 4px 9px',
              borderRadius: 8, fontSize: 11.5, fontWeight: 600, background: 'var(--bg-page)',
              border: '1px solid var(--border-2)', color
            }}>
              {v}
              {editable && (
                <button onClick={() => quitar(v)} title={`Quitar ${v}`} style={{
                  border: 'none', background: 'transparent', cursor: 'pointer', minHeight: 0, padding: 0,
                  color: 'var(--text-muted)', fontSize: 13, lineHeight: 1, width: 14
                }}>×</button>
              )}
            </span>
          ))}
      ...
      {editable && (
        <>
          <div style={{ display: 'flex', gap: 6 }}>
            <input value={txt} list={sugerencias.length ? listaId : undefined} placeholder={ph}
              onChange={e => setTxt(e.target.value)}
              onKeyDown={e => {
                if (e.key === 'Enter') { e.preventDefault(); if (txt.trim()) agregar(txt) }
                if (e.key === 'Backspace' && !txt && lista.length) quitar(lista[lista.length - 1])
              }}
              onBlur={() => { if (txt.trim()) agregar(txt) }}
              style={{ ...css.input, flex: 1, fontSize: 12.5, padding: '7px 9px' }} />
            <Bt v="sec" sm dis={!txt.trim()} onClick={() => agregar(txt)} title="Agregar a la lista">＋</Bt>
          </div>
          ...
          {lista.length === 0 && <Hint>{vacio}</Hint>}
        </>
      )}
    </div>
  )
