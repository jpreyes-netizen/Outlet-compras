...
export function flujoSVG(proceso, fases, pasos, opts = {}) {
  ...
  barras.forEach(b => {
    ...
    const yTit = conApoyo ? py + 21 : py + 27
    o.push(`<text x="${x + w / 2}" y="${yTit}" font-size="11.5" font-weight="700" fill="#FFFFFF" text-anchor="middle">${esc(wrap(label, Math.floor(w / 7.2), 1)[0] || '')}</text>`)
    if (conApoyo) {
      const resp = [b.fase.responsable_principal, ...apoyos].filter(Boolean).join(' · ')
      o.push(`<text x="${x + w / 2}" y="${py + 34}" font-size="9" font-weight="600" fill="#FFFFFF" fill-opacity="0.82" text-anchor="middle">${esc(wrap(resp, Math.floor(w / 5.4), 1)[0] || '')}</text>`)
    }
    o.push('</g>')
  })
  ...
  cols.forEach((c, i) => {
    ...
    if (tags.length && !p.es_decision) {
      ...
      o.push(`<text x="${cx}" y="${y + FLU.BOX_H - (parts.length ? 20 : 9)}" font-size="9" font-weight="600" fill="#64748B" text-anchor="middle">${esc(t)}</text>`)
    }
    // participantes: cargos que intervienen sin ser dueños del carril
    if (parts.length && !p.es_decision) {
      const t = wrap('con ' + parts.join(', '), 40, 1)[0]
      o.push(`<text x="${cx}" y="${y + FLU.BOX_H - 8}" font-size="8.5" font-style="italic" fill="#94A3B8" text-anchor="middle">${esc(t)}</text>`)
    }
