...
import { rl, useToast, Tabs, Bd, Bt } from './prcUI'
import { PrcHoy } from './PrcHoy'
...
export function ProcesosApp({ cu, setAppActual, onVolverHub, onCerrarSesion }) {
  ...
  const [tab, setTab] = useState(() => { try { return localStorage.getItem('prc_tab') || 'hoy' } catch { return 'hoy' } })
    ...
    <div style={{ minHeight: '100vh', background: 'var(--bg-page)' }}>
      <header style={{
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
          {!loading && (
            <Bt sm dis={generando} onClick={informe}
              title="Informe de avance del programa de procesos y comités (P21 fase 7). Se abre en una pestaña nueva: Imprimir → Guardar como PDF.">
      ...
      <main style={{ padding: '20px 24px 60px', maxWidth: sel ? 1500 : 1600, margin: '0 auto' }}>
        ...
        {!loading && !sel && tab === 'comites'     && <PrcComites matriz={matriz} cat={cat} cu={cu} onAbrir={abrir} toast={toast} onRecargar={cargar} onIrConfig={() => setTab('config')} />}
