...
// Fases del comité de trabajo: las 7 fases del método P21 "Construcción y aprobación de procesos"
export const FASES_P37 = ['Activación y asignación', 'Encuadre', 'Diagnóstico', 'Diseño', 'Piloto', 'Validación', 'Bajada y traspaso']
export const FASES_ENCARGO = FASES_P37
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ───────────────────────────────────────────────────────────────────────────
   Antes de meter a alguien en un comité hay que saber tres cosas: en cuántos
   está ya, cuántas horas al mes le significa y —sobre todo— si el día y hora
   que le vamos a asignar choca con otro comité suyo. Una persona no puede
   estar en dos salas a la vez: si eso pasa, el quórum de una de las dos se cae.

   Los comités de gobierno traen su ritmo en el reglamento (dia_semana,
   hora_inicio, hora_fin); las sesiones ya agendadas traen fecha y hora
   concretas. Con eso se detectan ambos tipos de choque.
   ═══════════════════════════════════════════════════════════════════════════ */

const nm = s => String(s || '').trim().toLowerCase().normalize('NFD').replace(/[\u0300-\u036f]/g, '')
export const mismaPersona = (a, b) => !!a && !!b && nm(a) === nm(b)

/** "09:30" → 570 minutos. Tolera "09:30:00" y vacíos. */
export const minutosDe = h => {
  const m = String(h || '').match(/^(\d{1,2}):(\d{2})/)
  return m ? (+m[1]) * 60 + (+m[2]) : null
}

/** Cuántas veces al mes sesiona un comité según su periodicidad. */
const VECES_MES = { SEMANAL: 4.3, QUINCENAL: 2.2, MENSUAL: 1, TRIMESTRAL: 0.33 }

/** Duración en minutos de la sesión de un comité (del reglamento, o su mínimo). */
export function duracionComite(c) {
  const ini = minutosDe(c?.hora_inicio), fin = minutosDe(c?.hora_fin)
  if (ini != null && fin != null && fin > ini) return fin - ini
  return Number(c?.duracion_min || 60)
}

/** ¿Se solapan dos bloques [inicio, inicio+dur)? */
function solapan(ini1, dur1, ini2, dur2) {
  if (ini1 == null || ini2 == null) return false
  return ini1 < ini2 + dur2 && ini2 < ini1 + dur1
}

/**
 * Comités de gobierno en que participa una persona (como integrante, quien
 * preside o secretario), con el ritmo que tengan configurado.
 */
export function comitesDe(persona, comites = []) {
  return (comites || []).filter(c =>
    (c.integrantes || []).some(x => mismaPersona(x, persona)) ||
    mismaPersona(c.responsable, persona) ||
    mismaPersona(c.secretario, persona))
}

/**
 * Disponibilidad de una persona frente a una asignación propuesta.
 *
 * @param persona   nombre
 * @param propuesta { codigo, dia_semana, hora_inicio, hora_fin, duracion_min } — el comité que se le quiere asignar (opcional)
 * @param ctx       { comites, encargos, sesiones }
 * @returns { comites, nComites, encargos, nEncargos, lidera, horasMes, choques[], sobrecargada, libre }
 */
export function disponibilidad(persona, propuesta, { comites = [], encargos = [], sesiones = [] } = {}, { maxComites = 3 } = {}) {
  const mios = comitesDe(persona, comites).filter(c => c.codigo !== propuesta?.codigo)
  const activos = (encargos || []).filter(e => ['ACTIVO', 'EN_PILOTO', 'EN_APROBACION'].includes(e.estado))
  const misEncargos = activos.filter(e =>
    mismaPersona(e.lider, persona) || (e.integrantes || []).some(x => mismaPersona(x, persona)))
  const lidera = activos.filter(e => mismaPersona(e.lider, persona)).length

  const horasMes = mios.reduce((a, c) =>
    a + (duracionComite(c) / 60) * (VECES_MES[c.periodicidad] ?? 1), 0)

  const choques = []
  if (propuesta?.dia_semana && propuesta?.hora_inicio) {
    const ini = minutosDe(propuesta.hora_inicio)
    const dur = duracionComite(propuesta)
    // choque con el ritmo de otro comité de gobierno
    mios.forEach(c => {
      if (+c.dia_semana !== +propuesta.dia_semana) return
      if (solapan(ini, dur, minutosDe(c.hora_inicio), duracionComite(c)))
        choques.push({ tipo: 'COMITE', codigo: c.codigo, nombre: c.nombre, dia: +c.dia_semana, hora: String(c.hora_inicio).slice(0, 5) })
    })
    // choque con sesiones ya agendadas de otros comités en que participa
    const codigosMios = new Set(mios.map(c => c.codigo))
    ;(sesiones || []).forEach(s => {
      if (!codigosMios.has(s.comite_codigo) || s.estado === 'CANCELADA') return
      const d = new Date(String(s.fecha) + 'T00:00:00Z').getUTCDay()
      if ((d === 0 ? 7 : d) !== +propuesta.dia_semana) return
      if (solapan(ini, dur, minutosDe(s.hora_inicio), 60) &&
          !choques.some(x => x.codigo === s.comite_codigo))
        choques.push({ tipo: 'SESION', codigo: s.comite_codigo, fecha: s.fecha, hora: String(s.hora_inicio || '').slice(0, 5) })
    })
  }

  return {
    persona,
    comites: mios, nComites: mios.length,
    encargos: misEncargos, nEncargos: misEncargos.length, lidera,
    horasMes: Math.round(horasMes * 10) / 10,
    choques,
    sobrecargada: mios.length >= maxComites,
    libre: mios.length === 0 && misEncargos.length === 0
  }
}

/** Resumen de disponibilidad de un equipo completo frente a una propuesta. */
export function disponibilidadEquipo(personas = [], propuesta, ctx = {}, opts = {}) {
  const filas = personas.filter(Boolean).map(p => disponibilidad(p, propuesta, ctx, opts))
  return {
    filas,
    conChoque: filas.filter(f => f.choques.length),
    sobrecargadas: filas.filter(f => f.sobrecargada),
    libres: filas.filter(f => f.libre).length
  }
}

/** Texto corto de la carga de una persona, para mostrar junto a su nombre. */
export function etiquetaCarga(d) {
  if (!d) return ''
  const p = []
  if (d.nComites) p.push(`${d.nComites} comité${d.nComites > 1 ? 's' : ''}`)
  if (d.nEncargos) p.push(`${d.nEncargos} en curso`)
  if (!p.length) return 'sin carga'
  if (d.horasMes >= 1) p.push(`${d.horasMes} h/mes`)
  return p.join(' · ')
}
