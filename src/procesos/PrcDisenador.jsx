...
import { Cd, Bt, Bd, Campo, Hint, BtIc, BtEliminar, Vacio, Chips, css, uid } from './prcUI'
...
export function PrcDisenador({ proceso, fases, pasos, setFases, setPasos, editable, listaRolesId, sistemasId, sugRoles = [] }) {
  ...
  const insertarDespues = (p) => {
    const nuevoPaso = {
      ...
      participantes: [], es_control_critico: false, es_decision: false,
  ...
  const agregarFase = () => {
    const id = 'NUEVO-' + uid()
    setFases(prev => [...prev, {
      id, proceso_id: proceso.id, nombre: 'Nueva fase', responsable_principal: '', responsables_apoyo: [],
  ...
  // pasos numerados F.N para elegir destinos de decisión
  const opcionesPasos = useMemo(() => {
    const out = []
    fasesN.forEach((f, fi) => {
      pasosDe(f.id).forEach((x, i) => {
        out.push({ id: x.id, label: `${fi + 1}.${i + 1} — ${String(x.accion || 'sin texto').slice(0, 48)}` })
      })
    })
    return out
        ...
        {pasoSel && (
          <Cd accent="var(--accent)" style={{ padding: 14 }}>
            ...
            <div style={{ marginTop: 9 }}>
              <Campo l="Quién más participa" hint="Cargos que intervienen sin ser el responsable del carril. Salen dentro de la caja.">
                <Chips editable={editable} valores={pasoSel.participantes || []} sugerencias={sugRoles}
                  ph="Ej: Jefe de Tienda — escribe y Enter" vacio="Opcional."
                  onChange={v => setPaso(pasoSel.id, 'participantes', v)} />
              </Campo>
            </div>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginTop: 9 }}>
              <Campo l="Documento del paso" hint="Sale con símbolo de documento en el diagrama.">
                <input disabled={!editable} placeholder="Ej: Acta de sesión" value={pasoSel.documento || ''}
                  onChange={e => setPaso(pasoSel.id, 'documento', e.target.value)} style={inp} />
              </Campo>
              <Campo l="Enlace al estándar" hint="Plantilla o formato oficial.">
                <input disabled={!editable} placeholder="https://…" value={pasoSel.documento_url || ''}
                  onChange={e => setPaso(pasoSel.id, 'documento_url', e.target.value)} style={inp} />
              </Campo>
            </div>
            {pasoSel.es_decision && (
              <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginTop: 9 }}>
                <Campo l="Si SÍ → (texto)" hint="Qué pasa si la respuesta es afirmativa.">
                  <input disabled={!editable} value={pasoSel.rama_si || ''}
                    onChange={e => setPaso(pasoSel.id, 'rama_si', e.target.value)} style={inp} />
                </Campo>
                <Campo l="Si SÍ continúa en" hint="Por defecto, el paso siguiente.">
                  <select disabled={!editable} value={pasoSel.rama_si_destino || ''}
                    onChange={e => setPaso(pasoSel.id, 'rama_si_destino', e.target.value)}
                    style={{ ...inp, cursor: 'pointer' }}>
                    <option value="">Paso siguiente (normal)</option>
                    {opcionesPasos.filter(o => o.id !== pasoSel.id).map(o => <option key={o.id} value={o.id}>{o.label}</option>)}
                  </select>
                </Campo>
                <Campo l="Si NO → (texto)" hint="La ruta de excepción.">
                  <input disabled={!editable} value={pasoSel.rama_no || ''}
                    onChange={e => setPaso(pasoSel.id, 'rama_no', e.target.value)} style={inp} />
                </Campo>
                <Campo l="Si NO vuelve o salta a" hint="El diagrama dibuja la flecha de retorno.">
                  <select disabled={!editable} value={pasoSel.rama_no_destino || ''}
                    onChange={e => setPaso(pasoSel.id, 'rama_no_destino', e.target.value)}
                    style={{ ...inp, cursor: 'pointer' }}>
        ...
        {faseSel && (
          <Cd accent={faseSel.color || '#334155'} style={{ padding: 14 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr', gap: 9, marginTop: 9 }}>
              ...
              <Campo l="Otros responsables de la etapa" hint="Cargos que responden junto al principal.">
                <Chips editable={editable} valores={faseSel.responsables_apoyo || []} sugerencias={sugRoles}
                  ph="Escribe un cargo y Enter" vacio="Opcional."
                  onChange={v => setFase(faseSel.id, 'responsables_apoyo', v)} />
              </Campo>
            </div>
          </Cd>
        )}
