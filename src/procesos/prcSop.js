...
export function sopMarkdown(d) {
  ...
  fs.forEach((f, fi) => {
    ...
    L.push('')
    L.push('| N° | Acción | Responsable | Participan | Sistema | Control / tiempo |')
    L.push('|---|---|---|---|---|---|')
    ps.forEach((s, si) => {
      const marca = s.es_control_critico ? ' 🔴' : s.es_decision ? ' ◆' : ''
      const ramas = s.es_decision ? ` <br/>**Sí →** ${s.rama_si || '—'} <br/>**No →** ${s.rama_no || '—'}` : ''
      const parts = (s.participantes || []).filter(Boolean)
      L.push(`| ${fi + 1}.${si + 1}${marca} | ${s.accion}${ramas} | ${s.responsable || '—'} | ${parts.length ? parts.join(', ') : '—'} | ${s.sistema || '—'} | ${s.control_tiempo || '—'} |`)
    })
    L.push('')
    if (es.length) {
