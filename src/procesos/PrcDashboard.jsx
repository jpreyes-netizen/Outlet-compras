    ...
    <Cd accent="var(--accent)">
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap', marginBottom: 10 }}>
        ...
        <span style={{ marginLeft: 'auto', display: 'flex', gap: 6 }}>
          ...
          <Bt sm onClick={onInforme} title="Informe de avance para el Directorio: se abre en una pestaña nueva → Imprimir → Guardar como PDF">📄 Informe de avance</Bt>
        </span>
      </div>
      {d === null && <div style={{ fontSize: 12, color: 'var(--text-muted)' }}>Cargando indicadores del comité…</div>}
      {d === false && <div style={{ fontSize: 12, color: 'var(--warning-text)' }}>No se pudieron leer las vistas del comité (v_prc_sesiones, v_prc_acuerdos, v_prc_scorecard). Revisa que el SQL 09 esté aplicado.</div>}
      {d && <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(170px, 1fr))', gap: 8 }}>
