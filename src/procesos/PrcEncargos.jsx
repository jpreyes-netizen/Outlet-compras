...
import { PLANTILLAS, descargarPlantilla } from './prcPlantillas'
      ...
      {sel && (
        <Cd accent={sel.vencido ? 'var(--danger)' : (EST[sel.estado] || EST.ACTIVO).c}>
          ...
          {ACTIVOS.includes(sel.estado) && (() => {
            ...
            const ok = checksDe(sel.id)
            const hechos = mP.filter(p => ok.has(p.id)).length
            return (
              <div style={{ marginBottom: 13, padding: '11px 13px', borderRadius: 11, background: 'var(--accent-bg)', border: '1px solid var(--border-1)' }}>
                <div style={{ display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap', marginBottom: 8 }}>
                  <span style={{ fontSize: 13, fontWeight: 800 }}>📋 Fase {sel.fase_actual} según el método (P21 · {mF.nombre})</span>
                  <Bd c={hechos === mP.length ? 'var(--success)' : 'var(--accent)'}>{hechos} de {mP.length} pasos</Bd>
                  <span style={{ fontSize: 11.5, color: 'var(--text-muted)' }}>Marca cada paso al completarlo; queda registrado quién y cuándo.</span>
                </div>
                <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(320px, 1fr))', gap: 6 }}>
                  {mP.map(pM => {
                    const c = ok.get(pM.id)
                    return (
                      <label key={pM.id} style={{ display: 'flex', gap: 8, alignItems: 'flex-start', padding: '7px 9px', borderRadius: 9, background: 'var(--bg-surface)', cursor: editable ? 'pointer' : 'default', opacity: c ? .75 : 1 }}>
                        <input type="checkbox" disabled={!editable} checked={!!c} onChange={ev => marcarPaso(pM, ev.target.checked)} style={{ marginTop: 2, accentColor: 'var(--accent)' }} />
                        <span style={{ fontSize: 12, lineHeight: 1.45, minWidth: 0 }}>
                          <b style={{ color: pM.es_control_critico ? 'var(--danger)' : 'var(--accent)' }}>{sel.fase_actual}.{pM.orden}</b>{pM.es_decision ? ' ◆' : ''} <span style={{ textDecoration: c ? 'line-through' : 'none' }}>{pM.accion}</span>
                          <span style={{ display: 'block', fontSize: 10.5, color: 'var(--text-muted)' }}>
                            {pM.documento ? `📄 ${pM.documento}` : ''}{pM.documento && pM.control_tiempo ? ' · ' : ''}{pM.control_tiempo ? `⏱ ${pM.control_tiempo}` : ''}
                            {c ? ` · ✓ ${c.por || ''} ${fFecha(c.fecha)}` : ''}
                          </span>
                        </span>
                      </label>
                    )
                ...
                {gate && <div style={{ marginTop: 8, fontSize: 11.5, color: 'var(--warning-text)', background: 'var(--warning-bg)', padding: '6px 10px', borderRadius: 8, borderLeft: '3px solid var(--warning)' }}><b>Gate de salida de la fase:</b> {gate}</div>}
              </div>
            )
          })()}
...
function EvidenciasFase({ encargo, fase, docs = [], cu, toast, editable, onCambio, enlace }) {
  const [subiendo, setSubiendo] = useState(false)
  const [verEnlace, setVerEnlace] = useState(false)
  const inputRef = useRef(null)
  const mios = docs.filter(d => d.encargo_id === encargo.id && +d.fase === +fase)
  const plantilla = PLANTILLAS[fase]
  ...
  const subir = async (file) => {
    ...
    const path = `${encargo.id}/fase${fase}/${Date.now()}_${limpio}`
    ...
    const { error } = await supabase.from('prc_encargo_docs').insert({
      id: `DOC-${uid()}`, encargo_id: encargo.id, fase, nombre: file.name,
    ...
    <div style={{ marginTop: 4 }}>
      ...
      {editable && (
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', marginTop: 3 }}>
          ...
          <span onClick={() => setVerEnlace(x => !x)} title="Pegar un enlace externo (Drive, etc.)"
