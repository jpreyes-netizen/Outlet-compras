...
import { formatoDePaso, descargarFormatoPaso, imprimirFormatoPaso } from './prcFormatosPaso'
...
export function EncargoSheet({ open, onClose, matriz, cat, cu, toast, usuarios = [], sesion, comiteCodigo, encargos, reasignarDe, onGuardado }) {
  ...
  useEffect(() => {
    ...
    setForm({ ...base, fecha_inicio: hoy(), fecha_limite: sumarMeses(hoy(), 2), autoriza: '', fundamento: '' })
  ...
  const guardar = async () => {
    ...
    const enSesion = !!sesion && !['SIN_QUORUM', 'ANULADA'].includes(sesion.estado)
    if (!enSesion && !form.comite_codigo) return toast('Elige el comité de gobierno que asigna el encargo.', 'err')
    const autoriza = form.autoriza || (cat.comites || []).find(c => c.codigo === form.comite_codigo)?.responsable || ''
    if (!enSesion && !autoriza.trim()) return toast('Fuera de sesión, el encargo requiere la resolución de quien preside el comité que asigna.', 'err')
    if (!enSesion && !form.fundamento?.trim()) return toast('Indica el fundamento de la resolución.', 'err')
    setBusy(true)
    ...
    const { error } = await supabase.from('prc_encargos').insert({
      ...
      origen: (sesion && !['SIN_QUORUM', 'ANULADA'].includes(sesion.estado)) ? 'SESION' : 'SIN_DECISION'
    })
    ...
    if (!(sesion && !['SIN_QUORUM', 'ANULADA'].includes(sesion.estado))) {
      const { error: e3 } = await supabase.rpc('fn_prc_resolucion_encargo', { p_encargo: id, p_autoriza: autoriza, p_fundamento: form.fundamento, p_usuario: cu?.nombre || '—' })
      if (e3) toast('El encargo se creó pero la resolución no se registró: ' + e3.message + '. Formalízala desde el encargo.', 'err')
    }
    setBusy(false)
  ...
  return (
    <Sheet open={open} onClose={onClose} title={reasignarDe ? `Reasignar comité de trabajo · ${reasignarDe.proceso_id}` : 'Encargar proceso a un comité de trabajo'} ancho={680}>
      <div style={{ display: 'flex', flexDirection: 'column', gap: 11 }}>
        ...
        {!(sesion && !['SIN_QUORUM', 'ANULADA'].includes(sesion.estado)) && (
          <div style={{ padding: '9px 12px', borderRadius: 9, background: 'var(--warning-bg)', display: 'flex', flexDirection: 'column', gap: 8 }}>
            <div style={{ fontSize: 12, color: 'var(--warning-text)' }}>
              <b>Resolución fuera de sesión.</b> No se está encargando desde una sesión con quórum: queda como resolución de quien preside el comité que asigna, y se ratifica en su próxima sesión.
            </div>
            <div style={{ display: 'grid', gridTemplateColumns: '1fr 2fr', gap: 9 }}>
              <Campo l="Autoriza (preside el comité)" obligatorio>
                <SelPersona valor={form.autoriza || ((cat.comites || []).find(c => c.codigo === form.comite_codigo)?.responsable || '')}
                  onChange={v => setForm({ ...form, autoriza: v })} personas={personas} ph="— elegir —" />
              </Campo>
              <Campo l="Fundamento" obligatorio hint="Por qué se encarga este proceso ahora.">
                <input style={css.input} value={form.fundamento || ''} onChange={e => setForm({ ...form, fundamento: e.target.value })} />
              </Campo>
            </div>
          </div>
        )}
...
export function PrcEncargos({ matriz, cat, cu, onAbrir, toast }) {
  ...
  /* gate de la fase actual evaluado en la base (misma regla que aplica el avance) */
  const [gateEval, setGateEval] = useState(null)
  const [excepcion, setExcepcion] = useState(null)   // null = cerrado · '' = abierto
  const [busyAv, setBusyAv] = useState(false)
  const [verFaltan, setVerFaltan] = useState(false)   // panel "qué falta para avanzar"
  const [foco, setFoco] = useState(null)              // paso a desplegar en la hoja de trabajo
  const evaluarGate = useCallback(async (id) => {
    if (!id || !supabase.rpc) return setGateEval(null)
    const { data, error } = await supabase.rpc('fn_prc_gate_fase', { p_encargo: id })
    setGateEval(error ? null : data)
  }, [])
  ...
  useEffect(() => { setExcepcion(null); evaluarGate(sel && ACTIVOS.includes(sel.estado) ? sel.id : null) }, [sel, checks, adjuntos, evaluarGate])
  ...
  /* Avanzar y cerrar se ejecutan en la base (fn_prc_avanzar_fase / fn_prc_cerrar_encargo):
     validan el gate y cambian todo en una sola transacción. La excepción exige motivo
     y queda en la bitácora del proceso. */
  const ejecutar = async (fn, motivo) => {
    setBusyAv(true)
    const { data, error } = await supabase.rpc(fn, { p_encargo: sel.id, p_usuario: cu?.nombre || '—', p_excepcion: motivo || null })
    setBusyAv(false)
    if (error) return toast(String(error.message || error).replace(/^.*?: /, ''), 'err')
    setExcepcion(null)
    cargar(true)
    if (fn === 'fn_prc_cerrar_encargo') toast('Comité de trabajo cerrado. El proceso queda en manos de su dueño.')
    else toast(`Fase ${data?.fase} · ${FASES_P37[(data?.fase || 1) - 1]} en curso${data?.excepcion ? ' (por excepción, quedó en la bitácora)' : ''}`)
  }
  // con gate incompleto el botón no se esconde: muestra qué falta y lleva a resolverlo
  const avanzar = () => { if (!gateEval?.ok) { setVerFaltan(true); return } ejecutar('fn_prc_avanzar_fase') }
  const faltantes = (gateEval?.criterios || []).filter(c => !c.ok)
  const irA = (k) => {
    setVerFaltan(false)
    if (['pasos', 'resultados', 'verificacion'].includes(k)) {
      setFoco({ k, t: Date.now() })
      setTimeout(() => document.getElementById('hoja-trabajo')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50)
    } else if (k === 'decision') {
      setTimeout(() => document.getElementById('resolucion-encargo')?.scrollIntoView({ behavior: 'smooth', block: 'center' }), 50)
    } else {
      setTimeout(() => document.getElementById('fases-encargo')?.scrollIntoView({ behavior: 'smooth', block: 'start' }), 50)
    }
  }
  const cerrar = () => ejecutar('fn_prc_cerrar_encargo')
  const conExcepcion = () => {
    if (!String(excepcion || '').trim()) return toast('Escribe el motivo de la excepción: queda registrado en la bitácora.', 'err')
    ejecutar(sel.fase_actual === 7 ? 'fn_prc_cerrar_encargo' : 'fn_prc_avanzar_fase', excepcion.trim())
  }
        ...
        <Cd accent={sel.vencido ? 'var(--danger)' : (EST[sel.estado] || EST.ACTIVO).c}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
              ...
              {editable && ACTIVOS.includes(sel.estado) && sel.fase_actual < 7 && (
                <Bt sm v={gateEval && !gateEval.ok ? 'warn' : 'pri'} dis={busyAv} onClick={avanzar}
                  title={gateEval?.ok ? 'El gate de la fase está completo: cierra la fase y abre la siguiente.' : 'Muestra lo que falta del gate para poder avanzar.'}>
                  {busyAv ? 'Avanzando…' : gateEval && !gateEval.ok
                    ? `Avanzar a fase ${sel.fase_actual + 1} · falta${faltantes.length > 1 ? 'n' : ''} ${faltantes.length}`
                    : `Avanzar a fase ${sel.fase_actual + 1} →`}
                </Bt>
              )}
              {editable && ACTIVOS.includes(sel.estado) && sel.fase_actual === 7 && (
                <Bt v={gateEval && !gateEval.ok ? 'warn' : 'ok'} sm dis={busyAv} onClick={() => gateEval?.ok ? cerrar() : setVerFaltan(true)} title="Cierra el comité de trabajo con el gate de cierre completo: el proceso queda con su dueño">✓ Cerrar comité de trabajo</Bt>
              )}
              {aprueba && ACTIVOS.includes(sel.estado) && gateEval && !gateEval.ok && excepcion === null && (
                <Bt sm v="ghost" onClick={() => setExcepcion('')} title="Solo dirección: avanzar sin gate completo, con motivo que queda en la bitácora">Excepción…</Bt>
              )}
          ...
          {ACTIVOS.includes(sel.estado) && verFaltan && gateEval && !gateEval.ok && (
            <div style={{ marginBottom: 13, padding: '11px 14px', borderRadius: 11, background: 'var(--warning-bg)', border: '1px solid var(--warning)' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 7 }}>
                <b style={{ fontSize: 13, color: 'var(--warning-text)' }}>
                  Para {sel.fase_actual === 7 ? 'cerrar el encargo' : `pasar a la fase ${sel.fase_actual + 1}`} falta{faltantes.length > 1 ? 'n' : ''} {faltantes.length} requisito{faltantes.length > 1 ? 's' : ''} del gate:
                </b>
                <Bt sm v="ghost" style={{ marginLeft: 'auto' }} onClick={() => setVerFaltan(false)}>Cerrar</Bt>
              </div>
              {faltantes.map(c => (
                <div key={c.k} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '4px 0', fontSize: 12.5, flexWrap: 'wrap' }}>
                  <span style={{ color: 'var(--danger)', fontWeight: 800 }}>✗</span>
                  <b>{c.label}</b>
                  <span style={{ color: 'var(--text-muted)' }}>· {c.detalle}</span>
                  <Bt sm v="sec" style={{ marginLeft: 'auto' }} onClick={() => irA(c.k)}>Resolver →</Bt>
                </div>
              ))}
              {aprueba && excepcion === null && (
                <div style={{ marginTop: 6, fontSize: 11.5, color: 'var(--text-muted)' }}>
                  ¿Corresponde avanzar igual? <span onClick={() => { setVerFaltan(false); setExcepcion('') }} style={{ color: 'var(--accent)', fontWeight: 700, cursor: 'pointer' }}>Avanzar por excepción con motivo…</span>
                </div>
              )}
            </div>
          )}

          {ACTIVOS.includes(sel.estado) && excepcion !== null && (
            <div style={{ marginBottom: 13, padding: '10px 13px', borderRadius: 11, background: 'var(--danger-bg)', display: 'flex', gap: 8, alignItems: 'center', flexWrap: 'wrap' }}>
              <span style={{ fontSize: 12.5, color: 'var(--danger-text)', flex: '1 1 260px' }}>
                <b>Avance por excepción.</b> El gate no está completo; si igual corresponde {sel.fase_actual === 7 ? 'cerrar' : 'avanzar'}, deja el motivo. Queda en la bitácora del proceso con tu nombre.
              </span>
              <input style={{ ...css.input, flex: '2 1 300px' }} value={excepcion} placeholder="Motivo de la excepción" onChange={e => setExcepcion(e.target.value)} />
              <Bt sm v="warn" dis={busyAv} onClick={conExcepcion}>{sel.fase_actual === 7 ? 'Cerrar por excepción' : 'Avanzar por excepción'}</Bt>
              <Bt sm v="ghost" onClick={() => setExcepcion(null)}>Cancelar</Bt>
            </div>
          )}

          {ACTIVOS.includes(sel.estado) && !sel.decision_id && (
            <div id="resolucion-encargo"><ResolucionEncargo encargo={sel} cat={cat} cu={cu} personas={personas} toast={toast} editable={aprueba || editable} onHecho={() => cargar(true)} /></div>
          )}
          ...
          {ACTIVOS.includes(sel.estado) && (() => {
            ...
            return (
              <HojaDeTrabajo encargo={sel} fase={sel.fase_actual} faseMetodo={mF} pasos={mP} gate={gate} gateEval={gateEval} foco={foco}
                registros={registrosDe(sel.id)} docs={adjuntos} cu={cu} editable={editable} toast={toast}
                personas={personas} onCambio={() => cargar(true)} />
            )
          })()}

          <div id="fases-encargo" style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 14, alignItems: 'start' }}>
                ...
                <tbody>{fasesSel.map(f => {
                  ...
                  return (
                    <tr key={f.id} style={{ background: f.fase === sel.fase_actual && ACTIVOS.includes(sel.estado) ? 'var(--accent-bg)' : 'transparent' }}>
                      ...
                      <td style={css.td} title="El estado cambia solo con Avanzar, que valida el gate"><Bd c={EST_FASE[f.estado]}>{f.estado}</Bd></td>
...
function EvidenciasFase({ encargo, fase, pasoId = null, paso = null, docs = [], cu, toast, editable, onCambio, enlace, compacto = false }) {
  ...
  const fmtPaso = paso ? formatoDePaso(fase, paso) : null
    ...
    <div style={{ marginTop: 4 }}>
      ...
      {editable && (
        <div style={{ display: 'flex', gap: 6, alignItems: 'center', flexWrap: 'wrap', marginTop: 3 }}>
          ...
          {fmtPaso && (<>
            <span onClick={() => descargarFormatoPaso(fase, paso, encargo)}
              title={`Descarga el formato ${fmtPaso.codigo} (${fmtPaso.titulo}) en Word, pre-llenado con los datos del encargo`}
              style={{ fontSize: 10.5, fontWeight: 700, color: 'var(--accent)', cursor: 'pointer' }}>
              📄 Formato {fmtPaso.codigo} (Word)
            </span>
            <span onClick={() => { if (!imprimirFormatoPaso(fase, paso, encargo)) toast('El navegador bloqueó la ventana: permite ventanas emergentes para este sitio.', 'err') }}
              title={`Abre el formato ${fmtPaso.codigo} listo para imprimir y llenar a mano; después se escanea y se carga con 📎 Adjuntar`}
              style={{ fontSize: 10.5, fontWeight: 700, color: 'var(--accent)', cursor: 'pointer' }}>
              🖨 Imprimir
            </span>
          </>)}
...
function HojaDeTrabajo({ encargo, fase, faseMetodo, pasos, gate, gateEval, foco, registros, docs, cu, editable, toast, personas, onCambio }) {
  const [abierto, setAbierto] = useState(null)      // paso_id expandido
  /* lo que le falta a cada paso para contar en el gate (misma regla que la base) */
  const faltaDe = (p) => {
    const r = registros.get(p.id) || {}
    const est = r.estado || 'PENDIENTE'
    const f = []
    if (!['HECHO', 'NO_APLICA'].includes(est)) f.push('hacer')
    if (est === 'HECHO' && (p.es_decision || p.es_control_critico) && !String(r.resultado || '').trim()) f.push(p.es_decision ? 'resultado SÍ/NO' : 'resultado OK/NO OK')
    if (['HECHO', 'NO_APLICA'].includes(est) && !r.verificado_at) f.push('verificar')
    return f
  }
  useEffect(() => {
    if (!foco) return
    const p = pasos.find(x => faltaDe(x).length)
  ...
  return (
    <div id="hoja-trabajo" style={{ marginBottom: 13, borderRadius: 12, border: '1px solid var(--accent)', overflow: 'hidden', scrollMarginTop: 16 }}>
              ...
              <div style={{ display: 'flex', gap: 10, alignItems: 'flex-start', padding: '9px 14px', cursor: 'pointer' }} onClick={() => setAbierto(exp ? null : p.id)}>
                ...
                <div style={{ flex: 1, minWidth: 0 }}>
                  ...
                  <div style={{ fontSize: 10.5, color: 'var(--text-muted)', marginTop: 2, display: 'flex', gap: 8, flexWrap: 'wrap' }}>
                    ...
                    {faltaDe(p).length > 0 && <span style={{ color: 'var(--danger)', fontWeight: 700 }}>⚠ falta: {faltaDe(p).join(' · ')}</span>}
                  </div>
                </div>
              ...
              {exp && (
                <div style={{ padding: '4px 14px 12px 54px', display: 'flex', flexDirection: 'column', gap: 9 }}>
                  ...
                  <div>
                    ...
                    <EvidenciasFase encargo={encargo} fase={fase} pasoId={p.id} paso={p} docs={docs} cu={cu} toast={toast} editable={editable} onCambio={onCambio} compacto />
                  </div>
      ...
      {(gate || gateEval) && (
        <div style={{ padding: '10px 14px', fontSize: 11.5, background: gateEval?.ok ? 'var(--success-bg)' : 'var(--warning-bg)', borderTop: '1px solid var(--border)' }}>
          <div style={{ fontWeight: 800, color: gateEval?.ok ? 'var(--success)' : 'var(--warning-text)', marginBottom: 5 }}>
            {gateEval?.ok ? '✓ Gate de salida completo: la fase puede cerrarse' : 'Gate de salida de la fase — lo que falta para avanzar'}
          </div>
          {gate && <div style={{ color: 'var(--text-muted)', marginBottom: 6 }}>{gate}</div>}
          {(gateEval?.criterios || []).map(c => (
            <div key={c.k} style={{ display: 'flex', gap: 7, alignItems: 'baseline', padding: '2px 0' }}>
              <span style={{ fontWeight: 800, color: c.ok ? 'var(--success)' : 'var(--danger)', minWidth: 14 }}>{c.ok ? '✓' : '✗'}</span>
...
/* ═══════════════════════════════════════════════════════════════════════════
   El encargo nace de una decisión — regularización por resolución
   ───────────────────────────────────────────────────────────────────────────
   Todo comité de trabajo debe tener un origen trazable: una decisión tomada en
   sesión con quórum o, mientras el gobierno no sesiona, una resolución fuera de
   sesión de quien preside el comité que asigna (queda como decisión formal y
   se ratifica en la próxima sesión). Sin eso la fase 1 no cierra.
   ═══════════════════════════════════════════════════════════════════════════ */
function ResolucionEncargo({ encargo, cat, cu, personas, toast, editable, onHecho }) {
  const comite = (cat.comites || []).find(c => c.codigo === encargo.comite_codigo)
  const [abierto, setAbierto] = useState(false)
  const [autoriza, setAutoriza] = useState(comite?.responsable || '')
  const [fundamento, setFundamento] = useState(encargo.objetivo || '')
  const [busy, setBusy] = useState(false)
  useEffect(() => { setAutoriza(comite?.responsable || ''); setFundamento(encargo.objetivo || '') }, [encargo.id])   // eslint-disable-line react-hooks/exhaustive-deps

  const resolver = async () => {
    if (!autoriza?.trim()) return toast('Indica quién autoriza la resolución.', 'err')
    if (!fundamento?.trim()) return toast('Indica el fundamento: por qué se encarga este proceso ahora.', 'err')
    setBusy(true)
    const { error } = await supabase.rpc('fn_prc_resolucion_encargo', { p_encargo: encargo.id, p_autoriza: autoriza, p_fundamento: fundamento, p_usuario: cu?.nombre || '—' })
    setBusy(false)
    if (error) return toast(String(error.message || error), 'err')
    toast('Encargo formalizado por resolución. Queda para ratificar en la próxima sesión del comité.')
    setAbierto(false); onHecho?.()
  }

  return (
    <div style={{ marginBottom: 13, padding: '10px 13px', borderRadius: 11, background: 'var(--warning-bg)', color: 'var(--warning-text)', fontSize: 12.5, lineHeight: 1.6 }}>
      <div style={{ display: 'flex', gap: 10, alignItems: 'center', flexWrap: 'wrap' }}>
        <span style={{ flex: 1, minWidth: 260 }}>
          <b>Este encargo no tiene decisión formal.</b> No se tomó en una sesión con quórum{comite ? ` del ${comite.nombre}` : ''}.
          Formalízalo con una resolución de quien preside el comité que asigna; se ratifica en la próxima sesión. Sin esto la fase 1 no cierra.
        </span>
        {editable && !abierto && <Bt sm onClick={() => setAbierto(true)}>Formalizar por resolución →</Bt>}
      </div>
      {abierto && (
        <div style={{ display: 'grid', gridTemplateColumns: 'minmax(220px, 1fr) minmax(260px, 2fr) auto', gap: 10, alignItems: 'end', marginTop: 9 }}>
          <Campo l="Autoriza (preside el comité)" obligatorio>
            <SelPersona valor={autoriza} onChange={setAutoriza} personas={personas} ph="— elegir —" />
          </Campo>
          <Campo l="Fundamento" obligatorio hint="Por qué se encarga este proceso ahora.">
            <input style={css.input} value={fundamento} onChange={e => setFundamento(e.target.value)} />
          </Campo>
          <div style={{ display: 'flex', gap: 6 }}>
            <Bt sm dis={busy} onClick={resolver}>{busy ? 'Registrando…' : 'Registrar resolución'}</Bt>
            <Bt sm v="ghost" onClick={() => setAbierto(false)}>Cancelar</Bt>
          </div>
        </div>
      )}
    </div>
