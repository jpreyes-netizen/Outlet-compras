...
export function PrcEncargos({ matriz, cat, cu, onAbrir, toast }) {
  ...
  const avanzar = () => ejecutar('fn_prc_avanzar_fase')
      ...
      {sel && (
        <Cd accent={sel.vencido ? 'var(--danger)' : (EST[sel.estado] || EST.ACTIVO).c}>
          <div style={{ display: 'flex', gap: 12, alignItems: 'flex-start', flexWrap: 'wrap', marginBottom: 12 }}>
            ...
            <div style={{ display: 'flex', gap: 6, flexWrap: 'wrap', justifyContent: 'flex-end' }}>
              ...
              {editable && ACTIVOS.includes(sel.estado) && sel.fase_actual < 7 && (
                <Bt sm dis={busyAv || !gateEval?.ok} onClick={avanzar}
                  title={gateEval?.ok ? 'El gate de la fase está completo: cierra la fase y abre la siguiente.' : 'El gate de la fase no está completo: revisa la lista del gate en la hoja de trabajo.'}>
                  {busyAv ? 'Avanzando…' : `Avanzar a fase ${sel.fase_actual + 1} →`}
                </Bt>
              )}
              {editable && ACTIVOS.includes(sel.estado) && sel.fase_actual === 7 && (
                <Bt v="ok" sm dis={busyAv || !gateEval?.ok} onClick={cerrar} title="Cierra el comité de trabajo con el gate de cierre completo: el proceso queda con su dueño">✓ Cerrar comité de trabajo</Bt>
              )}
          ...
          {ACTIVOS.includes(sel.estado) && !sel.decision_id && (
            <ResolucionEncargo encargo={sel} cat={cat} cu={cu} personas={personas} toast={toast} editable={aprueba || editable} onHecho={() => cargar(true)} />
          )}
          ...
          {ACTIVOS.includes(sel.estado) && (() => {
            ...
            return (
              <HojaDeTrabajo encargo={sel} fase={sel.fase_actual} faseMetodo={mF} pasos={mP} gate={gate} gateEval={gateEval}
...
function HojaDeTrabajo({ encargo, fase, faseMetodo, pasos, gate, gateEval, registros, docs, cu, editable, toast, personas, onCambio }) {
