...
export function PrcEncargos({ matriz, cat, cu, onAbrir, toast }) {
  ...
  const avanzar = async () => {
    ...
    if (!actual) return
    // nómina: vale la marca de conformación o, como respaldo, una nómina válida ya registrada
    const nomina = (sel.integrantes || []).filter(Boolean)
    const equipoOk = !!sel.equipo_confirmado_at || (nomina.length >= 3 && nomina.length % 2 === 1)
    if (sel.fase_actual === 1 && !equipoOk)
    ...
    const sig = fasesSel.find(f => f.fase === nf)
    // cada paso se verifica: si algo falla se avisa y no queda un estado a medias sin saberlo
    const r1 = await supabase.from('prc_encargo_fases').update({ estado: 'COMPLETADA', fecha_fin: actual.fecha_fin || hoy(), fecha_inicio: actual.fecha_inicio || hoy() }).eq('id', actual.id)
    if (r1.error) return err(r1.error)
    if (sig) {
      const r2 = await supabase.from('prc_encargo_fases').update({ estado: 'EN_CURSO', fecha_inicio: sig.fecha_inicio || hoy() }).eq('id', sig.id)
      if (r2.error) return err(r2.error)
    }
    const r3 = await supabase.from('prc_encargos').update({ fase_actual: nf, estado }).eq('id', sel.id)
