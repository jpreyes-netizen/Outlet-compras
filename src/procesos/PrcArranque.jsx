...
import { Cd, Bt, Bd, Hint, Campo, Chips, Vacio, css, hoy, uid, fFecha, Barra } from './prcUI'
...
export function PrcArranque({ cat, matriz = [], cu, toast, onCerrar, onListo }) {
  ...
  /* candidatos que a esa hora están libres y no están ya en el equipo */
  const libres = useMemo(() => {
    if (!cd || !comiteSel) return []
    const propuesta = { codigo: comiteSel.codigo, dia_semana: cd.dia_semana, hora_inicio: cd.hora_inicio, hora_fin: cd.hora_fin, duracion_min: comiteSel.duracion_min }
    return sugerencias
      .filter(n => !(cd.integrantes || []).some(x => mismo(x, n)))
      .filter(n => { const d = disponibilidad(n, propuesta, { comites: mundo, encargos, sesiones }); return !d.choques.length && !d.sobrecargada })
  }, [cd, comiteSel, sugerencias, mundo, encargos, sesiones])
          ...
          {cd && comiteSel && (
            <div style={{ flex: '1 1 460px', minWidth: 320, display: 'flex', flexDirection: 'column', gap: 12 }}>
              ...
              <Campo l="Integrantes con derecho a voto" obligatorio
                ...
                <Chips valores={cd.integrantes} onChange={v => set('integrantes', v)} sugerencias={sugerencias} ph="Nombre — escribe y Enter" />
              </Campo>
                ...
                <div>
                  ...
                  {sugerencias.length > 0 && (
                    <div style={{ marginTop: 7 }}>
                      <div style={{ fontSize: 11.5, color: 'var(--text-muted)', marginBottom: 4 }}>Sin carga y sin choque a esa hora — clic para sumar:</div>
                      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 5 }}>
                        {libres.slice(0, 10).map(n => (
                          <span key={n} onClick={() => set('integrantes', [...cd.integrantes, n])}
                            style={{ fontSize: 11.5, padding: '3px 9px', borderRadius: 999, cursor: 'pointer', background: 'var(--accent-bg)', color: 'var(--accent)', fontWeight: 600 }}>
                            + {n}
                          </span>
                        ))}
                        {!libres.length && <span style={{ fontSize: 11.5, color: 'var(--text-muted)' }}>Todos los candidatos del ERP ya tienen algo a esa hora o ya están en el equipo.</span>}
                      </div>
                    </div>
                  )}
                </div>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <Campo l="Secretario/a de actas" obligatorio hint="Por nombre, no por área.">
                  <select style={css.select} value={cd.secretario} onChange={e => set('secretario', e.target.value)}>
                    <option value="">— elegir —</option>
                    {cd.integrantes.map(n => <option key={n} value={n}>{n}</option>)}
                  </select>
                </Campo>
