...
import { sumarDias, TIPOS_OD } from './prcComite'
...
export function PrcArranque({ cat, matriz = [], cu, toast, onCerrar, onListo }) {
  ...
  useEffect(() => {
    supabase.from('usuarios').select('id, nombre, cargo, rol').limit(300)
      .then(r => { setUsuarios(r.error ? [] : (r.data || []).filter(u => u.nombre)); setUCargados(true) })
  }, [])
  ...
  const sugerencias = useMemo(() => [...new Set([
    ...usuarios.map(u => u.nombre),
    ...matriz.map(p => p.dueno_persona).filter(Boolean)
  ])].sort(), [usuarios, matriz])
