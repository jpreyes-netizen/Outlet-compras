...
//
// Convención visual (estándar ODP · simbología BPMN 2.0 adaptada, 09-2026):
// carriles blancos con cabecera gris y bandas de etapa en pastel; evento de
// INICIO = círculo verde de trazo fino con señal de partida; evento de FIN =
// círculo rojo de trazo grueso con punto de término; tareas = rectángulos
// redondeados (celeste #DAE8FC si generan documento) con insignias de tipo en
// el borde superior (escudo = control crítico, engranaje = usa sistema, reloj =
// con plazo, personas = participan otros roles); decisiones = rombo amarillo
// con marca × (gateway exclusivo) y ramas SÍ/NO; retornos y saltos en naranjo
// discontinuo por calles; "viene de / continúa en" = evento de enlace (círculo
// doble con flecha: rellena si el flujo sale, hueca si entra).
//
...
export function flujoSVG(proceso, fases, pasos, opts = {}) {
  ...
  /* ── simbología BPMN: iconos vectoriales por paso ────────────────────────── */
  const GRIS_IC = '#475569'
  const icGear = (cx2, cy2) => {
    o.push(`<circle cx="${cx2}" cy="${cy2}" r="3.3" fill="none" stroke="${GRIS_IC}" stroke-width="2.1" stroke-dasharray="2 1.7"/>`)
    o.push(`<circle cx="${cx2}" cy="${cy2}" r="1.1" fill="${GRIS_IC}"/>`)
  }
  const icClock = (cx2, cy2) => {
    o.push(`<circle cx="${cx2}" cy="${cy2}" r="4.1" fill="none" stroke="${GRIS_IC}" stroke-width="1.3"/>`)
    o.push(`<path d="M ${cx2} ${cy2 - 2.3} V ${cy2} L ${cx2 + 1.9} ${cy2 + 1.4}" fill="none" stroke="${GRIS_IC}" stroke-width="1.3" stroke-linecap="round"/>`)
  }
  const icPpl = (cx2, cy2) => {
    o.push(`<circle cx="${cx2 + 1.7}" cy="${cy2 - 1.5}" r="1.7" fill="none" stroke="${GRIS_IC}" stroke-width="1.1"/>`)
    o.push(`<path d="M ${cx2 - 0.4} ${cy2 + 4.3} q 2.1 -3.4 4.2 0" fill="none" stroke="${GRIS_IC}" stroke-width="1.1"/>`)
    o.push(`<circle cx="${cx2 - 1.8}" cy="${cy2 - 2}" r="1.9" fill="#FFFFFF" stroke="${GRIS_IC}" stroke-width="1.2"/>`)
    o.push(`<path d="M ${cx2 - 4.3} ${cy2 + 4.1} q 2.5 -3.9 5 0" fill="#FFFFFF" stroke="${GRIS_IC}" stroke-width="1.2"/>`)
  }
  const icShield = (cx2, cy2) => {
    o.push(`<path d="M ${cx2} ${cy2 - 4.4} L ${cx2 + 3.4} ${cy2 - 3} V ${cy2 + 0.4} Q ${cx2 + 3.4} ${cy2 + 3.2} ${cx2} ${cy2 + 4.5} Q ${cx2 - 3.4} ${cy2 + 3.2} ${cx2 - 3.4} ${cy2 + 0.4} V ${cy2 - 3} Z" fill="${CRITICO}"/>`)
    o.push(`<text x="${cx2}" y="${cy2 + 2.6}" font-size="6.2" font-weight="900" fill="#FFFFFF" text-anchor="middle">!</text>`)
  }
  const icDoc = (cx2, cy2) => {
    o.push(`<path d="M ${cx2 - 3.4} ${cy2 - 4.4} h 4.2 l 2.6 2.6 v 6.2 h -6.8 z" fill="${DOC_BG}" stroke="${DOC_BORDE}" stroke-width="1.1"/>`)
    o.push(`<path d="M ${cx2 + 0.8} ${cy2 - 4.4} v 2.6 h 2.6" fill="none" stroke="${DOC_BORDE}" stroke-width="1.1"/>`)
  }
  // insignia sobre el borde superior de la tarea (estilo marcador BPMN)
  const badge = (cx2, cy2, dibujo, borde = '#CBD5E1') => {
    o.push(`<circle cx="${cx2}" cy="${cy2}" r="7.5" fill="#FFFFFF" stroke="${borde}" stroke-width="1.2"/>`)
    dibujo(cx2, cy2)
  }
  ...
  // evento de enlace BPMN: círculo doble con flecha (rellena = el flujo sale; hueca = entra)
  const conector = (cx, cy, rotulo, ref, sub, sale) => {
    o.push(`<text x="${cx}" y="${cy - 20}" font-size="7" font-weight="800" letter-spacing=".8" fill="${CONECTOR}" text-anchor="middle">${esc(rotulo)}</text>`)
    o.push(`<circle cx="${cx}" cy="${cy}" r="13" fill="#F8FAFC" stroke="${CONECTOR}" stroke-width="1.5"/>`)
    o.push(`<circle cx="${cx}" cy="${cy}" r="10" fill="none" stroke="${CONECTOR}" stroke-width="1"/>`)
    const rel = sale ? `fill="${CONECTOR}" stroke="none"` : `fill="#F8FAFC" stroke="${CONECTOR}" stroke-width="1.3"`
    o.push(`<path d="M ${cx - 5.2} ${cy - 2.6} h 4.6 v -2.6 l 5.8 5.2 -5.8 5.2 v -2.6 h -4.6 z" ${rel}/>`)
    o.push(`<text x="${cx}" y="${cy + 27}" font-size="11" font-weight="800" fill="#1E293B" text-anchor="middle">${esc(ref)}</text>`)
    if (sub) o.push(`<text x="${cx}" y="${cy + 37}" font-size="8" fill="#64748B" text-anchor="middle">${esc(wrap(sub, 16, 1)[0])}</text>`)
  }

  if (m.conInicio) {
    // evento de inicio BPMN: círculo de trazo fino con señal de partida
    o.push(`<circle cx="${iniCx}" cy="${iniCy}" r="15" fill="#D5E8D4" stroke="#82B366" stroke-width="2" filter="url(#sh)"/>`)
    o.push(`<path d="M ${iniCx - 3.6} ${iniCy - 5.6} L ${iniCx + 6.2} ${iniCy} L ${iniCx - 3.6} ${iniCy + 5.6} Z" fill="#1F5B24"/>`)
    o.push(`<text x="${iniCx}" y="${iniCy + 29}" font-size="9.5" font-weight="800" letter-spacing="1" fill="#1F5B24" text-anchor="middle">INICIO</text>`)
    o.push(`<path d="M ${iniCx + 15} ${iniCy} L ${xIn} ${iniCy}" fill="none" stroke="#82B366" stroke-width="1.8" marker-end="url(#ar)"/>`)
  } else if (m.entrada) {
    conector(iniCx, iniCy, 'VIENE DE', m.entrada.num || '—', m.entrada.fase, false)
    o.push(`<path d="M ${iniCx + 13} ${iniCy} L ${xIn} ${iniCy}" fill="none" stroke="${CONECTOR}" stroke-width="1.6" marker-end="url(#arp)"/>`)
  }
  if (m.conFin) {
    // evento de fin BPMN: círculo de trazo grueso con punto de término
    o.push(`<circle cx="${finCx}" cy="${finCy}" r="15" fill="#F8CECC" stroke="${CRITICO}" stroke-width="3.6" filter="url(#sh)"/>`)
    o.push(`<circle cx="${finCx}" cy="${finCy}" r="5.4" fill="#7C2B25"/>`)
    o.push(`<text x="${finCx}" y="${finCy + 30}" font-size="9.5" font-weight="800" letter-spacing="1.5" fill="#7C2B25" text-anchor="middle">FIN</text>`)
    if (ramaFin !== null) {
      o.push(`<path d="M ${xOut} ${finCy} L ${finCx - 17} ${finCy}" fill="none" stroke="${CRITICO}" stroke-width="1.8" marker-end="url(#arc)"/>`)
      etiquetaRama((xOut + finCx - 17) / 2, finCy - 7, ramaFin)
    }
  } else if (m.salida && ramaFin !== null) {
    conector(finCx, finCy, 'CONTINÚA EN', m.salida.num || '—', m.salida.fase, true)
    o.push(`<path d="M ${xOut} ${finCy} L ${finCx - 13} ${finCy}" fill="none" stroke="${CONECTOR}" stroke-width="1.6" marker-end="url(#arp)"/>`)
    etiquetaRama((xOut + finCx - 13) / 2, finCy - 7, ramaFin)
  }
  ...
  cols.forEach((c, i) => {
    ...
    if (p.es_decision) {
      // gateway exclusivo BPMN: rombo con marca ×
      ...
      o.push(`<path d="M ${cx - 3.4} ${y + 8.6} l 6.8 6.8 M ${cx + 3.4} ${y + 8.6} l -6.8 6.8" stroke="#B8860B" stroke-width="2.2" stroke-linecap="round" fill="none"/>`)
    } else {
      // tarea BPMN: rectángulo de esquinas redondeadas con insignias de tipo en el borde
      o.push(`<rect x="${x}" y="${y}" width="${D.BOX_W}" height="${D.BOX_H}" rx="10" fill="${fondoCaja}" stroke="${stroke}" stroke-width="${sw}" filter="url(#sh)"/>`)
      if (p.es_control_critico) {
        o.push(`<rect x="${x}" y="${y + 8}" width="5" height="${D.BOX_H - 16}" rx="2.5" fill="${CRITICO}"/>`)
      }
      const marcas = []
      if (p.es_control_critico) marcas.push(cx2 => badge(cx2, y, icShield, CRITICO))
      if (p.sistema) marcas.push(cx2 => badge(cx2, y, icGear))
      if (p.control_tiempo) marcas.push(cx2 => badge(cx2, y, icClock))
      if ((p.participantes || []).filter(Boolean).length) marcas.push(cx2 => badge(cx2, y, icPpl))
      let bx = x + D.BOX_W - (p.documento ? 34 : 14)
      marcas.forEach(fn => { fn(bx); bx -= 17 })
    }
  ...
  if (!opts.sinLeyenda) {
    ...
    item(x => {
      o.push(`<circle cx="${x + 7}" cy="${ly - 7}" r="6.5" fill="#D5E8D4" stroke="#82B366" stroke-width="1.6"/>`)
      o.push(`<path d="M ${x + 5.2} ${ly - 9.6} L ${x + 9.8} ${ly - 7} L ${x + 5.2} ${ly - 4.4} Z" fill="#1F5B24"/>`)
    }, 14, 'Inicio', 30)
    item(x => {
      o.push(`<circle cx="${x + 7}" cy="${ly - 7}" r="6.5" fill="#F8CECC" stroke="${CRITICO}" stroke-width="2.4"/>`)
      o.push(`<circle cx="${x + 7}" cy="${ly - 7}" r="2.4" fill="#7C2B25"/>`)
    }, 14, 'Fin', 18)
    item(x => {
      o.push(`<polygon points="${x + 8},${ly - 15} ${x + 17},${ly - 7} ${x + 8},${ly + 1} ${x - 1},${ly - 7}" fill="#FFF2CC" stroke="#D6B656" stroke-width="1.6"/>`)
      o.push(`<path d="M ${x + 5.8} ${ly - 9.2} l 4.4 4.4 M ${x + 10.2} ${ly - 9.2} l -4.4 4.4" stroke="#B8860B" stroke-width="1.7" stroke-linecap="round"/>`)
    }, 17, 'Decisión (SÍ / NO)', 88)
    item(x => {
    ...
    }, 13, 'Genera documento', 90)
    item(x => badge(x + 7, ly - 7, icShield, CRITICO), 15, 'Control crítico', 72)
    item(x => badge(x + 7, ly - 7, icGear), 15, 'Usa sistema', 58)
    item(x => badge(x + 7, ly - 7, icClock), 15, 'Con plazo', 48)
    item(x => badge(x + 7, ly - 7, icPpl), 15, 'Participan otros roles', 106)
    ...
    item(x => {
      o.push(`<circle cx="${x + 7}" cy="${ly - 7}" r="6.8" fill="#F8FAFC" stroke="${CONECTOR}" stroke-width="1.2"/>`)
      o.push(`<path d="M ${x + 4.2} ${ly - 8.2} h 2.4 v -1.4 l 3.2 2.6 -3.2 2.6 v -1.4 h -2.4 z" fill="${CONECTOR}"/>`)
    }, 15, 'Viene de / continúa en (enlace)', 146)
...
export function flujoDrawio(proceso, fases, pasos, opts = {}) {
  ...
  cols.forEach((c, i) => {
    ...
    const style = p.es_decision
      ...
      : p.es_control_critico
        ? `rounded=1;arcSize=12;whiteSpace=wrap;html=1;fillColor=${fondoPaso};strokeColor=${CRITICO};strokeWidth=3;fontSize=11;align=center;`
        : `rounded=1;arcSize=12;whiteSpace=wrap;html=1;fillColor=${fondoPaso};strokeColor=${bordePaso};strokeWidth=1;fontSize=11;align=center;`
    cells.push(
  ...
  const finId = nid()
  cells.push(
    `<mxCell id="${finId}" value="FIN" style="ellipse;html=1;fillColor=#F8CECC;strokeColor=${CRITICO};strokeWidth=4;fontSize=11;fontStyle=1;fontColor=#7C2B25;" `
