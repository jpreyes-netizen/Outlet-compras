...
const ROLES = [
  ...
  },
  {
    ...
    sucursales:["suc-maipu","suc-lg","suc-la"],
...
const PV_CAP_MAP = {
  ...
  'form3':             'pv.form3',
  'form4':             'pv.form4',
  'form4_transfer':    'pv.form4.transfer',
...
// Estacion Finanzas / FORM 4A: unica fuente de verdad para "confirmar transferencia".
// RBAC: capability pv.form4.transfer. Fallback legado: roles caja / gerencia / admin.
const puedeTransferir = u => hp(u, 'form4_transfer')

// Admin postventa: rol legado 'admin' O capability pv.admin (usuario_acceso postventa.admin).
// Permite que usuarios con rol global distinto (ej. jefe_admin_finanzas) operen como admin PV.
const esAdminPV = cu =>
  cu?.rol === 'admin' || !!canSync(cu, 'postventa', 'pv.admin')
...
const ESTADOS = {
  ...
  pendiente_decision_postventa: {l:"Pendiente decisión PV", c:"#FF9500", bg:"#FF950015", ic:"⚠️"},
...
// El envio usa el canal que ya opera en el ERP de Compras: se inserta una
// fila por destinatario en la tabla `notificaciones` y el trigger
// trg_send_email_on_notification despacha el correo via la edge function
// send-notification-email. Insertar y registrar son el mismo acto: no puede
// salir un correo sin quedar en la tabla, ni quedar en la tabla sin
// intentarse el envio. No requiere API key en el frontend.
//
// Se mantiene la firma sendEmail(tipo, data, destinatarios) para no tocar
// los puntos de llamada. El asunto y el cuerpo se componen aca, porque el
// canal del ERP recibe el mensaje ya armado.

const URL_ERP = 'https://outletdepuertas-erp.netlify.app'

const PLANTILLAS_NOTIF = {
  caso_creado: d => ({
    asunto: `${d.numero} · Nuevo caso postventa — ${d.cliente_nombre}`,
    mensaje:
`Se registró un nuevo caso de postventa.

Caso: ${d.numero}
Cliente: ${d.cliente_nombre} · ${d.cliente_rut}
Documento: ${(d.doc_tipo || '').toUpperCase()} N°${d.doc_numero}
Sucursal: ${NOMBRE_SUC[d.sucursal] || d.sucursal}
Canal: ${d.canal_ingreso}
Clasificación provisional: ${d.codigo_provisional || '—'}
Motivo del cliente: ${d.motivo_cliente}
${d.requiere_form2 ? 'Requiere validación técnica (FORM 2).\n' : ''}${d.es_critico ? 'CASO CRÍTICO — escalado automático.\n' : ''}Registrado por: ${d.ejecutivo_nombre}

Revisa el detalle en el ERP:
${URL_ERP}`
  }),

  form2_completado: d => ({
    asunto: `${d.numero} · Validación técnica: ${d.procede_reclamo ? 'PROCEDE' : 'NO procede'}`,
    mensaje:
`Validación técnica completada para el caso ${d.numero} (${d.cliente_nombre}).

Veredicto: ${d.procede_reclamo ? 'PROCEDE el reclamo — pasa a resolución' : 'NO procede según Operaciones — Postventa debe decidir'}
${d.producto_nombre ? 'Producto: ' + d.producto_nombre + '\n' : ''}Conclusión: ${d.conclusion_tecnica}
${!d.procede_reclamo && d.motivo_rechazo ? 'Argumento técnico: ' + d.motivo_rechazo + '\n' : ''}Validado por: ${d.validador_nombre}

Revisa el detalle en el ERP:
${URL_ERP}`
  }),

  diferencia_cantidades: d => ({
    asunto: `${d.numero} · ⚠ Diferencia de cantidades en validación`,
    mensaje:
`Se detectó una diferencia entre lo reclamado por el cliente y lo recibido por Operaciones.

Caso: ${d.numero} · ${d.cliente_nombre} (${d.cliente_rut})
...
${URL_ERP}`
  ...
  form3_completado: d => {
    const tipos = {
      ...
      nc_abono:    'NC con saldo a favor',
      nc_transfer: 'NC con transferencia',
      rechazar:    'Caso rechazado',
      escalar:     'Escalado a jefatura',
    }
    const esTransfer = d.tipo_resolucion === 'nc_transfer'
    return {
      asunto: esTransfer
        ? `${d.numero} · 🏦 Transferencia por ejecutar — ${fmt(Number(d.monto) || 0)}`
        : `${d.numero} · Resolución: ${tipos[d.tipo_resolucion] || d.tipo_resolucion}`,
      mensaje:
`El caso ${d.numero} de ${d.cliente_nombre} registró resolución.

Tipo: ${tipos[d.tipo_resolucion] || d.tipo_resolucion}
${d.bsale_doc_numero ? 'Documento BSALE: N°' + d.bsale_doc_numero + '\n' : ''}${Number(d.monto) > 0 ? 'Monto: ' + fmt(Number(d.monto)) + '\n' : ''}${esTransfer ? `
INSTRUCCIÓN DE TRANSFERENCIA PARA CAJA
Titular: ${d.nombre_titular} · RUT ${d.rut_titular}
Banco: ${d.banco} · ${d.tipo_cuenta} · Cuenta ${d.num_cuenta}
` : ''}Resuelto por: ${d.resuelto_por_nombre}
...
${URL_ERP}`
  ...
  caso_escalado: d => ({
    asunto: `${d.numero} · 🔺 Caso escalado a jefatura`,
    mensaje:
`El caso ${d.numero} de ${d.cliente_nombre} fue escalado y requiere tu decisión.

Sucursal: ${NOMBRE_SUC[d.sucursal] || d.sucursal}
Motivo del escalamiento: ${d.escalar_motivo}
Escalado por: ${d.escalado_por}
...
${URL_ERP}`
  ...
  form4_completado: d => ({
    asunto: `${d.numero} · Decisión de jefatura`,
    mensaje:
`Jefatura resolvió el escalamiento del caso ${d.numero}.

Decisión: ${d.decision_label || '—'}
Observaciones: ${d.observaciones || '—'}
Resuelto por: ${d.cerrado_por_nombre}

Revisa el detalle en el ERP:
${URL_ERP}`
  }),

  transfer_ejecutada: d => ({
    asunto: `${d.numero} · 💸 Transferencia ejecutada`,
    mensaje:
`Caja ejecutó la transferencia del caso ${d.numero} (${d.cliente_nombre}).

N° de operación: ${d.num_operacion}
Fecha: ${d.fecha_transfer}
Ejecutado por: ${d.ejecutado_por}

El caso quedó cerrado. Revisa el detalle en el ERP:
${URL_ERP}`
  }),

  generico: d => ({
    asunto: `${d.numero || 'Postventa'} · Notificación del sistema`,
    mensaje: `Actividad registrada en el módulo de Postventa.
...
const sendEmail = async (tipo, data, destinatarios) => {
  try {
    ...
    if (!validos.length) {
      console.warn('[Notif] ' + tipo + ': sin destinatarios con correo')
      return { ok: false, error: 'sin destinatarios' }
    }
    const { asunto, mensaje } = (PLANTILLAS_NOTIF[tipo] || PLANTILLAS_NOTIF.generico)(data || {})
    const ahora = new Date()
    const filas = validos.map(d => ({
      oc_id:          data?.numero || null,   // referencia del caso, visible en la tabla
      tipo:           'Email',
      destino_correo: d.email,
      destino_nombre: d.nombre || null,
      asunto,
      mensaje,
      estado:         'Pendiente',            // el trigger lo toma desde aqui
      accion:         tipo,
      usuario:        data?.ejecutivo_nombre || data?.validador_nombre ||
                      data?.resuelto_por_nombre || data?.ejecutado_por ||
                      data?.cerrado_por_nombre || null,
    ...
    if (error) {
      console.warn('[Notif] ' + tipo + ':', error.message)
...
// Destinatarios de un evento. Primero la configuracion explicita de
// config_sistema (notif_<evento>, editable en Configuracion > Notificaciones);
// si no existe, fallback por rol restringido a quienes tienen acceso activo
// a Postventa. Si el evento trae sucursal, se excluye a los usuarios
// amarrados a otra sucursal (los de alcance global reciben siempre).
const getEmailsParaEvento = async (eventoKey, roles = [], sucursal = null) => {
  try {
  ...
  // El rol autoritativo es el de usuario_acceso (postventa.<rol>), no el
  // legacy de usuarios: Bodega Angeles es "cajero" legacy pero opera como
  // operaciones en Postventa, Omar Marquez es "jefe_bodega" legacy pero
  // jefe de tienda en el modulo, etc.
  const rolIds = roles.map(r => 'postventa.' + r)
  const { data: accesos } = await supabase
    .from('usuario_acceso')
    .select('usuario_id, rol_id, sucursal_id')
    .eq('app_codigo', 'postventa')
    .eq('activo', true)
    .in('rol_id', rolIds)
  const filtrados = (accesos || []).filter(a =>
    !sucursal || !a.sucursal_id || a.sucursal_id === sucursal)
  if (!filtrados.length) return []
  const { data: usrs } = await supabase
    .from('usuarios')
    .select('id, nombre, correo')
    .in('id', filtrados.map(a => a.usuario_id))
    .eq('activo', true)
    .not('correo', 'is', null)
  return (usrs || []).map(u => ({ email: u.correo, nombre: u.nombre }))
}
...
const Dashboard = ({casos, codigos, cu, onNuevo, onVerCaso, sucFiltro}) => {
  const h = p => hp(cu, p)
  ...
  const ESTADOS_EN_CURSO = ['en_validacion_tecnica','en_resolucion','transfer_pendiente','pendiente_decision_postventa']
  ...
  const porCodigo = useMemo(() => {
    ...
    casosFil.forEach(c => {
      ...
      m[cod].desc = info?.nombre?.slice(0,32)||cod
  ...
  // ── Monto reclamado (opción C: solo monto_reclamado, sin fallback) ──
  const montoReclamado = useMemo(() =>
    casosFil.reduce((s,c) => s + (Number(c.monto_reclamado)||0), 0)
  , [casosFil])

  // ── Casos sin monto registrado (cobertura del dato) ──
  const casosSinMonto = useMemo(() =>
    casosFil.filter(c => !c.monto_reclamado || Number(c.monto_reclamado) <= 0).length
  , [casosFil])

  // ── Tiempo promedio de resolución (de cerrados) ──
  const tProm = useMemo(() => {
    const cerr = casosFil.filter(c => c.estado==='cerrado' && c.cerrado_at && c.created_at)
    if (!cerr.length) return null
    const hrs = cerr.map(c => (new Date(c.cerrado_at) - new Date(c.created_at)) / 3600000)
      .filter(h => h >= 0 && h < 9999)
    if (!hrs.length) return null
    return Math.round(hrs.reduce((s,h)=>s+h, 0) / hrs.length)
  }, [casosFil])

  // ── Por sucursal (3 sucursales fijas, mostrar todas aunque tengan 0) ──
  const SUCURSALES = [
    {k:'suc-lg',    l:'La Granja',    ic:'🏢'},
    {k:'suc-maipu', l:'Tienda Maipú', ic:'🏢'},
    {k:'suc-la',    l:'Los Ángeles',  ic:'🏢'},
  ].filter(s => !sucFiltro || s.k === sucFiltro)
  const porSucursal = useMemo(() => {
    return SUCURSALES.map(s => {
      const cs = casosFil.filter(c => c.sucursal === s.k)
      const ccerr = cs.filter(c => c.estado === 'cerrado').length
      return {
        ...s,
        total: cs.length,
        monto: cs.reduce((sum,c) => sum + (Number(c.monto_reclamado)||0), 0),
        vencidos: cs.filter(c => !ESTADOS_CERR.includes(c.estado) && horasDesde(c.created_at) > SLA.resolucion).length,
        escalados: cs.filter(c => c.estado === 'escalado').length,
        cerrados: ccerr,
        pctCierre: cs.length > 0 ? Math.round(ccerr / cs.length * 100) : 0,
      }
    })
  }, [casosFil])

  // ── Por bloque B1-B6 (causa raíz para GG) ──
  const BLOQUES_INFO = {
    B1: {l:'Errores Internos',   c:PBI.accent},
    B2: {l:'Falla Producto',     c:PBI.orange},
    B3: {l:'Error Cliente',      c:PBI.blue},
    B4: {l:'Devolución Dinero',  c:PBI.purple},
    B5: {l:'Caso Crítico',       c:PBI.accent},
    B6: {l:'Error Despacho',     c:PBI.green},
  }
  const porBloque = useMemo(() => {
    const m = {}
    casosFil.forEach(c => {
      const cod = c.codigo_final || c.codigo_provisional || ''
      const bl = cod.slice(0,2) || 'S/C'
      if (!m[bl]) m[bl] = 0
      m[bl]++
    })
    return Object.entries(m)
      .map(([bl,cnt]) => ({
        bl,
        cnt,
        l: BLOQUES_INFO[bl]?.l || 'Sin clasificar',
        c: BLOQUES_INFO[bl]?.c || PBI.muted,
        pct: casosFil.length > 0 ? Math.round(cnt / casosFil.length * 100) : 0,
      }))
      .sort((a,b) => b.cnt - a.cnt)
  }, [casosFil])
  const maxBloque = Math.max(...porBloque.map(b=>b.cnt), 1)

  // ── Clientes reincidentes (3+ casos en el período) ──
  const reincidentes = useMemo(() => {
    const m = {}
    casosFil.forEach(c => {
      const rut = c.cliente_rut
      if (!rut) return
      if (!m[rut]) m[rut] = {rut, nombre: c.cliente_nombre, casos: 0, monto: 0, ultimo: null}
      m[rut].casos++
      m[rut].monto += Number(c.monto_reclamado) || 0
      const fecha = new Date(c.created_at)
      if (!m[rut].ultimo || fecha > m[rut].ultimo) m[rut].ultimo = fecha
    })
    return Object.values(m).filter(x => x.casos >= 3).sort((a,b) => b.casos - a.casos).slice(0, 8)
  }, [casosFil])
  ...
  const etapas = [
    ...
    {k:'pendiente_decision_postventa', l:'Decisión PV', ic:'⚠️', v:casosFil.filter(c=>c.estado==='pendiente_decision_postventa').length, c:PBI.orange},
  ...
  return (
    <div style={{background: PBI.bg, minHeight:"100vh", padding:"0 0 24px", fontFamily:FONT}}>
      ...
      <div style={{padding:"0 16px"}}>
        ...
        <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(120px,1fr))", gap:10, marginBottom:14}}>
          {[
            {l:"Total casos",      v:total,        ic:"📋", c:PBI.blue,
              sub:`${cerrados} cerrados · ${total-cerrados-rechazados} activos`},
            {l:"Monto reclamado",
              v: montoReclamado >= 1000000
                ...
                : montoReclamado >= 1000
                  ...
                  : fmt(montoReclamado),
              ic:"💸", c:PBI.green,
              sub:`${total - casosSinMonto} de ${total} casos con monto`},
            {l:"Sin monto registrado", v:casosSinMonto, ic:"⚠️",
              c:casosSinMonto/Math.max(total,1) > 0.3 ? PBI.orange : PBI.muted,
              sub:`${total>0?Math.round(casosSinMonto/total*100):0}% de los casos`,
              alert: casosSinMonto/Math.max(total,1) > 0.3},
            {l:"T. prom. resolución", v:tProm?tProm+"h":"—", ic:"⏱️",
              c:tProm?(tProm<=48?PBI.green:PBI.accent):PBI.muted,
              sub:tResp?`SLA cumplido: ${tResp.pct}%`:"sin datos"},
            {l:"Fuera de SLA ahora", v:vencidos, ic:"⛔",
              c:vencidos>0?PBI.accent:PBI.muted,
              sub:"activos vencidos", alert:vencidos>0},
            {l:"Reincidentes",     v:reincidentes.length, ic:"🚨",
              c:reincidentes.length>0?PBI.orange:PBI.muted,
              sub:"clientes con 3+ casos", alert:reincidentes.length>0},
          ].map((m,i) => (
            <div key={m.l}
        ...
        <div style={{display:"grid", gridTemplateColumns:"repeat(auto-fit,minmax(220px,1fr))", gap:12, marginBottom:14}}>
          {porSucursal.map(s => (
            <div key={s.k} style={{
              ...PANEL,
              padding:"14px 16px",
              opacity: s.total === 0 ? 0.6 : 1,
            }}>
              <div style={{display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:8}}>
                <div>
                  <div style={{fontSize:11, color:PBI.muted, fontWeight:700, textTransform:"uppercase", letterSpacing:"0.05em"}}>
                    {s.ic} {s.l}
                  </div>
                  <div style={{fontSize:24, fontWeight:900, color:"#fff", letterSpacing:"-0.03em", marginTop:4}}>
                    {s.total} <span style={{fontSize:11, fontWeight:600, color:PBI.muted}}>casos</span>
                  </div>
                </div>
                <div style={{textAlign:"right"}}>
                  <div style={{fontSize:11, color:PBI.muted, fontWeight:600}}>Reclamado</div>
                  <div style={{fontSize:15, fontWeight:800, color:PBI.green, marginTop:2}}>
                    {s.monto >= 1000000 ? `$${(s.monto/1000000).toFixed(1)}M`
                      : s.monto >= 1000 ? `$${Math.round(s.monto/1000)}K`
                      : s.monto > 0 ? fmt(s.monto) : "—"}
                  </div>
                </div>
              </div>
              {s.total > 0 ? (
                <div style={{display:"grid", gridTemplateColumns:"repeat(3,1fr)", gap:6, marginTop:10, paddingTop:10, borderTop:`1px solid ${PBI.border}`}}>
                  <div>
                    <div style={{fontSize:10, color:PBI.muted}}>Vencidos</div>
                    <div style={{fontSize:14, fontWeight:800, color:s.vencidos>0?PBI.accent:"#fff"}}>{s.vencidos}</div>
                  </div>
                  <div>
                    <div style={{fontSize:10, color:PBI.muted}}>Escalados</div>
                    <div style={{fontSize:14, fontWeight:800, color:s.escalados>0?PBI.orange:"#fff"}}>{s.escalados}</div>
                  </div>
                  <div>
                    <div style={{fontSize:10, color:PBI.muted}}>Cierre</div>
                    <div style={{fontSize:14, fontWeight:800, color:s.pctCierre>=70?PBI.green:PBI.orange}}>{s.pctCierre}%</div>
                  </div>
                </div>
              ) : (
                <div style={{marginTop:10, paddingTop:10, borderTop:`1px solid ${PBI.border}`, fontSize:11, color:PBI.muted, fontStyle:"italic"}}>
        ...
        {/* ── FILA BLOQUES B1-B6 (causa raíz) ── */}
        <div style={{...PANEL, padding:"14px 16px", marginBottom:14}}>
          <div style={{fontSize:13, fontWeight:700, color:"#fff", marginBottom:4}}>
            🧩 Distribución por bloque — causa raíz
          </div>
          <div style={{fontSize:11, color:PBI.muted, marginBottom:14}}>
            Identifica si los problemas son internos, del cliente o de despacho
          </div>
          {porBloque.length === 0 ? (
            <div style={{textAlign:"center", padding:"20px 0", color:PBI.muted, fontSize:12}}>
              Sin casos clasificados
            </div>
          ) : (
            porBloque.map(b => (
              <div key={b.bl} style={{marginBottom:10}}>
                <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4}}>
                  <div style={{fontSize:12, color:"#fff", fontWeight:600}}>
                    <span style={{color:b.c, fontWeight:800, marginRight:6}}>{b.bl}</span>
                    {b.l}
                  </div>
                  <div style={{fontSize:12, fontWeight:700, color:b.c}}>
                    {b.cnt} <span style={{color:PBI.muted, fontWeight:500}}>· {b.pct}%</span>
                  </div>
                </div>
                <div style={{height:8, background:PBI.grid, borderRadius:4, overflow:"hidden"}}>
                  <div style={{
                    width:`${b.cnt/maxBloque*100}%`, height:"100%",
                    background:b.c, borderRadius:4, transition:"width 0.3s"
                  }}/>
                </div>
              </div>
            ))
        ...
        <div style={{...PANEL, padding:"14px 16px", marginBottom:12}}>
          <div style={{display:"flex", justifyContent:"space-between", alignItems:"center", marginBottom:4}}>
            <div>
              ...
              <div style={{fontSize:11, color:PBI.muted, marginTop:1}}>
                Clientes con 3 o más casos en el período
              </div>
            </div>
            <Bd c={reincidentes.length>0?PBI.orange:PBI.muted} bg={(reincidentes.length>0?PBI.orange:PBI.muted)+"15"} lg>
              {reincidentes.length} {reincidentes.length===1?'cliente':'clientes'}
            </Bd>
          </div>
          {reincidentes.length === 0 ? (
            <div style={{textAlign:"center", padding:"20px 0", color:PBI.muted, fontSize:12, fontStyle:"italic"}}>
              ✅ Sin clientes con reincidencia en el período
            </div>
          ) : (
            <div style={{marginTop:12, overflowX:"auto"}}>
              <table style={{width:"100%", borderCollapse:"collapse"}}>
                <thead>
                  <tr>
                    {["Cliente","RUT","# Casos","Monto reclamado","Último caso"].map(h => (
                      <th key={h} style={{
                ...
                <tbody>
                  {reincidentes.map((r,i) => (
                    <tr key={r.rut} style={{
                      borderBottom: i < reincidentes.length-1 ? `1px solid ${PBI.border}` : 'none'
                    }}>
                      <td style={{padding:"8px 12px", fontSize:12, color:"#fff", fontWeight:600}}>
                        {r.nombre || '—'}
                      </td>
                      <td style={{padding:"8px 12px", fontSize:11, color:PBI.muted, fontFamily:"monospace"}}>
                        {r.rut}
                      </td>
                      <td style={{padding:"8px 12px"}}>
                        <Bd c={r.casos>=5?PBI.accent:PBI.orange} bg={(r.casos>=5?PBI.accent:PBI.orange)+"15"}>
                          {r.casos}
                        </Bd>
                      </td>
                      <td style={{padding:"8px 12px", fontSize:12, color:PBI.green, fontWeight:700}}>
                        {r.monto > 0 ? fmt(r.monto) : "—"}
                      </td>
                      <td style={{padding:"8px 12px", fontSize:11, color:PBI.muted}}>
                        {r.ultimo ? r.ultimo.toLocaleDateString('es-CL') : "—"}
                      </td>
                    </tr>
                  ))}
                </tbody>
...
const ListaCasos = ({casos, cu, onVerCaso, onNuevo, onRefresh}) => {
  ...
  const esAdmin = esAdminPV(cu)

  // Estados "activos" = todo lo que no está cerrado ni rechazado
  const ESTADOS_ACTIVOS = ['abierto','en_validacion_tecnica','en_resolucion','escalado','transfer_pendiente','pendiente_decision_postventa']
      ...
      <Cd>
        ...
        <div style={{display:"flex", gap:6, flexWrap:"wrap"}}>
          {[
            ...
            {k:"pendiente_decision_postventa", l:"Decisión PV", cnt: casos.filter(c=>!c.deleted_at&&c.estado==='pendiente_decision_postventa').length, c:"#FF9500"},
          ...
          <div style={{
            ...
            <div>
              ...
              <select style={css.select} value={filtSuc}
                ...
                <option value="suc-lg">La Granja</option>
                <option value="suc-maipu">Tienda Maipú</option>
                <option value="suc-la">Los Ángeles</option>
              </select>
            </div>
            <div>
              <div style={{fontSize:10, color:"#8E8E93", fontWeight:700,
...
const Form2Validacion = ({caso, cu, onClose, onGuardado}) => {
  ...
  const guardar = async () => {
    ...
    // Si Operaciones rechaza, vuelve a Postventa para que tome la decisión final
    const nuevoEstado = f.procede_reclamo ? 'en_resolucion' : 'pendiente_decision_postventa'
    ...
    await supabase.from('caso_eventos').insert({
      ...
      estado_nuevo:   f.procede_reclamo ? 'en_resolucion' : 'pendiente_decision_postventa',
    ...
    getEmailsParaEvento('form2_completado', ['postventa','jefe_tienda','admin','gerencia'], caso.sucursal).then(dest => {
    ...
    if (diferencias.length > 0) {
      getEmailsParaEvento('diferencia_cantidades', ['postventa','admin','gerencia'], caso.sucursal).then(dest => {
  ...
  const puedeEditar  = ['operaciones','jefe_tienda','admin','gerencia'].includes(cu.rol) || esAdminPV(cu)
...
// Facultad de excepcion para mover un caso que quedo trabado fuera del flujo
// normal. Reservada a quien tenga la capability pv.admin.
//
// Reglas de diseno:
//  · El motivo es obligatorio. Ningun cambio queda sin justificacion escrita.
//  · Todo cambio escribe un evento en caso_eventos con estado anterior y
//    nuevo, el motivo y el usuario. Queda en la bitacora como intervencion
//    administrativa, distinguible de un avance normal del flujo.
//  · No se tocan los formularios. Solo se mueve el caso.
//  · Las incoherencias se advierten pero no se bloquean: el administrador
//    decide con la informacion a la vista.
const PanelAdminCaso = ({caso, cu, users, form2Data, form3Data, form4Data, onHecho}) => {
  const [abierto,  setAbierto]  = useState(false)
  const [accion,   setAccion]   = useState(null)   // 'estado' | 'derivar' | 'cerrar'
  const [saving,   setSaving]   = useState(false)
  const [err,      setErr]      = useState("")
  const [motivo,   setMotivo]   = useState("")
  const [nvoEstado,setNvoEstado]= useState("")
  const [escalarA, setEscalarA] = useState("jefe_tienda")
  const [nvaSuc,   setNvaSuc]   = useState(caso.sucursal || "")
  const [nvoEjec,  setNvoEjec]  = useState("")

  if (!esAdminPV(cu)) return null

  const reset = () => {
    setAccion(null); setMotivo(""); setNvoEstado(""); setErr("")
    setNvaSuc(caso.sucursal || ""); setNvoEjec(""); setEscalarA("jefe_tienda")
  }

  // Incoherencias entre el estado destino y los formularios existentes
  const avisos = () => {
    const a = []
    const destino = accion === 'cerrar' ? 'cerrado' : nvoEstado
    if (!destino) return a
    if (destino === 'cerrado' && !form3Data)
      a.push("El caso no tiene resolucion registrada en FORM 3. Se cerrara sin respaldo de resolucion.")
    if (destino === 'cerrado' && form3Data?.tipo_resolucion === 'nc_transfer'
        && !form4Data?.transfer_comprobante)
      a.push("La resolucion es NC con transferencia y no hay comprobante de Caja. Se cerrara sin evidencia de pago.")
    if (destino === 'transfer_pendiente' && form3Data?.tipo_resolucion !== 'nc_transfer')
      a.push("El caso quedara esperando a Caja, pero su resolucion no es NC con transferencia.")
    if (destino === 'en_validacion_tecnica' && !caso.requiere_form2)
      a.push("El caso no esta marcado como requiere validacion tecnica. Se marcara para que Operaciones pueda intervenir.")
    return a
  }

  const guardar = async () => {
    if (!motivo.trim() || motivo.trim().length < 10) {
      setErr("Describe el motivo con al menos 10 caracteres. Queda registrado en la bitacora.")
      return
    }
    if (accion === 'estado' && !nvoEstado) { setErr("Selecciona el estado destino."); return }
    if (accion === 'estado' && nvoEstado === caso.estado) {
      setErr("El estado destino es el mismo que el actual."); return
    }
    if (accion === 'derivar' && nvaSuc === caso.sucursal && !nvoEjec) {
      setErr("Cambia la sucursal o el ejecutivo responsable."); return
    }
    setSaving(true); setErr("")

    const upd = {}
    let evento, detalle, payload

    if (accion === 'cerrar') {
      const horas = Math.round((Date.now() - new Date(caso.created_at).getTime()) / 3600000)
      upd.estado               = 'cerrado'
      upd.cerrado_at           = new Date().toISOString()
      upd.sla_resolucion_horas = horas
      upd.sla_cumplido         = horas <= 48
      evento  = 'admin_cierre'
      detalle = `Cierre administrativo por ${cu.nombre}. Motivo: ${motivo.trim()}`
      payload = { motivo: motivo.trim(), estado_anterior: caso.estado, horas }
    }

    if (accion === 'estado') {
      upd.estado = nvoEstado
      if (nvoEstado === 'escalado') {
        upd.escalado_a      = escalarA
        upd.escalado_motivo = motivo.trim()
        upd.escalado_fecha  = new Date().toISOString()
      } else if (caso.estado === 'escalado') {
        upd.escalado_a = null; upd.escalado_motivo = null; upd.escalado_fecha = null
      }
      if (nvoEstado === 'en_validacion_tecnica' && !caso.requiere_form2)
        upd.requiere_form2 = true
      if (['cerrado','rechazado'].includes(nvoEstado)) {
        const horas = Math.round((Date.now() - new Date(caso.created_at).getTime()) / 3600000)
        upd.cerrado_at = new Date().toISOString()
        upd.sla_resolucion_horas = horas
        upd.sla_cumplido = horas <= 48
      ...
      evento  = 'admin_estado'
      detalle = `Estado movido de "${ESTADOS[caso.estado]?.l || caso.estado}" a ` +
                `"${ESTADOS[nvoEstado]?.l || nvoEstado}" por ${cu.nombre}. Motivo: ${motivo.trim()}`
      payload = { motivo: motivo.trim(), desde: caso.estado, hacia: nvoEstado,
                  escalado_a: nvoEstado === 'escalado' ? escalarA : null }
    }

    if (accion === 'derivar') {
      const partes = []
      if (nvaSuc && nvaSuc !== caso.sucursal) {
        upd.sucursal = nvaSuc
        partes.push(`sucursal ${caso.sucursal} → ${nvaSuc}`)
      }
      if (nvoEjec) {
        const u = users?.find(x => x.id === nvoEjec)
        upd.ejecutivo_id     = nvoEjec
        upd.ejecutivo_nombre = u?.nombre || nvoEjec
        partes.push(`responsable ${caso.ejecutivo_nombre || '—'} → ${u?.nombre || nvoEjec}`)
      }
      evento  = 'admin_derivacion'
      detalle = `Derivacion administrativa por ${cu.nombre}: ${partes.join(' · ')}. Motivo: ${motivo.trim()}`
      payload = { motivo: motivo.trim(), sucursal_anterior: caso.sucursal,
                  sucursal_nueva: upd.sucursal || caso.sucursal,
                  ejecutivo_nuevo: upd.ejecutivo_nombre || null }
    }

    const { error } = await supabase.from('casos_postventa').update(upd).eq('id', caso.id)
    if (error) { setErr("Error al guardar: " + error.message); setSaving(false); return }

    await supabase.from('caso_eventos').insert({
      caso_id:         caso.id,
      evento:          evento,
      estado_anterior: caso.estado,
      estado_nuevo:    upd.estado || caso.estado,
      detalle:         detalle,
      payload:         payload,
      usuario_id:      cu.id,
      usuario_nombre:  cu.nombre,
  ...
  const ACCIONES = [
    {k:'estado',  ic:'🔀', l:'Mover estado',  d:'Llevar el caso a cualquier etapa del flujo'},
    {k:'derivar', ic:'📍', l:'Derivar',       d:'Cambiar la sucursal o el responsable'},
    {k:'cerrar',  ic:'🔒', l:'Cerrar caso',   d:'Cierre administrativo fuera del flujo'},
  ]
  ...
  return (
    <div style={{marginTop:14}}>
      <div style={css.divider}/>
      <div
        onClick={() => { setAbierto(!abierto); if (abierto) reset() }}
        style={{
          display:"flex", justifyContent:"space-between", alignItems:"center",
          padding:"10px 14px", borderRadius:12, cursor:"pointer",
          background: abierto ? "#FF3B3010" : "#F7F7F8",
          border:`1px solid ${abierto ? "#FF3B3040" : "#E5E5EA"}`
        }}>
        <div>
          <div style={{fontWeight:700, fontSize:13, color: abierto ? "#FF3B30" : "#3C3C43"}}>
            🛠️ Administración del caso
          </div>
          <div style={{fontSize:11, color:"#8E8E93", marginTop:2}}>
            Intervención de excepción. Todo cambio queda registrado en la bitácora.
          </div>
        </div>
        <span style={{fontSize:13, color:"#8E8E93"}}>{abierto ? "▲" : "▼"}</span>
      </div>

      {abierto && (
        <div style={{
          border:"1px solid #FF3B3030", borderTop:"none",
          borderRadius:"0 0 12px 12px", padding:"14px", background:"#fff"
        }}>
          {/* Estado actual */}
          <div style={{
            background:"#F7F7F8", borderRadius:10, padding:"8px 12px",
            marginBottom:12, fontSize:12, display:"flex", gap:8, alignItems:"center"
          }}>
            <span style={{color:"#8E8E93"}}>Estado actual:</span>
            <Bd c={ESTADOS[caso.estado]?.c} bg={ESTADOS[caso.estado]?.bg}>
              {ESTADOS[caso.estado]?.ic} {ESTADOS[caso.estado]?.l || caso.estado}
            </Bd>
            <span style={{color:"#8E8E93", marginLeft:"auto"}}>
              📍 {caso.sucursal} · 👤 {caso.ejecutivo_nombre || '—'}
            </span>
          </div>
          ...
          <div style={{display:"grid", gridTemplateColumns:"1fr 1fr 1fr", gap:8, marginBottom:12}}>
            {ACCIONES.map(a => (
              <div key={a.k}
                onClick={() => { setAccion(a.k); setErr("") }}
                style={{
                  borderRadius:12, padding:"10px 12px", cursor:"pointer",
                  border:`2px solid ${accion===a.k ? "#FF3B30" : "#E5E5EA"}`,
                  background: accion===a.k ? "#FF3B3008" : "#fff",
                  transition:"all 0.15s"
                }}>
                <div style={{fontSize:17, marginBottom:3}}>{a.ic}</div>
                <div style={{fontWeight:700, fontSize:12,
                  color: accion===a.k ? "#FF3B30" : "#1C1C1E"}}>{a.l}</div>
                <div style={{fontSize:10, color:"#8E8E93", marginTop:2, lineHeight:1.35}}>{a.d}</div>
              </div>
            ))}
          </div>
          ...
          {accion === 'estado' && (
            <div style={css.row}>
              <div style={css.col}>
                <Fl l="Estado destino" req>
                  <select style={css.select} value={nvoEstado}
                    onChange={e => { setNvoEstado(e.target.value); setErr("") }}>
                    <option value="">Seleccionar...</option>
                    {Object.entries(ESTADOS)
                      .filter(([k]) => k !== caso.estado)
          ...
          {accion === 'derivar' && (
            <div style={css.row}>
              <div style={css.col}>
                <Fl l="Sucursal">
                  <select style={css.select} value={nvaSuc}
                    onChange={e => { setNvaSuc(e.target.value); setErr("") }}>
                    <option value="suc-lg">La Granja</option>
                    <option value="suc-maipu">Tienda Maipú</option>
                    <option value="suc-la">Los Ángeles</option>
                  </select>
              ...
              <div style={css.col}>
                <Fl l="Reasignar responsable (opcional)">
                  <select style={css.select} value={nvoEjec}
                    onChange={e => { setNvoEjec(e.target.value); setErr("") }}>
                    <option value="">Mantener a {caso.ejecutivo_nombre || '—'}</option>
          ...
          {accion === 'cerrar' && (
            <div style={{
              ...
              El caso quedará cerrado y se registrará el tiempo transcurrido como SLA.
              Usa esta opción solo cuando el caso se resolvió fuera del sistema.
            </div>
          )}
          ...
          {accion && (
            <>
              {listaAvisos.length > 0 && (
                <div style={{
                  background:"#FF950010", border:"1px solid #FF950040",
                  borderRadius:10, padding:"10px 12px", marginBottom:12
                }}>
                  <div style={{fontSize:11, fontWeight:700, color:"#FF9500", marginBottom:5}}>
                    ⚠️ Revisa antes de confirmar
                  </div>
                  {listaAvisos.map((a, i) => (
                    <div key={i} style={{fontSize:11, color:"#3C3C43", marginBottom:3}}>· {a}</div>
                  ))}
                </div>
              )}

              <Fl l="Motivo de la intervención" req>
                <textarea style={{...css.textarea, minHeight:64}}
                  value={motivo}
                  onChange={e => { setMotivo(e.target.value); setErr("") }}
                  placeholder="Ej: el cliente resolvió directamente con la sucursal y no corresponde emitir NC. Confirmado con Elizabeth el 03/09."/>
                <div style={{fontSize:10, color:"#8E8E93", marginTop:3}}>
                  Queda visible en la bitácora del caso con tu nombre y la fecha.
                </div>
              </Fl>

              {err && (
                <div style={{
                  background:"#FF3B3010", borderRadius:8, padding:"8px 12px",
                  marginBottom:10, fontSize:12, color:"#FF3B30", fontWeight:600
                }}>⚠️ {err}</div>
              )}

              <div style={{display:"flex", gap:10}}>
                <Bt variant="secondary" fw onClick={reset}>Cancelar</Bt>
                <Bt fw variant="danger" loading={saving} onClick={guardar}>
                  {accion === 'cerrar' ? "🔒 Cerrar el caso" :
                   accion === 'derivar' ? "📍 Derivar el caso" :
                   "🔀 Mover el estado"}
                </Bt>
              </div>
            </>
...
const DetalleCaso = ({caso, cu, codigos, users, onClose, onRefresh}) => {
  if (!caso) return null
  ...
  const puedeForm2 = ['operaciones','jefe_tienda','admin','gerencia'].includes(cu.rol) || esAdminPV(cu)
  ...
  const getSteps = () => {
    ...
    const idx = {
      ...
      pendiente_decision_postventa: tieneF2 ? 2 : 1,
      cerrado:               tieneF2 ? 3 : 2,
  ...
  // ── Linea de tiempo de la bitacora ──────────────────────────────────────
  // Combina dos fuentes: los eventos reales de caso_eventos y los hitos que
  // se reconstruyen desde los formularios cuando no hay evento que los cubra.
  //
  // Los casos migrados se importaron sin caso_eventos, asi que su bitacora
  // sale vacia aunque los formularios existan. Los hitos reconstruidos se
  // marcan con _derivado para no confundirlos con un registro de auditoria
  // real: caso_eventos nunca se completa con datos inventados.
  const timeline = useMemo(() => {
    const reales = (eventos || []).map(e => ({ ...e, _derivado: false }))
    const hay    = k => reales.some(e => e.evento === k)
    const extra  = []

    const add = (evento, fecha, usuario) => {
      if (!fecha) return
      extra.push({ evento, created_at: fecha, usuario_nombre: usuario || null,
                   _derivado: true })
    }

    if (!hay('creado'))
      add('creado', casoActual.created_at, casoActual.ejecutivo_nombre)

    if (form2Data && !hay('form2_completado'))
      add('form2_completado', form2Data.fecha_validacion, form2Data.validador_nombre)

    if (form3Data && !hay('form3_completado') && !hay('escalado'))
      add(form3Data.tipo_resolucion === 'escalar' ? 'escalado' : 'form3_completado',
          form3Data.fecha_resolucion, form3Data.resuelto_por_nombre)

    // Hito propio: la instruccion a Caja. No existe como evento en la base,
    // por eso antes solo se veia si el caso pasaba por form3_completado.
    if (form3Data?.tipo_resolucion === 'nc_transfer')
      add('solicitud_financiera', form3Data.fecha_resolucion, form3Data.resuelto_por_nombre)

    if (form4Data?.transfer_comprobante && !hay('transfer_ejecutada'))
      add('transfer_ejecutada', form4Data.transfer_fecha || form4Data.fecha_cierre,
          form4Data.transfer_ejecutado_nombre)

    return [...reales, ...extra].sort(
      (a, b) => new Date(a.created_at || 0) - new Date(b.created_at || 0)
    )
  }, [eventos, form2Data, form3Data, form4Data, casoActual])

  const nDerivados = timeline.filter(e => e._derivado).length
    ...
    <Sheet open wide onClose={onClose} title={null}>
      ...
      {codigoInfo && (
        <div style={{...css.cardSm, marginBottom:14, borderLeft:`3px solid ${cl.c}`}}>
          <div style={{fontSize:13, fontWeight:600}}>{codigoInfo.nombre}</div>
      ...
      {(casoActual.estado === 'en_resolucion' ||
        casoActual.estado === 'pendiente_decision_postventa' ||
        (casoActual.estado === 'abierto' && !casoActual.requiere_form2)) && (
        <div style={{marginBottom:14}}>
          <div style={css.divider}/>
          {/* Banner especial cuando viene de rechazo técnico */}
          {casoActual.estado === 'pendiente_decision_postventa' && (
            <div style={{
              background:"#FF950015", border:"1.5px solid #FF9500",
              borderRadius:12, padding:"12px 14px", marginBottom:12
            }}>
              <div style={{fontWeight:700, fontSize:13, color:"#FF9500", marginBottom:4}}>
                ⚠️ Operaciones rechazó el reclamo — Postventa debe decidir
              </div>
              <div style={{fontSize:12, color:"#3C3C43"}}>
                El área técnica determinó que el reclamo <strong>no procede</strong>.
                Revisa el argumento técnico y toma la decisión final: rechazar, compensar o escalar.
              </div>
            </div>
          )}
          ...
          {form3Data && form3Data.tipo_resolucion !== 'escalar' ? (
            ...
            <div style={{
              ...
              {casoActual.estado === 'transfer_pendiente' && (['admin','jefe_tienda','gerencia'].includes(cu.rol) || esAdminPV(cu)) && (
      ...
      {(() => {
        ...
        if (esAbono && casoActual.estado === 'en_resolucion' && hp(cu, 'form3')) {
          return (
            <div style={{marginBottom:14}}>
              <div style={css.divider}/>
              ...
              <div style={{borderRadius:14,padding:"14px 16px",
                ...
                <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
                  ...
                  <div style={{display:'flex',gap:8,alignItems:'center'}}>
                    {(['admin','jefe_tienda','gerencia'].includes(cu.rol) || esAdminPV(cu)) && (
                      <button onClick={() => { setShowCambioTipo(true); setCambioTipoDatos({ banco:'', tipo_cuenta:'Cuenta Corriente', num_cuenta:'', nombre_titular:'' }) }} style={{
      ...
      <div style={{marginBottom:14}}>
        ...
        <div style={{fontWeight:700, fontSize:14, marginBottom:16, display:"flex", gap:8, alignItems:"center"}}>
          ...
          <span style={{fontSize:11, color:"#8E8E93", fontWeight:400}}>
            ...
            {nDerivados > 0 && (
              <span style={{color:"#FF9500"}}> · {nDerivados} reconstruido{nDerivados!==1?'s':''}</span>
            )}
          </span>
        </div>
            ...
            <div style={{
              ...
              <div>
                ...
                <div style={{fontSize:18, fontWeight:900, color:"#FF9500"}}>
                  {fmt(casoActual.monto_reclamado || casoActual.doc_monto || 0)}
                </div>
              </div>
            </div>
            {/* Vendedor BSALE */}
        ...
        {timeline.map((ev, i) => {
          const CFG = {
            ...
            solicitud_financiera:  {ic:'🏦', c:'#5856D6', l:'Solicitud financiera a Caja'},
            cambio_tipo_resolucion:{ic:'🔄', c:'#FF9500', l:'Cambio de método de pago'},
            caso_cerrado:          {ic:'✅', c:'#34C759', l:'Cierre del caso'},
            admin_estado:          {ic:'🔀', c:'#FF3B30', l:'Estado movido por administración'},
            admin_derivacion:      {ic:'📍', c:'#FF3B30', l:'Derivación administrativa'},
            admin_cierre:          {ic:'🔒', c:'#FF3B30', l:'Cierre administrativo'},
          }
            ...
            <div key={i} style={{display:"flex", gap:0, marginBottom:0, position:"relative"}}>
              ...
              <div style={{
                ...
                <div style={{
                  ...
                  <div style={{fontWeight:800, fontSize:13, color:cfg.c,
                    display:"flex", gap:6, alignItems:"center"}}>
                    {cfg.l}
                    {ev._derivado && (
                      <span style={{
                      ...
                      }} title="Reconstruido desde el formulario: este caso no registro el evento en su momento">
                        reconstruido
                      </span>
                    )}
                  </div>
                ...
                <div style={{padding:"10px 14px"}}>
                  ...
                  {(ev.evento === 'form3_completado' || ev.evento === 'escalado') && form3Data && (
                    <div>
                      ...
                      {/* Los datos bancarios se muestran en el hito
                          "Solicitud financiera a Caja", que aparece siempre
                          que haya instruccion de transferencia. */}
                  ...
                  {/* ── SOLICITUD FINANCIERA: instruccion a Caja ── */}
                  {ev.evento === 'solicitud_financiera' && form3Data && (
                    <div>
                      <div style={{fontSize:11, color:"#8E8E93", marginBottom:8}}>
                        Instruccion emitida a Caja para restituir al cliente por transferencia.
                      </div>
                      <div style={{
                        ...
                        {[
                          ...
                          {l:"Tipo cta.",  v: form3Data.tipo_cuenta},
                          {l:"N° cuenta",  v: form3Data.num_cuenta},
                          {l:"NC BSALE",   v: form3Data.bsale_doc_numero},
                          {l:"Monto a girar", v: fmt(form3Data.monto), highlight:true},
                        ].map(d => (
                          <div key={d.l}>
                            <div style={{fontSize:9, color:"rgba(255,255,255,0.4)",
                              textTransform:"uppercase", fontWeight:700, marginBottom:2}}>{d.l}</div>
                            <div style={{fontSize:12, fontWeight: d.highlight ? 800 : 600,
                              color: d.highlight ? "#34C759" : "#fff"}}>
                              {d.v || "—"}
                            </div>
                          </div>
                        ))}
                      ...
                      <div style={{
                        background: form4Data?.transfer_comprobante ? "#34C75910" : "#FF950012",
                        border:`1px solid ${form4Data?.transfer_comprobante ? "#34C75940" : "#FF950040"}`,
                        borderRadius:8, padding:"7px 12px",
                        fontSize:11, fontWeight:700,
                        color: form4Data?.transfer_comprobante ? "#34C759" : "#FF9500"
                      }}>
                        {form4Data?.transfer_comprobante
                          ? "✅ Ejecutada por Caja"
                          : "⏳ Pendiente de ejecucion por Caja"}
                      </div>
                      {form3Data.resuelto_por_nombre && (
                        <div style={{fontSize:10, color:"#AEAEB2", marginTop:8}}>
                          Solicitada por <strong style={{color:"#8E8E93"}}>
                            {form3Data.resuelto_por_nombre}
                          </strong>
                        </div>
                      )}
                  ...
                  {ev.evento === 'transfer_ejecutada' && form4Data && (
                    <div>
                      ...
                      <div style={{display:"grid", gridTemplateColumns:"1fr 1fr", gap:8, marginBottom:8}}>
                        ...
                        <div style={{background:"#F7F7F8", borderRadius:8, padding:"8px 10px"}}>
                          <div style={{fontSize:10, color:"#8E8E93", fontWeight:700,
                            textTransform:"uppercase", marginBottom:1}}>Fecha ejecución</div>
                      ...
                      {/* Fila 2: Datos bancarios desde form3 */}
                      {form3Data && (
                        <div style={{
                          background:"#1C1C1E", borderRadius:10,
                          padding:"10px 14px", marginBottom:8,
                          ...
                          {[
                            ...
                            {l:"Tipo cta.", v: form3Data.tipo_cuenta},
                            {l:"N° cuenta", v: form3Data.num_cuenta},
                            {l:"Monto",     v: fmt(form3Data.monto), highlight: true},
                          ].map(d => (
                            <div key={d.l}>
                              <div style={{fontSize:9, color:"rgba(255,255,255,0.4)",
                                textTransform:"uppercase", fontWeight:700, marginBottom:2}}>{d.l}</div>
                      ...
                      {form4Data.transfer_comprobante_url ? (
                        <div style={{marginBottom:8}}>
                          <div style={{fontSize:10, color:"#8E8E93", fontWeight:700,
                            textTransform:"uppercase", marginBottom:6}}>📎 Boucher adjunto</div>
                          {form4Data.transfer_comprobante_url.match(/\.(jpg|jpeg|png|webp|heic)$/i) ? (
                            <div style={{position:"relative", display:"inline-block"}}>
                              <img
                                src={form4Data.transfer_comprobante_url}
                                alt="Boucher transferencia"
                                style={{
                                  maxWidth:"100%", maxHeight:220,
                                  borderRadius:10, border:"2px solid #34C75940",
                            ...
                            <a href={form4Data.transfer_comprobante_url} target="_blank"
                              ...
                              style={{
                                ...
                                background:"#34C75912", border:"1px solid #34C75940",
                        ...
                        <div style={{
                          background:"#F7F7F8", borderRadius:8, padding:"8px 12px",
                          fontSize:11, color:"#AEAEB2", marginBottom:8
                        }}>
                          📎 Sin boucher adjunto
                        </div>
                      )}
                      ...
                      {form4Data.observaciones_finales && (
                        <div style={{
                          ...
                          <span style={{fontWeight:700, color:"#8E8E93", fontSize:10,
                            textTransform:"uppercase"}}>Observaciones: </span>
                          {form4Data.observaciones_finales}
                        </div>
                      )}

                      <div style={{fontSize:10, color:"#AEAEB2"}}>
                        Ejecutado por <strong style={{color:"#8E8E93"}}>
                          {form4Data.transfer_ejecutado_nombre}
                        </strong>
                      </div>
                    </div>
                  ...
                  {/* Fallback: cubre los hitos sin plantilla propia y tambien
                      aquellos cuya plantilla existe pero no tiene datos que
                      mostrar, que antes quedaban con el cuerpo en blanco. */}
                  {!(
                     (ev.evento === 'creado') ||
                     (ev.evento === 'form2_completado'     && form2Data) ||
                     (ev.evento === 'form3_completado'     && form3Data) ||
                     (ev.evento === 'escalado'             && form3Data) ||
                     (ev.evento === 'form4_completado'     && form4Data) ||
                     (ev.evento === 'transfer_ejecutada'   && form4Data) ||
                     (ev.evento === 'solicitud_financiera' && form3Data)
                   ) && (
                    <div style={{fontSize:12, color:"#3C3C43"}}>
                      {ev.detalle || <span style={{color:"#AEAEB2", fontStyle:"italic"}}>
                        Sin detalle registrado para este hito
                      </span>}
                    </div>
                  )}
      ...
      {/* ── Panel de administración (solo con capability pv.admin) ── */}
      <PanelAdminCaso
        caso={casoActual} cu={cu} users={users}
        form2Data={form2Data} form3Data={form3Data} form4Data={form4Data}
        onHecho={recargarCaso}
      />
...
const Form4Transfer = ({caso, form3, cu, onClose, onGuardado}) => {
  ...
  const puedeCaja = puedeTransferir(cu)
  ...
  const guardar = async () => {
    ...
    getEmailsParaEvento('transfer_ejecutada', ['postventa','admin','gerencia'], caso.sucursal).then(dest => {
...
const Form4Escalamiento = ({caso, form3, cu, onClose, onGuardado}) => {
  ...
  const puedeActuar = esAdminPV(cu) || cu.rol === 'gerencia' ||
  ...
  const guardar = async () => {
    ...
    getEmailsParaEvento('form4_completado', ['postventa','admin','gerencia'], caso.sucursal).then(dest => {
...
const Form3Resolucion = ({caso, cu, codigos, onClose, onGuardado}) => {
  ...
  const puedeResolver = hp(cu, 'form3')
  ...
  const guardar = async () => {
    ...
    if (tipo === 'escalar') {
      ...
      const rolDestino = f.escalar_a === 'jefe_tienda'
        ...
        : ['gerencia','admin']
      getEmailsParaEvento('caso_escalado', rolDestino, caso.sucursal).then(dest => {
...
// ─── NUEVO CASO (FORM 1) ──────────────────────────────────────────────────
const NuevoCaso = ({cu, codigos, sucFiltro, onClose, onCreado}) => {
  const [saving, setSaving] = useState(false)
  const [errs, setErrs]     = useState({})
  ...
  const guardar = async () => {
    ...
    // Se avisa SIEMPRE a los destinatarios del evento, requiera o no
    // validacion tecnica. Antes los casos directos solo avisaban a quien
    // los creaba y Postventa central no se enteraba del ingreso.
    getEmailsParaEvento('caso_creado', ['operaciones','postventa','admin'], f.sucursal).then(dest => {
      const propio = (cu.correo || cu.email)
        ? [{ email: cu.correo || cu.email, nombre: cu.nombre }] : []
      const todos = [...propio, ...dest.filter(o => o.email !== (cu.correo || cu.email))]
      ...
      <div style={css.row}>
        <div style={css.col}>
          <Fl l="Sucursal" req err={errs.sucursal}>
            <select style={css.select} value={f.sucursal}
              disabled={!!sucFiltro}
...
const ModuloEscalados = ({casos, cu, onVerCaso, onRefresh}) => {
  ...
  const puedeActuar = (item) => {
    ...
    return esAdminPV(cu) || cu.rol==='gerencia' ||
...
const BrevoPanelCompleto = ({brevoKey, setBrevoKey, brevoFrom, setBrevoFrom,
  ...
  const cargarConfig = async () => {
    setLoadingCfg(true)
    const [{ data: users }, { data: cfgs }, { data: accesos }] = await Promise.all([
      ...
      supabase.from('usuario_acceso').select('usuario_id,rol_id')
        .eq('app_codigo','postventa').eq('activo', true),
    ])
    const rolPV = new Map((accesos || []).map(a => [a.usuario_id, a.rol_id]))
    setUsuarios(users || [])
    ...
    EVENTOS_EMAIL.forEach(ev => {
      if (!cfgMap[ev.k]) {
        const rolIds = ev.roles_default.map(r => 'postventa.' + r)
        cfgMap[ev.k] = (users||[])
          .filter(u => rolIds.includes(rolPV.get(u.id)) && u.correo)
          .map(u => u.id)
      }
    })
    setConfig(cfgMap)
...
const ModuloMatriz = ({codigos, onRefresh}) => {
  ...
  const testBrevo = async () => {
    // El envio ya no pasa por Brevo: inserta en la tabla notificaciones del
    // ERP y el trigger despacha. No requiere API key.
...
// Nombre legible de cada sucursal (para el indicador del header)
const NOMBRE_SUC = {
  'suc-lg':    'La Granja',
  'suc-maipu': 'Tienda Maipú',
  'suc-mp':    'CD Maipú',
  'suc-la':    'Los Ángeles',
  'suc-web':   'Página Web',
  'suc-admin': 'Matriz',
...
// Devuelve el ID de sucursal al que esta limitado el usuario en Postventa,
// o null si ve todas.
//
// Lee usuario_acceso DIRECTAMENTE, sin la cascada a usuarios que hace
// userScopeSync: en este modulo usuario_acceso es la unica fuente valida.
// Si se usara la cascada, un usuario definido como global en usuario_acceso
// quedaria amarrado a la sucursal que traiga la tabla usuarios.
//
// Sin sucursal asignada => ve todo. Es deliberado: preferimos que alguien
// mal configurado siga trabajando a que quede bloqueado sin poder operar.
const resolverSucursal = async (cu) => {
  if (!cu?.id) return null
  const scope = canSync(cu, 'postventa', 'pv.casos')
  if (scope !== 'sucursal') return null   // admin, caja, gerencia => ven todo
  const { data } = await supabase
    .from('usuario_acceso')
    .select('sucursal_id')
    .eq('usuario_id', cu.id)
...
export function PostventaApp({ cu, setAppActual }) {
  ...
  const [sucFiltro, setSucFiltro] = useState(null)   // null = ve todas

  // Carga inicial: casos, usuarios, codigos + precargar capabilities RBAC
  // Las capabilities se cargan ANTES que los casos: sin ellas no se puede
  // resolver el alcance y la primera carga traeria todas las sucursales.
  useEffect(() => {
    if (!cu?.id) return
    ;(async () => {
      await preloadCaps(cu, 'postventa')
      setCapsLoaded(true)

      const suc = await resolverSucursal(cu)
      setSucFiltro(suc)

      let qCasos = supabase.from('casos_postventa')
        .select('*').is('deleted_at', null)
      if (suc) qCasos = qCasos.eq('sucursal', suc)
      ...
      const [{data: u}, {data: cod}, {data: cas}] = await Promise.all([
        ...
        qCasos.order('created_at', { ascending: false }),
      ])
      setUsers(u || [])
      setCodigos(cod || [])
      setCasos(cas || [])
  ...
  const cargarCasos = async () => {
    let q = supabase.from('casos_postventa').select('*').is('deleted_at', null)
    if (sucFiltro) q = q.eq('sucursal', sucFiltro)
  ...
  const TABS = [
    ...
    {k:'casos',      l:'Casos',      ic:'📋'},
    // Tab Finanzas — quien pueda confirmar transferencias (capability pv.form4.transfer)
    ...(puedeTransferir(cu)
    ...
    ...((['jefe_tienda','gerencia','admin'].includes(cu.rol) || esAdminPV(cu))
      ? [{k:'escalados', l:'Escalados', ic:'🔺',
          badge: casos.filter(c => c.estado === 'escalado').length || null}]
      : []),
    ...(esAdminPV(cu) || cu.rol==='gerencia'
  ...
  return (
    <div style={{...css.page, fontFamily:FONT}}>
          ...
          <div style={{minWidth:0}}>
            ...
            {!isXs && (
              <div style={{fontSize:11, color:"var(--text-muted)", fontWeight:500}}>
                Outlet de Puertas SpA · 📍 {sucFiltro ? (NOMBRE_SUC[sucFiltro] || sucFiltro) : "Todas las sucursales"}
              </div>
            )}
          </div>
      ...
      {tab === 'dashboard' && (
        <Dashboard
          casos={casos} codigos={codigos} cu={cu} sucFiltro={sucFiltro}
      ...
      {modalNew && (
        <NuevoCaso
          cu={cu} codigos={codigos} sucFiltro={sucFiltro}
      ...
      {casoSel && (
        <DetalleCaso
          caso={casoSel} cu={cu} codigos={codigos} users={users}
