...
// Los roles de la app no coinciden con los valores reales de usuarios.rol:
// "caja" en la tabla es "cajero" y "gerencia" es "dir_general". Sin este
// mapa, el fallback devolvia cero destinatarios para esos roles y el correo
// no se enviaba.
const ROLES_APP_A_DB = {
  caja:     ['cajero'],
  gerencia: ['gerencia', 'dir_general', 'dir_negocios'],
}
const expandRolesApp = roles =>
  [...new Set((roles || []).flatMap(r => ROLES_APP_A_DB[r] || [r]))]
...
const getEmailsParaEvento = async (eventoKey, roles = [], sucursal = null) => {
  ...
  if (lista.length) {
    const { data: accesos } = await supabase
      ...
      .eq('activo', true)
      .in('usuario_id', lista.map(u => u.id))
    const mapa = new Map((accesos || []).map(a => [a.usuario_id, a.sucursal_id]))
    lista = lista.filter(u =>
      mapa.has(u.id) &&
      (!sucursal || !mapa.get(u.id) || mapa.get(u.id) === sucursal)
    )
  }
  return lista.map(u => ({ email: u.correo, nombre: u.nombre }))
}
...
const BrevoPanelCompleto = ({brevoKey, setBrevoKey, brevoFrom, setBrevoFrom,
  ...
  const cargarConfig = async () => {
    ...
    EVENTOS_EMAIL.forEach(ev => {
      if (!cfgMap[ev.k]) {
        const rolesDb = expandRolesApp(ev.roles_default)
        cfgMap[ev.k] = (users||[])
          .filter(u => rolesDb.includes(u.rol) && u.correo)
          .map(u => u.id)
      }
    })
    setConfig(cfgMap)
