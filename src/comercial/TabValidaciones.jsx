...
const DOCS = [
  { k: 'borrador',   l: 'Borrador de venta' },
  { k: 'nota_venta', l: 'Nota de venta' },
  { k: 'cotizacion', l: 'Cotización' },
  { k: 'abono',      l: 'Abono de cliente' },
  { k: 'otro',       l: 'Otro' },
]
...
export function TabValidaciones({ sucursales, sucSel, setSucSel, cu, esGerente, isMobile }) {
  ...
  const [sucUsuario, setSucUsuario] = useState(cu?.sucursal_id || '')

  // cu no siempre trae sucursal_id — se resuelve desde la tabla usuarios
  useEffect(() => {
    if (cu?.sucursal_id || !cu?.id) return
    let cancel = false
    supabase.from('usuarios').select('sucursal_id').eq('id', cu.id).maybeSingle()
      .then(({ data }) => { if (!cancel && data?.sucursal_id) setSucUsuario(data.sucursal_id) }, () => {})
    return () => { cancel = true }
  }, [cu?.id, cu?.sucursal_id])

  const miSucursal = useMemo(() => {
    if (sucUsuario) return sucUsuario
    if (sucSel && sucSel !== 'TODAS') return sucSel
    return (sucursales || [])[0]?.id || ''
  }, [sucUsuario, sucSel, sucursales])
  ...
  const cargar = async (silencioso) => {
    ...
    try {
      ...
      if (!esGerente) q = q.eq('sucursal_id', miSucursal)
  ...
  useEffect(() => { if (miSucursal || esGerente) cargar() }, [miSucursal, esGerente])

  /* Realtime — una sola suscripción; el filtro por sucursal se hace acá */
  useEffect(() => {
    const visible = r => esGerente || r.sucursal_id === miSucursal
    const ch = supabase
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center', marginBottom: 14 }}>
        <button onClick={nuevaSolicitud} style={{
        ...
        {esGerente && <span style={{ fontSize: 11, color: '#8b88a8' }}>Viendo todas las sucursales</span>}
      ...
      {form && (
        <FormularioSolicitud
          form={form} setForm={setForm} sucursales={sucursales} cu={cu} esGerente={esGerente}
...
function FormularioSolicitud({ form, setForm, sucursales, cu, esGerente, onClose, onCreada }) {
  ...
  const enviar = async () => {
    ...
    if (!form.sucursal_id) { setError('Falta la sucursal.'); return }
  ...
  const sucs = esGerente ? (sucursales || []) : (sucursales || []).filter(s => s.id === form.sucursal_id)
        ...
        <div style={{ padding: 18 }}>
          ...
          <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap' }}>
            <Campo l="Documento asociado" flex>
              <select value={form.doc_tipo} onChange={ev => set('doc_tipo', ev.target.value)} style={inp}>
                {DOCS.map(d => <option key={d.k} value={d.k}>{d.l}</option>)}
              </select>
            </Campo>
          ...
          {esGerente && sucs.length > 1 && (
            <Campo l="Sucursal">
              <select value={form.sucursal_id} onChange={ev => set('sucursal_id', ev.target.value)} style={inp}>
