    ...
    <div style={{ fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif", background: '#f4f4fb', minHeight: '100vh', fontSize: 14, color: '#1c1c1e' }}>
      ...
      <div style={{ padding: isMobile ? '12px 12px 40px' : '18px 22px 48px', maxWidth: 1280, margin: '0 auto' }}>
        {loadingBase ? (
          ...
          <>
            {tab === 'midia' && <TabMiDia {...{ sucursales, vendedores, metas, seg, setSeg, cu, esGerente, isMobile, onVerRadar: () => { setCotVista('radar'); setTab('cotizaciones') } }} />}
...
function TabVambeMonitor({ sucursales, vendedores, cu, esGerente, isMobile, anio, setAnio, mes, setMes }) {
  ...
  return (
    <div>
      ...
      {(() => {
        const hoyD = new Date().toLocaleDateString('en-CA')
        const nomAg = id => normNom(d.ag.find(a => a.agent_id === id)?.nombre || 'Ejecutivo')
        const gdV = (d.gd || []).filter(g => (!sucVista || g.sucursal_id === sucVista) && (perGd === '7d' || g.dia === hoyD))
        const porAg = {}
        gdV.forEach(g => {
          const o = porAg[g.agent_id] || (porAg[g.agent_id] = { id: g.agent_id, suc: g.sucursal_id, msj: 0, cli: 0, leid: 0, resp: 0, meds: [] })
          o.msj += g.mensajes || 0; o.cli += g.contactos || 0; o.leid += g.leidos || 0; o.resp += g.respuestas || 0
          if (g.mediana_resp_min != null) o.meds.push(Number(g.mediana_resp_min))
        })
        const filas = Object.values(porAg).sort((a, b) => b.msj - a.msj)
        ...
        const fmtMin = m => m == null ? '—' : m < 60 ? `${Math.round(m)} min` : `${(m / 60).toFixed(1).replace('.', ',')} h`
        const colMin = m => m == null ? '#8b88a8' : m <= 15 ? '#248A3D' : m <= 60 ? '#B25000' : '#FF3B30'
        const espV = (d.esp || []).filter(x => !sucVista || x.sucursal_id === sucVista)
        const totMsj = filas.reduce((s, f) => s + f.msj, 0)
        const nomAsig = x => { const a = Array.isArray(x?.agentes_asignados) ? x.agentes_asignados : []; return a.map(y => normNom(y?.name || `${y?.first_name || ''} ${y?.last_name || ''}`.trim() || nomAg(y?.id))).filter(Boolean).join(', ') }
        return (
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.25fr 1fr', gap: 12, marginBottom: 14 }}>
            <div style={{ ...card, overflowX: 'auto' }}>
              <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8, flexWrap: 'wrap' }}>
                <div style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase', color: '#8b88a8' }}>Gestión de mensajes por vendedor · tiempo real</div>
                <div style={{ display: 'inline-flex', background: '#eceaf6', borderRadius: 8, padding: 2, marginLeft: 'auto' }}>
                  {[['hoy', 'Hoy'], ['7d', '7 días']].map(([k, l]) => (
                    <button key={k} onClick={() => setPerGd(k)} style={{ background: perGd === k ? '#fff' : 'transparent', color: perGd === k ? C2 : '#8b88a8', border: 'none', borderRadius: 6, padding: '4px 10px', fontSize: 11.5, fontWeight: 800, cursor: 'pointer' }}>{l}</button>
                  ))}
                </div>
              </div>
              <table className="com" style={{ width: '100%' }}>
                <thead><tr><th>Vendedor</th>{!sucVista && <th>Tienda</th>}<th style={{ textAlign: 'right' }}>Mensajes</th><th style={{ textAlign: 'right' }}>Clientes</th><th style={{ textAlign: 'right' }}>Respuestas</th><th style={{ textAlign: 'right' }}>Mediana resp.</th><th style={{ textAlign: 'right' }}>Leídos</th></tr></thead>
                <tbody>
                  {filas.map(f => {
                    const m = med(f.meds)
                    return (
                      <tr key={f.id}>
                        <td style={{ fontWeight: 600, whiteSpace: 'nowrap' }}>{nomAg(f.id)}</td>
                        {!sucVista && <td style={{ fontSize: 11.5, color: '#8b88a8' }}>{nomSuc(f.suc)}</td>}
                        <td style={{ textAlign: 'right', fontWeight: 800 }}>{fN(f.msj)}</td>
                        <td style={{ textAlign: 'right' }}>{fN(f.cli)}</td>
                        <td style={{ textAlign: 'right' }}>{fN(f.resp)}</td>
                        <td style={{ textAlign: 'right', fontWeight: 800, color: colMin(m) }}>{fmtMin(m)}</td>
                        <td style={{ textAlign: 'right', color: '#5a5a6e' }}>{f.msj ? pct(f.leid, f.msj) + '%' : '—'}</td>
                      </tr>
                    )
                  })}
                  {!filas.length && <tr><td colSpan={7} style={{ textAlign: 'center', color: '#8b88a8', padding: 16 }}>Sin mensajes de vendedores en el período</td></tr>}
                </tbody>
              </table>
              <div style={{ fontSize: 10.5, color: '#a6a3bd', marginTop: 6 }}>
                Solo mensajes escritos por personas (la IA y las respuestas automáticas se excluyen). Mediana de respuesta: minutos entre el primer mensaje del cliente sin contestar y la respuesta del vendedor. {totMsj ? `${fN(totMsj)} mensajes humanos en el período.` : ''}
              </div>
            </div>

            <div style={{ ...card, overflowX: 'auto', borderLeft: `4px solid ${espV.length ? '#FF3B30' : '#248A3D'}` }}>
              <div style={{ fontSize: 12, fontWeight: 800, textTransform: 'uppercase', color: '#8b88a8', marginBottom: 8 }}>Clientes esperando respuesta ahora ({espV.length})</div>
              {espV.slice(0, 25).map(x => (
                <div key={x.ai_contact_id} style={{ display: 'flex', alignItems: 'center', gap: 8, padding: '6px 2px', borderBottom: '1px solid #f4f3fa' }}>
                  <div style={{ flex: 1, minWidth: 0 }}>
                    <div style={{ fontSize: 12.5, fontWeight: 700, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{normNom(x.nombre || x.telefono)}</div>
                    <div style={{ fontSize: 10.5, color: '#8b88a8' }}>{[!sucVista && nomSuc(x.sucursal_id), x.etapa, nomAsig(x) || 'sin vendedor asignado'].filter(Boolean).join(' · ')}</div>
                  </div>
                  <span style={{ fontSize: 12, fontWeight: 800, color: x.minutos_esperando >= 60 ? '#FF3B30' : x.minutos_esperando >= 15 ? '#B25000' : '#248A3D', whiteSpace: 'nowrap' }}>{fmtMin(Number(x.minutos_esperando))}</span>
                  <a href={`https://app.vambeai.com/chat?chatContactId=${x.ai_contact_id}`} target="_blank" rel="noreferrer"
                    style={{ fontSize: 11, fontWeight: 800, color: C2, background: '#f0eff7', borderRadius: 6, padding: '3px 8px', textDecoration: 'none' }}>Abrir chat</a>
                </div>
              ))}
              {!espV.length && <div style={{ fontSize: 12.5, color: '#248A3D', fontWeight: 700 }}>✓ Ningún cliente esperando respuesta de un vendedor</div>}
              <div style={{ fontSize: 10.5, color: '#a6a3bd', marginTop: 6 }}>Último mensaje del cliente sin respuesta y la conversación no la está atendiendo la IA. Se actualiza cada 5 minutos.</div>
            </div>
          </div>
        )
      })()}
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════════ */
function TabMiDia({ sucursales, vendedores, metas, seg, setSeg, cu, esGerente, isMobile, onVerRadar }) {
