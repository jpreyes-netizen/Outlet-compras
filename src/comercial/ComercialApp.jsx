...
/* Días hábiles del mes ya transcurridos: hasta hoy si es el mes en curso,
   el mes completo si ya pasó, 0 si es futuro. Mismo criterio (lun–sáb menos
   feriados no trabajados) que diasHabiles(). Base para prorrateo y run-rate. */
    ...
    <div style={{ fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif", background: '#f4f4fb', minHeight: '100vh', fontSize: 14, color: '#1c1c1e' }}>
      ...
      <div style={{ padding: isMobile ? '12px 12px 40px' : '18px 22px 48px', maxWidth: 1280, margin: '0 auto' }}>
        {loadingBase ? (
          ...
          <>
            {tab === 'dashboard' && <TabDashboard {...{ sucursales, metas, seg, anio, setAnio, mes, setMes, feriados, isMobile }} />}
...
function TabDashboard({ sucursales, metas, seg, anio, setAnio, mes, setMes, feriados, isMobile }) {
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      setLoading(true)
      try {
        const [ci, inc] = await Promise.all([
  ...
  const porSuc = useMemo(() => activas.map(s => {
    ...
    const metaFecha = dhTot > 0 ? meta * (dhTrans / dhTot) : 0        // meta prorrateada a hoy
    const proy = dhTrans > 0 ? (venta / dhTrans) * dhTot : (esMesFuturo ? 0 : venta) // run-rate
  ...
  }), [cierres, metas, anio, mes, sucursales, feriados])
  ...
  return (
    <div>
      <div style={{ display: 'inline-flex', background: '#eceaf6', borderRadius: 10, padding: 3, marginBottom: 14 }}>
        {[['rendimiento', '📊 Rendimiento'], ['explorar', '🔧 Explorar']].map(([k, l]) => (
      ...
      {modo === 'rendimiento'
        ? <VambeRendimiento {...{ vendedores, anio, setAnio, mes, setMes, isMobile }} />
        : <VambeExplorador {...{ cu, isMobile }} />}
    </div>
  )
}
...
function VambeRendimiento({ vendedores, anio, setAnio, mes, setMes, isMobile }) {
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      setLoading(true)
      try {
        const { data } = await supabase.from('vambe_eventos').select('*')
  ...
  // mapa vambe_agent_id -> vendedor del ERP
  const vmap = useMemo(() => {
    const m = {}
    vendedores.forEach(v => { if (v.vambe_agent_id) m[v.vambe_agent_id] = v })
    return m
  }, [vendedores])
  const nombreVend = e => vmap[e.agent_id]?.nombre || e.agent_name || e.agent_email || 'Sin identificar'
  const sucVend = e => vmap[e.agent_id]?.sucursal_id || null

  const asignaciones = useMemo(() => eventos.filter(e => e.evento === 'agente_asignado'), [eventos])
  const cierres = useMemo(() => eventos.filter(e => e.evento === 'ticket_cerrado' || ['won', 'lost'].includes((e.ticket_status || '').toLowerCase())), [eventos])

  // ranking por vendedor
  const ranking = useMemo(() => {
    const acc = {}
    asignaciones.forEach(e => {
      const key = e.agent_id || e.agent_email || 'sin'
      if (!acc[key]) acc[key] = { key, nombre: nombreVend(e), suc: sucVend(e), asigs: 0, contactos: new Set(), ganados: 0, perdidos: 0, ultima: e.created_at }
      acc[key].asigs += 1
      if (e.contact_id) acc[key].contactos.add(e.contact_id)
    })
    cierres.forEach(e => {
      const key = e.agent_id || e.agent_email || 'sin'
      if (!acc[key]) acc[key] = { key, nombre: nombreVend(e), suc: sucVend(e), asigs: 0, contactos: new Set(), ganados: 0, perdidos: 0, ultima: e.created_at }
      if ((e.ticket_status || '').toLowerCase() === 'won') acc[key].ganados += 1
      else if ((e.ticket_status || '').toLowerCase() === 'lost') acc[key].perdidos += 1
    })
    return Object.values(acc).map(r => ({ ...r, nContactos: r.contactos.size })).sort((a, b) => b.asigs - a.asigs)
  }, [asignaciones, cierres, vmap])

  const totalAsig = asignaciones.length
  const vendActivos = ranking.length
  const contactosUnicos = new Set(asignaciones.map(e => e.contact_id).filter(Boolean)).size

  // desglose por canal y pipeline
  const porClave = (arr, campo) => {
    const m = {}
    arr.forEach(e => { const k = e[campo] || '—'; m[k] = (m[k] || 0) + 1 })
    return Object.entries(m).sort((a, b) => b[1] - a[1])
  }
  const porCanal = useMemo(() => porClave(asignaciones, 'channel'), [asignaciones])
  const porPipe = useMemo(() => porClave(asignaciones, 'pipeline'), [asignaciones])

  const selStyle = { padding: '7px 10px', border: '1px solid #e0def0', borderRadius: 8, fontSize: 13, background: '#fff' }
  const noIdent = ranking.filter(r => r.nombre === 'Sin identificar').length
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        {loading && <span style={{ fontSize: 12, color: '#8b88a8' }}>Cargando…</span>}
        <span style={{ fontSize: 11.5, color: '#8b88a8', marginLeft: 'auto' }}>{eventos.length} eventos capturados</span>
      </div>

      {eventos.length === 0 ? (
        <div style={{ padding: 16, background: '#FF950012', color: '#B25000', borderRadius: 10, fontSize: 12.5, fontWeight: 600 }}>
          Aún no hay eventos de Vambe en {MESES[mes - 1]} {anio}. El workflow captura de aquí en adelante: cada asignación de un contacto a un vendedor aparecerá acá. Activa más workflows (ticket cerrado, ingreso a etapa) para enriquecer las métricas.
        </div>
      ) : (
        <>
          {/* KPIs */}
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(4,1fr)', gap: 10, marginBottom: 16 }}>
            {[
              ['Asignaciones', fN(totalAsig), C1],
              ['Vendedores activos', String(vendActivos), '#1c1c1e'],
              ['Contactos únicos', fN(contactosUnicos), '#1c1c1e'],
              ['Cierres (won/lost)', cierres.length ? fN(cierres.length) : '—', cierres.length ? '#34C759' : '#c9c7dd'],
            ].map(([l, v, c]) => (
              <div key={l} style={{ background: '#fff', borderRadius: 12, padding: '12px 14px', boxShadow: '0 1px 3px rgba(0,0,0,.05)' }}>
                <div style={{ fontSize: 10.5, textTransform: 'uppercase', letterSpacing: '.04em', color: '#8b88a8', fontWeight: 700 }}>{l}</div>
                <div style={{ fontSize: 20, fontWeight: 800, color: c, marginTop: 2 }}>{v}</div>
          ...
          <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 1px 3px rgba(0,0,0,.05)', overflowX: 'auto', marginBottom: 16 }}>
            <table className="com">
              <thead><tr><th style={{ textAlign: 'center' }}>#</th><th>Vendedor</th><th>Sucursal</th><th style={{ textAlign: 'right' }}>Asignaciones</th><th style={{ textAlign: 'right' }}>Contactos únicos</th>{cierres.length > 0 && <><th style={{ textAlign: 'right' }}>Ganados</th><th style={{ textAlign: 'right' }}>Perdidos</th></>}<th>Última actividad</th></tr></thead>
              <tbody>
                {ranking.map((r, i) => (
                  <tr key={r.key}>
                    <td style={{ textAlign: 'center', fontWeight: 800, color: i < 3 ? C1 : '#8b88a8' }}>{i + 1}</td>
                    <td style={{ fontWeight: 600, whiteSpace: 'nowrap' }}>{r.nombre}{r.nombre === 'Sin identificar' && <span title="Sin cruce con com_vendedores" style={{ color: '#FF9500' }}> ⚠</span>}</td>
                    <td style={{ fontSize: 11.5, color: '#5a5a6e' }}>{r.suc ? shortKey(r.suc).toUpperCase() : '—'}</td>
                    <td style={{ textAlign: 'right', fontWeight: 700 }}>{fN(r.asigs)}</td>
                    <td style={{ textAlign: 'right' }}>{fN(r.nContactos)}</td>
                    {cierres.length > 0 && <><td style={{ textAlign: 'right', color: '#248A3D', fontWeight: 700 }}>{r.ganados || '—'}</td><td style={{ textAlign: 'right', color: '#8b88a8' }}>{r.perdidos || '—'}</td></>}
                    <td style={{ fontSize: 11, color: '#8b88a8', whiteSpace: 'nowrap' }}>{(r.ultima || '').slice(0, 16).replace('T', ' ')}</td>
                  </tr>
                ))}
              </tbody>
            </table>
          </div>
          {noIdent > 0 && <div style={{ fontSize: 11, color: '#B25000', marginBottom: 16 }}>⚠ Hay agentes sin cruce con tus vendedores (posiblemente Claudia Reyes, Valeska o Rodrigo). Si los quieres medir, agrégalos a com_vendedores con su vambe_agent_id.</div>}
            ...
            {[['Por canal', porCanal], ['Por embudo', porPipe]].map(([titulo, datos]) => (
              <div key={titulo} style={{ background: '#fff', borderRadius: 12, padding: 14, boxShadow: '0 1px 3px rgba(0,0,0,.05)' }}>
                ...
                {datos.map(([k, n]) => {
                  ...
                  return (
                    <div key={k} style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 6 }}>
                      <div style={{ width: 140, fontSize: 11.5, color: '#5a5a6e', overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{k}</div>
