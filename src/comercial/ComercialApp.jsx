...
function TabCotizaciones({ sucursales, vendedores, sucSel, setSucSel, seg, setSeg, cu, isMobile }) {
  ...
  const abandonadas = sinContactar.filter(r => daysAgo(r.date) >= 2).length
    ...
    <div>
      ...
      {(montoRiesgo > 0 || vencidas > 0 || abandonadas > 0) && (
        <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 12 }}>
          ...
          {abandonadas > 0 && <div style={{ padding: '8px 12px', background: '#FF950012', color: '#B25000', borderRadius: 10, fontSize: 12, fontWeight: 700 }}>⏰ {abandonadas} llevan 2+ días sin contacto</div>}
      ...
      <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #eceaf6', overflow: 'auto', marginBottom: 12 }}>
        ...
        <table className="com">
          <thead><tr><th>Vendedor</th><th style={{ textAlign: 'right' }}>Cotiz.</th><th style={{ textAlign: 'right' }}>Monto cotizado</th><th style={{ textAlign: 'right' }}>Sin contactar</th><th style={{ textAlign: 'right' }}>Pipeline</th><th style={{ textAlign: 'right' }}>Convertidas</th><th style={{ textAlign: 'right' }}>Monto conv.</th><th>Conversión</th></tr></thead>
          <tbody>
            {(() => {
              const acc = {}
              base.forEach(r => {
                const k = String(r.seller?.id || 's/n')
                if (!acc[k]) acc[k] = { k, nombre: r.seller?.name || '—', n: 0, monto: 0, sin: 0, pipe: 0, conv: 0, montoConv: 0 }
      ...
      {(() => {
        ...
        const muertas = base.filter(r => r.estado === 'sin_contactar' && daysAgo(r.date) >= 5)
        ...
        const copiar = () => {
          const lineas = [
            ...
            ``,
            `1) COTIZACIONES MUERTAS (5+ días sin contactar): ${muertas.length} por ${fmt(mMuertas)}`,
        ...
        return (
          <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #FF3B3030', padding: '12px 16px', marginBottom: 12 }}>
            ...
            <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1fr 1fr 1fr', gap: 12 }}>
              <div>
                <div style={{ fontSize: 11, fontWeight: 800, color: '#FF3B30', marginBottom: 4 }}>MUERTAS (5+ días sin contacto) · {muertas.length} · {fmtK(mMuertas)}</div>
