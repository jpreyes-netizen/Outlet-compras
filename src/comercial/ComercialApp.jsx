      ...
      <div style={{ padding: isMobile ? '12px 12px 40px' : '18px 22px 48px', maxWidth: 1280, margin: '0 auto' }}>
        {loadingBase ? (
          ...
          <>
            ...
            {tab === 'dashboard' && <TabDashboard {...{ sucursales, vendedores, metas, seg, anio, setAnio, mes, setMes, feriados, isMobile }} />}
            ...
            {tab === 'vendedores' && <TabVendedores {...{ sucursales, vendedores, seg, anio, setAnio, mes, setMes, isMobile }} />}
            {tab === 'turnos' && <TabTurnos {...{ sucursales, vendedores, sucSel, setSucSel, anio, setAnio, mes, setMes, cu, isMobile }} />}
            {tab === 'bitacora' && <TabBitacora {...{ sucursales, vendedores, metas, feriados, cu, sucSel, setSucSel, anio, setAnio, mes, setMes, isMobile }} />}
            {tab === 'reportes' && <TabReportes {...{ sucursales, metas, feriados, cu, anio, setAnio, mes, setMes, isMobile }} />}
            {tab === 'incidencias' && <TabIncidencias {...{ sucursales, sucSel, cu, isMobile }} />}
...
function TabMetas({ sucursales, vendedores, feriados, metas, anio, setAnio, mes, setMes, isMobile }) {
  ...
  const [acum, setAcum] = useState({})       // { sucursal_id: venta acumulada del mes }

  const activas = sucursales.filter(s => s.bsale_office_id && s.activa)

  // Acumulado del mes: com_ventas_dia (snapshot BSALE) y, para días sin
  // snapshot, completa con com_cierres.venta_dia. Nunca suma doble.
  const cargarAcum = async () => {
    const desde = `${anio}-${String(mes).padStart(2, '0')}-01`
    const hastaM = `${anio}-${String(mes).padStart(2, '0')}-${String(new Date(anio, mes, 0).getDate()).padStart(2, '0')}`
    const [vd, ci] = await Promise.all([
      supabase.from('com_ventas_dia').select('fecha,sucursal_id,venta').gte('fecha', desde).lte('fecha', hastaM),
      supabase.from('com_cierres').select('fecha,sucursal_id,venta_dia').gte('fecha', desde).lte('fecha', hastaM),
    ])
    const mapa = {}   // `${fecha}|${suc}` -> venta (snapshot manda)
    ;(ci.data || []).forEach(c => { mapa[`${c.fecha}|${c.sucursal_id}`] = Number(c.venta_dia || 0) })
    ;(vd.data || []).forEach(v => { mapa[`${v.fecha}|${v.sucursal_id}`] = Number(v.venta || 0) })
    const out = {}
    Object.entries(mapa).forEach(([k, v]) => { const suc = k.split('|')[1]; out[suc] = (out[suc] || 0) + v })
    setAcum(out)
  }
  useEffect(() => { cargarAcum() /* eslint-disable-next-line */ }, [anio, mes])
  ...
  const cargarVentas = async () => {
    ...
    try {
      ...
      setVentas(out)
      // ── respaldo automático en Supabase (upsert por fecha+sucursal) ──
      const filas = activas.filter(s => !out[s.sucursal_id]?.error).map(s => ({
        fecha, sucursal_id: s.sucursal_id,
        venta: out[s.sucursal_id]?.total || 0,
        docs: out[s.sucursal_id]?.docs || 0,
        ventas_vendedor: out[s.sucursal_id]?.ventas || [],
        actualizado_at: new Date().toISOString(),
      }))
      if (filas.length) {
        const { error } = await supabase.from('com_ventas_dia').upsert(filas, { onConflict: 'fecha,sucursal_id' })
        if (!error) cargarAcum()
      }
  ...
  return (
    <div>
      ...
      {/* Acumulado del mes: por sucursal + general */}
      <div style={{ background: '#fff', borderRadius: 14, border: '1px solid #eceaf6', padding: '14px 16px', marginBottom: 16 }}>
        <div style={{ fontWeight: 800, fontSize: 12.5, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '.04em', color: '#8b88a8' }}>
          Acumulado {MESES[mes - 1]} · venta registrada vs meta prorrateada a hoy
        </div>
        {(() => {
          const filasAc = activas.map(s => {
            const sk = shortKey(s.sucursal_id)
            const venta = acum[s.sucursal_id] || 0
            const meta = metaMes(s.sucursal_id)
            const dhTot = dh(s.sucursal_id)
            const dhTrans = diasHabilesTranscurridos(anio, mes, sk, feriados)
            const metaFecha = dhTot > 0 ? meta * (dhTrans / dhTot) : 0
            return { s, venta, meta, metaFecha, cump: pct(venta, metaFecha), cumpMes: pct(venta, meta) }
          })
          const tVenta = filasAc.reduce((a, x) => a + x.venta, 0)
          const tMetaF = filasAc.reduce((a, x) => a + x.metaFecha, 0)
          const tMeta = filasAc.reduce((a, x) => a + x.meta, 0)
          const gen = { venta: tVenta, metaFecha: tMetaF, meta: tMeta, cump: pct(tVenta, tMetaF), cumpMes: pct(tVenta, tMeta) }
          const fila = (nombre, x, esTotal) => (
            <div key={nombre} style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 7, paddingTop: esTotal ? 8 : 0, borderTop: esTotal ? '1px solid #f0eff7' : 'none' }}>
              <div style={{ width: isMobile ? 90 : 150, fontSize: 12, fontWeight: esTotal ? 800 : 600, whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{nombre}</div>
              <div style={{ flex: 1 }}><Bar v={x.cump} color={colorCump(x.cump)} /></div>
              <div style={{ width: 46, textAlign: 'right', fontSize: 12.5, fontWeight: 800, color: colorCump(x.cump) }}>{x.cump}%</div>
              <div style={{ width: isMobile ? 110 : 210, textAlign: 'right', fontSize: 11, color: '#5a5a6e', fontVariantNumeric: 'tabular-nums', whiteSpace: 'nowrap' }}>
                {fmtK(x.venta)} / {fmtK(x.metaFecha)}{!isMobile && <span style={{ color: '#c9c7dd' }}> · mes {x.cumpMes}% de {fmtK(x.meta)}</span>}
              </div>
            </div>
          )
          return (
            <>
              {filasAc.map(x => fila(x.s.nombre, x, false))}
              {fila('TOTAL GENERAL', gen, true)}
              <div style={{ fontSize: 10.5, color: '#a6a3bd', marginTop: 6 }}>
                Fuente: registro diario BSALE (com_ventas_dia) + cierres de caja para días sin registro. Consulta un día en esta pestaña y queda guardado como respaldo automáticamente.
              </div>
            </>
          )
        })()}
...
function TabCotizaciones({ sucursales, vendedores, sucSel, setSucSel, seg, setSeg, cu, isMobile }) {
  ...
  const base = rows.filter(r => !fSuc || r.sucursal_id === fSuc)
  ...
  const vendsFiltro = fSuc ? vendedores.filter(v => v.sucursal_id === fSuc) : vendedores
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
        <select className="com-inp" style={{ width: 170 }} value={fSuc} onChange={e => { setFSuc(e.target.value); setFVend('') }}>
...
function TabDashboard({ sucursales, vendedores, metas, seg, anio, setAnio, mes, setMes, feriados, isMobile }) {
  ...
  const [fSuc, setFSuc] = useState('')          // '' = todas las sucursales
...
function TabBitacora({ sucursales, vendedores, metas, feriados, cu, sucSel, setSucSel, anio, setAnio, mes, setMes, isMobile }) {
    ...
    const cargar = async () => {
      ...
      try {
        const [ap, ci] = await Promise.all([
          supabase.from('com_aperturas').select('*').eq('sucursal_id', sucSel).gte('fecha', desde).lte('fecha', hasta),
          supabase.from('com_cierres').select('*').eq('sucursal_id', sucSel).gte('fecha', desde).lte('fecha', hasta),
        ])
        if (cancel) return
...
function SheetDia({ dia, onClose, isMobile }) {
  ...
  return (
    ...
    {abrirAp && (() => {
      const suc = sucursales.find(s => s.sucursal_id === sucSel)
      const hoyIso = new Date().toLocaleDateString('en-CA')
...
/* ═══════════════════════════════════════════════════════════════════════════
   ...
   ═══════════════════════════════════════════════════════════════════════════ */
function TabVendedores({ sucursales, vendedores, seg, anio, setAnio, mes, setMes, isMobile }) {
  const [cierres, setCierres] = useState([])
  const [loading, setLoading] = useState(false)
  const [fSuc, setFSuc] = useState('')
  ...
  const filas = useMemo(() => {
    ...
    if (fSuc) arr = arr.filter(r => r.sucursal_id === fSuc)
...
function TabMiDia({ sucursales, vendedores, metas, seg, setSeg, cu, esGerente, isMobile }) {
  ...
  useEffect(() => { if (!vendSel && (propio || vendedores.length)) setVendSel(propio?.bsale_user_id ?? vendedores.filter(v => v.activo !== false)[0]?.bsale_user_id) }, [propio, vendedores])
  const vend = vendedores.find(v => String(v.bsale_user_id) === String(vendSel))
  const bloqueado = !!propio && !esGerente   // vendedor solo se ve a sí mismo
  ...
  const cargar = async () => {
    ...
    try {
      const [cp, vd, cq, cm, vc] = await Promise.all([
        ...
        supabase.from('com_ventas_dia').select('venta,docs,ventas_vendedor,actualizado_at').eq('fecha', hoyIso).eq('sucursal_id', vend.sucursal_id).maybeSingle(),
        ...
        supabase.from('com_cierres').select('ventas_vendedor').eq('sucursal_id', vend.sucursal_id).gte('fecha', mesIni).lte('fecha', hoyIso),
        supabase.from('vambe_contactos').select('contact_id,contact_name,phone,agent_principal,resolution_status'),
      ])
      setCompHoy(Number(cp.data?.compromiso || 0) || null)
      ...
      setCots(cq.data || [])
      // SLA: primera gestión de sus cotizaciones (bitácora)
      ...
      setSlaMapMi(sm)
      // venta acumulada del mes (desde cierres)
      let vAc = 0, dAc = 0
      ;(cm.data || []).forEach(c => { const me = (c.ventas_vendedor || {})[String(vend.bsale_user_id)]; if (me) { vAc += Number(me.venta || 0); dAc += Number(me.docs || 0) } })
      setVentaMes({ venta: vAc, docs: dAc })
      // chats sin responder: contactos asignados a este vendedor
      const nn = normN(vend.nombre)
      const mios = (vc.data || []).filter(x => x.agent_principal && (normN(x.agent_principal) === nn || normN(x.agent_principal).includes(nn) || nn.includes(normN(x.agent_principal))))
      ...
      for (let i = 0; i < cids.length; i += 100) {
        const { data: mg } = await supabase.from('vambe_mensajes')
          .select('ai_contact_id,direction,user_id,assistant_id,created_at,body_preview')
          .in('ai_contact_id', cids.slice(i, i + 100)).gte('created_at', desdeM).order('created_at', { ascending: true })
        msgs = msgs.concat(mg || [])
      }
      const em = m => m.direction === 'inbound' ? 'cliente' : m.assistant_id ? 'bot' : m.user_id === VAMBE_WORKSPACE ? 'auto' : 'humano'
      ...
      Object.entries(porC).forEach(([cid, list]) => {
        ...
        list.forEach(m => { const e = em(m); const t = new Date(m.created_at).getTime()
          if (e === 'humano' || e === 'bot') { lastHum = Math.max(lastHum, t); }
        })
        list.forEach(m => { if (em(m) !== 'cliente') return; const t = new Date(m.created_at).getTime()
          if (t > lastHum) { if (primeraEspera === null) primeraEspera = t; preview = m.body_preview || preview } })
        if (primeraEspera !== null) {
          const inf = infoC[cid] || {}
          cola.push({ cid, nombre: inf.contact_name || inf.phone || '—', fono: (inf.phone || '').replace(/[^\d]/g, ''), espera: (Date.now() - primeraEspera) / 3600000, preview })
        }
      })
  ...
  const refrescar = async () => {
    const suc = sucursales.find(s => s.sucursal_id === vend?.sucursal_id)
  ...
  if (!vend) return <div style={{ padding: 20, color: '#8b88a8', fontSize: 13 }}>No hay vendedores configurados.</div>
  return (
    <div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center', marginBottom: 12 }}>
        {bloqueado ? (
          ...
          <>
            <select className="com-inp" style={{ width: 190 }} value={vendSel || ''} onChange={e => setVendSel(e.target.value)}>
              {vendedores.filter(v => v.activo !== false).map(v => <option key={v.bsale_user_id} value={v.bsale_user_id}>{v.nombre}</option>)}
            </select>
            <span style={{ fontSize: 12, color: '#8b88a8' }}>{fmtFecha(hoyIso)} · vista de coaching (el vendedor ve solo la suya)</span>
          </>
        )}
        {loading && <span style={{ fontSize: 12, color: '#8b88a8' }}>Cargando…</span>}
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(5,1fr)', gap: 10, marginBottom: 14 }}>
        <div style={{ ...card, padding: '12px 14px', borderTop: `3px solid ${cump === null ? '#c9c7dd' : colorCump(cump)}` }}>
          ...
          <div style={{ fontSize: 10.5, color: '#8b88a8' }}>{compHoy ? `compromiso ${fmtK(compHoy)} · ${cump ?? '—'}%` : 'sin compromiso registrado'}</div>
        </div>
        {[
          ['Docs hoy', ventaHoy ? fN(ventaHoy.docs) : '—', '#1c1c1e', ''],
          ['Venta del mes', fmtK(ventaMes.venta), C1, `${fN(ventaMes.docs)} docs`],
          ['Conversión mes', gestMes ? pct(convMes, mesRows.length) + '%' : '—', '#248A3D', `${convMes} de ${mesRows.length} cotiz.`],
          ['Mi SLA 24h', sla24 === null ? '—' : sla24 + '%', sla24 === null ? '#c9c7dd' : sla24 >= 80 ? '#248A3D' : sla24 >= 50 ? '#B25000' : '#FF3B30', 'contacto en 24h'],
        ].map(([l, v, c2, sub]) => (
          <div key={l} style={{ ...card, padding: '12px 14px' }}>
