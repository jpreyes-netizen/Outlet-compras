...
import { TabValidaciones } from './TabValidaciones'
import { PmoGestion, PmoResumenHoy } from '../proyectos/ProyectosApp'
    ...
    <div style={{ fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif", background: '#f4f4fb', minHeight: '100vh', fontSize: 14, color: '#1c1c1e' }}>
      <style>{`
        *{box-sizing:border-box}
        ...
        button[data-ayuda],a[data-ayuda]{cursor:pointer}
        div[data-ayuda],span[data-ayuda],th[data-ayuda],label[data-ayuda]{text-decoration:underline dotted rgba(22,33,62,.32);text-underline-offset:3px;text-decoration-thickness:1px}
      ...
      <div style={{ padding: isMobile ? '12px 12px 40px' : '18px 28px 48px', maxWidth: ['vambe', 'midia', 'dashboard', 'metas', 'cotizaciones'].includes(tab) ? 'none' : 1280, margin: '0 auto' }} className={['vambe', 'midia', 'dashboard', 'metas', 'cotizaciones'].includes(tab) ? 'com-amplio' : undefined}>
            <AyudaGlobal />
            <GuiaPestana tab={tab} rol={perfilNav.rol} vista={cotVista} />
        {loadingBase ? (
          ...
          <>
            {(tab === 'midia' || tab === 'bitacora') && <PmoResumenHoy cu={cu} onAbrir={() => setTab('gestion')} />}
            ...
            {tab === 'gestion' && <PmoGestion cu={cu} area="comercial" />}
            ...
            {tab === 'validaciones' && <TabValidaciones {...{ sucursales, sucSel, setSucSel, cu, esGerente, isMobile }} />}
          ...
          return (
            <div key={s.sucursal_id} style={{ ...card, padding: 0, overflow: 'hidden' }}>
              ...
              <div style={{ padding: '12px 14px 4px' }}>
                <GraficoVentaDiaria serie={serie.map(d => ({ fecha: d.fecha, venta: d.venta, meta: d.meta }))} feriados={feriados} alto={140} />
              </div>
    ...
    const cargar = async () => {
      ...
      try {
        const [ci, vdq, inc, cot, msj, cv] = await Promise.all([
          ...
          Promise.resolve({ data: [] }),   // (antes: vambe_mensajes congelada; la mensajería vive en el Centro de conversaciones)
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '3fr 2fr', gap: 12, marginBottom: 16 }}>
        ...
        <div style={{ background: '#fff', borderRadius: 12, padding: 14, boxShadow: '0 1px 3px rgba(0,0,0,.05)' }}>
          ...
          <GraficoVentaDiaria feriados={feriados} alto={160}
            serie={porDia.map(([f, v]) => ({ fecha: f, venta: v, meta: tot.dhTot > 0 ? tot.meta / tot.dhTot : 0 }))} />
        </div>
        {/* Funnel cotizaciones */}
        <div style={{ background: '#fff', borderRadius: 12, padding: 14, boxShadow: '0 1px 3px rgba(0,0,0,.05)' }}>
          ...
          <ComoLeer texto="cada barra es la proporción de cotizaciones del mes en ese estado. Lo sano es que crezcan Convertidas y se achique Sin contactar."
            conclusion={gestTot > 0 ? `${funnel.sin_contactar || 0} de ${gestTot} cotizaciones del mes siguen sin contactar (${Math.round((funnel.sin_contactar || 0) / gestTot * 100)}%)` : null}
            tono={gestTot > 0 && (funnel.sin_contactar || 0) / gestTot > 0.2 ? '#D92D20' : '#1E7A44'} />
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(0,1.7fr) minmax(0,1fr)', gap: 14 }}>
        <div style={card}>
          ...
          {(() => {
            const gaps = rangoH.map(h => ({ h, c: hs[h]?.c || 0, v: (hs[h]?.v || 0) + (hs[h]?.ia || 0) })).filter(x => x.c > 0)
            const peor = gaps.slice().sort((a, b) => (b.c - b.v) - (a.c - a.v))[0]
            return <ComoLeer texto="por cada hora, la barra lila son los mensajes de clientes y las oscuras y verdes, las respuestas. Si la lila supera mucho a las otras, faltó gente atendiendo."
              conclusion={peor && peor.c > peor.v ? `La hora más descubierta fue las ${peor.h}:00: ${peor.c} mensajes de clientes contra ${peor.v} respuestas.` : (gaps.length ? 'Todas las horas con demanda tuvieron respuesta equivalente.' : null)}
              tono={peor && peor.c > peor.v * 1.5 ? '#D92D20' : '#1E7A44'} />
          })()}
        </div>
        <div style={card}>
          ...
          <ComoLeer texto="tiempo hábil entre el primer mensaje del cliente sin respuesta y la respuesta del vendedor (sin IA ni respuestas automáticas). Lo ideal es que domine el verde."
            conclusion={tramosAll.reduce((s, x) => s + x, 0) ? `${Math.round((tramosAll[0] + tramosAll[1]) / tramosAll.reduce((s, x) => s + x, 0) * 100)}% de las respuestas llegó en 15 minutos o menos.` : null}
            tono={tramosAll.reduce((s, x) => s + x, 0) && (tramosAll[0] + tramosAll[1]) / tramosAll.reduce((s, x) => s + x, 0) >= 0.7 ? '#1E7A44' : '#B25E09'} />
        </div>
      </div>
    </div>
  )
    ...
    <div style={{ marginBottom: 16 }}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : 'minmax(0,1.35fr) minmax(0,1fr)', gap: 14, marginBottom: 14 }}>
        <div style={card}>
          ...
          <ComoLeer texto="cada barra es el monto en esa etapa; el número de la derecha, cuántas cotizaciones. El pipeline sano avanza hacia Convertida."
            conclusion={ab.length ? `${pct(sum(riesgo, x => x.total), sum(ab, x => x.total))}% del pipeline abierto está en riesgo · la etapa con más dinero es ${(et.slice().sort((a, b) => b.m - a.m)[0] || {}).l}` : null}
            tono={riesgo.length && sum(riesgo, x => x.total) / Math.max(1, sum(ab, x => x.total)) > 0.3 ? '#D92D20' : '#1E7A44'} />
        </div>
        <div style={card}>
...
function PacingMes({ gen, resumenes, activas, isMobile }) {
  ...
  return (
    <div style={{ ...card, marginBottom: 16 }}>
      ...
      <ComoLeer texto="la barra de color es lo vendido; la marca oscura, lo que deberíamos llevar hoy; la zona clara, la proyección de cierre si seguimos al mismo ritmo."
        conclusion={gen.venta >= gen.metaFecha ? `Vamos ${fmtK(gen.venta - gen.metaFecha)} sobre lo esperado a hoy: manteniendo el ritmo cerramos en ${gen.cumpProy}% de la meta.`
          : `Vamos ${fmtK(gen.metaFecha - gen.venta)} atrasados respecto de lo esperado a hoy: hay que vender ${fmtK(gen.ritmoReq)} por día hábil para llegar a la meta.`}
        tono={gen.venta >= gen.metaFecha ? '#1E7A44' : '#D92D20'} />
      {resumenes.length > 1 && (
...
/* ═══════════════════════════════════════════════════════════════════════════
   SISTEMA DE AYUDA · la app se explica sola
   · AYUDA: diccionario etiqueta → explicación (botones, columnas, indicadores, títulos)
   · AyudaGlobal: muestra la explicación al pasar el mouse (o tocar un título en celular)
   · GuiaPestana: recuadro "cómo usar esta pantalla" al inicio de cada pestaña, por rol
   · ComoLeer: guía de lectura + conclusión automática bajo cada gráfico
   ═══════════════════════════════════════════════════════════════════════════ */
const normAy = t => String(t || '').normalize('NFD').replace(/[\u0300-\u036f]/g, '').toLowerCase()
  .replace(/\(.*?\)/g, ' ').replace(/[^a-z0-9ñ%+≤ ]/g, ' ').replace(/\s+/g, ' ').trim()
const AYUDA_RAW = {
  // ── pestañas ──
  'b:mi dia': 'Tu pantalla de trabajo del día: ventas, cotizaciones por contactar, chats de WhatsApp esperando y prioridades.',
  'b:dashboard': 'Foto completa de la gestión: venta vs meta, alertas en vivo, pipeline, conversaciones y scorecard por vendedor.',
  'b:metas de venta': 'Avance de la venta contra la meta del día y del mes, ritmo necesario para cumplir y ranking del equipo.',
  'b:cotizaciones': 'Todas las cotizaciones de BSALE con su estado de gestión. Aquí se registra cada contacto con el cliente.',
  'b:despachos': 'Pedidos por entregar o retirar: estado del despacho y del picking de bodega.',
  'b:vendedores': 'Resultados de cada vendedor: venta, tickets, conversión y actividad en WhatsApp.',
  'b:turnos': 'Parrilla de turnos del mes. El jefe la programa; cada vendedor ve la suya.',
  'b:bitacora': 'Apertura y cierre diario de la tienda: checklist, compromisos por vendedor y venta del día.',
  'b:incidencias': 'Registro de problemas de la tienda (caja, infraestructura, stock, personal) y su seguimiento.',
  'b:reportes': 'Informes diario, semanal y mensual para enviar a gerencia, con métricas ya calculadas.',
  'b:vambe': 'Centro de conversaciones de WhatsApp: quién responde, qué tan rápido y qué clientes esperan.',
  'b:configuracion': 'Parámetros del módulo: metas mensuales, sucursales BSALE, vendedores y reglas.',
  'b:gestion': 'Tus tareas y proyectos del área (PMO): qué tienes asignado, plazos y avance.',
  'b:validar pagos': 'Pide a Tesorería validar una transferencia o pago Webpay con su comprobante, y sigue el estado en vivo mientras el cliente espera.',
  'b:derivar tarea': 'Convierte esta incidencia en una tarea con responsable y plazo. Llega a su pestaña Gestión y al correo diario.',
  'b:crear tarea en gestion': 'Crea la tarea en Gestión y avisa por correo al responsable.',
  // ── períodos y navegación ──
  'b:hoy': 'Muestra solo los datos de hoy.', 'b:ayer': 'Muestra los datos de ayer completos.',
  'b:7 dias': 'Muestra los últimos 7 días, incluido hoy.', 'b:30 dias': 'Muestra los últimos 30 días, incluido hoy.',
  'b:mes': 'Muestra el mes seleccionado completo.', 'b:este mes': 'Muestra desde el día 1 del mes hasta hoy.',
  'b:diario': 'Informe de cumplimiento de un día: venta vs meta, acumulado y top vendedores.',
  'b:semanal': 'Reporte de gestión de la semana con tus observaciones y compromisos.',
  'b:mensual': 'Reporte de cierre del mes: logros, problemas y plan para el mes siguiente.',
  'b:lista': 'Lista completa de cotizaciones con su estado y alerta.', 'b:radar': 'Salud del pipeline: qué cotizaciones están en riesgo y qué hacer primero.',
  'b:mi radar': 'Tus cotizaciones abiertas ordenadas por valor y urgencia: parte por la de arriba.',
  'b:monitor': 'Estado en vivo de WhatsApp: conversaciones, respuestas y cola de espera.',
  'b:mensajeria': 'Vista histórica de mensajería (datos hasta julio 2026).',
  'b:ver radar completo': 'Abre el Radar completo dentro de Cotizaciones.',
  'b:centro de conversaciones': 'Abre el centro de conversaciones de WhatsApp con el detalle por vendedor.',
  'b:abrir centro de conversaciones': 'Abre el centro de conversaciones de WhatsApp con el detalle por vendedor.',
  // ── acciones ──
  'b:actualizar': 'Vuelve a leer los datos ahora, sin esperar la actualización automática.',
  'b:actualizar venta': 'Consulta BSALE en este momento y trae la venta más reciente.',
  'b:actualizar ventas': 'Consulta BSALE en este momento y trae la venta más reciente de cada tienda.',
  'b:actualizar dia': 'Consulta BSALE para la fecha elegida y actualiza el informe.',
  'b:recalcular mes': 'Vuelve a traer desde BSALE la venta de todos los días del mes. Úsalo si ves días faltantes.',
  'b:sincronizar': 'Trae desde BSALE las cotizaciones nuevas o modificadas.',
  'b:guardar seguimiento': 'Registra esta gestión: estado, próximo paso y nota. Queda en la bitácora con tu nombre.',
  'b:guardar': 'Guarda los cambios.', 'b:cancelar': 'Cierra sin guardar cambios.', 'b:registrar': 'Registra el dato ingresado.',
  'b:copiar para enviar': 'Copia el informe como texto listo para pegar en WhatsApp o correo.',
  'b:copiar reporte': 'Copia el reporte como texto listo para pegar en WhatsApp o correo.',
  'b:descargar pdf': 'Descarga el informe en PDF para enviarlo o imprimirlo.',
  'b:descargar clientes': 'Descarga a Excel los clientes de las cotizaciones filtradas. Solo jefatura; queda registrado.',
  'b:responder': 'Abre este chat en Vambe para responder al cliente.', 'b:abrir': 'Abre este chat en Vambe.',
  'b:abrir chat': 'Abre este chat en Vambe para responder al cliente.',
  'b:ver toda la tienda': 'Quita el filtro de vendedor y muestra la cola completa.',
  'b:marcar todas': 'Marca todas las cotizaciones de la lista para cerrarlas juntas.',
  'b:desmarcar todas': 'Quita la marca de todas las cotizaciones de la lista.',
  'b:cerrar como perdidas': 'Cierra las cotizaciones marcadas como perdidas con el motivo elegido. Queda en la bitácora con tu nombre.',
  'b:delegar': 'Autoriza a esta persona a hacer la apertura o el cierre de la tienda.',
  'b:quitar': 'Elimina este registro.', 'b:ver descartadas': 'Muestra también las cotizaciones descartadas (pruebas o duplicadas).',
  'b:explorar': 'Abre el detalle para revisarlo.', 'b:ver menos': 'Contrae la lista.',
  // ── estados de cotización ──
  'b:sin contactar': 'Estado inicial automático: nadie ha contactado al cliente todavía.',
  'b:contactado': 'Ya hablaste con el cliente (o intentaste). Obliga a agendar el próximo contacto.',
  'b:en negociacion': 'El cliente está evaluando: precio, medidas o condiciones. Obliga a agendar el próximo contacto.',
  'b:en proceso de despacho': 'Vendida y en camino. Obliga a indicar la fecha de despacho comprometida.',
  'b:convertida': 'Se transformó en venta. Registra el N° de boleta o factura.',
  'b:perdida': 'No se concretó. Obliga a elegir el motivo de pérdida.',
  'b:descartada': 'No era una cotización real (prueba, duplicada). Se excluye de las métricas; se puede revertir.',
  // ── indicadores ──
  'venta mes': 'Venta acumulada del mes según BSALE (boletas + facturas − notas de crédito).',
  'meta mes': 'Meta de venta del mes definida en Configuración.',
  'cumplimiento': 'Venta ÷ meta. Verde ≥ 100 %, naranja ≥ 80 %, rojo bajo 80 %.',
  'transacciones': 'Cantidad de documentos de venta emitidos (boletas y facturas).',
  'ticket prom': 'Venta promedio por documento: venta ÷ transacciones.',
  'ticket': 'Venta promedio por documento: venta ÷ documentos.',
  'incidencias abiertas': 'Problemas registrados que todavía no se cierran.',
  'venta del dia': 'Lo vendido hoy según BSALE.', 'meta del dia': 'Meta del mes dividida por los días hábiles.',
  'cumplimiento dia': 'Venta de hoy ÷ meta del día.', 'ticket promedio hoy': 'Venta de hoy ÷ documentos de hoy.',
  'mi venta de hoy': 'Tu venta de hoy según BSALE, comparada con el compromiso que te fijó tu jefe.',
  'mi venta del mes': 'Tu venta acumulada del mes según BSALE.',
  'cumpl compromiso mes': 'Tu venta del mes ÷ la suma de tus compromisos diarios.',
  'avance a la fecha': 'Cuánto va la venta contra lo que debería ir a esta altura del mes.',
  'falta para la meta': 'Lo que queda por vender para cumplir la meta del mes.',
  'ritmo requerido dia': 'Venta diaria necesaria en los días hábiles que quedan para llegar a la meta.',
  'ritmo diario actual': 'Venta promedio por día hábil transcurrido.',
  'ritmo diario requerido': 'Venta diaria necesaria en los días hábiles que quedan para llegar a la meta.',
  'proyeccion cierre': 'Venta estimada al cierre del mes si se mantiene el ritmo actual.',
  'proyeccion de cierre': 'Venta estimada al cierre del mes si se mantiene el ritmo actual.',
  'proyeccion de cierre de mes': 'Venta estimada al cierre del mes si se mantiene el ritmo actual.',
  'cumplimiento a la fecha': 'Venta ÷ meta prorrateada a hoy. Es el mejor indicador de si vamos bien.',
  'meta a la fecha': 'La parte de la meta mensual que debería estar vendida a esta altura del mes.',
  'dias sobre meta': 'Días en que la venta superó la meta diaria, sobre los días hábiles transcurridos.',
  'pipeline abierto': 'Monto total de cotizaciones todavía en juego (sin contactar, contactadas o en negociación).',
  'pronostico ponderado': 'Venta esperada del pipeline: cada cotización × su probabilidad según la etapa (10 %, 25 %, 50 %).',
  'en riesgo': 'Cotizaciones con alerta: sin contactar a tiempo, seguimiento vencido, estancadas o por decidir.',
  'salud del pipeline': 'Porcentaje de cotizaciones abiertas que no tienen ninguna alerta.',
  'conversion': 'Cotizaciones ganadas ÷ (ganadas + perdidas).', 'conversion dias': 'Cotizaciones ganadas ÷ (ganadas + perdidas) de los últimos 90 días.',
  'cierre sugerido': 'Cotizaciones con 21+ días sin decisión: revísalas y ciérralas si ya no van.',
  'sin contactar h': 'Cotizaciones que pasaron el primer día sin ningún contacto. Es la alerta más urgente.',
  'seguimientos vencidos': 'Cotizaciones cuya fecha de próximo contacto ya pasó sin gestión.',
  'estancadas': 'Cotizaciones abiertas con 5 o más días sin ninguna gestión registrada.',
  'para cierre': 'Cotizaciones con 21+ días sin decisión: revísalas y ciérralas si ya no van.',
  'chats esperando': 'Clientes de WhatsApp cuyo último mensaje no ha sido respondido por un vendedor.',
  'chats sin vendedor': 'Clientes esperando respuesta que no tienen ningún vendedor asignado en Vambe.',
  'esperando respuesta': 'Clientes de WhatsApp cuyo último mensaje no ha sido respondido por un vendedor.',
  'esperandote': 'Tus clientes de WhatsApp que están esperando tu respuesta ahora.',
  'espera mas larga': 'El cliente que más tiempo lleva esperando respuesta.',
  'mediana': 'Valor del medio: la mitad de las respuestas fue más rápida y la otra mitad más lenta.',
  'cumplimiento de meta': 'Porcentaje de respuestas dadas dentro de la meta de minutos hábiles.',
  'mensajes de vendedores': 'Mensajes de WhatsApp escritos por personas (sin IA ni respuestas automáticas).',
  'clientes atendidos': 'Clientes distintos que recibieron al menos un mensaje de un vendedor.',
  'tasa de lectura': 'Porcentaje de mensajes que el cliente abrió y leyó.',
  'conversaciones abiertas': 'Conversaciones de WhatsApp activas en Vambe.',
  'sin ejecutivo': 'Conversaciones abiertas sin vendedor asignado (incluye las que atiende la IA).',
  'humanas sin actividad': 'Conversaciones en etapa de vendedor sin ningún mensaje en 3+ días.',
  'asignadas hoy': 'Conversaciones que Vambe asignó hoy a vendedores.',
  'leads hoy': 'Contactos nuevos que escribieron hoy por WhatsApp.', 'leads dias': 'Contactos nuevos de los últimos 7 días.',
  'escaladas a ti': 'Cotizaciones que ya pasaron del vendedor al jefe por atraso o falta de decisión.',
  'docs hoy': 'Documentos de venta emitidos hoy.', 'venta del mes': 'Venta acumulada del mes según BSALE.',
  'conversion mes': 'Cotizaciones del mes convertidas en venta.', 'mi sla h': 'Porcentaje de tus cotizaciones contactadas dentro de 24 horas.',
  'sla h tienda': 'Porcentaje de cotizaciones de la tienda contactadas dentro de 24 horas.',
  // ── columnas ──
  'th:vendedor': 'Persona responsable.', 'th:tienda': 'Sucursal.', 'th:sucursal': 'Sucursal.',
  'th:mensajes': 'Mensajes de WhatsApp enviados por la persona (sin IA).',
  'th:clientes': 'Clientes distintos atendidos.', 'th:respuestas': 'Veces que respondió a un cliente que esperaba.',
  'th:1ª resp mediana': 'Tiempo típico (mediana) para responder a un cliente, en minutos hábiles.',
  'th:1ª resp': 'Tiempo típico (mediana) para responder, en minutos hábiles.', 'th:1ª resp mes': 'Tiempo típico de respuesta del mes, en minutos hábiles.',
  'th:p90': 'El 90 % de las respuestas fue más rápido que este tiempo. Muestra las esperas largas.',
  'th:meta': 'Porcentaje de respuestas dentro de la meta de minutos hábiles.', 'th:meta resp': 'Porcentaje de respuestas dentro de la meta de minutos hábiles.',
  'th:velocidad': 'Cómo se reparten sus respuestas: verde = rápidas, rojo = más de 4 horas.',
  'th:leidos': 'Porcentaje de sus mensajes que el cliente leyó.', 'th:ultimo msj': 'Hace cuánto envió su último mensaje.',
  'th:esperando': 'Clientes asignados a esta persona que esperan respuesta ahora.',
  'th:chats esperando': 'Clientes asignados a esta persona que esperan respuesta ahora.',
  'th:venta mes': 'Venta del mes según BSALE.', 'th:venta': 'Venta según BSALE.', 'th:docs': 'Documentos de venta emitidos.',
  'th:pipeline abierto': 'Monto de sus cotizaciones todavía en juego.', 'th:pipeline': 'Monto de cotizaciones abiertas.',
  'th:en riesgo': 'Monto de sus cotizaciones con alerta.', 'th:conversion': 'Ganadas ÷ (ganadas + perdidas).', 'th:conv': 'Ganadas ÷ (ganadas + perdidas).',
  'th:venc': 'Cotizaciones con seguimiento vencido.', 'th:estanc': 'Cotizaciones con 5+ días sin gestión.', 'th:cierre': 'Cotizaciones con 21+ días sin decisión.',
  'th:etapa': 'En qué punto del proceso comercial está.', 'th:alerta': 'Estado de salud de la cotización según las reglas de seguimiento.',
  'th:dias': 'Días desde que se emitió la cotización. ⚠ desde el tercer día sin contacto.',
  'th:prox despacho': 'Fecha del próximo contacto comprometido o de despacho.', 'th:prox contacto': 'Fecha del próximo contacto comprometido.',
  'th:gestiones': 'Cantidad de gestiones registradas en su bitácora.', 'th:monto': 'Monto de la cotización.', 'th:cliente': 'Cliente de la cotización.',
  'th:estado': 'Estado actual de la gestión.', 'th:compromiso': 'Venta que el vendedor se comprometió a lograr en la apertura.',
  'th:compromiso acum': 'Suma de sus compromisos diarios del mes.', 'th:cumpl compromiso': 'Venta ÷ compromiso.', 'th:cumpl': 'Venta ÷ meta del día.',
  'th:meta dia': 'Meta del mes dividida por los días hábiles.', 'th:fecha': 'Día.', 'th:abiertas': 'Conversaciones a su cargo ahora.',
  'th:hoy': 'Asignadas hoy.', 'th:manual d': 'Asignaciones hechas a mano (no automáticas) en 7 días.',
  'th:prom dias': 'Días promedio que llevan las conversaciones en esta etapa.', 'th:max': 'La conversación más antigua de la etapa.',
  'th:ultima nota': 'Lo último que se anotó en la bitácora de la cotización.', 'th:msj': 'Mensajes enviados hoy.',
  // ── títulos de sección ──
  'analisis de cumplimiento': 'Compara lo vendido con lo que se debería llevar a esta altura del mes y proyecta el cierre.',
  'venta diaria consolidada': 'Venta de cada día del período. La línea marca la meta diaria.',
  'cotizaciones gestionadas en el mes': 'Cuántas cotizaciones del mes hay en cada estado.',
  'rendimiento por vendedor': 'Tabla comparativa de velocidad y calidad de respuesta en WhatsApp.',
  'scorecard por vendedor mes en curso': 'Resumen de cada vendedor: venta, pipeline, conversión y atención de WhatsApp.',
  'cola priorizada valor × urgencia': 'Cotizaciones ordenadas por monto y atraso: gestiona de arriba hacia abajo.',
  'prioridad de hoy valor × urgencia': 'Lo más valioso y más atrasado primero.',
  'velocidad de respuesta de la tienda': 'Cómo se reparten los tiempos de respuesta de todo el equipo.',
  'salud de la lectura automatica': 'Registro de las lecturas automáticas de Vambe. Todas deberían estar en ✅.',
}
const AYUDA = {}
Object.entries(AYUDA_RAW).forEach(([k, v]) => { const [t, r] = k.includes(':') ? [k.split(':')[0] + ':', k.slice(k.indexOf(':') + 1)] : ['', k]; AYUDA[t + normAy(r)] = v })
const PREF_AY = Object.keys(AYUDA).filter(k => k.startsWith('b:')).map(k => k.slice(2)).sort((a, b) => b.length - a.length)
const AYUDA_EXTRA = {
  // indicadores y títulos
  '1ª respuesta · mediana': 'Tiempo típico que demora un vendedor en responder a un cliente que espera, en minutos hábiles.',
  'Sin contactar +24 h': 'Cotizaciones que pasaron el primer día sin ningún contacto. Es la alerta más urgente.',
  'Para cierre (21+ días)': 'Cotizaciones con 21+ días sin decisión: revísalas y ciérralas si ya no van.',
  'Acumulado mes': 'Venta sumada desde el día 1 del mes hasta la fecha.',
  'Venta acumulada': 'Venta sumada desde el día 1 del mes hasta la fecha.',
  'Venta diaria': 'Venta de cada día según BSALE.', 'Venta día': 'Venta del día según BSALE.',
  'Venta por vendedor (BSALE al cierre)': 'Venta de cada vendedor en el día, según BSALE al momento del cierre.',
  'Top del día': 'Los vendedores con más venta del día.',
  'Carga y embudo de Vambe': 'Cuántas conversaciones tiene cada vendedor y en qué etapa están.',
  'Conversaciones de WhatsApp · hoy': 'Resumen de la atención por WhatsApp de hoy: espera y velocidad de respuesta.',
  'Pipeline de cotizaciones · 90 días': 'Cotizaciones de los últimos 90 días por etapa, con su monto.',
  'Equipo hoy': 'Cómo va cada vendedor hoy en WhatsApp: clientes esperando, mensajes y velocidad.',
  'Vendedores con más riesgo': 'Vendedores con más dinero en cotizaciones con alerta.',
  'Por vendedor': 'Comparación entre los vendedores de la tienda.',
  'Por vendedor · carga y asignaciones': 'Conversaciones a cargo de cada vendedor y cuántas recibió.',
  'Leads nuevos por día': 'Contactos nuevos que escribieron cada día por WhatsApp.',
  'Gestión de cotizaciones del mes': 'Cotizaciones del mes gestionadas y convertidas por el equipo.',
  'Oportunidades de conversión perdidas': 'Cotizaciones que se perdieron o se dejaron morir: monto y motivos.',
  'Resumen del mes': 'Balance del mes para el reporte a gerencia.',
  'Rendimiento': 'Resultados comparados del período.',
  'SLA de contacto 24/72/7:': 'Regla de seguimiento: contactar antes de 24 h, 2º contacto a las 72 h y decidir al día 7.',
  'SLA 24h': 'Porcentaje de cotizaciones contactadas dentro de 24 horas.',
  'Tasa conversión': 'Cotizaciones convertidas ÷ cotizaciones gestionadas.',
  'Mi ticket promedio': 'Tu venta promedio por documento.',
  'Meta del día comunicada': 'Checklist: el equipo conoce la meta del día.',
  'Presentes y compromisos': 'Quiénes están en tienda hoy y cuánto se compromete a vender cada uno.',
  'Checklist de apertura': 'Revisión obligatoria al abrir la tienda. Márcalo en orden.',
  'Checklist de cierre': 'Revisión obligatoria al cerrar la tienda.',
  'Apertura': 'Registro de inicio del día de la tienda.', 'Cierre': 'Registro de término del día de la tienda.',
  'Bitácora': 'Apertura y cierre diario de la tienda.',
  'Día 7': 'Cotizaciones que llegaron al 7º día sin decisión: conviértelas, agenda un paso concreto o márcalas perdidas.',
  'Vencidas': 'Cotizaciones con el próximo contacto atrasado.',
  'Contactos atendidos': 'Clientes distintos atendidos.', 'Mensajes enviados': 'Mensajes enviados por el vendedor.',
  'Msgs / contacto': 'Promedio de mensajes por cliente atendido.',
  'Monto cotizado': 'Suma de los montos de las cotizaciones.', 'Monto convertido': 'Suma de las cotizaciones que se hicieron venta.',
  'Monto conv.': 'Suma de las cotizaciones que se hicieron venta.', 'Monto ganado': 'Suma de las cotizaciones ganadas.',
  'Cot. conv.': 'Cotizaciones convertidas en venta.', 'Cotiz.': 'Cantidad de cotizaciones.',
  'Gestionadas / Convertidas': 'Cotizaciones con gestión registrada y cuántas terminaron en venta.',
  'Ganados': 'Casos marcados como ganados.', 'Perdidos': 'Casos marcados como perdidos.',
  'Tickets': 'Cantidad de documentos de venta.', 'Trans.': 'Transacciones: documentos de venta emitidos.',
  'En pipeline': 'Cotizaciones todavía abiertas.', 'Convertidas': 'Cotizaciones que terminaron en venta.',
  'Contactos': 'Clientes distintos.', 'Total': 'Suma total.', 'Período': 'Rango de fechas del reporte.',
  'Último mensaje': 'Hace cuánto fue el último mensaje de la conversación.',
  // checklist de bitácora
  'BSALE operativo': 'Checklist: el sistema de ventas funciona y emite documentos.',
  'Bodega en orden': 'Checklist: bodega ordenada y despejada.',
  'Briefing realizado': 'Checklist: reunión breve con el equipo para repartir la meta y la cola pendiente.',
  'Caja cuadrada al abrir': 'Checklist: el fondo de caja coincide con lo registrado.',
  'Cierre de caja realizado': 'Checklist: caja cuadrada y cerrada al terminar el día.',
  'Despachos del día completados': 'Checklist: todos los despachos programados salieron.',
  'Despachos pendientes revisados': 'Checklist: se revisaron los despachos pendientes y sus fechas.',
  'Equipo completo y presentable': 'Checklist: todo el equipo presente y con uniforme.',
  'Exhibición en orden': 'Checklist: productos de exhibición limpios, completos y bien ubicados.',
  'Iluminación funcionando': 'Checklist: todas las luces de la tienda operativas.',
  'Material POP en su lugar': 'Checklist: material promocional y precios de ofertas visibles.',
  'Orden de bodega verificado': 'Checklist: bodega ordenada al cierre.',
  'Precios visibles y correctos': 'Checklist: todos los productos exhibidos tienen su precio correcto.',
  'Recepción de mercadería revisada': 'Checklist: la mercadería recibida se revisó contra la guía.',
  'Sin productos dañados a la vista': 'Checklist: no hay productos dañados en exhibición.',
  'Tienda ordenada': 'Checklist: tienda limpia y ordenada al cierre.',
  'Vambe/WhatsApp operativo': 'Checklist: WhatsApp funcionando y sin chats pendientes al abrir.',
  '¿Alertas operacionales?': 'Marca si hubo algo que gerencia deba saber y descríbelo.',
  '¿Hubo incidencias?': 'Marca si hubo algún problema en el día y descríbelo.',
  '¿Hubo quiebres de stock?': 'Marca si faltó algún producto que los clientes pidieron. Esta información va a compras.',
  '¿Se requiere reposición?': 'Marca si hay productos que reponer en exhibición.',
  // reportes
  'Acciones correctivas': 'Qué vas a cambiar para corregir lo que no funcionó.',
  'Compromisos para la próxima semana': 'Metas concretas que asume el equipo para la semana siguiente.',
  'Evaluación del equipo': 'Tu lectura del desempeño del equipo en el período.',
  'Observaciones de la semana': 'Lo más relevante de la semana, en tus palabras.',
  'Logros': 'Lo que salió bien en el período.', 'Problemas': 'Lo que no salió bien y su causa.',
  'Plan para el próximo mes': 'Acciones y focos para el mes siguiente.',
  'Solicitudes a gerencia': 'Lo que necesitas de gerencia para mejorar los resultados.',
  // despachos, incidencias, configuración
  'Despacho a domicilio': 'El pedido se envía a la dirección del cliente.', 'Retiro en tienda': 'El cliente retira el pedido en la tienda.',
  'Entrega': 'Cómo se entrega el pedido.', 'Emitida': 'Documento ya emitido en BSALE.', 'En curso': 'En proceso.',
  'Pagado': 'El cliente ya pagó.', 'Por pagar': 'Pendiente de pago.', 'Folio': 'Número del documento en BSALE.',
  'Boleta': 'Documento de venta a persona.', 'Factura': 'Documento de venta a empresa.',
  'Cotización': 'Número de la cotización en BSALE.', 'Gravedad': 'Qué tan grave es: define la prioridad de atención.',
  'Reportó': 'Quién registró la incidencia.', 'Título': 'Descripción breve.', 'Tipo': 'Categoría.', 'Autor': 'Quién lo registró.',
  'Rol': 'Función en la tienda: vendedor, coordinador o jefe.', 'Nombre': 'Nombre de la persona.',
  'ID BSALE': 'Identificador del vendedor en BSALE: une sus ventas y cotizaciones con su ficha.',
  'ID interno': 'Código de la sucursal en el ERP.', 'Office ID BSALE': 'Número de la sucursal en BSALE: sin él no se puede leer su venta.',
  'Activa': 'Si la sucursal participa en el módulo comercial.', 'Día': 'Fecha.',
  'Apps': 'Vuelve al menú de aplicaciones del ERP.',
  'Meta ≤': 'Meta de velocidad: responder dentro de esta cantidad de minutos hábiles.',
}
Object.entries(AYUDA_EXTRA).forEach(([k, v]) => { const n = normAy(k); if (!AYUDA[n]) AYUDA[n] = v; if (!AYUDA['b:' + n]) AYUDA['b:' + n] = v; if (!AYUDA['th:' + n]) AYUDA['th:' + n] = v })
PREF_AY.splice(0, PREF_AY.length, ...Object.keys(AYUDA).filter(k => k.startsWith('b:')).map(k => k.slice(2)).sort((a, b) => b.length - a.length))
function buscarAyuda(tipo, texto) {
  const n = normAy(texto); if (!n || n.length < 2) return null
  const sd = n.replace(/\d+/g, ' ').replace(/\s+/g, ' ').trim()
  if (tipo === 'b') {
    if (AYUDA['b:' + n]) return AYUDA['b:' + n]
    if (AYUDA['b:' + sd]) return AYUDA['b:' + sd]
    const p = PREF_AY.find(k => sd.startsWith(k + ' ') || sd === k || (k.length > 6 && sd.includes(k))); if (p) return AYUDA['b:' + p]
  }
  if (tipo === 'th') { if (AYUDA['th:' + n]) return AYUDA['th:' + n]; if (AYUDA['th:' + sd]) return AYUDA['th:' + sd] }
  return AYUDA[n] || AYUDA[sd] || null
}

function AyudaGlobal() {
  const [tip, setTip] = useState(null)
  useEffect(() => {
    if (typeof document === 'undefined' || typeof MutationObserver === 'undefined') return
    const marcar = raiz => {
      if (!raiz || !raiz.querySelectorAll) return
      raiz.querySelectorAll('button, th, a, div, span, label').forEach(el => {
        if (el.closest('[data-sin-ayuda]')) return
        const tag = el.tagName
        let txt
        if (tag === 'BUTTON' || tag === 'A' || tag === 'TH') txt = el.textContent
        else { if (el.children.length) return; txt = el.textContent }
        if (!txt || txt.length > 70) return
        const t = buscarAyuda(tag === 'BUTTON' || tag === 'A' ? 'b' : tag === 'TH' ? 'th' : 'x', txt)
        if (t) { if (el.getAttribute('data-ayuda') !== t) el.setAttribute('data-ayuda', t) }
        else if (el.hasAttribute('data-ayuda') && !el.hasAttribute('data-ayuda-fija')) el.removeAttribute('data-ayuda')
      })
    }
    let pend = false
    const obs = new MutationObserver(() => { if (pend) return; pend = true; setTimeout(() => { pend = false; marcar(document.body) }, 350) })
    marcar(document.body)
    obs.observe(document.body, { childList: true, subtree: true, characterData: true })
    let timer = null
    const mostrar = el => { const r = el.getBoundingClientRect(); setTip({ t: el.getAttribute('data-ayuda'), x: r.left + r.width / 2, y: r.bottom, top: r.top }) }
    const over = e => {
      const el = e.target.closest && e.target.closest('[data-ayuda]'); clearTimeout(timer)
      if (!el) { setTip(null); return }
      timer = setTimeout(() => mostrar(el), 280)
    }
    const tocar = e => {
      const el = e.target.closest && e.target.closest('[data-ayuda]')
      if (el && !el.closest('button, a, select, input')) { mostrar(el); clearTimeout(timer); timer = setTimeout(() => setTip(null), 4500) }
    }
    const ocultar = () => { clearTimeout(timer); setTip(null) }
    document.addEventListener('mouseover', over); document.addEventListener('touchstart', tocar, { passive: true })
    window.addEventListener('scroll', ocultar, true)
    return () => { obs.disconnect(); document.removeEventListener('mouseover', over); document.removeEventListener('touchstart', tocar); window.removeEventListener('scroll', ocultar, true); clearTimeout(timer) }
  }, [])
  if (!tip) return null
  const W = 300, vw = typeof window !== 'undefined' ? window.innerWidth : 1200, vh = typeof window !== 'undefined' ? window.innerHeight : 800
  const left = Math.max(8, Math.min(vw - W - 8, tip.x - W / 2))
  const arriba = tip.y + 90 > vh
  return (
    <div role="tooltip" style={{ position: 'fixed', left, top: arriba ? undefined : tip.y + 8, bottom: arriba ? vh - tip.top + 8 : undefined, width: W, zIndex: 9999,
      background: '#16213E', color: '#fff', borderRadius: 10, padding: '10px 13px', fontSize: 12.5, lineHeight: 1.45, boxShadow: '0 8px 24px rgba(22,33,62,.28)', pointerEvents: 'none' }}>
      <span style={{ fontWeight: 800, color: '#9FD7B0', marginRight: 4 }}>ⓘ</span>{tip.t}
    </div>
  )
}

const GUIAS = {
  midia: { t: 'Mi Día · tu pantalla de trabajo',
    vendedor: ['Arriba ves tu venta de hoy contra el compromiso que te fijó tu jefe, y tu venta del mes.',
      '"Mis chats de WhatsApp" muestra los clientes que esperan tu respuesta: el cronómetro se pone rojo cuando pasas la meta. Toca "Responder" para ir directo al chat.',
      '"Por contactar" son cotizaciones nuevas: contáctalas antes de 24 horas. "Mi agenda" son los clientes que prometiste llamar hoy.',
      'Meta del día: dejar el contador en "✓ Al día" antes de irte.'],
    jefe: ['Arriba eliges ver la tienda completa o a un vendedor en particular.',
      '"Chats de WhatsApp de la tienda" muestra la cola de clientes esperando con su vendedor asignado, y la tabla "Equipo hoy" con la velocidad de cada uno.',
      '"Radar de la tienda" resume las cotizaciones en riesgo y lo que se escaló a ti.',
      'Úsala en el briefing de la mañana para repartir la cola pendiente.'] },
  dashboard: { t: 'Dashboard · la foto completa de la gestión',
    jefe: ['Fila superior: venta del mes contra la meta. Verde = sobre la meta, naranja = cerca, rojo = lejos.',
      '"Análisis de cumplimiento" compara lo vendido con lo que se debería llevar hoy y proyecta el cierre del mes.',
      '"Gestión comercial en vivo" muestra las alertas que necesitan acción ahora. Toca una alerta para ir a resolverla.',
      'El "Scorecard" compara a cada vendedor en venta, pipeline, conversión y atención de WhatsApp. Pasa el mouse sobre cualquier título o columna para ver su explicación.'] },
  metas: { t: 'Metas de venta · ¿llegamos a la meta?',
    vendedor: ['Tu venta de hoy y del mes, y tu cumplimiento contra tus compromisos.', 'La tabla del equipo te muestra dónde estás en el ranking de la tienda.'],
    jefe: ['"Ritmo del mes": la barra verde es lo vendido; la marca oscura es dónde deberían ir hoy; la zona clara es la proyección de cierre.',
      'Si la barra no alcanza la marca, la tienda va atrasada: mira el "Ritmo requerido / día" para saber cuánto vender cada día que queda.',
      'Cada tienda tiene su panel con venta diaria y su gráfico: las barras verdes son días sobre la meta y las rojas, días bajo el 80 %.',
      'Si falta algún día, usa "Recalcular mes (BSALE)".'] },
  cotizaciones: { t: 'Cotizaciones · el pipeline de ventas',
    vendedor: ['Cada cotización de BSALE aparece aquí. Tócala para registrar la gestión: en qué quedó el cliente, el próximo paso y una nota.',
      'La barra de etapas de arriba muestra cuántas tienes en cada estado. Tócala para filtrar.',
      'La columna "Alerta" te avisa qué necesita atención: sin contactar, seguimiento vencido o estancada.',
      '"Mi radar" ordena tus cotizaciones abiertas por valor y urgencia: parte por la de arriba.'],
    jefe: ['La barra de etapas muestra el pipeline de la tienda: cantidad y monto por estado. Tócala para filtrar.',
      'La columna "Alerta" indica la salud de cada cotización; "escalada" significa que ya pasó a tu responsabilidad.',
      '"Radar" muestra el pipeline en riesgo, la antigüedad por etapa, el ranking por vendedor y el cierre asistido de las cotizaciones de 21+ días.',
      'Regla de oro: toda gestión registra un resultado. "Sin contactar" es solo el estado inicial.'] },
  despachos: { t: 'Despachos · pedidos por entregar',
    vendedor: ['Pedidos de tus clientes que están por despacharse o retirarse, con el estado del picking en bodega.'],
    jefe: ['Pedidos de la tienda por despachar o retirar. Revisa los atrasados y coordina con bodega.'] },
  vendedores: { t: 'Vendedores · resultados por persona',
    jefe: ['Venta, tickets, conversión y actividad de WhatsApp de cada vendedor. Pasa el mouse sobre las columnas para ver cómo se calcula cada una.'],
    vendedor: ['Resultados del equipo de tu tienda. Úsalo para ver en qué posición vas.'] },
  turnos: { t: 'Turnos · parrilla del mes',
    jefe: ['Asigna el turno de cada vendedor por semana. Los cambios se guardan al instante y cada vendedor los ve en su pantalla.'],
    vendedor: ['Tu turno programado para cada semana del mes, destacado con ⭐. Lo define tu jefe de tienda.'] },
  bitacora: { t: 'Bitácora · apertura y cierre del día',
    jefe: ['Apertura: checklist de la tienda, meta del día y compromiso de venta de cada vendedor.',
      'Cierre: trae la venta desde BSALE, compara cada vendedor con su compromiso y registra incidencias o quiebres de stock.',
      'En "Delegados" autorizas a otra persona a hacer la apertura o el cierre cuando no estés.'] },
  incidencias: { t: 'Incidencias · problemas de la tienda',
    jefe: ['Registra cualquier problema (caja, infraestructura, stock, personal) con su gravedad. Queda el seguimiento hasta cerrarlo.'] },
  reportes: { t: 'Reportes · informes para gerencia',
    jefe: ['"Diario": cumplimiento del día con acumulado y top vendedores, listo para copiar o descargar en PDF.',
      '"Semanal" y "Mensual": las métricas se calculan solas; tú escribes la lectura, los compromisos y las solicitudes a gerencia.'] },
  vambe: { t: 'Vambe · centro de conversaciones de WhatsApp',
    jefe: ['Indicadores de arriba: clientes esperando, espera más larga, tiempo típico de respuesta y porcentaje de respuestas a tiempo.',
      '"Rendimiento por vendedor": tócalo para ver la cola de ese vendedor. La barra de colores muestra qué tan rápido responde (verde = rápido, rojo = lento).',
      '"Cola en vivo": clientes esperando, ordenados por tiempo de espera. "Responder" abre el chat en Vambe.',
      'Los tiempos cuentan en horario hábil (L-V 09:30–18:00, Sáb 09:30–13:30). Se actualiza cada minuto.'] },
  gestion: { t: 'Gestión · tareas y proyectos del área',
    jefe: ['Tareas asignadas a ti y a tu equipo, con responsable, plazo y estado.', 'Las incidencias derivadas desde la pestaña Incidencias llegan aquí como tareas.'],
    vendedor: ['Tus tareas asignadas, con su plazo. Actualiza el estado a medida que avanzas.'] },
  validaciones: { t: 'Validar pagos · confirmación de transferencias y Webpay',
    vendedor: ['Cuando un cliente paga por transferencia o link Webpay, levanta aquí la solicitud con el comprobante y su nota de venta o cotización.',
      'Tesorería la revisa desde Finanzas y el estado cambia en vivo: Esperando a Finanzas → Finanzas revisando → Aprobada o Rechazada.',
      'Emite el documento solo cuando diga "Aprobada" (fondos confirmados).'],
    jefe: ['Solicitudes de validación de pago de tu tienda, con su estado en vivo y el tiempo que llevan esperando.',
      'Tesorería las resuelve desde Finanzas; el documento se emite solo con la solicitud "Aprobada".', 'Gerencia ve las solicitudes de todas las sucursales.'] },
  config: { t: 'Configuración', jefe: ['Metas mensuales por tienda, sucursales conectadas a BSALE y vendedores. Los cambios afectan a todo el módulo.'] },
}
function GuiaPestana({ tab, rol, vista }) {
  // los hooks van SIEMPRE antes de cualquier return (si no, React se rompe al cambiar de pestaña)
  const g = GUIAS[tab]
  const clave = `com_guia_oculta_${tab}${tab === 'cotizaciones' ? '_' + vista : ''}`
  const [oculta, setOculta] = useState(() => { try { return localStorage.getItem(clave) === '1' } catch (e) { return false } })
  useEffect(() => { try { setOculta(localStorage.getItem(clave) === '1') } catch (e) { setOculta(false) } }, [clave])
  if (!g) return null
  const esVend = rol === 'vendedor'
  let puntos = (esVend ? g.vendedor : g.jefe) || g.jefe || g.vendedor || []
  let titulo = g.t
  if (tab === 'cotizaciones' && vista === 'radar') { titulo = 'Radar · salud del pipeline'; puntos = esVend
    ? ['Tus cotizaciones abiertas ordenadas por valor y urgencia. Las de arriba son las que más importan hoy.', 'Filtra por tipo de alerta con los recuadros de colores.']
    : ['Indicadores de arriba: pipeline abierto, pronóstico ponderado, lo que está en riesgo y la conversión real.', 'Matriz de antigüedad: lo sano está a la izquierda; lo que se acumula a la derecha se está enfriando.', 'Cierre asistido: marca las cotizaciones de 21+ días que ya no van y ciérralas juntas con su motivo.'] }
  const set = v => { setOculta(v); try { localStorage.setItem(clave, v ? '1' : '0') } catch (e) { /* sin almacenamiento */ } }
  if (oculta) return (
    <div data-sin-ayuda style={{ marginBottom: 10 }}>
      <button onClick={() => set(false)} style={{ background: '#eef3ff', color: '#16213E', border: '1px solid #d9e2fb', borderRadius: 20, padding: '5px 12px', fontSize: 12, fontWeight: 800, cursor: 'pointer' }}>❔ Cómo usar esta pantalla</button>
    </div>
  )
  return (
    <div data-sin-ayuda style={{ background: 'linear-gradient(135deg,#f3f6ff,#f7f5ff)', border: '1px solid #dfe4fb', borderLeft: '5px solid #16213E', borderRadius: 12, padding: '12px 16px', marginBottom: 14 }}>
      <div style={{ display: 'flex', alignItems: 'center', gap: 10, marginBottom: 6 }}>
        <span style={{ fontSize: 15 }}>🧭</span>
        <div style={{ fontSize: 14.5, fontWeight: 800, color: '#16213E' }}>{titulo}</div>
        <button onClick={() => set(true)} style={{ marginLeft: 'auto', background: 'transparent', color: '#6E6E73', border: 'none', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Ocultar guía ✕</button>
      </div>
      <ul style={{ margin: 0, paddingLeft: 20, display: 'grid', gridTemplateColumns: puntos.length > 2 ? 'repeat(auto-fit,minmax(320px,1fr))' : '1fr', gap: '4px 28px' }}>
        {puntos.map((p, i) => <li key={i} style={{ fontSize: 13, color: '#3a3a3c', lineHeight: 1.5 }}>{p}</li>)}
      </ul>
      <div style={{ fontSize: 11.5, color: '#6E6E73', marginTop: 6 }}>💡 Pasa el mouse sobre cualquier botón, título o columna para ver qué significa.</div>
    </div>
  )
}

function ComoLeer({ texto, conclusion, tono = '#16213E' }) {
  return (
    <div data-sin-ayuda style={{ marginTop: 10, background: '#f7f8fb', borderRadius: 9, padding: '8px 11px', fontSize: 12, lineHeight: 1.5, color: '#3a3a3c' }}>
      <span style={{ fontWeight: 800, color: '#16213E' }}>📖 Cómo leerlo: </span>{texto}
      {conclusion && <div style={{ marginTop: 4, fontWeight: 700, color: tono }}>→ {conclusion}</div>}
    </div>
  )
}

/* Gráfico de venta diaria con línea de meta, colores por cumplimiento y conclusión automática */
function GraficoVentaDiaria({ serie, alto = 150, feriados = [], titulo }) {
  const esHabil = f => { const d = new Date(f + 'T12:00:00'); return d.getDay() !== 0 && !(feriados || []).find(x => x.fecha === f) }
  const pts = (serie || []).map(p => ({ ...p, habil: esHabil(p.fecha), metaD: esHabil(p.fecha) ? Number(p.meta || 0) : 0 }))
  if (!pts.length) return <div style={{ color: '#8b88a8', fontSize: 13, padding: 20, textAlign: 'center' }}>Aún no hay ventas registradas en el período.</div>
  const maxY = Math.max(1, ...pts.map(p => Math.max(p.venta, p.metaD))) * 1.12
  const hab = pts.filter(p => p.habil && p.metaD > 0)
  const sobre = hab.filter(p => p.venta >= p.metaD).length
  const mejor = pts.reduce((a, b) => b.venta > a.venta ? b : a, pts[0])
  const bajoH = hab.length ? hab.reduce((a, b) => b.venta < a.venta ? b : a, hab[0]) : null
  const prom = hab.length ? hab.reduce((s, p) => s + p.venta, 0) / hab.length : 0
  const metaProm = hab.length ? hab.reduce((s, p) => s + p.metaD, 0) / hab.length : 0
  const col = p => !p.habil || !p.metaD ? '#C7C7CC' : p.venta >= p.metaD ? '#1E7A44' : p.venta >= p.metaD * 0.8 ? '#FF9F0A' : '#D92D20'
  return (
    <div>
      {titulo}
      <div style={{ position: 'relative', height: alto, display: 'flex', alignItems: 'flex-end', gap: 3, paddingBottom: 18 }}>
        {pts.map(p => (
          <div key={p.fecha} title={`${fmtFecha(p.fecha)} · venta ${fmt(p.venta)}${p.metaD ? ` · meta ${fmtK(p.metaD)} · ${pct(p.venta, p.metaD)}%` : ' · día no hábil'}`}
            style={{ flex: '1 0 8px', minWidth: 8, height: '100%', position: 'relative', display: 'flex', alignItems: 'flex-end', justifyContent: 'center' }}>
            <div style={{ width: '78%', height: `${(p.venta / maxY) * 100}%`, background: col(p), borderRadius: '3px 3px 0 0', minHeight: p.venta ? 2 : 0 }} />
            {p.metaD > 0 && <div style={{ position: 'absolute', left: 0, right: 0, bottom: `${(p.metaD / maxY) * 100}%`, borderTop: '2px dashed #16213E', opacity: .55 }} />}
            <div style={{ position: 'absolute', bottom: -17, fontSize: 9.5, color: p.habil ? '#8b88a8' : '#C7C7CC', fontWeight: 600 }}>{p.fecha.slice(8)}</div>
          </div>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', fontSize: 11.5, color: '#6E6E73', marginTop: 4 }}>
        {[['#1E7A44', 'Sobre la meta'], ['#FF9F0A', 'Entre 80 % y 100 %'], ['#D92D20', 'Bajo 80 %'], ['#C7C7CC', 'Domingo o feriado']].map(([c2, l]) => (
          <span key={l} style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}><span style={{ width: 10, height: 10, borderRadius: 2, background: c2 }} />{l}</span>
        ))}
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: 5 }}><span style={{ width: 16, borderTop: '2px dashed #16213E', opacity: .6 }} />Meta diaria</span>
      </div>
      <ComoLeer texto="cada barra es la venta de un día; la línea punteada es la meta de ese día. Si la barra la supera, el día se cumplió."
        conclusion={hab.length ? `${sobre} de ${hab.length} días hábiles sobre la meta · promedio ${fmtK(prom)} por día vs meta ${fmtK(metaProm)} · mejor día ${fmtFecha(mejor.fecha)} (${fmtK(mejor.venta)})${bajoH ? ` · más bajo ${fmtFecha(bajoH.fecha)} (${fmtK(bajoH.venta)})` : ''}` : null}
        tono={hab.length && sobre / hab.length >= 0.5 ? '#1E7A44' : '#B25E09'} />
    </div>
  )
}

/* ═══ INCIDENCIA → GESTIÓN ═══
   Convierte una incidencia en tarea(s) del PMO con responsable y fecha (pmo_tareas con
   origen_tipo='incidencia_com', origen_id=<id incidencia>). La tarea vive en Gestión: aparece en
   Mi semana del responsable, en el correo diario y en la rendición. Aquí se ve su estado.
   No modifica la incidencia: solo agrega una nota a su bitácora para trazabilidad. */
const TAREA_EST_COM = { pendiente: ['#8E8E93', 'Pendiente'], en_curso: ['#639922', 'En curso'], en_revision: ['#185FA5', 'En revisión'], bloqueada: ['#E24B4A', 'Bloqueada'], completada: ['#34C759', 'Completada'] }
function GestionIncidencia({ inc, cu, onLog }) {
  const [tareas, setTareas] = useState([])
  const [usuarios, setUsuarios] = useState([])
  const [abierto, setAbierto] = useState(false)
  const [f, setF] = useState({ titulo: '', resp: '', fecha: '' })
  const [busy, setBusy] = useState(false)
  const [err, setErr] = useState('')
  const cargar = async () => {
    const { data } = await supabase.from('pmo_tareas').select('id,titulo,responsable_id,estado,fecha_vencimiento')
      .eq('origen_tipo', 'incidencia_com').eq('origen_id', inc.id).is('eliminado_en', null).order('created_at')
    setTareas(data || [])
  }
  useEffect(() => {
    cargar()
    supabase.from('usuarios').select('id,nombre,correo').order('nombre').then(({ data }) => setUsuarios(data || []), () => { })
  }, [inc.id])
  const nombre = id => (usuarios.find(u => u.id === id) || {}).nombre || id || '—'
  const hoyL = new Date().toLocaleDateString('en-CA')
  const abrir = () => { setF({ titulo: `${inc.id} · ${inc.titulo}`.slice(0, 160), resp: '', fecha: '' }); setErr(''); setAbierto(true) }
  const crear = async () => {
    if (!f.titulo.trim() || !f.resp || !f.fecha) { setErr('Título, responsable y fecha son obligatorios: sin dueño y plazo no hay seguimiento.'); return }
    if (!cu?.id) { setErr('Sesión sin usuario del ERP.'); return }
    setBusy(true); setErr('')
    try {
      const id = 'TSK-' + Date.now().toString(36) + Math.random().toString(36).slice(2, 6)
      const desc = [`Derivada desde incidencia ${inc.id} (${shortKey(inc.sucursal_id).toUpperCase()} · ${inc.tipo} · gravedad ${inc.gravedad})`,
        inc.descripcion ? 'Descripción: ' + inc.descripcion : '', inc.accion_tomada ? 'Acción ya tomada: ' + inc.accion_tomada : ''].filter(Boolean).join('\n')
      const { error } = await supabase.from('pmo_tareas').insert({
        id, titulo: f.titulo.trim(), descripcion: desc, responsable_id: f.resp, asignado_por_id: cu.id, created_by: cu.id,
        fecha_inicio: hoyL, fecha_vencimiento: f.fecha, estado: 'pendiente', prioridad: inc.gravedad === 'alta' ? 'alta' : 'media',
        origen_tipo: 'incidencia_com', origen_id: inc.id
      })
      if (error) throw error
      const u = usuarios.find(x => x.id === f.resp)
      const nota = `Derivada a Gestión: tarea ${id} → ${u?.nombre || f.resp}, vence ${f.fecha}.`
      const { data: lg } = await supabase.from('com_incidencias_log').insert({ incidencia_id: inc.id, estado: inc.estado, nota, user_name: cu?.nombre || cu?.correo || '' }).select().maybeSingle()
      if (lg && onLog) onLog(lg)
      if (u?.correo?.includes('@') && f.resp !== cu.id) {
        try {
          const ahora = new Date()
          await supabase.from('notificaciones').insert({
            id: 'id' + Date.now().toString(36) + Math.random().toString(36).slice(2, 5), tipo: 'Email', destino_correo: u.correo, destino_nombre: u.nombre || u.correo,
            asunto: 'Nueva tarea desde incidencia ' + inc.id,
            mensaje: 'Se te asigno una tarea derivada de una incidencia comercial.\n\nTarea: ' + f.titulo.trim() + '\nIncidencia: ' + inc.id + ' - ' + inc.titulo + '\nVence: ' + f.fecha + '\nAsignada por: ' + (cu.nombre || cu.id) + '\n\nLa encuentras en la pestaña Gestión de tu app.\n\n- ERP Outlet de Puertas SpA',
            estado: 'Pendiente', usuario: cu.nombre || cu.id, rol: cu.rol || null, fecha: hoyL, hora: ahora.toTimeString().slice(0, 5)
          })
        } catch (e) { /* el correo nunca bloquea */ }
      }
      setAbierto(false); await cargar()
    } catch (e) { setErr(e.message || String(e)) } finally { setBusy(false) }
  }
  return (
    <div style={{ margin: '12px 0 4px', padding: 10, border: '1px solid #e7e5f2', borderRadius: 10, background: '#fcfcff' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', gap: 8 }}>
        <div style={{ fontSize: 11, textTransform: 'uppercase', letterSpacing: '.04em', color: '#8b88a8', fontWeight: 800 }}>🎯 Plan de acción en Gestión {tareas.length ? `(${tareas.filter(t => t.estado === 'completada').length}/${tareas.length})` : ''}</div>
        {inc.estado !== 'cerrada' && !abierto && <button onClick={abrir} style={{ background: '#16213E', color: '#fff', border: 'none', borderRadius: 8, padding: '5px 11px', fontSize: 11.5, fontWeight: 700, cursor: 'pointer' }}>+ Derivar tarea</button>}
      </div>
      {!tareas.length && !abierto && <div style={{ fontSize: 12, color: '#8b88a8', marginTop: 6 }}>Sin tareas derivadas. Si la incidencia requiere acción de alguien con plazo, derívala: queda en su Gestión con seguimiento y correo diario.</div>}
      {tareas.map(t => { const [c, l] = TAREA_EST_COM[t.estado] || TAREA_EST_COM.pendiente; const venc = t.estado !== 'completada' && t.fecha_vencimiento && t.fecha_vencimiento < hoyL
        return (
          <div key={t.id} style={{ display: 'flex', alignItems: 'center', gap: 8, fontSize: 12, marginTop: 6 }}>
            <Dot c={c} />
            <div style={{ flex: 1, minWidth: 0 }}><strong>{t.titulo}</strong> <span style={{ color: '#8b88a8' }}>· {nombre(t.responsable_id)} · vence {fmtFecha(t.fecha_vencimiento)}</span></div>
            <span style={{ fontSize: 11, fontWeight: 800, color: venc ? '#FF3B30' : c, whiteSpace: 'nowrap' }}>{venc ? '⚠ Vencida' : l}</span>
          </div>
        ) })}
      {abierto && (
        <div style={{ marginTop: 8 }}>
          <input className="com-inp" value={f.titulo} onChange={e => setF(x => ({ ...x, titulo: e.target.value }))} placeholder="Qué hay que hacer" style={{ marginBottom: 6 }} />
          <div style={{ display: 'grid', gridTemplateColumns: '2fr 1fr', gap: 6, marginBottom: 6 }}>
            <select className="com-inp" value={f.resp} onChange={e => setF(x => ({ ...x, resp: e.target.value }))}>
              <option value="">Responsable…</option>
              {usuarios.map(u => <option key={u.id} value={u.id}>{u.nombre || u.correo}</option>)}
            </select>
            <input className="com-inp" type="date" min={hoyL} value={f.fecha} onChange={e => setF(x => ({ ...x, fecha: e.target.value }))} />
          </div>
          {err && <div style={{ fontSize: 12, color: '#FF3B30', marginBottom: 6 }}>{err}</div>}
          <div style={{ display: 'flex', justifyContent: 'flex-end', gap: 6 }}>
            <button onClick={() => setAbierto(false)} style={{ background: '#f0eff7', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>Cancelar</button>
            <button onClick={crear} disabled={busy} style={{ background: busy ? '#c9c7dd' : '#16213E', color: '#fff', border: 'none', borderRadius: 8, padding: '6px 12px', fontSize: 12, fontWeight: 700, cursor: 'pointer' }}>{busy ? 'Creando…' : 'Crear tarea en Gestión'}</button>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <div onClick={onClose} style={{ position: 'fixed', inset: 0, background: 'rgba(20,20,40,.45)', zIndex: 50, display: 'flex', alignItems: isMobile ? 'flex-end' : 'center', justifyContent: 'center' }}>
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: isMobile ? '16px 16px 0 0' : 16, width: isMobile ? '100%' : 620, maxHeight: '88vh', overflowY: 'auto', padding: 18 }}>
        ...
        <GestionIncidencia inc={inc} cu={cu} onLog={nuevo => setLog(l => [...l, nuevo])} />
