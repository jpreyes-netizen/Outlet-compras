...
export function ComercialApp({ cu, setAppActual }) {
  ...
  useEffect(() => {
    if (perfilNav.rol === 'vendedor' && ['dashboard', 'reportes', 'incidencias'].includes(tab)) setTab('midia')
        ...
        <table className="com">
          <thead>
            <tr>
              ...
              <th style={{ textAlign: 'right' }}>Monto</th><th>Estado</th><th>Próx. / Despacho</th><th>Días</th><th style={{ textAlign: 'center' }}></th>
            </tr>
          </thead>
          <tbody>
            {filtradas.length === 0 ? (
              <tr><td colSpan={10} style={{ textAlign: 'center', padding: 40, color: '#8b88a8' }}>{loading ? 'Cargando…' : 'Sin cotizaciones para el filtro actual.'}</td></tr>
            ) : filtradas.map(r => {
      ...
      <div onClick={e => e.stopPropagation()} style={{ background: '#fff', borderRadius: '18px 18px 0 0', padding: '10px 20px 28px', width: '100%', maxWidth: 560, maxHeight: '90vh', overflow: 'auto' }}>
        ...
        <div style={{ marginBottom: 12 }}>
          ...
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: 6 }}>
            {Object.entries(ESTADOS).filter(([k]) => k !== 'descartada' || puedeDescartar || estado === 'descartada').map(([k, v]) => (
        ...
        <div style={{ display: 'flex', gap: 8, justifyContent: 'flex-end' }}>
          ...
          <button onClick={guardar} disabled={saving || !notaOk}
