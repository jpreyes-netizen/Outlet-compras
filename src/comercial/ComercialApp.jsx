    ...
    try {
      for (let i = 0; i < 40; i++) {
        ...
        for (let intento = 0; intento < 2; intento++) {
          ;({ data, error } = await supabase.functions.invoke('vambe-mensajes-sync', {
        ...
        if (error) {
          ...
          if (msg.includes('Failed to send a request')) throw new Error("No se pudo contactar la edge function 'vambe-mensajes-sync'. Verifica en Supabase → Edge Functions que exista y esté desplegada (redespliega el archivo vambe-mensajes-sync_index.ts si es necesario).")
          throw new Error(msg)
        }
