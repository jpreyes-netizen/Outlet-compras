...
export function ComercialApp({ cu, setAppActual }) {
  ...
  useEffect(() => {
    if (!perfilNav.verTodo && ['dashboard', 'reportes', 'turnos', 'bitacora', 'incidencias'].includes(tab)) setTab('midia')
  ...
  ].filter(tb => perfilNav.verTodo || !['dashboard', 'reportes', 'turnos', 'bitacora', 'incidencias'].includes(tb.k))
      ...
      <div style={{ padding: isMobile ? '12px 12px 40px' : '18px 22px 48px', maxWidth: 1280, margin: '0 auto' }}>
        {loadingBase ? (
          ...
          <>
            ...
            {tab === 'dashboard' && perfilNav.verTodo && <TabDashboard {...{ sucursales, vendedores, metas, seg, cu, esGerente, anio, setAnio, mes, setMes, feriados, isMobile }} />}
            ...
            {tab === 'turnos' && perfilNav.verTodo && <TabTurnos {...{ sucursales, vendedores, sucSel, setSucSel, anio, setAnio, mes, setMes, cu, isMobile }} />}
            {tab === 'bitacora' && perfilNav.verTodo && <TabBitacora {...{ sucursales, vendedores, metas, feriados, cu, sucSel, setSucSel, anio, setAnio, mes, setMes, isMobile }} />}
            {tab === 'reportes' && perfilNav.verTodo && <TabReportes {...{ sucursales, metas, feriados, cu, anio, setAnio, mes, setMes, isMobile }} />}
            {tab === 'incidencias' && perfilNav.verTodo && <TabIncidencias {...{ sucursales, sucSel, cu, isMobile }} />}
...
function resolverPerfil(cu, vendedores, esGerente) {
  ...
  if (yo.rol === 'jefe' || yo.rol === 'gerencia') return { rol: 'jefe', vendedor: yo, sucursal: yo.sucursal_id, verTodo: true }
...
function TabCotizaciones({ sucursales, vendedores, sucSel, setSucSel, seg, setSeg, cu, esGerente, isMobile }) {
  ...
  const rowsV = esVendC && miIdC ? rows.filter(r => String(r.seller?.id) === miIdC) : rows
  ...
  const vendsFiltro = esVendC ? vendedores.filter(v => String(v.bsale_user_id) === miIdC) : (fSuc ? vendedores.filter(v => v.sucursal_id === fSuc) : vendedores)
...
function TabMiDia({ sucursales, vendedores, metas, seg, setSeg, cu, esGerente, isMobile }) {
  ...
  const bloqueado = !perfilMi.verTodo   // solo gestión (gerente/jefe) puede ver a otros
