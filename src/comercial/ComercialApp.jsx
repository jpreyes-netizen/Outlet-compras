      ...
      <div style={{ padding: isMobile ? '12px 12px 40px' : '18px 22px 48px', maxWidth: 1280, margin: '0 auto' }}>
        {loadingBase ? (
          ...
          <>
            ...
            {tab === 'dashboard' && <TabDashboard {...{ sucursales, vendedores, metas, seg, cu, esGerente, anio, setAnio, mes, setMes, feriados, isMobile }} />}
...
function resolverPerfil(cu, vendedores, esGerente) {
  ...
  if (esGerente) return { rol: 'gerente', vendedor: null, sucursal: null }
  ...
  if (!yo) return { rol: 'vendedor', vendedor: null, sucursal: null }
  if (yo.rol === 'jefe' || yo.rol === 'gerencia') return { rol: 'jefe', vendedor: yo, sucursal: yo.sucursal_id }
  return { rol: 'vendedor', vendedor: yo, sucursal: yo.sucursal_id }
}
...
function TabMetas({ sucursales, vendedores, feriados, metas, cu, esGerente, anio, setAnio, mes, setMes, isMobile }) {
  ...
  const activas = useMemo(() => {
    ...
    if (perfil.rol === 'gerente') return base
  ...
  const Controles = (
    <div style={{ display: 'flex', flexWrap: 'wrap', gap: 10, alignItems: 'center', marginBottom: 14 }}>
      ...
      {perfil.rol !== 'gerente' && perfil.sucursal && <span style={{ fontSize: 11.5, fontWeight: 700, color: C2, background: '#f0eff7', borderRadius: 7, padding: '5px 11px' }}>{esVend ? '👤 Solo mis datos' : '🏬 Mi equipo'} · {activas[0]?.nombre || ''}</span>}
    </div>
  )
  ...
  const rankVend = Object.values(porVendMes)
    .filter(v => !esVend)
    .filter(v => perfil.rol === 'gerente' || v.suc === perfil.sucursal)
  ...
  return (
    <div>
      ...
      {perfil.rol === 'gerente' && activas.length > 1 && (
      ...
      <div style={{ ...card, padding: 0, overflowX: 'auto', marginBottom: 16 }}>
        ...
        <table className="com">
          <thead><tr><th style={{ textAlign: 'center' }}>#</th><th>Vendedor</th>{perfil.rol === 'gerente' && <th>Sucursal</th>}<th style={{ textAlign: 'right' }}>Venta mes</th><th style={{ textAlign: 'right' }}>Docs</th><th style={{ textAlign: 'right' }}>Ticket</th><th style={{ textAlign: 'right' }}>Compromiso</th><th>Cumplimiento</th></tr></thead>
          <tbody>
            {rankVend.length === 0 ? <tr><td colSpan={perfil.rol === 'gerente' ? 8 : 7} style={{ textAlign: 'center', padding: 30, color: '#8b88a8' }}>Sin ventas registradas este mes.</td></tr> : rankVend.map((v, i) => (
              <tr key={v.sid}>
                <td style={{ textAlign: 'center', fontWeight: 800, color: i < 3 ? C1 : '#8b88a8' }}>{i + 1}</td>
                <td style={{ fontWeight: 600, whiteSpace: 'nowrap' }}>{v.name}</td>
                {perfil.rol === 'gerente' && <td style={{ fontSize: 11.5, color: '#5a5a6e' }}>{v.suc ? shortKey(v.suc).toUpperCase() : '—'}</td>}
      ...
      {perfil.rol === 'gerente' && sucursales.filter(s => !s.bsale_office_id).map(s => (
...
function TabDashboard({ sucursales, vendedores, metas, seg, cu, esGerente, anio, setAnio, mes, setMes, feriados, isMobile }) {
  ...
  const [fSuc, setFSuc] = useState(perfilD.rol === 'gerente' ? '' : (perfilD.sucursal || ''))
  ...
  return (
    <div>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 14, flexWrap: 'wrap' }}>
        ...
        <select value={fSuc} onChange={e => setFSuc(e.target.value)} style={selStyle} disabled={perfilD.rol !== 'gerente'}>
          {perfilD.rol === 'gerente' && <option value="">Todas las sucursales</option>}
          {sucursales.filter(s => s.activa && (perfilD.rol === 'gerente' || s.sucursal_id === perfilD.sucursal)).map(s => <option key={s.sucursal_id} value={s.sucursal_id}>{s.nombre}</option>)}
        </select>
