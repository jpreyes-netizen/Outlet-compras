...
function TabVambeMonitor({ sucursales, vendedores, cu, esGerente, isMobile, anio, setAnio, mes, setMes }) {
  ...
  const [d, setD] = useState(globalThis.__VM__ || { pipes: [], carga: [], agentes: [], etapas: [], metr: [], log: [] })
  const [loading, setLoading] = useState(false)
  const [err, setErr] = useState('')
  ...
  const cargar = async () => {
    ...
    try {
      ...
      const [pp, cg, ag, et, mt, lg] = await Promise.all([
  ...
  useEffect(() => { cargar(); const t = setInterval(cargar, 5 * 60000); return () => clearInterval(t) /* eslint-disable-next-line */ }, [])
