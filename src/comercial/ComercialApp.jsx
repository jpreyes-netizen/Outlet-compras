    ...
    <div style={{ fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif", background: '#f4f4fb', minHeight: '100vh', fontSize: 14, color: '#1c1c1e' }}>
      ...
      <div style={{ position: 'sticky', top: 0, zIndex: 20, background: 'linear-gradient(135deg,#1a1a2e,#16213e)', color: '#fff', padding: isMobile ? '10px 14px' : '12px 22px', display: 'flex', alignItems: 'center', gap: 14, boxShadow: '0 2px 10px rgba(0,0,0,.15)' }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          {!isMobile && <div style={{ fontSize: 10.5, color: '#9aa0c0' }}>Dashboard · metas · cotizaciones · turnos · bitácora · incidencias</div>}
        </div>
        {!isMobile && (
...
function TabDashboard({ sucursales, metas, seg, anio, setAnio, mes, setMes, feriados, isMobile }) {
  ...
  const porSuc = useMemo(() => activas.map(s => {
    ...
    return { ...s, venta, trans, meta, dias: cs.length, ticket: trans > 0 ? venta / trans : 0 }
  }), [cierres, metas, anio, mes, sucursales])

  const totVenta = porSuc.reduce((a, s) => a + s.venta, 0)
  const totMeta = porSuc.reduce((a, s) => a + s.meta, 0)
  const totTrans = porSuc.reduce((a, s) => a + s.trans, 0)
    ...
    <div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(6,1fr)', gap: 10, marginBottom: 16 }}>
        {[
          ...
          ['Cumplimiento', pct(totVenta, totMeta) + '%', pct(totVenta, totMeta) >= 100 ? '#34C759' : pct(totVenta, totMeta) >= 80 ? '#FF9500' : '#FF3B30'],
        ...
        {porSuc.map(s => (
          <div key={s.sucursal_id} style={{ background: '#fff', borderRadius: 12, padding: 14, boxShadow: '0 1px 3px rgba(0,0,0,.05)' }}>
            <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'baseline', marginBottom: 6 }}>
              ...
              <div style={{ fontSize: 11, color: '#8b88a8' }}>{s.dias} días con cierre</div>
            </div>
            <div style={{ fontSize: 22, fontWeight: 800, color: C2 }}>{fmtK(s.venta)}</div>
            <div style={{ fontSize: 11.5, color: '#8b88a8', marginBottom: 6 }}>meta {fmtK(s.meta)} · ticket {s.ticket ? fmtK(s.ticket) : '—'}</div>
            <Bar v={pct(s.venta, s.meta)} color={pct(s.venta, s.meta) >= 100 ? '#34C759' : C1} />
            <div style={{ fontSize: 11, fontWeight: 700, marginTop: 4, color: pct(s.venta, s.meta) >= 100 ? '#34C759' : C2 }}>{pct(s.venta, s.meta)}% de la meta</div>
          </div>
        ))}
      </div>
