...
function VambeRendimiento({ vendedores, anio, setAnio, mes, setMes, isMobile }) {
  ...
  const porCanal = useMemo(() => porClave('channel'), [filas])
...
function VambeMensajeria({ vendedores, anio, setAnio, mes, setMes, isMobile }) {
  const [filas, setFilas] = useState([])
  const [loading, setLoading] = useState(false)
  const [sync, setSync] = useState(null)        // { offset, total, mensajes } mientras sincroniza
    ...
    try {
      ...
      while (true) {
        const { data, error } = await supabase.from('vambe_mensajes')
          .select('ai_contact_id,direction,user_id,assistant_id,created_at')
  ...
  const stats = useMemo(() => {
    const s = { entrantes: 0, humano: 0, automatico: 0, bot: 0, contactos: new Set() }
    filas.forEach(m => {
      const e = emisor(m)
      if (e === 'cliente') s.entrantes++
      else s[e]++
    ...
    return { ...s, salientes, nContactos: s.contactos.size,
      pctAuto: salientes ? Math.round(((s.automatico + s.bot) / salientes) * 100) : 0 }
  }, [filas])
  ...
  // ── distribución por hora (entrantes, hora local del navegador = Chile) ──
  ...
  const fueraHorario = porHora.filter(x => x.fuera).reduce((a, x) => a + x.ent, 0)
  const pctFuera = stats.entrantes ? Math.round((fueraHorario / stats.entrantes) * 100) : 0

  // ── ranking por vendedor (salientes humanos) ──
  const vmap = useMemo(() => {
    const m = {}
    ...
    <div>
      <div style={{ display: 'flex', gap: 8, alignItems: 'center', marginBottom: 12, flexWrap: 'wrap' }}>
        ...
        <button onClick={sincronizar} disabled={!!sync || loading}
          ...
          {sync ? `Sincronizando… ${sync.offset}${sync.total ? '/' + sync.total : ''} contactos (${fN(sync.mensajes)} msgs)` : '⟳ Sincronizar con Vambe'}
        </button>
        {loading && <span style={{ fontSize: 12, color: '#8b88a8' }}>Cargando…</span>}
        <span style={{ fontSize: 11.5, color: '#8b88a8', marginLeft: 'auto' }}>{fN(filas.length)} mensajes en el período</span>
      </div>
        ...
        <>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr 1fr' : 'repeat(5,1fr)', gap: 10, marginBottom: 16 }}>
            {[
              ...
              ['Automáticos + bot', fN(stats.automatico + stats.bot), '#FF9500'],
              ['% automatización', stats.pctAuto + '%', stats.pctAuto > 50 ? '#FF9500' : '#1c1c1e'],
          ...
          <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.2fr 1fr', gap: 12, marginBottom: 16 }}>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 14, boxShadow: '0 1px 3px rgba(0,0,0,.05)' }}>
              <div style={{ fontWeight: 800, fontSize: 12.5, marginBottom: 10, textTransform: 'uppercase', letterSpacing: '.04em', color: '#8b88a8' }}>
                Mensajes por día <span style={{ fontWeight: 600, textTransform: 'none' }}>· <Dot c={C1} /> entrantes · <Dot c='#c9c7dd' /> salientes</span>
              </div>
            ...
            <div style={{ background: '#fff', borderRadius: 12, padding: 14, boxShadow: '0 1px 3px rgba(0,0,0,.05)' }}>
              <div style={{ fontWeight: 800, fontSize: 12.5, marginBottom: 4, textTransform: 'uppercase', letterSpacing: '.04em', color: '#8b88a8' }}>Entrantes por hora</div>
              <div style={{ fontSize: 11.5, color: pctFuera > 30 ? '#B25000' : '#5a5a6e', fontWeight: 700, marginBottom: 10 }}>
                {pctFuera}% llega fuera de horario (antes de 09 / después de 18) — {fN(fueraHorario)} mensajes
              </div>
              {porHora.map(x => (
          ...
          <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 1px 3px rgba(0,0,0,.05)', overflowX: 'auto' }}>
            <table className="com">
              <thead><tr><th style={{ textAlign: 'center' }}>#</th><th>Vendedor</th><th>Sucursal</th><th style={{ textAlign: 'right' }}>Mensajes enviados</th><th style={{ textAlign: 'right' }}>Contactos atendidos</th><th style={{ textAlign: 'right' }}>Msgs / contacto</th></tr></thead>
              <tbody>
                {ranking.map((r, i) => (
                  <tr key={r.k}>
                    <td style={{ textAlign: 'center', fontWeight: 800, color: i < 3 ? C1 : '#8b88a8' }}>{i + 1}</td>
          ...
          <div style={{ fontSize: 11, color: '#a6a3bd', marginTop: 10 }}>
            "Sincronizar con Vambe" trae las conversaciones actualizadas de todos los contactos (tarda unos minutos). Los mensajes automáticos son las respuestas de fuera de horario; el bot IA se contabiliza aparte cuando esté activo.
          </div>
        </>
      )}
    </div>
