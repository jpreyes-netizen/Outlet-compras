...
export function ComercialApp({ cu, setAppActual }) {
  ...
  useEffect(() => {
    supabase.from('com_delegaciones').select('*').eq('activo', true)
      .then(({ data }) => setDelegs(data || [])).catch(() => {})
  }, [])
  ...
  const miDelegacion = useMemo(() => {
    const nn = normD(cu?.nombre)
    if (!nn) return null
    return delegs.find(d => { const dn = normD(d.nombre); return dn === nn || dn.includes(nn) || nn.includes(dn) }) || null
  }, [delegs, cu])
  useEffect(() => {
    if (perfilNav.rol === 'vendedor' && ['dashboard', 'reportes', 'turnos', 'incidencias'].includes(tab)) setTab('midia')
  ...
  ].filter(tb => perfilNav.rol !== 'vendedor' || (miDelegacion && tb.k === 'bitacora') || !['dashboard', 'reportes', 'turnos', 'bitacora', 'incidencias'].includes(tb.k))
    ...
    try {
      for (const s of activas) {
        for (let d = 1; d <= ultimo; d++) {
          ...
          } catch (e) { /* día con error: continuar */ }
        }
      }
      await cargarMes()
    } finally { setRecal('') }
...
function TabCotizaciones({ sucursales, vendedores, sucSel, setSucSel, seg, setSeg, cu, esGerente, isMobile }) {
  ...
  const cambiarEstadoRapido = async (r, nuevo) => {
    ...
    if (!error) setSeg(prev => { const o = prev.filter(x => x.doc_id !== row.doc_id); return [...o, row] })
  }
  ...
  return (
    <div>
      ...
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, marginBottom: 10 }}>
        ...
        {(() => { const nDesc = baseAll.filter(r => r.estado === 'descartada').length; return nDesc > 0 ? (
...
function TabDespachos({ sucursales, vendedores, sucSel, setSucSel, cu, esGerente, isMobile }) {
  ...
  const CAMPOS = 'id,folio,bsale_doc_type,cliente_nombre,cliente_rut,sucursal_codigo,vendedor_nombre,' +
    'fecha_programada,despacho_direccion,despacho_comuna,despacho_contacto,despacho_telefono,despacho_obs,' +
...
function SheetDespacho({ orden, cu, onClose, onGuardar }) {
  ...
  const guardar = async () => {
    if (!fecha) { alert('Indica la fecha tentativa'); return }
    if (modalidad === 'despacho' && (!direccion.trim() || !comuna.trim())) { alert('Dirección y comuna son obligatorias para despacho a domicilio'); return }
    setBusy(true)
    const payload = {
      ...
      despacho_direccion: modalidad === 'despacho' ? (direccion || null) : null,
      despacho_comuna: modalidad === 'despacho' ? (comuna || null) : null,
      despacho_contacto: modalidad === 'despacho' ? (contacto || null) : null,
      despacho_telefono: modalidad === 'despacho' ? (telefono || null) : null,
    }
  ...
  return (
    <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,.45)', zIndex: 9000, display: 'flex', alignItems: 'center', justifyContent: 'center', padding: 16 }} onClick={onClose}>
      <div style={{ background: '#fff', borderRadius: 16, padding: 20, width: '100%', maxWidth: 420, maxHeight: '90vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
        <div style={{ fontSize: 14, fontWeight: 800, marginBottom: 2 }}>#{orden.folio} · {orden.cliente_nombre || 'Sin cliente'}</div>
        <div style={{ fontSize: 11.5, color: '#8b88a8', marginBottom: 14 }}>Vendió: {orden.vendedor_nombre || '—'}</div>

        <div style={{ fontSize: 11, fontWeight: 700, color: '#6D6D72', marginBottom: 4 }}>Modalidad de entrega</div>
        <div style={{ display: 'flex', gap: 8, marginBottom: 14 }}>
          {[['retiro', '🏪 Retiro en tienda'], ['despacho', '🚚 Despacho a domicilio']].map(([m, l]) => (
            <button key={m} onClick={() => setModalidad(m)}
              style={{ flex: 1, padding: '9px 0', borderRadius: 10, border: 'none', fontSize: 12, fontWeight: 700, cursor: 'pointer', background: modalidad === m ? '#1a1a2e' : '#F2F2F7', color: modalidad === m ? '#fff' : '#6D6D72' }}>{l}</button>
          ))}
        </div>

        <div style={{ fontSize: 11, fontWeight: 700, color: '#6D6D72', marginBottom: 4 }}>Fecha tentativa</div>
        <input type="date" className="com-inp" value={fecha} min={hoy()} onChange={e => setFecha(e.target.value)} style={{ marginBottom: 14 }} />

        {modalidad === 'despacho' && (<>
          <div style={{ fontSize: 11, fontWeight: 700, color: '#6D6D72', marginBottom: 4 }}>Dirección</div>
          <input className="com-inp" value={direccion} onChange={e => setDireccion(e.target.value)} style={{ marginBottom: 10 }} />
          <div style={{ fontSize: 11, fontWeight: 700, color: '#6D6D72', marginBottom: 4 }}>Comuna</div>
          <input className="com-inp" value={comuna} onChange={e => setComuna(e.target.value)} style={{ marginBottom: 10 }} />
          <div style={{ display: 'flex', gap: 10, marginBottom: 10 }}>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#6D6D72', marginBottom: 4 }}>Contacto</div>
              <input className="com-inp" value={contacto} onChange={e => setContacto(e.target.value)} />
            </div>
            <div style={{ flex: 1 }}>
              <div style={{ fontSize: 11, fontWeight: 700, color: '#6D6D72', marginBottom: 4 }}>Teléfono</div>
              <input className="com-inp" value={telefono} onChange={e => setTelefono(e.target.value)} />
            </div>
          </div>
        </>)}

        <div style={{ fontSize: 11, fontWeight: 700, color: '#6D6D72', marginBottom: 4 }}>Observaciones {modalidad === 'retiro' ? '(opcional)' : ''}</div>
...
function TabBitacora({ sucursales, vendedores, metas, feriados, cu, esGerente, delegacion, sucSel, setSucSel, anio, setAnio, mes, setMes, isMobile }) {
  ...
  const puedeAperturar = perfilB.rol !== 'vendedor'
  const puedeCerrar = perfilB.rol !== 'vendedor' || esDelegado
  ...
  const agregarDeleg = async () => {
    const n = delegNuevo.trim()
    if (!n) return
    await supabase.from('com_delegaciones').insert({ sucursal_id: sucEff, nombre: n, permiso: 'cierre', otorgado_por: cu?.nombre || cu?.correo || null })
      ...
      {delegPanel && (
        <div style={{ background: '#fff', borderRadius: 12, padding: 14, marginBottom: 12, boxShadow: '0 1px 3px rgba(0,0,0,.05)' }}>
          ...
          <div style={{ display: 'flex', gap: 8, marginTop: 10 }}>
            <input className="com-inp" style={{ flex: 1, maxWidth: 320 }} placeholder="Nombre del usuario (tal como inicia sesión)…" value={delegNuevo} onChange={e => setDelegNuevo(e.target.value)} onKeyDown={e => e.key === 'Enter' && agregarDeleg()} />
            <button onClick={agregarDeleg} style={{ background: `linear-gradient(135deg,${C1},${C2})`, color: '#fff', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 12.5, fontWeight: 700, cursor: 'pointer' }}>+ Delegar cierre</button>
          </div>
          <div style={{ fontSize: 10.5, color: '#a6a3bd', marginTop: 8 }}>La persona delegada verá la pestaña Bitácora solo con el botón de cierre, anclada a esta sucursal. El nombre debe coincidir con el de su usuario en el sistema.</div>
        </div>
      )}
      <div style={{ background: '#fff', borderRadius: 12, boxShadow: '0 1px 3px rgba(0,0,0,.05)', overflowX: 'auto' }}>
        <table className="com">
