...
function TabDashboard({ sucursales, metas, seg, anio, setAnio, mes, setMes, feriados, isMobile }) {
  ...
  useEffect(() => {
    ...
    const cargar = async () => {
      setLoading(true)
      try {
        const [ci, inc, cot, msj] = await Promise.all([
          ...
          supabase.from('com_cotizaciones').select('id,total,fecha').gte('fecha', desde).lte('fecha', hasta),
  ...
  /* ── alertas de gestión: cotizaciones + mensajería del mes ── */
  const alertas = useMemo(() => {
    ...
    let cola = 0, nunca = 0
    Object.values(porC).forEach(list => {
