...
export function ComercialApp({ cu, setAppActual }) {
  ...
  useEffect(() => {
    if (perfilNav.rol === 'vendedor' && ['dashboard', 'reportes', 'incidencias', 'radar'].includes(tab)) setTab('midia')
  ...
  ].filter(tb => perfilNav.rol !== 'vendedor' || (miDelegacion && tb.k === 'bitacora') || !['dashboard', 'reportes', 'bitacora', 'incidencias', 'radar'].includes(tb.k))
      ...
      <div style={{ padding: isMobile ? '12px 12px 40px' : '18px 22px 48px', maxWidth: 1280, margin: '0 auto' }}>
        {loadingBase ? (
          ...
          <>
            ...
            {tab === 'radar' && perfilNav.rol !== 'vendedor' && <TabRadar {...{ sucursales, vendedores, cu, esGerente, isMobile }} />}
...
function TabRadar({ sucursales, vendedores, cu, esGerente, isMobile }) {
  ...
  const cargar = async () => {
    if (!perfilR.verTodo && !perfilR.sucursal) return
    setLoading(true); setErr('')
    try {
      ...
      if (sucVista) q = q.eq('sucursal_id', sucVista)
  ...
  useEffect(() => { cargar() /* eslint-disable-next-line */ }, [sucVista])
  ...
  return (
    <div>
      <div style={{ display: 'flex', flexWrap: 'wrap', gap: 8, alignItems: 'center', marginBottom: 12 }}>
        <select value={perfilR.verTodo ? fSuc : (perfilR.sucursal || '')} disabled={!perfilR.verTodo} onChange={e => setFSuc(e.target.value)} style={{ ...selStyle, opacity: perfilR.verTodo ? 1 : 0.75 }}>
        ...
        <select value={fVend} onChange={e => setFVend(e.target.value)} style={selStyle}>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: isMobile ? '1fr' : '1.15fr 1fr', gap: 12, marginBottom: 14 }}>
