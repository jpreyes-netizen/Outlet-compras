...
export function RemuneracionesEmpleados({ cu }) {
  ...
  const cargar = async () => {
    setLoading(true)
    const [{ data: e }, { data: s }, { data: c }, { data: cm }] = await Promise.all([
      ...
      supabase.from('cuentas_madre').select('codigo,nombre,tipo,activa').eq('activa',true).order('codigo')
    ])
  ...
  const cambiar = (cod, campo, valor) => {
    setEmps(prev => prev.map(e => e.cod_contaline === cod ? {...e, [campo]:valor} : e))
    setDirty(prev => {
      const m = new Map(prev)
      const cur = m.get(cod) || {}
      m.set(cod, {...cur, [campo]:valor})
      return m
    })
  }
