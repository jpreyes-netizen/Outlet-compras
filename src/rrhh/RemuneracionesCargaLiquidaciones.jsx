...
export function RemuneracionesCarga({ cu, onCargado }) {
  ...
  const confirmarCarga = async () => {
    ...
    setGuardando(true)
    try {
      // 1. Reemplazo si ya existe el período (verificado — un delete silenciosamente
      //    bloqueado por RLS hace chocar el insert con "duplicate key")
      const { count: countPrev } = await supabase
        .from('rrhh_liquidaciones').select('id', { count: 'exact', head: true }).eq('periodo', preview.periodo)
      if ((countPrev || 0) > 0) {
        const { error: eDel } = await supabase
          .from('rrhh_liquidaciones').delete().eq('periodo', preview.periodo)
        if (eDel) throw new Error(`No se pudo borrar la carga previa del período: ${eDel.message}`)
        // Verificar que el borrado fue efectivo (las líneas caen por ON DELETE CASCADE)
        const { count: countPost } = await supabase
          .from('rrhh_liquidaciones').select('id', { count: 'exact', head: true }).eq('periodo', preview.periodo)
        if ((countPost || 0) > 0) {
          throw new Error(`Quedaron ${countPost} cabeceras del período sin borrar. Falta policy DELETE (RLS) en rrhh_liquidaciones.`)
        }
      }
