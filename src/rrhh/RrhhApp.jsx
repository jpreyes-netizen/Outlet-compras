...
export function RrhhApp({ cu, setAppActual }) {
  ...
  const [accesos, setAccesos] = useState({ remuneraciones: false, asistencia: false })
  ...
  useEffect(() => {
    ...
    const verificar = async () => {
      try {
        ...
        // Lectura de capabilities — admin siempre pasa
        const esAdmin = cu?.rol === 'admin'
        let accRem = !!canSync(cu, 'rrhh', 'rrhh.remuneraciones')
        let accAsi = !!canSync(cu, 'rrhh', 'rrhh.asistencia')

        // Fallback legado: si no hay capabilities asignadas pero el rol es histórico
        if (!accRem && !accAsi && ROLES_LEGADO_RRHH.includes(cu?.rol)) {
          accRem = true
          accAsi = true
        }
        if (esAdmin) { accRem = true; accAsi = true }

        setAccesos({ remuneraciones: accRem, asistencia: accAsi })
      } finally {
  ...
  const ningunAcceso = !accesos.remuneraciones && !accesos.asistencia
  if (ningunAcceso) {
    return (
