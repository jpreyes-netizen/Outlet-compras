...
const SUBAPPS = [
  ...
  },
  {
    ...
    ic: "🎯",
    c: "#16213E",
    desc: "Mis tareas, compromisos, reuniones y objetivos del área",
    tabs: ["Mi semana","Objetivos","Proyectos","Reuniones"],
    cap: null   // PMO embebido: visible con cualquier acceso a RRHH; cada persona ve solo lo suyo
  }
]

// Shell mínimo del panel Gestión (las sub-apps RRHH traen su propio chrome; el PMO embebido no)
function GestionShell({ cu, onVolver, onCerrarSesion }) {
  return (
    <div style={{ minHeight: "100vh", background: "#f4f5f9", fontFamily: "-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif" }}>
      <div style={{ position: "sticky", top: 0, zIndex: 50, background: "#16213E", color: "#fff", padding: "10px 16px", display: "flex", alignItems: "center", gap: 12 }}>
        <button onClick={onVolver} style={{ background: "rgba(255,255,255,.12)", border: "none", color: "#fff", borderRadius: 8, padding: "6px 10px", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>← Personas</button>
        <div style={{ flex: 1, minWidth: 0 }}>
          <div style={{ fontSize: 15, fontWeight: 700, letterSpacing: "-0.01em" }}>Gestión</div>
          <div style={{ fontSize: 10.5, color: "#9aa3bd", whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>Gestión de Personas · {cu?.nombre}</div>
        </div>
        <button onClick={onCerrarSesion} title="Cerrar sesión" style={{ background: "rgba(255,255,255,.12)", border: "none", color: "#FCA5A5", borderRadius: 8, padding: "6px 10px", fontSize: 12, cursor: "pointer" }}>⏻</button>
      </div>
      <div style={{ padding: "16px clamp(10px, 2vw, 22px) 40px", maxWidth: 1280, margin: "0 auto" }}>
        <PmoGestion cu={cu} area="personas" />
      </div>
    </div>
  )
}
...
export function RrhhApp({ cu, setAppActual }) {
  const [subapp, setSubapp] = useState(() => {
    try {
      if (deepLink?.app === 'rrhh' && deepLink?.modulo) return deepLink.modulo
      return localStorage.getItem("rrhh_subapp") || null
    } catch { return null }
  })
  ...
  if (subapp === 'gestion') {
    return <GestionShell cu={cu} onVolver={() => setSubapp(null)} onCerrarSesion={cerrarSesion} />
  }

  // ─── Selector de sub-apps (hub intermedio) ─────────────────────
  const subappsDisponibles = SUBAPPS.filter(s => s.k === 'gestion' || accesos[s.k])
