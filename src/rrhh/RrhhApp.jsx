...
export function RrhhApp({ cu, setAppActual }) {
  ...
  const [accesos, setAccesos] = useState({ remuneraciones: false, asistencia: false, organigrama: false })
  ...
  useEffect(() => {
    ...
    const verificar = async () => {
      ...
      if (esAdmin || ROLES_LEGADO_RRHH.includes(cu?.rol)) {
        setAccesos({ remuneraciones: true, asistencia: true, organigrama: true })
      ...
      try {
        ...
        setAccesos({ remuneraciones: accRem, asistencia: accAsi, organigrama: accOrg })
      } catch (e) {
        if (!cancel) setAccesos({ remuneraciones: false, asistencia: false, organigrama: false })
      } finally {
  ...
  const ningunAcceso = !accesos.remuneraciones && !accesos.asistencia && !accesos.organigrama
  if (ningunAcceso) {
    return (
