...
const OC_CSS = `
.oc-viewport { position:relative; overflow:hidden; border-radius:14px; background:
...
.oc-rama { display:flex; flex-direction:column; align-items:center; position:relative; padding:26px 10px 0; }
.oc-rama::before { content:''; position:absolute; top:0; left:50%; width:1.5px; height:26px; background:var(--border); }
.oc-rama::after  { content:''; position:absolute; top:-1.5px; height:1.5px; background:var(--border); left:0; right:0; }
.oc-rama:first-child::after { left:50%; }
.oc-rama:last-child::after  { right:50%; }
.oc-rama:only-child::after  { display:none; }
.oc-stub { width:1.5px; height:16px; background:var(--border); }
...
.oc-colapsar { margin-top:8px; font-size:10px; font-weight:700; padding:2px 10px; border-radius:100px;
  border:1px solid var(--border); background:var(--bg-surface); color:var(--text-muted); cursor:pointer; }
