    ...
    ;(async () => {
      try {
        ...
        if (scope === 'sucursal') {
          ...
          if (cancel) return
          setAlc({ cargando:false, scope, admin:false, sucursal:suc,
            codes:[...new Set((data||[]).map(r=>r.cod_contaline).filter(c=>c!=null))] })
          return
        }

        // 'equipo' | 'propio' → equipo directo vía organigrama
        let cod = cu?.cod_contaline ?? null
        if (cod == null) {
          const { data } = await supabase.from('usuarios')
            .select('cod_contaline').eq('id', cu.id).maybeSingle()
          cod = data?.cod_contaline ?? null
        }
        if (cancel) return
        if (cod == null) {
        ...
        const { data: eq } = await supabase.from('v_org_equipo')
          .select('cod_miembro').eq('cod_jefe', cod)
        if (cancel) return
        const codes = [...new Set((eq||[]).map(r=>r.cod_miembro).filter(c=>c!=null))]
        setAlc({ cargando:false, scope:scope||'equipo', admin:false, miCod:cod, codes,
