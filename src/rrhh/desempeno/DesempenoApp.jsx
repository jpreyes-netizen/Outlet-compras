  ...
  return (
    <div style={{minHeight:'100vh',background:'var(--bg-app)'}}>
      ...
      <main style={{padding:24,maxWidth:1100,margin:'0 auto'}}>
        ...
        {ruta.v === 'trabajadores' && (
          <VistaTrabajadores cu={cu} setMsg={setMsg} procesos={procesos}
            onFicha={(cod)=>setRuta({v:'ficha', cod})}
            onEvaluar={(proc, ev)=>setRuta({v:'form', proc, evalRow:ev})} />
        )}
        {ruta.v === 'ficha' && (
          <FichaTrabajador cod={ruta.cod} setMsg={setMsg}
...
function VistaTrabajadores({ cu, setMsg, procesos, onFicha, onEvaluar }) {
  const [emps, setEmps] = useState([])
  const [evals, setEvals] = useState([])
  ...
  async function cargar() {
    ...
    try {
      const [e, ev, a, c, j] = await Promise.all([
        supabase.from('rrhh_empleados').select('cod_contaline,nombre,rut,sucursal_id,cargo,fecha_ingreso').eq('activo',true).order('nombre'),
        supabase.from('eval_evaluaciones').select('id,proceso_id,cod_contaline,estado,nota_final,categoria,enviada_at,alertas'),
        ...
        supabase.from('v_asis_jornadas').select('cod_contaline,estado_dia').gte('fecha', desde90).limit(20000),
      ])
...
function FichaTrabajador({ cod, setMsg, onVolver, onInforme }) {
  const [emp, setEmp] = useState(null)
