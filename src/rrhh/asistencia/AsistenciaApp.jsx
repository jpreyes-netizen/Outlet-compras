...
export function AsistenciaApp({ cu, onVolverHubRrhh, onCerrarSesion }) {
  ...
  const [areaScope, setAreaScope] = useState(null)   // { nombre, filtro } | null
  ...
  async function cargarPendientes() {
    try {
      // Solo el mes en curso: los meses anteriores ya pasaron por
      // remuneraciones y no se regularizan, así que no son trabajo pendiente.
      const h = new Date()
  ...
  useEffect(() => { if (scope && scope !== false) cargarPendientes() }, [scope, scopeSuc, areaScope, tab])
  ...
  const [puedeCosto, setPuedeCosto] = useState(false)
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    if (tab === 'costo' && !puedeCosto) setTab('dashboard')
  }, [restringido, tab])
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg-app)"}}>
      ...
      <main style={{padding:24}}>
        {tab==="dashboard" && <AsisDashboard cu={cu} onIrASync={irASync} onNavegar={navegar} scopeSuc={scopeSuc} areaScope={areaScope} pend={pend}/>}
        ...
        {tab==="hhee"      && <AsisHHEE      cu={cu} scopeSuc={scopeSuc} areaScope={areaScope} initDominio={hheeInit?.dominio} initCat={hheeInit?.cat} onCambio={cargarPendientes}/>}
