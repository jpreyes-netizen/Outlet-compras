...
const TABS = [
  ...
  { k:"hhee",      l:"Excepciones", ic:"⏱" },
  { k:"dotacion",  l:"Dotación",   ic:"👥" },
...
export function AsistenciaApp({ cu, onVolverHubRrhh, onCerrarSesion }) {
  ...
  // scopeSuc: null (ve todo) | NOMBRE de sucursal (limitado a esa sucursal)
  //
  // IMPORTANTE: userScope() devuelve el ID de sucursal ('suc-lg'), pero las
  // vistas de asistencia (v_asis_jornadas, v_asis_ausencias) exponen el
  // NOMBRE ('La Granja') y los tabs filtran por sucursal_nombre. Sin esta
  // traducción el filtro nunca calza y los jefes de sucursal ven la bandeja
  // VACÍA — parecía falta de permisos, pero era este desajuste.
  // Se traduce una sola vez acá porque los 4 tabs reciben el mismo scopeSuc.
  ...
  useEffect(() => {
    ...
    async function resolver() {
      try {
        ...
        if (!suc) { setScopeSuc(null); return }
        // ID → nombre. Si la sucursal no se encuentra, se mantiene el valor
        // original (fail-closed: filtra por algo y no abre datos de más).
        let nombre = suc
        try {
          const { data } = await supabase.from('sucursales')
            .select('nombre').eq('id', suc).maybeSingle()
          if (data?.nombre) nombre = data.nombre
        } catch { /* se conserva el id como filtro */ }
        if (!cancel) setScopeSuc(nombre)
      } catch {
  ...
  return (
    <div style={{minHeight:"100vh",background:"var(--bg-app)"}}>
      ...
      <main style={{padding:24}}>
        {tab==="dashboard" && <AsisDashboard cu={cu} onIrASync={irASync} onNavegar={navegar} scopeSuc={scopeSuc}/>}
        ...
        {tab==="hhee"      && <AsisHHEE      cu={cu} scopeSuc={scopeSuc} initDominio={hheeInit?.dominio} initCat={hheeInit?.cat}/>}
