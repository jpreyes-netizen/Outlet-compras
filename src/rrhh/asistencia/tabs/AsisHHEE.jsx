...
export function AsisHHEE({ cu, scopeSuc, initDominio, initCat, onCambio }) {
  ...
  const filasAus = useMemo(() => ausencias.map(a => {
    ...
    return { ...a, nombre: a.empleado, sucursal_id: a.sucursal_nombre,
      ...
      cat: clasificarAus(a, ges), key:`${a.cod_contaline}|${a.fecha}` }
  }), [ausencias, gesPor])
...
function VistaValidacion({ filas, pendientes, puedeValidar, sel, toggleSel, selItems, setSel, onDecidir, onAnular, nomValidadores, onFicha }) {
  ...
  for (const f of lista) {
    ...
    if (!g) {
      g = { cod:f.cod_contaline, nombre:f.nombre, sucursal:f.sucursal_id,
            items:[], min:0, minPend:0, nPend:0, excede:0, reparos:0,
  ...
  const selGrupo = (g, on) => {
    const n = new Set(sel)
    for (const it of g.items) if (it.cat === 'pendiente') on ? n.add(it.key) : n.delete(it.key)
    setSel(n)
  }
                ...
                <div style={{display:'flex',alignItems:'center',gap:10,padding:'11px 14px',
                  ...
                  {puedeValidar && g.nPend > 0 && (
                    <div style={{display:'flex',gap:6}}>
                      <button onClick={()=>onDecidir(g.items.filter(i=>i.cat==='pendiente'),'autorizada')}
                        style={{...btnPri,background:'#1E7A44',padding:'6px 12px',fontSize:12}}>
                        Autorizar {g.nPend}
                      </button>
                      <button onClick={()=>onDecidir(g.items.filter(i=>i.cat==='pendiente'),'rechazada')}
