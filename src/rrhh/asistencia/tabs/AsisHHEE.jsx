...
export function AsisHHEE({ cu, scopeSuc }) {
  ...
  async function cargar() {
    ...
    try {
      ...
      const [{ data: j, error: e1 }, { data: v, error: e2 }] = await Promise.all([qJ, qV])
      if (e1) throw e1
      if (e2) throw e2
      setJornadas(j||[]); setVals(v||[]); setSel(new Set())

      const ids = [...new Set((v||[]).map(x=>x.validado_por))]
      if (ids.length) {
  ...
  return (
    <div>
      ...
      {cargando ? <div style={{padding:50,textAlign:'center',color:'var(--text-muted)'}}>Cargando jornadas con horas extra...</div> :
       vista === 'validacion' ? (
        <VistaValidacion filas={filas} pendientes={pendientes} puedeValidar={puedeValidar}
          ...
          onDecidir={(items,dec)=>setModal({ decision:dec, items })}
          onAnular={anular} nomValidadores={nomValidadores}/>
      ) : (
        <VistaReporte filas={filas} nomValidadores={nomValidadores}
          onXLSX={exportXLSX} onPDF={exportPDF}/>
      )}
