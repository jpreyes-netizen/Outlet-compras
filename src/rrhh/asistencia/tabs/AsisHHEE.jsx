...
export function AsisHHEE({ cu, scopeSuc }) {
  const [dominio, setDominio] = useState('hhee')     // 'hhee' | 'ausencias'
  const [vista, setVista] = useState('validacion')   // 'validacion' | 'reporte'
  ...
  const [modal, setModal] = useState(null)           // { decision, items:[jornada] }
  ...
  async function cargar() {
    ...
    try {
      let qJ = supabase.from('v_asis_jornadas')
        .select('cod_contaline,nombre,fecha,sucursal_id,estado_dia,min_extra_dia,workshift_name,entrada_real,salida_real')
        .gt('min_extra_dia', 0)
      ...
      let qA = supabase.from('v_asis_jornadas')
        .select('cod_contaline,nombre,fecha,sucursal_id,workshift_name')
  ...
  const filas = useMemo(() => jornadas.map(j => {
    ...
    return { ...j, val, cat: clasificar(j, val), key: `${j.cod_contaline}|${j.fecha}` }
  }), [jornadas, valPor])
  ...
  const filasAus = useMemo(() => ausencias.map(a => {
    ...
    return { ...a, ges, cat: clasificarAus(a, ges), key:`${a.cod_contaline}|${a.fecha}` }
  }), [ausencias, gesPor])
    ...
    <div>
      ...
      <div style={{display:'flex',justifyContent:'space-between',alignItems:'center',flexWrap:'wrap',gap:10,marginBottom:14}}>
        <div style={{display:'flex',gap:2,background:'var(--bg-surface)',border:'1px solid var(--border)',borderRadius:10,padding:3}}>
          <button onClick={()=>setVista('validacion')} style={{...tabB, ...(vista==='validacion'?tabBOn:{})}}>
            ✅ Validación {pendientes.length>0 && <span style={{marginLeft:6,fontSize:10,fontWeight:800,padding:'1px 7px',borderRadius:100,background:'#FF950020',color:'#FF9500'}}>{pendientes.length}</span>}
          </button>
          <button onClick={()=>setVista('reporte')} style={{...tabB, ...(vista==='reporte'?tabBOn:{})}}>📊 Reporte</button>
        </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(170px,1fr))',gap:10,marginBottom:16}}>
        {dominio==='hhee' ? Object.entries(CATS).map(([k,c])=>(
          <div key={k} style={{background:'var(--bg-surface)',border:`1px solid ${c.c}35`,borderLeft:`4px solid ${c.c}`,borderRadius:10,padding:'10px 14px'}}>
      ...
      {cargando ? <div style={{padding:50,textAlign:'center',color:'var(--text-muted)'}}>Cargando…</div> :
       dominio === 'hhee' ? (
         vista === 'validacion'
          ...
          : <VistaReporte filas={filas} nomValidadores={nomValidadores} onXLSX={exportXLSX} onPDF={exportPDF}/>
       ) : (
         vista === 'validacion'
          ...
          : <VistaReporteAus filas={filasAus} noms={nomValidadores} onXLSX={exportAusXLSX} onPDF={exportAusPDF}/>
       )}
...
function VistaReporte({ filas, nomValidadores, onXLSX, onPDF }) {
...
function VistaReporteAus({ filas, noms, onXLSX, onPDF }) {
  const [fCat, setFCat] = useState('todas')
