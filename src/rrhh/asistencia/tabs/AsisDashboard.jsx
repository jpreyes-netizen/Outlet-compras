...
export function AsisDashboard({ cu, onIrASync, onNavegar, scopeSuc, areaScope, pend }) {
  const [data, setData]         = useState(null)
  const [cargando, setCargando] = useState(true)
  const [orden, setOrden]       = useState({ col:'jornadas', dir:'desc' })

  useEffect(() => { cargar() }, [scopeSuc, areaScope])
  ...
  async function cargar() {
    ...
    try {
      ...
      const sel = 'cod_contaline,estado_dia,min_atraso_contable,min_extra_dia,sucursal_id,sucursal_nombre,departamento'
      let qMes  = supabase.from('v_asis_jornadas').select(sel)
        .gte('fecha', iMes).lte('fecha', fHoy).limit(20000)
      let qPrev = supabase.from('v_asis_jornadas').select('cod_contaline,estado_dia,departamento')
      ...
      // Alcance por área: en La Granja y Tienda Maipú conviven dos jefaturas
      // sobre la misma sucursal. Sin esto, cada una vería las métricas de la
      // otra. El criterio evalúa departamento + cargo, igual que los correos.
      let enArea = () => true
      if (areaScope?.filtro) {
        const { data: emp } = await supabase.from('rrhh_empleados')
          .select('cod_contaline,cargo').limit(20000)
        const cargo = {}; for (const e of (emp || [])) cargo[e.cod_contaline] = e.cargo || ''
        try {
          const rx = new RegExp(areaScope.filtro, 'i')
          enArea = r => rx.test(`${r.departamento || ''} ${cargo[r.cod_contaline] || ''}`)
        } catch { /* filtro inválido: no se restringe */ }
      }
      const mesFil  = (jorMes.data  || []).filter(enArea)
      const prevFil = (jorPrev.data || []).filter(enArea)

      const jd = mesFil.filter(r => r.estado_dia !== 'sin_turno')
      const sinTurno = mesFil.length - jd.length
      const jp = prevFil.filter(r => r.estado_dia !== 'sin_turno')
    ...
    <div style={{maxWidth:1280}}>
      ...
      {totalPend > 0 ? (
        <Foco tono="critico"
          ...
          ].filter(Boolean).join(' · ') + ' en este mes. Solo las horas autorizadas se pagan.'}
          accion="Revisar y validar"
          onClick={() => onNavegar?.('hhee', { dominio:'hhee' })}/>
      ) : pend && (
        <Foco tono="ok" titulo="Sin validaciones pendientes"
          texto="Las horas extra y las ausencias del mes en curso están resueltas. El período puede ir a remuneraciones."/>
      )}
      ...
      {data.sucursales.length > 1 && (
        <Bloque titulo={areaScope ? `Detalle · ${areaScope.nombre}` : "Comparación por sucursal"}
