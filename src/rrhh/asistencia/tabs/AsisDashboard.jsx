...
export function AsisDashboard({ cu, onIrASync }) {
  ...
  async function cargar() {
    ...
    try {
      ...
      const puntuales   = jd.filter(r => r.estado_dia === 'puntual').length
      const atrasos     = jd.filter(r => r.estado_dia === 'atraso').length
      const extras      = jd.filter(r => r.estado_dia === 'hizo_extra').length
      const corridos    = jd.filter(r => r.estado_dia === 'turno_corrido').length
      const incompletas = jd.filter(r => r.estado_dia === 'incompleta' || r.estado_dia === 'sin_marcas').length
      const minAtraso   = jd.reduce((s,r) => s + (r.min_atraso_contable||0), 0)
      const minExtra    = jd.reduce((s,r) => s + (r.min_extra_dia||0), 0)
      const excesosLeg  = jd.filter(r => (r.min_extra_dia||0) > 120).length
      const pctPuntual  = jd.length > 0 ? Math.round(puntuales / jd.length * 100) : 0
      ...
      setData({
        ...
        jornadas: jd.length, puntuales, atrasos, extras, corridos,
      ...
      <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',marginBottom:20}}>
        <div>
          ...
          <div style={{fontSize:13,color:'var(--text-muted)',marginTop:2}}>
            {data.mes} · {data.jornadas} jornadas registradas
