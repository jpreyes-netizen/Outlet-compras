...
export function AsisDashboard({ cu, onIrASync, onNavegar, scopeSuc }) {
  ...
  async function cargar() {
    ...
    try {
      ...
      let qJor = supabase.from('v_asis_jornadas').select('estado_dia,min_atraso_contable,min_extra_dia')
        .gte('fecha', iMes).lte('fecha', fHoy)
        .limit(20000) // evita truncamiento silencioso a 1.000 filas de Supabase
      if (scopeSuc) qJor = qJor.eq('sucursal_id', scopeSuc)
      ...
      const [
        ...
        scopeSuc
          ? supabase.from('v_asis_permisos').select('id',{count:'exact',head:true}).gte('inicio', iMes).eq('sucursal_id', scopeSuc)
  ...
  return (
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:10,marginBottom:20}}>
        <Kpi ic="✅" l="Puntualidad" v={data.pctPuntual+'%'} sub={`${data.puntuales} días puntuales`} c="var(--success,#34C759)" onClick={()=>onNavegar?.('analisis')}/>
        <Kpi ic="⚠️" l="Atrasos" v={data.atrasos} sub={fMin(data.minAtraso)+' acum.'} c="#FF3B30" onClick={()=>onNavegar?.('analisis')}/>
        <Kpi ic="⏱" l="Horas extra" v={data.extras} sub={fMin(data.minExtra)+' acum.'} c="var(--accent,#007AFF)" onClick={()=>onNavegar?.('hhee',{dominio:'hhee'})}/>
        <Kpi ic="🔄" l="Turno corrido" v={data.corridos} sub="Revisar en Workera" c="#FF9500"
          alerta={data.corridos > 0} onClick={()=>onNavegar?.('analisis')}/>
        <Kpi ic="🚨" l="Exceso tope 2h" v={data.excesosLeg} sub=">2h extra en 1 jornada" c="#FF3B30"
          alerta={data.excesosLeg > 0} onClick={()=>onNavegar?.('hhee',{dominio:'hhee',cat:'aut_exceso'})}/>
        <Kpi ic="❓" l="Incompletas" v={data.incompletas} sub="Sin entrada o salida" c="#AF52DE" onClick={()=>onNavegar?.('analisis')}/>
      </div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(150px,1fr))',gap:10,marginBottom:20}}>
        ...
        {!scopeSuc && <Kpi ic="👥" l="Empleados mapeados" v={data.mapeados} sub="en el ERP" c="var(--success,#34C759)"/>}
        {!scopeSuc && data.sinMapear > 0 && <Kpi ic="🔗" l="Sin mapear" v={data.sinMapear} sub="requieren acción" c="#FF3B30" alerta onClick={onIrASync}/>}
      </div>

      {/* Alertas */}
      {((!scopeSuc && data.sinMapear > 0) || data.excesosLeg > 0 || data.corridos > 5) && (
        <div style={{display:'flex',flexDirection:'column',gap:8,marginBottom:20}}>
          ...
          {data.excesosLeg > 0 && (
            <Alerta tipo="danger" ic="🚨"
              msg={`${data.excesosLeg} jornadas superan el tope de 2h extra diarias este mes`}
              accion="Ver en Excepciones" onClick={() => onNavegar?.('hhee',{dominio:'hhee',cat:'aut_exceso'})}/>
          )}
          {data.corridos > 5 && (
            <Alerta tipo="warning" ic="🔄"
              msg={`${data.corridos} jornadas marcadas como turno corrido — posibles turnos mal asignados en Workera`}
              accion="Ver en Análisis" onClick={() => onNavegar?.('analisis')}/>
          )}
        </div>
      )}
...
function Kpi({ ic, l, v, sub, c, alerta, onClick }) {
  ...
  return (
    <div onClick={onClick} role={clickable?'button':undefined} tabIndex={clickable?0:undefined}
      ...
      onMouseEnter={clickable ? (e=>{ e.currentTarget.style.boxShadow='0 4px 14px rgba(0,0,0,0.10)'; e.currentTarget.style.transform='translateY(-2px)' }) : undefined}
      ...
      {clickable && <div style={{fontSize:10,color:'var(--accent)',marginTop:4,fontWeight:600}}>Ver detalle →</div>}
    </div>
  )
}
