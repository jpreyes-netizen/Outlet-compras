...
export function AsisRegistros({ cu, onIrASync }) {
  ...
  return (
    <div>
      ...
      {sub === 'marcaciones' && <TabMarcaciones onIrASync={onIrASync} />}
      {sub === 'permisos'    && <TabPermisos    onIrASync={onIrASync} />}
    </div>
  )
}

function TabMarcaciones({ onIrASync }) {
...
function TabPermisos({ onIrASync }) {
  const [rows, setRows]     = useState([])
  const [total, setTotal]   = useState(0)
  const [cargando, setCarg] = useState(true)
  const [pagina, setPag]    = useState(0)
