...
const AUS_CATS = {
  ...
  vacaciones:          { l:'Vacaciones',          c:'#AF52DE', ic:'🏖️' },
  otro:                { l:'Otro (novedad)',      c:'#8E8E93', ic:'📌' },
}
...
function VistaBitacora({ agruTrab, datosFil, drillCod, drillNom, setDrillCod, setDrillNom, datos, drillSuc, drillArea, fil, cu, puedeCorregir, puedeAprobar, onRecargar, onNavegar }) {
  ...
  const ausPend = useMemo(()=>ausDias.filter(r=>!gesAus[r.fecha] && !r.novedad_tipo).length,[ausDias,gesAus])
  ...
  function exportarExcelIndividual() {
    ...
    const detHoja = dias.map(r=>({
      ...
      'Gestión ausencia': r.estado_dia==='sin_marcas'
        ? ((AUS_CATS[gesAus[r.fecha]?.clasificacion]||{}).l || (r.novedad_tipo ? `${(AUS_CATS[r.novedad_tipo]||AUS_CATS.otro).l} (novedad)` : 'Pendiente — falta injustificada')) : '',
    ...
    doc.autoTable({
      ...
      body: dias.map(r=>([
        ...
        r.estado_dia==='sin_marcas'
          ? `Ausencia — ${(AUS_CATS[gesAus[r.fecha]?.clasificacion]||{}).l || (r.novedad_tipo ? (AUS_CATS[r.novedad_tipo]||AUS_CATS.otro).l + ' (novedad)' : 'falta injust. (pendiente)')}`
...
function VistaAusenciasGlobal({ datos, fil, onNavegar, onVerBitacora }) {
  ...
  const aus = useMemo(() => datos
    ...
    .map(r => {
      const g = ges[`${r.cod_contaline}|${r.fecha}`] || null
      // novedad registrada en Dotación = gestión automática
      const gesEff = g || (r.novedad_tipo ? { clasificacion: r.novedad_tipo, justificacion: 'Novedad registrada en Dotación', auto: true } : null)
      return { ...r, ges: gesEff }
    })
            ...
            {lista.map(a=>{
              ...
              return (
                <tr key={`${a.cod_contaline}|${a.fecha}`} style={{borderBottom:'1px solid var(--border)'}}>
                  ...
                  <td style={{padding:'8px 12px',fontSize:11,fontStyle:a.ges?'italic':'normal',color:a.ges?'var(--text)':'var(--text-muted)',maxWidth:260}}>{a.ges?.justificacion||'—'}{a.ges?.auto && <span style={{color:'var(--text-muted)'}}> · auto (Dotación)</span>}</td>
      ...
      <tr style={{background: r.estado_dia==='turno_corrido'?'#FF950006' : r.estado_dia==='atraso'?'#FF3B3006' : undefined, cursor: tieneDetalle?'pointer':'default'}}
        ...
        <td style={tdS}>
          <div style={{display:'flex',flexDirection:'column',gap:3}}>
            ...
            {r.estado_dia==='sin_marcas' && !ges && r.novedad_tipo && (
              <span title="Justificada automáticamente por novedad registrada en Dotación"
                style={{fontSize:10,fontWeight:700,padding:'1px 7px',borderRadius:100,whiteSpace:'nowrap',
                  background:`${(AUS_CATS[r.novedad_tipo]||AUS_CATS.otro).c}15`,color:(AUS_CATS[r.novedad_tipo]||AUS_CATS.otro).c}}>
                {(AUS_CATS[r.novedad_tipo]||AUS_CATS.otro).ic} {(AUS_CATS[r.novedad_tipo]||AUS_CATS.otro).l} · auto
              </span>
            )}
            {r.estado_dia==='sin_marcas' && !(!ges && r.novedad_tipo) && (
              ges ? (
