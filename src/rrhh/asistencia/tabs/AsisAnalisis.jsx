...
export function AsisAnalisis({ cu, onIrASync }) {
  ...
  const datosFil = useMemo(() => {
    ...
    return d
  },[datos,fil.sucursal,drillSuc,drillArea,drillCod])

  const agruSuc  = useMemo(()=>agrupar(datos.filter(r=>fil.sucursal==='todas'||r.sucursal_nombre===fil.sucursal),'sucursal_nombre'),[datos,fil.sucursal])
  const agruArea = useMemo(()=>agrupar(datos.filter(r=>(!drillSuc||r.sucursal_nombre===drillSuc)&&(fil.sucursal==='todas'||r.sucursal_nombre===fil.sucursal)),'departamento'),[datos,drillSuc,fil.sucursal])
  const agruTrab = useMemo(()=>agruparTrab(datos.filter(r=>(!drillSuc||r.sucursal_nombre===drillSuc)&&(!drillArea||r.departamento===drillArea)&&(fil.sucursal==='todas'||r.sucursal_nombre===fil.sucursal))),[datos,drillSuc,drillArea,fil.sucursal])

  // KPIs globales (datos filtrados por sucursal pero sin drill)
  const base = useMemo(()=>datos.filter(r=>fil.sucursal==='todas'||r.sucursal_nombre===fil.sucursal),[datos,fil.sucursal])
  const kpis = useMemo(()=>({
    tot: base.length,
    puntual: base.filter(r=>r.estado_dia==='puntual').length,
    atraso:  base.filter(r=>r.estado_dia==='atraso').length,
    extra:   base.filter(r=>r.estado_dia==='hizo_extra').length,
    corrido: base.filter(r=>r.estado_dia==='turno_corrido').length,
    incompleta: base.filter(r=>['incompleta','sin_marcas'].includes(r.estado_dia)).length,
    minAtraso: base.reduce((s,r)=>s+(r.min_atraso_contable||0),0),
    minExtra:  base.reduce((s,r)=>s+(r.min_extra_dia||0),0),
    excesos:   base.filter(r=>(r.min_extra_dia||0)>LIMITE_EXTRA_MIN).length,
    pct: base.length>0?Math.round(base.filter(r=>r.estado_dia==='puntual').length/base.length*100):0,
  }),[base])

  function exportarExcel() {
    const base2 = datos.filter(r=>fil.sucursal==='todas'||r.sucursal_nombre===fil.sucursal)
    const trabMap = {}
  ...
  return (
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:8,marginBottom:16}}>
        <MiniKpi ic="✅" l="Puntualidad" v={kpis.pct+'%'} c="#34C759"/>
        <MiniKpi ic="⚠️" l="Atrasos" v={kpis.atraso+' días'} sub={fMin(kpis.minAtraso)} c="#FF3B30"/>
        <MiniKpi ic="⏱" l="Extra" v={kpis.extra+' días'} sub={fMin(kpis.minExtra)} c="#007AFF"/>
        ...
        <MiniKpi ic="❓" l="Incompletas" v={kpis.incompleta} c="#AF52DE"/>
      </div>
      ...
      {cargando ? (
        ...
        <VistaLegal datos={base} />
      ) : (
        <VistaBitacora
          agruTrab={agruTrab} datosFil={datosFil}
          drillCod={drillCod} drillNom={drillNom}
          setDrillCod={setDrillCod} setDrillNom={setDrillNom}
          datos={datos.filter(r=>fil.sucursal==='todas'||r.sucursal_nombre===fil.sucursal)}
...
function VistaLegal({ datos }) {
  ...
  return (
    <div>
      {excesos.length===0&&corridos.length===0&&(
        ...
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
          ...
          <tbody>
            {dias.map((r,i)=>{
              ...
              return (
                <tr key={i} style={{background:r.estado_dia==='turno_corrido'?'#FF950006':r.estado_dia==='atraso'?'#FF3B3006':undefined}}>
                  ...
                  <td style={{...tdS,textAlign:'center',fontSize:11,fontFamily:'monospace'}}>{fHora(r.entrada_real)} → {fHora(r.salida_real)}</td>
            ...
            {dias.length===0&&<tr><td colSpan={7} style={{padding:24,textAlign:'center',color:'var(--text-muted)'}}>Sin jornadas en el período</td></tr>}
          </tbody>
        </table>
      </div>
    </div>
