...
export function AsisAnalisis({ cu, onIrASync }) {
  ...
  // KPIs sobre la base (con filtro sin_turno aplicado)
  const kpis = useMemo(()=>({
    tot: datosBase.length,
    puntual: datosBase.filter(r=>r.estado_dia==='puntual').length,
    atraso:  datosBase.filter(r=>r.estado_dia==='atraso').length,
    extra:   datosBase.filter(r=>r.estado_dia==='hizo_extra').length,
    corrido: datosBase.filter(r=>r.estado_dia==='turno_corrido').length,
    incompleta: datosBase.filter(r=>['incompleta','sin_marcas'].includes(r.estado_dia)).length,
    sinTurno: datos.filter(r=>r.estado_dia==='sin_turno' && (fil.sucursal==='todas'||r.sucursal_nombre===fil.sucursal)).length,
    minAtraso: datosBase.reduce((s,r)=>s+(r.min_atraso_contable||0),0),
    minExtra:  datosBase.reduce((s,r)=>s+(r.min_extra_dia||0),0),
    excesos:   datosBase.filter(r=>(r.min_extra_dia||0)>LIMITE_EXTRA_MIN).length,
    pct: datosBase.length>0?Math.round(datosBase.filter(r=>r.estado_dia==='puntual').length/datosBase.length*100):0,
    // Colaciones — solo jornadas donde aplica
    colNoMarcada: datosBase.filter(r=>r.estado_colacion==='no_marcada').length,
    colParcial:   datosBase.filter(r=>r.estado_colacion==='parcial').length,
    colBreve:     datosBase.filter(r=>r.estado_colacion==='breve').length,
    colOk:        datosBase.filter(r=>r.estado_colacion==='ok').length,
    colAplica:    datosBase.filter(r=>r.estado_colacion && r.estado_colacion!=='no_aplica').length,
  }),[datosBase, datos, fil.sucursal])
  ...
  return (
    <div>
      ...
      <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fit,minmax(130px,1fr))',gap:8,marginBottom:12}}>
        ...
        <MiniKpi ic="🍴" l="Sin colación" v={kpis.colNoMarcada} sub={kpis.colAplica>0?Math.round(kpis.colNoMarcada/kpis.colAplica*100)+'% del total':''} c="#FF3B30" alerta={kpis.colNoMarcada>0}/>
