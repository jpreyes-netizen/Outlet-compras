...
export function AsisAnalisis({ cu, onIrASync, scopeSuc }) {
  ...
  // Solo admin y Gestión de Personas (capability rrhh.asistencia.corregir_turno)
  const puedeCorregir = !!canSync(cu, 'rrhh', 'rrhh.asistencia.corregir_turno')
    ...
    <div>
      ...
      {cargando ? (
        ...
        <VistaBitacora
          ...
          fil={fil}
          cu={cu} puedeCorregir={puedeCorregir} onRecargar={cargar}
        />
      )}
    </div>
  )
...
// ─── Vista Bitácora ───────────────────────────────────────────────────────────
function VistaBitacora({ agruTrab, datosFil, drillCod, drillNom, setDrillCod, setDrillNom, datos, drillSuc, drillArea, fil, cu, puedeCorregir, onRecargar }) {
    ...
    <div>
      ...
      <div style={{border:'1px solid var(--border)',borderRadius:10,overflow:'hidden',background:'var(--bg-surface)'}}>
        <table style={{width:'100%',borderCollapse:'collapse',fontSize:13}}>
          ...
          <tbody>
            {dias.map((r,i)=>(<FilaDia key={i} r={r} puedeCorregir={puedeCorregir} onCorregir={()=>setModalCorr(r)}/>))}
...
function FilaDia({ r, puedeCorregir, onCorregir }) {
...
function ModalCorregirTurno({ r, cu, onCerrar, onGuardado }) {
  ...
  async function guardar() {
    ...
    try {
      const { error } = await supabase.from('asis_correcciones_turno').upsert({
  ...
  return (
    <div onClick={onCerrar} style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.45)',zIndex:1000,display:'flex',alignItems:'center',justifyContent:'center',padding:20}}>
      <div onClick={e=>e.stopPropagation()} style={{background:'var(--bg-surface)',borderRadius:14,padding:24,width:'100%',maxWidth:440,border:'1px solid var(--border)'}}>
        <div style={{fontSize:16,fontWeight:700,marginBottom:2}}>✏️ Corregir turno</div>
