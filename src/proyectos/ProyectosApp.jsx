...
const REU_FORM_VACIO = { titulo: "", fecha: "", hora: "", lugar: "", proyecto_id: "", asistentes: [], resumen: "" }
const TEMA_VACIO = { tema: "", acuerdo: "", responsable_id: "", fecha_compromiso: "", crear: true }
const TESTADOS = {
...
export function ProyectosApp({ cu, setAppActual }) {
  ...
  const cargar = async () => {
    setLoading(true)
    try {
      const [rp, ru, rt, re, rc, rr, rm] = await Promise.all([
  ...
  const guardarReunion = async () => {
    ...
    try {
      const head = {
        ...
        proyecto_id: reunionForm.proyecto_id || null,
      ...
      const proyDest = reunionForm.proyecto_id || null
      if (validos.some(t => t.crear && t.responsable_id) && !proyDest)
        throw new Error("Para generar tareas desde los acuerdos, asocia la reunión a un proyecto.")
      ...
      for (let i = 0; i < validos.length; i++) {
        const t = validos[i]
        let tareaId = null
        if (t.crear && t.responsable_id) {
          tareaId = "TSK-" + uid()
          const { error: eT } = await supabase.from('pmo_tareas').insert({
            id: tareaId, proyecto_id: proyDest, tarea_padre_id: null,
            titulo: (t.acuerdo.trim() || t.tema.trim()),
            descripcion: 'Acuerdo de reunión: ' + reunionForm.titulo.trim() + ' (' + reunionForm.fecha + ')',
        ...
        const { error: eM } = await supabase.from('pmo_reunion_temas').insert({
          id: "TEM-" + uid(), reunion_id: reuId, orden: base + i,
          tema: t.tema.trim() || null, acuerdo: t.acuerdo.trim() || null,
          responsable_id: t.responsable_id || null, fecha_compromiso: t.fecha_compromiso || null,
          tarea_id: tareaId
        })
        if (eM) throw eM
      }
      if (generadas && proyDest) await syncAvanceProyecto(proyDest)
    ...
    <div style={{ fontFamily: FONT, margin: 0, padding: isMobile ? "0 10px 60px" : "0 20px 80px", background: "#f4f5f9", minHeight: "100vh", fontSize: 14 }}>
      ...
      {!proyOpen && tab === "panel" && (
        <>
          ...
          <TablaProyectos rows={proyectos.slice(0, 8)} loading={loading} nombreDe={nombreDe} onEditar={puedeEditar ? abrirEditar : null} onAbrir={p => { setProyOpen(p); setSubTab("resumen"); setTareaProyId(p.id) }} isMobile={isMobile} />
        </>
      )}
      ...
      {!proyOpen && tab === "reuniones" && (
        <ReunionesPanel reuniones={reuniones} temas={reunionTemas} tareas={tareas} nombreDe={nombreDe} nombreProy={nombreProy}
          isMobile={isMobile} loading={loading} puedeCrear={puedeTareas}
      ...
      {/* ═══ REUNIÓN / ACTA ═══ */}
      <Sheet show={showReunion} onClose={() => setShowReunion(false)} title={reunionSel ? "Acta: " + (reunionSel.titulo || "") : "Nueva reunión"}>
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 12 }}>
          ...
          <Fl l="Proyecto asociado (destino de los acuerdos)">
        ...
        <Fl l={"Asistentes (" + reunionForm.asistentes.length + ")"}>
          <div style={{ maxHeight: 130, overflowY: "auto", border: "1px solid #e5e5ea", borderRadius: 12, padding: "8px 12px", display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 4 }}>
            {usuarios.map(u => (
        ...
        {/* Temas ya registrados (con estado de su tarea) */}
        {reunionSel && reunionTemas.filter(t => t.reunion_id === reunionSel.id).length > 0 && (
          <div style={{ marginBottom: 14 }}>
            <div style={{ fontSize: 13, fontWeight: 700, color: "#3A3A3C", marginBottom: 6 }}>Temas y acuerdos registrados</div>
            {reunionTemas.filter(t => t.reunion_id === reunionSel.id).map(t => {
              const tk = t.tarea_id ? tareas.find(x => x.id === t.tarea_id) : null
              const es = tk ? (TESTADOS[tk.estado] || {}) : null
              return (
                <div key={t.id} style={{ padding: "8px 0", borderTop: "1px solid #f0f1f5" }}>
                  {t.tema && <div style={{ fontSize: 13, color: "#1C1C1E", fontWeight: 600 }}>{t.tema}</div>}
                  {t.acuerdo && <div style={{ fontSize: 12, color: "#3A3A3C" }}>↳ Acuerdo: {t.acuerdo}</div>}
                  <div style={{ fontSize: 11, color: "#AEAEB2", marginTop: 2, display: "flex", gap: 10, flexWrap: "wrap" }}>
                    {t.responsable_id && <span>👤 {nombreDe(t.responsable_id)}</span>}
                    {t.fecha_compromiso && <span>📅 {fFecha(t.fecha_compromiso)}</span>}
                    {es && <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><span style={{ width: 7, height: 7, borderRadius: "50%", background: es.dot }} /><span style={{ color: es.c, fontWeight: 600 }}>{es.l}</span></span>}
                    {!t.tarea_id && <span>· sin tarea vinculada</span>}
                  </div>
                </div>
              )
            })}
        ...
        {temasNuevos.map((t, i) => (
          <div key={i} style={{ border: "1px solid #eceef3", borderRadius: 12, padding: "10px 12px", marginBottom: 8, background: "#fafbfd" }}>
            <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr", gap: 8 }}>
              <input value={t.tema} onChange={e => updTema(i, "tema", e.target.value)} placeholder="Tema tratado" style={{ ...css.input, padding: "8px 12px", fontSize: 13 }} />
              <input value={t.acuerdo} onChange={e => updTema(i, "acuerdo", e.target.value)} placeholder="Acuerdo / compromiso" style={{ ...css.input, padding: "8px 12px", fontSize: 13 }} />
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginTop: 6 }}>
              <label style={{ display: "flex", alignItems: "center", gap: 6, fontSize: 12, color: "#3A3A3C", cursor: "pointer" }}>
                <input type="checkbox" checked={t.crear} onChange={e => updTema(i, "crear", e.target.checked)} /> Generar tarea derivada al responsable
              </label>
...
function ReunionesPanel({ reuniones, temas, tareas, nombreDe, nombreProy, isMobile, loading, puedeCrear, onNueva, onAbrir }) {
  const acuerdos = temas.filter(t => t.tarea_id)
  const cumplidos = acuerdos.filter(t => (tareas.find(x => x.id === t.tarea_id) || {}).estado === "completada")
  const tasa = acuerdos.length ? Math.round(100 * cumplidos.length / acuerdos.length) : null
  const statsDe = r => {
    const ts = temas.filter(t => t.reunion_id === r.id)
    const acs = ts.filter(t => t.tarea_id)
    const cmp = acs.filter(t => (tareas.find(x => x.id === t.tarea_id) || {}).estado === "completada")
  ...
  return (
    <>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(4,1fr)", gap: 10, marginBottom: 14 }}>
        ...
        <KpiCard l="Acuerdos con tarea" v={acuerdos.length} sub="compromisos trazables" c="#185FA5" />
        <KpiCard l="Cumplidos" v={cumplidos.length} sub="tareas completadas" c="#3B6D11" />
        <KpiCard l="Cumplimiento acuerdos" v={tasa === null ? "—" : tasa + "%"} sub="cumplidos / generados" c={tasa === null ? "#8E8E93" : tasa >= 80 ? "#3B6D11" : tasa >= 50 ? "#854F0B" : "#A32D2D"} />
      </div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8 }}>
                ...
                {reuniones.map(r => {
                  ...
                  return (
                    <tr key={r.id} className="pmo-tr" onClick={() => onAbrir(r)} style={{ cursor: "pointer" }}>
                      ...
                      <td className="pmo-td"><div style={{ fontSize: 13, fontWeight: 600, color: "#1C1C1E" }}>{r.titulo}</div>{r.lugar && <div style={{ fontSize: 11, color: "#AEAEB2" }}>{r.lugar}</div>}</td>
