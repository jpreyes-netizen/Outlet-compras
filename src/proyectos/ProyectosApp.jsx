...
export function ProyectosApp({ cu, setAppActual }) {
  ...
  return (
    <div style={{ fontFamily: FONT, margin: 0, padding: isMobile ? "0 10px 60px" : "0 20px 80px", background: "#f4f5f9", minHeight: "100vh", fontSize: 14 }}>
      ...
      {!proyOpen && tab === "reuniones" && (
        <>
          ...
          {reuVista === "compromisos" && (
            <CompromisosPanel temas={reunionTemas} reuniones={reuniones} tareas={tareas} usuarios={usuarios}
      ...
      {/* ═══ REUNIÓN / ACTA ═══ */}
      <Sheet show={showReunion} onClose={() => setShowReunion(false)} title={reunionSel ? "Acta: " + (reunionSel.titulo || "") : (reunionForm.tipo === "directorio" ? "Nueva acta de directorio" : "Nueva reunión")}>
            ...
            {reunionTemas.filter(t => t.reunion_id === reunionSel.id).map(t => {
              ...
              return (
                <div key={t.id} style={{ padding: "8px 0", borderTop: "1px solid #f0f1f5" }}>
                  ...
                  {t.acuerdo && <div style={{ fontSize: 12, color: "#3A3A3C", whiteSpace: "pre-wrap" }}>↳ {t.acuerdo}</div>}
                  <div style={{ fontSize: 11, color: "#AEAEB2", marginTop: 3, display: "flex", gap: 10, flexWrap: "wrap", alignItems: "center" }}>
                    {t.responsable_id && <span>👤 {nombreDe(t.responsable_id)}{corr.length ? " + " + corr.map(nombreDe).join(", ") : ""}</span>}
                    {t.fecha_compromiso && <span style={{ color: vencidoTema(t) ? "#A32D2D" : "#AEAEB2", fontWeight: vencidoTema(t) ? 700 : 400 }}>📅 {fFecha(t.fecha_compromiso)}{vencidoTema(t) ? " ⚠" : ""}</span>}
                    <span style={{ display: "inline-flex", alignItems: "center", gap: 4 }}><span style={{ width: 7, height: 7, borderRadius: "50%", background: ec.dot }} /><span style={{ color: ec.c, fontWeight: 600 }}>{ec.l}</span></span>
                    {t.tarea_id
                      ? <button onClick={() => { const tk = tareas.find(x => x.id === t.tarea_id); if (tk) { setShowReunion(false); abrirEditarTarea(tk) } }} style={{ border: "none", background: "#f4f5f9", borderRadius: 6, padding: "2px 8px", fontSize: 11, color: NAVY, cursor: "pointer", fontWeight: 600 }}>Abrir tarea ↗</button>
                      : <select value={t.estado || "no_iniciado"} onChange={e => marcarTema(t, e.target.value)} style={{ ...css.select, padding: "2px 8px", fontSize: 11, width: "auto" }}>
                ...
                {reuniones.map(r => {
                  ...
                  return (
                    <tr key={r.id} className="pmo-tr" onClick={() => onAbrir(r)} style={{ cursor: "pointer" }}>
                      ...
                      <td className="pmo-td"><div style={{ fontSize: 13, fontWeight: 600, color: "#1C1C1E", display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>{r.titulo}{r.tipo === "directorio" && <span style={{ fontSize: 9, fontWeight: 700, color: "#3C3489", background: "#EEEDFE", borderRadius: 6, padding: "1px 6px" }}>DIRECTORIO</span>}</div>{r.lugar && <div style={{ fontSize: 11, color: "#AEAEB2" }}>{r.lugar}</div>}</td>
...
function CompromisosPanel({ temas, reuniones, tareas, usuarios, nombreDe, nombreProy, isMobile, estadoTema, vencidoTema, onMarcar, onAbrirTarea, onAbrirActa }) {
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 860 }}>
              <thead><tr style={{ background: "#f8f9fc" }}>
                ...
                <th className="pmo-th" style={{ cursor: "default", width: 180 }}>Estado</th>
              </tr></thead>
              <tbody>
                {lista.map(t => {
                  const r = reuDe(t.reunion_id)
