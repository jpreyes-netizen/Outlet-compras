...
const TABS_EMBEBIDO = new Set(["semana", "objetivos", "proyectos", "reuniones"])
const SUBTABS = [
...
function ReunionesPanel({ reuniones, temas, tareas, nombreDe, nombreProy, isMobile, loading, puedeCrear, estadoTema, onNueva, onAbrir, onImportar }) {
  ...
  const mesas = series.filter(x => x.esMesa).sort((a, b) => (a.raiz.tipo === "directorio" ? 1 : 0) - (b.raiz.tipo === "directorio" ? 1 : 0) || String(a.ult.fecha).localeCompare(String(b.ult.fecha)))
  const trabajo = series.filter(x => !x.esMesa).filter(x => !soloPend || x.abiertos > 0).sort((a, b) => String(b.ult.fecha).localeCompare(String(a.ult.fecha)))
            ...
            {mesas.map(m => (
              <div key={m.raiz.id} onClick={() => onAbrir(m.ult)} className="pmo-tr" style={{ background: m.raiz.tipo === "directorio" ? "#1A1A2E" : "#fff", border: "1px solid " + (m.raiz.tipo === "directorio" ? "#1A1A2E" : "#eceef3"), borderRadius: 14, padding: "13px 15px", cursor: "pointer" }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "flex-start" }}>
                  <div style={{ fontSize: 14.5, fontWeight: 800, color: m.raiz.tipo === "directorio" ? "#fff" : "#1C1C1E", lineHeight: 1.25 }}>{m.raiz.titulo}</div>
