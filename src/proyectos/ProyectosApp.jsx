...
export function ProyectosApp({ cu, setAppActual }) {
  ...
  const [medForm, setMedForm] = useState({ valor: "", fecha: "", nota: "" })
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        ...
        if (!cancel) { setAgenda((data && data.eventos) || []); setAgendaMsg("") }
      } catch (e) { if (!cancel) { setAgenda([]); setAgendaMsg("No se pudo leer tu agenda de Google.") } }
    })()
    return () => { cancel = true }
  ...
  const alertas = useMemo(() => {
    ...
    return out
  }, [misGrupos, misCompromisos, misQuietos, misEntregables, escaladosAmi])
  ...
  const objStats = o => {
    ...
    const act = (o.valor_actual === null || o.valor_actual === undefined || o.valor_actual === "") ? ini : Number(o.valor_actual)
    ...
    const est = (o.estado === "logrado" || pct >= 100) ? "logrado"
      : o.estado === "pausado" ? "pausado"
      : diasRest < 0 ? "vencido"
      : brecha >= 0 ? "ganando" : brecha >= -10 ? "riesgo" : "perdiendo"
    return { ini, meta, act, pct, esperado, brecha, diasRest, est }
  }
  ...
  const alineacion = useMemo(() => {
    ...
    return { tot, ali, pct: tot ? Math.round((ali / tot) * 100) : 0, proySin, compSin }
  }, [proyVis, temasVis, tareas])
  ...
  const guardarMedicion = async () => {
    ...
    try {
      const { error } = await supabase.from('pmo_objetivo_mediciones').insert({
        ...
        valor: Number(medForm.valor), nota: medForm.nota || null, autor_id: cu.id
      })
      if (error) throw error
      setMsg({ t: "ok", x: "Medición registrada" })
      setShowMedir(null); setMedForm({ valor: "", fecha: "", nota: "" })
      await cargar()
    } catch (e) { setMsg({ t: "error", x: e.message }) } finally { setObjBusy(false) }
  }
  ...
  const cargarElimLog = async () => {
    try {
      const { data } = await supabase.from('pmo_eliminaciones').select('*').order('created_at', { ascending: false }).limit(200)
      setElimLog(data || []); setVerElim(true)
    ...
    <div style={{ fontFamily: FONT, margin: 0, padding: isMobile ? "0 10px 60px" : "0 20px 80px", background: "#f4f5f9", minHeight: "100vh", fontSize: 14 }}>
      ...
      {!proyOpen && tab === "semana" && (
        <MiSemanaPanel
          ...
          entregablesRev={misEntregables} agenda={agenda} agendaMsg={agendaMsg} gcalOn={cfgSys.gcal_activo === "true"}
              ...
              return (
                <div key={t.id} style={{ border: "1px solid #eceef3", borderLeft: "3px solid " + ec.dot, borderRadius: 12, padding: "10px 14px", marginBottom: 8, background: quieto ? "#fffbf2" : "#fff" }}>
                  ...
                  <div style={{ fontSize: 11.5, color: "#8E8E93", marginTop: 7, display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
                    ...
                    {!cerrado && t.fecha_compromiso && <button onClick={() => { setReprog(t); setReprogForm({ fecha: "", motivo: "" }) }} style={{ border: "none", background: "#fdf3e6", borderRadius: 6, padding: "3px 10px", fontSize: 11, color: "#854F0B", cursor: "pointer", fontWeight: 700 }}>⟳ Reprogramar</button>}
                    ...
                    {puedeEliminarTema(t) && <button onClick={() => { setElimTema(t); setElimMotivo("") }} title="Eliminar este punto (queda registrado)" style={{ border: "none", background: "#FDEAEA", borderRadius: 6, padding: "3px 10px", fontSize: 11, color: "#A32D2D", cursor: "pointer", fontWeight: 700 }}>🗑</button>}
...
function MiSemanaPanel({ cu, isMobile, loading, grupos, compromisos, entregablesRev, agenda, agendaMsg, gcalOn, nombreDe, nombreProy, reuniones, avMap, diasDesde, estadoTema, vencidoTema, diasAbiertoTema, chkMap, onCompletar, onAbrirTarea, onAvanceComp, onRevisarEnt, onNueva }) {
  const h = hoy()
  ...
  const Chip = ({ n, l, c, bg }) => (
    <div style={{ flex: "1 1 90px", background: bg, borderRadius: 12, padding: "10px 12px", minWidth: 84 }}>
      <div style={{ fontSize: 22, fontWeight: 800, color: c, lineHeight: 1 }}>{n}</div>
      <div style={{ fontSize: 10.5, color: c, opacity: 0.85, fontWeight: 600, marginTop: 3 }}>{l}</div>
    </div>
  )
  ...
  return (
    <>
      ...
      <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginBottom: 16 }}>
        <Chip n={grupos.vencido.length} l="Vencidas" c="#A32D2D" bg="#FDEAEA" />
        <Chip n={grupos.hoy.length} l="Vencen hoy" c="#854F0B" bg="#fdf3e6" />
        <Chip n={grupos.semana.length} l="Esta semana" c="#0C447C" bg="#E6F1FB" />
        <Chip n={compromisos.length} l="Compromisos" c="#3C3489" bg="#EEEDFE" />
        {quietos > 0 && <Chip n={quietos} l="Sin movimiento" c="#854F0B" bg="#fdf3e6" />}
      </div>
      ...
      {gcalOn && (
        <div style={{ marginBottom: 16 }}>
          ...
          <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #eceef3", overflow: "hidden" }}>
            ...
            {agenda !== null && !agenda.length && <div style={{ padding: "12px 14px", fontSize: 12.5, color: "#AEAEB2" }}>{agendaMsg || "Sin eventos hoy en tu calendario."}</div>}
            {(agenda || []).map((ev, i) => (
      ...
      <Bloque titulo="⚠ Vencidas" items={grupos.vencido} color="#A32D2D" bg="#FDEAEA" />
      <Bloque titulo="Hoy" items={grupos.hoy} color="#854F0B" bg="#fdf3e6" />
      <Bloque titulo="Esta semana" items={grupos.semana} color="#0C447C" bg="#E6F1FB" />
...
function ObjetivosPanel({ objetivos, objStats, vinculos, alineacion, mediciones, nombreDe, isMobile, loading, puedeEditar, onNueva, onEditar, onMedir }) {
  ...
  const Card = ({ o }) => {
    ...
    const meds = mediciones.filter(m => m.objetivo_id === o.id).slice(0, 4)
    return (
      <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #eceef3", padding: isMobile ? 14 : 18, marginBottom: 12 }}>
        ...
        <div style={{ display: "flex", alignItems: "flex-end", gap: 16, flexWrap: "wrap", marginBottom: 6 }}>
          <div>
            <div style={{ fontSize: isMobile ? 29 : 37, fontWeight: 800, color: sem.c, lineHeight: 1, letterSpacing: "-0.03em" }}>{fmtVal(st.act, o.unidad)}</div>
            <div style={{ fontSize: 10, color: "#AEAEB2", fontWeight: 700, marginTop: 3, letterSpacing: "0.04em" }}>VALOR ACTUAL</div>
          </div>
        ...
        {meds.length > 0 && (
          <div style={{ marginTop: 10, display: "flex", gap: 14, flexWrap: "wrap" }}>
            {meds.map(m => (
              <span key={m.id} style={{ fontSize: 11, color: "#8E8E93" }}>
                <strong style={{ color: "#3A3A3C" }}>{fmtVal(Number(m.valor), o.unidad)}</strong> · {fFecha(m.fecha)}{m.nota ? " · " + m.nota : ""}
              </span>
            ))}
          </div>
        )}
      ...
      <div style={{ background: NAVY, borderRadius: 14, padding: isMobile ? 14 : 18, marginBottom: 16, color: "#eef1f8" }}>
        <div style={{ display: "flex", alignItems: "center", gap: 18, flexWrap: "wrap" }}>
          <div>
            ...
            <div style={{ fontSize: 10, color: "#9aa3bd", fontWeight: 700, marginTop: 4, letterSpacing: "0.04em" }}>ALINEACIÓN</div>
          </div>
          <div style={{ flex: 1, minWidth: 210 }}>
            <div style={{ fontSize: 13, fontWeight: 600, marginBottom: 3 }}>{alineacion.ali} de {alineacion.tot} iniciativas activas empujan una meta</div>
            <div style={{ fontSize: 11.5, color: "#9aa3bd", lineHeight: 1.5 }}>
              Sin meta asociada: {alineacion.proySin} proyecto(s) y {alineacion.compSin} compromiso(s). Eso es el torbellino: trabajo real que no mueve las metas del trimestre.
            </div>
          </div>
        </div>
      </div>
