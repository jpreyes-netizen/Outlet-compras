...
const TABS_EMBEBIDO = new Set(["semana", "objetivos", "proyectos", "reuniones", "rutinas"])
const SUBTABS = [
  ...
  const nuevaSesion = async rBase => {
    ...
    try {
      ...
      const row = {
        ...
        convocante_id: cu.id, created_by: cu.id, reunion_padre_id: ult.id, area: ult.area || null
      }
  ...
  return (
    <div style={{ fontFamily: FONT, margin: 0, padding: embedded ? "0 0 70px" : (isMobile ? "0 10px 60px" : "0 20px 80px"), background: embedded ? "transparent" : "#f4f5f9", minHeight: embedded ? undefined : "100vh", fontSize: 14 }}>
      ...
      {!proyOpen && <div style={{ display: "flex", gap: 3, marginBottom: 14, background: "#e6e8f2", borderRadius: 10, padding: 3, overflowX: "auto" }}>
        {TABS.filter(t => (!t.admin || esAdmin) && (!embedded || TABS_EMBEBIDO.has(t.k))).map(t => (
