      ...
      <FullSheet show={showReunion} onClose={() => setShowReunion(false)} title={reunionSel ? "Acta: " + (reunionSel.titulo || "") : (reunionForm.tipo === "directorio" ? "Nueva acta de directorio" : "Nueva reunión")}>
        ...
        {reunionSel && reunionTemas.filter(t => t.reunion_id === reunionSel.id).length > 0 && (
          <div style={{ marginBottom: 14 }}>
            ...
            {reunionTemas.filter(t => t.reunion_id === reunionSel.id).map((t, idx) => {
              ...
              const abierto = diasAbiertoTema(t, reunionSel.fecha)
              const av = avMap[t.id]
                ...
                <div key={t.id} style={{ border: "1px solid #eceef3", borderLeft: "3px solid " + ec.dot, borderRadius: 12, padding: "10px 14px", marginBottom: 8, background: quieto ? "#fffbf2" : "#fff" }}>
                  <div style={{ display: "flex", justifyContent: "space-between", gap: 12, alignItems: "flex-start", flexWrap: "wrap" }}>
                    <div style={{ flex: 1, minWidth: 260 }}>
                      <div style={{ fontSize: 13.5, color: "#1C1C1E", fontWeight: 700 }}>{(idx + 1) + ". " + (t.tema || "(sin tema)")}</div>
                  ...
                  <div style={{ fontSize: 11.5, color: "#8E8E93", marginTop: 7, display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
                    ...
                    {t.fecha_compromiso && <span style={{ color: vencidoTema(t) ? "#A32D2D" : "#8E8E93", fontWeight: vencidoTema(t) ? 700 : 400 }}>📅 plazo {fFecha(t.fecha_compromiso)}{vencidoTema(t) ? " · vencido hace " + diasDesde(t.fecha_compromiso) + " d ⚠" : ""}</span>}
                    <span style={{ color: agingC, fontWeight: !cerrado && abierto >= 15 ? 700 : 400 }}>⏱ {abierto} d abierto</span>
                    <span style={{ color: quieto ? "#854F0B" : "#8E8E93", fontWeight: quieto ? 700 : 400 }}>💬 {av?.n || 0} avance(s){av?.ult ? " · últ. " + (ultD === 0 ? "hoy" : "hace " + ultD + " d") : ""}{quieto ? " · 🔕 SIN MOVIMIENTO" : ""}</span>
