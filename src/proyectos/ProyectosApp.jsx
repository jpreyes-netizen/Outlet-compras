...
const TABS = [
  ...
  { k: "equipo",      l: "Mi equipo",   ic: "👥" },
  { k: "objetivos",   l: "Objetivos",   ic: "🧭" },
  { k: "panel",       l: "Panel",       ic: "📊" },
  { k: "proyectos",   l: "Proyectos",   ic: "📋" },
  { k: "reuniones",   l: "Reuniones",   ic: "🗓" },
  { k: "rutinas",     l: "Rutinas",     ic: "↻" },
  { k: "informes",    l: "Informes",    ic: "📈" },
  { k: "organigrama", l: "Organigrama", ic: "🏛", admin: true }
]
// Tabs disponibles cuando el módulo corre embebido como "Gestión" dentro de una app de área
const TABS_EMBEBIDO = new Set(["semana", "equipo", "objetivos", "proyectos", "reuniones", "rutinas"])
const SUBTABS = [
...
const REU_FORM_VACIO = { titulo: "", fecha: "", hora: "", lugar: "", proyecto_id: "", asistentes: [], resumen: "", tipo: "operativa", area: "" }
const TEMA_VACIO = { tema: "", acuerdo: "", responsable_id: "", corresponsables: [], fecha_compromiso: "", proyecto_id: "", estado: "no_iniciado", crear: true }
const RTIPOS = { operativa: "Reunión de trabajo", directorio: "Directorio" }
// Una MESA es una serie de gobierno instalada (comités + directorio). El resto son reuniones de trabajo.
const esMesaRaiz = r => !r.reunion_padre_id && (String(r.id).startsWith("REU-COM-") || r.id === "REU-DIR-SOCIOS-001" || /^comité\s/i.test(r.titulo || ""))
const TEMA_ESTADOS = {
...
const GUIA_TABS = {
  ...
  equipo:     "Lo de las personas que te reportan (según el organigrama): qué tienen abierto, qué está vencido y qué te escalaron. Solo lectura: cada uno gestiona lo suyo.",
  rutinas:    "Tareas que se repiten solas: define qué, quién y cada cuánto. El sistema crea la tarea del día temprano, antes del correo diario, y aparece en Mi semana.",
...
/* ═══ COMPONENTE RAÍZ ═══ */
export function ProyectosApp({ cu, setAppActual, embedded = false, area = null }) {
  const [tab, setTab] = useState(() => {
    try {
      const t = localStorage.getItem(embedded ? "pmo_tab_emb" : "pmo_tab")
      const valida = TABS.some(x => x.k === t) && (!embedded || TABS_EMBEBIDO.has(t))
      return valida ? t : "semana"  // guard: tabs antiguas guardadas no rompen la vista
  ...
  const [vistaPanel, setVistaPanel] = useState("tarjetas")
  // Entregables
  ...
  const [showOnboard, setShowOnboard] = useState(() => { if (embedded) return false; try { return !localStorage.getItem("pmo_onboard_v1") } catch (e) { return false } })
  ...
  const [derivaTema, setDerivaTema] = useState(null)
  const [resolverEsc, setResolverEsc] = useState(null)      // guía: resolver escalamiento
  const [resTexto, setResTexto] = useState("")
  const [estConfirm, setEstConfirm] = useState(null)         // guía: cierre de estado {t, v}
  const [estNota, setEstNota] = useState("")
  const [confSesion, setConfSesion] = useState(null)         // guía: nueva sesión
  const [derivaSel, setDerivaSel] = useState("")
  const [derivaBusy, setDerivaBusy] = useState(false)
  ...
  const [restConfirm, setRestConfirm] = useState("")
  const [archReu, setArchReu] = useState(null)          // reunión a archivar
  ...
  const [comites, setComites] = useState([])   // reglamento de comités (app Procesos · prc_comites), solo lectura
  ...
  useEffect(() => {
    ...
    const init = async () => {
      try {
        ...
        // Embebido como "Gestión": el acceso lo dio la app anfitriona; no se exige acceso a la app Proyectos
        if (embedded) { if (!cancel) setTieneAcceso(true); return }
  ...
  useEffect(() => { try { localStorage.setItem(embedded ? "pmo_tab_emb" : "pmo_tab", tab) } catch (e) { } }, [tab, embedded])
  ...
  const cargar = async () => {
    ...
    try {
      ...
      const reuArch = new Set((rr.data || []).filter(r => r.eliminado_en).map(r => r.id))
      setReuniones((rr.data || []).filter(r => !r.eliminado_en))
      setReunionTemas((rm.data || []).filter(t => !t.eliminado_en && !reuArch.has(t.reunion_id)))
      ...
      try {
        const { data: cm } = await supabase.from('prc_comites').select('codigo,nombre,proposito,integrantes,secretario,responsable,quorum_min,periodicidad,dia_semana,hora_inicio,hora_fin,lugar')
        setComites(cm || [])
      } catch (e) { /* sin reglamento disponible: el acta funciona igual */ }
  ...
  useEffect(() => {
    if (embedded || deepLinkHecho || loading || !tareas.length) return
    let id = ""
  ...
  // Embebido es SIEMPRE estricto: cada persona ve solo lo suyo; la mirada global vive en la app Proyectos (gerencia)
  const esGlobal = !embedded && capsReady && (cu.rol === 'admin' || canSync(cu, 'proyectos', 'proyectos.admin') === 'all')
  ...
  // Embebido estricto: en Objetivos cada persona ve solo las metas de las que es responsable
  const objetivosVis = useMemo(() => embedded ? objetivos.filter(o => o.responsable_id === cu.id) : objetivos, [objetivos, embedded, cu.id])
  ...
  const misSubordinados = useMemo(() => subordinadosDe(cu.id), [subordinadosDe, cu.id])
  const tieneEquipo = misSubordinados.size > 0
  useEffect(() => { if (!loading && tab === "equipo" && !tieneEquipo) setTab("semana") }, [loading, tab, tieneEquipo])
  ...
  const nuevaSesion = async rBase => {
    ...
    try {
      ...
      const row = {
        ...
        convocante_id: cu.id, created_by: cu.id, reunion_padre_id: ult.id, area: ult.area || null, comite_codigo: ult.comite_codigo || null
      }
  ...
  const marcarTema = (t, nuevoEstado) => {
    // Los estados de cierre son decisiones: se confirman con guía. El resto es directo.
    if (["cumplido", "aprobada", "permanente"].includes(nuevoEstado)) { setEstConfirm({ t, v: nuevoEstado }); setEstNota(""); return }
    return marcarTemaDirecto(t, nuevoEstado)
  }
  const marcarTemaDirecto = async (t, nuevoEstado, nota) => {
    try {
      ...
      if (error) throw error
      try { await supabase.from('pmo_tema_avances').insert({ id: "AVA-" + uid(), tema_id: t.id, contenido: "Estado cambiado a: " + ((TEMA_ESTADOS[nuevoEstado] || {}).l || nuevoEstado) + (nota ? " — " + nota : ""), autor_id: cu.id }) } catch (e2) { }
      await cargar()
    } catch (e) { setMsg({ t: "error", x: e.message }) }
  }
  ...
  const resolverEscalamiento = async (t, queSeResolvio) => {
    try {
      ...
      if (error) throw error
      await supabase.from('pmo_tema_avances').insert({ id: "AVA-" + uid(), tema_id: t.id, contenido: "⬇ Escalamiento resuelto por " + (cu.nombre || cu.id) + ": " + queSeResolvio, autor_id: cu.id })
      setMsg({ t: "ok", x: "Escalamiento resuelto — el compromiso vuelve a su responsable con el camino destrabado" })
      setResolverEsc(null); setResTexto("")
      await cargar()
    } catch (e) { setMsg({ t: "error", x: e.message }) }
  }
  ...
  /* ── Archivar una reunión: solo su convocante (o gerencia), con motivo y huella.
        Bloqueos: no se archiva una sesión con continuación (cortaría la serie) ni con compromisos abiertos
        (se derivan o se cierran primero). Los temas cerrados quedan archivados junto a la reunión. ── */
  const puedeArchivarReu = r => !!r && (esGlobal || r.convocante_id === cu.id || r.created_by === cu.id)
  const bloqueosArchivo = r => {
    if (!r) return []
    const out = []
    const hijas = reuniones.filter(x => x.reunion_padre_id === r.id)
    if (hijas.length) out.push("Tiene " + hijas.length + " sesión(es) de seguimiento posteriores: archiva primero la última de la serie.")
    const abiertos = reunionTemas.filter(t => t.reunion_id === r.id && !TEMA_CERRADOS.includes(estadoTema(t)))
    if (abiertos.length) out.push("Tiene " + abiertos.length + " compromiso(s) abierto(s): derívalos a otra mesa, ciérralos o elimínalos antes de archivar.")
    return out
  }
  const archivarReunion = async () => {
    const r = archReu
    if (!r || !archMotivo.trim()) { setMsg({ t: "error", x: "El motivo es obligatorio: el archivo queda registrado" }); return }
    if (!puedeArchivarReu(r)) { setMsg({ t: "error", x: "Solo quien convocó la reunión puede archivarla" }); return }
    const bl = bloqueosArchivo(r)
    if (bl.length) { setMsg({ t: "error", x: bl[0] }); return }
    setElimBusy(true)
    try {
      const { error } = await supabase.from('pmo_reuniones').update({
        eliminado_en: new Date().toISOString(), eliminado_por: cu.id, eliminado_motivo: archMotivo.trim()
      }).eq('id', r.id).is('eliminado_en', null)
      if (error) throw error
      // si tenía sesión anterior en la serie, se vuelve a abrir esa; si no, se cierra el acta
      const previa = r.reunion_padre_id ? reuniones.find(x => x.id === r.reunion_padre_id) : null
      setArchReu(null); setArchMotivo("")
      await cargar()
      if (previa) abrirReunionVer(previa); else { setReunionSel(null); setShowReunion(false) }
      setMsg({ t: "ok", x: "Reunión archivada — queda en la bitácora de eliminaciones y gerencia puede restaurarla" })
    } catch (e) { setMsg({ t: "error", x: e.message }) } finally { setElimBusy(false) }
  }
  ...
  const restaurar = async reg => {
    ...
    setRestBusy(reg.id)
    try {
      const tabla = reg.entidad === 'tarea' ? 'pmo_tareas' : reg.entidad === 'reunion' ? 'pmo_reuniones' : 'pmo_reunion_temas'
  ...
  /* ── Derivar un compromiso a una mesa: la reunión de trabajo produce, la mesa hace el seguimiento ── */
  const mesasCabeceras = useMemo(() => reuniones.filter(esMesaRaiz), [reuniones])
  const ultimaSesionDe = raizId => {
    let cur = reuniones.find(r => r.id === raizId), guard = 0
    while (cur && guard++ < 100) { const c = reuniones.find(x => x.reunion_padre_id === cur.id); if (!c) break; cur = c }
    return cur
  }
  const derivarTema = async () => {
    if (!derivaTema || !derivaSel) return
    setDerivaBusy(true)
    try {
      const destinoRaiz = reuniones.find(r => r.id === derivaSel)
      const destino = ultimaSesionDe(derivaSel) || destinoRaiz
      const origen = reuniones.find(r => r.id === derivaTema.reunion_id)
      const { error } = await supabase.from('pmo_reunion_temas').update({ reunion_id: destino.id }).eq('id', derivaTema.id)
      if (error) throw error
      await supabase.from('pmo_tema_avances').insert({
        id: "AVA-" + uid(), tema_id: derivaTema.id,
        contenido: '→ Derivado a "' + (destinoRaiz?.titulo || "mesa") + '" para su seguimiento' + (origen ? ' (origen: ' + origen.titulo + ' · ' + fFecha(origen.fecha) + ')' : ''),
        autor_id: cu.id
      })
      setMsg({ t: "ok", x: 'Compromiso derivado a "' + (destinoRaiz?.titulo || "la mesa") + '" — conserva su plan y su historia' })
      setDerivaTema(null); setDerivaSel("")
      await cargar()
    } catch (e) { setMsg({ t: "error", x: e.message }) } finally { setDerivaBusy(false) }
  }
  ...
  const cargarElimLog = async () => {
    try {
      ...
      if (ids.reunion.length) {
        const { data: dr } = await supabase.from('pmo_reuniones').select('id,eliminado_en').in('id', ids.reunion)
        ;(dr || []).forEach(x => { if (!x.eliminado_en) vivos.add(x.id) })
      }
  ...
  const [confCompleta, setConfCompleta] = useState(null)
  const avanceRapido = (t, nuevoEstado) => {
    if (nuevoEstado === "completada") {
      const subAbiertas = tareas.filter(x => x.tarea_padre_id === t.id && x.estado !== "completada")
      const chkPend = (chkMap[t.id] || []).filter(c => !c.completado)
      if (subAbiertas.length || chkPend.length) { setConfCompleta({ t, subAbiertas, chkPend }); return }
    }
    return avanceRapidoDirecto(t, nuevoEstado)
  }
  const avanceRapidoDirecto = async (t, nuevoEstado) => {
    try {
  ...
  const enviarAsignacion = async t => {
    ...
    try {
      ...
      const asunto = "Tarea para ti: " + t.titulo.slice(0, 80)
      const cuerpo =
        "Hola " + ((u.nombre || "").split(" ")[0] || "") + "," + "\n\n" +
        "Te asigné esta tarea:" + "\n" +
        "· " + t.titulo + "\n" +
        (t.descripcion ? "· " + t.descripcion + "\n" : "") +
        (t.fecha_vencimiento ? "· Vence el " + fFecha(t.fecha_vencimiento) + "\n" : "· Sin fecha definida\n") +
        "· " + ctx + "\n\n" +
        "Ábrela aquí para marcarla resuelta o subir el entregable:" + "\n" +
        link + "\n\n" +
        (t.fecha_vencimiento ? "Ya quedó agendada en tu calendario." + "\n\n" : "") +
        (cu.nombre || cu.id)
      ...
      if (miCorreo && miCorreo.includes("@") && miCorreo !== u.correo) {
        await supabase.from('notificaciones').insert({
          ...
          asunto: "[Copia] " + asunto,
          mensaje: "Copia del aviso que le enviaste a " + (u.nombre || u.correo) + ":\n\n" + cuerpo,
  ...
  const abrirNuevaReunion = () => {
    setReunionSel(null)
    setReunionForm({ ...REU_FORM_VACIO, fecha: hoy(), asistentes: [cu.id], area: area && AREAS[area] ? area : "" })
  ...
  const abrirReunionVer = r => {
    ...
    setReunionForm({
      ...
      tipo: r.tipo || "operativa", area: r.area || ""   // fix: sin esto, editar la cabecera reescribía tipo (Directorio→operativa) y borraba el área
    })
    setTemasNuevos([])
  ...
  const parsearImport = async () => {
    ...
    try {
      ...
      setReunionForm({ titulo: p.titulo || "Reunión importada", fecha: p.fecha || hoy(), hora: "", lugar: "Google Meet", proyecto_id: "", asistentes: asis, resumen: p.resumen || "", tipo: "operativa", area: area && AREAS[area] ? area : "" })
  ...
  const guardarReunion = async () => {
    ...
    // Regla de oro del método: compromiso = responsable + fecha. Decisiones (aprobada) y estándares (permanente) no llevan plazo.
    const sinFecha = temasNuevos.filter(t => (t.tema.trim() || t.acuerdo.trim()) && t.responsable_id && !t.fecha_compromiso && !["aprobada", "permanente"].includes(t.estado))
    if (sinFecha.length) {
      setMsg({ t: "error", x: sinFecha.length + " compromiso(s) tienen responsable pero no fecha: " + sinFecha.map(t => "\"" + (t.tema || t.acuerdo).trim().slice(0, 40) + "\"").join(", ") + ". Sin plazo no hay seguimiento — asigna fecha, o márcalo como Aprobada (decisión) o Permanente (estándar)." })
      return
    }
    setReuSaving(true)
    try {
  ...
  /* ── Acciones ── */
  // Embebido: el proyecto nuevo parte en el área de la app anfitriona (editable). Standalone (area=null) → sin cambio
  const abrirNuevo = () => { setEditando(null); setForm(area && AREAS[area] ? { ...FORM_VACIO, area } : FORM_VACIO); setBitacora([]); setShowForm(true) }
  const abrirEditar = p => {
    setEditando(p)
    setForm({
  ...
  if (verificando) {
    if (embedded) return <div style={{ padding: "48px 0", textAlign: "center", fontFamily: FONT, fontSize: 13, color: "#8E8E93" }}>Cargando tu gestión...</div>
    return (
  ...
  return (
    <div style={{ fontFamily: FONT, margin: 0, padding: embedded ? "0 0 70px" : (isMobile ? "0 10px 60px" : "0 20px 80px"), background: embedded ? "transparent" : "#f4f5f9", minHeight: embedded ? undefined : "100vh", fontSize: 14 }}>
      <style>{`
      ...
      {/* HEADER — solo standalone; embebido usa el chrome de la app anfitriona */}
      {!embedded && <div style={{ position: "sticky", top: 0, zIndex: 50, background: NAVY, padding: "12px 16px", margin: isMobile ? "0 -10px 10px" : "0 -20px 12px", display: "flex", justifyContent: "space-between", alignItems: "center", flexWrap: "wrap", gap: 8 }}>
      ...
      {!proyOpen && <div style={{ display: "flex", gap: 3, marginBottom: 14, background: "#e6e8f2", borderRadius: 10, padding: 3, overflowX: "auto" }}>
        {TABS.filter(t => (!t.admin || esAdmin) && (!embedded || TABS_EMBEBIDO.has(t.k)) && (t.k !== "equipo" || tieneEquipo)).map(t => (
      ...
      {!proyOpen && tab === "objetivos" && (
        <ObjetivosPanel objetivos={objetivosVis} objStats={objStats} vinculos={objVinculos} alineacion={alineacion}
      ...
      {!showRapidaMia && (
        <button onClick={() => { setRmForm({ titulo: "", resp: "", fecha: "" }); setShowRapidaMia(true) }} title="Nueva tarea rápida"
          style={{ position: "fixed", right: 18, bottom: embedded ? "calc(84px + env(safe-area-inset-bottom, 0px))" : 18, width: 52, height: 52, borderRadius: 999, background: NAVY, color: "#fff", border: "none", cursor: "pointer", fontSize: 25, lineHeight: "52px", boxShadow: "0 6px 20px rgba(22,33,62,0.4)", zIndex: 90 }}>+</button>
      )}

      {!proyOpen && tab === "panel" && (
        <>
          <div style={{ display: "flex", justifyContent: "flex-end", marginBottom: 8 }}>
            <div style={{ display: "inline-flex", background: "#e6e8f2", borderRadius: 9, padding: 3, gap: 2 }}>
              {[["tarjetas", "📊 Tarjetas"], ["gantt", "📅 Carta Gantt"]].map(([k, l]) => (
                <button key={k} onClick={() => setVistaPanel(k)} style={{ border: "none", cursor: "pointer", borderRadius: 7, padding: "6px 14px", fontSize: 12, fontWeight: 700, background: vistaPanel === k ? "#fff" : "transparent", color: vistaPanel === k ? NAVY : "#7c839a" }}>{l}</button>
              ))}
            </div>
          </div>
          {vistaPanel === "gantt" && (
            <GanttPanel proyectos={proyVis} tareas={tareasVis} ganttProyId={ganttProyId} setGanttProyId={setGanttProyId}
              isMobile={isMobile} nombreDe={nombreDe} onEditarProyecto={abrirEditar} onEditarTarea={abrirEditarTarea} />
          )}
          {vistaPanel === "tarjetas" && (<>
      ...
      {/* ═══ MI EQUIPO (supervisión de jefaturas, solo lectura) ═══ */}
      {!proyOpen && tab === "equipo" && tieneEquipo && (
        <EquipoPanel cuId={cu.id} subs={misSubordinados} usuarios={usuarios} tareas={tareas} temas={reunionTemas}
          estadoTema={estadoTema} avMap={avMap} nombreDe={nombreDe} nombreProy={nombreProy} isMobile={isMobile} loading={loading} />
      )}

      {/* ═══ RUTINAS (tareas programadas) ═══ */}
      {!proyOpen && tab === "rutinas" && (
        <RutinasPanel cu={cu} usuarios={usuarios} nombreDe={nombreDe} esGlobal={esGlobal} puedeAsignar={puedeTareas || esAdmin}
          areaDefault={area} isMobile={isMobile} setMsg={setMsg} onGenerado={cargar} />
      )}
      ...
      {!proyOpen && tab === "informes" && (
        <>
          {esGlobal && (
            <SaludPanel temas={reunionTemas} tareas={tareas} reuniones={reuniones} proyectos={proyectos} objetivos={objetivos}
              temaAvances={temaAvances} usuarios={usuarios} estadoTema={estadoTema} nombreDe={nombreDe} isMobile={isMobile} />
          )}
      ...
      <FullSheet show={showReunion} onClose={() => setShowReunion(false)} title={reunionSel ? "Acta: " + (reunionSel.titulo || "") : (reunionForm.tipo === "directorio" ? "Nueva acta de directorio" : "Nueva reunión")}>
          ...
          <div style={{ background: "linear-gradient(135deg,#1a1a2e,#16213e)", borderRadius: 14, padding: "16px 20px", marginBottom: 14, color: "#eef1f8" }}>
            <div style={{ display: "flex", justifyContent: "space-between", gap: 16, flexWrap: "wrap", alignItems: "flex-start" }}>
              <div style={{ flex: 1, minWidth: 260 }}>
                ...
                {(() => {
                  const cod = (serieSel[0] || {}).comite_codigo || reunionSel.comite_codigo
                  const c = cod && comites.find(x => x.codigo === cod)
                  if (!c) return null
                  const DS = ["", "lunes", "martes", "miércoles", "jueves", "viernes", "sábado", "domingo"]
                  const integ = Array.isArray(c.integrantes) ? c.integrantes.filter(Boolean) : []
                  const constituido = integ.length >= 3 && c.dia_semana
                  return (
                    <div style={{ marginTop: 8, fontSize: 11.5, color: "#c7cee6", background: "rgba(255,255,255,0.06)", borderRadius: 8, padding: "7px 11px", maxWidth: 760, lineHeight: 1.5 }}>
                      🏛 <b style={{ color: "#eef1f8" }}>Reglamento {c.codigo} · {c.nombre}</b>{c.proposito ? " — " + c.proposito : ""}
                      <div style={{ color: constituido ? "#c7cee6" : "#f0b25a" }}>
                        {constituido
                          ? "Sesiona " + String(c.periodicidad || "").toLowerCase() + " · " + (DS[c.dia_semana] || "") + " " + String(c.hora_inicio || "").slice(0, 5) + (c.lugar ? " · " + c.lugar : "") + " · " + integ.length + " integrantes · quórum " + (c.quorum_min ?? "—") + (c.secretario ? " · secretaría: " + c.secretario : "")
                          : "⚠ Comité sin constituir: falta definir integrantes, día y hora en Procesos → Comités → \"Constituirlos ahora\"."}
                      </div>
                    </div>
                  )
                })()}
            ...
            <div style={{ display: "flex", gap: 6, marginTop: 12, flexWrap: "wrap", alignItems: "center" }}>
              ...
              {serieSel.length > 0 && serieSel[serieSel.length - 1].id === reunionSel.id && (
                <button onClick={() => setConfSesion(reunionSel)} disabled={reuSaving} style={{ border: "1px dashed #4a5680", cursor: "pointer", borderRadius: 8, padding: "5px 11px", fontSize: 11.5, fontWeight: 700, background: "transparent", color: "#7ed957" }}>▶ Nueva sesión de seguimiento</button>
              )}
              ...
              {puedeArchivarReu(reunionSel) && (
                <button onClick={() => { setArchReu(reunionSel); setArchMotivo("") }} title="Archivar esta sesión (queda en la bitácora)" style={{ border: "none", cursor: "pointer", borderRadius: 8, padding: "5px 11px", fontSize: 11.5, fontWeight: 700, background: "#26305a", color: "#f2a3a9" }}>Archivar sesión</button>
              )}
            </div>
          </div>
        )}
        ...
        <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "2fr 1fr 1fr 1fr 1fr", gap: 12 }}>
          ...
          <Fl l="Área">
            <select value={reunionForm.area || ""} onChange={e => setReunionForm(f => ({ ...f, area: e.target.value }))} style={css.select}>
              ...
              return (
                <div key={t.id} style={{ border: "1px solid #eceef3", borderLeft: "3px solid " + ec.dot, borderRadius: 12, padding: "10px 14px", marginBottom: 8, background: quieto ? "#fffbf2" : "#fff" }}>
                  ...
                  <div style={{ fontSize: 11.5, color: "#8E8E93", marginTop: 7, display: "flex", gap: 14, flexWrap: "wrap", alignItems: "center" }}>
                    ...
                    {t.escalado_a === cu.id && <button onClick={() => { setResolverEsc(t); setResTexto("") }} style={{ border: "none", background: "#7A1FA2", borderRadius: 6, padding: "3px 10px", fontSize: 11, color: "#fff", cursor: "pointer", fontWeight: 700 }}>⬇ Resolver escalamiento</button>}
                        ...
                        {menuTema === t.id && (
                          <>
                            ...
                            <span style={{ position: "absolute", top: 26, right: 0, zIndex: 41, background: "#fff", borderRadius: 10, boxShadow: "0 10px 32px rgba(0,0,0,0.18)", border: "1px solid #eceef3", minWidth: 210, overflow: "hidden", display: "block" }}>
                              ...
                              {puedeEliminarTema(t) && mesasCabeceras.length > 0 && !esMesaRaiz(serieSel[0] || {}) && (
                                <button onClick={() => { setMenuTema(null); setDerivaTema(t); setDerivaSel("") }} style={{ display: "block", width: "100%", textAlign: "left", border: "none", background: "#fff", padding: "9px 13px", fontSize: 12.5, color: NAVY, cursor: "pointer", fontWeight: 700, borderTop: "1px solid #f4f5f9" }}>→ Derivar a una mesa…</button>
                              )}
                              {puedeEliminarTema(t) && (
        ...
        {/* Temas nuevos */}
        <div style={{ fontSize: 13, fontWeight: 700, color: "#3A3A3C", marginBottom: 2 }}>{reunionSel ? "Agregar temas / acuerdos" : "Temas y acuerdos"}</div>
        <div style={{ fontSize: 11.5, color: "#8E8E93", marginBottom: 8, lineHeight: 1.45 }}>Cada punto necesita <strong>responsable y fecha</strong> — sin dueño no hay seguimiento. Si es solo una decisión tomada, márcala luego como "Aprobada"; si es un estándar continuo, "Permanente".</div>
        {temasNuevos.map((t, i) => {
      ...
      {/* ═══ GUÍA · COMPLETAR CON PENDIENTES ═══ */}
      <Sheet show={!!confCompleta} onClose={() => setConfCompleta(null)} title="Completar tarea con pendientes">
        {confCompleta && (
          <>
            <div style={{ background: "#f8f9fc", borderRadius: 10, padding: "10px 13px", marginBottom: 12 }}>
              <div style={{ fontSize: 13.5, fontWeight: 700, color: "#1C1C1E" }}>{confCompleta.t.titulo}</div>
            </div>
            <div style={{ fontSize: 12.5, color: "#854F0B", background: "#fdf3e6", borderRadius: 10, padding: "10px 13px", marginBottom: 12, lineHeight: 1.6 }}>
              Esta tarea aún tiene
              {confCompleta.subAbiertas.length ? <> <strong>{confCompleta.subAbiertas.length} subtarea(s) abierta(s)</strong> ({confCompleta.subAbiertas.slice(0, 3).map(x => x.titulo).join(" · ")}{confCompleta.subAbiertas.length > 3 ? "…" : ""})</> : null}
              {confCompleta.subAbiertas.length && confCompleta.chkPend.length ? " y" : ""}
              {confCompleta.chkPend.length ? <> <strong>{confCompleta.chkPend.length} ítem(s) del checklist sin marcar</strong></> : null}.
            </div>
            <div style={{ fontSize: 12.5, color: "#3A3A3C", lineHeight: 1.55, marginBottom: 12 }}>
              Completarla dice <strong>"esto se hizo y se puede verificar"</strong>. Si esos pendientes son parte del trabajo, ciérralos primero; confirma solo si ya no aplican (y explícalo en un avance para que quede registro).
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <Bt v="gry" full onClick={() => setConfCompleta(null)}>Volver a revisar</Bt>
              <Bt v="pri" full onClick={() => { const c = confCompleta; setConfCompleta(null); avanceRapidoDirecto(c.t, "completada") }}>Completar de todos modos</Bt>
            </div>
          </>
        )}
      </Sheet>

      {/* ═══ GUÍA · RESOLVER ESCALAMIENTO ═══ */}
      <Sheet show={!!resolverEsc} onClose={() => setResolverEsc(null)} title="Resolver escalamiento">
        {resolverEsc && (
          <>
            <div style={{ background: "#F5EFFA", border: "1px solid #e3d1f0", borderRadius: 10, padding: "11px 14px", marginBottom: 12 }}>
              <div style={{ fontSize: 13.5, fontWeight: 700, color: "#7A1FA2" }}>{resolverEsc.tema || resolverEsc.acuerdo}</div>
              <div style={{ fontSize: 11.5, color: "#7A1FA2", opacity: 0.85, marginTop: 3 }}>
                Responsable: {nombreDe(resolverEsc.responsable_id)} · escaló hacia ti el {fFecha(resolverEsc.escalado_en)}{resolverEsc.escalado_motivo ? " — " + resolverEsc.escalado_motivo : ""}
              </div>
            </div>
            <div style={{ fontSize: 12.5, color: "#3A3A3C", lineHeight: 1.55, marginBottom: 10 }}>
              Este botón <strong>no marca el compromiso como cumplido</strong>: solo declara que tú, como jefatura, ya destrabaste lo que lo tenía detenido, y lo devuelve a gestión normal. Antes de confirmar:
            </div>
            <div style={{ background: "#f8f9fc", borderRadius: 10, padding: "11px 14px", marginBottom: 12, fontSize: 12.5, color: "#3A3A3C", lineHeight: 1.7 }}>
              <strong>1 · Destraba.</strong> Habla con {String(nombreDe(resolverEsc.responsable_id) || "el responsable").split(" ")[0]}: ¿qué falta — una decisión tuya, recursos, prioridad?<br/>
              <strong>2 · Acuerda la acción concreta</strong> y, si el plazo ya no sirve, reprográmalo con motivo (menú ⋯).<br/>
              <strong>3 · Escribe abajo qué se resolvió</strong> — queda como avance en la bitácora del compromiso.
            </div>
            <Fl l="¿Qué se resolvió? (queda en la bitácora)" req>
              <textarea value={resTexto} onChange={e => setResTexto(e.target.value)} rows={3}
                placeholder={'Ej: "Conversado con ' + String(nombreDe(resolverEsc.responsable_id) || "el responsable").split(" ")[0] + ': la sesión de inventario queda agendada para el miércoles 12:00; se priorizó sobre X"'}
                style={{ ...css.input, resize: "vertical" }} autoFocus />
            </Fl>
            <div style={{ fontSize: 11.5, color: "#854F0B", background: "#fdf3e6", borderRadius: 9, padding: "8px 12px", marginBottom: 12, lineHeight: 1.5 }}>
              ⚠ Si resuelves sin destrabar nada, el compromiso seguirá quieto y volverá a escalar en unos días — con tu nombre en la bitácora. Si el compromiso ya no tiene sentido, no lo resuelvas: ciérralo (Cumplido/Aprobada) o elimínalo con motivo.
            </div>
            <div style={{ display: "flex", gap: 8 }}>
              <Bt v="gry" full onClick={() => setResolverEsc(null)}>Cancelar</Bt>
              <Bt v="pri" full dis={!resTexto.trim()} onClick={() => resolverEscalamiento(resolverEsc, resTexto.trim())}>⬇ Resolver y devolver a gestión</Bt>
            </div>
          </>
        )}
      </Sheet>

      {/* ═══ GUÍA · CIERRE DE ESTADO ═══ */}
      <Sheet show={!!estConfirm} onClose={() => setEstConfirm(null)} title={estConfirm ? "Marcar como " + ((TEMA_ESTADOS[estConfirm.v] || {}).l || "") : ""}>
        {estConfirm && (() => {
          const t = estConfirm.t, v = estConfirm.v
          const plan = tareas.filter(x => x.tema_id === t.id)
          const abiertasPlan = plan.filter(x => x.estado !== "completada")
          const G = {
            cumplido: {
              que: "Se hizo lo comprometido y se puede verificar. El compromiso sale del seguimiento y suma a la tasa de cumplimiento.",
              ej: 'Ej: "Protocolo de desvinculación entregado y revisado con Rocío — documento en el plan de acción".',
              nota: "¿Cómo se verificó? (recomendado — queda en la bitácora)"
            },
            aprobada: {
              que: "El punto era una DECISIÓN y quedó tomada: no requería ejecución ni seguimiento. No suma ni resta a la tasa.",
              ej: 'Ej: "Se aprueba abrir los domingos en Maipú desde septiembre". Si además genera trabajo, ese trabajo debe quedar como un compromiso aparte con responsable y fecha.',
              nota: "¿Qué se decidió? (recomendado)"
            },
            permanente: {
              que: "Es un ESTÁNDAR de trabajo continuo, no un compromiso que vence: sale de vencidos, de sin-movimiento y de la tasa. Úsalo solo para prácticas permanentes.",
              ej: 'Ej: "Supervisión de conducta del equipo" o "Liderar reposiciones semanales". Si tiene una fecha de término real, NO es permanente.',
              nota: "Nota (opcional)"
            }
          }[v]
          return (
            <>
              <div style={{ background: "#f8f9fc", borderRadius: 10, padding: "10px 13px", marginBottom: 12 }}>
                <div style={{ fontSize: 13.5, fontWeight: 700, color: "#1C1C1E" }}>{t.tema || t.acuerdo}</div>
                <div style={{ fontSize: 11.5, color: "#8E8E93", marginTop: 2 }}>{t.responsable_id ? "Responsable: " + nombreDe(t.responsable_id) : ""}</div>
              </div>
              <div style={{ fontSize: 12.5, color: "#3A3A3C", lineHeight: 1.55, marginBottom: 8 }}>{G.que}</div>
              <div style={{ fontSize: 12, color: "#8E8E93", fontStyle: "italic", marginBottom: 12, lineHeight: 1.5 }}>{G.ej}</div>
              {v === "cumplido" && abiertasPlan.length > 0 && (
                <div style={{ fontSize: 12, color: "#854F0B", background: "#fdf3e6", borderRadius: 9, padding: "9px 12px", marginBottom: 12, lineHeight: 1.5 }}>
                  ⚠ Su plan de acción tiene <strong>{abiertasPlan.length} tarea(s) abierta(s)</strong> ({plan.length - abiertasPlan.length}/{plan.length} completadas). Si de verdad está cumplido, ciérralas primero en el Plan de acción — o confirma solo si esas tareas ya no aplican.
                </div>
              )}
              <Fl l={G.nota}>
                <textarea value={estNota} onChange={e => setEstNota(e.target.value)} rows={2} style={{ ...css.input, resize: "vertical" }} autoFocus />
              </Fl>
              <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                <Bt v="gry" full onClick={() => setEstConfirm(null)}>Cancelar</Bt>
                <Bt v="pri" full onClick={() => { const c = estConfirm; setEstConfirm(null); marcarTemaDirecto(c.t, c.v, estNota.trim() || null) }}>Confirmar: {(TEMA_ESTADOS[v] || {}).l}</Bt>
              </div>
            </>
          )
        })()}
      ...
      {/* ═══ GUÍA · NUEVA SESIÓN ═══ */}
      <Sheet show={!!confSesion} onClose={() => setConfSesion(null)} title="Nueva sesión de seguimiento">
        {confSesion && (() => {
          const serie = serieDe(confSesion)
          const ids = new Set(serie.map(x => x.id))
          const abiertos = reunionTemas.filter(t => ids.has(t.reunion_id) && t.responsable_id && !TEMA_CERRADOS.includes(estadoTema(t)))
          return (
            <>
              <div style={{ fontSize: 12.5, color: "#3A3A3C", lineHeight: 1.6, marginBottom: 12 }}>
                Se creará la <strong>sesión {serie.length + 1}</strong> de "{serie[0]?.titulo}" con fecha de hoy. Qué ocurre:
              </div>
              <div style={{ background: "#f8f9fc", borderRadius: 10, padding: "11px 14px", marginBottom: 12, fontSize: 12.5, color: "#3A3A3C", lineHeight: 1.7 }}>
                <strong>1 ·</strong> Los <strong>{abiertos.length} compromiso(s) abierto(s)</strong> de la serie se arrastran automáticamente — no se copian ni se pierden.<br/>
                <strong>2 ·</strong> Los cumplidos quedan en el archivo de sesiones anteriores.<br/>
                <strong>3 ·</strong> Si el calendario está activo y defines hora, la cita llega a los asistentes.
              </div>
              <div style={{ fontSize: 11.5, color: "#8E8E93", marginBottom: 12, lineHeight: 1.5 }}>
                Consejo: crea la sesión <strong>al momento de reunirse</strong>, no antes — así el acta refleja la fecha real y el Modo Reunión ordena lo vigente.
              </div>
              <div style={{ display: "flex", gap: 8 }}>
                <Bt v="gry" full onClick={() => setConfSesion(null)}>Cancelar</Bt>
                <Bt v="pri" full dis={reuSaving} onClick={() => { const r = confSesion; setConfSesion(null); nuevaSesion(r) }}>{reuSaving ? "..." : "▶ Crear sesión " + (serie.length + 1)}</Bt>
              </div>
            </>
          )
        })()}
      ...
      {/* ═══ DERIVAR A UNA MESA ═══ */}
      <Sheet show={!!derivaTema} onClose={() => setDerivaTema(null)} title="Derivar compromiso a una mesa">
        {derivaTema && (
          <>
            <div style={{ background: "#f8f9fc", borderRadius: 10, padding: "10px 13px", marginBottom: 12 }}>
              <div style={{ fontSize: 13.5, fontWeight: 700, color: "#1C1C1E" }}>{derivaTema.tema || derivaTema.acuerdo}</div>
              <div style={{ fontSize: 11.5, color: "#8E8E93", marginTop: 3 }}>{derivaTema.responsable_id ? "Responsable: " + nombreDe(derivaTema.responsable_id) : "Sin responsable"}</div>
            </div>
            <div style={{ fontSize: 12, color: "#8E8E93", marginBottom: 12, lineHeight: 1.5 }}>
              La reunión de trabajo produce el compromiso; <strong>la mesa le hace el seguimiento semanal</strong>. Se moverá a la sesión vigente de la mesa que elijas, con su plan de acción y su historia, y quedará la huella del traslado.
            </div>
            <Fl l="Mesa que hará el seguimiento" req>
              <select value={derivaSel} onChange={e => setDerivaSel(e.target.value)} style={css.select}>
                <option value="">— Selecciona la mesa —</option>
                {mesasCabeceras.map(m => <option key={m.id} value={m.id}>{m.titulo}</option>)}
              </select>
            </Fl>
            <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
              <Bt v="gry" full onClick={() => setDerivaTema(null)}>Cancelar</Bt>
              <Bt v="pri" full dis={!derivaSel || derivaBusy} onClick={derivarTema}>{derivaBusy ? "..." : "Derivar a la mesa"}</Bt>
            </div>
          </>
        )}
      </Sheet>
      ...
      {/* ═══ ARCHIVAR REUNIÓN ═══ */}
      <Sheet show={!!archReu} onClose={() => setArchReu(null)} title="Archivar reunión">
        {archReu && (() => {
          const bl = bloqueosArchivo(archReu)
          const nCerr = reunionTemas.filter(t => t.reunion_id === archReu.id).length
          const dis = !archMotivo.trim() || elimBusy || bl.length > 0
          return (
            <>
              <div style={{ background: "#FDEAEA", borderRadius: 10, padding: "11px 14px", marginBottom: 14 }}>
                <div style={{ fontSize: 13.5, fontWeight: 700, color: "#A32D2D" }}>{archReu.titulo}</div>
                <div style={{ fontSize: 11.5, color: "#A32D2D", opacity: 0.85, marginTop: 3 }}>
                  {fFecha(archReu.fecha)}{archReu.hora ? " · " + archReu.hora : ""}{archReu.area && AREAS[archReu.area] ? " · " + AREAS[archReu.area].l : ""} · convocó {nombreDe(archReu.convocante_id)}
                  {nCerr > 0 && !bl.length ? " · sus " + nCerr + " punto(s) cerrado(s) se archivan con ella" : ""}
                </div>
              </div>
              {bl.length > 0 ? (
                <div style={{ background: "#FFF4E5", borderRadius: 10, padding: "10px 14px", marginBottom: 14, fontSize: 12.5, color: "#854F0B", lineHeight: 1.5 }}>
                  <strong>No se puede archivar todavía:</strong>
                  {bl.map((b, i) => <div key={i}>· {b}</div>)}
                </div>
              ) : (
                <div style={{ fontSize: 12, color: "#8E8E93", marginBottom: 12, lineHeight: 1.5 }}>
                  La reunión sale de la vista, pero <strong>no se destruye</strong>: queda archivada con tu nombre, la fecha y el motivo en la bitácora de eliminaciones, y gerencia puede restaurarla.
                </div>
              )}
              <Fl l="Motivo del archivo" req>
                <textarea value={archMotivo} onChange={e => setArchMotivo(e.target.value)} rows={3} disabled={bl.length > 0} placeholder="Ej: sesión duplicada · reunión que no se realizó · reemplazada por el Comité de Operaciones" style={{ ...css.input, resize: "vertical" }} autoFocus />
              </Fl>
              <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
                <Bt v="gry" full onClick={() => setArchReu(null)}>Cancelar</Bt>
                <button onClick={archivarReunion} disabled={dis} style={{ flex: 1, padding: "11px 16px", borderRadius: 10, border: "none", background: dis ? "#e8b4b4" : "#A32D2D", color: "#fff", fontSize: 14, fontWeight: 700, cursor: dis ? "not-allowed" : "pointer" }}>{elimBusy ? "..." : "Archivar reunión"}</button>
              </div>
            </>
          )
        })()}
          ...
          <div key={e.id} style={{ borderTop: "1px solid #f0f1f5", padding: "10px 0" }}>
            ...
            {esGlobal && !String(e.titulo || "").startsWith("↩") && (
              e.activo
                ...
                : (restConfirm === e.id
                  ? <span style={{ display: "inline-flex", gap: 6, alignItems: "center", marginTop: 6, flexWrap: "wrap" }}>
                      <span style={{ fontSize: 11, color: "#5F5E5A" }}>Vuelve a estar visible y activo, con huella "restaurado por ti".</span>
                      <button onClick={() => { setRestConfirm(""); restaurar(e) }} disabled={restBusy === e.id} style={{ border: "none", background: NAVY, color: "#fff", borderRadius: 8, padding: "4px 11px", fontSize: 11.5, cursor: "pointer", fontWeight: 700 }}>{restBusy === e.id ? "..." : "✓ Confirmar"}</button>
                      <button onClick={() => setRestConfirm("")} style={{ border: "1px solid #d8dcea", background: "#fff", color: "#5F5E5A", borderRadius: 8, padding: "4px 10px", fontSize: 11.5, cursor: "pointer", fontWeight: 700 }}>Cancelar</button>
                    </span>
                  : <button onClick={() => setRestConfirm(e.id)} style={{ marginTop: 6, border: "1px solid #cfd6e6", background: "#fff", borderRadius: 8, padding: "4px 11px", fontSize: 11.5, color: NAVY, cursor: "pointer", fontWeight: 700 }}>↩ Restaurar…</button>)
            )}
          </div>
        ))}
      </Sheet>
      ...
      <Sheet show={!!reprog} onClose={() => setReprog(null)} title="Reprogramar plazo">
        {reprog && (
          <>
            ...
            <div style={{ fontSize: 11.5, color: "#8E8E93", marginBottom: 8, lineHeight: 1.5 }}>
              Un buen motivo dice <strong>qué cambió y cuál es el nuevo plan</strong>. Ej: "el proveedor confirmó entrega para el 22, se recibe ese día" — no "falta tiempo".
            </div>
      ...
      <Sheet show={showRevision} onClose={() => setShowRevision(false)} title={"Revisar: " + (entSel?.nombre || "")}>
        ...
        <div style={{ background: "#f8f9fc", borderRadius: 10, padding: "10px 13px", marginBottom: 12, fontSize: 12, color: "#3A3A3C", lineHeight: 1.6 }}>
          <strong>✓ Aprobar</strong> es la verificación formal: declaras que lo revisaste y cumple lo pedido — queda con tu nombre y cierra el ciclo del entregable.<br/>
          <strong>✕ Rechazar</strong> lo devuelve a quien lo subió con tus observaciones; vuelve al estado pendiente hasta la nueva entrega.<br/>
          <span style={{ color: "#8E8E93", fontStyle: "italic" }}>Ej. de rechazo útil: "falta la columna de costos y la firma del proveedor en la página 2" — no "está incompleto".</span>
        </div>
      ...
      <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #eceef3", overflow: "hidden" }}>
        <div style={{ overflowX: "auto" }}>
          <div style={{ minWidth: isMobile ? 680 : 0 }}>
            {/* Header de meses */}
            <div style={{ display: "flex", borderBottom: "1px solid #eceef3", background: "#f8f9fc", position: "relative" }}>
              {hoyPct !== null && <span style={{ position: "absolute", left: "calc(" + wLbl + "px + (100% - " + wLbl + "px) * " + (hoyPct / 100) + " - 14px)", top: 4, fontSize: 8.5, fontWeight: 800, color: "#fff", background: "#E24B4A", borderRadius: 4, padding: "1px 5px", zIndex: 3 }}>HOY</span>}
              ...
              return (
                <div key={f.id} style={{ display: "flex", alignItems: "center", borderTop: "1px solid #f0f1f5", cursor: (f.esProy ? onEditarProyecto : onEditarTarea) ? "pointer" : "default" }}
                  ...
                  <div style={{ flex: 1, position: "relative", height: 36 }}>
                    {hoyPct !== null && <div style={{ position: "absolute", left: hoyPct + "%", top: 0, bottom: 0, width: 2, background: "#E24B4A", opacity: 0.6, zIndex: 2 }} />}
                    {f.hito ? (
                      ...
                      <>
                        <div title={fFecha(f.ini) + " → " + fFecha(f.fin) + " · " + f.av + "%"} style={{ position: "absolute", left: izq + "%", width: ancho + "%", top: 9, height: 18, borderRadius: 5, background: bg, overflow: "hidden", zIndex: 1, boxShadow: "inset 0 0 0 1px rgba(0,0,0,0.06)" }}>
                          <div style={{ width: f.av + "%", height: "100%", background: fill }} />
                          {ancho > 9 && <span style={{ position: "absolute", left: 6, top: 0, lineHeight: "18px", fontSize: 10, fontWeight: 800, color: f.av > 45 ? "#fff" : "#1C1C1E", textShadow: f.av > 45 ? "0 1px 2px rgba(0,0,0,0.25)" : "none" }}>{f.av}%</span>}
                        </div>
                        {(izq + ancho) < 88 && <span style={{ position: "absolute", left: "calc(" + (izq + ancho) + "% + 6px)", top: 11, fontSize: 9.5, color: f.atr ? "#A32D2D" : "#AEAEB2", fontWeight: 700, whiteSpace: "nowrap", zIndex: 1 }}>{fFecha(f.fin)}</span>}
                      </>
                    )}
                  </div>
                </div>
...
function ReunionesPanel({ reuniones, temas, tareas, nombreDe, nombreProy, isMobile, loading, puedeCrear, estadoTema, onNueva, onAbrir, onImportar }) {
  ...
  const [fAreaR, setFAreaR] = useState("")   // "" todas · "_sin" transversales · clave de AREAS
  const pasaArea = x => !fAreaR || (fAreaR === "_sin" ? !x.raiz.area : x.raiz.area === fAreaR)
  /* Agrupar por serie: cada raíz con su cadena de sesiones */
  const series = useMemo(() => {
    const raices = reuniones.filter(r => !r.reunion_padre_id)
    return raices.map(raiz => {
      const cadena = [raiz]; let cur = raiz, guard = 0
      while (guard++ < 100) { const c = reuniones.find(x => x.reunion_padre_id === cur.id); if (!c) break; cadena.push(c); cur = c }
      const ids = new Set(cadena.map(x => x.id))
      const ts = temas.filter(t => ids.has(t.reunion_id))
      const conResp = ts.filter(t => t.responsable_id)
      const abiertos = conResp.filter(t => !TEMA_CERRADOS.includes(estadoTema(t)))
      const vencidos = abiertos.filter(t => t.fecha_compromiso && t.fecha_compromiso < hoy())
      const exigibles = conResp.filter(t => !["aprobada", "permanente"].includes(estadoTema(t)))
      const cumplidos = conResp.filter(t => estadoTema(t) === "cumplido")
      const ult = cadena[cadena.length - 1]
      const cad = (String(raiz.resumen || "").match(/^(SEMANAL|QUINCENAL|MENSUAL)/i) || [])[0] || null
      return { raiz, ult, n: cadena.length, abiertos: abiertos.length, vencidos: vencidos.length,
               tasa: exigibles.length ? Math.round(100 * cumplidos.length / exigibles.length) : null,
               cadencia: cad, esMesa: esMesaRaiz(raiz), futura: ult.fecha >= hoy() }
    })
  }, [reuniones, temas, tareas])
  const mesas = series.filter(x => x.esMesa).filter(pasaArea).sort((a, b) => (a.raiz.tipo === "directorio" ? 1 : 0) - (b.raiz.tipo === "directorio" ? 1 : 0) || String(a.ult.fecha).localeCompare(String(b.ult.fecha)))
  const trabajo = series.filter(x => !x.esMesa).filter(pasaArea).filter(x => !soloPend || x.abiertos > 0).sort((a, b) => String(b.ult.fecha).localeCompare(String(a.ult.fecha)))
  ...
  return (
    <>
      ...
      {/* ── MESAS DE GOBIERNO Y COMITÉS ── */}
      {mesas.length > 0 && (
        <>
          <div style={{ fontSize: 12.5, fontWeight: 800, color: "#3A3A3C", textTransform: "uppercase", letterSpacing: "0.03em", marginBottom: 8 }}>🏛 Mesas de gobierno y comités <span style={{ color: "#AEAEB2", fontWeight: 600 }}>· cadencia fija, compromisos que se arrastran</span></div>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(auto-fill, minmax(330px, 1fr))", gap: 10, marginBottom: 20 }}>
            {mesas.map(m => (
              <div key={m.raiz.id} onClick={() => onAbrir(m.ult)} className="pmo-tr" style={{ background: m.raiz.tipo === "directorio" ? "#1A1A2E" : "#fff", border: "1px solid " + (m.raiz.tipo === "directorio" ? "#1A1A2E" : "#eceef3"), borderRadius: 14, padding: "13px 15px", cursor: "pointer" }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "flex-start" }}>
                  <div style={{ fontSize: 14.5, fontWeight: 800, color: m.raiz.tipo === "directorio" ? "#fff" : "#1C1C1E", lineHeight: 1.25 }}>{m.raiz.titulo}{AREAS[m.raiz.area] && <span style={{ marginLeft: 7, verticalAlign: "middle", fontSize: 9.5, fontWeight: 800, borderRadius: 6, padding: "2px 7px", color: AREAS[m.raiz.area].c, background: AREAS[m.raiz.area].bg }}>{AREAS[m.raiz.area].l}</span>}</div>
                  {m.cadencia && <span style={{ fontSize: 9.5, fontWeight: 800, color: m.raiz.tipo === "directorio" ? "#CADCFC" : NAVY, background: m.raiz.tipo === "directorio" ? "#26305a" : "#eef1f8", borderRadius: 6, padding: "2px 8px", whiteSpace: "nowrap" }}>{m.cadencia.toUpperCase()}</span>}
                </div>
                <div style={{ fontSize: 11.5, color: m.raiz.tipo === "directorio" ? "#9aa3bd" : "#8E8E93", marginTop: 3 }}>
                  Convoca {nombreDe(m.raiz.convocante_id)} · {m.n} sesión(es) · {m.futura ? "próxima" : "última"}: {fFecha(m.ult.fecha)}{m.ult.hora ? " " + m.ult.hora : ""}
                </div>
                <div style={{ display: "flex", gap: 7, alignItems: "center", marginTop: 9, flexWrap: "wrap" }}>
                  <span style={{ fontSize: 11, fontWeight: 700, borderRadius: 999, padding: "2px 10px", background: m.abiertos ? "#E6F1FB" : "#F2F2F7", color: m.abiertos ? "#0C447C" : "#8E8E93" }}>{m.abiertos} abierto(s)</span>
                  {m.vencidos > 0 && <span style={{ fontSize: 11, fontWeight: 800, borderRadius: 999, padding: "2px 10px", background: "#FDEAEA", color: "#A32D2D" }}>⚠ {m.vencidos} vencido(s)</span>}
                  {m.tasa !== null && <span style={{ fontSize: 11, fontWeight: 700, color: m.raiz.tipo === "directorio" ? "#CADCFC" : (m.tasa >= 70 ? "#3B6D11" : m.tasa >= 40 ? "#854F0B" : "#A32D2D") }}>{m.tasa}% cumplimiento</span>}
                  <span style={{ flex: 1 }} />
                  <span style={{ fontSize: 11.5, fontWeight: 700, color: m.raiz.tipo === "directorio" ? "#CADCFC" : NAVY }}>Abrir acta →</span>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* ── REUNIONES DE TRABAJO (agrupadas por serie) ── */}
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 8, flexWrap: "wrap", gap: 8 }}>
        <span style={{ fontSize: 12.5, fontWeight: 800, color: "#3A3A3C", textTransform: "uppercase", letterSpacing: "0.03em" }}>💼 Reuniones de trabajo <span style={{ color: "#AEAEB2", fontWeight: 600 }}>· análisis, definiciones, 1:1 — sus compromisos pueden derivarse a una mesa</span></span>
        <div style={{ display: "flex", gap: 6, alignItems: "center" }}>
          <select value={fAreaR} onChange={e => setFAreaR(e.target.value)} title="Filtrar mesas y reuniones por área" style={{ ...css.select, padding: "5px 10px", fontSize: 12, width: "auto" }}>
            <option value="">Todas las áreas</option>
            {Object.entries(AREAS).map(([k, a]) => <option key={k} value={k}>{a.l}</option>)}
            <option value="_sin">Transversales</option>
          </select>
          <label style={{ fontSize: 11.5, color: "#5F5E5A", display: "flex", alignItems: "center", gap: 5, cursor: "pointer" }}>
          ...
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", minWidth: 700 }}>
              <thead><tr style={{ background: "#f8f9fc" }}>
                ...
                <th className="pmo-th" style={{ cursor: "default" }}>Compromisos</th>
                <th className="pmo-th" style={{ cursor: "default", width: 150 }}>Cumplimiento serie</th>
              </tr></thead>
              <tbody>
                {trabajo.map(x => (
                  <tr key={x.raiz.id} className="pmo-tr" onClick={() => onAbrir(x.ult)} style={{ cursor: "pointer" }}>
                    <td className="pmo-td" style={{ fontSize: 12, color: "#3A3A3C", whiteSpace: "nowrap" }}>{fFecha(x.ult.fecha)}{x.ult.hora ? " " + x.ult.hora : ""}</td>
                    <td className="pmo-td">
                      <div style={{ fontSize: 13, fontWeight: 600, color: "#1C1C1E", display: "flex", alignItems: "center", gap: 6, flexWrap: "wrap" }}>
                        {x.raiz.titulo}
                        {x.n > 1 && <span style={{ fontSize: 9, fontWeight: 700, color: "#0C447C", background: "#E6F1FB", borderRadius: 6, padding: "1px 6px" }}>SERIE · {x.n} SESIONES</span>}
                      </div>
                      {x.raiz.lugar && <div style={{ fontSize: 11, color: "#AEAEB2" }}>{x.raiz.lugar}</div>}
                    </td>
                    <td className="pmo-td" style={{ fontSize: 12, color: "#3A3A3C", whiteSpace: "nowrap", maxWidth: 180, overflow: "hidden", textOverflow: "ellipsis" }}>{x.raiz.proyecto_id ? nombreProy(x.raiz.proyecto_id) : "—"}</td>
                    <td className="pmo-td" style={{ fontSize: 12, color: "#3A3A3C", whiteSpace: "nowrap" }}>{nombreDe(x.raiz.convocante_id)}</td>
                    <td className="pmo-td" style={{ fontSize: 12 }}>{Array.isArray(x.ult.asistentes) ? x.ult.asistentes.length : 0}</td>
                    <td className="pmo-td" style={{ fontSize: 12 }}>
                      {x.abiertos ? <span style={{ fontWeight: 800, color: "#0C447C" }}>{x.abiertos} abierto(s)</span> : <span style={{ color: "#AEAEB2" }}>al día</span>}
                      {x.vencidos > 0 && <span style={{ fontWeight: 800, color: "#A32D2D" }}> · ⚠ {x.vencidos}</span>}
                    </td>
                    <td className="pmo-td">
                      {x.tasa === null ? <span style={{ fontSize: 12, color: "#AEAEB2" }}>sin acuerdos</span> : (
                        <div style={{ display: "flex", alignItems: "center", gap: 7 }}>
                          <div style={{ flex: 1, height: 6, borderRadius: 999, background: "#eceef3", overflow: "hidden", minWidth: 50 }}>
                            <div style={{ width: x.tasa + "%", height: "100%", background: x.tasa >= 80 ? "#639922" : x.tasa >= 50 ? "#BA7517" : "#E24B4A" }} />
                          </div>
                          <span style={{ fontSize: 11, fontWeight: 600, color: "#3A3A3C", whiteSpace: "nowrap" }}>{x.tasa}%</span>
                        </div>
                      )}
                    </td>
                  </tr>
                ))}
                {!trabajo.length && <tr><td className="pmo-td" colSpan={7} style={{ textAlign: "center", color: "#AEAEB2", fontSize: 12.5, padding: 22 }}>Sin reuniones de trabajo{soloPend ? " con pendientes" : ""}.</td></tr>}
              </tbody>
            </table>
          </div>
        </div>
...
function ObjetivosPanel({ objetivos, objStats, vinculos, alineacion, mediciones, nombreDe, isMobile, loading, puedeEditar, puedeMedir, onAnular, cuId, onNueva, onEditar, onMedir }) {
  const [anulaMed, setAnulaMed] = useState(null)   // { m, o }
            ...
            {meds.map(m => (
              <div key={m.id} style={{ display: "flex", gap: 8, alignItems: "center", flexWrap: "wrap", fontSize: 11, color: "#8E8E93", padding: "2px 0", textDecoration: m.anulada_en ? "line-through" : "none", opacity: m.anulada_en ? 0.6 : 1 }}>
                ...
                {!m.anulada_en && puedeMedir && puedeMedir(o) && (
                  <button onClick={() => { setAnulaMed({ m, o }); setAnulaMotivo("") }} title="Anular medición errónea" style={{ border: "none", background: "transparent", color: "#A32D2D", cursor: "pointer", fontSize: 11, fontWeight: 700 }}>anular</button>
                )}
              </div>
            ))}
          </div>
  ...
  return (
    <>
      ...
      {/* GUÍA · ANULAR MEDICIÓN */}
      <Sheet show={!!anulaMed} onClose={() => setAnulaMed(null)} title="Anular medición">
        {anulaMed && (
          <>
            <div style={{ background: "#FDEAEA", borderRadius: 10, padding: "10px 13px", marginBottom: 12 }}>
              <div style={{ fontSize: 13.5, fontWeight: 700, color: "#A32D2D" }}>{fmtVal(Number(anulaMed.m.valor), anulaMed.o.unidad)} · {fFecha(anulaMed.m.fecha)}</div>
              <div style={{ fontSize: 11.5, color: "#A32D2D", opacity: 0.85, marginTop: 2 }}>Medición de "{anulaMed.o.nombre}" registrada por {nombreDe(anulaMed.m.autor_id)}</div>
            </div>
            <div style={{ fontSize: 12.5, color: "#3A3A3C", lineHeight: 1.6, marginBottom: 12 }}>
              Anular <strong>no borra</strong>: la medición queda tachada en el historial con tu motivo, y el <strong>valor actual de la meta vuelve a la medición válida anterior</strong>. Úsalo para errores de digitación o datos de fuente equivocada — nunca para "mejorar" el marcador.
            </div>
            <Fl l="¿Por qué se anula? (queda visible en el historial)" req>
              <textarea value={anulaMotivo} onChange={e => setAnulaMotivo(e.target.value)} rows={2}
                placeholder={'Ej: "error de digitación: era 5' + '4%, no 45%" · "se contó solo La Granja, la meta es de empresa"'}
                style={{ ...css.input, resize: "vertical" }} autoFocus />
            </Fl>
            <div style={{ display: "flex", gap: 8, marginTop: 4 }}>
              <Bt v="gry" full onClick={() => setAnulaMed(null)}>Cancelar</Bt>
              <button onClick={() => { const a = anulaMed; setAnulaMed(null); onAnular(a.m, anulaMotivo.trim()) }} disabled={!anulaMotivo.trim()}
                style={{ flex: 1, padding: "11px 16px", borderRadius: 10, border: "none", background: !anulaMotivo.trim() ? "#e8b4b4" : "#A32D2D", color: "#fff", fontSize: 14, fontWeight: 700, cursor: !anulaMotivo.trim() ? "not-allowed" : "pointer" }}>
                Anular medición
              </button>
            </div>
          </>
...
/* ═══ MODO EMBEBIDO — tab "Gestión" dentro de las apps de área ═══
   Uso: <PmoGestion cu={cu} area="comercial" />
   área ∈ AREAS (finanzas · comercial · negocios · operacion · personas); la app Compras usa "negocios".
   Reglas: estricto SIEMPRE (cada usuario ve solo lo asignado o donde participa, incluso admin);
   sin header propio, sin onboarding, sin deep-link ?tarea=; tab persistida en pmo_tab_emb.
   `area` queda reservada para defaults de la Etapa 1 (no altera visibilidad: lo estricto manda). */
export function PmoGestion({ cu, area = null }) {
  return <ProyectosApp cu={cu} setAppActual={() => { }} embedded area={area} />
}

/* ═══ RUTINAS — tareas programadas recurrentes ═══
   Tabla pmo_rutinas · generador fn_pmo_generar_rutinas() · cron pmo_rutinas_generar 11:30 UTC.
   Cada ocurrencia es una pmo_tarea normal (origen_tipo='rutina'): aparece en Mi semana, en el correo
   diario y en la rendición. Visibilidad estricta: responsable o creador (gerencia ve todo en standalone). */
const DIAS_SEM = [[1, "L"], [2, "M"], [3, "X"], [4, "J"], [5, "V"], [6, "S"], [7, "D"]]
const RUT_VACIA = { titulo: "", descripcion: "", responsable_id: "", frecuencia: "dias", dias_semana: [1, 2, 3, 4, 5], dia_mes: "1", fecha_inicio: "", fecha_fin: "", prioridad: "media", area: "" }
const RUT_FREC = { dias: "Días de la semana", quincenal: "Cada 2 semanas", mensual: "Mensual" }
const descFrec = r => {
  if (r.frecuencia === "quincenal") return "Cada 2 semanas"
  if (r.frecuencia === "mensual") return "Mensual · día " + r.dia_mes
  const d = (r.dias_semana || []).slice().sort()
  if (d.length === 7) return "Todos los días"
  if (d.join() === "1,2,3,4,5") return "Lunes a viernes"
  return DIAS_SEM.filter(([n]) => d.includes(n)).map(([, l]) => l).join(" · ") || "—"
}

function RutinasPanel({ cu, usuarios, nombreDe, esGlobal, puedeAsignar, areaDefault, isMobile, setMsg, onGenerado }) {
  ...
  const [f, setF] = useState(RUT_VACIA)
  const [busy, setBusy] = useState(false)

  const cargarR = async () => {
    setLoading(true); setErr("")
    try {
      const { data, error } = await supabase.from('pmo_rutinas').select('*').order('created_at', { ascending: false })
      if (error) throw error
      const vis = esGlobal ? (data || []) : (data || []).filter(r => r.responsable_id === cu.id || r.created_by === cu.id)
      setRows(vis)
      const ids = vis.map(r => r.id)
      const m = {}
      if (ids.length) {
        const desde = new Date(Date.now() - 30 * 864e5).toISOString().slice(0, 10)
        const { data: ts } = await supabase.from('pmo_tareas').select('origen_id,estado,eliminado_en')
          .eq('origen_tipo', 'rutina').in('origen_id', ids).gte('fecha_vencimiento', desde).lte('fecha_vencimiento', hoy())
        ;(ts || []).filter(t => !t.eliminado_en).forEach(t => { const x = m[t.origen_id] = m[t.origen_id] || { tot: 0, ok: 0 }; x.tot++; if (t.estado === "completada") x.ok++ })
      }
      setConteo(m)
    } catch (e) { setErr(e.message) } finally { setLoading(false) }
  }
  useEffect(() => { cargarR() }, [cu.id, esGlobal])

  const puedeEditarR = r => esGlobal || r.created_by === cu.id
  const abrirNueva = () => { setEdit(null); setF({ ...RUT_VACIA, responsable_id: cu.id, fecha_inicio: hoy(), area: areaDefault && AREAS[areaDefault] ? areaDefault : "" }); setShow(true) }
  const abrirEdit = r => {
    setEdit(r)
    setF({ titulo: r.titulo || "", descripcion: r.descripcion || "", responsable_id: r.responsable_id || "", frecuencia: r.frecuencia || "dias",
      dias_semana: Array.isArray(r.dias_semana) ? r.dias_semana : [], dia_mes: String(r.dia_mes || 1), fecha_inicio: r.fecha_inicio || hoy(),
      fecha_fin: r.fecha_fin || "", prioridad: r.prioridad || "media", area: r.area || "" })
    setShow(true)
  }
  const togDia = d => setF(x => ({ ...x, dias_semana: x.dias_semana.includes(d) ? x.dias_semana.filter(y => y !== d) : [...x.dias_semana, d].sort() }))

  const guardar = async () => {
    if (!f.titulo.trim()) { setMsg({ t: "error", x: "La rutina necesita un título" }); return }
    if (!f.responsable_id) { setMsg({ t: "error", x: "La rutina necesita un responsable" }); return }
    if (f.responsable_id !== cu.id && !puedeAsignar) { setMsg({ t: "error", x: "Solo puedes programar rutinas para ti" }); return }
    if (f.frecuencia === "dias" && !f.dias_semana.length) { setMsg({ t: "error", x: "Elige al menos un día de la semana" }); return }
    const dm = Number(f.dia_mes)
    if (f.frecuencia === "mensual" && !(dm >= 1 && dm <= 28)) { setMsg({ t: "error", x: "El día del mes debe estar entre 1 y 28" }); return }
    if (!f.fecha_inicio) { setMsg({ t: "error", x: "Indica desde cuándo corre la rutina" }); return }
    if (f.fecha_fin && f.fecha_fin < f.fecha_inicio) { setMsg({ t: "error", x: "La fecha de término no puede ser anterior al inicio" }); return }
    setBusy(true)
    try {
      const row = {
        titulo: f.titulo.trim(), descripcion: f.descripcion.trim() || null, responsable_id: f.responsable_id,
        area: f.area || null, frecuencia: f.frecuencia,
        dias_semana: f.frecuencia === "dias" ? f.dias_semana : null,
        dia_mes: f.frecuencia === "mensual" ? dm : null,
        fecha_inicio: f.fecha_inicio, fecha_fin: f.fecha_fin || null, prioridad: f.prioridad,
        updated_at: new Date().toISOString()
      }
      let error
      if (edit) ({ error } = await supabase.from('pmo_rutinas').update(row).eq('id', edit.id))
      else ({ error } = await supabase.from('pmo_rutinas').insert({ ...row, id: "RUT-" + uid(), created_by: cu.id, activo: true }))
      if (error) throw error
      // Genera ya la ocurrencia de hoy (idempotente: no duplica lo que el cron haya creado)
      let n = 0
      try { const r = await supabase.rpc('fn_pmo_generar_rutinas'); n = Number(r.data) || 0 } catch (e) { }
      setMsg({ t: "ok", x: (edit ? "Rutina actualizada" : "Rutina programada") + (n ? " · " + n + " tarea(s) de hoy creadas" : "") })
      setShow(false); setEdit(null)
      await cargarR()
      if (n && onGenerado) await onGenerado()
    } catch (e) { setMsg({ t: "error", x: e.message }) } finally { setBusy(false) }
  }
  const togActivo = async r => {
    const { error } = await supabase.from('pmo_rutinas').update({ activo: !r.activo, updated_at: new Date().toISOString() }).eq('id', r.id)
    if (error) { setMsg({ t: "error", x: error.message }); return }
    setMsg({ t: "ok", x: r.activo ? "Rutina pausada: no generará más tareas" : "Rutina reactivada" })
    cargarR()
  }
  const eliminar = async r => {
    if (!window.confirm("¿Eliminar la rutina \"" + r.titulo + "\"? Las tareas ya generadas se conservan.")) return
    const { error } = await supabase.from('pmo_rutinas').delete().eq('id', r.id)
    if (error) { setMsg({ t: "error", x: error.message }); return }
    setMsg({ t: "ok", x: "Rutina eliminada (sus tareas históricas se conservan)" })
    cargarR()
  }

  const th = { fontSize: 11, fontWeight: 700, color: "#8E8E93", textTransform: "uppercase", letterSpacing: "0.03em", textAlign: "left", padding: "8px 10px", whiteSpace: "nowrap" }
  return (
    <>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 10 }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 800, color: "#1C1C1E" }}>↻ Rutinas · tareas programadas</div>
          <div style={{ fontSize: 12, color: "#8E8E93" }}>Lo que se repite no se recuerda: se programa. Cada día que toca, la tarea aparece sola en Mi semana y en el correo diario de la mañana.</div>
        </div>
        <Bt v="pri" sm ic="➕" onClick={abrirNueva}>Nueva rutina</Bt>
      </div>
      {err && <div style={{ padding: 12, background: "#fdeaea", color: "#A32D2D", borderRadius: 10, fontSize: 12.5, marginBottom: 10 }}>⚠ {err}</div>}
      {loading ? <div style={{ textAlign: "center", padding: 40, color: "#8E8E93", fontSize: 13 }}>Cargando...</div> : !rows.length ? (
        <div style={{ textAlign: "center", padding: 36, background: "#fff", borderRadius: 14, border: "1px solid #eceef3", color: "#8E8E93", fontSize: 13, lineHeight: 1.6 }}>
          Aún no tienes rutinas.<br />Ejemplos: "Revisar cuadratura de caja" (lunes a viernes) · "Conteo cíclico clase A" (lunes y jueves) · "Informe de ventas al directorio" (mensual, día 5).
        </div>
      ) : (
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #eceef3", overflow: "hidden" }}>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
              <thead><tr style={{ background: "#fafbfd" }}>
                <th style={th}>Rutina</th><th style={th}>Responsable</th><th style={th}>Frecuencia</th><th style={th}>Área</th>
                <th style={th}>Vigencia</th><th style={th}>Cumplimiento 30 d</th><th style={th}>Estado</th><th style={th}></th>
              </tr></thead>
              <tbody>
                {rows.map(r => {
                  const c = conteo[r.id]; const pct = c && c.tot ? Math.round(100 * c.ok / c.tot) : null
                  return (
                    <tr key={r.id} className="pmo-tr" style={{ opacity: r.activo ? 1 : 0.55 }}>
                      <td className="pmo-td"><div style={{ fontWeight: 700, color: "#1C1C1E" }}>{r.titulo}</div>{r.descripcion && <div style={{ fontSize: 11, color: "#AEAEB2", maxWidth: 320, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}>{r.descripcion}</div>}</td>
                      <td className="pmo-td" style={{ whiteSpace: "nowrap" }}>{nombreDe(r.responsable_id)}</td>
                      <td className="pmo-td" style={{ whiteSpace: "nowrap" }}>{descFrec(r)}</td>
                      <td className="pmo-td">{AREAS[r.area] ? <Bd c={AREAS[r.area].c} bg={AREAS[r.area].bg}>{AREAS[r.area].l}</Bd> : <span style={{ color: "#AEAEB2", fontSize: 12 }}>—</span>}</td>
                      <td className="pmo-td" style={{ whiteSpace: "nowrap", fontSize: 12 }}>{fFecha(r.fecha_inicio)}{r.fecha_fin ? " → " + fFecha(r.fecha_fin) : " → sin término"}</td>
                      <td className="pmo-td" style={{ whiteSpace: "nowrap" }}>{pct === null ? <span style={{ color: "#AEAEB2", fontSize: 12 }}>sin ocurrencias</span> : <span style={{ fontWeight: 800, color: pct >= 80 ? "#3B6D11" : pct >= 50 ? "#854F0B" : "#A32D2D" }}>{pct}% <span style={{ fontWeight: 500, color: "#8E8E93", fontSize: 11 }}>({c.ok}/{c.tot})</span></span>}</td>
                      <td className="pmo-td">{r.activo ? <Bd c="#27500A" bg="#E1F5EE">Activa</Bd> : <Bd c="#5F5E5A" bg="#F2F2F7">Pausada</Bd>}</td>
                      <td className="pmo-td" style={{ whiteSpace: "nowrap", textAlign: "right" }}>
                        {puedeEditarR(r) && <>
                          <Bt v="gry" sm onClick={() => abrirEdit(r)}>✎</Bt>{" "}
                          <Bt v="gry" sm onClick={() => togActivo(r)}>{r.activo ? "⏸" : "▶"}</Bt>{" "}
                          <Bt v="gry" sm onClick={() => eliminar(r)}>🗑</Bt>
                        </>}
                      </td>
                    </tr>
                  )
      ...
      <Sheet show={show} onClose={() => { setShow(false); setEdit(null) }} title={edit ? "Editar rutina" : "Nueva rutina"}>
        <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
          <Fl l="¿Qué hay que hacer?" req>
            <input value={f.titulo} onChange={e => setF(x => ({ ...x, titulo: e.target.value }))} placeholder="Ej: Revisar cuadratura de caja del día anterior" style={css.input} autoFocus />
          </Fl>
          <Fl l="Detalle / criterio de cumplimiento">
            <textarea value={f.descripcion} onChange={e => setF(x => ({ ...x, descripcion: e.target.value }))} rows={2} placeholder="Cómo se sabe que quedó hecha" style={{ ...css.input, resize: "vertical" }} />
          </Fl>
          <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1fr 1fr", gap: 12 }}>
            <Fl l="Responsable" req>
              <select value={f.responsable_id} onChange={e => setF(x => ({ ...x, responsable_id: e.target.value }))} style={css.select} disabled={!puedeAsignar}>
                {(puedeAsignar ? usuarios : usuarios.filter(u => u.id === cu.id)).map(u => <option key={u.id} value={u.id}>{u.nombre || u.correo}</option>)}
              </select>
            </Fl>
            <Fl l="Área">
              <select value={f.area} onChange={e => setF(x => ({ ...x, area: e.target.value }))} style={css.select}>
                <option value="">Transversal</option>
                {Object.entries(AREAS).map(([k, a]) => <option key={k} value={k}>{a.l}</option>)}
              </select>
            </Fl>
            <Fl l="Prioridad">
              <select value={f.prioridad} onChange={e => setF(x => ({ ...x, prioridad: e.target.value }))} style={css.select}>
                {Object.entries(PRIORIDADES).map(([k, l]) => <option key={k} value={k}>{l}</option>)}
              </select>
            </Fl>
          </div>
          <Fl l="Frecuencia" req>
            <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
              {Object.entries(RUT_FREC).map(([k, l]) => (
                <button key={k} onClick={() => setF(x => ({ ...x, frecuencia: k }))} style={{ border: "1px solid " + (f.frecuencia === k ? NAVY : "#d8dcea"), background: f.frecuencia === k ? NAVY : "#fff", color: f.frecuencia === k ? "#fff" : NAVY, borderRadius: 8, padding: "7px 12px", fontSize: 12.5, fontWeight: 700, cursor: "pointer" }}>{l}</button>
              ))}
            </div>
          </Fl>
          {f.frecuencia === "dias" && (
            <Fl l="¿Qué días?">
              <div style={{ display: "flex", gap: 6, flexWrap: "wrap", alignItems: "center" }}>
                {DIAS_SEM.map(([d, l]) => (
                  <button key={d} onClick={() => togDia(d)} style={{ width: 38, height: 38, borderRadius: 10, border: "1px solid " + (f.dias_semana.includes(d) ? NAVY : "#d8dcea"), background: f.dias_semana.includes(d) ? NAVY : "#fff", color: f.dias_semana.includes(d) ? "#fff" : NAVY, fontWeight: 800, cursor: "pointer" }}>{l}</button>
                ))}
                <button onClick={() => setF(x => ({ ...x, dias_semana: [1, 2, 3, 4, 5] }))} style={{ border: "none", background: "transparent", color: "#185FA5", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>L a V</button>
                <button onClick={() => setF(x => ({ ...x, dias_semana: [1, 2, 3, 4, 5, 6, 7] }))} style={{ border: "none", background: "transparent", color: "#185FA5", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>Todos</button>
              </div>
            </Fl>
          )}
          {f.frecuencia === "quincenal" && <div style={{ fontSize: 12, color: "#5F5E5A", background: "#f4f5f9", borderRadius: 8, padding: "8px 11px" }}>Se repite cada 14 días, el mismo día de la semana de la fecha de inicio.</div>}
          {f.frecuencia === "mensual" && (
            <Fl l="Día del mes (1 a 28)">
              <input type="number" min={1} max={28} value={f.dia_mes} onChange={e => setF(x => ({ ...x, dia_mes: e.target.value }))} style={{ ...css.input, width: 110 }} />
            </Fl>
          )}
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 12 }}>
            <Fl l="Desde" req><input type="date" value={f.fecha_inicio} onChange={e => setF(x => ({ ...x, fecha_inicio: e.target.value }))} style={css.input} /></Fl>
            <Fl l="Hasta (opcional)"><input type="date" value={f.fecha_fin} onChange={e => setF(x => ({ ...x, fecha_fin: e.target.value }))} style={css.input} /></Fl>
          </div>
          <Bt v="pri" onClick={guardar} dis={busy}>{busy ? "Guardando..." : (edit ? "Guardar cambios" : "Programar rutina")}</Bt>
        </div>
      </Sheet>
    </>
  )
...
/* ═══ FRANJA "TU GESTIÓN HOY" — para las bitácoras de las apps de área ═══
   Solo lectura sobre pmo_*: no escribe nada ni toca tablas de la app anfitriona.
   Uso: <PmoResumenHoy cu={cu} onAbrir={() => irAGestion()} /> */
export function PmoResumenHoy({ cu, onAbrir }) {
  ...
  useEffect(() => {
    ...
    ;(async () => {
      try {
        const h = hoy()
        const [rt, r1, r2] = await Promise.all([
          supabase.from('pmo_tareas').select('id,fecha_vencimiento').eq('responsable_id', cu.id).neq('estado', 'completada')
            .is('eliminado_en', null).not('fecha_vencimiento', 'is', null).lte('fecha_vencimiento', h),
          supabase.from('pmo_reunion_temas').select('id,fecha_compromiso,estado').eq('responsable_id', cu.id).is('eliminado_en', null),
          supabase.from('pmo_reunion_temas').select('id,fecha_compromiso,estado').contains('corresponsables', [cu.id]).is('eliminado_en', null)
        ])
        if (rt.error) throw rt.error
        const vistos = new Set()
        const temas = [...(r1.data || []), ...(r2.data || [])].filter(t => !vistos.has(t.id) && vistos.add(t.id))
          .filter(t => !TEMA_CERRADOS.includes(t.estado || "no_iniciado") && t.fecha_compromiso && t.fecha_compromiso <= h)
        const tareas = rt.data || []
        if (!cancel) setD({
          tVenc: tareas.filter(t => t.fecha_vencimiento < h).length, tHoy: tareas.filter(t => t.fecha_vencimiento === h).length,
          cVenc: temas.filter(t => t.fecha_compromiso < h).length, cHoy: temas.filter(t => t.fecha_compromiso === h).length
        })
      } catch (e) { if (!cancel) setD(null) }
    })()
    return () => { cancel = true }
  }, [cu?.id])
  if (!d) return null
  const venc = d.tVenc + d.cVenc, hoyN = d.tHoy + d.cHoy
  const alDia = !venc && !hoyN
  const partes = []
  if (hoyN) partes.push(hoyN + " para hoy")
  if (venc) partes.push(venc + " vencido(s)")
  return (
    <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap", background: alDia ? "#E1F5EE" : venc ? "#FDEAEA" : "#E6F1FB", border: "1px solid " + (alDia ? "#bfe6d4" : venc ? "#f3c7c7" : "#cfe3f7"), borderRadius: 10, padding: "8px 13px", marginBottom: 12, fontFamily: FONT }}>
      <span style={{ fontSize: 15 }}>🎯</span>
      <span style={{ flex: 1, minWidth: 180, fontSize: 12.5, color: alDia ? "#27500A" : venc ? "#A32D2D" : "#0C447C", lineHeight: 1.4 }}>
        <b>Tu gestión:</b> {alDia ? "al día, sin tareas ni compromisos pendientes para hoy." : partes.join(" · ") + " — entre tareas (" + (d.tHoy + d.tVenc) + ") y compromisos de reunión (" + (d.cHoy + d.cVenc) + ")."}
      </span>
      {onAbrir && <button onClick={onAbrir} style={{ border: "none", background: alDia ? "#27500A" : venc ? "#A32D2D" : "#0C447C", color: "#fff", borderRadius: 7, padding: "5px 12px", fontSize: 12, fontWeight: 700, cursor: "pointer", whiteSpace: "nowrap" }}>Ver en Gestión →</button>}
    </div>
  )
}

/* ═══ MI EQUIPO — supervisión de jefaturas (solo lectura) ═══
   Muestra lo de quienes reportan al usuario según usuarios.reporta_a (directos e indirectos).
   No habilita edición: cada responsable gestiona lo suyo; la jefatura ve, prioriza y conversa. */
function EquipoPanel({ cuId, subs, usuarios, tareas, temas, estadoTema, avMap, nombreDe, nombreProy, isMobile, loading }) {
  const h = hoy()
  const quieto = t => { const u = (avMap[t.id] || {}).ult || t.created_at; if (!u) return false; return (new Date(h) - new Date(String(u).slice(0, 10))) / 864e5 >= 14 }
  const filas = useMemo(() => [...subs].map(id => {
    const u = usuarios.find(x => x.id === id) || { id, nombre: id }
    const tAb = tareas.filter(t => t.responsable_id === id && t.estado !== "completada")
    const cAb = temas.filter(t => (t.responsable_id === id || (Array.isArray(t.corresponsables) && t.corresponsables.includes(id))) && !TEMA_CERRADOS.includes(estadoTema(t)))
    return {
      id, nombre: u.nombre || u.correo || id, jefe: u.reporta_a,
      tAb: tAb.length, tVen: tAb.filter(t => t.fecha_vencimiento && t.fecha_vencimiento < h).length,
      cAb: cAb.length, cVen: cAb.filter(t => t.fecha_compromiso && t.fecha_compromiso < h).length,
      cQui: cAb.filter(quieto).length, cSinF: cAb.filter(t => !t.fecha_compromiso).length
    }
  }).sort((a, b) => (b.tVen + b.cVen) - (a.tVen + a.cVen) || a.nombre.localeCompare(b.nombre)), [subs, usuarios, tareas, temas, avMap])
  const escaladosAMi = temas.filter(t => t.escalado_a === cuId && !TEMA_CERRADOS.includes(estadoTema(t)))
  const vencidos = useMemo(() => [
    ...tareas.filter(t => subs.has(t.responsable_id) && t.estado !== "completada" && t.fecha_vencimiento && t.fecha_vencimiento < h)
      .map(t => ({ k: "t" + t.id, tipo: "Tarea", txt: t.titulo, quien: t.responsable_id, f: t.fecha_vencimiento, ctx: t.proyecto_id ? nombreProy(t.proyecto_id) : (t.origen_tipo === "rutina" ? "↻ Rutina" : "") })),
    ...temas.filter(t => subs.has(t.responsable_id) && !TEMA_CERRADOS.includes(estadoTema(t)) && t.fecha_compromiso && t.fecha_compromiso < h)
      .map(t => ({ k: "c" + t.id, tipo: "Compromiso", txt: t.tema || t.acuerdo, quien: t.responsable_id, f: t.fecha_compromiso, ctx: t.escalado_a ? "⬆ escalado" : "" }))
  ].sort((a, b) => String(a.f).localeCompare(String(b.f))), [tareas, temas, subs])
  const dias = f => Math.round((new Date(h) - new Date(f)) / 864e5)
  const tot = filas.reduce((a, x) => ({ ab: a.ab + x.tAb + x.cAb, ven: a.ven + x.tVen + x.cVen }), { ab: 0, ven: 0 })
  const th = { fontSize: 11, fontWeight: 700, color: "#8E8E93", textTransform: "uppercase", letterSpacing: "0.03em", textAlign: "left", padding: "8px 10px", whiteSpace: "nowrap" }
  const num = (v, rojo) => <span style={{ fontWeight: 800, color: v ? (rojo ? "#A32D2D" : "#1C1C1E") : "#C7C7CC" }}>{v}</span>
  if (loading) return <div style={{ textAlign: "center", padding: 40, color: "#8E8E93", fontSize: 13 }}>Cargando...</div>
  return (
    <>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(4,1fr)", gap: 10, marginBottom: 14 }}>
        <KpiCard l="Personas a cargo" v={filas.length} sub="directos e indirectos" c={NAVY} />
        <KpiCard l="Abierto del equipo" v={tot.ab} sub="tareas + compromisos" c="#185FA5" />
        <KpiCard l="Vencido del equipo" v={tot.ven} sub="requiere conversación" c={tot.ven ? "#A32D2D" : "#8E8E93"} />
        <KpiCard l="Escalado hacia ti" v={escaladosAMi.length} sub="lo cierras tú" c={escaladosAMi.length ? "#854F0B" : "#8E8E93"} />
      </div>
      <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #eceef3", overflow: "hidden", marginBottom: 16 }}>
        <div style={{ overflowX: "auto" }}>
          <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 13 }}>
            <thead><tr style={{ background: "#fafbfd" }}>
              <th style={th}>Persona</th><th style={th}>Reporta a</th><th style={th}>Tareas abiertas</th><th style={th}>Tareas vencidas</th>
              <th style={th}>Compromisos abiertos</th><th style={th}>Vencidos</th><th style={th}>Sin mov. 14 d</th><th style={th}>Sin fecha</th>
            </tr></thead>
            <tbody>{filas.map(x => (
              <tr key={x.id} className="pmo-tr">
                <td className="pmo-td" style={{ fontWeight: 700 }}>{x.nombre}</td>
                <td className="pmo-td" style={{ color: "#8E8E93", fontSize: 12 }}>{x.jefe === cuId ? "Directo" : nombreDe(x.jefe)}</td>
                <td className="pmo-td">{num(x.tAb)}</td><td className="pmo-td">{num(x.tVen, true)}</td>
                <td className="pmo-td">{num(x.cAb)}</td><td className="pmo-td">{num(x.cVen, true)}</td>
                <td className="pmo-td">{num(x.cQui, true)}</td><td className="pmo-td">{num(x.cSinF, true)}</td>
              </tr>
            ))}</tbody>
          </table>
        </div>
      </div>
      {escaladosAMi.length > 0 && (
        <div style={{ background: "#FFF8EC", border: "1px solid #f3dcb0", borderRadius: 12, padding: "10px 14px", marginBottom: 16 }}>
          <div style={{ fontSize: 12.5, fontWeight: 800, color: "#854F0B", marginBottom: 6 }}>⬆ Escalado hacia ti ({escaladosAMi.length}) — destrábalo desde Mi semana o en el acta</div>
          {escaladosAMi.slice(0, 12).map(t => (
            <div key={t.id} style={{ fontSize: 12.5, padding: "3px 0", color: "#3A3A3C" }}>• <b>{nombreDe(t.responsable_id)}</b>: {(t.tema || t.acuerdo || "").slice(0, 110)} <span style={{ color: "#AEAEB2" }}>· {t.escalado_motivo || ""}</span></div>
          ))}
        </div>
      )}
      <div style={{ fontSize: 13, fontWeight: 700, color: "#3A3A3C", marginBottom: 8 }}>Lo vencido del equipo ({vencidos.length})</div>
      {!vencidos.length ? (
        <div style={{ textAlign: "center", padding: 28, background: "#fff", borderRadius: 14, border: "1px solid #eceef3", color: "#27500A", fontSize: 13 }}>Tu equipo no tiene nada vencido. 👌</div>
      ) : (
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #eceef3", overflow: "hidden" }}>
          {vencidos.slice(0, 40).map(v => (
            <div key={v.k} style={{ display: "flex", gap: 10, alignItems: "center", padding: "8px 14px", borderTop: "1px solid #eceef3", fontSize: 12.5 }}>
              <span style={{ fontSize: 10.5, fontWeight: 800, color: v.tipo === "Tarea" ? "#185FA5" : "#3C3489", width: 78, flexShrink: 0 }}>{v.tipo.toUpperCase()}</span>
              <span style={{ flex: 1, minWidth: 0, overflow: "hidden", textOverflow: "ellipsis", whiteSpace: "nowrap" }}><b>{v.txt}</b>{v.ctx ? <span style={{ color: "#AEAEB2" }}> · {v.ctx}</span> : null}</span>
              <span style={{ color: "#5F5E5A", whiteSpace: "nowrap" }}>{nombreDe(v.quien)}</span>
              <span style={{ color: "#A32D2D", fontWeight: 800, whiteSpace: "nowrap", width: 86, textAlign: "right" }}>{dias(v.f)} d vencido</span>
            </div>
          ))}
        </div>
      )}
...
/* ═══ SALUD DE LA GESTIÓN — tablero de gerencia ═══
   Mide si el sistema se está usando bien: vencimientos, quietud, compromisos sin plazo, escalamientos,
   alineación a metas y última actividad, por área y por persona. Solo gerencia (standalone, esGlobal). */
function SaludPanel({ temas, tareas, reuniones, proyectos, objetivos, temaAvances, usuarios, estadoTema, nombreDe, isMobile }) {
  const h = hoy()
  const d = useMemo(() => {
    const byId = {}; reuniones.forEach(r => { byId[r.id] = r })
    const raiz = r => { let x = r, g = 0; while (x && x.reunion_padre_id && byId[x.reunion_padre_id] && g++ < 100) x = byId[x.reunion_padre_id]; return x }
    const proyArea = {}; proyectos.forEach(p => { proyArea[p.id] = p.area })
    const ultAv = {}; temaAvances.forEach(a => { if (a.autor_id && (!ultAv[a.tema_id] || a.created_at > ultAv[a.tema_id])) ultAv[a.tema_id] = a.created_at })
    const diasDe = f => f ? Math.round((new Date(h) - new Date(String(f).slice(0, 10))) / 864e5) : null
    const areaTema = t => { const r = raiz(byId[t.reunion_id]); return (r && r.area) || proyArea[t.proyecto_id] || "_trans" }
    const ab = temas.filter(t => t.responsable_id && !TEMA_CERRADOS.includes(estadoTema(t)))
    const exig = temas.filter(t => t.responsable_id && !["aprobada", "permanente"].includes(estadoTema(t)))
    const tAb = tareas.filter(t => t.estado !== "completada")
    const met = arr => {
      const n = arr.length, pc = k => n ? Math.round(100 * arr.filter(k).length / n) : 0
      return { n, ven: pc(t => t.fecha_compromiso && t.fecha_compromiso < h), qui: pc(t => (diasDe(ultAv[t.id] || t.created_at) ?? 0) >= 14),
        sinF: pc(t => !t.fecha_compromiso), esc: pc(t => t.escalado_a), ali: pc(t => t.objetivo_id) }
    }
    const tasa = arr => { const e = arr.length; return e ? Math.round(100 * arr.filter(t => estadoTema(t) === "cumplido").length / e) : null }
    const areas = [...Object.keys(AREAS), "_trans"].map(k => {
      const a = ab.filter(t => areaTema(t) === k), e = exig.filter(t => areaTema(t) === k)
      const ta = tAb.filter(t => (proyArea[t.proyecto_id] || "_trans") === k)
      return { k, l: k === "_trans" ? "Transversal" : AREAS[k].l, ...met(a), tasa: tasa(e), tAb: ta.length, tVen: ta.filter(t => t.fecha_vencimiento && t.fecha_vencimiento < h).length }
    }).filter(x => x.n || x.tAb)
    const resp = [...new Set([...ab.map(t => t.responsable_id), ...tAb.map(t => t.responsable_id)].filter(Boolean))].map(id => {
      const a = ab.filter(t => t.responsable_id === id), e = exig.filter(t => t.responsable_id === id), ta = tAb.filter(t => t.responsable_id === id)
      const ult = [...temaAvances.filter(x => x.autor_id === id).map(x => x.created_at), ...tareas.filter(t => t.responsable_id === id).map(t => t.updated_at)].filter(Boolean).sort().pop()
      return { id, nombre: nombreDe(id), ...met(a), tasa: tasa(e), tAb: ta.length, tVen: ta.filter(t => t.fecha_vencimiento && t.fecha_vencimiento < h).length, ult: ult ? String(ult).slice(0, 10) : null }
    }).sort((x, y) => (y.tVen + Math.round(y.ven * y.n / 100)) - (x.tVen + Math.round(x.ven * x.n / 100)))
    const ultima = [...temas.map(t => t.created_at), ...tareas.map(t => t.created_at), ...temaAvances.filter(a => a.autor_id).map(a => a.created_at)].filter(Boolean).sort().pop()
    return { g: met(ab), tasaG: tasa(exig), tAb: tAb.length, tVenPct: tAb.length ? Math.round(100 * tAb.filter(t => t.fecha_vencimiento && t.fecha_vencimiento < h).length / tAb.length) : 0,
      metas: objetivos.filter(o => o.estado === "activo").length, ultima: ultima ? String(ultima).slice(0, 10) : null, diasUlt: diasDe(ultima), areas, resp }
  }, [temas, tareas, reuniones, proyectos, objetivos, temaAvances, usuarios])
  // semáforo: valor bajo es bueno salvo alineación y tasa (alto es bueno)
  const sem = (v, am, rj, inverso) => { if (v === null || v === undefined) return "#8E8E93"; const x = inverso ? 100 - v : v; return x <= am ? "#3B6D11" : x <= rj ? "#854F0B" : "#A32D2D" }
  const exportar = () => {
    const wb = XLSX.utils.book_new()
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(d.areas.map(a => ({ "Área": a.l, "Compromisos abiertos": a.n, "% vencidos": a.ven, "% sin mov 14d": a.qui, "% sin fecha": a.sinF, "% escalados": a.esc, "% alineados a meta": a.ali, "Cumplimiento %": a.tasa, "Tareas abiertas": a.tAb, "Tareas vencidas": a.tVen }))), "Por área")
    XLSX.utils.book_append_sheet(wb, XLSX.utils.json_to_sheet(d.resp.map(r => ({ "Persona": r.nombre, "Compromisos abiertos": r.n, "% vencidos": r.ven, "% sin mov 14d": r.qui, "% sin fecha": r.sinF, "% escalados": r.esc, "Cumplimiento %": r.tasa, "Tareas abiertas": r.tAb, "Tareas vencidas": r.tVen, "Última gestión": r.ult || "" }))), "Por persona")
    XLSX.writeFile(wb, "salud_gestion_" + hoy() + ".xlsx")
  }
  const th = { fontSize: 10.5, fontWeight: 700, color: "#8E8E93", textTransform: "uppercase", letterSpacing: "0.03em", textAlign: "right", padding: "7px 9px", whiteSpace: "nowrap" }
  const td = (v, c, suf = "%") => <td className="pmo-td" style={{ textAlign: "right", fontWeight: 700, color: c, whiteSpace: "nowrap" }}>{v === null || v === undefined ? "—" : v + suf}</td>
  const g = d.g
  return (
    <div style={{ marginBottom: 22 }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", gap: 10, flexWrap: "wrap", marginBottom: 10 }}>
        <div>
          <div style={{ fontSize: 14, fontWeight: 800, color: "#1C1C1E" }}>🩺 Salud de la gestión</div>
          <div style={{ fontSize: 12, color: "#8E8E93" }}>¿Se está usando bien el sistema? Lo que no se registra no existe: estos números miden disciplina, no resultados.</div>
        </div>
        <Bt v="gry" sm ic="📥" onClick={exportar}>Excel</Bt>
      </div>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2,1fr)" : "repeat(8,1fr)", gap: 8, marginBottom: 12 }}>
        <KpiCard l="Compromisos abiertos" v={g.n} sub="con responsable" c={NAVY} />
        <KpiCard l="Vencidos" v={g.ven + "%"} sub="meta ≤ 10%" c={sem(g.ven, 10, 25)} />
        <KpiCard l="Sin movimiento" v={g.qui + "%"} sub="+14 d · meta ≤ 20%" c={sem(g.qui, 20, 40)} />
        <KpiCard l="Sin fecha" v={g.sinF + "%"} sub="meta 0%" c={sem(g.sinF, 0, 10)} />
        <KpiCard l="Escalados" v={g.esc + "%"} sub="meta ≤ 15%" c={sem(g.esc, 15, 30)} />
        <KpiCard l="Alineados a meta" v={g.ali + "%"} sub="meta ≥ 60%" c={sem(g.ali, 40, 70, true)} />
        <KpiCard l="Metas activas" v={d.metas} sub="ideal 3 a 5" c={d.metas >= 3 && d.metas <= 5 ? "#3B6D11" : "#854F0B"} />
        <KpiCard l="Última actividad" v={d.diasUlt === null ? "—" : d.diasUlt + " d"} sub={d.ultima ? "el " + fFecha(d.ultima) : "sin registros"} c={sem(d.diasUlt, 2, 7)} />
      </div>
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "1fr 1.4fr", gap: 12 }}>
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #eceef3", overflow: "hidden" }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: "#3A3A3C", padding: "10px 12px 4px" }}>Por área</div>
          <div style={{ overflowX: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
              <thead><tr><th style={{ ...th, textAlign: "left" }}>Área</th><th style={th}>Abiertos</th><th style={th}>Venc.</th><th style={th}>Quietos</th><th style={th}>S/fecha</th><th style={th}>Cumpl.</th><th style={th}>Tareas venc.</th></tr></thead>
              <tbody>{d.areas.map(a => (
                <tr key={a.k} className="pmo-tr">
                  <td className="pmo-td" style={{ fontWeight: 700 }}>{a.l}</td>{td(a.n, "#1C1C1E", "")}{td(a.ven, sem(a.ven, 10, 25))}{td(a.qui, sem(a.qui, 20, 40))}{td(a.sinF, sem(a.sinF, 0, 10))}{td(a.tasa, sem(a.tasa, 20, 50, true))}{td(a.tVen, a.tVen ? "#A32D2D" : "#C7C7CC", "")}
                </tr>
              ))}</tbody>
            </table>
          </div>
        ...
        <div style={{ background: "#fff", borderRadius: 14, border: "1px solid #eceef3", overflow: "hidden" }}>
          <div style={{ fontSize: 12, fontWeight: 800, color: "#3A3A3C", padding: "10px 12px 4px" }}>Por persona <span style={{ fontWeight: 500, color: "#AEAEB2" }}>· de más a menos vencido</span></div>
          <div style={{ overflowX: "auto", maxHeight: 360, overflowY: "auto" }}>
            <table style={{ width: "100%", borderCollapse: "collapse", fontSize: 12.5 }}>
              <thead><tr><th style={{ ...th, textAlign: "left" }}>Persona</th><th style={th}>Abiertos</th><th style={th}>Venc.</th><th style={th}>Quietos</th><th style={th}>S/fecha</th><th style={th}>Cumpl.</th><th style={th}>Tareas venc.</th><th style={th}>Últ. gestión</th></tr></thead>
              <tbody>{d.resp.map(r => (
                <tr key={r.id} className="pmo-tr">
                  <td className="pmo-td" style={{ fontWeight: 700, whiteSpace: "nowrap" }}>{r.nombre}</td>{td(r.n, "#1C1C1E", "")}{td(r.ven, sem(r.ven, 10, 25))}{td(r.qui, sem(r.qui, 20, 40))}{td(r.sinF, sem(r.sinF, 0, 10))}{td(r.tasa, sem(r.tasa, 20, 50, true))}{td(r.tVen, r.tVen ? "#A32D2D" : "#C7C7CC", "")}
                  <td className="pmo-td" style={{ textAlign: "right", fontSize: 12, color: "#5F5E5A", whiteSpace: "nowrap" }}>{r.ult ? fFecha(r.ult) : "nunca"}</td>
                </tr>
              ))}</tbody>
            </table>
          </div>
