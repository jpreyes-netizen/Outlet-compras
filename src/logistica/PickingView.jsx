...
// Módulo Picking & Entrega (retail): cola BSALE, checklist 2 fases,
// comprobantes, respaldos y reporte. Extraído del monolito
// LogisticaApp.jsx (líneas ~23021-24151) en Fase 0 del refactor.
// ============================================================
...
function PickingView({cu, sucs}) {
  ...
  const [pickTurno,setPickTurno]= useState(()=>{try{return localStorage.getItem('picking_turno_'+(sucPropia||'cd'))||''}catch(e){return ''}})
  ...
  useEffect(()=>{ try{localStorage.setItem('picking_turno_'+(sucPropia||'cd'), pickTurno)}catch(e){} },[pickTurno])
  ...
  const tomar = async (orden)=>{
    const t = trabs.find(x=>x.id===pickTurno)
    if(!t){ flash('⚠️ Selecciona el pickeador en turno (arriba) antes de tomar'); return }
    setBusy(true)
    const nom = `${t.nombre} ${t.apellido||''}`.trim()
    const {error,data} = await supabase.from('log_picking_ordenes').update({
      estado:'asignada', pickeador_id:t.id, pickeador_nombre:nom,
  ...
  const checkBtn = (on,color)=>({width:26,height:26,borderRadius:7,border:on?'none':'2px solid #D1D1D6',
    background:on?color:'#fff',color:'#fff',fontSize:14,fontWeight:900,cursor:'pointer',lineHeight:'22px',padding:0})
  const stepBtn = {width:20,height:20,borderRadius:5,border:'1px solid #D1D1D6',background:'#fff',color:'#3A3A3C',
    fontSize:12,fontWeight:900,cursor:'pointer',lineHeight:'16px',padding:0}
      ...
      <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
        <div style={{padding:'11px 14px',borderBottom:'1px solid #E5E5EA',background:'#FAFAFC'}}>
          ...
          <div style={{fontSize:11,color:'#8E8E93',marginTop:3,display:'flex',gap:10,flexWrap:'wrap',alignItems:'center'}}>
            ...
            {sel.pickeador_nombre&&<span>🏃 {sel.pickeador_nombre}</span>}
        ...
        <div style={{flex:1,overflowY:'auto'}}>
          ...
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            <thead><tr>
              <th style={{...th,width:34,textAlign:'center'}} title="Fase 1: validado en picking">✔₁</th>
              <th style={{...th,width:34,textAlign:'center'}} title="Fase 2: entregado al cliente">✔₂</th>
              <th style={th}>PRODUCTO</th>
              <th style={{...th,textAlign:'center',width:92}}>VALIDADO</th>
              <th style={{...th,textAlign:'right',width:44}}>CANT</th>
              <th style={{...th,textAlign:'right',width:44}}></th>
            </tr></thead>
            <tbody>
              {items.map(it=>{
                ...
                return (<React.Fragment key={it.id}>
                  <tr style={{background: flashRow?'#34C75925': conMotivo?'#FF3B3006': entOK?'#34C75908': valOK?'#5856D608':'transparent',
                    ...
                    <td style={{...td,whiteSpace:'normal'}}>
                      <div style={{fontWeight:700,fontSize:12.5}}>{flete?'🚚 ':''}{it.producto}</div>
                      <div style={{fontSize:10,color:'#8E8E93',fontFamily:'monospace'}}>{it.sku||'—'}</div>
                      {conMotivo&&<div style={{fontSize:10.5,color:'#FF3B30',fontWeight:700}}>✗ {it.motivo_no_entrega} · disp. {val}/{cant}</div>}
                    </td>
                    <td style={{...td,textAlign:'center'}}>
                    ...
                    <td style={{...td,textAlign:'right',fontWeight:900,fontSize:15}}>{cant}</td>
        ...
        <div style={{padding:'10px 14px',borderTop:'1px solid #E5E5EA',background:'#FAFAFC'}}>
          {sel.estado==='pendiente'&&(
            <button style={{...btMini('#007AFF','#fff'),width:'100%',padding:'10px 0',fontSize:13}}
              disabled={busy} onClick={()=>tomar(sel)}>🏃 Tomar esta orden ({trabs.find(t=>t.id===pickTurno)?trabs.find(t=>t.id===pickTurno).nombre:'selecciona pickeador arriba'})</button>
          )}
          {enPicking&&(
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10,flexWrap:'wrap'}}>
        ...
        {tabVista==='cola'&&(
          <span style={{display:'inline-flex',alignItems:'center',gap:6}}>
            <span style={{fontSize:11,fontWeight:700,color:'#6D6D72'}}>🏃 En turno:</span>
            <select value={pickTurno} onChange={e=>setPickTurno(e.target.value)} style={{...css.select,width:180,padding:'6px 10px'}}>
              <option value="">— seleccionar —</option>
              {(sucSel!=='todas'?trabsDeSuc(sucSel):trabs).map(t=>(
      ...
      {tabVista==='cola'&&(
        ...
        <div style={{display:'flex',gap:12,alignItems:'stretch',flexWrap:'wrap'}}>
          <div style={{flex:'1 1 470px',minWidth:390,border:'1px solid #E5E5EA',borderRadius:10,
            ...
            {/* Bandejas */}
            <div style={{display:'flex',gap:2,padding:'8px 8px 0',background:'#FAFAFC',position:'sticky',top:0,zIndex:2}}>
              {[['inmediata',`⚡ Entrega inmediata · ${inmediatas.length}`],
                ['programada',`🚚 Por despachar · ${programadas.length}${progVencidas?` (${progVencidas} para hoy ⚠)`:''}`]].map(([k,l])=>(
                <button key={k} onClick={()=>setBandeja(k)}
                  style={{flex:1,padding:'7px 0',border:'none',borderRadius:'8px 8px 0 0',fontSize:11.5,fontWeight:800,cursor:'pointer',
                    background:bandeja===k?'#fff':'transparent',
                    color:bandeja===k?(k==='programada'?'#5856D6':'#1C1C1E'):'#8E8E93',
                    borderBottom:bandeja===k?'2px solid '+(k==='programada'?'#5856D6':'#007AFF'):'2px solid transparent'}}>{l}</button>
              ))}
            </div>
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead><tr>
