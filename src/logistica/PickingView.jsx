...
function PickingView({cu, sucs}) {
  ...
  const marcarLista = async ()=>{
    ...
    if(!sel?.pickeador_id){
      ...
      setPickerSel({orden:sel, reasignar:false, alCerrar:true})
      return
    }
  ...
  const confirmarEntrega = async ()=>{
    ...
    // Entrega PARCIAL: exige el número de la Nota de Crédito emitida en el POS
    const hayFaltantes = items.some(i=>Number(i.cantidad_confirmada)<Number(i.cantidad))
    if(hayFaltantes && !ncParcial.trim()){ flash('⚠️ Entrega parcial: registra el N° de la Nota de Crédito'); return }
    setGenPdf(true)
    try{
      const ahora = new Date().toISOString()
      ...
      const dibujarCopia = (badge)=>{
        ...
        // Pie
        doc.setFontSize(7.5)
        doc.text(`Emitida ${fmtT(sel.emitida_at)} · En cola ${fmtT(sel.recibida_at)} · Lista ${fmtT(sel.lista_at)} · ${completa?'ENTREGA COMPLETA':`ENTREGA PARCIAL — NC ${ncParcial.trim()}`}`, 14, 286)
      ...
      if(error) throw error
      await etapaCerrar(sel)
      flash(`✅ #${sel.folio} entregada${completa?'':` (parcial — NC ${ncParcial.trim()})`} — comprobante en 2 copias generado`)
          ...
          {enEntrega&&(
            <div>
              ...
              <button style={{...btMini((receptor.trim()&&(faltantes.length===0||ncParcial.trim()))?'#1a1a2e':'#E5E5EA', (receptor.trim()&&(faltantes.length===0||ncParcial.trim()))?'#fff':'#8E8E93'),
                ...
                disabled={!receptor.trim()||(faltantes.length>0&&!ncParcial.trim())||genPdf} onClick={confirmarEntrega}>
