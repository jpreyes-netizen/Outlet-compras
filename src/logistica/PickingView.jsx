...
function PickingView({cu, sucs}) {
  ...
  const confirmarEntrega = async ()=>{
    ...
    const esDespacho = sel.modalidad_entrega==='despacho'
    const chofer = esDespacho ? choferes.find(c=>c.id===choferSel) : null
    if(esDespacho && !chofer){ flash('⚠️ Selecciona el chofer que entrega el despacho'); return }
    setGenPdf(true)
    try{
      const ahora = new Date().toISOString()
      ...
      const dibujarCopia = (badge)=>{
        ...
        if(chofer) linea('Chofer que entrega:', `${chofer.nombre} ${chofer.apellido||''}`.trim()+(chofer.patente?` · ${chofer.patente}`:''))
        if(esDespacho&&sel.despacho_pago){
          linea('Despacho:', sel.despacho_pago==='pagado'
            ? `PAGADO · ${fmtCLP(sel.despacho_valor)}`
            : `POR PAGAR · ${fmtCLP(sel.despacho_valor)} — cobrar al recibir`)
        }
      ...
      const {error} = await supabase.from('log_picking_ordenes').update({
        ...
        ...(chofer?{chofer_id:chofer.id, chofer_patente:chofer.patente||null,
          chofer_nombre:`${chofer.nombre} ${chofer.apellido||''}`.trim()}:{}),
      }).eq('id',sel.id)
      if(error) throw error
      await etapaCerrar(sel)
  ...
  const listaPickingPDF = async ()=>{
    ...
    doc.text(sel.bsale_doc_type==='factura'?'FACTURA':'BOLETA', W-14, 13, {align:'right'})
    ...
    autoTable(doc,{ startY:y, styles:{fontSize:10.5,cellPadding:3.4}, headStyles:{fillColor:[26,26,46],fontSize:9.5},
      head:[['SKU','PRODUCTO','CANT','CHECK']],
      body: items.map(i=>[ i.sku||'—', i.producto, String(i.cantidad), '' ]),
      columnStyles:{0:{cellWidth:36,fontSize:9},2:{cellWidth:18,halign:'right',fontStyle:'bold',fontSize:12},
        3:{cellWidth:20,halign:'center'}},
      alternateRowStyles:{fillColor:[248,249,252]},
      didDrawCell: (d)=>{
        if(d.section!=='body'||d.column.index!==3) return
        const s = 6.4
        const cx = d.cell.x + d.cell.width/2 - s/2
        const cy = d.cell.y + d.cell.height/2 - s/2
        doc.setDrawColor(90,90,110); doc.setLineWidth(0.45)
    ...
    doc.setFillColor(247,249,252); doc.roundedRect(14,y-4,182,44,2,2,'F')
  ...
  const exportarReporte = async ()=>{
    ...
    const filas = repData.rows.map(o=>({
      ...
      Chofer:o.chofer_nombre||'', ChoferPatente:o.chofer_patente||'',
  ...
  const PanelOrden = ()=>{
    ...
    const faltaChofer = sel.modalidad_entrega==='despacho' && !choferSel
    const entregaOK = !!receptor.trim() && !faltaChofer
        ...
        <div style={{padding:'10px 14px',borderTop:'1px solid #E5E5EA',background:'#FAFAFC'}}>
          ...
          {enEntrega&&(
            <div>
              ...
              {sel.modalidad_entrega==='despacho'&&(
                <div style={{marginBottom:8,padding:'8px 10px',borderRadius:9,background:'#5856D608',border:'1px solid #5856D625'}}>
                  <div style={{fontSize:10,fontWeight:800,color:'#5856D6',marginBottom:5,letterSpacing:'0.03em'}}>
                    🚛 CHOFER QUE ENTREGA EL DESPACHO *
                  ...
                  <select value={choferSel} onChange={e=>setChoferSel(e.target.value)}
                    style={{...css.select,width:'100%',padding:'8px 10px'}}>
              ...
              <button style={{...btMini(entregaOK?'#1a1a2e':'#E5E5EA', entregaOK?'#fff':'#8E8E93'),
                width:'100%',padding:'11px 0',fontSize:13}}
                disabled={!entregaOK||genPdf} onClick={confirmarEntrega}>
                {genPdf?'Generando comprobante…'
                  :!receptor.trim()?'Indica quién recibe'
                  :faltaChofer?'Selecciona el chofer que entrega'
