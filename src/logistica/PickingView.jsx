...
function PickingView({cu, sucs}) {
  ...
  const inmediatas = activasAll.filter(o=>o.tipo_entrega!=='programada').filter(pasaSeg)
  const programadasAll = activasAll.filter(o=>o.tipo_entrega==='programada').filter(pasaSeg)
          ...
          <div style={{flex:'1 1 470px',minWidth:390,border:'1px solid #E5E5EA',borderRadius:10,background:'#fff',
            ...
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              ...
              <tbody>
                ...
                {bandeja==='programada'&&esCD&&porConfirmar.length>0&&(<>
                  <tr onClick={()=>setVerPorConf(v=>!v)} style={{cursor:'pointer'}}>
                    <td colSpan={sucSel==='todas'?8:7}
                      ...
                      <span style={{float:'right',fontWeight:900}}>{verPorConf?'▲ plegar':'▼ ver'}</span>
                    </td></tr>
                  {verPorConf&&porConfirmar.map(o=><FilaCola key={o.id} o={o}/>)}
                </>)}
                {bandeja==='inmediata'&&(entregadasHoy.length>0||anuladas.length>0||desistidas.length>0)&&(
                  <tr><td colSpan={sucSel==='todas'?8:7} style={{...th,fontSize:9,paddingTop:10}}>
                    TERMINADAS HOY · {entregadasHoy.length} entregadas{anuladas.length?` · ${anuladas.length} anuladas`:''}{desistidas.length?` · ${desistidas.length} desistidas`:''}</td></tr>
                )}
                ...
                {bandeja==='inmediata'&&desistidas.slice(0,8).map(o=><FilaCola key={o.id} o={o} terminada/>)}
              </tbody>
            </table>
          </div>
