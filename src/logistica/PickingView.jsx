...
function PickingView({cu, sucs}) {
  ...
  const PanelOrden = ()=>{
    ...
    const salidaOK = !!choferSel && !!guiaNum.trim()
        ...
        <div style={{padding:'10px 14px',borderTop:'1px solid #E5E5EA',background:'#FAFAFC'}}>
          ...
          {enSalida&&(
            <div>
              {faltantes.length>0&&(
                <div style={{fontSize:10.5,color:'#C93400',fontWeight:700,marginBottom:6}}>
                  ◐ Salida PARCIAL — sin preparar: {faltantes.map(f=>`${f.producto} (${Number(f.cantidad)-Number(f.cantidad_confirmada)})`).join(' · ')}
                </div>
              )}
            ...
            <div style={{overflowX:'auto',border:'1px solid #E5E5EA',borderRadius:10}}>
              <table style={{width:'100%',borderCollapse:'collapse'}}>
                ...
                <tbody>
                  {filtradas.map(o=>(
                    <tr key={o.id}>
                      <td style={{...td,fontFamily:'monospace',fontWeight:800}}>#{o.folio} <span style={{fontSize:9,color:'#8E8E93'}}>{DOC_S(o.bsale_doc_type)}</span>
