...
function PickingView({cu, sucs}) {
  ...
  const checkBtn = (on,color)=>({width:38,height:38,borderRadius:10,border:on?'none':'2px solid #D1D1D6',
    background:on?color:'#fff',color:'#fff',fontSize:18,fontWeight:900,cursor:'pointer',lineHeight:'34px',padding:0})
  const stepBtn = {width:32,height:32,borderRadius:8,border:'1px solid #D1D1D6',background:'#fff',color:'#3A3A3C',
    fontSize:17,fontWeight:900,cursor:'pointer',lineHeight:'28px',padding:0}
  const tdCk = {...td,padding:'12px 8px'}   // celdas del checklist: mas aire vertical
                ...
                return (<React.Fragment key={it.id}>
                  <tr style={{background: flashRow?'#34C75925': conMotivo?'#FF3B3006': entOK?'#34C75908': valOK?'#5856D608':'transparent',
                    ...
                    <td style={{...tdCk,whiteSpace:'normal'}}>
                      <div style={{fontWeight:700,fontSize:14,lineHeight:1.3}}>{flete?'🚚 ':''}{it.producto}</div>
                      <div style={{fontSize:11.5,color:'#8E8E93',fontFamily:'monospace',marginTop:2}}>{it.sku||'—'}</div>
                      {conMotivo&&<div style={{fontSize:11,color:'#FF3B30',fontWeight:700,marginTop:2}}>✗ {it.motivo_no_entrega} · disp. {val}/{cant}</div>}
                    </td>
                    <td style={{...tdCk,textAlign:'center'}}>
                      {enPicking&&!conMotivo&&cant>1?(
                        <span style={{display:'inline-flex',alignItems:'center',gap:6}}>
                          <button style={stepBtn} onClick={()=>validarItem(it, val-1)}>−</button>
                          <span style={{fontSize:16,fontWeight:900,minWidth:26,color:valOK?'#5856D6':val>0?'#FF9500':'#C7C7CC'}}>{val}</span>
                          <button style={stepBtn} onClick={()=>validarItem(it, val+1)}>+</button>
                        </span>
                      ):(
                        <span style={{fontSize:16,fontWeight:900,color:valOK?'#5856D6':val>0?'#FF9500':'#C7C7CC'}}>{val}</span>
                      )}
                    </td>
                    <td style={{...tdCk,textAlign:'right',fontWeight:900,fontSize:18}}>{cant}</td>
