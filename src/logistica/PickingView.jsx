    ...
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10,flexWrap:'wrap'}}>
        ...
        <div style={{display:'inline-flex',background:'#F2F2F7',borderRadius:9,padding:2}}>
          {[['cola','Cola'],['respaldos','Respaldos'],['reporte','Reporte'],['pantalla','📺 Pantalla']].map(([k,l])=>(
      ...
      {tabVista==='pantalla'&&(()=>{
        // Pantalla publica para el cliente en sala. Solo ordenes de RETIRO
        // (un despacho a domicilio no tiene cliente esperando en tienda).
        // Privacidad: se muestra folio + primer nombre, nunca RUT ni nombre completo.
        ...
        const col = (titulo, ic, lista, color, grande)=>(
          <div style={{flex:1,minWidth:280,display:'flex',flexDirection:'column'}}>
            <div style={{background:color,color:'#fff',padding:'14px 18px',borderRadius:'14px 14px 0 0',
              display:'flex',alignItems:'center',justifyContent:'space-between'}}>
              <span style={{fontSize:pantallaFull?26:18,fontWeight:900,letterSpacing:'0.02em'}}>{ic} {titulo}</span>
              <span style={{fontSize:pantallaFull?26:18,fontWeight:900,background:'rgba(255,255,255,0.22)',
                padding:'2px 14px',borderRadius:20}}>{lista.length}</span>
            </div>
            <div style={{flex:1,background:'#fff',border:`2px solid ${color}`,borderTop:'none',borderRadius:'0 0 14px 14px',
              padding:14,display:'flex',flexWrap:'wrap',gap:10,alignContent:'flex-start',
              minHeight:pantallaFull?'62vh':240,overflowY:'auto'}}>
              {lista.length===0
                ? <div style={{width:'100%',textAlign:'center',color:'#C7C7CC',fontSize:pantallaFull?22:15,
                    fontWeight:700,padding:'30px 0'}}>—</div>
                : lista.map(o=>(
                  <div key={o.id} style={{background:grande?color+'12':'#F7F8FA',
                    border:`2px solid ${grande?color:'#E5E5EA'}`,borderRadius:12,
                    padding:pantallaFull?'14px 20px':'9px 14px',minWidth:pantallaFull?150:110,textAlign:'center'}}>
                    <div style={{fontFamily:'monospace',fontWeight:900,
                      fontSize:pantallaFull?(grande?46:36):(grande?26:22),
                      color:grande?color:'#1C1C1E',lineHeight:1.1}}>{o.folio}</div>
                    {primerNombre(o.cliente_nombre)&&
                      <div style={{fontSize:pantallaFull?16:12,fontWeight:700,color:'#6D6D72',marginTop:2}}>
        ...
        return (
          <div style={pantallaFull
            ? {position:'fixed',inset:0,zIndex:9500,background:'#0E0E1A',padding:'18px 20px',
               display:'flex',flexDirection:'column',gap:14,overflow:'hidden'}
            : {display:'flex',flexDirection:'column',gap:12}}>
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',flexWrap:'wrap',gap:10}}>
              <div>
                <div style={{fontSize:pantallaFull?30:17,fontWeight:900,color:pantallaFull?'#fff':'#1C1C1E'}}>
                  🛒 Estado de tu pedido
                </div>
                <div style={{fontSize:pantallaFull?15:11.5,color:pantallaFull?'#9A9AB0':'#8E8E93',fontWeight:600}}>
                  {sucSel==='todas'?'Todas las tiendas':nomSuc(sucSel)} · actualización automática
                </div>
              </div>
              <div style={{display:'flex',gap:8,alignItems:'center'}}>
                <button onClick={()=>setPantallaSon(s=>!s)} title="Aviso sonoro al quedar un pedido listo"
                  style={{...btMini(pantallaSon?'#34C75918':'#E5E5EA', pantallaSon?'#248A3D':'#8E8E93'),padding:'7px 12px'}}>
                  {pantallaSon?'🔔 Aviso ON':'🔕 Aviso OFF'}
                </button>
                <button onClick={()=>setPantallaFull(f=>!f)}
                  style={{...btMini('#007AFF','#fff'),padding:'7px 14px'}}>
                  {pantallaFull?'✕ Salir de pantalla completa':'⛶ Pantalla completa'}
                </button>
              </div>
            </div>
            <div style={{display:'flex',gap:14,flexWrap:'wrap',flex:1}}>
              {col('EN PREPARACIÓN','⏳',preparando,'#FF9500',false)}
              {col('LISTO PARA RETIRAR','✅',listas,'#34C759',true)}
            </div>
            {pantallaFull&&(
              <div style={{textAlign:'center',color:'#6D6D80',fontSize:13,fontWeight:600}}>
                Cuando tu número aparezca en verde, acércate al mostrador · Outlet de Puertas
              </div>
            )}
          </div>
        )
