...
const SKU_RETIRO_PROG='78693262654391'
const ES_RETIRO_PROG=(it)=>String(it?.sku||'').trim()===SKU_RETIRO_PROG
  ||/retiro\s*(programad|diferid|posterior)/i.test(it?.producto||'')
// Líneas de SERVICIO (despacho/flete/retiro programado): no se pickean, no cuentan unidades
const ES_FLETE = (it)=>/despacho|flete|env[ií]o/i.test(it.producto||'')||/despacho|flete/i.test(it.sku||'')
  ||ES_RETIRO_PROG(it)
// ── Entrega por zonas (Sucursal Maipú): bodega-primera vs segunda/descontinuados ──
const ES_SEGUNDA_TIPO  = t =>/(^|[^A-ZÁÉÍÓÚ0-9])(2DA|SEGUNDA)([^A-ZÁÉÍÓÚ0-9]|$)/i.test(t||'')
// SKUs marcados a mano en log_sku_segunda (productos que ni por tipo ni por
// nombre se delatan: modelos de 1ª que salieron de línea). Se carga al montar;
// agregar uno nuevo es un INSERT, NO requiere deploy.
const SKUS_SEGUNDA = new Map()   // sku → 'descontinuado' | 'segunda'
const MOTIVO_SKU = it => SKUS_SEGUNDA.get(String(it?.sku||'').trim()) || null
const ES_DESCONTINUADO = it=>/(^|[^A-ZÁÉÍÓÚ0-9])DESCONTINUAD[OA]S?([^A-ZÁÉÍÓÚ0-9]|$)/i.test(it?.producto||'')
  ||MOTIVO_SKU(it)==='descontinuado'
const zonaItem = it => (ES_SEGUNDA_TIPO(it?.tipo_producto)||ES_DESCONTINUADO(it)||MOTIVO_SKU(it)) ? 'segunda' : 'bodega'
const ZONA_L = {bodega:'🏬 ZONA BODEGA · PRIMERA', segunda:'⚠ ZONA SEGUNDA / DESCONTINUADOS'}
const ZONA_C = {bodega:'#007AFF', segunda:'#FF9500'}
const EST_PICK = {
  ...
  desistida:  {l:'DESISTIDA', c:'#E0457B'},
}
// Motivos predefinidos de desistimiento del cliente
const MOTIVOS_DESIST = [
  {k:'cambio_producto', l:'Cliente se arrepintió — cambió de producto', e:'🔄'},
  {k:'anular_compra',   l:'Cliente se arrepintió — desea anular la compra', e:'🚫'},
]
...
// ── Buscador de cola: normaliza para que el texto calce igual escriba como escriba ──
// Quita acentos, mayúsculas, puntos, guiones, espacios y el # del folio. Así
// "12.345.678-9" = "123456789", "#0084" = "84" y "Muñoz" = "munoz".
const normBusq = (s)=> String(s??'')
...
function PickingView({cu, sucs}) {
  ...
  // ── CANDADO DE SUCURSAL: la ubicación declarada al entrar al WMS
  //    (contexto_activo_sesion, pantalla "¿Desde dónde estás trabajando hoy?")
  //    define QUÉ cola ve el usuario. Solo admin/jefe_bodega/coordinador
  //    conservan el selector completo (monitoreo multi-tienda).
  //    La bodega auxiliar de LA opera la cola de su sucursal madre.
  const CTX_PICK = c => c==='la_bodega' ? 'la' : c
  const sucContexto = CTX_PICK(cu?.contexto_activo_sesion || cu?.sucursal_codigo || '') || null
  const sucPropia = !esCD ? sucContexto : null
  ...
  const [colaFull,setColaFull]=useState(false)           // tablero operativo bodega en modo TV
  const [colaMax,setColaMax]=useState(false)             // espacio de trabajo (lista+detalle) ampliado a toda la pantalla
  useEffect(()=>{                                         // ESC sale de cualquier modo TV
    if(!colaFull&&!pantallaFull&&!colaMax) return
    const h=e=>{ if(e.key==='Escape'){ setColaFull(false); setPantallaFull(false); setColaMax(false) } }
    window.addEventListener('keydown',h)
    return ()=>window.removeEventListener('keydown',h)
  },[colaFull,pantallaFull,colaMax])
  ...
  const [filSeg,   setFilSeg]   = useState('todos')     // todos | si | no — filtro 2ª selección
  const [colaTxt,  setColaTxt]  = useState('')          // buscador de la cola: folio · cliente · RUT · guía · dirección
  const [sucSel,   setSucSel]   = useState(sucPropia || 'todas')
  useEffect(()=>{ if(sucPropia && sucSel!==sucPropia) setSucSel(sucPropia) },[sucPropia,sucSel])
  ...
  const [retiroFile,setRetiroFile]=useState(null)        // comprobante de retiro firmado por el transportista externo
  ...
  const [desistirSel, setDesistirSel]= useState(null)   // orden en proceso de desistimiento
  const [desistDetalle, setDesistDetalle]= useState('')
  const [ncParcial, setNcParcial]     = useState('')     // NC obligatoria en entrega parcial
  const [reversarSel, setReversarSel] = useState(null)   // orden entregada en proceso de reversa
  const [reversaNC, setReversaNC]     = useState('')
  const [reversaDetalle, setReversaDetalle]= useState('')
  ...
  const cargar = async () => {
    try{
      ...
      let q = supabase.from('log_picking_ordenes').select('*')
        ...
        .or(`estado.in.(pendiente,asignada,en_picking,lista,despachada),and(estado.in.(entregada,anulada,desistida),recibida_at.gte.${desde})`)
  ...
  // Catálogo de SKUs marcados como segunda/descontinuado (una vez, al montar)
  const [skusSegVer,setSkusSegVer]=useState(0)
  useEffect(()=>{ (async()=>{
    const {data,error}=await supabase.from('log_sku_segunda').select('sku,motivo').eq('activo',true)
    if(error) return                                   // tabla aún sin crear → reglas por tipo/nombre siguen operando
    SKUS_SEGUNDA.clear()
    ;(data||[]).forEach(r=>SKUS_SEGUNDA.set(String(r.sku||'').trim(), r.motivo||'segunda'))
    setSkusSegVer(v=>v+1)                              // fuerza re-render con el catálogo ya cargado
  })() },[])
  ...
  useEffect(()=>{
    ...
    supabase.from('log_choferes')
      .select('id,nombre,apellido,patente,empresa_transporte,es_externo,activo')
  ...
  useEffect(()=>{
    ...
    // Firma: precarga el nombre/RUT del cliente de la boleta para no escribirlo
    // de nuevo; editable si retira una persona distinta al comprador.
    setReceptor(o?.receptor_nombre||o?.cliente_nombre||'')
    setReceptorRut(o?.receptor_rut||o?.cliente_rut||'')
    setMotivoEdit(null)
    setRechazoEdit(null)
  ...
  // ── Acciones de flujo ─────────────────────────────────────────────────────
  // TOMAR ya no exige elegir pickeador: la orden pasa a 'asignada' y se puede
  // imprimir la hoja de inmediato. QUIÉN hizo el picking se registra al CERRAR
  // (marcarLista), que es cuando el dato es cierto y verificable.
  const tomar = async (orden)=>{
    setBusy(true)
    const {error,data} = await supabase.from('log_picking_ordenes').update({
      estado:'asignada', asignada_por:cu?.nombre||'', asignada_at:new Date().toISOString(),
    }).eq('id',orden.id).eq('estado','pendiente').select('id')
    setBusy(false)
    if(error){ flash('⚠️ '+error.message); return }
    if(!data?.length){ flash('⚠️ Otro usuario tomó esta orden primero'); return }
    setOrdenes(prev=>prev.map(o=>o.id===orden.id?{...o,estado:'asignada'}:o))
    setSelId(orden.id)
    flash(`🏃 #${orden.folio} tomada — imprime la hoja y a pickear`)
    setTimeout(()=>scanRef.current?.focus(), 300)
  }
  // Asignar/cambiar pickeador sigue disponible como acción explícita (opcional
  // antes de cerrar, obligatoria al cerrar).
  const asignar   = (orden)=> setPickerSel({orden, reasignar:false, alCerrar:false})
  const reasignar = (orden)=> setPickerSel({orden, reasignar:true,  alCerrar:false})

  const asignarPicker = async (t)=>{
    // ── Fase 2 del cierre MIXTO: responsable de ZONA SEGUNDA (el de bodega ya quedó) ──
    if(pickerSel?.mixtaFase==='segunda'){
      const {orden, p1} = pickerSel
      const nom2 = `${t.nombre} ${t.apellido||''}`.trim()
      const combo = `${p1.nombre} + ${nom2}`
      setBusy(true)
      const {error} = await supabase.from('log_picking_ordenes').update({
        pickeador_id:p1.id, pickeador_nombre:combo,
      }).eq('id',orden.id)
      setBusy(false)
      if(error){ flash('⚠️ '+error.message); return }
      setOrdenes(prev=>prev.map(o=>o.id===orden.id?{...o,pickeador_id:p1.id,pickeador_nombre:combo}:o))
      // Etapa del pickeador de ZONA SEGUNDA (rendimiento acredita a ambos)
      supabase.from('log_modulo_etapa_trabajadores').insert({
        ...
        trabajador_id:t.id, nombre_trabajador:nom2, rol_etapa:t.rol_operativo||'',
        hora_inicio:orden.inicio_picking_at||orden.asignada_at||new Date().toISOString(),
        asignado_por_nombre:cu?.nombre||'Sistema', asignado_por_rol:cu?.rol||'',
      }).then(()=>{},()=>{})
      setPickerSel(null)
      flash(`👥 Picking mixto — bodega: ${p1.nombre} · segunda: ${nom2}`)
      await cerrarOrden({...orden, pickeador_id:p1.id, pickeador_nombre:combo})
      return
    }
    const {orden, reasignar:esReasig, alCerrar} = pickerSel
    setBusy(true)
    ...
    }).eq('id',orden.id).select('id')
    setBusy(false)
    if(error){ flash('⚠️ '+error.message); return }
    if(!data?.length){ flash('⚠️ No se pudo registrar el pickeador'); return }
    // Optimista: reflejar el pickeador en la UI al instante
    setOrdenes(prev=>prev.map(o=>o.id===orden.id?{...o,
      pickeador_id:t.id, pickeador_nombre:nom}:o))
    ...
    setSelId(orden.id)
    if(alCerrar){
      if(pickerSel.mixtaFase==='bodega'){
        // Orden MIXTA: registrado ZONA BODEGA → sigue el responsable de ZONA SEGUNDA
        flash(`👤 ZONA BODEGA: ${nom}`)
        setPickerSel({orden:{...orden,pickeador_id:t.id,pickeador_nombre:nom},
          reasignar:false, alCerrar:true, mixtaFase:'segunda', p1:{id:t.id,nombre:nom}})
        return
      }
      // venía del cierre: registrado el pickeador, se cierra la orden
      flash(`👤 Picking de #${orden.folio} registrado a ${nom}`)
      await cerrarOrden({...orden, pickeador_id:t.id, pickeador_nombre:nom})
      return
    }
    flash(esReasig?`🔄 Reasignada a ${nom}`:`👤 Pickeador: ${nom}`)
    setTimeout(()=>scanRef.current?.focus(), 300)
  }
  ...
  // ── Desistimiento del cliente: la orden sale del flujo con motivo ──
  const desistir = async (motivo, detalle)=>{
    if(!desistirSel) return
    setBusy(true)
    const {error} = await supabase.from('log_picking_ordenes').update({
      estado:'desistida', desistida_motivo:motivo, desistida_detalle:(detalle||'').trim()||null,
      desistida_at:new Date().toISOString(), desistida_por:cu?.nombre||'',
    }).eq('id',desistirSel.id)
    setBusy(false)
    if(error){ flash('⚠️ '+error.message); return }
    setOrdenes(prev=>prev.map(o=>o.id===desistirSel.id?{...o,estado:'desistida',
      desistida_motivo:motivo, desistida_por:cu?.nombre||''}:o))
    await etapaCerrar(desistirSel)   // cierra la etapa de trabajo si estaba abierta
    flash(`🚫 #${desistirSel.folio} desistida por el cliente`)
    setDesistirSel(null); setDesistDetalle('')
  }
  // ── Desiste de la ENTREGA INMEDIATA (no del pedido): pasa a POR DESPACHAR ──
  // La orden NO muere: se reclasifica a bandeja programada con modalidad despacho,
  // conservando estado y avance de picking. El despacho se coordina después.
  const desistirADespacho = async ()=>{
    if(!desistirSel) return
    setBusy(true)
    const {error} = await supabase.from('log_picking_ordenes').update({
      tipo_entrega:'programada', modalidad_entrega:'despacho', fecha_programada:null,
    }).eq('id',desistirSel.id)
    setBusy(false)
    if(error){ flash('⚠️ '+error.message); return }
    setOrdenes(prev=>prev.map(o=>o.id===desistirSel.id?{...o,
      tipo_entrega:'programada', modalidad_entrega:'despacho', fecha_programada:null}:o))
    flash(`🚚 #${desistirSel.folio} pasó a POR DESPACHAR — coordinar el despacho con el cliente`)
    setDesistirSel(null); setDesistDetalle('')
  }
  // ── REVERSA DE ENTREGA: el cliente ya firmó pero no puede llevarse el pedido ──
  // Opción A: agendar despacho — vuelve a LISTA en bandeja POR DESPACHAR (el
  // picking ya está hecho); se limpian los datos de entrega y queda auditoría.
  const reversarADespacho = async ()=>{
    if(!reversarSel) return
    setBusy(true)
    const {error} = await supabase.from('log_picking_ordenes').update({
      estado:'lista', tipo_entrega:'programada', modalidad_entrega:'despacho', fecha_programada:null,
      entregada_at:null, entregado_por:null, entrega_completa:null,
      receptor_nombre:null, receptor_rut:null,
      reversada_at:new Date().toISOString(), reversada_por:cu?.nombre||'',
      reversa_motivo:'agendar_despacho', reversa_detalle:(reversaDetalle||'').trim()||null,
    }).eq('id',reversarSel.id).eq('estado','entregada')
    setBusy(false)
    if(error){ flash('⚠️ '+error.message); return }
    setOrdenes(prev=>prev.map(o=>o.id===reversarSel.id?{...o,
      estado:'lista', tipo_entrega:'programada', modalidad_entrega:'despacho', fecha_programada:null,
      entregada_at:null, entregado_por:null, entrega_completa:null,
      receptor_nombre:null, receptor_rut:null,
      reversada_por:cu?.nombre||'', reversa_motivo:'agendar_despacho'}:o))
    flash(`↩️ #${reversarSel.folio} reversada — LISTA en POR DESPACHAR, coordinar datos de despacho`)
    setReversarSel(null); setReversaNC(''); setReversaDetalle('')
  }
  // Opción B: el cliente anula la compra — exige N° de Nota de Crédito del POS.
  const reversarAnular = async ()=>{
    if(!reversarSel) return
    if(!reversaNC.trim()){ flash('⚠️ Registra el N° de la Nota de Crédito para anular'); return }
    setBusy(true)
    const {error} = await supabase.from('log_picking_ordenes').update({
      estado:'anulada',
      motivo_anulacion:`Reversa post-entrega — cliente anula la compra · NC ${reversaNC.trim()}`,
      nc_reversa_numero:reversaNC.trim(),
      reversada_at:new Date().toISOString(), reversada_por:cu?.nombre||'',
      reversa_motivo:'anular_compra', reversa_detalle:(reversaDetalle||'').trim()||null,
    }).eq('id',reversarSel.id).eq('estado','entregada')
    setBusy(false)
    if(error){ flash('⚠️ '+error.message); return }
    setOrdenes(prev=>prev.map(o=>o.id===reversarSel.id?{...o,
      estado:'anulada', motivo_anulacion:`Reversa post-entrega — cliente anula la compra · NC ${reversaNC.trim()}`,
      reversada_por:cu?.nombre||'', reversa_motivo:'anular_compra'}:o))
    flash(`⛔ #${reversarSel.folio} anulada por reversa — NC ${reversaNC.trim()}`)
    setReversarSel(null); setReversaNC(''); setReversaDetalle('')
  }

  // Cierre efectivo de la orden (ya validado que hay pickeador registrado)
  const cerrarOrden = async (orden)=>{
    const {error} = await supabase.from('log_picking_ordenes').update({
      estado:'lista', lista_at:new Date().toISOString(),
    }).eq('id',orden.id)
    if(error){ flash('⚠️ '+error.message); return }
    setOrdenes(prev=>prev.map(o=>o.id===orden.id?{...o,estado:'lista'}:o))
    // Registro de trabajo por etapa: si no se abrió al asignar, se abre aquí
    supabase.from('log_modulo_etapa_trabajadores').insert({
      modulo:'picking', referencia_id:orden.id, referencia_folio:orden.folio,
      sucursal_codigo:orden.sucursal_codigo, etapa:'picking',
      trabajador_id:orden.pickeador_id, nombre_trabajador:orden.pickeador_nombre,
      hora_inicio:orden.inicio_picking_at||orden.asignada_at||new Date().toISOString(),
      asignado_por_nombre:cu?.nombre||'Sistema', asignado_por_rol:cu?.rol||'',
    }).then(()=>{},()=>{})
    await etapaCerrar(orden)
    flash(`✅ #${orden.folio} lista para entrega · picking de ${orden.pickeador_nombre}`)
  }

  // Marcar lista: aquí es OBLIGATORIO saber quién hizo el picking.
  const marcarLista = async ()=>{
    ...
    const mixCierre = sel.sucursal_codigo==='mp' && sel.modalidad_entrega!=='despacho'
      && new Set(items.filter(i=>!ES_FLETE(i)).map(zonaItem)).size>1
    if(!sel?.pickeador_id){
      // Sin pickeador registrado → se pide ahora y el cierre se encadena solo.
      // En orden MIXTA el flujo pide DOS responsables (uno por zona).
      setPickerSel({orden:sel, reasignar:false, alCerrar:true, ...(mixCierre?{mixtaFase:'bodega'}:{})})
      return
    }
    if(mixCierre){
      // Ya hay responsable de ZONA BODEGA (el registrado) → falta el de ZONA SEGUNDA
      setPickerSel({orden:sel, reasignar:false, alCerrar:true, mixtaFase:'segunda',
        p1:{id:sel.pickeador_id, nombre:sel.pickeador_nombre}})
      return
    }
    await cerrarOrden(sel)
  }
  ...
  const registrarSalida = async ()=>{
    ...
    // Transportista externo: se congela en la orden quién se llevó la carga. La
    // entrega al externo y la recepción del cliente son dos hitos distintos.
    const esExt = !!chofer.es_externo
    const empExt = esExt ? (chofer.empresa_transporte||'Transporte externo') : null
    const {error,data} = await supabase.from('log_picking_ordenes').update({
      ...
      transporte_externo:esExt, externo_empresa:empExt,
    ...
    setOrdenes(prev=>prev.map(o=>o.id===sel.id?{...o,estado:'despachada',despachada_at:ahora,
      guia_numero:guiaNum.trim(),chofer_id:chofer.id,chofer_nombre:nom,chofer_patente:chofer.patente||null,
      transporte_externo:esExt,externo_empresa:empExt}:o))
    flash(esExt
      ? `🚛 #${sel.folio} entregada a ${empExt} (${nom}) — imprime el comprobante de retiro para que lo firme`
      : `🚚 #${sel.folio} EN RUTA — guía ${guiaNum.trim()} · ${nom}`)
    // Con externo el comprobante de retiro sale solo: es la evidencia del hito 1
    if(esExt) try{
      await comprobanteRetiroPDF({...sel, chofer_nombre:nom, chofer_patente:chofer.patente||null,
        guia_numero:guiaNum.trim(), externo_empresa:empExt}, items)
    }catch(e){ console.error('[picking comprobante retiro]',e) }
  }

  // ── Comprobante de retiro: evidencia de que el transportista EXTERNO se llevó
  //    la carga. Es el hito 1 (bodega → distribuidor); la recepción del cliente
  //    es el hito 2 y se acredita después con la guía firmada. Dos copias.
  const comprobanteRetiroPDF = async (orden=sel, lineas=items)=>{
    if(!orden) return
    const {jsPDF, autoTable} = await cargarPdf()
    const doc = new jsPDF()
    const W = 210
    const empresa = orden.externo_empresa || 'Transporte externo'
    const totU = lineas.reduce((s,i)=>s+Number(i.cantidad||0),0)
    const emitida = new Date().toLocaleString('es-CL',{dateStyle:'short',timeStyle:'short'})

    // Media hoja por copia: arriba transportista, abajo bodega
    const copia = (yTop, titulo, subtitulo)=>{
      doc.setFillColor(26,26,46); doc.rect(0,yTop,W,22,'F')
      doc.setFillColor(232,102,10); doc.rect(0,yTop+22,W,1.3,'F')
      try{ doc.addImage(LOGO_B64,'PNG',12,yTop+4,28,14) }catch(e){}
      doc.setTextColor(255,255,255)
      doc.setFontSize(13); doc.setFont(undefined,'bold')
      doc.text('COMPROBANTE DE RETIRO', 46, yTop+10)
      doc.setFontSize(7.5); doc.setFont(undefined,'normal'); doc.setTextColor(205,205,222)
      doc.text('Outlet de Puertas SpA · Entrega de carga a transportista', 46, yTop+15.5)
      doc.setFontSize(7); doc.setTextColor(232,102,10)
      doc.text(titulo, 46, yTop+19.5)
      doc.setFontSize(8); doc.setFont(undefined,'normal'); doc.setTextColor(200,200,215)
      doc.text(DOC_L(orden.bsale_doc_type).toUpperCase(), W-12, yTop+8, {align:'right'})
      doc.setFontSize(18); doc.setFont(undefined,'bold'); doc.setTextColor(255,255,255)
      doc.text(`#${orden.folio}`, W-12, yTop+17, {align:'right'})

      let y = yTop + 31
      const campo = (label, valor, x, ancho)=>{
        doc.setFontSize(6.5); doc.setFont(undefined,'bold'); doc.setTextColor(130,130,145)
        doc.text(label.toUpperCase(), x, y)
        doc.setFontSize(9.5); doc.setFont(undefined,'bold'); doc.setTextColor(25,25,35)
        doc.text(doc.splitTextToSize(String(valor??'—'), ancho)[0], x, y+5)
      }
      campo('Transportista', empresa, 12, 90)
      campo('Conductor', orden.chofer_nombre||'—', 108, 50)
      campo('Patente', orden.chofer_patente||'—', 164, 34)
      y += 11
      campo('Guía de despacho BSALE', orden.guia_numero||'—', 12, 90)
      campo('Sucursal de origen', nomSuc(orden.sucursal_codigo), 108, 50)
      campo('Emitido', emitida, 164, 34)
      y += 11
      campo('Cliente destinatario', orden.cliente_nombre||'Sin cliente (boleta)', 12, 90)
      campo('Dirección de entrega',
        `${orden.despacho_direccion||'—'}${orden.despacho_comuna?', '+orden.despacho_comuna:''}`, 108, 90)
      y += 9

      autoTable(doc,{ startY:y, margin:{left:12,right:12},
        styles:{fontSize:8,cellPadding:1.7}, headStyles:{fillColor:[26,26,46],fontSize:7},
        head:[['SKU','PRODUCTO','CANT']],
        body: lineas.map(i=>[i.sku||'—', i.producto, String(i.cantidad)]),
        columnStyles:{0:{cellWidth:28,fontSize:7},2:{cellWidth:14,halign:'right',fontStyle:'bold'}},
        alternateRowStyles:{fillColor:[248,249,252]},
      })
      let y2 = doc.lastAutoTable.finalY + 4
      doc.setFontSize(8); doc.setFont(undefined,'bold'); doc.setTextColor(60,60,75)
      doc.text(`${lineas.length} líneas · ${totU} unidades entregadas al transportista`, 198, y2, {align:'right'})
      y2 += 5

      // Declaración: qué está firmando el transportista
      doc.setFillColor(255,250,242); doc.setDrawColor(230,200,160)
      doc.roundedRect(12, y2, 186, 13, 1.5, 1.5, 'FD')
      doc.setFontSize(6.6); doc.setFont(undefined,'normal'); doc.setTextColor(140,95,30)
      doc.text(doc.splitTextToSize(
        'El transportista declara recibir conforme la carga detallada, en buen estado y en las cantidades indicadas, '+
        'asumiendo su traslado y entrega al cliente destinatario. La entrega al cliente se acredita con esta guía '+
        'firmada por quien recibe, que debe devolverse a Outlet de Puertas.', 180), 15, y2+4)
      y2 += 18

      const fy = y2 + 12
      doc.setDrawColor(120,120,140); doc.setLineWidth(0.3)
      doc.line(18, fy, 92, fy); doc.line(116, fy, 190, fy)
      doc.setFontSize(7); doc.setFont(undefined,'bold'); doc.setTextColor(40,40,55)
      doc.text('FIRMA Y RUT DEL TRANSPORTISTA', 55, fy+4, {align:'center'})
      doc.text('ENTREGA BODEGA', 153, fy+4, {align:'center'})
      doc.setFontSize(7); doc.setFont(undefined,'normal'); doc.setTextColor(100,100,120)
      doc.text(orden.chofer_nombre||'', 55, fy+8, {align:'center'})
      doc.text(cu?.nombre||'', 153, fy+8, {align:'center'})
      doc.setFontSize(6); doc.setTextColor(165,165,180)
      doc.text(subtitulo, 105, fy+14, {align:'center'})
    }

    copia(0,   'COPIA 1 · TRANSPORTISTA', 'Copia para el transportista · Outlet de Puertas SpA')
    doc.setDrawColor(190,190,205); doc.setLineDashPattern([2,2],0)
    doc.line(0,147,W,147); doc.setLineDashPattern([],0)
    copia(150, 'COPIA 2 · BODEGA (queda firmada)', 'Copia que queda firmada en bodega · respaldo del retiro')

    doc.save(`retiro_${orden.folio}.pdf`)
  }

  // ── Subir el comprobante de retiro firmado por el transportista externo ───
  const subirComprobanteRetiro = async (file)=>{
    if(!file||!sel) return
    setBusy(true)
    try{
      const ahora = new Date().toISOString()
      const ext = (file.name.split('.').pop()||'jpg').toLowerCase()
      const path = `picking-retiros/${sel.sucursal_codigo}/${sel.folio}_retiro_${sel.id.slice(0,8)}.${ext}`
      ...
      const {error} = await supabase.from('log_picking_ordenes').update({
        retiro_comprobante_url:url, retiro_comprobante_at:ahora, retiro_comprobante_por:cu?.nombre||'',
      }).eq('id',sel.id)
      if(error) throw error
      setOrdenes(prev=>prev.map(o=>o.id===sel.id?{...o,retiro_comprobante_url:url,
        retiro_comprobante_at:ahora,retiro_comprobante_por:cu?.nombre||''}:o))
      setRetiroFile(null)
      flash(`🖋 Comprobante de retiro de #${sel.folio} archivado`)
    }catch(e){ console.error('[picking retiro]',e); flash('⚠️ '+e.message) }
    setBusy(false)
  }
  ...
  const confirmarEntrega = async (zona=null)=>{
    if(!receptor.trim()){ flash('⚠️ Nombre del receptor es obligatorio'); return }
    // ── Entrega por zonas (Maipú): zona='bodega'|'segunda' entrega SOLO esa zona;
    //    el acto que deja cero zonas pendientes CIERRA la orden ──
    const actosPrev = Array.isArray(sel?.entregas_parciales)?sel.entregas_parciales:[]
    const itemsActo = zona ? items.filter(i=>zonaItem(i)===zona) : items
    if(zona && !itemsActo.length){ flash('⚠️ Esa zona no tiene productos'); return }
    const otrasPend = zona ? ['bodega','segunda'].filter(z=>z!==zona
      && !actosPrev.some(a=>a.zona===z)
      && items.some(i=>zonaItem(i)===z && Math.min(Number(i.cantidad_confirmada),Number(i.cantidad))>0)) : []
    const cierra = !zona || otrasPend.length===0
    // Entrega PARCIAL (faltantes reales): exige NC — pero SOLO en el acto que cierra
    const hayFaltantes = items.some(i=>Number(i.cantidad_confirmada)<Number(i.cantidad))
    if(cierra && hayFaltantes && !ncParcial.trim()){ flash('⚠️ Entrega parcial: registra el N° de la Nota de Crédito'); return }
    setGenPdf(true)
    try{
      const ahora = new Date().toISOString()
      // Check unico: lo entregado ES lo validado (con sus motivos de parcialidad)
      const entregas = itemsActo.map(i=>({...i, cantidad_entregada: Math.min(Number(i.cantidad_confirmada),Number(i.cantidad))}))
      await Promise.all(entregas.map(i=>supabase.from('log_picking_items').update({
        cantidad_entregada:i.cantidad_entregada, entregado_at:ahora, entregado_por:cu?.nombre||'',
      }).eq('id',i.id)))
      // completa se evalúa GLOBAL (todos los items de la orden), no solo el acto
      const completa = items.every(i=>Math.min(Number(i.cantidad_confirmada),Number(i.cantidad))>=Number(i.cantidad))
      const {jsPDF, autoTable} = await cargarPdf()
      const doc = new jsPDF()
      ...
      const dibujarCopia = (badge)=>{
        ...
        if(sel.emitido_por&&sel.emitido_por!==sel.vendedor_nombre) linea('Emitió (caja):', sel.emitido_por)
        linea('Pickeador:', sel.pickeador_nombre||'—')
        linea('Fecha entrega:', fmtT(ahora))
        if(zona){
          doc.setFillColor(...(zona==='segunda'?[255,236,209]:[220,235,255]))
          doc.roundedRect(14,y-3.5,182,8,1.5,1.5,'F')
          doc.setFont(undefined,'bold'); doc.setFontSize(9)
          doc.setTextColor(...(zona==='segunda'?[178,80,0]:[0,80,200]))
          doc.text(`ENTREGA POR ZONAS — ${zona==='bodega'?'ZONA BODEGA · PRIMERA':'ZONA SEGUNDA / DESCONTINUADOS'} · ACTO ${actosPrev.length+1}${cierra?' (CIERRA LA ORDEN)':''}`, 105, y+1.8, {align:'center'})
          doc.setTextColor(30,30,30); doc.setFont(undefined,'normal'); doc.setFontSize(10)
          y+=9
        }
        y+=1
        ...
        // Pie
        doc.setFontSize(7.5)
        doc.text(`Emitida ${fmtT(sel.emitida_at)} · En cola ${fmtT(sel.recibida_at)} · Lista ${fmtT(sel.lista_at)} · ${!cierra?'ORDEN CON ENTREGA POR ZONAS EN CURSO':completa?'ENTREGA COMPLETA':`ENTREGA PARCIAL — NC ${ncParcial.trim()}`}`, 14, 286)
      ...
      doc.save(`entrega_${sel.folio}${zona?'_'+zona:''}.pdf`)

      let pdfUrl = null
      try{
        const path = `picking/${sel.sucursal_codigo}/${sel.folio}_${sel.id.slice(0,8)}${zona?'_'+zona:''}.pdf`
      ...
      const actoReg = zona ? {zona, receptor:receptor.trim(), receptor_rut:receptorRut.trim()||null,
        entregado_por:cu?.nombre||'', at:ahora, pdf:pdfUrl,
        items:entregas.map(i=>({sku:i.sku, producto:i.producto, cant:i.cantidad_entregada}))} : null
      const nuevosActos = actoReg ? [...actosPrev, actoReg] : actosPrev
      if(cierra){
        const {error} = await supabase.from('log_picking_ordenes').update({
          ...
          nc_parcial_numero: completa?null:(ncParcial.trim()||null),
          ...(zona?{entregas_parciales:nuevosActos}:{}),
        }).eq('id',sel.id)
        if(error) throw error
        setOrdenes(prev=>prev.map(o=>o.id===sel.id?{...o,estado:'entregada',entregas_parciales:nuevosActos}:o))
        await etapaCerrar(sel)
        flash(`✅ #${sel.folio} entregada${zona?` (${nuevosActos.length} actos por zona)`:''}${completa?'':` (parcial — NC ${ncParcial.trim()})`} — comprobante en 2 copias generado`)
        setReceptor(''); setReceptorRut(''); setNcParcial(''); setSelId(null)
      } else {
        const {error} = await supabase.from('log_picking_ordenes').update({
          entregas_parciales:nuevosActos,
        }).eq('id',sel.id)
        if(error) throw error
        setOrdenes(prev=>prev.map(o=>o.id===sel.id?{...o,entregas_parciales:nuevosActos}:o))
        flash(`✅ #${sel.folio} — ${zona==='bodega'?'ZONA BODEGA':'ZONA SEGUNDA'} entregada a ${receptor.trim()} · pendiente la otra zona`)
        setReceptor(''); setReceptorRut('')
      }
  ...
  const listaPickingPDF = async ()=>{
    ...
    const mixHoja = sel.sucursal_codigo==='mp' && new Set(items.filter(i=>!ES_FLETE(i)).map(zonaItem)).size>1
    const itsHoja = mixHoja ? [...items].sort((a,b)=>zonaItem(a).localeCompare(zonaItem(b))||String(a.producto).localeCompare(String(b.producto))) : items
    autoTable(doc,{ startY:y, styles:{fontSize:10,cellPadding:2.6}, headStyles:{fillColor:[26,26,46],fontSize:8.5},
      ...
      body: itsHoja.flatMap((i,ix,arr)=>{
        const filas = []
        if(mixHoja&&(ix===0||zonaItem(arr[ix-1])!==zonaItem(i)))
          filas.push([{content: zonaItem(i)==='bodega'?'▶ ZONA BODEGA · PRIMERA — pickeador: ____________':'▶ ZONA SEGUNDA / DESCONTINUADOS — pickeador: ____________',
            colSpan:5, styles:{fontStyle:'bold', fontSize:9.5, textColor:zonaItem(i)==='segunda'?[178,80,0]:[0,80,200],
              fillColor:zonaItem(i)==='segunda'?[255,236,209]:[220,235,255]}}])
        filas.push([ i.sku||'—', i.producto, String(i.cantidad), '', '' ])
        return filas
      }),
  ...
  const PanelOrden = ()=>{
    ...
    const activa = !['entregada','anulada','desistida'].includes(sel.estado)
    ...
    // ── Entrega por zonas: solo Maipú, retiro, orden con ambas zonas ──
    const actosZona = Array.isArray(sel.entregas_parciales)?sel.entregas_parciales:[]
    const esMixta = sel.sucursal_codigo==='mp' && sel.modalidad_entrega!=='despacho'
      && new Set(items.filter(i=>!ES_FLETE(i)).map(zonaItem)).size>1
    const zonasEntregables = esMixta ? ['bodega','segunda'].filter(z=>
      items.some(i=>zonaItem(i)===z && Math.min(Number(i.cantidad_confirmada),Number(i.cantidad))>0)) : []
    const zonasPend = zonasEntregables.filter(z=>!actosZona.some(a=>a.zona===z))
    ...
    return (
      <div style={{display:'flex',flexDirection:'column',height:'100%'}}>
        <div style={{padding:'11px 14px',borderBottom:'1px solid #E5E5EA',background:'#FAFAFC'}}>
          ...
          {sel.cotizacion_editada&&(
            <div style={{marginTop:6,padding:'7px 11px',borderRadius:9,background:'#FF3B3012',
              border:'1.5px solid #FF3B3050',fontSize:11,fontWeight:800,color:'#C41E12'}}>
              ⚠ Cotización EDITADA en BSALE después de iniciado el picking — verificar los ítems
              contra el documento vigente antes de continuar; si difiere, usar Rechazar picking.
            </div>
          )}
          <div style={{fontSize:12,color:'#3A3A3C',marginTop:5,fontWeight:600}}>
            ...
            {actosZona.length>0&&sel.estado==='lista'&&<span style={{marginLeft:8,fontSize:10.5,fontWeight:900,color:'#AF52DE',
              background:'#AF52DE12',border:'1px solid #AF52DE35',padding:'2px 9px',borderRadius:9}}>
              🔄 ENTREGA POR ZONAS · {actosZona.length}/{zonasEntregables.length||2}</span>}
            {sel.tiene_segunda&&<span style={{marginLeft:8,fontSize:10.5,fontWeight:800,color:'#B25000',
              background:'#FF950015',border:'1px solid #FF950040',padding:'2px 9px',borderRadius:9}}>
              ⚠ Incluye productos de segunda selección</span>}
          </div>
          <div style={{fontSize:11,color:'#8E8E93',marginTop:3,display:'flex',gap:10,flexWrap:'wrap',alignItems:'center'}}>
            ...
            {activa&&!(Array.isArray(sel.entregas_parciales)&&sel.entregas_parciales.length>0)&&
              <span onClick={()=>{setDesistDetalle('');setDesistirSel(sel)}} title="El cliente se arrepintió del pedido"
                style={{color:'#E0457B',fontWeight:700,cursor:'pointer'}}>🚫 Cliente desiste</span>}
            {activa&&Array.isArray(sel.entregas_parciales)&&sel.entregas_parciales.length>0&&
              <span title="Con un acto de entrega firmado, desistir/reversar se gestiona por postventa (NC en el POS)"
                style={{color:'#AF52DE',fontWeight:800}}>🔒 Entrega por zonas en curso</span>}
          </div>
        ...
        <div style={{flex:1,overflowY:'auto'}}>
          ...
          <table style={{width:'100%',borderCollapse:'collapse'}}>
            ...
            <tbody>
              {(esMixta?[...items].sort((a,b)=>zonaItem(a).localeCompare(zonaItem(b))||String(a.producto).localeCompare(String(b.producto))):items).map((it,ix,arr)=>{
                ...
                return (<React.Fragment key={it.id}>
                  {esMixta&&(ix===0||zonaItem(arr[ix-1])!==zonaItem(it))&&(
                    <tr><td colSpan={5} style={{padding:'6px 10px',background:ZONA_C[zonaItem(it)]+'14',
                      fontSize:10,fontWeight:900,color:zonaItem(it)==='segunda'?'#B25000':ZONA_C[zonaItem(it)],letterSpacing:'0.05em',
                      borderTop:`2px solid ${ZONA_C[zonaItem(it)]}40`}}>
                      {ZONA_L[zonaItem(it)]}
                      {actosZona.some(a=>a.zona===zonaItem(it))&&<span style={{marginLeft:8,color:'#34C759'}}>✔ ENTREGADA</span>}
                    </td></tr>
                  )}
                  <tr style={{background: flashRow?'#34C75925': conMotivo?'#FF3B3006': entOK?'#34C75908': valOK?'#5856D608':'transparent',
                    ...
                    <td style={{...tdCk,whiteSpace:'normal'}}>
                      <div style={{fontWeight:700,fontSize:12.5,lineHeight:1.25}}>{flete?'🚚 ':''}{it.producto}
                        {(ES_DESCONTINUADO(it)||MOTIVO_SKU(it))&&<span style={{fontSize:9,fontWeight:900,color:'#FF9500',marginLeft:5}}>
                          {ES_DESCONTINUADO(it)?'DESCONTINUADO':'2ª SELECCIÓN'}</span>}</div>
        ...
        <div style={{padding:'10px 14px',borderTop:'1px solid #E5E5EA',background:'#FAFAFC'}}>
          {sel.estado==='pendiente'&&(
            <button style={{...btMini('#007AFF','#fff'),width:'100%',padding:'10px 0',fontSize:13}}
              disabled={busy} onClick={()=>tomar(sel)}>🏃 Tomar esta orden e imprimir la hoja</button>
          )}
          {enPicking&&(
            <div style={{display:'flex',gap:8}}>
              ...
              <button onClick={()=>asignar(sel)} title="Registrar o cambiar quién está haciendo el picking"
                style={{...btMini(sel?.pickeador_nombre?'#34C75915':'#FF950015', sel?.pickeador_nombre?'#248A3D':'#B36B00'),padding:'9px 12px',whiteSpace:'nowrap'}}>
                {sel?.pickeador_nombre?`👤 ${sel.pickeador_nombre.split(' ')[0]}`:'👤 Sin registrar'}
              </button>
              <button style={{...btMini(todoValidado?'#34C759':'#E5E5EA', todoValidado?'#fff':'#8E8E93'),flex:1,padding:'9px 0'}}
                ...
                {!todoValidado ? 'Valida o justifica todo primero'
                  : sel?.pickeador_nombre ? '📦 Marcar LISTA para entrega'
                  : '📦 Marcar LISTA — registrar quién pickeó'}
              </button>
            </div>
          )}
          ...
          {enEntrega&&(
            <div>
              {faltantes.length>0&&(!esMixta||zonasPend.length<=1)&&(
                <div style={{marginBottom:6}}>
                  ...
                  <input value={ncParcial} onChange={e=>setNcParcial(e.target.value)}
                    placeholder="N° Nota de Crédito (obligatorio en entrega parcial) *"
                    style={{...css.input,width:'100%',padding:'8px 10px',border:'1.5px solid #C9340060',background:'#FFF5F5'}}/>
                  <div style={{fontSize:9.5,color:'#8E8E93',marginTop:3}}>
                    La NC de lo no entregado se emite primero en el POS según el SOP de postventa — aquí se registra su número.
                  </div>
                </div>
              )}
              ...
              {!esMixta ? (
              <button style={{...btMini((receptor.trim()&&(faltantes.length===0||ncParcial.trim()))?'#1a1a2e':'#E5E5EA', (receptor.trim()&&(faltantes.length===0||ncParcial.trim()))?'#fff':'#8E8E93'),
                width:'100%',padding:'11px 0',fontSize:13}}
                disabled={!receptor.trim()||(faltantes.length>0&&!ncParcial.trim())||genPdf} onClick={()=>confirmarEntrega(null)}>
                {genPdf?'Generando comprobante…':!receptor.trim()?'Indica quién recibe'
                  :(faltantes.length>0&&!ncParcial.trim())?'Registra la NC de la entrega parcial'
                  :`✅ CONFIRMACIÓN FINAL — comprobante 2 copias #${sel.folio}`}
              </button>
              ) : (<>
                <div style={{fontSize:10.5,fontWeight:900,color:'#5856D6',marginBottom:6,letterSpacing:'0.02em'}}>
                  🏬⚠ ORDEN MIXTA · entrega en {zonasEntregables.length} actos por zona — receptor y firma por cada acto
                </div>
                {actosZona.map((a,ix)=>(
                  <div key={ix} style={{fontSize:10.5,color:'#34C759',fontWeight:700,marginBottom:4,
                    padding:'5px 9px',background:'#34C75910',borderRadius:8}}>
                    ✔ {a.zona==='bodega'?'ZONA BODEGA':'ZONA SEGUNDA'} entregada · {a.receptor}
                    {' · '}{new Date(a.at).toLocaleTimeString('es-CL',{hour:'2-digit',minute:'2-digit'})} · entregó {a.entregado_por}
                  </div>
                ))}
                {zonasPend.map(z=>{
                  const esCierre = zonasPend.length===1
                  const habil = receptor.trim() && (!esCierre || faltantes.length===0 || ncParcial.trim())
                  return (<button key={z} style={{...btMini(habil?ZONA_C[z]:'#E5E5EA', habil?'#fff':'#8E8E93'),
                    width:'100%',padding:'11px 0',fontSize:12.5,marginBottom:6}}
                    disabled={!habil||genPdf} onClick={()=>confirmarEntrega(z)}>
                    {genPdf?'Generando comprobante…':!receptor.trim()?'Indica quién recibe este acto'
                      :(esCierre&&faltantes.length>0&&!ncParcial.trim())?'Registra la NC — este acto cierra la orden'
                      :`✅ ENTREGAR ${z==='bodega'?'ZONA BODEGA · PRIMERA':'ZONA SEGUNDA / DESCONT.'}${esCierre?` — CIERRA #${sel.folio}`:` · acto ${actosZona.length+1} de ${zonasEntregables.length}`}`}
                  </button>)
                })}
              </>)}
              {/* FIX 11-08: la guarda de actosZona OCULTA el bloque completo; antes
                  estaba dentro del ternario e inverta el guard → editor con estado
                  null → null.trim() → crash (error boundary "Ocurrió un error"). */}
              {actosZona.length===0&&(rechazoEdit===null
              ...
              <div style={{marginBottom:8,padding:'8px 10px',borderRadius:9,background:'#AF52DE08',border:'1px solid #AF52DE30'}}>
                ...
                <div style={{display:'flex',gap:8,flexWrap:'wrap'}}>
                  <select value={choferSel} onChange={e=>setChoferSel(e.target.value)}
                    ...
                    {choferes.some(c=>!c.es_externo)&&(
                      <optgroup label="🚚 Reparto propio">
                        {choferes.filter(c=>!c.es_externo).map(c=>(
                          <option key={c.id} value={c.id}>
                            {c.nombre} {c.apellido||''}{c.patente?` · ${c.patente}`:''}
                          </option>
                        ))}
                      </optgroup>
                    )}
                    {choferes.some(c=>c.es_externo)&&(
                      <optgroup label="🚛 Transportistas externos">
                        {choferes.filter(c=>c.es_externo).map(c=>(
                          <option key={c.id} value={c.id}>
                ...
                {(()=>{ const ch = choferes.find(c=>c.id===choferSel)
                  return ch?.es_externo&&(
                    <div style={{marginTop:6,padding:'7px 10px',borderRadius:8,background:'#FF950012',
                      border:'1px solid #FF950040',fontSize:11,color:'#B25000',fontWeight:700}}>
                      🚛 Transporte externo · {ch.empresa_transporte||'sin empresa'} — al registrar la salida se
                      imprime el comprobante de retiro para que lo firme. La orden queda EN RUTA hasta que vuelva
                      la guía firmada por el cliente.
                    </div>
                  )})()}
          ...
          {enRuta&&(
            <div>
              {(()=>{
                const dias = sel.despachada_at
                  ? Math.floor((Date.now()-new Date(sel.despachada_at))/86400000) : null
                const ext = !!sel.transporte_externo
                const c = ext?'#FF9500':'#AF52DE'
                return (
                  <div style={{marginBottom:8,padding:'8px 10px',borderRadius:9,
                    background:c+'0D',border:`1px solid ${c}40`,fontSize:11.5}}>
                    <b style={{color:c}}>{ext?'🚛 CON TRANSPORTE EXTERNO':'🚚 EN RUTA'}</b>
                    {ext&&sel.externo_empresa&&<> · <b>{sel.externo_empresa}</b></>}
                    {' '}— guía <b style={{fontFamily:'monospace'}}>{sel.guia_numero||'—'}</b>
                    ...
                    {dias!=null&&dias>=3&&
                      <span style={{marginLeft:6,fontWeight:900,color:dias>=8?'#FF3B30':'#B25000'}}>
                        · {dias} días sin acreditar entrega {dias>=8?'⚠':''}</span>}
                    {sel.despacho_pago==='por_pagar'&&
              ...
              {/* HITO 1 · retiro del transportista externo — evidencia del tramo bodega→distribuidor */}
              {sel.transporte_externo&&(
                <div style={{marginBottom:8,padding:'8px 10px',borderRadius:9,background:'#FF950008',border:'1px solid #FF950030'}}>
                  <div style={{fontSize:10,fontWeight:800,color:'#B25000',marginBottom:5,letterSpacing:'0.03em'}}>
                    1️⃣ RETIRO DEL TRANSPORTISTA — comprobante firmado al llevarse la carga
                  </div>
                  {sel.retiro_comprobante_url
                    ? <div style={{fontSize:11.5,color:'#248A3D',fontWeight:700}}>
                        ✅ Archivado {sel.retiro_comprobante_at?new Date(sel.retiro_comprobante_at).toLocaleDateString('es-CL'):''}
                        {sel.retiro_comprobante_por?` por ${sel.retiro_comprobante_por}`:''} ·{' '}
                        <a href={sel.retiro_comprobante_url} target="_blank" rel="noreferrer"
                          style={{color:'#007AFF',fontWeight:800}}>🖋 Ver comprobante</a>
                      </div>
                    : <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
                        <input type="file" accept="image/*,application/pdf"
                          onChange={e=>{ const f=e.target.files?.[0]||null; setRetiroFile(f); if(f) subirComprobanteRetiro(f) }}
                          style={{fontSize:11.5,flex:1,minWidth:170}}/>
                        <Bt v="gry" sm onClick={()=>comprobanteRetiroPDF(sel, items)}>🖨 Reimprimir</Bt>
                      </div>}
                  {!sel.retiro_comprobante_url&&
                    <div style={{fontSize:10,color:'#8E8E93',marginTop:4}}>
                      Sube la copia que firmó el transportista. No bloquea el cierre, pero es el respaldo de que la carga salió de bodega.
                    </div>}
                </div>
              )}
              ...
              <div style={{marginBottom:8,padding:'8px 10px',borderRadius:9,background:'#34C75908',border:'1px solid #34C75930'}}>
                <div style={{fontSize:10,fontWeight:800,color:'#248A3D',marginBottom:5,letterSpacing:'0.03em'}}>
                  {sel.transporte_externo?'2️⃣ RECEPCIÓN DEL CLIENTE':'✅ CIERRE DE DESPACHO'} — sube la guía firmada con recepción conforme del cliente *
                </div>
                <div style={{display:'flex',gap:8,flexWrap:'wrap',alignItems:'center'}}>
                  <input type="file" accept="image/*,application/pdf"
          ...
          {terminada&&(
            <div style={{fontSize:12,color:'#8E8E93',display:'flex',gap:12,alignItems:'center',flexWrap:'wrap'}}>
              {sel.estado==='entregada'&&<>
                ...
                {sel.entrega_completa===false&&sel.nc_parcial_numero&&
                  <span style={{color:'#C93400',fontWeight:800}}>◐ Parcial · NC {sel.nc_parcial_numero}</span>}
                {sel.modalidad_entrega==='retiro'&&!(Array.isArray(sel.entregas_parciales)&&sel.entregas_parciales.length>0)&&
                  <span onClick={()=>{setReversaNC('');setReversaDetalle('');setReversarSel(sel)}}
                    title="El cliente firmó pero no puede llevarse el pedido"
                    style={{color:'#AF52DE',fontWeight:700,cursor:'pointer'}}>↩️ Reversar entrega</span>}
              </>}
              {sel.estado==='anulada'&&<span style={{color:'#FF3B30'}}>✗ {sel.motivo_anulacion||'Anulada'}</span>}
              {sel.estado==='desistida'&&<span style={{color:'#E0457B'}}>🚫 {MOTIVOS_DESIST.find(m=>m.k===sel.desistida_motivo)?.l||sel.desistida_motivo||'Desistida'}{sel.desistida_detalle?` — "${sel.desistida_detalle}"`:''}{sel.desistida_por?` (${sel.desistida_por})`:''}</span>}
            </div>
          )}
        </div>
      </div>
  ...
  // Filtro de segunda selección (opcional): todos | solo con 2ª | solo sin 2ª
  const pasaSeg = o=>filSeg==='todos'||(filSeg==='si'?!!o.tiene_segunda:!o.tiene_segunda)
  // ── BUSCADOR DE COLA ──────────────────────────────────────────────────────
  // Filtra la bandeja visible por folio, cliente, RUT, receptor, guía, dirección
  // o vendedor. Los contadores de las tres bandejas pasan a mostrar coincidencias,
  // así se ve al tiro si el documento buscado está en otra bandeja.
  const busq = normBusq(colaTxt)
  const busqNum = sinCeros(busq)
  const pasaTxt = o=>{
    if(!busq) return true
    if(sinCeros(normBusq(o.folio)).includes(busqNum)) return true
    return [o.cliente_nombre,o.cliente_rut,o.receptor_nombre,o.receptor_rut,o.guia_numero,
            o.despacho_direccion,o.despacho_comuna,o.despacho_contacto,o.despacho_telefono,
            o.vendedor_nombre,o.pickeador_nombre,o.chofer_nombre,o.externo_empresa]
      .some(v=>v&&normBusq(v).includes(busq))
  }
  const nSegundaTotal = activasAll.filter(o=>o.tiene_segunda).length
  const inmediatas = activasAll.filter(o=>o.tipo_entrega!=='programada').filter(pasaSeg).filter(pasaTxt)
  const programadasAll = activasAll.filter(o=>o.tipo_entrega==='programada').filter(pasaSeg).filter(pasaTxt)
    .sort((a,b)=>(a.fecha_programada||'9999').localeCompare(b.fecha_programada||'9999'))
  // GATE COMERCIAL: una cotización existe para bodega SOLO cuando el vendedor
  // la registró en el módulo Despachos de la app Comercial (despacho_registrado_at).
  // Las no registradas se OCULTAN de la operación (evita confusión); los roles
  // de jefatura (esCD) conservan un subgrupo plegado como radar del limbo.
  // El trabajo ya iniciado nunca se oculta. Auto-anulación a los 7 días las limpia.
  const esPorConfirmar = o=>o.bsale_doc_type==='cotizacion'&&!o.despacho_registrado_at
    &&o.estado==='pendiente'&&!o.pickeador_id
  const porConfirmar = programadasAll.filter(esPorConfirmar)
  const programadas  = programadasAll.filter(o=>!esPorConfirmar(o))
  const [verPorConf,setVerPorConf]=useState(false)
  // BANDEJA EN RUTA: las despachadas esperando acreditar la entrega del cliente.
  // Antes no se listaban en ninguna parte — se despachaban y desaparecían.
  const enRutaCola = ordenes.filter(o=>o.estado==='despachada').filter(pasaSeg).filter(pasaTxt)
    .sort((a,b)=>String(a.despachada_at||'').localeCompare(String(b.despachada_at||'')))
  const diasFuera = o=> o.despachada_at
    ? Math.floor((Date.now()-new Date(o.despachada_at))/86400000) : null
  const enRutaAtrasadas = enRutaCola.filter(o=>(diasFuera(o)??0)>=8).length
  const activas = bandeja==='enruta'?enRutaCola:bandeja==='programada'?programadas:inmediatas
  const hoy0 = new Date(); hoy0.setHours(0,0,0,0)
  ...
  const desistidas = ordenes.filter(o=>o.estado==='desistida')
  ...
  // Vistas filtradas por el buscador (los KPI de arriba se calculan con el total del día)
  const entregadasHoyV = entregadasHoy.filter(pasaTxt)
  const anuladasV      = anuladas.filter(pasaTxt)
  const desistidasV    = desistidas.filter(pasaTxt)
  // Coincidencias en las bandejas que NO se están viendo (para avisar dónde está el doc)
  const otrasBandejas = busq ? [
    {k:'inmediata',  l:'⚡ ENTREGA INMEDIATA', c:'#007AFF', n:inmediatas.length},
    {k:'programada', l:'🚚 POR DESPACHAR',     c:'#5856D6', n:programadas.length},
    {k:'enruta',     l:'🚛 EN RUTA',           c:'#AF52DE', n:enRutaCola.length},
  ].filter(b=>b.k!==bandeja&&b.n>0) : []
  const nOtraBandeja = otrasBandejas.reduce((s,b)=>s+b.n,0)
  ...
  const FilaCola = ({o, terminada=false})=>{
    ...
    const esRuta = bandeja==='enruta'&&!terminada
    const dFuera = esRuta?diasFuera(o):null
    const cRuta = dFuera==null?'#AF52DE':dFuera>=8?'#FF3B30':dFuera>=3?'#FF9500':'#AF52DE'
    ...
    return (
      <tr onClick={()=>setSelId(o.id)}
        ...
        <td style={{...td,borderLeft:`3px solid ${activaSel?'#007AFF':terminada?'#E5E5EA':esRuta?cRuta:esProg?(vencida?'#FF3B30':'#5856D6'):semaforo(m)}`}}>
          {esRuta
            ?<span style={{fontWeight:800,fontSize:11,color:cRuta}}>
               {dFuera==null?'—':dFuera===0?'hoy':`${dFuera}d`}{dFuera>=8?' ⚠':''}</span>
            :esProg
        ...
        <td style={{...td,maxWidth:140,overflow:'hidden',textOverflow:'ellipsis',fontSize:11.5}}>
          ...
          {o.tipo_entrega==='programada'&&o.modalidad_entrega!=='despacho'&&o.bsale_doc_type!=='cotizacion'&&
            <span style={{fontSize:9,color:'#5856D6',fontWeight:900,display:'inline-block',marginTop:1,
              background:'#5856D612',border:'1px solid #5856D635',borderRadius:5,padding:'1px 6px'}}>
              🛍 RETIRO PROGRAMADO — el cliente viene a buscar</span>}
          {o.bsale_doc_type==='cotizacion'&&o.cotizacion_editada&&
            <span style={{fontSize:9,color:'#fff',fontWeight:900,display:'inline-block',marginTop:1,
              background:'#FF3B30',borderRadius:5,padding:'1px 6px'}}>⚠ EDITADA EN BSALE</span>}
          {Array.isArray(o.entregas_parciales)&&o.entregas_parciales.length>0&&o.estado==='lista'&&
            <span style={{fontSize:9,color:'#AF52DE',fontWeight:900,display:'block',marginTop:1}}>
              🔄 ENTREGA POR ZONAS · {o.entregas_parciales.length}/2</span>}
          {o.tiene_segunda&&<span style={{fontSize:9,color:'#B25000',fontWeight:800,display:'block',
            background:'#FF950018',borderRadius:5,padding:'1px 5px',marginTop:1,width:'fit-content'}}>
            ⚠ Incluye productos de segunda selección</span>}
          {o.estado==='despachada'
            ?<span style={{fontSize:9.5,color:o.transporte_externo?'#B25000':'#AF52DE',fontWeight:800,display:'block'}}>
               {o.transporte_externo?'🚛':'🚚'} guía {o.guia_numero||'—'}
               {o.transporte_externo&&o.externo_empresa?` · ${o.externo_empresa}`:o.chofer_nombre?` · ${o.chofer_nombre}`:''}
               {o.transporte_externo&&!o.retiro_comprobante_url&&<span style={{color:'#FF9500'}}> · sin comprobante de retiro</span>}
             </span>
  ...
  return (
    <div>
      ...
      <div style={{display:'flex',alignItems:'center',gap:10,marginBottom:10,flexWrap:'wrap'}}>
        ...
        {(esCD||sucPropia)&&(
          sucPropia
          ? <span title="Tu vista está limitada a la ubicación con la que entraste al WMS — para cambiarla, vuelve al selector de ubicación"
              style={{display:'inline-flex',alignItems:'center',gap:6,padding:'6px 12px',borderRadius:9,
                background:'#EAF3DE',border:'1px solid #B7D89C',fontSize:12,fontWeight:800,color:'#3B6D11'}}>
              📍 {(sucsTienda.find(s=>s.codigo===sucPropia)?.nombre)||sucPropia} <span style={{fontSize:10,opacity:0.7}}>🔒</span>
            </span>
      ...
      {desistirSel&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.45)',zIndex:9000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}
          onClick={()=>setDesistirSel(null)}>
          <div style={{background:'#fff',borderRadius:16,padding:20,width:'100%',maxWidth:440}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:14,fontWeight:800,marginBottom:2}}>🚫 Desistimiento del cliente</div>
            <div style={{fontSize:11.5,color:'#8E8E93',marginBottom:12}}>
              #{desistirSel.folio} · {desistirSel.cliente_nombre||'Sin cliente'} — elige qué pasa con la orden
            </div>
            {desistirSel.tipo_entrega==='inmediata'&&
            <button disabled={busy} onClick={desistirADespacho}
              style={{display:'block',width:'100%',textAlign:'left',padding:'12px 14px',marginBottom:8,
                borderRadius:12,border:'1.5px solid #5856D640',background:'#5856D608',cursor:'pointer',
                fontSize:13,fontWeight:700,color:'#1C1C1E'}}>
              🚚 Desiste de esperar la entrega — pasar a POR DESPACHAR
              <span style={{display:'block',fontSize:10.5,fontWeight:600,color:'#8E8E93',marginTop:2}}>
                La orden sigue viva: se reclasifica como despacho programado</span>
            </button>}
            {MOTIVOS_DESIST.map(m=>(
              <button key={m.k} disabled={busy} onClick={()=>desistir(m.k, desistDetalle)}
                style={{display:'block',width:'100%',textAlign:'left',padding:'12px 14px',marginBottom:8,
                  borderRadius:12,border:'1.5px solid #E0457B40',background:'#E0457B08',cursor:'pointer',
                  fontSize:13,fontWeight:700,color:'#1C1C1E'}}>
                {m.e} {m.l}
              </button>
            ))}
            <input value={desistDetalle} onChange={e=>setDesistDetalle(e.target.value)}
              placeholder="Detalle opcional (ej: cambia a puerta 80x200, vuelve el sábado…)"
              style={{width:'100%',padding:'9px 12px',border:'1px solid #D8D8DE',borderRadius:9,fontSize:12.5,marginTop:2}}/>
            <div style={{fontSize:10.5,color:'#8E8E93',marginTop:8}}>
              Se registra quién lo marcó y cuándo. La boleta/NC se gestiona en el POS según el SOP de postventa.
            </div>
            <button onClick={()=>setDesistirSel(null)}
              style={{marginTop:10,width:'100%',padding:'9px 0',borderRadius:10,border:'none',
                background:'#F2F2F7',fontSize:12.5,fontWeight:700,cursor:'pointer'}}>Cancelar</button>
          </div>
        </div>
      )}
      {reversarSel&&(
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.45)',zIndex:9000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}
          onClick={()=>setReversarSel(null)}>
          <div style={{background:'#fff',borderRadius:16,padding:20,width:'100%',maxWidth:460}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:14,fontWeight:800,marginBottom:2}}>↩️ Reversar entrega</div>
            <div style={{fontSize:11.5,color:'#8E8E93',marginBottom:12}}>
              #{reversarSel.folio} · {reversarSel.receptor_nombre||reversarSel.cliente_nombre||'Sin cliente'} — firmó la recepción pero no puede llevarse el pedido
            </div>
            <button disabled={busy} onClick={reversarADespacho}
              style={{display:'block',width:'100%',textAlign:'left',padding:'12px 14px',marginBottom:8,
                borderRadius:12,border:'1.5px solid #5856D640',background:'#5856D608',cursor:'pointer',
                fontSize:13,fontWeight:700,color:'#1C1C1E'}}>
              🚚 No le da la capacidad — agendar DESPACHO
              <span style={{display:'block',fontSize:10.5,fontWeight:600,color:'#8E8E93',marginTop:2}}>
                Vuelve LISTA a la bandeja POR DESPACHAR — el picking se conserva</span>
            </button>
            <button disabled={busy||!reversaNC.trim()} onClick={reversarAnular}
              style={{display:'block',width:'100%',textAlign:'left',padding:'12px 14px',marginBottom:8,
                borderRadius:12,border:'1.5px solid #FF3B3040',background:reversaNC.trim()?'#FF3B3008':'#F7F7F9',
                cursor:reversaNC.trim()?'pointer':'not-allowed',fontSize:13,fontWeight:700,
                color:reversaNC.trim()?'#1C1C1E':'#AEAEB2'}}>
              ⛔ Cliente anula la compra{reversaNC.trim()?'':' — registra la NC primero'}
              <span style={{display:'block',fontSize:10.5,fontWeight:600,color:'#8E8E93',marginTop:2}}>
                Requiere la Nota de Crédito emitida en el POS</span>
            </button>
            <input value={reversaNC} onChange={e=>setReversaNC(e.target.value)}
              placeholder="N° Nota de Crédito (obligatorio para anular) *"
              style={{width:'100%',padding:'9px 12px',border:'1.5px solid #FF3B3040',borderRadius:9,fontSize:12.5,marginTop:2,background:'#FFFBFB'}}/>
            <input value={reversaDetalle} onChange={e=>setReversaDetalle(e.target.value)}
              placeholder="Detalle opcional (ej: vuelve el sábado con camioneta…)"
              style={{width:'100%',padding:'9px 12px',border:'1px solid #D8D8DE',borderRadius:9,fontSize:12.5,marginTop:8}}/>
            <div style={{fontSize:10.5,color:'#8E8E93',marginTop:8}}>
              Se registra quién reversó y cuándo. El comprobante firmado original queda como respaldo histórico.
            </div>
            <button onClick={()=>setReversarSel(null)}
              style={{marginTop:10,width:'100%',padding:'9px 0',borderRadius:10,border:'none',
                background:'#F2F2F7',fontSize:12.5,fontWeight:700,cursor:'pointer'}}>Cancelar</button>
          </div>
        </div>
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(0,0,0,0.45)',zIndex:9000,display:'flex',alignItems:'center',justifyContent:'center',padding:16}}
          ...
          <div style={{background:'#fff',borderRadius:16,padding:20,width:'100%',maxWidth:420,maxHeight:'80vh',overflowY:'auto'}} onClick={e=>e.stopPropagation()}>
            <div style={{fontSize:14,fontWeight:800,marginBottom:2}}>
              {pickerSel.mixtaFase?('✍️ ¿Quién pickeó '+(pickerSel.mixtaFase==='bodega'?'ZONA BODEGA · PRIMERA':'ZONA SEGUNDA / DESCONT.')+'?')
                :pickerSel.alCerrar?'✍️ ¿Quién hizo este picking?'
                :pickerSel.reasignar?'🔄 Cambiar pickeador':'👤 Registrar pickeador'}
            </div>
            {pickerSel.alCerrar&&(
              <div style={{fontSize:11,fontWeight:700,color:'#B36B00',background:'#FF950012',
                border:'1px solid #FF950030',borderRadius:8,padding:'6px 9px',margin:'6px 0 4px'}}>
                Obligatorio para cerrar: al elegir, la orden queda LISTA para entrega.
              </div>
            )}
      ...
      {colaFull&&(()=>{
        // ── MODO TV BODEGA · tablero operativo: la cola y el picking, a lo grande ──
        // Columnas = estados de trabajo. Fuente y grilla se auto-ajustan a la carga.
        const cols=[
          ['POR TOMAR',   '📥', ordenes.filter(o=>o.estado==='pendiente'
            &&!(o.bsale_doc_type==='cotizacion'&&!o.despacho_registrado_at&&!o.pickeador_id)),  '#8E8E93'],
          ['ASIGNADAS',   '👤', ordenes.filter(o=>o.estado==='asignada'),   '#007AFF'],
          ['EN PICKING',  '🏃', ordenes.filter(o=>o.estado==='en_picking'), '#FF9500'],
          ['LISTAS',      '✅', ordenes.filter(o=>o.estado==='lista'),      '#34C759'],
        ]
        const total=cols.reduce((a,[,,l])=>a+l.length,0)
        // escala global: con poca carga las tarjetas son gigantes; con mucha, compactas
        const g = total>40?0.62 : total>26?0.74 : total>14?0.88 : 1
        const gx=(n)=>Math.round(n*g)
        const minCol=(o)=>Math.max(0,Math.round((reloj-new Date(o.recibida_at).getTime())/60000))
        const semaf=(m)=>m>=30?'#FF453A':m>=15?'#FFD60A':'#34C759'
        const nom1=(n)=>{const t=(n||'').trim().split(/\s+/)[0]||'';return t.charAt(0).toUpperCase()+t.slice(1).toLowerCase()}
        return (
          <div style={{position:'fixed',inset:0,zIndex:9600,display:'flex',flexDirection:'column',gap:gx(16),
            background:'radial-gradient(circle at 20% 0%, #24243e 0%, #0f0c29 55%, #08061a 100%)',padding:'22px 26px'}}>
            {/* cabecera */}
            <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:14,
              paddingBottom:gx(12),borderBottom:'1px solid rgba(255,255,255,0.1)'}}>
              <div style={{display:'flex',alignItems:'center',gap:gx(16)}}>
                <div style={{fontSize:gx(30),fontWeight:900,color:'#fff'}}>🖥 TABLERO DE BODEGA</div>
                <div style={{fontSize:gx(16),fontWeight:700,color:'rgba(255,255,255,0.5)'}}>
                  {sucSel==='todas'?'Todas las tiendas':nomSuc(sucSel)} · {total} órdenes en juego
                </div>
              </div>
              <div style={{display:'flex',alignItems:'center',gap:gx(16)}}>
                <div style={{fontSize:gx(30),fontWeight:900,color:'#fff',fontFamily:'ui-monospace,monospace'}}>
                  {new Date(reloj).toLocaleTimeString('es-CL',{hour:'2-digit',minute:'2-digit'})}
                </div>
                <button onClick={()=>setColaFull(false)}
                  style={{padding:'8px 16px',borderRadius:9,border:'1px solid rgba(255,255,255,0.25)',cursor:'pointer',
                    background:'rgba(255,255,255,0.08)',color:'#fff',fontSize:13,fontWeight:800}}>✕ Salir (ESC)</button>
              </div>
            </div>
            {/* 4 columnas de estados */}
            <div style={{flex:1,display:'grid',gridTemplateColumns:'repeat(4,1fr)',gap:gx(16),minHeight:0}}>
              {cols.map(([tit,ic,lista,c])=>(
                <div key={tit} style={{display:'flex',flexDirection:'column',minHeight:0,borderRadius:gx(16),
                  background:'rgba(255,255,255,0.045)',border:`1px solid ${c}44`,overflow:'hidden'}}>
                  <div style={{padding:`${gx(10)}px ${gx(14)}px`,borderBottom:`3px solid ${c}`,
                    background:`linear-gradient(90deg, ${c}30, transparent)`,
                    display:'flex',alignItems:'center',justifyContent:'space-between'}}>
                    <span style={{fontSize:gx(19),fontWeight:900,color:'#fff',letterSpacing:'0.05em'}}>{ic} {tit}</span>
                    <span style={{fontSize:gx(19),fontWeight:900,color:c,background:'rgba(255,255,255,0.12)',
                      padding:`${gx(1)}px ${gx(12)}px`,borderRadius:gx(20)}}>{lista.length}</span>
                  </div>
                  <div style={{flex:1,overflowY:'auto',padding:gx(10),display:'flex',flexDirection:'column',gap:gx(8)}}>
                    {lista.length===0&&<div style={{textAlign:'center',color:'rgba(255,255,255,0.16)',
                      fontSize:gx(18),fontWeight:800,padding:`${gx(24)}px 0`,letterSpacing:'0.08em'}}>VACÍO</div>}
                    {lista.map(o=>{const m=minCol(o);return(
                      <div key={o.id} style={{borderRadius:gx(12),padding:`${gx(9)}px ${gx(12)}px`,
                        background:o.urgente?'rgba(255,69,58,0.14)':'rgba(255,255,255,0.06)',
                        border:o.urgente?'1.5px solid #FF453A':'1px solid rgba(255,255,255,0.1)'}}>
                        <div style={{display:'flex',alignItems:'baseline',justifyContent:'space-between',gap:8}}>
                          <span style={{fontFamily:'ui-monospace,monospace',fontWeight:900,fontSize:gx(30),
                            color:'#fff',lineHeight:1}}>{o.folio}{o.urgente?' ⚡':''}</span>
                          <span style={{fontSize:gx(16),fontWeight:900,color:semaf(m),whiteSpace:'nowrap'}}>{m}′</span>
                        </div>
                        <div style={{display:'flex',alignItems:'center',justifyContent:'space-between',gap:8,marginTop:gx(4)}}>
                          <span style={{fontSize:gx(14),fontWeight:700,color:'rgba(255,255,255,0.55)',
                            overflow:'hidden',textOverflow:'ellipsis',whiteSpace:'nowrap'}}>
                            {o.modalidad_entrega==='despacho'?'🚚':'🏬'} {nom1(o.cliente_nombre)||'—'}
                          </span>
                          {o.tipo_entrega==='programada'&&
                            <span style={{fontSize:gx(11),fontWeight:900,whiteSpace:'nowrap',padding:`${gx(1)}px ${gx(7)}px`,
                              borderRadius:gx(7),color:'#0A0A14',
                              background:(o.bsale_doc_type==='pedido_web'||/p[aá]gina\s*web/i.test(o.vendedor_nombre||''))?'#64D2FF':'#BF5AF2'}}>
                              {(o.bsale_doc_type==='pedido_web'||/p[aá]gina\s*web/i.test(o.vendedor_nombre||''))?'🌐 WEB':'📦 POR DESPACHAR'}
                            </span>}
                          {o.pickeador_nombre&&<span style={{fontSize:gx(13),fontWeight:800,color:'#0A84FF',whiteSpace:'nowrap'}}>
                            👤 {nom1(o.pickeador_nombre)}</span>}
                        </div>
                      </div>
                    )})}
                  </div>
            ...
            <div style={{textAlign:'center',fontSize:gx(12),color:'rgba(255,255,255,0.3)',fontWeight:600}}>
              Actualización automática en vivo · semáforo de espera: verde &lt;15′ · amarillo 15–30′ · rojo ≥30′
            </div>
          </div>
        )
      })()}
      ...
      {tabVista==='pantalla'&&(()=>{
        ...
        // EN PREPARACIÓN: solo entregas INMEDIATAS (cliente presente en tienda).
        // Los pedidos web con retiro (programada/retiro) NO se muestran mientras se
        // preparan — nadie los espera en el mesón — pero SÍ aparecen en LISTO PARA
        // RETIRAR cuando quedan listos, para el cliente que llega avisado.
        const preparando = deRetiro.filter(o=>o.tipo_entrega!=='programada'
          && ['pendiente','asignada','en_picking'].includes(o.estado))
        ...
        const columna = (titulo, ic, lista, color, destacada)=>(
          <div style={{flex:destacada?1.15:1,minWidth:px(360),display:'flex',flexDirection:'column',
            ...
            {(()=>{ // escala por densidad: más pedidos → tarjetas y fuente más compactas
              const k=lista.length
              window.__pkCS = k>28?0.58 : k>18?0.72 : k>10?0.86 : 1
              return null })()}
            <div style={{flex:1,padding:px(20),display:'grid',gap:px(12),alignContent:'flex-start',
              gridTemplateColumns:`repeat(auto-fill, minmax(${Math.round(px(170)*(window.__pkCS||1))}px, 1fr))`,
                ...
                : lista.map(o=>(
                  <div key={o.id} className={destacada?'pk-listo':''}
                    style={{background:destacada?`linear-gradient(145deg, ${color}, #1F9B49)`:'rgba(255,255,255,0.07)',
                      ...
                      borderRadius:px(16),padding:`${Math.round(px(14)*(window.__pkCS||1))}px ${Math.round(px(18)*(window.__pkCS||1))}px`,textAlign:'center',
                    ...
                    <div style={{fontFamily:'ui-monospace,SFMono-Regular,monospace',fontWeight:900,
                      fontSize:Math.round(px(destacada?60:44)*(window.__pkCS||1)),lineHeight:1,
                    ...
                    {primerNombre(o.cliente_nombre)&&
                      <div style={{fontSize:Math.round(px(17)*(window.__pkCS||1)),fontWeight:700,marginTop:px(4),letterSpacing:'0.04em',
      ...
      {tabVista==='cola'&&(
        ...
        <div style={colaMax
          ? {position:'fixed',inset:0,zIndex:9550,background:'#F2F2F7',padding:10,
             display:'flex',gap:10,alignItems:'stretch',flexWrap:'wrap',overflowY:'auto'}
          : {display:'flex',gap:12,alignItems:'stretch',flexWrap:'wrap'}}>
          <div style={{flex:'1 1 470px',minWidth:390,border:'1px solid #E5E5EA',borderRadius:10,background:'#fff',
            maxHeight:colaMax?'calc(100vh - 20px)':'74vh',overflowY:'auto',overflowX:'auto'}}>
            {/* Bandejas — identidad visual fuerte por tipo de entrega */}
            <div style={{padding:'8px',background:'#FAFAFC',position:'sticky',top:0,zIndex:2,
              borderBottom:`3px solid ${bandeja==='enruta'?'#AF52DE':bandeja==='programada'?'#5856D6':'#007AFF'}`}}>
            <div style={{display:'flex',gap:6}}>
              {[['inmediata','⚡','ENTREGA INMEDIATA',inmediatas.length,'#007AFF',''],
                ['programada','🚚','POR DESPACHAR',programadas.length,'#5856D6',progVencidas?`${progVencidas} para hoy ⚠`:''],
                ['enruta','🚛','EN RUTA',enRutaCola.length,'#AF52DE',enRutaAtrasadas?`${enRutaAtrasadas} +8 días ⚠`:'']].map(([k,ic,l,n,c,extra])=>(
              ...
              <button onClick={()=>setFilSeg(f=>f==='todos'?'si':f==='si'?'no':'todos')}
                title="Filtro de segunda selección — clic para alternar: todas → solo 2ª → sin 2ª"
                style={{marginLeft:'auto',padding:'0 13px',borderRadius:9,cursor:'pointer',fontSize:11,fontWeight:900,
                  whiteSpace:'nowrap',transition:'background 0.15s',
                  border:filSeg==='todos'?'1.5px solid #E0C9A0':'none',
                  background:filSeg==='si'?'#FF9500':filSeg==='no'?'#34C759':'#FFF9EF',
                  color:filSeg==='todos'?'#B25000':'#fff'}}>
                {filSeg==='todos'?`⚠ 2ª SELECCIÓN · ${nSegundaTotal}`
                  :filSeg==='si'?'⚠ SOLO 2ª SELECCIÓN'
                  :'✓ SIN 2ª SELECCIÓN'}
              </button>
              <button onClick={()=>setColaMax(m=>!m)} title="Ampliar el espacio de trabajo a toda la pantalla (ESC para salir)"
                style={{padding:'0 14px',borderRadius:9,border:'none',cursor:'pointer',
                  background:colaMax?'#FF9500':'#007AFF',color:'#fff',fontSize:11.5,fontWeight:900,whiteSpace:'nowrap'}}>
                {colaMax?'✕ REDUCIR':'⛶ AMPLIAR'}
              </button>
              <button onClick={()=>setColaFull(true)} title="Tablero de bodega en pantalla completa (ESC para salir)"
                style={{padding:'0 14px',borderRadius:9,border:'none',cursor:'pointer',
                  background:'#1a1a2e',color:'#fff',fontSize:11.5,fontWeight:900,whiteSpace:'nowrap'}}>
                🖥 MODO TV
              </button>
            </div>
            {/* BUSCADOR DE COLA — filtra la bandeja visible; Enter abre el único resultado */}
            <div style={{display:'flex',gap:6,alignItems:'center',marginTop:6}}>
              <div style={{position:'relative',flex:1,display:'flex',alignItems:'center'}}>
                <span style={{position:'absolute',left:10,fontSize:12,pointerEvents:'none',opacity:0.55}}>🔍</span>
                <input value={colaTxt} onChange={e=>setColaTxt(e.target.value)}
                  onKeyDown={e=>{
                    if(e.key==='Escape'){ e.stopPropagation(); setColaTxt('') }
                    if(e.key==='Enter'&&activas.length===1){ setSelId(activas[0].id) }
                  }}
                  placeholder="Buscar en la bandeja — folio, cliente, RUT, guía, dirección…"
                  style={{...css.input,width:'100%',padding:'7px 30px 7px 28px',fontSize:12,
                    border:`1.5px solid ${busq?(bandeja==='programada'?'#5856D6':'#007AFF'):'#E5E5EA'}`,
                    background:busq?'#fff':'#F7F7FA'}}/>
                {colaTxt&&(
                  <button onClick={()=>setColaTxt('')} title="Limpiar la búsqueda (ESC)"
                    style={{position:'absolute',right:6,border:'none',background:'transparent',cursor:'pointer',
                      fontSize:13,fontWeight:900,color:'#8E8E93',lineHeight:1,padding:'2px 4px'}}>✕</button>
                )}
              </div>
              {busq&&(
                <span style={{fontSize:11,fontWeight:800,whiteSpace:'nowrap',
                  color:activas.length?(bandeja==='programada'?'#5856D6':'#007AFF'):'#FF3B30'}}>
                  {activas.length} {activas.length===1?'resultado':'resultados'}
                  {activas.length===1&&<span style={{color:'#8E8E93',fontWeight:700}}> · Enter para abrir</span>}
                  {!activas.length&&nOtraBandeja>0&&<span style={{color:'#FF9500'}}> · {nOtraBandeja} en la otra bandeja</span>}
                </span>
              )}
            </div>
            </div>
            <table style={{width:'100%',borderCollapse:'collapse'}}>
              <thead><tr>
                <th style={th}>{bandeja==='enruta'?'FUERA':bandeja==='programada'?'FECHA':'ESPERA'}</th><th style={th}>FOLIO</th><th style={{...th,width:30}}>MOD</th>
              ...
              <tbody>
                ...
                {!activas.length&&(
                  <tr><td colSpan={sucSel==='todas'?8:7} style={{...td,textAlign:'center',padding:'28px 0',color:'#8E8E93'}}>
                    {busq
                      ? <>🔍 Sin coincidencias para <b style={{color:'#1C1C1E'}}>«{colaTxt.trim()}»</b> en esta bandeja
                          {otrasBandejas.length>0&&(
                            <div style={{marginTop:8,display:'flex',gap:6,justifyContent:'center',flexWrap:'wrap'}}>
                              {otrasBandejas.map(b=>(
                                <button key={b.k} onClick={()=>setBandeja(b.k)}
                                  style={{padding:'6px 14px',borderRadius:9,border:'none',cursor:'pointer',fontSize:11.5,fontWeight:900,
                          ...
                          <div style={{marginTop:8,fontSize:11}}>
                            Buscá por folio, cliente, RUT, guía o dirección
                            {filSeg!=='todos'&&<> · ⚠ tenés activo el filtro de 2ª selección</>}
                            {sucSel!=='todas'&&<> · viendo solo {nomSuc(sucSel)}</>}
                          </div>
                        </>
                      : bandeja==='enruta'?'🚛 Nada en ruta — todos los despachos tienen su entrega acreditada'
                      : bandeja==='programada'?'📅 Sin entregas programadas':'🛒 Bandeja vacía — cada venta aparece aquí en menos de un minuto'}
                  </td></tr>
                )}
                {bandeja==='programada'&&esCD&&porConfirmar.length>0&&(<>
                  <tr onClick={()=>setVerPorConf(v=>!v)} style={{cursor:'pointer'}}>
                    <td colSpan={sucSel==='todas'?8:7}
                      style={{...th,fontSize:9.5,paddingTop:10,paddingBottom:8,color:'#8A6116',background:'#FFF9EF'}}>
                      📋 COTIZACIONES SIN REGISTRO EN COMERCIAL · {porConfirmar.length}
                      <span style={{fontWeight:600,textTransform:'none',letterSpacing:0,marginLeft:8}}>
                        invisibles para la operación hasta que el vendedor las registre en Despachos · 7+ días se anulan solas</span>
                      <span style={{float:'right',fontWeight:900}}>{busq?'🔍 resultados':verPorConf?'▲ plegar':'▼ ver'}</span>
                    </td></tr>
                  {(verPorConf||!!busq)&&porConfirmar.map(o=><FilaCola key={o.id} o={o}/>)}
                </>)}
                {bandeja==='inmediata'&&(entregadasHoyV.length>0||anuladasV.length>0||desistidasV.length>0)&&(
                  <tr><td colSpan={sucSel==='todas'?8:7} style={{...th,fontSize:9,paddingTop:10}}>
                    TERMINADAS HOY · {entregadasHoyV.length} entregadas{anuladasV.length?` · ${anuladasV.length} anuladas`:''}{desistidasV.length?` · ${desistidasV.length} desistidas`:''}</td></tr>
                )}
                {bandeja==='inmediata'&&entregadasHoyV.slice(0,15).map(o=><FilaCola key={o.id} o={o} terminada/>)}
                {bandeja==='inmediata'&&anuladasV.slice(0,8).map(o=><FilaCola key={o.id} o={o} terminada/>)}
                {bandeja==='inmediata'&&desistidasV.slice(0,8).map(o=><FilaCola key={o.id} o={o} terminada/>)}
              </tbody>
            </table>
          </div>
          <div style={{flex:'1 1 490px',minWidth:410,border:'1px solid #E5E5EA',borderRadius:10,background:'#fff',
            maxHeight:colaMax?'calc(100vh - 20px)':undefined,overflowY:colaMax?'auto':undefined,
      ...
      {tabVista==='reporte'&&(<>
        ...
        {!repLoading&&repData&&(()=>{
          ...
          rows.forEach(o=>{
            ...
            else if(o.estado==='anulada'||o.estado==='desistida') s.anul++
            else s.proceso++
          })
          ...
          const pendientes = rows.filter(o=>!['entregada','anulada','desistida'].includes(o.estado))
                ...
                <table style={{width:'100%',borderCollapse:'collapse'}}>
                  ...
                  <tbody>{pendientes.map(o=>{const m=minsDesde(o.recibida_at);const e=EST_PICK[o.estado];return(
                    <tr key={o.id} style={{cursor:'pointer'}} onClick={()=>{
                      setTabVista('cola')
                      setBandeja(o.estado==='despachada'?'enruta':o.tipo_entrega==='programada'?'programada':'inmediata')
                      setSelId(o.id)}}>
