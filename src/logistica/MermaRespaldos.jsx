...
export default function MermaRespaldos({ merma, cu, onChange, compacto = false }) {
  ...
  async function subir(files, clase, origen = 'ARCHIVO') {
    ...
    if (!lista.length) {
      setMsg({t:'error', m:'No se recibió ningún archivo. Intenta de nuevo.'})
      return
    }
    ...
    try {
      ...
      if (ok > 0) {
        ...
        const { data: upd, error: eUpd } = await supabase.from('log_mermas')
          .update(patch).eq('id', merma.id).select('estado').maybeSingle()
        if (eUpd) errores.push(`No se pudo actualizar la baja: ${eUpd.message}`)
        else onChange && onChange(upd?.estado)
      }
              ...
              <>
                <label style={btn('solid')}>
                  ...
                  <input ref={camRef} type="file" accept="image/*" capture="environment" multiple
                    ...
                    onChange={e=>{const f=Array.from(e.target.files||[]); e.target.value=''; subir(f,'FOTO','CAMARA')}}/>
                </label>
                <label style={btn('outline')}>
                  SUBIR IMÁGENES
                  <input type="file" accept="image/*" multiple style={{display:'none'}} disabled={!!subiendo}
                    onChange={e=>{const f=Array.from(e.target.files||[]); e.target.value=''; subir(f,'FOTO')}}/>
                </label>
              </>
            )}
            {clase !== 'FOTO' && (
              <label style={btn(falta ? 'solid' : 'outline')}>
                ...
                <input type="file" accept={cfg.accept} multiple style={{display:'none'}} disabled={!!subiendo}
                  onChange={e=>{const f=Array.from(e.target.files||[]); e.target.value=''; subir(f,clase)}}/>
              </label>
            )}
          </div>
        </div>
