...
export default function InvAnalisisView({cu, sucs = [], soloSuc = null, embedded = false, onBack, onPdfTrimestral}) {
  ...
  const D = useMemo(() => {
    ...
    return {cabsA, cabsP, detsA, detsP, abc, M, Mprev, pPrev, ventaSuc, cruces, rebPrev:rebPrevP, evento, libro, arrastresN, porSuc, porCat, porInv, topSku, top10Share,
  ...
  const TABS = [
    ...
    {k:'rebajas',   l:'REBAJAS PREVIAS', r:!puedeVerCostos},
    {k:'libro',     l:'LIBRO'},
    ...
    <div style={embedded ? {padding:0} : css.body}>
      ...
      {cabs === null ? <Vacio t="Cargando análisis…"/>
        ...
        : (<>
          ...
          {tab === 'rebajas' && <TabRebajas {...ctx}/>}
          {tab === 'libro' && <TabLibro {...ctx}/>}
...
export function TabResumen({D, riesgo, puedeVerCostos, nombreSuc, setTab, setSuc, setPeriodo, anio, suc, periodo, padreDe, hayVentas}) {
  ...
  const puertas = [
    ...
    ...(puedeVerCostos ? [{k:'rebajas', t:'Rebajas previas', d:'Pérdida que se rebajó antes de contar', v:fmtM((D.rebPrev || []).filter(x => x.cuadro).reduce((s, x) => s + x.val, 0)), c:(D.rebPrev || []).some(x => x.cuadro) ? IV.rojo : IV.verde}] : []),
    {k:'libro', t:'Libro', d:'Conteo por conteo y el acumulado por categoría', v:`${fmtN(D.porInv.length)} asientos`, c:IV.navy},
  ...
  return (<>
    ...
    <div style={{marginBottom:20}}>
      ...
      <Caja><div style={{padding:'10px 8px 4px'}}><GrafEvento b={D.evento || {}} base={base}/></div>
        <Leyenda items={[[IV.ambar, 'antes del conteo'], [IV.navy, 'el día del conteo'], [IV.slate, 'después del conteo']]}/></Caja>
    </div>
...
export function TabLibro({D, nombreSuc, puedeVerCostos, abrirFicha}) {
  ...
  return (<>
    ...
    <div style={{marginBottom:20}}>
      ...
      <Caja><div style={{padding:'10px 8px 4px'}}><GrafAcumulado series={series} puntos={puntos} fmt={aV ? fmtSM : (v => `${v > 0 ? '+' : ''}${fmtN(v)}`)}/></div></Caja>
    </div>
