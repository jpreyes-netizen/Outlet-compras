...
export default function InvAnalisisView({cu, sucs = [], soloSuc = null, embedded = false, onBack, onPdfTrimestral}) {
  ...
  const D = useMemo(() => {
    ...
    const porInv = cabsP.map(c => ({cab:c, ...calcular(detsP.filter(d => d.inventario_id === c.id), abc, cabIdx, precioDe)}))
    ...
    return {cabsA, cabsP, detsA, detsP, abc, M, Mprev, pPrev, ventaSuc, porSuc, porCat, porInv, topSku, top10Share,
  ...
  const ctx = {D, cabs:cabs || [], cabIdx, nombreSuc, sucsDisp, puedeVerCostos, setTab, setSuc, setPeriodo,
               ...
               ventaSuc:D.ventaSuc, padreDe, hayVentas}
...
export function GrafPuente({items, total}) {
  ...
  return (
    <svg viewBox={`0 0 ${GW} ${H}`} width="100%" role="img" style={{display:'block'}}>
      ...
      {segs.map((s, i) => {
        ...
        return (
          <g key={i}>
            <title>{`${s.label}: ${fmtCLP(s.v)}${s.total ? '' : ` · acumulado ${fmtCLP(s.b)}`}`}</title>
            ...
            <text x={GW - RW + 10} y={y + RH / 2 - 1} dominantBaseline="middle" fontSize="11.5"
              fontWeight={s.total ? 800 : 700} fill={c}>{fmtSM(s.v)}</text>
          </g>
        )
      })}
    </svg>
