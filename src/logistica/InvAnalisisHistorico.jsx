...
export function InvAnalisisHistorico({ cu, sucs, onBack }) {
  ...
  const [detalle, setDetalle] = useState(null)   // {tipo|null, mes} → modal de cálculo
  ...
  const celdaBalance = (c) => {
    ...
    const netoC = c.sobrante - c.perdida
    filasB.push(
      <div key="n" style={{ borderTop: '1px dashed #C7C7CC', marginTop: 2, paddingTop: 1.5,
        color: netoC < 0 ? '#C93400' : netoC > 0 ? '#248A3D' : '#8E8E93', fontWeight: 900 }}>
        ={fmtM(netoC)}
        <span style={{ fontWeight: 700, fontSize: '0.85em', opacity: 0.8 }}> ·{c.uds > 0 ? '+' : ''}{fmtN(c.uds)}u</span>
      </div>)
  ...
  return (
    <div style={{ padding: 16 }}>
      ...
      {tab === 'grilla' && (
        <>
          ...
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'center',
            ...
            <span style={{ color: '#007AFF', fontWeight: 700 }}>👆 pincha cualquier celda para ver cómo se calculó</span>
                  ...
                  return (
                    <tr key={t}>
                      ...
                      {meses.map(m => {
                        ...
                        return (
                          <td key={m} onClick={() => c && c.contadas > 0 && setDetalle({ tipo: t, mes: m })}
                            style={{ ...td, textAlign: 'right', fontWeight: v ? 700 : 400,
                            color: colorCelda(v, c),
                            fontSize: metrica === 'balance' ? 10 : undefined,
                            cursor: c && c.contadas > 0 ? 'pointer' : 'default',
              ...
              <tfoot>
                <tr style={{ background: '#FAFAFC', borderTop: '2px solid #E5E5EA' }}>
                  ...
                  {meses.map(m => {
                    ...
                    return <td key={m} onClick={() => totMes[m] && totMes[m].contadas > 0 && setDetalle({ tipo: null, mes: m })}
                      style={{ ...td, textAlign: 'right', fontWeight: 900, color: colorCelda(v),
                        cursor: totMes[m] && totMes[m].contadas > 0 ? 'pointer' : 'default',
                        fontSize: metrica === 'balance' ? 10 : undefined }}>
      ...
      {/* ══ MODAL: cómo se calculó este número ══ */}
      {detalle && (() => {
        const c = detalle.tipo === null ? totMes[detalle.mes] : mat[detalle.tipo]?.[detalle.mes]
        if (!c) return null
        const netoC = c.sobrante - c.perdida
        const eriC = c.contadas > 0 ? Math.round(c.cuadran / c.contadas * 100) : null
        const mermaC = c.valorInv > 0 ? ((c.perdida - c.sobrante) / c.valorInv * 100) : null
        const descC = c.valorInv > 0 ? ((c.perdida + c.sobrante) / c.valorInv * 100) : null
        // Líneas de origen: los conteos que componen esta celda
        const lineasCel = detsFil.filter(d => {
          const cab = cabById[d.inventario_id]; if (!cab || mesDe(cab) !== detalle.mes) return false
          if (detalle.tipo !== null && (d.tipo_producto || '(sin tipología)').trim() !== detalle.tipo) return false
          return d.stock_fisico !== null && d.stock_fisico !== undefined
        })
        const conDif = lineasCel.filter(d => Number(d.diferencia || 0) !== 0)
          .sort((a, b) => Math.abs(Number(b.diferencia) * Number(b.costo_unitario || 0))
                        - Math.abs(Number(a.diferencia) * Number(a.costo_unitario || 0)))
        const invsCel = [...new Set(lineasCel.map(d => d.inventario_id))]
          .map(id => cabById[id]).filter(Boolean)
        const filaF = (nombre, formula, resultado, activa) => (
          <div key={nombre} style={{ display: 'flex', gap: 8, alignItems: 'baseline', padding: '6px 9px',
            borderRadius: 8, background: activa ? '#007AFF0C' : 'transparent',
            border: activa ? '1px solid #007AFF30' : '1px solid transparent' }}>
            <span style={{ fontSize: 10, fontWeight: 900, minWidth: 92,
              color: activa ? '#007AFF' : '#6D6D72' }}>{nombre}{activa ? ' ◀' : ''}</span>
            <span style={{ fontSize: 11, fontFamily: 'monospace', color: '#3C3C3E', flex: 1 }}>{formula}</span>
            <span style={{ fontSize: 12.5, fontWeight: 900 }}>{resultado}</span>
          </div>
        )
        return (
          <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 9000,
            ...
            <div style={{ background: '#fff', borderRadius: 16, padding: 18, width: '100%', maxWidth: 780,
              maxHeight: '88vh', overflowY: 'auto' }} onClick={e => e.stopPropagation()}>
              <div style={{ fontSize: 15, fontWeight: 900 }}>
                {detalle.tipo === null ? 'TOTAL — todas las tipologías' : detalle.tipo}
              </div>
              <div style={{ fontSize: 11.5, color: '#8E8E93', fontWeight: 600, marginBottom: 12 }}>
                {mesLabel(detalle.mes)} · {sucSel === 'todas' ? 'todas las sucursales' : sucSel} ·
                {' '}{fmtN(c.contadas)} líneas contadas de {fmtN(c.lineas)}
              </div>

              {/* Los ingredientes del cálculo */}
              <div style={{ display: 'flex', gap: 8, flexWrap: 'wrap', marginBottom: 12 }}>
                {[
                  ['LÍNEAS CONTADAS', fmtN(c.contadas), '#1C1C1E'],
                  ['CUADRAN EXACTO', fmtN(c.cuadran), '#248A3D'],
                  ['FALTANTE', `${fmtCLP(c.perdida)} · ${fmtN(c.udsF)}u`, '#C93400'],
                  ['SOBRANTE', `${fmtCLP(c.sobrante)} · ${fmtN(c.udsS)}u`, '#248A3D'],
                  ['INVENTARIO CONTADO', fmtCLP(c.valorInv), '#5856D6'],
                ].map(([l, v, col]) => (
                  <div key={l} style={{ flex: '1 1 130px', padding: '7px 10px', borderRadius: 9,
                    background: col + '0A', border: `1px solid ${col}22` }}>
                    <div style={{ fontSize: 8.5, fontWeight: 800, color: col }}>{l}</div>
                    <div style={{ fontSize: 12.5, fontWeight: 900, color: '#1C1C1E' }}>{v}</div>
                  </div>
                ))}
              </div>

              {/* Las 4 fórmulas con los números reales */}
              <div style={{ fontSize: 10, fontWeight: 800, color: '#6D6D72', margin: '0 2px 4px' }}>
                CÓMO SE CALCULA CADA INDICADOR (el marcado es el que estás viendo)
              </div>
              <div style={{ border: '1px solid #EFEFF4', borderRadius: 10, padding: 4, marginBottom: 12 }}>
                {filaF('BALANCE', `${fmtM(c.sobrante)} sobrante − ${fmtM(c.perdida)} faltante`,
                  fmtM(netoC) + ` · ${c.uds > 0 ? '+' : ''}${fmtN(c.uds)}u`, metrica === 'balance')}
                {filaF('ERI %', `${fmtN(c.cuadran)} cuadran ÷ ${fmtN(c.contadas)} contadas`,
                  eriC === null ? '—' : eriC + '%', metrica === 'eri')}
                {filaF('MERMA %', `(${fmtM(c.perdida)} − ${fmtM(c.sobrante)}) ÷ ${fmtM(c.valorInv)} inventario`,
                  mermaC === null ? '—' : mermaC.toFixed(1) + '%', metrica === 'merma')}
                {filaF('DESCUADRE %', `(${fmtM(c.perdida)} + ${fmtM(c.sobrante)}) ÷ ${fmtM(c.valorInv)} inventario`,
                  descC === null ? '—' : descC.toFixed(1) + '%', metrica === 'descuadre')}
              </div>

              {/* Fuente del dato */}
              <div style={{ fontSize: 10, fontWeight: 800, color: '#6D6D72', margin: '0 2px 4px' }}>
                FUENTE DEL DATO · {invsCel.length} inventario(s)
              </div>
              <div style={{ fontSize: 10.5, color: '#8E8E93', margin: '0 2px 6px', lineHeight: 1.4 }}>
                Conteos físicos del módulo Inventario contra el stock sistema de la foto BSALE al corte.
                El costo unitario es el vigente al momento del conteo.
              </div>
              <div style={{ display: 'flex', flexDirection: 'column', gap: 4, marginBottom: 12 }}>
                {invsCel.map(iv => (
                  <div key={iv.id} style={{ display: 'flex', gap: 8, alignItems: 'center', padding: '6px 10px',
                    borderRadius: 8, background: '#FAFAFC', border: '1px solid #EFEFF4', fontSize: 11 }}>
                    <span style={{ fontFamily: 'monospace', fontWeight: 700 }}>{iv.id}</span>
                    <span style={{ color: '#8E8E93' }}>{iv.fecha_planificada || String(iv.fecha_corte_stock || '').slice(0, 10)}</span>
                    <span style={{ color: '#8E8E93' }}>{iv.sucursal_nombre || iv.sucursal_codigo}</span>
                    <span style={{ marginLeft: 'auto', fontSize: 10, color: '#6D6D72' }}>{iv.categoria_asignada || ''}</span>
                  </div>
                ))}
              </div>

              {/* Las líneas que componen el número */}
              <div style={{ fontSize: 10, fontWeight: 800, color: '#6D6D72', margin: '0 2px 4px' }}>
                LÍNEAS CON DIFERENCIA · {conDif.length} de {fmtN(c.contadas)} contadas
                {c.cuadran > 0 && <span style={{ fontWeight: 600, color: '#8E8E93' }}> · las otras {fmtN(c.cuadran)} cuadran exacto</span>}
              </div>
              <div style={{ border: '1px solid #E5E5EA', borderRadius: 9, overflow: 'hidden' }}>
                <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                  <thead><tr>
                  ...
                  <tbody>
                    {conDif.slice(0, 40).map((d, i) => {
                      const dif = Number(d.diferencia || 0)
                      const cu = Number(d.costo_unitario || 0)
                      return (
                        <tr key={i}>
                          <td style={{ ...td, fontFamily: 'monospace', fontSize: 10 }}>{d.sku}</td>
                          <td style={{ ...td, maxWidth: 230, overflow: 'hidden', textOverflow: 'ellipsis',
                            whiteSpace: 'nowrap' }} title={d.producto}>{d.producto}</td>
                          <td style={{ ...td, textAlign: 'right' }}>{fmtN(d.stock_sistema)}</td>
                          <td style={{ ...td, textAlign: 'right' }}>{fmtN(d.stock_fisico)}</td>
                          <td style={{ ...td, textAlign: 'right', fontWeight: 800,
                            color: dif < 0 ? '#C93400' : '#248A3D' }}>{dif > 0 ? '+' : ''}{fmtN(dif)}</td>
                          <td style={{ ...td, textAlign: 'right', color: cu ? '#3C3C3E' : '#FF9500' }}>
                            {cu ? fmtCLP(cu) : 'sin costo'}</td>
                          <td style={{ ...td, textAlign: 'right', fontWeight: 700,
                            color: dif * cu < 0 ? '#C93400' : '#248A3D' }}>{fmtCLP(dif * cu)}</td>
                        </tr>
                      )
                    })}
                  </tbody>
              ...
              {conDif.length > 40 &&
                <div style={{ fontSize: 10.5, color: '#8E8E93', marginTop: 5 }}>
                  Mostrando las 40 de mayor impacto de {conDif.length}. El Excel exporta todo.</div>}
              <div style={{ display: 'flex', justifyContent: 'flex-end', marginTop: 12 }}>
                <Bt v="gry" sm onClick={() => setDetalle(null)}>Cerrar</Bt>
              </div>
            </div>
          </div>
        )
