...
import MermaRespaldos from './MermaRespaldos.jsx'
...
export function MermasView({ cu, sucs, soloSuc = null }) {
  ...
  // R6-TOTAL (26-08): soloSuc puede llegar como código único o como FAMILIA
  // (array sala+bodega auxiliar, ej: ['la','la_bodega']). En el ERP son una
  // sola unidad: el usuario de la sala ve y gestiona las mermas de ambas.
  const soloRaw = Array.isArray(soloSuc) ? soloSuc : (soloSuc ? [soloSuc] : null)
  const sinAlcance = (soloRaw||[]).includes('__sin_sucursal__') || (soloRaw||[]).includes('__sin_usuario__')
  const sucFamilia = sinAlcance ? null : (soloRaw ? soloRaw.map(normSuc).filter(Boolean) : null)
  const sucPropia = sucFamilia?.[0] || null
  const verTodo = !sucPropia && !sinAlcance
    ...
    try {
      let q = supabase.from('log_mermas')
        .select('id,bsale_consumption_id,bsale_office_id,sucursal_codigo,fecha,nota,tipo,' +
          'tipo_manual,clasificado_por,clasificado_at,' +
      ...
      if (sinAlcance) { setRows([]); setLoading(false); return }   // sin sucursal asignada
      if (sucFamilia?.length) q = q.in('sucursal_codigo', sucFamilia)
  ...
  const clasificar = async (m, tipo) => {
    ...
    // FIX 24-08: se declara la clasificación como MANUAL. Sin esta marca, el
    // sync de mermas (cada 3 h) reclasifica con los patrones automáticos y
    // revierte el trabajo del equipo (reporte de Omar: "se borran las de 2da").
    const patch = {
      tipo, tipo_manual: true,
      clasificado_por: cu?.nombre || '',
    ...
    flash(`Clasificada como ${tipoInfo(tipo).l} — queda protegida del reclasificador`)
  }
  ...
  if (sinAlcance) return (
    <div style={{ padding: 18 }}>
      <div style={{ padding: '14px 16px', borderRadius: 12, background: '#FF950010',
        border: '1px solid #FF950035' }}>
        <div style={{ fontSize: 13.5, fontWeight: 900, color: '#C93400', marginBottom: 4 }}>
          Sin sucursal asignada
        </div>
        <div style={{ fontSize: 12, color: '#8E4A42', fontWeight: 600, lineHeight: 1.45 }}>
          Las bajas de stock se muestran por sucursal, y tu usuario no tiene una asignada.
          Pídele a administración que te asigne la sucursal correspondiente en Accesos.
        </div>
      </div>
    </div>
  )
  ...
  let filas = rows.filter(m => {
    if (sinAlcance) return false                                     // sin alcance: nada
  ...
  const sinInforme = rows.filter(m => m.estado !== 'respaldado' && m.estado !== 'no_aplica').length
                    ...
                    <div style={{ flex: 1 }}>
                      <div onClick={() => abrir(m)}
                        style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:6,
                          cursor:'pointer', borderRadius:8, fontWeight:800, fontSize:12.5, padding:'10px 0',
                          background: m.estado === 'respaldado' ? '#F2F2F7' : '#1a1a2e',
                          color: m.estado === 'respaldado' ? '#6D6D72' : '#fff' }}>
                        {m.estado === 'respaldado' ? 'VER RESPALDOS' : 'ADJUNTAR RESPALDOS'}
                      </div>
                    </div>
                  </div>
                    ...
                    <tr key={m.id} style={{ background: m.estado === 'respaldado' ? 'transparent' : '#FFF9F8' }}>
                      ...
                      <td style={{ ...td, textAlign: 'right', whiteSpace: 'nowrap' }}>
                        <span style={{ display: 'inline-flex', gap: 6, alignItems: 'center' }}>
                          <MermaRespaldos merma={m} cu={cu} compacto/>
                          <span onClick={() => abrir(m)} title="Abrir para adjuntar respaldos"
                            style={{ cursor: 'pointer', fontSize: 10.5, fontWeight: 800, color: '#175CD3' }}>
                            ADJUNTAR
                          </span>
                        </span>
                      </td>
        ...
        <div style={{ position: 'fixed', inset: 0, background: 'rgba(0,0,0,0.45)', zIndex: 9000,
          ...
          <div style={{ background: '#fff', borderRadius: 16, padding: 18, width: '100%', maxWidth: 640,
            ...
            <div style={{ padding: '9px 11px', borderRadius: 9, marginBottom: 10,
              ...
              {sel.tipo_manual && (
                <div style={{ fontSize: 10.5, fontWeight: 700, color: '#248A3D', marginBottom: 5 }}>
                  🔒 Clasificada por {sel.clasificado_por || 'el equipo'}
                  {sel.clasificado_at ? ` · ${new Date(sel.clasificado_at).toLocaleDateString('es-CL',{day:'2-digit',month:'2-digit',hour:'2-digit',minute:'2-digit'})}` : ''}
                  {' — protegida del reclasificador automático'}
                </div>
              )}
            ...
            {/* ── Respaldos: informe obligatorio, fotos y documentos opcionales ── */}
            <MermaRespaldos merma={sel} cu={cu}
              onChange={(estadoNuevo, tipoNuevo)=>{
                const patch = {}
                if (estadoNuevo) patch.estado = estadoNuevo
                if (tipoNuevo)   { patch.tipo = tipoNuevo; patch.tipo_manual = true }
                if (!Object.keys(patch).length) return
                setRows(prev => prev.map(r => r.id === sel.id ? {...r, ...patch} : r))
                setSel(s => s ? {...s, ...patch} : s)
              }}/>
