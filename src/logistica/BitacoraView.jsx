...
const FLUJOS = [
  {k:'ENTRADA',  l:'Entradas',   c:'#1E7A44', d:'Recepciones de proveedor, CD o traslados entrantes'},
  {k:'SALIDA',   l:'Salidas',    c:'#175CD3', d:'Picking, retiros de cliente y despachos'},
  {k:'TRASLADO', l:'Traslados',  c:'#B25E09', d:'Reposiciones y devoluciones entre bodegas'},
  {k:'BAJA',     l:'Bajas',      c:'#B42318', d:'Mermas y consumos que salen de stock'},
  {k:'CONTEO',   l:'Conteos',    c:'#16213E', d:'Inventarios cíclicos y generales'},
]
const CAT_CHECKS = [
...
// ═══════════════════════════════════════════════════════════════════════════
// soloSuc: null = ve todas las bodegas · array de códigos = acotado a esa familia.
// Lo calcula LogisticaApp con alcanceMermasFamilia(), el mismo criterio de Mermas:
// cada bodega responde por la suya y el CD es una bodega más. Solo gerencia y
// administración del sistema ven el consolidado.
export default function BitacoraView({cu, sucs = [], soloSuc = undefined, onBack}) {
  // Fallback defensivo por si el módulo se monta sin la prop (no debería ocurrir)
  const ROLES_GLOBAL = ['admin','admin_sistema','dir_general','gerente','director']
  const alcance = useMemo(() => {
    if (soloSuc !== undefined) return soloSuc            // lo que decidió el shell
    if (ROLES_GLOBAL.includes(cu?.rol)) return null
    const base = cu?.contexto_activo_sesion || cu?.sucursal_codigo
    if (!base) return ['__sin_sucursal__']
    const padre = sucs.find(s=>s.codigo===base)?.codigo_padre || base
    const hijos = sucs.filter(s=>s.codigo_padre===padre).map(s=>s.codigo)
    return [padre, ...hijos]
  }, [soloSuc, cu?.rol, cu?.contexto_activo_sesion, cu?.sucursal_codigo, sucs])

  const esGlobal = alcance === null
  // Códigos que este usuario puede consultar; [] cuando ve todo
  const scope = useMemo(() => Array.isArray(alcance) ? alcance : [], [alcance])

  const bodegasVisibles = useMemo(() => {
    if (esGlobal) return BODEGAS
    return BODEGAS.filter(b => scope.includes(b.k))
  }, [esGlobal, scope])

  const [vista,   setVista]   = useState('dia')
  const [bodega,  setBodega]  = useState(bodegasVisibles[0]?.k || null)
  const [fecha,   setFecha]   = useState(hoyISO())
  const [msg,     setMsg]     = useState(null)

  const flash = (tipo, texto) => { setMsg({tipo, texto}); setTimeout(()=>setMsg(null), 4000) }

  // El usuario no tiene ninguna bodega en su alcance → no se muestra nada
  if (!esGlobal && bodegasVisibles.length === 0) return (
    <div style={css.body}>
      <div style={{border:`1px solid ${BV.line}`, borderRadius:4, background:'#fff',
        padding:'28px 26px', maxWidth:520, margin:'36px auto 0'}}>
        <div style={{fontSize:10.5, fontWeight:700, letterSpacing:1.2, color:BV.slate,
          textTransform:'uppercase'}}>Logística · Bitácora</div>
        <div style={{fontSize:17, fontWeight:800, color:BV.ink, margin:'4px 0 10px'}}>
          Sin bodega asignada
        </div>
        <div style={{fontSize:13, color:BV.slate, lineHeight:1.55}}>
          Tu cuenta no tiene una bodega asociada en Logística, así que no hay bitácora que mostrar.
          Pídele a un administrador que te asigne la sucursal correspondiente.
        </div>
      </div>
    </div>
  )
  ...
  return (
    <div style={css.body}>
      ...
      {vista==='hist'  && <VistaHist   cu={cu} bodega={bodega} flash={flash} esGlobal={esGlobal} scope={scope}
                                       onAbrir={(f)=>{setFecha(f); setVista('dia')}}/>}
      {vista==='cumpl'  && <VistaCumpl  bodegas={bodegasVisibles} esGlobal={esGlobal} scope={scope}/>}
      {vista==='riesgo' && <VistaRiesgo bodegas={bodegasVisibles} esGlobal={esGlobal} scope={scope} bodega={bodega}/>}
    </div>
  )
}
...
function VistaDia({cu, bodega, fecha, flash, esGlobal}) {
  ...
  const [sist, setSist]         = useState(null)   // resumen del WMS ese día
  const [movs, setMovs]         = useState([])     // libro de movimientos del día
  const [movFiltro, setMovFil]  = useState('TODOS')
  ...
  async function cargar() {
    if (!bodega) { setLoading(false); return }
    setLoading(true)
    try {
  ...
  // ── Libro de movimientos del día (lo que efectivamente registró el WMS) ──
  async function cargarSistema() {
    try {
      const { data, error } = await supabase.from('v_log_movimientos_dia')
        .select('*').eq('sucursal_codigo', bodega).eq('fecha', fecha)
        .order('hora', {ascending:true}).limit(1000)
      if (error) throw error
      const lista = data || []
      setMovs(lista)

      // Resumen por flujo, para el contraste con lo declarado
      const por = k => lista.filter(m => m.flujo === k)
      const invAct = await supabase.from('log_inv_cabeceras')
        .select('id, estado, fecha_planificada').eq('sucursal_codigo', bodega)
        .neq('estado','CERRADO').eq('es_prueba', false).limit(50)
      setSist({
        entradas:     por('ENTRADA').length,
        salidas:      por('SALIDA').length,
        traslados:    por('TRASLADO').length,
        bajas:        por('BAJA').length,
        conteos:      por('CONTEO').length,
        pendientes:   lista.filter(m => m.pendiente).length,
        unidadesOut:  por('SALIDA').reduce((s,m)=>s + (Number(m.unidades)||0), 0),
        montoBajas:   por('BAJA').reduce((s,m)=>s + (Number(m.monto_clp)||0), 0),
        invActivos:   (invAct.data || []).length,
        invDelDia:    (invAct.data || []).filter(i => i.fecha_planificada === fecha).length,
      })
    } catch (e) { setSist(null); setMovs([]) }
  }
  ...
  const contrasteDe = (act, l) => {
    ...
    if (act === 'RECEPCION') {
      if ((temaEsNo || l.avance === 'NO_APLICA') && sist.entradas > 0)
        return {t:`El WMS registra ${sist.entradas} recepción(es) hoy`, sev:'alto'}
      if (l.avance === 'REALIZADO' && !temaEsNo && sist.entradas === 0)
        return {t:'Sin recepciones registradas en el WMS', sev:'info'}
    }
    if (act === 'DESPACHO' && (temaEsNo || l.avance === 'NO_APLICA') && sist.salidas > 0)
      return {t:`El WMS registra ${sist.salidas} salida(s) hoy`, sev:'alto'}
    if (act === 'REPOSICIONES' && (temaEsNo || l.avance === 'NO_APLICA') && sist.traslados > 0)
      return {t:`Hay ${sist.traslados} traslado(s) del día`, sev:'alto'}
    if (act === 'LIMPIEZA' && l.avance === 'NO_APLICA' && sist.bajas > 0)
      return {t:`Se registraron ${sist.bajas} baja(s) hoy`, sev:'info'}
  ...
  function copiarInforme() {
    ...
    if (sist) L.push(`Movimientos WMS: ${sist.entradas} entradas · ${sist.salidas} salidas · ${sist.traslados} traslados · ${sist.bajas} bajas · ${sist.conteos} conteos · ${sist.pendientes} pendientes`)
    if (inc.length) {
  ...
  return (
    <>
      ...
      {/* Movimientos registrados por el WMS ese día */}
      {sist && (
        <div style={{display:'flex', flexWrap:'wrap', gap:0, border:`1px solid ${BV.line}`,
          ...
          fontSize:11.5, color:BV.slate, cursor:'pointer'}}
          onClick={()=>setSecc('movs')} title="Ver el detalle de los movimientos">
          ...
          {FLUJOS.map(f => {
            const n = {ENTRADA:sist.entradas, SALIDA:sist.salidas, TRASLADO:sist.traslados,
                       BAJA:sist.bajas, CONTEO:sist.conteos}[f.k]
            return (
              <div key={f.k} style={{padding:'7px 14px', borderRight:`1px solid ${BV.lineSoft}`}} title={f.d}>
                <span style={{fontWeight:800, color:n>0?f.c:BV.line, fontVariantNumeric:'tabular-nums'}}>{n}</span>
                {' '}{f.l}
              </div>
            )
          })}
          {sist.pendientes > 0 && (
            <div style={{padding:'7px 14px', borderLeft:`1px solid ${BV.line}`, marginLeft:'auto',
              color:BV.rojo, fontWeight:700}}>
              {sist.pendientes} sin cerrar
            </div>
          )}
        </div>
      )}
      ...
      <div style={{display:'flex', gap:20, borderBottom:`1px solid ${BV.line}`, marginBottom:12}}>
        {[{k:'checklist', l:`ACTIVIDADES (${checklist.length})`},
          ...
          {k:'movs',      l:`MOVIMIENTOS DEL DÍA (${movs.length})`},
      ...
      {/* ── MOVIMIENTOS DEL DÍA — registro de lo que hizo el WMS ── */}
      {secc==='movs' && (<>
        <BvGuia titulo="QUÉ ES ESTE REGISTRO">
          <strong>No se llena: se lee.</strong> Es todo lo que el sistema registró hoy en esta bodega,
          tomado directamente de los módulos de recepción, picking, reposición, mermas e inventario.
          <div style={{marginTop:8}}>
            Sirve para dos cosas. Primero, para declarar el checklist mirando lo que realmente pasó y no
            de memoria. Segundo, para ver qué quedó abierto: las filas marcadas como pendientes son
            documentos que no llegaron a su estado final y que mañana siguen ahí.
          </div>
          <div style={{marginTop:8}}>
            {FLUJOS.map(f => (
              <div key={f.k} style={{marginTop:3}}>
                <strong style={{color:f.c}}>{f.l}:</strong> {f.d}
              </div>
            ))}
          </div>
        </BvGuia>
        ...
        <div style={{display:'flex', gap:8, alignItems:'center', marginBottom:10, flexWrap:'wrap'}}>
          <select style={bvInput(160)} value={movFiltro} onChange={e=>setMovFil(e.target.value)}>
            <option value="TODOS">Todos los flujos</option>
            {FLUJOS.map(f=><option key={f.k} value={f.k}>{f.l}</option>)}
          </select>
          <button onClick={()=>setSoloPend(v=>!v)}
            style={{...bvBtn(soloPend?'solid':'outline'),
              ...(soloPend?{background:BV.rojo, borderColor:BV.rojo}:{})}}>
            {soloPend ? 'VIENDO SOLO PENDIENTES' : `SOLO PENDIENTES (${movs.filter(m=>m.pendiente).length})`}
          </button>
          <span style={{marginLeft:'auto', fontSize:11.5, color:BV.slate, fontVariantNumeric:'tabular-nums'}}>
            {movs.filter(m=>(movFiltro==='TODOS'||m.flujo===movFiltro) && (!soloPend||m.pendiente)).length}
            {' '}de {movs.length} movimientos
          </span>
        </div>
        ...
        <div style={{border:`1px solid ${BV.line}`, borderRadius:4, background:'#fff', overflowX:'auto'}}>
          <table style={{width:'100%', borderCollapse:'collapse', minWidth:900}}>
            <thead><tr style={{background:BV.bgHead}}>
              <th style={{...thS(), width:60}}>Hora</th>
              <th style={thS()}>Flujo</th>
              <th style={thS()}>Tipo</th>
              <th style={thS()}>Documento</th>
              <th style={thS()}>Referencia</th>
              <th style={thS(true)}>Unidades</th>
              <th style={thS(true)}>Monto</th>
              <th style={thS()}>Estado</th>
              <th style={thS()}>Responsable</th>
            </tr></thead>
            <tbody>
              {movs.length === 0 ? (
                <tr><td colSpan={9} style={{padding:'28px 14px', textAlign:'center', color:BV.slate, fontSize:12.5}}>
                  El WMS no registró movimientos en esta bodega para la fecha seleccionada.
                </td></tr>
              ) : movs
                  .filter(m => (movFiltro==='TODOS' || m.flujo===movFiltro) && (!soloPend || m.pendiente))
                  .map((m, i) => {
                    const f = FLUJOS.find(x=>x.k===m.flujo) || {l:m.flujo, c:BV.slate}
                    const monto = Number(m.monto_clp) || 0
                    return (
                      <tr key={m.flujo+m.documento+i}
                        style={{borderBottom:`1px solid ${BV.lineSoft}`,
                          boxShadow: m.pendiente ? `inset 3px 0 0 ${BV.ambar}` : 'none'}}>
                        <td style={{...tdS, color:BV.slate, fontVariantNumeric:'tabular-nums', whiteSpace:'nowrap'}}>
                          {m.hora ? String(m.hora).slice(0,5) : '—'}</td>
                        <td style={{...tdS, whiteSpace:'nowrap'}}>
                          <span style={{display:'inline-flex', alignItems:'center', gap:6, fontSize:10.5,
                            fontWeight:700, letterSpacing:0.3, color:f.c}}>
                            <span style={{width:7, height:7, borderRadius:'50%', background:f.c, flexShrink:0}}/>
                            {f.l}
                          </span>
                        </td>
                        <td style={{...tdS, whiteSpace:'nowrap'}}>{m.tipo}</td>
                        <td style={{...tdS, fontWeight:700, whiteSpace:'nowrap', maxWidth:150,
                          overflow:'hidden', textOverflow:'ellipsis'}} title={m.documento}>{m.documento}</td>
                        <td style={{...tdS, maxWidth:230, overflow:'hidden', textOverflow:'ellipsis',
                          whiteSpace:'nowrap'}} title={m.nota || m.referencia}>{m.referencia}</td>
                        <td style={{...tdS, textAlign:'right', fontVariantNumeric:'tabular-nums'}}>
                          {m.unidades !== null && m.unidades !== undefined
                            ? new Intl.NumberFormat('es-CL').format(Math.round(m.unidades)) : '—'}</td>
                        <td style={{...tdS, textAlign:'right', fontVariantNumeric:'tabular-nums',
                          color: m.flujo==='BAJA' && monto>0 ? BV.rojo
                               : m.flujo==='CONTEO' && monto<0 ? BV.rojo
                               : m.flujo==='CONTEO' && monto>0 ? BV.ambar : BV.ink}}>
                          {monto ? new Intl.NumberFormat('es-CL',
                            {style:'currency', currency:'CLP', maximumFractionDigits:0}).format(monto) : '—'}</td>
                        <td style={{...tdS, whiteSpace:'nowrap'}}>
                          <span style={{fontSize:10.5, fontWeight:600,
                            color: m.pendiente ? BV.ambar : BV.slate}}>
                            {m.estado || '—'}{m.pendiente ? ' · abierto' : ''}
                          </span>
                        </td>
                        <td style={{...tdS, color:BV.slate, fontSize:11.5, maxWidth:150,
          ...
          {movs.length > 0 && (
            <div style={{padding:'8px 12px', fontSize:11, color:BV.slate, borderTop:`1px solid ${BV.lineSoft}`}}>
              Las filas con banda ámbar son documentos que no llegaron a su estado final. Este registro se
              alimenta solo del WMS: no se edita desde acá.
            </div>
          )}
        </div>
      </>)}
...
// ═══════════════════════ VISTA HISTÓRICO ═══════════════════════════════════
function VistaHist({cu, bodega, flash, esGlobal, scope = [], onAbrir}) {
  const [loading, setLoading] = useState(true)
  ...
  async function cargar() {
    ...
    if (todas && esGlobal)        q = q                                  // consolidado real
    else if (todas && !esGlobal)  q = q.in('sucursal_codigo', scope)     // "todas" = su familia
    else                          q = q.eq('sucursal_codigo', bodega)
    const { data } = await q
    setFilas(data || [])
    setLoading(false)
  }
    ...
    <>
      <div style={{display:'flex', gap:8, alignItems:'center', marginBottom:10, flexWrap:'wrap'}}>
        {esGlobal ? (
          ...
          <button onClick={()=>setTodas(v=>!v)} style={bvBtn(todas?'solid':'outline')}>
            {todas ? 'INCLUYE BODEGA AUXILIAR' : 'INCLUIR BODEGA AUXILIAR'}
          </button>
        ) : null}
        {pendientes > 0 && (
...
// ═══════════════════════ VISTA CUMPLIMIENTO ════════════════════════════════
function VistaCumpl({bodegas, esGlobal, scope = []}) {
  const [loading, setLoading] = useState(true)
  ...
  async function cargar() {
    ...
    try {
      // Todas las consultas se acotan al alcance del usuario
      const fRes  = supabase.from('v_log_bitacora_resumen').select('*').gte('fecha', d0).limit(2000)
      const fBits = supabase.from('log_bitacora').select('sucursal_codigo, fecha').eq('fecha', hoyISO())
      // Los motivos se leen a través de la cabecera para poder filtrar por bodega
      const fLin  = supabase.from('log_bitacora_lineas')
        .select('motivo, actividad_codigo, log_bitacora!inner(sucursal_codigo, fecha)')
        .not('motivo','is',null).gte('log_bitacora.fecha', d0).limit(5000)
      const [res, lin, bits] = await Promise.all([
        esGlobal ? fRes  : fRes.in('sucursal_codigo', scope),
        esGlobal ? fLin  : fLin.in('log_bitacora.sucursal_codigo', scope),
        esGlobal ? fBits : fBits.in('sucursal_codigo', scope),
      ])
      const porSuc = {}
      ;(res.data || []).forEach(r => {
      ...
      let qp = supabase.from('v_log_bit_cat_estado').select('*').eq('pendiente', true)
        .order('dias_pendiente', {ascending:false}).limit(40)
      if (!esGlobal) qp = qp.in('sucursal_codigo', scope)
      const { data:pn } = await qp
      setPend(pn || [])
    } catch (e) { /* vista vacía */ }
    setLoading(false)
  }
  ...
  return (
    <>
      ...
      <SemanaGrid bodegas={bodegas} esGlobal={esGlobal} scope={scope}/>
...
function VistaRiesgo({bodegas, esGlobal, scope = [], bodega}) {
  const [loading, setLoading] = useState(true)
  const [filas, setFilas]     = useState([])
  ...
  useEffect(() => { (async () => {
    ...
    else if (!esGlobal) q = q.in('sucursal_codigo', scope)   // nunca fuera del alcance
    const { data } = await q
    setFilas(data || [])
    setLoading(false)
  })() }, [soloMia, bodega, esGlobal, scope])
  ...
  return (
    <>
      ...
      {(esGlobal || scope.length > 1) && (
        <div style={{marginBottom:10}}>
          <button onClick={()=>setSoloMia(v=>!v)} style={bvBtn(soloMia?'outline':'solid')}>
            {soloMia
              ? (esGlobal ? 'VER TODAS LAS BODEGAS' : 'INCLUIR BODEGA AUXILIAR')
              : (esGlobal ? `VIENDO ${bodegas.length} BODEGAS` : 'VIENDO MI FAMILIA')}
          </button>
        </div>
      )}
