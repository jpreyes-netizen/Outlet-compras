...
//   DÍA          captura del checklist + dotación del turno (coordinador/jefe)
//   HISTÓRICO    worklist de bitácoras por bodega, con validación (jefe/director)
//   CUMPLIMIENTO comparado entre las 4 bodegas + motivos de incumplimiento
//
...
function VistaDia({cu, bodega, fecha, flash, esGlobal}) {
  ...
  async function cargar() {
    setLoading(true)
    try {
      const [chk, ops, mot, cab] = await Promise.all([
      ...
      if (cab.data) {
        const [lin, tur] = await Promise.all([
    ...
    <>
      ...
      <div style={{display:'flex', gap:20, borderBottom:`1px solid ${BV.line}`, marginBottom:12}}>
        {[{k:'checklist', l:`ACTIVIDADES (${checklist.length})`},
          {k:'turnos',    l:`DOTACIÓN Y TURNOS (${operarios.length})`}].map(t=>(
              ...
              {checklist.map((c, i) => {
                ...
                return (
                  <tr key={c.actividad_codigo} style={{borderBottom:`1px solid ${BV.lineSoft}`,
                    boxShadow: pideMotivo && !l.motivo ? `inset 3px 0 0 ${BV.rojo}` : 'none'}}>
                    <td style={{...tdS, color:BV.slate, fontVariantNumeric:'tabular-nums'}}>{i+1}</td>
                    <td style={{...tdS, fontWeight:700, minWidth:170}}>{c.actividad}</td>
                    <td style={tdS}>
