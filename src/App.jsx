...
/* ═══ MAIN APP ═══ */
const TABS=[{k:"monitor",l:"Monitor",ic:"📊"},{k:"repo",l:"Reposición",ic:"📦"},{k:"nueva",l:"Nueva OC",ic:"➕"},{k:"ordenes",l:"Órdenes",ic:"📋"},{k:"config",l:"Config",ic:"⚙️"}]
...
export default function App(){
  ...
  return<div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif",maxWidth:900,margin:"0 auto",padding:"0 16px 100px",background:"#F2F2F7",minHeight:"100vh"}}>
    <style>{`@keyframes slideUp{from{transform:translateY(100%)}to{transform:translateY(0)}}*{box-sizing:border-box;margin:0;padding:0}body{background:#F2F2F7}input:focus,select:focus,textarea:focus{border-color:#007AFF!important;box-shadow:0 0 0 3px rgba(0,122,255,0.1)}::selection{background:#007AFF;color:#fff}`}</style>

    {/* HEADER */}
    <div style={{position:"sticky",top:0,zIndex:50,background:"rgba(242,242,247,0.85)",backdropFilter:"blur(20px)",padding:"12px 0 8px",marginBottom:8}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{fontSize:22,fontWeight:800,color:"#1C1C1E",letterSpacing:"-0.03em"}}>Outlet de Puertas</div><div style={{fontSize:11,color:rd.c,fontWeight:600}}>{rd.l} — {cu.nombre}</div></div>
    ...
    {tab==="ordenes"&&<OCListView ocs={ocs} firmas={firmas} pagos={pagos} updOC={updOC} addFirma={addFirma} setDet={setDet} cu={cu} h={h}/>}
    ...
    {/* BOTTOM TAB BAR */}
    <div style={{position:"fixed",bottom:0,left:0,right:0,background:"rgba(255,255,255,0.92)",backdropFilter:"blur(20px)",borderTop:"0.5px solid rgba(0,0,0,0.1)",display:"flex",justifyContent:"center",padding:"6px 0 env(safe-area-inset-bottom,8px)",zIndex:50}}>
      <div style={{display:"flex",gap:0,maxWidth:500,width:"100%"}}>
        {TABS.filter(t=>{if(t.k==="config")return h("config")||cu.rol==="admin";if(t.k==="nueva")return h("crear_oc");return true}).map(t=><button key={t.k} onClick={()=>setTab(t.k)} style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",gap:2,padding:"6px 4px",background:"none",border:"none",cursor:"pointer"}}>
          <span style={{fontSize:20,opacity:tab===t.k?1:0.4}}>{t.ic}</span>
          <span style={{fontSize:9,fontWeight:600,color:tab===t.k?"#007AFF":"#8E8E93"}}>{t.l}{t.k==="ordenes"&&pend>0?` (${pend})`:""}</span>
        </button>)}
      </div>
    </div>
  </div>
...
function RepoView({prods,cart,setCart,go,config,params,paramsABCD}){
  const[q,setQ]=useState("");const[fc,setFc]=useState("T");const[ft,setFt]=useState("T");const[sortBy,setSortBy]=useState("costo_reposicion")
  const tipos=[...new Set(prods.map(p=>p.tipo_producto).filter(Boolean))].sort()
  const fil=prods.filter(i=>(fc==="T"||i.clasif_abcd===fc)&&(ft==="T"||i.tipo_producto===ft)&&(!q||i.producto?.toLowerCase().includes(q.toLowerCase())||i.sku?.toLowerCase().includes(q.toLowerCase()))).sort((a,b)=>{if(sortBy==="dias_cobertura")return(a.dias_cobertura||999)-(b.dias_cobertura||999);return(b[sortBy]||0)-(a[sortBy]||0)})
  ...
  const resumen={total:prods.length,repos:prods.filter(p=>p.estado==="Reposición").length,a:prods.filter(p=>p.clasif_abcd==="A").length,b:prods.filter(p=>p.clasif_abcd==="B").length,c:prods.filter(p=>p.clasif_abcd==="C").length,d:prods.filter(p=>p.clasif_abcd==="D").length}

  const exportCSV=()=>{
    const h=["Tipo","Producto","SKU","Clasif","Stock Actual","Stock Tránsito","Cobertura (días)","Vta Prom Comp","Pto Reorden","Reponer","Costo Unit","Costo Reposición","Estado"]
    const rows=fil.map(i=>[i.tipo_producto,`"${i.producto}"`,i.sku,i.clasif_abcd,i.stock_actual,i.stock_transito||0,Math.round(i.dias_cobertura||0),Math.round(i.vta_prom_compensada||0),Math.round(i.punto_reorden||0),i.reposicion_necesaria,i.costo_unitario,i.costo_reposicion,i.estado])
    const csv="\uFEFF"+[h,...rows].map(r=>r.join(";")).join("\n")
    const blob=new Blob([csv],{type:"text/csv;charset=utf-8"});const url=URL.createObjectURL(blob);const a=document.createElement("a");a.href=url;a.download=`reposicion_${hoy()}.csv`;a.click()
  }

  return<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
      <div><div style={{fontSize:22,fontWeight:800,color:"#1C1C1E",letterSpacing:"-0.02em"}}>Análisis de reposición</div><div style={{fontSize:13,color:"#8E8E93"}}>{resumen.total} SKUs · {resumen.repos} requieren reposición · Umbral: {config.umbral_quiebre_pct||30}%</div></div>
    ...
    <div style={{display:"flex",gap:4,marginBottom:8,overflowX:"auto"}}>{[["T","Todos",resumen.total],["A","Clase A",resumen.a],["B","Clase B",resumen.b],["C","Clase C",resumen.c],["D","Clase D",resumen.d]].map(([k,l,n])=><button key={k} onClick={()=>setFc(k)} style={{padding:"6px 12px",borderRadius:8,fontSize:11,fontWeight:600,border:"none",cursor:"pointer",whiteSpace:"nowrap",background:fc===k?(k==="T"?"#007AFF":CL[k]?.bg||"#007AFF"):"#F2F2F7",color:fc===k?(k==="T"?"#fff":CL[k]?.c||"#fff"):"#8E8E93"}}>{l} ({n})</button>)}</div>

    {cL>0&&<Cd ac="#007AFF" s={{marginBottom:10,background:"#007AFF08"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{fontSize:13,fontWeight:600,color:"#007AFF"}}>{cL} SKUs seleccionados · {fmt(tC)}</div></div>
        <div style={{display:"flex",gap:4}}><Bt sm v="pri" onClick={go} ic="➕">Crear OC</Bt><Bt sm v="gry" onClick={()=>setCart({})}>Vaciar</Bt></div>
      </div>
    </Cd>}

    <div style={{display:"flex",gap:6,marginBottom:10}}>
      <input placeholder="Buscar producto o SKU..." value={q} onChange={e=>setQ(e.target.value)} style={{...css.input,flex:1}}/>
      <select value={ft} onChange={e=>setFt(e.target.value)} style={{...css.select,width:140}}><option value="T">Todos los tipos</option>{tipos.map(t=><option key={t} value={t}>{t}</option>)}</select>
      <select value={sortBy} onChange={e=>setSortBy(e.target.value)} style={{...css.select,width:130}}><option value="costo_reposicion">Mayor inversión</option><option value="dias_cobertura">Menor cobertura</option><option value="reposicion_necesaria">Mayor cantidad</option></select>
    </div>
    ...
    <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:950}}>
      <thead><tr style={{background:"#F2F2F7"}}>{["","Tipo / Producto / SKU","Clasif.","Stock","Tránsito","Cobertura","Vta Comp.","Pto Reorden","Reponer","Costo U.","Inversión"].map((h,i)=><th key={i} style={{padding:"8px 6px",textAlign:i>1?"right":"left",fontSize:10,fontWeight:700,color:"#8E8E93",borderBottom:"2px solid #E5E5EA",textTransform:"uppercase",letterSpacing:"0.03em",whiteSpace:"nowrap"}}>{h}</th>)}</tr></thead>
      <tbody>{fil.map((it,idx)=>{const inC=!!cart[it.sku];const crit=(it.dias_cobertura||999)<15&&it.clasif_abcd==="A";const isR=it.estado==="Reposición"
        return<tr key={it.sku} onClick={()=>tog(it)} style={{background:inC?"#007AFF08":idx%2?"#fafafa":"#fff",cursor:"pointer",borderBottom:"1px solid #F2F2F7"}}>
          <td style={{padding:"8px 6px",width:30}}><input type="checkbox" checked={inC} onChange={()=>{}}/></td>
          <td style={{padding:"8px 6px"}}><div style={{display:"flex",alignItems:"center",gap:4,marginBottom:1}}><Bd c={CL[it.clasif_abcd]?.c} bg={CL[it.clasif_abcd]?.bg}>{it.clasif_abcd}</Bd>{crit&&<Bd c="#FF3B30" bg="#FF3B3015">URGENTE</Bd>}{isR&&<Bd c="#FF9500" bg="#FF950015">Reponer</Bd>}</div><div style={{fontSize:13,fontWeight:600,color:"#1C1C1E",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap",maxWidth:260}}>{it.producto}</div><div style={{fontSize:10,color:"#AEAEB2"}}>{it.tipo_producto} · {it.sku}</div></td>
          <td style={{padding:"8px 4px",textAlign:"center"}}><Bd c={CL[it.clasif_abcd]?.c} bg={CL[it.clasif_abcd]?.bg}>{it.clasif_abcd}</Bd></td>
          <td style={{padding:"8px 4px",textAlign:"right",fontWeight:600,color:it.stock_actual<=0?"#FF3B30":"#1C1C1E"}}>{fN(it.stock_actual)}</td>
          <td style={{padding:"8px 4px",textAlign:"right",color:it.stock_transito?"#007AFF":"#D1D1D6"}}>{it.stock_transito?fN(it.stock_transito):"—"}</td>
          <td style={{padding:"8px 4px",textAlign:"right"}}><span style={{fontWeight:600,color:crit?"#FF3B30":(it.dias_cobertura||0)<30?"#FF9500":"#34C759"}}>{it.dias_cobertura?Math.round(it.dias_cobertura)+"d":"—"}</span></td>
          <td style={{padding:"8px 4px",textAlign:"right",color:"#8E8E93"}}>{fN(it.vta_prom_compensada)}</td>
          <td style={{padding:"8px 4px",textAlign:"right",color:"#8E8E93"}}>{fN(it.punto_reorden)}</td>
          <td style={{padding:"8px 4px",textAlign:"right"}}>{inC?<input type="number" value={cart[it.sku]?.cp||0} onChange={e=>{e.stopPropagation();setQty(it.sku,e.target.value)}} onClick={e=>e.stopPropagation()} style={{width:65,textAlign:"center",fontSize:12,fontWeight:700,padding:"3px",border:"2px solid #007AFF",borderRadius:8,color:"#007AFF"}}/>:<strong style={{color:isR?"#FF3B30":"#1C1C1E"}}>{fN(it.reposicion_necesaria)}</strong>}</td>
          <td style={{padding:"8px 4px",textAlign:"right",color:"#8E8E93"}}>{it.costo_unitario?fmt(it.costo_unitario):"—"}</td>
          <td style={{padding:"8px 6px",textAlign:"right",fontWeight:600}}>{it.costo_reposicion?fmt(it.costo_reposicion):"—"}</td>
        </tr>})}</tbody>
    </table></div>
    <div style={{padding:"10px 12px",background:"#F2F2F7",borderRadius:"0 0 10px 10px",fontSize:11,color:"#8E8E93",display:"flex",justifyContent:"space-between"}}><span>{fil.length} productos mostrados</span><span>Inversión reposición: <strong style={{color:"#1C1C1E"}}>{fmt(fil.filter(i=>i.estado==="Reposición").reduce((s,i)=>s+(i.costo_reposicion||0),0))}</strong></span></div>
  </div>
}
...
/* ═══ OC LIST ═══ */
function OCListView({ocs,firmas,pagos,updOC,addFirma,setDet,cu,h}){
  ...
  const firma=async(oc,acc,nE,nF)=>{await addFirma(oc.id,acc);await updOC(oc.id,{estado:nE,fase_actual:nF??oc.fase_actual})}

  return<div>
    <div style={{fontSize:22,fontWeight:800,color:"#1C1C1E",letterSpacing:"-0.02em",marginBottom:12}}>Órdenes</div>
    ...
    {fil.map(oc=>{const ocF=firmas.filter(fi=>fi.oc_id===oc.id);const isI=oc.tipo_oc==="Importación"
      return<Cd key={oc.id} s={{marginBottom:8}} onClick={()=>setDet(oc)}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"start",marginBottom:8}}>
          <div><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:3}}><span style={{fontSize:15,fontWeight:700}}>{oc.id}</span><Bd c={isI?"#FF3B30":"#007AFF"} bg={isI?"#FF3B3015":"#007AFF15"}>{isI?"IMP":"NAC"}</Bd></div><Bd c={STS[oc.estado]?.c} bg={STS[oc.estado]?.bg} lg>{STS[oc.estado]?.ic} {oc.estado}</Bd><div style={{fontSize:11,color:"#8E8E93",marginTop:4}}>{oc.proveedor_id} · {oc.fecha_creacion} · {oc.condicion_pago}</div></div>
          <div style={{textAlign:"right"}}><div style={{fontSize:18,fontWeight:800}}>{fmt(oc.total_clp)}</div>{oc.total_usd>0&&<div style={{fontSize:11,color:"#8E8E93"}}>{fU(oc.total_usd)}</div>}</div>
        </div>
...
function OCDetView({oc,firmas,pagos,updOC,addFirma,setPagos,close,cu,h}){
  const[rf,setRf]=useState(hoy());const[rr,setRr]=useState("Jefe Bodega");const[rc,setRc]=useState("100%");const[rd,setRd]=useState("");const isI=oc.tipo_oc==="Importación"
  const[items,setItems]=useState([])
  useEffect(()=>{supabase.from('oc_items').select('*').eq('oc_id',oc.id).then(r=>setItems(r.data||[]))},[oc.id])
  ...
  const saveR=async()=>{await supabase.from('recepcion').insert({id:uid(),oc_id:oc.id,fecha_recepcion:rf,responsable_id:cu.id,nombre_responsable:rr,porcentaje_recibido:rc,discrepancias:rd,firma_recepcion:cu.firma_digital});await addFirma(oc.id,"Recepción: "+rc);await updOC(oc.id,{estado:rc==="100%"?"Recibida OK":"Recibida parcial",fase_actual:isI?13:6});close()}
  ...
  return<div>
    ...
    {items.length>0&&<><div style={{fontSize:15,fontWeight:700,marginBottom:6}}>Productos ({items.length})</div>
      {items.map((i,idx)=><div key={idx} style={{display:"flex",justifyContent:"space-between",padding:"8px 0",borderBottom:"1px solid #F2F2F7"}}><div><div style={{fontSize:13,fontWeight:600}}>{i.producto}</div><div style={{fontSize:11,color:"#8E8E93"}}>Sugerido: {fN(i.cantidad_sugerida)}</div></div><div style={{textAlign:"right"}}><div style={{fontSize:14,fontWeight:700,color:"#007AFF"}}>{fN(i.cantidad_pedida)}</div><div style={{fontSize:11,color:"#8E8E93"}}>{fmt((i.cantidad_pedida||0)*(i.costo_unitario||0))}</div></div></div>)}</>}
    ...
    {((isI&&oc.estado==="Transporte")||(!isI&&oc.estado==="Despacho nac."))&&h("recibir")&&<div style={{borderTop:"2px solid #F2F2F7",paddingTop:16,marginTop:16}}>
      <div style={{fontSize:15,fontWeight:700,marginBottom:10}}>📦 Registrar recepción</div>
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:10}}>
        <Fl l="Fecha"><input type="date" value={rf} onChange={e=>setRf(e.target.value)} style={css.input}/></Fl>
        <Fl l="Responsable"><input value={rr} onChange={e=>setRr(e.target.value)} style={css.input}/></Fl>
        <Fl l="Cantidad"><select value={rc} onChange={e=>setRc(e.target.value)} style={css.select}><option>100%</option><option>Parcial 75%</option><option>Parcial 50%</option></select></Fl>
        <Fl l="Discrepancias"><input value={rd} onChange={e=>setRd(e.target.value)} placeholder="Describir..." style={css.input}/></Fl>
      </div>
      ...
      <div style={{marginTop:12}}><Bt v="suc" full onClick={saveR} ic="✍️">Firmar recepción</Bt></div>
    </div>}
...
function ConfigView({config,saveConfig,params,setParams,paramsABCD,setParamsABCD,provs,setProvs,users,setUsers,h,configTab,setConfigTab}){
  const tabs=[{k:"params",l:"Reposición",ic:"📊"},{k:"bsale",l:"BSALE",ic:"🔗"},{k:"provs",l:"Proveedores",ic:"🏢"},{k:"users",l:"Usuarios",ic:"👤"}]
...
function BsaleConfig({config,saveConfig,bsaleToken,setBsaleToken,saveBsale}){
  ...
  const fullSync=async()=>{
    ...
    addLog("Iniciando sincronización directa con BSALE...")
    try{
      // 1. Fetch all products with variants and product_type
      ...
      addLog(`✅ ${allProds.length} productos descargados`,"success")

      // 2. Fetch stock WITH expand=[variant] - THIS IS CRITICAL
      addLog("Descargando stock actual (con variantes)...")
      ...
      // 3. Get params from Supabase
      const{data:pTypes}=await supabase.from('parametros_tipo').select('*')
      const{data:pABCD}=await supabase.from('parametros_abcd').select('*')
      const umbralPct=(Number(config.umbral_quiebre_pct)||30)/100

      addLog("Ejecutando algoritmo de reposición...")

      // 4. Build stock map by variant ID (sum across warehouses)
      const stockMap={}
      for(const s of allStock){
        const vid=s.variant?.id||s.variantId
        if(vid){stockMap[vid]=(stockMap[vid]||0)+(s.quantityAvailable||s.quantity||0)}
      }
      addLog(`📦 ${Object.keys(stockMap).length} variantes con stock mapeadas`,"success")

      // 5. Build product list from all variants
      const prodList=[]
      for(const p of allProds){
        const vars=p.variants?.items||[]
        for(const v of vars){
          ...
          prodList.push({sku,producto:v.description||p.name||sku,codigo_barras:v.barCode||"",tipo_producto:typeName,stock_actual:stockActual,costo_unitario:costoUnit,dias_fabricacion:tp.dias_fabricacion,periodo_cubrir:tp.periodo_cobertura,_val:costoUnit*stockActual})
        }
      }
      addLog(`🔄 ${prodList.length} variantes procesadas`,"success")

      // 6. ABCD Classification by value
      const totalVal=prodList.reduce((s,p)=>s+p._val,0)
      prodList.sort((a,b)=>b._val-a._val)
      let acum=0;const abcdC={A:0,B:0,C:0,D:0}
      for(const p of prodList){
        acum+=totalVal>0?p._val/totalVal:0
        p.clasif_abcd=acum<=0.8?"A":acum<=0.95?"B":acum<=0.99?"C":"D"
        abcdC[p.clasif_abcd]++
        const abcd=(pABCD||[]).find(a=>a.clasificacion===p.clasif_abcd)||{dias_emergencia:5}
        p.dias_emergencia=abcd.dias_emergencia
        // Calculate reorder point and replenishment
        p.vta_prom_diaria=0;p.vta_prom_compensada=0
        p.punto_reorden=0;p.dias_cobertura=p.stock_actual>0?999:0
        p.reposicion_necesaria=0;p.costo_reposicion=0
        p.estado=p.stock_actual>0?"Stock suficiente":"Sin ventas"
        delete p._val
      }
      ...
      addLog(`💾 ${saved} productos guardados en Supabase`,"success")
      const repos=prodList.filter(p=>p.estado==="Reposición").length
      const invR=prodList.filter(p=>p.estado==="Reposición").reduce((s,p)=>s+(p.costo_reposicion||0),0)
      setResult({ok:true,productos_procesados:prodList.length,clase_A:abcdC.A,clase_B:abcdC.B,clase_C:abcdC.C,clase_D:abcdC.D,requieren_reposicion:repos,inversion_reposicion:invR})
      await saveConfig("bsale_activo","true")
  ...
  return<div>
    <Cd><div style={{fontSize:15,fontWeight:700,marginBottom:10}}>🔗 Conexión BSALE</div>
      ...
      <div style={{display:"flex",gap:8,marginBottom:14}}>
        ...
        <Bt v="suc" onClick={fullSync} dis={!bsaleToken||syncing} ic="🔄">{syncing?"Sincronizando...":"Sincronizar ahora"}</Bt>
      </div>
