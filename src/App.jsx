...
export default function App(){
  ...
  return<div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif",margin:0,padding:"0 20px 100px",background:"#F2F2F7",minHeight:"100vh",fontSize:14}}>
    ...
    <div style={{position:"sticky",top:0,zIndex:50,background:"rgba(242,242,247,0.9)",backdropFilter:"blur(20px)",padding:"14px 0 10px",marginBottom:10,borderBottom:"1px solid rgba(0,0,0,0.06)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <a href="/Manual_ERP_Compras_v1.pdf" target="_blank" rel="noopener" style={{width:32,height:32,borderRadius:10,background:"#007AFF15",border:"none",cursor:"pointer",fontSize:13,color:"#007AFF",display:"flex",alignItems:"center",justifyContent:"center",textDecoration:"none"}} title="Manual de Usuario">📖</a>
    ...
    <Sheet show={!!det} onClose={()=>setDet(null)} title={det?.id||""}>{det&&<OCDetView oc={det} firmas={firmas.filter(f=>f.oc_id===det.id)} pagos={pagos.filter(p=>p.oc_id===det.id)} provs={provs} updOC={updOC} addFirma={addFirma} setPagos={setPagos} close={()=>{setDet(null);loadAll()}} cu={cu} h={h}/>}</Sheet>
...
function SolView({cart,setCart,provs,users,cu,setOcs,addFirma,goOC,ocs}){
  ...
  const submit=async()=>{
    ...
    const oc={id,fecha_creacion:hoy(),solicitante_id:cu.id,proveedor_id:prov,tipo_oc:tipo,estado:"Pend. Dir. Negocios",fase_actual:1,total_clp:total,total_usd:isI?Math.round(total/950):0,condicion_pago:condPago,pct_fab:isI?(cuotas.find(c=>c.concepto.toLowerCase().includes("fabri"))?.pct||0):0,pct_embarque:isI?(cuotas.find(c=>c.concepto.toLowerCase().includes("embar"))?.pct||0):0,pct_puerto:isI?(cuotas.find(c=>c.concepto.toLowerCase().includes("puerto")||c.concepto.toLowerCase().includes("saldo"))?.pct||0):0,fecha_estimada:fEst||null,estado_pago:"Pago pend.",notas}
    ...
    if(!error){
      ...
      await supabase.from('oc_items').insert(items)
      ...
      await addFirma(id,"Solicitud creada");setOcs(p=>[oc,...p]);setCart({});setManualItems([]);setDone(oc)
    };setSaving(false)}
...
function OCDetView({oc,firmas,pagos,provs=[],updOC,addFirma,setPagos,close,cu,h}){
  ...
  const saveItemEdit=async(it)=>{
    ...
    await updOC(oc.id,{total_clp:nuevoTotal,total_usd:isI?Math.round(nuevoTotal/950):0})
...
function CosteoImpView({config,saveConfig,ocs}){
  ...
  // Margen: si modo=margen, calcular precio sugerido para ese margen. Si modo=costo, calcular margen actual.
  ...
  const gananciaTotal=gananciaUnit*p.cantUds
  ...
  const productCosts=ocItems.map(item=>{
    ...
    const pctDelFOB=fobCLP>0?fobProducto/fobCLP:0
    ...
    return{...item,qty,costoUnitOrigen,fobProducto,pctDelFOB,prFlete,prSeguro,prArancel,prIVA,prPuerto,prAgente,prTransporte,prFinanciero,totalInternacion,costoTotalProducto,costoUnitFinal,incremento,precioVentaSug}
  })
    ...
    <div style={{background:"linear-gradient(135deg,#1a1a2e 0%,#16213e 100%)",borderRadius:16,padding:"20px 24px",marginBottom:12,color:"#fff",position:"relative",overflow:"hidden"}}>
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:14,flexWrap:"wrap",gap:14,position:"relative"}}>
        ...
        <div style={{display:"flex",gap:8,alignItems:"center"}}>
          ...
          {productCosts.length>0&&<button onClick={exportCosteo} style={{background:"#34C759",color:"#fff",border:"none",padding:"10px 16px",borderRadius:10,fontSize:12,fontWeight:600,cursor:"pointer"}}>📥 Exportar</button>}
        ...
        {tab==="productos"&&productCosts.length>0&&<div style={{background:"#fff",borderRadius:14,padding:"16px 20px",boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
            <div>
              ...
              <div style={{fontSize:11,color:"#8E8E93",marginTop:1}}>Prorrateo de internación proporcional al FOB de cada producto</div>
            </div>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
            <thead><tr style={{background:"#F8F8FA"}}>
              ...
              <th style={{padding:"8px 6px",textAlign:"right",fontSize:10,fontWeight:700,color:"#1C1C1E",borderBottom:"2px solid #E5E5EA"}}>COSTO TOTAL</th>
            </tr></thead>
            <tbody>{productCosts.map((item,i)=><tr key={i} style={{borderBottom:"1px solid #F2F2F7"}}>
              <td style={{padding:"8px 6px"}}><div style={{fontWeight:600,fontSize:12,maxWidth:220,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{item.producto}</div><div style={{fontSize:10,color:"#AEAEB2"}}>{item.sku}</div></td>
              ...
              <td style={{padding:"8px 4px",textAlign:"right"}}><span style={{padding:"2px 7px",borderRadius:4,fontSize:11,fontWeight:700,color:item.incremento>50?"#FF3B30":item.incremento>30?"#FF9500":"#34C759",background:item.incremento>50?"#FF3B3012":item.incremento>30?"#FF950012":"#34C75912"}}>+{item.incremento}%</span></td>
              <td style={{padding:"8px 4px",textAlign:"right",color:"#34C759",fontWeight:700}}>{fmt(item.precioVentaSug)}</td>
              <td style={{padding:"8px 6px",textAlign:"right",fontWeight:700}}>{fmt(item.costoTotalProducto)}</td>
            </tr>)}</tbody>
            <tfoot><tr style={{borderTop:"2px solid #1C1C1E",background:"#F8F8FA"}}>
              <td style={{padding:"10px 6px",fontWeight:700}}>TOTALES</td>
              <td style={{padding:"10px 4px",textAlign:"right",fontWeight:700}}>{fN(productCosts.reduce((s,i)=>s+i.qty,0))}</td>
              <td></td>
              <td style={{padding:"10px 4px",textAlign:"right",fontWeight:700,color:"#AF52DE"}}>{fmt(productCosts.reduce((s,i)=>s+i.totalInternacion,0))}</td>
              <td></td><td></td><td></td>
              <td style={{padding:"10px 6px",textAlign:"right",fontSize:16,fontWeight:800}}>{fmt(costoTotalCLP)}</td>
            </tr></tfoot>
          </table></div>
        </div>}
