...
function FinanzasView({ocs,provs,pagos,setPagos}){
  const enTransito=ocs.filter(o=>!["Cerrada","Rechazada","Pend. Dir. Negocios","Pend. Dir. Finanzas","Recibida OK"].includes(o.estado))
  ...
  const hitos=useMemo(()=>{
    const out=[]
    enTransito.forEach(oc=>{
      const pv=provs.find(p=>p.id===oc.proveedor_id)
      const pagosOC=pagos.filter(p=>p.oc_id===oc.id&&p.estado==="Pagado")
      ...
      if(oc.tipo_oc==="Importación"){
        ...
        if(pctF>0)out.push({oc_id:oc.id,oc,prov:pv,hito:"fabricacion",label:"Pago fabricación",pct:pctF,monto:totalRef*pctF/100,moneda,fecha_proy:oc.fecha_creacion,pagado:!!pagosOC.find(p=>p.concepto?.toLowerCase().includes("fabri")),fecha_real:pagosOC.find(p=>p.concepto?.toLowerCase().includes("fabri"))?.fecha_pago})
        if(pctE>0)out.push({oc_id:oc.id,oc,prov:pv,hito:"embarque",label:"Pago embarque",pct:pctE,monto:totalRef*pctE/100,moneda,fecha_proy:oc.etd||addDias(oc.fecha_creacion,45),pagado:!!pagosOC.find(p=>p.concepto?.toLowerCase().includes("embar")),fecha_real:pagosOC.find(p=>p.concepto?.toLowerCase().includes("embar"))?.fecha_pago})
        if(pctP>0)out.push({oc_id:oc.id,oc,prov:pv,hito:"puerto",label:"Pago puerto",pct:pctP,monto:totalRef*pctP/100,moneda,fecha_proy:oc.eta||addDias(oc.fecha_creacion,75),pagado:!!pagosOC.find(p=>p.concepto?.toLowerCase().includes("puerto")),fecha_real:pagosOC.find(p=>p.concepto?.toLowerCase().includes("puerto"))?.fecha_pago})
      }else{
        ...
        out.push({oc_id:oc.id,oc,prov:pv,hito:dias===0?"contado":`credito_${dias}`,label:dias===0?"Pago contado":`Crédito ${dias} días`,pct:100,monto:totalRef,moneda,fecha_proy:dias===0?oc.fecha_creacion:addDias(fechaBase,dias),pagado:pagosOC.length>0,fecha_real:pagosOC[0]?.fecha_pago})
      }
    })
    return out
  },[enTransito,pagos,provs])
  ...
  const marcarPagado=async(h)=>{
    const fecha=prompt(`Fecha de pago real (YYYY-MM-DD):`,hoy())
    if(!fecha)return
    const monto=prompt(`Monto pagado en ${h.moneda}:`,h.monto.toFixed(0))
    if(!monto)return
    const p={id:uid(),oc_id:h.oc_id,concepto:h.label,monto:parseFloat(monto),moneda:h.moneda,fecha_pago:fecha,estado:"Pagado",etapa:h.hito}
  ...
  return<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14,flexWrap:"wrap",gap:8}}>
      <div>
        ...
        <div style={{fontSize:13,color:"#8E8E93"}}>{enTransito.length} OC en tránsito · {pendientes.length} pagos pendientes · {fmt(totalPend)} comprometido</div>
      </div>
      ...
      {vencidos.length>0&&<Cd ac="#FF3B30" s={{marginTop:12}}>
        ...
        {vencidos.map((h,i)=><div key={i} style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"8px 0",borderBottom:i<vencidos.length-1?"1px solid #F2F2F7":"none"}}>
          ...
          <div style={{display:"flex",gap:8,alignItems:"center"}}>
            ...
            <Bt v="suc" sm onClick={()=>marcarPagado(h)}>✓ Pagar</Bt>
          </div>
        </div>)}
      </Cd>}
    </div>
    ...
    <div>
      {vencidos.length>0&&<Cd ac="#FF3B30" s={{background:"#FF3B3008"}}>
        ...
        {vencidos.map((h,i)=><HitoRow key={i} h={h} onPagar={()=>marcarPagado(h)}/>)}
      </Cd>}
      ...
      {semanas.slice(0,12).map((sem,i)=>{
        ...
        return<Cd key={i} s={{marginBottom:8}}>
          ...
          {sem.hitos.map((h,j)=><HitoRow key={j} h={h} onPagar={()=>marcarPagado(h)}/>)}
        </Cd>
      })}
