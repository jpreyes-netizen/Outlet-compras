...
La OC original ${oc.id} quedó en estado "Dividida".`)
  ...
  const saveItemEdit=async(it)=>{
    ...
    await updOC(oc.id,isI?{total_clp:Math.round(nuevoNeto*TC_USD),total_usd:Math.round(nuevoNeto),total_neto:0,iva_monto:0}:{total_clp:Math.round(nuevoNeto*1.19),total_usd:0,total_neto:Math.round(nuevoNeto),iva_monto:Math.round(nuevoNeto*0.19)})
  ...
  return<div>
    ...
    {vista==="tabla"&&<div style={{background:"#fff",borderRadius:12,border:"1px solid #E5E5EA",overflow:"hidden"}}>
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
          <thead><tr style={{background:"#F2F2F7"}}>
            {["OC","Proveedor","Hito","%","Monto","Pagado","Pendiente","Moneda","Fecha Proy.","Días","Estado","Comprobante","Acción"].map((h,i)=><th key={i} style={{padding:"8px 10px",textAlign:i>3?"right":"left",fontSize:10,fontWeight:700,color:"#8E8E93",borderBottom:"2px solid #E5E5EA",whiteSpace:"nowrap"}}>{h}</th>)}
          </tr></thead>
