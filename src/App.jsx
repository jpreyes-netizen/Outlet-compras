...
La OC original ${oc.id} quedó en estado "Dividida".`)
  ...
  const guardarRecepcion=async()=>{
    ...
    if(tipoEsRec){
      if(nuevoPctRecibido>=100)await updOC(oc.id,{estado:"Recibida OK",fase_actual:esImp?13:6,fecha_real_recepcion:recForm.fecha,recepcion_cuadrada:true,recepcion_forzada:false})
      else if(nuevoPctRecibido>0)await updOC(oc.id,{estado:"Recibida parcial",fase_actual:esImp?13:6,fecha_real_recepcion:oc.fecha_real_recepcion||recForm.fecha,recepcion_cuadrada:false})
    }
      ...
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:12,flexWrap:"wrap",gap:10}}>
        ...
        {oc.estado!=="Cerrada"&&<div style={{display:"flex",gap:6}}>
          ...
          {!["Recibida OK","Cerrada"].includes(oc.estado)&&h("recibir")&&<Bt v="suc" sm ic="✔" onClick={async()=>{
            ...
            if(!window.confirm(msg))return
            try{
              await updOC(oc.id,{estado:"Recibida OK",fase_actual:esImp?13:6,fecha_real_recepcion:oc.fecha_real_recepcion||hoy(),recepcion_cuadrada:cuadrada,recepcion_forzada:!cuadrada})
