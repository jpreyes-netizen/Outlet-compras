...
function OCListView({ocs,firmas,pagos,updOC,addFirma,setDet,cu,h,provs,setOcs,notificarFlujo,ROL_POR_ESTADO={}}){
  ...
  useEffect(()=>{
    ...
    ;(async()=>{
      // ⭐ Carga en paralelo + UN SOLO setState al final → evita race condition
      //    (antes cada Map se seteaba por separado y el render pintaba con Maps vacíos)
      const [docsR,facsR,ctsR,rcsR]=await Promise.all([
        supabase.from('documentos_import').select('id,oc_id,nombre_archivo,url_archivo,tipo_documento').limit(20000),
...
function FinanzasView({ocs,provs,pagos,setPagos,config={},cu,h:hp}){
  ...
  useEffect(()=>{(async()=>{
    ...
    setFacturasPorOC(fm)
    const{data:docs}=await supabase.from('documentos_import').select('id,oc_id,nombre_archivo,url_archivo,tipo_documento').limit(20000)
    const dm=new Map()
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:1450}}>
            ...
            <tbody>{sorted.map((r,i)=>{
              ...
              <tr style={{borderBottom:"1px solid #F2F2F7",background:i%2===0?"#fff":"#FAFAFA"}}>
                ...
                <td style={{padding:"6px 10px",fontSize:11,maxWidth:200}}>{r.docs.length>0?r.docs.map((dc,di)=><div key={di} style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:190}}>{dc.url_archivo?<a href={dc.url_archivo.startsWith("data:")?undefined:dc.url_archivo} onClick={dc.url_archivo.startsWith("data:")?(e)=>{e.preventDefault();try{const u=dc.url_archivo;const ci2=u.indexOf(",");const mime=u.slice(5,ci2).split(";")[0]||"application/octet-stream";const bin=atob(u.slice(ci2+1));const by=new Uint8Array(bin.length);for(let bx=0;bx<bin.length;bx++)by[bx]=bin.charCodeAt(bx);window.open(URL.createObjectURL(new Blob([by],{type:mime})),"_blank","noopener")}catch(er){alert("No se pudo abrir: "+er.message)}}:undefined} target="_blank" rel="noreferrer" style={{color:"#007AFF",textDecoration:"none"}} title={dc.nombre_archivo}>📎 {(dc.nombre_archivo||dc.tipo_documento||"doc").slice(0,24)}</a>:<span style={{color:"#8E8E93"}}>📎 {(dc.nombre_archivo||dc.tipo_documento||"doc").slice(0,24)}</span>}</div>):<span style={{color:"#C7C7CC"}}>—</span>}</td>
