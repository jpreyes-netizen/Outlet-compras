...
function TooltipFormula({it,mNames,params=[],paramsABCD=[]}){
  ...
  const sales=[it.venta_mes_1||0,it.venta_mes_2||0,it.venta_mes_3||0,it.venta_mes_4||0]
  ...
  return<div style={{position:"relative",display:"inline-block"}}
    ...
    {show&&<div style={{position:"fixed",zIndex:9999,background:"linear-gradient(135deg,#1a1a2e,#16213e)",color:"#fff",padding:"14px 16px",borderRadius:12,fontSize:11,lineHeight:1.55,width:360,maxWidth:"calc(100vw - 40px)",boxShadow:"0 8px 32px rgba(0,0,0,0.4)",fontWeight:400,textAlign:"left",letterSpacing:0.1,top:tooltipPos.top,left:tooltipPos.left,pointerEvents:"none"}}>
      ...
      <div style={{paddingLeft:8,marginBottom:8,opacity:0.9}}>
        {mNames.map((m,i)=><span key={i}>{m}: <b>{fN(sales[i])}</b>{i<3?" · ":""}</span>)}<br/>
        Total 4 meses: <b>{fN(ventaTotal)}u</b>
      </div>
...
function RepoView({prods,prodsSuc=[],cart,setCart,go,config,params,paramsABCD,sucursales=[]}){
  ...
  const mNames=[abrevMes(config.mes_1_nombre)||"Mes 1",abrevMes(config.mes_2_nombre)||"Mes 2",abrevMes(config.mes_3_nombre)||"Mes 3",abrevMes(config.mes_4_nombre)||"Mes 4"]
  ...
  return<div>
    ...
    <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,tableLayout:"auto"}}>
      <thead style={{position:"sticky",top:0,zIndex:5}}><tr style={{background:"#E5E5EA"}}>
        {[["",""],["Producto","producto"],["Tipo","tipo_producto"],["Clasif.","clasif_abcd"],["Estado","estado"],[mNames[0],"venta_mes_1"],[mNames[1],"venta_mes_2"],[mNames[2],"venta_mes_3"],[mNames[3],"venta_mes_4"],["Vta Total","venta_total"],["Quieb.","meses_quiebre"],["Vta Comp.","vta_prom_compensada"],["Vta/Día","vta_prom_diaria"],["Stock","stock_actual"],["Tráns.","stock_transito"],["Cob.","dias_cobertura"],["Pto Re.","punto_reorden"],["Reponer","reposicion_necesaria"],["Costo U.","costo_unitario"],["Inversión","costo_reposicion"]].map(([label,col],i)=>
...
function CoberturaView({prods=[],ocs=[],config={},params=[],paramsABCD=[]}){
  ...
  const mesesValidos=excluirActual?Math.max(1,totalMeses-1):totalMeses

  // Venta diaria respetando config
  const ventaDia=(p)=>{
...
function BsaleConfig({config,saveConfig,bsaleToken,setBsaleToken,saveBsale,loadAll}){
  ...
  const processExcel=async(file)=>{
    ...
    try{
      ...
      // ⭐ Detección DINÁMICA de columnas de venta mensual: cualquier "Unidades Vendida <Mes> <Año>"
      // se captura en orden de aparición (izq→der = más antiguo→más reciente). No depende de meses fijos.
      const colMeses=[] // {idx, nombre}
      headers.forEach((h,i)=>{
        const hl=h.toLowerCase()
        if((hl.includes("vendida")||hl.includes("vendidas"))&&hl.includes("unidades")){
          // Extraer nombre legible del mes: "Unidades Vendida Mayo 2026" → "Mayo 2026"
          const m=h.replace(/unidades\s+vendidas?\s*/i,"").trim()
          colMeses.push({idx:i,nombre:m||`Mes ${colMeses.length+1}`})
        }
      })
      // Tomar los últimos 4 meses del reporte (los más recientes) y asignarlos a m1..m4
      const ultimos=colMeses.slice(-4)
      colMap.m1=ultimos[0]?.idx
      colMap.m2=ultimos[1]?.idx
      colMap.m3=ultimos[2]?.idx
      colMap.m4=ultimos[3]?.idx
      // Guardar nombres de meses en config para que la tabla de Reposición muestre los encabezados correctos
      if(ultimos.length>0){
        const nombres=["mes_1_nombre","mes_2_nombre","mes_3_nombre","mes_4_nombre"]
        for(let k=0;k<4;k++){if(ultimos[k]?.nombre)await saveConfig(nombres[k],ultimos[k].nombre)}
      }
      if(colMeses.length===0){addLog("⚠ No se detectaron columnas 'Unidades Vendida'. Verificá el formato del reporte BSALE.","error")}
      else if(colMeses.length<4){addLog(`⚠ Solo se detectaron ${colMeses.length} mes(es) de venta: ${colMeses.map(c=>c.nombre).join(", ")}. El cálculo usará los disponibles.`,"warn")}
      else if(colMeses.length>4){addLog(`ℹ El reporte trae ${colMeses.length} meses. Se usan los 4 más recientes: ${ultimos.map(c=>c.nombre).join(", ")}`,"info")}
      addLog(`📅 Meses detectados (m1→m4): ${ultimos.map(c=>c.nombre).join(" · ")}`,"success")
      ...
      for(let i=headerIdx+1;i<rows.length;i++){
        ...
        if(!sucursalMap[key]){
          ...
          sucursalMap[key]={
            ...
            stock_actual:0,venta_mes_1:0,venta_mes_2:0,venta_mes_3:0,venta_mes_4:0,
            costo_stock:0
          }
        }
        const ps=sucursalMap[key]
        ps.stock_actual+=Number(r[colMap.stock])||0
        ps.venta_mes_1+=Number(r[colMap.m1])||0
        ps.venta_mes_2+=Number(r[colMap.m2])||0
        ps.venta_mes_3+=Number(r[colMap.m3])||0
        ps.venta_mes_4+=Number(r[colMap.m4])||0
      ...
      // venta_total por sucursal
      for(const p of sucList){
        const allM=[p.venta_mes_1,p.venta_mes_2,p.venta_mes_3,p.venta_mes_4].slice(0,totalMesesRep)
        const mP=excluirMesActual?allM.slice(0,mesesValidosRep):allM
