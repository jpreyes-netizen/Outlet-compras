        ...
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"start",marginBottom:8}}>
          ...
          <div style={{textAlign:"right"}}><div style={{fontSize:18,fontWeight:800}}>{fmt(oc.total_clp)}</div>{oc.total_usd>0&&<div style={{fontSize:11,color:"#8E8E93"}}>{fU(oc.total_usd)}</div>}
            {isAdmin&&<div style={{display:"flex",gap:4,marginTop:4,justifyContent:"flex-end"}} onClick={e=>e.stopPropagation()}>
              ...
              {isI&&["Proforma OK","Pago fab.","Fabricación","Pago emb.","Naviera","Aduana"].includes(oc.estado)&&h("gest_imp")&&<button onClick={()=>{setDet(oc);setTimeout(()=>document.getElementById("btn-split-"+oc.id)?.click(),300)}} style={{padding:"4px 8px",borderRadius:6,fontSize:10,fontWeight:600,border:"1px solid #5856D6",background:"#5856D612",color:"#5856D6",cursor:"pointer"}}>✂ Dividir</button>}
