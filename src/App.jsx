...
function OCDetView({oc,firmas,pagos,updOC,addFirma,setPagos,close,cu,h}){
  ...
  const saveAllDocs=async()=>{
    ...
    for(const f of pendingFiles){
      ...
      if(error){errors.push(`${f.nombre_archivo}: ${error.message}`)}
    ...
    return<div style={{background:"#F2F2F7",borderRadius:10,padding:12,marginBottom:8}}>
      ...
      {savedDocs.map(d=><div key={d.id} style={{display:"flex",alignItems:"center",gap:8,marginTop:8,padding:10,background:"#fff",borderRadius:8,border:"1px solid #34C75950"}}>
        ...
        <div style={{display:"flex",gap:4,flexShrink:0}}>
          {d.url_archivo?.startsWith("data:")&&<button onClick={()=>{const w=window.open();w.document.write(`<iframe src="${d.url_archivo}" style="width:100%;height:100%;border:none"></iframe>`);w.document.title=d.nombre_archivo}} style={{padding:"5px 10px",borderRadius:6,fontSize:11,fontWeight:600,background:"#007AFF15",color:"#007AFF",border:"none",cursor:"pointer"}}>👁 Ver</button>}
