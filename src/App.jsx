...
function CoberturaView({prods=[],ocs=[],config={}}){
  const[nivel,setNivel]=useState("tipo") // abcd | tipo | sku
  const[metodo,setMetodo]=useState("compensada") // compensada | simple
  const[horizonte,setHorizonte]=useState(6) // 3/6/12/18 meses
  const[abcdSel,setAbcdSel]=useState({A:true,B:true,C:true,D:false})
  const[tipoSel,setTipoSel]=useState("")
  const[qSku,setQSku]=useState("")
  const[incluirTransito,setIncluirTransito]=useState(true)
  const[expanded,setExpanded]=useState({})
  const[hoverCell,setHoverCell]=useState(null)
  const[ordenarPor,setOrdenarPor]=useState("criticidad") // criticidad | venta | nombre

  const hoyDate=new Date()

  // Venta promedio diaria según método
  const ventaDia=(p)=>{
  ...
  // Venta promedio mensual (útil para mostrar demanda/mes)
