...
La OC original ${oc.id} quedó en estado "Dividida".`)
  ...
  const guardarRecepcion=async()=>{
    // Canal único (SOP): el registro manual queda como regularización de admin.
    if(cu?.rol!=="admin"){alert("Las recepciones se registran en el módulo Logística.\nEl registro manual en Compras quedó reservado a administración para regularizaciones.");return}
      ...
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"20px 20px 0 0",padding:"8px 24px 32px",width:"100%",maxWidth:760,maxHeight:"92vh",overflow:"auto",animation:"slideUp 0.3s ease"}}>
        ...
        <div style={{padding:"9px 12px",borderRadius:10,marginBottom:14,
          ...
          <div style={{fontSize:11,color:"#6D6D72",lineHeight:1.4}}>
            {cu?.rol==="admin"
              ?"El canal oficial es el módulo Logística (con inspección y respaldos). Usa este registro manual solo para regularizar casos históricos."
              :"La mercadería se recepciona en el módulo Logística — con inspección de bultos y respaldos — y se refleja automáticamente en esta OC. Si necesitas regularizar una recepción antigua, pídelo a administración."}
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:10,marginTop:14}}>
          <Bt v="pri" full onClick={guardarRecepcion} dis={guardandoRec||cu?.rol!=="admin"} ic={guardandoRec?"⏳":cu?.rol!=="admin"?"🔒":"✓"}>{guardandoRec?"Guardando...":cu?.rol!=="admin"?"Registro en Logística (canal único)":`Registrar ${recForm.tipo==="guia"?"guía":recForm.tipo==="factura"?"factura":"documento"} (regularización)`}</Bt>
