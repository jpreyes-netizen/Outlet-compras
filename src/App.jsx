...
/* ═══ COBERTURA — Timeline crítica con lead time, reorden y emergencia (v49)
   Cada fila es una barra horizontal que representa la cobertura en días.
   Marcadores verticales: punto de reorden, inicio emergencia, fecha de quiebre,
   fecha de llegada si se pide hoy (hoy + dias_fabricacion).
   Acción recomendada al lado del nombre según urgencia.
*/
function CoberturaView({prods=[],ocs=[],config={},params=[],paramsABCD=[]}){
  const[nivel,setNivel]=useLocalState("cob_nivel","sku") // abcd | tipo | sku
  ...
  const[ordenarPor,setOrdenarPor]=useLocalState("cob_orden","urgencia") // urgencia | venta | nombre
  ...
  // Venta diaria por producto respetando config
  const ventaDia=(p)=>{
  ...
  // Parámetros efectivos: viene del producto o se resuelven desde las tablas params/paramsABCD
  const getParams=(p)=>{
  ...
  const calcularProd=(p)=>{
    const vd=ventaDia(p)
    const disponible=(p.stock_actual||0)+(incluirTransito?(p.stock_transito||0):0)
    const{diasFab,periodoCob,diasEmerg}=getParams(p)

    const sinVenta=vd<=0
    const diasCobertura=sinVenta?999:Math.floor(disponible/vd)

    // Calcular días específicos
    // - diasHastaEmergencia: desde cuándo entra en zona de emergencia (stock == dias_emergencia × vd)
    // - diasHastaReorden: desde cuándo cruza el punto de reorden (stock == (fab+emerg) × vd)
    // - diasHastaQuiebre: = diasCobertura
    // - diasLeadTime: diasFab (si pido hoy, llega en X días)
    ...
    const diasHastaQuiebre=diasCobertura

    // Acción recomendada
    let accion="OK"; let accionColor="#34C759"
    if(sinVenta){accion="Sin venta"; accionColor="#8E8E93"}
    else if(diasHastaQuiebre<diasFab){accion="PEDIR YA"; accionColor="#FF3B30"} // Ni alcanza el lead time
    else if(diasHastaReorden<=0){accion="Pedir"; accionColor="#FF9500"} // Ya cruzó el punto de reorden
    else if(diasHastaReorden<=15){accion=`Pedir en ${diasHastaReorden}d`; accionColor="#FFD60A"}
    else {accion="Stock OK"; accionColor="#34C759"}

    return{
      ventaDia:vd,disponible,diasFab,periodoCob,diasEmerg,
      diasCobertura,diasHastaEmerg,diasHastaReorden,diasHastaQuiebre,
      sinVenta,accion,accionColor
    }
  }

  // Agrupar filas (A/B/C/D o por tipo)
  const agrupar=(key)=>{
    const groups={}
    prodsBase.forEach(p=>{
    ...
    return Object.entries(groups).map(([k,g])=>{
      const stockTotal=g.prods.reduce((s,p)=>s+(p.stock_actual||0),0)
      const transTotal=g.prods.reduce((s,p)=>s+(p.stock_transito||0),0)
      const disponible=stockTotal+(incluirTransito?transTotal:0)
      const vdTotal=g.prods.reduce((s,p)=>s+ventaDia(p),0)
      // Parámetros promedio ponderados por venta
      const pesoTotal=g.prods.reduce((s,p)=>s+ventaDia(p),0)
      let fabProm=30,cobProm=90,emergProm=5
      if(pesoTotal>0){
        fabProm=Math.round(g.prods.reduce((s,p)=>s+getParams(p).diasFab*ventaDia(p),0)/pesoTotal)
        cobProm=Math.round(g.prods.reduce((s,p)=>s+getParams(p).periodoCob*ventaDia(p),0)/pesoTotal)
        emergProm=Math.round(g.prods.reduce((s,p)=>s+getParams(p).diasEmerg*ventaDia(p),0)/pesoTotal)
      }
      const sinVenta=vdTotal<=0
      const diasCob=sinVenta?999:Math.floor(disponible/vdTotal)
      ...
      let accion="OK";let accionColor="#34C759"
      if(sinVenta){accion="Sin venta";accionColor="#8E8E93"}
      else if(diasCob<fabProm){accion="PEDIR YA";accionColor="#FF3B30"}
      else if(diasHastaReorden<=0){accion="Pedir";accionColor="#FF9500"}
      else if(diasHastaReorden<=15){accion=`Pedir en ${diasHastaReorden}d`;accionColor="#FFD60A"}

      return{
        key:k,label:key==="abcd"?`Clase ${k}`:k,abcd:key==="abcd"?k:null,
        nProds:g.prods.length,
        ventaDia:vdTotal,disponible,diasFab:fabProm,periodoCob:cobProm,diasEmerg:emergProm,
        diasCobertura:diasCob,diasHastaEmerg,diasHastaReorden,diasHastaQuiebre:diasCob,
        sinVenta,accion,accionColor,esGrupo:true,prods:g.prods
      }
    })
  }
  ...
  const filas=useMemo(()=>{
    ...
    if(ordenarPor==="urgencia"){
      // PEDIR YA > Pedir > Pedir en X > Stock OK > Sin venta
      ...
      arr.sort((a,b)=>b.ventaDia-a.ventaDia)
    }else{
  ...
  // Horizonte en días
  const diasTotal=horizonte*30
  const dayToPct=(d)=>Math.min(100,Math.max(0,(d/diasTotal)*100))
  ...
  // Fecha + dias → fecha resultante formato es-CL (dd MMM)
  ...
  return<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:12,flexWrap:"wrap",gap:10}}>
      <div>
        <div style={{fontSize:22,fontWeight:800,color:"#1C1C1E",letterSpacing:"-0.02em"}}>📅 Timeline Crítica de Cobertura</div>
        <div style={{fontSize:12,color:"#8E8E93"}}>Cada barra muestra los días de cobertura de stock. Los marcadores indican punto de reorden, zona de emergencia y quiebre. La línea vertical azul = fecha en que llegaría una OC si pedís HOY.</div>
      </div>
    </div>
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(170px,1fr))",gap:10}}>
        ...
        <div>
          ...
          <select value={ordenarPor} onChange={e=>setOrdenarPor(e.target.value)} style={css.select}>
            ...
            <option value="venta">Mayor venta diaria</option>
    ...
    {/* LEYENDA */}
    <div style={{display:"flex",gap:14,flexWrap:"wrap",fontSize:11,marginBottom:10,padding:"10px 14px",background:"#fff",borderRadius:8,alignItems:"center"}}>
      <div style={{display:"flex",alignItems:"center",gap:6}}>
        <div style={{width:40,height:14,borderRadius:3,background:"linear-gradient(to right,#34C759 0%,#34C759 60%,#FFD60A 60%,#FFD60A 80%,#FF3B30 80%,#FF3B30 100%)"}}/>
        <span>Stock normal → emergencia → crítico</span>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:4}}>
        <div style={{width:2,height:16,background:"#FFD60A",position:"relative"}}><div style={{position:"absolute",top:-4,left:-3,width:8,height:8,borderRadius:4,background:"#FFD60A"}}/></div>
        <span>Pto. reorden</span>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:4}}>
        <div style={{width:2,height:16,background:"#FF9500",position:"relative"}}><div style={{position:"absolute",top:-4,left:-3,width:8,height:8,borderRadius:4,background:"#FF9500"}}/></div>
        <span>Emergencia</span>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:4}}>
        <div style={{width:2,height:16,background:"#FF3B30",position:"relative"}}><div style={{position:"absolute",top:-4,left:-3,width:8,height:8,borderRadius:4,background:"#FF3B30"}}/></div>
        <span>Quiebre</span>
      </div>
      <div style={{display:"flex",alignItems:"center",gap:4}}>
        <div style={{width:2,height:16,background:"#007AFF",position:"relative",borderLeft:"2px dashed #007AFF"}}/>
        <span>Llegada si pido hoy</span>
      </div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(140px,1fr))",gap:8,marginBottom:12}}>
      ...
      <div style={{padding:"10px 12px",background:"#fff",borderRadius:10,borderLeft:"3px solid #FFD60A"}}>
        ...
        <div style={{fontSize:10,color:"#8E8E93"}}>en los próx. 15d</div>
      </div>
    ...
    <Cd>
      ...
      <div style={{overflow:"auto"}}>
        <div style={{minWidth:1000}}>
          {/* Escala de tiempo */}
          <div style={{display:"grid",gridTemplateColumns:"280px 110px 1fr 80px",gap:10,alignItems:"center",borderBottom:"2px solid #E5E5EA",paddingBottom:8,marginBottom:8}}>
            <div style={{fontSize:10,fontWeight:700,color:"#8E8E93",textTransform:"uppercase"}}>Ítem</div>
            <div style={{fontSize:10,fontWeight:700,color:"#8E8E93",textTransform:"uppercase"}}>Acción</div>
            <div style={{position:"relative",height:20}}>
              {/* Marcas cada mes */}
              {Array.from({length:horizonte+1}).map((_,i)=>{
                const pct=(i/horizonte)*100
                const fechaLbl=i===0?"Hoy":fmtFecha(i*30)
                return<div key={i} style={{position:"absolute",left:`${pct}%`,transform:i===0?"none":i===horizonte?"translateX(-100%)":"translateX(-50%)",fontSize:9,color:"#8E8E93",fontWeight:600,whiteSpace:"nowrap"}}>
                  {fechaLbl}
                </div>
              })}
            </div>
            <div style={{fontSize:10,fontWeight:700,color:"#8E8E93",textTransform:"uppercase",textAlign:"right"}}>Cobertura</div>
          </div>

          {/* Filas */}
          {filas.map(fila=>{
            const abcdColor=fila.abcd?CL[fila.abcd]:null
            const expandable=fila.esGrupo
            const estaExpandido=!!expanded[fila.key]
            const isHover=hoverItem===fila.key
            const rowBg=fila.accion==="PEDIR YA"?"#FF3B3008":fila.accion==="Pedir"?"#FF950008":"transparent"
            const rowBorderLeft=fila.accion==="PEDIR YA"?"3px solid #FF3B30":fila.accion==="Pedir"?"3px solid #FF9500":"3px solid transparent"

            return<div key={fila.key} style={{position:"relative"}}>
              <div onMouseEnter={()=>setHoverItem(fila.key)} onMouseLeave={()=>setHoverItem(null)} style={{display:"grid",gridTemplateColumns:"280px 110px 1fr 80px",gap:10,alignItems:"center",padding:"10px 8px",borderBottom:"1px solid #F2F2F7",cursor:expandable?"pointer":"default",background:rowBg,borderLeft:rowBorderLeft,transition:"background 0.15s"}} onClick={()=>{if(expandable)setExpanded(p=>({...p,[fila.key]:!p[fila.key]}))}}>
                {/* Columna ítem */}
                <div style={{minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    {expandable&&<span style={{fontSize:10,color:"#8E8E93"}}>{estaExpandido?"▼":"▶"}</span>}
                    ...
                    <span style={{fontSize:13,fontWeight:600,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{fila.label}</span>
                  </div>
                  <div style={{fontSize:10,color:"#AEAEB2",marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>
                    ...
                    Stock {fN(fila.disponible)}u · Venta {fN(fila.ventaDia*30)}/mes
                  </div>
                </div>

                {/* Columna acción recomendada */}
                <div style={{textAlign:"center"}}>
                  <div style={{display:"inline-block",padding:"4px 8px",borderRadius:6,background:fila.accionColor+"20",color:fila.accionColor,fontSize:11,fontWeight:700,border:`1px solid ${fila.accionColor}40`,whiteSpace:"nowrap"}}>
                    {fila.accion==="PEDIR YA"?"🚨 "+fila.accion:fila.accion==="Pedir"?"⚠ "+fila.accion:fila.accion==="Sin venta"?fila.accion:fila.accion}
                  </div>
                </div>

                {/* Barra de timeline */}
                <div style={{position:"relative",height:38,background:"#FAFAFA",borderRadius:6,border:"1px solid #F2F2F7"}}>
                  {/* Marcas de grilla mensuales */}
                  {Array.from({length:horizonte-1}).map((_,i)=>{
                    const pct=((i+1)/horizonte)*100
                    return<div key={i} style={{position:"absolute",left:`${pct}%`,top:0,bottom:0,width:1,background:"#E5E5EA",opacity:0.7}}/>
                  })}

                  {/* Barra segmentada: verde (normal) → amarillo (emergencia) → rojo (sin stock) */}
                  {!fila.sinVenta&&(()=>{
                    const pctReorden=dayToPct(fila.diasHastaReorden)
                    const pctEmerg=dayToPct(fila.diasHastaEmerg)
                    const pctQuiebre=dayToPct(fila.diasHastaQuiebre)
                    return<>
                      {/* Zona verde (stock cómodo): 0 → pto reorden */}
                      {fila.diasHastaReorden>0&&<div style={{position:"absolute",left:0,top:8,height:22,width:`${pctReorden}%`,background:"#34C759",borderRadius:"4px 0 0 4px",opacity:0.85}}/>}
                      {/* Zona amarilla (entre reorden y emergencia) */}
                      {pctEmerg>pctReorden&&<div style={{position:"absolute",left:`${pctReorden}%`,top:8,height:22,width:`${pctEmerg-pctReorden}%`,background:"#FFD60A",opacity:0.85}}/>}
                      {/* Zona naranja (emergencia) */}
                      {pctQuiebre>pctEmerg&&<div style={{position:"absolute",left:`${pctEmerg}%`,top:8,height:22,width:`${pctQuiebre-pctEmerg}%`,background:"#FF9500",opacity:0.85}}/>}
                      {/* Zona roja rayada (post-quiebre hasta fin horizonte) */}
                      {pctQuiebre<100&&<div style={{position:"absolute",left:`${pctQuiebre}%`,top:8,height:22,width:`${100-pctQuiebre}%`,background:"repeating-linear-gradient(45deg,#FF3B3030,#FF3B3030 4px,#FF3B3060 4px,#FF3B3060 8px)",borderRadius:"0 4px 4px 0"}}/>}

                      {/* Marcador: punto de reorden (amarillo) */}
                      {fila.diasHastaReorden>0&&fila.diasHastaReorden<diasTotal&&<div style={{position:"absolute",left:`${pctReorden}%`,top:0,bottom:0,width:2,background:"#FFD60A",zIndex:2}}>
                        <div style={{position:"absolute",top:-2,left:-4,width:10,height:10,borderRadius:5,background:"#FFD60A",border:"1.5px solid #fff",boxShadow:"0 1px 2px rgba(0,0,0,0.2)"}}/>
                      </div>}
                      {/* Marcador: inicio emergencia (naranja) */}
                      {fila.diasHastaEmerg>0&&fila.diasHastaEmerg<diasTotal&&<div style={{position:"absolute",left:`${pctEmerg}%`,top:0,bottom:0,width:2,background:"#FF9500",zIndex:2}}>
                        <div style={{position:"absolute",top:-2,left:-4,width:10,height:10,borderRadius:5,background:"#FF9500",border:"1.5px solid #fff",boxShadow:"0 1px 2px rgba(0,0,0,0.2)"}}/>
                      </div>}
                      {/* Marcador: fecha de quiebre (rojo) */}
                      {fila.diasHastaQuiebre<diasTotal&&<div style={{position:"absolute",left:`${pctQuiebre}%`,top:0,bottom:0,width:2,background:"#FF3B30",zIndex:2}}>
                        <div style={{position:"absolute",top:-2,left:-4,width:10,height:10,borderRadius:5,background:"#FF3B30",border:"1.5px solid #fff",boxShadow:"0 1px 2px rgba(0,0,0,0.2)"}}/>
                      </div>}
                    </>
                  })()}

                  {fila.sinVenta&&<div style={{position:"absolute",inset:"8px 0",display:"flex",alignItems:"center",justifyContent:"center",color:"#8E8E93",fontSize:11,fontStyle:"italic"}}>Sin venta — no se proyecta</div>}

                  {/* Línea vertical: hoy + diasFab (fecha de llegada si pido hoy) */}
                  {!fila.sinVenta&&fila.diasFab<diasTotal&&<div style={{position:"absolute",left:`${dayToPct(fila.diasFab)}%`,top:-2,bottom:-2,width:0,borderLeft:"2px dashed #007AFF",zIndex:3,pointerEvents:"none"}}>
                    <div style={{position:"absolute",top:-3,left:-5,width:10,height:10,borderRadius:5,background:"#007AFF",border:"2px solid #fff",boxShadow:"0 1px 3px rgba(0,122,255,0.5)"}}/>
                  </div>}
                </div>

                {/* Columna cobertura */}
                <div style={{textAlign:"right"}}>
                  <div style={{fontSize:16,fontWeight:800,color:fila.accionColor}}>{fila.sinVenta?"∞":fila.diasCobertura+"d"}</div>
                  {!fila.sinVenta&&<div style={{fontSize:9,color:"#8E8E93",marginTop:1}}>{fmtFecha(fila.diasCobertura)}</div>}
                </div>
              </div>

              {/* TOOLTIP RICO (al hover) */}
              {isHover&&!fila.sinVenta&&<div style={{position:"absolute",left:20,top:"100%",marginTop:-4,zIndex:100,background:"#1C1C1E",color:"#fff",padding:"12px 14px",borderRadius:8,fontSize:11,minWidth:320,maxWidth:400,boxShadow:"0 8px 24px rgba(0,0,0,0.25)",pointerEvents:"none",lineHeight:1.55}}>
                <div style={{fontWeight:700,fontSize:12,marginBottom:8,paddingBottom:6,borderBottom:"1px solid #3A3A3C"}}>{fila.label}</div>

                <div style={{display:"grid",gridTemplateColumns:"auto 1fr",gap:"4px 12px",marginBottom:8}}>
                  <span style={{color:"#8E8E93"}}>Stock + tránsito:</span>
                  <span style={{fontWeight:600}}>{fN(fila.disponible)} u</span>
                  <span style={{color:"#8E8E93"}}>Venta diaria:</span>
                  <span style={{fontWeight:600}}>{fila.ventaDia.toFixed(1)} u/día ({fN(fila.ventaDia*30)}/mes)</span>
                </div>

                <div style={{marginBottom:8,padding:"6px 8px",background:"#2A2A2C",borderRadius:5}}>
                  <div style={{fontSize:10,color:"#8E8E93",fontWeight:700,textTransform:"uppercase",marginBottom:4}}>Parámetros del producto</div>
                  <div style={{display:"grid",gridTemplateColumns:"repeat(3,1fr)",gap:6,fontSize:10}}>
                    <div><div style={{color:"#8E8E93"}}>Lead time</div><div style={{fontWeight:700,color:"#007AFF"}}>{fila.diasFab}d</div></div>
                    <div><div style={{color:"#8E8E93"}}>Cobertura</div><div style={{fontWeight:700,color:"#34C759"}}>{fila.periodoCob}d</div></div>
                    <div><div style={{color:"#8E8E93"}}>Emergencia</div><div style={{fontWeight:700,color:"#FF9500"}}>{fila.diasEmerg}d</div></div>
                  </div>
                </div>

                <div style={{marginBottom:8}}>
                  <div style={{fontSize:10,color:"#8E8E93",fontWeight:700,textTransform:"uppercase",marginBottom:4}}>Fechas clave</div>
                  {fila.diasHastaReorden>0&&<div style={{display:"flex",justifyContent:"space-between",padding:"2px 0"}}><span style={{color:"#FFD60A"}}>● Punto de reorden:</span><span style={{fontWeight:600}}>en {fila.diasHastaReorden}d ({fmtFecha(fila.diasHastaReorden)})</span></div>}
                  {fila.diasHastaEmerg>0&&<div style={{display:"flex",justifyContent:"space-between",padding:"2px 0"}}><span style={{color:"#FF9500"}}>● Inicio emergencia:</span><span style={{fontWeight:600}}>en {fila.diasHastaEmerg}d ({fmtFecha(fila.diasHastaEmerg)})</span></div>}
                  <div style={{display:"flex",justifyContent:"space-between",padding:"2px 0"}}><span style={{color:"#FF3B30"}}>● Quiebre estimado:</span><span style={{fontWeight:600}}>en {fila.diasHastaQuiebre}d ({fmtFecha(fila.diasHastaQuiebre)})</span></div>
                  <div style={{display:"flex",justifyContent:"space-between",padding:"2px 0",borderTop:"1px solid #3A3A3C",marginTop:4,paddingTop:4}}><span style={{color:"#007AFF"}}>● Si pido hoy, llega:</span><span style={{fontWeight:600}}>en {fila.diasFab}d ({fmtFecha(fila.diasFab)})</span></div>
                </div>

                <div style={{padding:"6px 8px",background:fila.accionColor+"20",borderRadius:5,borderLeft:`3px solid ${fila.accionColor}`}}>
                  <div style={{fontSize:10,color:"#8E8E93",fontWeight:700,textTransform:"uppercase",marginBottom:3}}>Recomendación</div>
                  <div style={{fontWeight:700,color:fila.accionColor,fontSize:12}}>
                    {fila.accion==="PEDIR YA"?`🚨 PEDIR YA — el stock se agota en ${fila.diasHastaQuiebre}d pero la reposición tarda ${fila.diasFab}d. Va a haber quiebre.`:
                     fila.accion==="Pedir"?`⚠ PEDIR — ya cruzaste el punto de reorden. Pedí ahora para mantener la cobertura.`:
                     fila.accion.startsWith("Pedir en")?`⏰ ${fila.accion} — acercate al punto de reorden, planifica la compra.`:
                     `✓ Stock OK — cobertura suficiente por ${fila.diasCobertura}d.`}
                  </div>
                </div>
              </div>}

              {/* Expansión de grupo */}
              {expandable&&estaExpandido&&<div style={{paddingLeft:20,background:"#FAFAFA",borderLeft:"3px solid #E5E5EA"}}>
                {fila.prods.slice(0,30).map(p=>{
                  const c=calcularProd(p)
                  const pctReo=dayToPct(c.diasHastaReorden)
                  const pctEmg=dayToPct(c.diasHastaEmerg)
                  const pctQbr=dayToPct(c.diasHastaQuiebre)
                  return<div key={p.sku||p.producto} style={{display:"grid",gridTemplateColumns:"280px 110px 1fr 80px",gap:10,alignItems:"center",padding:"6px 8px",borderBottom:"1px solid #F2F2F7",fontSize:11}}>
                    <div style={{paddingLeft:14,minWidth:0}}>
                      <div style={{fontSize:12,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.producto}</div>
                      <div style={{fontSize:9,color:"#AEAEB2"}}>{p.sku} · fab {c.diasFab}d · emerg {c.diasEmerg}d</div>
                    </div>
                    <div style={{textAlign:"center"}}>
                      <div style={{display:"inline-block",padding:"2px 6px",borderRadius:4,background:c.accionColor+"20",color:c.accionColor,fontSize:10,fontWeight:700,whiteSpace:"nowrap"}}>{c.accion}</div>
                    </div>
                    <div style={{position:"relative",height:22,background:"#fff",borderRadius:4,border:"1px solid #F2F2F7"}}>
                      {!c.sinVenta&&<>
                        {c.diasHastaReorden>0&&<div style={{position:"absolute",left:0,top:4,height:14,width:`${pctReo}%`,background:"#34C759",borderRadius:"3px 0 0 3px",opacity:0.8}}/>}
                        {pctEmg>pctReo&&<div style={{position:"absolute",left:`${pctReo}%`,top:4,height:14,width:`${pctEmg-pctReo}%`,background:"#FFD60A",opacity:0.8}}/>}
                        {pctQbr>pctEmg&&<div style={{position:"absolute",left:`${pctEmg}%`,top:4,height:14,width:`${pctQbr-pctEmg}%`,background:"#FF9500",opacity:0.8}}/>}
                        {pctQbr<100&&<div style={{position:"absolute",left:`${pctQbr}%`,top:4,height:14,width:`${100-pctQbr}%`,background:"repeating-linear-gradient(45deg,#FF3B3030,#FF3B3030 3px,#FF3B3060 3px,#FF3B3060 6px)",borderRadius:"0 3px 3px 0"}}/>}
                        {c.diasFab<diasTotal&&<div style={{position:"absolute",left:`${dayToPct(c.diasFab)}%`,top:-1,bottom:-1,width:0,borderLeft:"2px dashed #007AFF",zIndex:2}}/>}
                      </>}
                    </div>
                    <div style={{textAlign:"right",fontSize:11,fontWeight:700,color:c.accionColor}}>{c.sinVenta?"∞":c.diasCobertura+"d"}</div>
                  </div>
                })}
                {fila.prods.length>30&&<div style={{fontSize:10,color:"#8E8E93",textAlign:"center",padding:6,fontStyle:"italic"}}>+ {fila.prods.length-30} SKU más. Filtrá por tipo específico para verlos todos.</div>}
              </div>}
            </div>
          })}
        </div>
    ...
    <div style={{marginTop:12,padding:"10px 14px",background:"#007AFF08",borderRadius:8,fontSize:11,color:"#3A3A3C",lineHeight:1.5}}>
      <strong>📌 Cómo se lee el gráfico:</strong> Cada barra representa la cobertura actual de stock proyectada hacia adelante. La zona <span style={{color:"#34C759",fontWeight:700}}>verde</span> es tiempo cómodo, la <span style={{color:"#A07D00",fontWeight:700}}>amarilla</span> empieza cuando cruzás el punto de reorden, la <span style={{color:"#FF9500",fontWeight:700}}>naranja</span> es zona de emergencia, y la <span style={{color:"#FF3B30",fontWeight:700}}>roja rayada</span> es quiebre. La línea <span style={{color:"#007AFF",fontWeight:700}}>azul punteada</span> marca cuándo llegaría una OC si pedís HOY (hoy + días de fabricación). <strong>Si la línea azul queda DESPUÉS del marcador rojo → vas a tener quiebre antes de que llegue la reposición → PEDIR YA.</strong> Promedio calculado con {mesesValidos} de {totalMeses} meses{excluirActual?" (excluye mes en curso)":""} · método {metodo==="compensada"?"compensado (excluye quiebres)":"simple"}{incluirTransito?" · incluye stock en tránsito":""}.
    </div>
  </div>
}
