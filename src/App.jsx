...
export default function App(){
  ...
  const[cu,setCu]=useState(()=>{try{const s=localStorage.getItem('erp_cu');return s?JSON.parse(s):null}catch{return null}})
  ...
  const[tab,setTab]=useState(()=>localStorage.getItem('erp_tab')||"monitor")
  const[det,setDet]=useState(null)
  const[loading,setLoading]=useState(true)
  const[configTab,setConfigTab]=useState("params")

  // Persistir usuario en localStorage + sincronizar logout entre pestañas
  useEffect(()=>{
    if(cu){localStorage.setItem('erp_cu',JSON.stringify(cu))}
    else{localStorage.removeItem('erp_cu')}
  },[cu])

  // Persistir tab activo
  useEffect(()=>{localStorage.setItem('erp_tab',tab)},[tab])

  // Sincronizar logout entre pestañas
  useEffect(()=>{
    const onStorage=e=>{if(e.key==='erp_cu'&&!e.newValue)setCu(null)}
    ...
    <div style={{position:"sticky",top:0,zIndex:50,background:"rgba(242,242,247,0.9)",backdropFilter:"blur(20px)",padding:"14px 0 10px",marginBottom:10,borderBottom:"1px solid rgba(0,0,0,0.06)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <button onClick={()=>{localStorage.removeItem('erp_cu');localStorage.removeItem('erp_tab');setCu(null)}} style={{width:32,height:32,borderRadius:10,background:"#FF3B3015",border:"none",cursor:"pointer",fontSize:13,color:"#FF3B30"}}>⏻</button>
        </div>
      </div>
    </div>
