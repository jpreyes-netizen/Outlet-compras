...
function RepoView({prods,prodsSuc=[],cart,setCart,go,config,params,paramsABCD,sucursales=[]}){
  ...
  {/* ⭐ Sheet: Split de OC */}
  <Sheet open={showSplit} onClose={()=>setShowSplit(false)} title={`✂ Dividir ${oc.id} en dos partes`}>
    <div style={{fontSize:12,color:"#8E8E93",marginBottom:14,background:"#5856D610",borderRadius:8,padding:"10px 12px",border:"1px solid #5856D630"}}>
      Asigná cuántas unidades van en cada parte. <b>Parte A + Parte B debe sumar la cantidad original.</b> Ambas OCs nacerán en estado <b>{oc.estado}</b> heredando las aprobaciones actuales.
    </div>
    ...
    <div style={{overflowX:"auto",marginBottom:16}}>
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12}}>
        <thead><tr style={{background:"#F2F2F7"}}>
          ...
          <th style={{padding:"6px",textAlign:"right",fontSize:10,fontWeight:700,color:"#8E8E93"}}>✓</th>
        </tr></thead>
        <tbody>{items.map(i=>{
    ...
    {(()=>{
      ...
      const totalB=items.reduce((s,i)=>{const b=(i.cantidad_pedida||0)-(Number(splitQty[i.id])||0);return s+Math.max(0,b)*(i.costo_unitario||0)},0)
      return<div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:14}}>
        <div style={{background:"#5856D610",borderRadius:8,padding:10,textAlign:"center"}}>
          ...
          <div style={{fontSize:16,fontWeight:800,color:"#5856D6"}}>{isI?fUD(totalA,2):fmt(Math.round(totalA))}</div>
        </div>
        <div style={{background:"#007AFF10",borderRadius:8,padding:10,textAlign:"center"}}>
          ...
          <div style={{fontSize:16,fontWeight:800,color:"#007AFF"}}>{isI?fUD(totalB,2):fmt(Math.round(totalB))}</div>
        </div>
      </div>
    })()}
