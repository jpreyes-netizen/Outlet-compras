...
function RepoView({prods,cart,setCart,go,config,params,paramsABCD,sucursales=[]}){
  const[q,setQ]=useLocalState("repo_q","");const[fc,setFc]=useLocalState("repo_fc","T");const[ft,setFt]=useLocalState("repo_ft","T");const[sortBy,setSortBy]=useLocalState("repo_sortBy","costo_reposicion");const[fe,setFe]=useLocalState("repo_fe","T");const[sortDir,setSortDir]=useLocalState("repo_sortDir","desc")
  const[fsuc,setFsuc]=useLocalState("repo_fsuc","T") // ⭐ filtro sucursal
...
function BsaleConfig({config,saveConfig,bsaleToken,setBsaleToken,saveBsale,loadAll}){
  ...
  const processExcel=async(file)=>{
    ...
    try{
      ...
      // ⭐ v48: Respetar config.excluir_mes_actual
      ...
      addLog(`📅 Cálculo de promedio usa ${mesesValidosRep} de ${totalMesesRep} meses${excluirMesActual?" (mes en curso excluido)":""}`,"success")
      let reposCount=0
      for(const p of prodList){
