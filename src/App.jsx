...
export default function App(){
  ...
  // ⭐ Helper de notificaciones: crea una fila en notificaciones → trigger SQL la envía
  // Busca al destinatario según el rol que debe actuar en ese estado
  const notificarFlujo=async({ocId,tipo,nuevoEstado,asunto,mensaje,rolDestino})=>{
    if(config.email_activo!=="true"){console.log("📧 Email desactivado, no se notifica");return}
    if(!rolDestino){console.warn("notificarFlujo: falta rolDestino");return}
    // Buscar usuario con ese rol (primer activo con correo válido)
    const destinatarios=users.filter(u=>u.rol===rolDestino&&u.activo&&u.correo&&u.correo.includes("@"))
    if(destinatarios.length===0){console.warn(`notificarFlujo: sin usuarios activos con rol ${rolDestino}`);return}
    // Crear una notificación por cada destinatario (ej: múltiples analistas)
    const filas=destinatarios.map(u=>({
      id:uid(),
      oc_id:ocId,
      tipo:"Email", // canal de comunicación
      destino_correo:u.correo,
      destino_nombre:u.nombre,
    ...
    else{console.log(`📧 ${filas.length} notificación(es) creada(s) para rol ${rolDestino}`)}
  }
...
function SolView({cart,setCart,provs,users,sucursales=[],cu,setOcs,addFirma,goOC,ocs,config={},notificarFlujo}){
  ...
  const submit=async()=>{
    ...
    setSaving(true);const id=nextNum()
...
function OCListView({ocs,firmas,pagos,updOC,addFirma,setDet,cu,h,provs,setOcs,notificarFlujo,ROL_POR_ESTADO={}}){
  ...
  const firma=async(oc,acc,nE,nF)=>{
    ...
    // ⭐ Notificar al siguiente rol del flujo con email
    if(notificarFlujo&&ROL_POR_ESTADO[nE]){
      const pv=provs.find(p=>p.id===oc.proveedor_id)
      await notificarFlujo({
        ocId:oc.id,
        tipo:"Email",
        nuevoEstado:nE,
        rolDestino:ROL_POR_ESTADO[nE],
    ...
    {fil.map(oc=>{const ocF=firmas.filter(fi=>fi.oc_id===oc.id);const isI=oc.tipo_oc==="Importación";const pv=provs?.find(p=>p.id===oc.proveedor_id)
      return<Cd key={oc.id} s={{marginBottom:8}} onClick={()=>setDet(oc)}>
        ...
        <div style={{display:"flex",gap:6,flexWrap:"wrap",alignItems:"center"}} onClick={e=>e.stopPropagation()}>
          ...
          {oc.estado==="Pend. proveedor"&&h("valid_prov")&&<Bt sm v="suc" onClick={()=>showComment===oc.id+"a"?firma(oc,isI?"Proforma confirmada":"Proveedor confirmó",isI?"Proforma OK":"Confirmada prov.",isI?6:4):setShowComment(oc.id+"a")} ic="◉">{showComment===oc.id+"a"?"Confirmar":"Confirmado"}</Bt>}
...
function OCDetView({oc,firmas,pagos,provs=[],updOC,addFirma,setPagos,close,cu,h,config={},notificarFlujo,ROL_POR_ESTADO={},setTab}){
  ...
  const firma=async(a,nE,nF)=>{
    ...
    // ⭐ Notificar al siguiente rol del flujo
    if(notificarFlujo&&ROL_POR_ESTADO[nE]){
      const pv=provs.find(p=>p.id===oc.proveedor_id)
      await notificarFlujo({
        ocId:oc.id,
  ...
  return<div>
    ...
    {oc.estado==="Pend. proveedor"&&h("valid_prov")&&<div style={{borderTop:"2px solid #E5E5EA",paddingTop:16,marginTop:16}}>
      ...
      <Bt v="suc" full onClick={async()=>{
        ...
        await addFirma(oc.id,`Proforma confirmada${adj?" (ajustes: "+adj+")":""}${planPagoStr}${provNotas?" — "+provNotas:""}`)
        await updOC(oc.id,{estado:isI?"Proforma OK":"Confirmada prov.",fase_actual:isI?6:4,total_clp:isI?Math.round(newTotal*TC_USD):newTotal,total_usd:isI?Math.round(newTotal):(oc.total_usd||0)})
        close()
      }} ic="✓">Confirmar proforma del proveedor</Bt>
    </div>}
...
function CosteoImpView({config,saveConfig,ocs,cu,addFirma}){
  ...
  const guardarCosteo=async()=>{
    ...
    if(!window.confirm(`¿Anexar este costeo al historial de ${selOC}?`))return
    const row={
      ...
      fob_clp:toCLP(valorFOB_USD),flete_clp:toCLP(flete_USD),seguro_clp:toCLP(seguro_USD),
      aduana_total_clp:toCLP(adValorem_USD+iva_USD),puerto_total_clp:toCLP(puerto),
      agente_total_clp:toCLP(agenteAd_USD),transporte_total_clp:toCLP(fleteInterno_USD),
      financiero_total_clp:0,costo_total_clp:toCLP(totalImport_USD),
      costo_unit_bodega:totalCantidad>0?Math.round(toCLP(totalImport_USD)/totalCantidad):0,
      margen_pct:Math.round(margenPromedio),precio_venta_clp:totalCantidad>0?Math.round(totalVentaCLP/totalCantidad):0,
    ...
    if(addFirma)await addFirma(selOC,`Costeo IMP anexado: CU bodega promedio ${fmt(row.costo_unit_bodega)} · margen ${Math.round(margenPromedio)}% · total ${fmt(row.costo_total_clp)}`)
