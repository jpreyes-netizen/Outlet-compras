...
function FinanzasView({ocs,provs,pagos,setPagos}){
  ...
  const marcarPagado=async(h)=>{
    ...
    if(!monto)return
    // Si el pago ya existe en DB (fue creado con la OC), actualizarlo
    if(h.pago_id){
      const{error}=await supabase.from('pagos').update({estado:"Pagado",fecha_pago:fecha,monto:parseFloat(monto)}).eq('id',h.pago_id)
      if(error){alert("Error: "+error.message);return}
      setPagos(prev=>prev.map(p=>p.id===h.pago_id?{...p,estado:"Pagado",fecha_pago:fecha,monto:parseFloat(monto)}:p))
    }else{
      // Legacy: crear registro nuevo
