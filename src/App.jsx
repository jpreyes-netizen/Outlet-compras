...
function OCListView({ocs,firmas,pagos,updOC,addFirma,setDet,cu,h,provs,setOcs,notificarFlujo,ROL_POR_ESTADO={}}){
  ...
  const[docsOC,setDocsOC]=useState(new Map())
  const[facsOC,setFacsOC]=useState(new Map())
  const[costOC,setCostOC]=useState(new Map())   // oc_id → Set(tipo_costeo)
  const[recsOC,setRecsOC]=useState(new Map())   // oc_id → [{tipo,numero_doc,archivo_url,archivo_nombre}]
  useEffect(()=>{
    if(vistaOC!=="tabla")return
    ;(async()=>{
      ...
      const fm=new Map()
      for(const fc of(facs||[])){if(fc.anulado)continue;if(!fm.has(fc.oc_id))fm.set(fc.oc_id,[]);fm.get(fc.oc_id).push(fc)}
      setFacsOC(fm)
      const{data:cts}=await supabase.from('costeos_oc').select('oc_id,tipo_costeo').limit(20000)
      const cm=new Map()
      for(const c of(cts||[])){if(!c.oc_id)continue;if(!cm.has(c.oc_id))cm.set(c.oc_id,new Set());cm.get(c.oc_id).add(c.tipo_costeo)}
      setCostOC(cm)
      const{data:rcs}=await supabase.from('recepciones').select('id,oc_id,tipo,numero_doc,archivo_url,archivo_nombre').limit(20000)
      const rm=new Map()
      for(const r of(rcs||[])){if(!r.oc_id)continue;if(!rm.has(r.oc_id))rm.set(r.oc_id,[]);rm.get(r.oc_id).push(r)}
      setRecsOC(rm)
    })()
  },[vistaOC])
    ...
    {vistaOC==="tabla"&&(()=>{
      ...
      return<div style={{background:"#fff",borderRadius:12,border:"1px solid #E5E5EA",overflow:"hidden"}}>
        <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",padding:"10px 14px",borderBottom:"1px solid #F2F2F7"}}>
          <div style={{fontSize:13,fontWeight:700,color:"#1C1C1E"}}>📊 Órdenes de compra — vista completa</div>
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:2150}}>
            ...
            <tbody>{sorted.map((r,i)=>{
              return<tr key={r.id} style={{borderBottom:"1px solid #F2F2F7",background:i%2===0?"#fff":"#FAFAFA",cursor:"pointer"}} onClick={()=>{const oc=ocs.find(o=>o.id===r.id);if(oc)setDet(oc)}}>
                ...
                <td style={{padding:"6px 8px",textAlign:"center"}} onClick={e=>e.stopPropagation()}>{!r.isImp?<span style={{fontSize:11,color:"#C7C7CC"}}>—</span>:<span style={{fontSize:10,fontWeight:700,whiteSpace:"nowrap"}}><span style={{color:r.costeoP?"#34C759":"#FF3B30"}}>P{r.costeoP?"✓":"✗"}</span> <span style={{color:r.costeoF?"#34C759":"#FF9500"}}>F{r.costeoF?"✓":"✗"}</span></span>}</td>
