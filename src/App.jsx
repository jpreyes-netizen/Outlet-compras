...
export default function App(){
  ...
  const ROL_POR_ESTADO={
    ...
    "Aprobada":"analista", // cuando finanzas aprueba vuelve al analista para curse
  ...
  return<div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif",margin:0,padding:"0 20px 100px",background:"#F2F2F7",minHeight:"100vh",fontSize:14}}>
    ...
    <Sheet show={!!det} onClose={()=>setDet(null)} title={det?.id||""}>{det&&<OCDetView oc={det} firmas={firmas.filter(f=>f.oc_id===det.id)} pagos={pagos.filter(p=>p.oc_id===det.id)} provs={provs} updOC={updOC} addFirma={addFirma} setPagos={setPagos} close={()=>{setDet(null);loadAll()}} cu={cu} h={h} config={config} notificarFlujo={notificarFlujo} ROL_POR_ESTADO={ROL_POR_ESTADO}/>}</Sheet>
...
function OCDetView({oc,firmas,pagos,provs=[],updOC,addFirma,setPagos,close,cu,h,config={},notificarFlujo,ROL_POR_ESTADO={}}){
  const TC_USD=Number(config.tc_usd)||950
