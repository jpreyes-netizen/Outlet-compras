...
function ForecastView({prods,ocs,config,saveConfig}){
  // ═══════════════════════════════════════════════════════════════════
  // ⭐ FORECAST v2 — Proyección de compras por categoría con ESCENARIOS
  //    Motor: para cada SKU, la compra proyectada usa los parámetros
  //    vigentes (días fabricación + emergencia + período de cobertura
  //    por tipo) sobre la demanda compensada. El escenario aplica un
  //    uplift % (global o por categoría) sobre la venta proyectada.
  ...
  // — Expansión sucursal (se preserva del forecast v1) —
  ...
  const skuF=useMemo(()=>prods.map(p=>{
    ...
    const horizonte=(p.periodo_cubrir||90)+(p.dias_fabricacion||30)+(p.dias_emergencia||5)
    ...
    return{...p,dBase,dEsc,up:up*100,horizonte,stockDisp,compraBase,compraEsc,
  ...
  const cats=useMemo(()=>{
    ...
    for(const p of skuF){
      ...
      if(!m[t])m[t]={tipo:t,skus:0,vtaMesBase:0,vtaMesEsc:0,stock:0,transito:0,compraBase:0,compraEsc:0,invBase:0,invEsc:0,horiz:0,conCompra:0}
      const c=m[t]
      ...
      c.horiz+=p.horizonte
      if(p.compraEsc>0)c.conCompra++
    }
    return Object.values(m).map(c=>({...c,
      horizProm:Math.round(c.horiz/Math.max(1,c.skus)),
      cobDias:c.vtaMesBase>0?Math.round((c.stock+c.transito)/(c.vtaMesBase/30)):999,
      up:upliftDe(c.tipo)}))
  },[skuF,uplift,catUp])
  ...
  const T=cats.reduce((s,c)=>({vB:s.vB+c.vtaMesBase,vE:s.vE+c.vtaMesEsc,cB:s.cB+c.compraBase,cE:s.cE+c.compraEsc,iB:s.iB+c.invBase,iE:s.iE+c.invEsc}),{vB:0,vE:0,cB:0,cE:0,iB:0,iE:0})
  ...
  const exportarReporte=()=>{
    try{
      const wsResumen=XLSX.utils.json_to_sheet(cats.sort((a,b)=>b.invEsc-a.invEsc).map(c=>({
        ...
        Venta_mensual_base_uds:c.vtaMesBase,Venta_mensual_escenario_uds:c.vtaMesEsc,
        Stock_actual:c.stock,Transito:c.transito,Cobertura_dias:c.cobDias>=999?"":c.cobDias,
        Horizonte_dias:c.horizProm,
        Compra_base_uds:c.compraBase,Compra_escenario_uds:c.compraEsc,Delta_uds:c.compraEsc-c.compraBase,
        Inversion_base_CLP:c.invBase,Inversion_escenario_CLP:c.invEsc,Delta_CLP:c.invEsc-c.invBase})))
      const wsDetalle=XLSX.utils.json_to_sheet(skuF.filter(p=>p.compraBase>0||p.compraEsc>0).sort((a,b)=>b.invEsc-a.invEsc).map(p=>({
        SKU:p.sku,Producto:p.producto,Categoria:p.tipo_producto,ABCD:p.clasif_abcd,
        Vta_prom_mes:p.vta_prom_compensada||0,Uplift_pct:Math.round(p.up),
        Stock:p.stock_actual||0,Transito:p.stock_transito||0,
        Dias_fabricacion:p.dias_fabricacion,Dias_emergencia:p.dias_emergencia,Periodo_cobertura:p.periodo_cubrir,
        Compra_base_uds:p.compraBase,Compra_escenario_uds:p.compraEsc,
        Costo_unit_CLP:p.costo_unitario||0,Inversion_base_CLP:p.invBase,Inversion_escenario_CLP:p.invEsc})))
  ...
  const FCOLS=[
    {k:"tipo",l:"Categoría",a:"left"},{k:"skus",l:"SKUs",a:"right"},{k:"up",l:"Uplift %",a:"center"},
    {k:"vtaMesBase",l:"Vta/mes base",a:"right"},{k:"vtaMesEsc",l:"Vta/mes esc.",a:"right"},
    {k:"stock",l:"Stock",a:"right"},{k:"transito",l:"Tráns.",a:"right"},{k:"cobDias",l:"Cob. (d)",a:"right"},
    {k:"horizProm",l:"Horiz. (d)",a:"right"},
    {k:"compraBase",l:"Compra base",a:"right"},{k:"compraEsc",l:"Compra esc.",a:"right"},
    {k:"invBase",l:"Inv. base",a:"right"},{k:"invEsc",l:"Inv. escenario",a:"right"},{k:"delta",l:"Δ inv.",a:"right"}
  ]
  ...
  return<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",flexWrap:"wrap",gap:10,marginBottom:14}}>
      <div>
        ...
        <div style={{fontSize:12,color:"#8E8E93",marginTop:2}}>Proyección por categoría con parámetros vigentes · escenario base (venta actual) vs escenario con aumento de venta</div>
      </div>
    ...
    <div style={{background:"#F0F9FF",border:"1px solid #007AFF30",borderRadius:12,padding:"12px 16px",marginBottom:14,display:"flex",alignItems:"center",gap:16,flexWrap:"wrap"}}>
      ...
      <div style={{fontSize:11,color:"#8E8E93"}}>Aplica a todas las categorías · editable por categoría en la columna "Uplift %" de la tabla</div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:8,marginBottom:14}}>
      ...
      <Mt l="Venta mensual base → esc." v={`${fN(T.vB)} → ${fN(T.vE)}`} ic="🛒"/>
    </div>
    ...
    <div style={{background:"#fff",borderRadius:12,border:"1px solid #E5E5EA",overflow:"hidden",marginBottom:14}}>
      <div style={{overflowX:"auto"}}>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:1250}}>
          <thead><tr style={{background:"#F2F2F7"}}>
            {FCOLS.map(c=><th key={c.k} onClick={()=>setSortF(s=>({k:c.k==="delta"?"invEsc":c.k,dir:s.k===(c.k==="delta"?"invEsc":c.k)?-s.dir:-1}))} style={{padding:"8px 10px",textAlign:c.a,fontSize:10,fontWeight:700,color:sortF.k===c.k?"#007AFF":"#8E8E93",borderBottom:"2px solid #E5E5EA",whiteSpace:"nowrap",cursor:"pointer",userSelect:"none"}}>{c.l}{sortF.k===c.k?(sortF.dir>0?" ↑":" ↓"):""}</th>)}
          </tr></thead>
          <tbody>{catsSorted.map((c,i)=>{
            const delta=c.invEsc-c.invBase
            const critico=c.cobDias<30&&c.vtaMesBase>0
            return<tr key={c.tipo} style={{borderBottom:"1px solid #F2F2F7",background:critico?"#FFF8F0":i%2===0?"#fff":"#FAFAFA"}}>
              <td style={{padding:"7px 10px",fontWeight:700,whiteSpace:"nowrap"}}>{critico?"⚠ ":""}{c.tipo}</td>
              <td style={{padding:"7px 10px",textAlign:"right"}}>{c.skus}</td>
              ...
              <td style={{padding:"7px 10px",textAlign:"right"}}>{fN(c.vtaMesBase)}</td>
              <td style={{padding:"7px 10px",textAlign:"right",fontWeight:600,color:"#007AFF"}}>{fN(c.vtaMesEsc)}</td>
              <td style={{padding:"7px 10px",textAlign:"right"}}>{fN(c.stock)}</td>
              <td style={{padding:"7px 10px",textAlign:"right",color:"#8E8E93"}}>{c.transito>0?fN(c.transito):"—"}</td>
              <td style={{padding:"7px 10px",textAlign:"right",fontWeight:600,color:critico?"#FF9500":c.cobDias>=180?"#5856D6":"#3A3A3C"}}>{c.cobDias>=999?"—":c.cobDias}</td>
              <td style={{padding:"7px 10px",textAlign:"right",color:"#8E8E93"}}>{c.horizProm}</td>
              <td style={{padding:"7px 10px",textAlign:"right"}}>{c.compraBase>0?fN(c.compraBase):"—"}</td>
              <td style={{padding:"7px 10px",textAlign:"right",fontWeight:700,color:"#007AFF"}}>{c.compraEsc>0?fN(c.compraEsc):"—"}</td>
              <td style={{padding:"7px 10px",textAlign:"right"}}>{c.invBase>0?fmt(c.invBase):"—"}</td>
              <td style={{padding:"7px 10px",textAlign:"right",fontWeight:700,color:"#007AFF"}}>{c.invEsc>0?fmt(c.invEsc):"—"}</td>
              <td style={{padding:"7px 10px",textAlign:"right",fontWeight:600,color:delta>0?"#FF9500":"#8E8E93"}}>{delta!==0?fmt(delta):"—"}</td>
            </tr>
          })}</tbody>
          <tfoot><tr style={{background:"#1a1a2e"}}>
            <td colSpan={9} style={{padding:"8px 10px",color:"#fff",fontWeight:700,fontSize:11}}>TOTAL ({cats.filter(c=>c.compraEsc>0).length} categorías con compra)</td>
            ...
            <td style={{padding:"8px 10px",textAlign:"right",color:"#fff",fontWeight:700}}>{fmt(T.iB)}</td>
      ...
      <div style={{padding:"8px 14px",borderTop:"1px solid #F2F2F7",fontSize:10.5,color:"#8E8E93"}}>
        Fórmula: compra = demanda diaria × (período de cobertura + días fabricación + días emergencia) − stock − tránsito · Demanda base = venta promedio compensada (excluye quiebres) · ⚠ = cobertura crítica &lt;30 días · Uplift por categoría: escribí un % para pisar el global (vacío = usa global)
      </div>
    </div>
