...
function OCDetView({oc,firmas,pagos,updOC,addFirma,setPagos,close,cu,h}){
  ...
  // Upload document to Supabase Storage + register in documentos_import
  const uploadDoc=async(file,tipo,descripcion)=>{
    if(!file)return;setUploading(true)
    try{
      const ext=file.name.split('.').pop()
      const path=`${oc.id}/${tipo}/${Date.now()}.${ext}`
      // Try storage upload first
      const{data:upData,error:upErr}=await supabase.storage.from('documentos-oc').upload(path,file,{cacheControl:'3600',upsert:false})
      let url=""
      if(upErr){
        // If storage bucket doesn't exist, store as base64 in DB
        const reader=new FileReader()
        url=await new Promise(res=>{reader.onload=e=>res(e.target.result);reader.readAsDataURL(file)})
      }else{
        const{data:urlData}=supabase.storage.from('documentos-oc').getPublicUrl(path)
        url=urlData?.publicUrl||path
      }
      const doc={id:uid(),oc_id:oc.id,tipo_documento:tipo,nombre_archivo:file.name,url_archivo:url,descripcion:descripcion||tipo,subido_por:cu.nombre,fecha_subida:hoy(),hora_subida:hora()}
      await supabase.from('documentos_import').insert(doc)
      setDocs(p=>[doc,...p])
      await addFirma(oc.id,`Documento adjunto: ${tipo} — ${file.name}`)
    }catch(e){console.error('Upload error:',e)}
    setUploading(false)
  }

  // File input component
  const FileUpload=({tipo,label,desc})=><div style={{background:"#F2F2F7",borderRadius:10,padding:12,marginBottom:8}}>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      <div><div style={{fontSize:13,fontWeight:600,color:"#1C1C1E"}}>{label}</div><div style={{fontSize:11,color:"#8E8E93"}}>{desc}</div></div>
      <label style={{padding:"6px 12px",borderRadius:8,fontSize:12,fontWeight:600,background:"#007AFF",color:"#fff",cursor:"pointer",display:"inline-flex",alignItems:"center",gap:4}}>
        📎 Adjuntar
    ...
    {docs.filter(d=>d.tipo_documento===tipo).map(d=><div key={d.id} style={{display:"flex",alignItems:"center",gap:8,marginTop:8,padding:8,background:"#fff",borderRadius:8,border:"1px solid #E5E5EA"}}>
      <span style={{fontSize:16}}>{d.nombre_archivo?.endsWith('.pdf')?"📄":"🖼"}</span>
      <div style={{flex:1}}><div style={{fontSize:12,fontWeight:600,color:"#1C1C1E"}}>{d.nombre_archivo}</div><div style={{fontSize:10,color:"#8E8E93"}}>{d.subido_por} — {d.fecha_subida} {d.hora_subida}</div></div>
      {d.url_archivo&&<a href={d.url_archivo} target="_blank" rel="noopener noreferrer" style={{padding:"4px 8px",borderRadius:6,fontSize:10,fontWeight:600,background:"#007AFF15",color:"#007AFF",textDecoration:"none"}}>Ver</a>}
    </div>)}
  </div>
