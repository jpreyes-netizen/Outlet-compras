...
// ⭐ Helper para cargar tablas con paginación — Supabase limita a 1000 filas por default.
// Uso: await fetchAll(supabase.from('productos').select('*').order('x'))
// Hace requests en chunks de 1000 hasta traer todo.
const fetchAll=async(query)=>{
  ...
  while(true){
    ...
    from+=CHUNK
    if(from>100000){console.warn("[fetchAll] límite de seguridad 100k filas alcanzado");break}
  }
...
function ToastBanner(){
  ...
  const styles={
    ...
    success:{bg:"#34C759",bgLight:"#34C75908",border:"#34C759",ic:"✓",c:"#fff"}
  }
...
export default function App(){
  ...
  const loadAll=async()=>{
    try{
      const[ru,rp,rprod,rprodSuc,rsuc,roc,rf,rpag,rpt,rpa,rcfg]=await Promise.all([
        ...
        fetchAll(supabase.from('productos_sucursal').select('*').order('costo_reposicion',{ascending:false})),
  ...
  const addFirma=async(ocId,accion)=>{
    if(!cu)return
    const f={id:uid(),oc_id:ocId,usuario_id:cu.id,nombre_usuario:cu.nombre,rol_usuario:rl(cu).l,accion,firma_digital:cu.firma_digital,fecha:hoy(),hora:hora()}
    const r=await tryDb(()=>supabase.from('firmas').insert(f),"No se pudo registrar la firma")
    if(!r.error)setFirmas(p=>[...p,f])
  }
    ...
    <div style={{position:"sticky",top:0,zIndex:50,background:"rgba(242,242,247,0.9)",backdropFilter:"blur(20px)",padding:"14px 0 10px",marginBottom:10,borderBottom:"1px solid rgba(0,0,0,0.06)"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        ...
        <div style={{display:"flex",alignItems:"center",gap:8}}>
          ...
          <a href="/Manual_ERP_Compras_v1.pdf" target="_blank" rel="noopener" style={{width:32,height:32,borderRadius:10,background:"#007AFF15",border:"none",cursor:"pointer",fontSize:13,color:"#007AFF",display:"flex",alignItems:"center",justifyContent:"center",textDecoration:"none"}} title="Manual de Usuario (abre en nueva pestaña)">📖</a>
...
function SolView({cart,setCart,provs,users,cu,setOcs,addFirma,goOC,ocs,sucursales=[]}){
  ...
  // ⭐ Editar costo unitario del item — guarda costo_original para detectar modificaciones
  const setCosto=(sku,v)=>setCart(p=>{
    const it=p[sku];if(!it)return p
    const nuevoCosto=Math.max(0,Number(v)||0)
    // Guardar el costo original la primera vez que se modifica
  ...
  const submit=async()=>{
    ...
    // ⭐ Detectar costos modificados y preguntar si actualizar productos.costo_unitario
    ...
    if(costosModificados.length>0){
      const lista=costosModificados.slice(0,8).map(i=>`• ${i.producto} (${i.sku}): ${fmt(i.costo_original||0)} → ${fmt(i.costo_unitario)}`).join("\n")
      actualizarCostosBD=window.confirm(`✏ Modificaste el costo unitario de ${costosModificados.length} producto(s).\n\n${lista}${costosModificados.length>8?`\n...y ${costosModificados.length-8} más`:""}\n\n¿Querés ACTUALIZAR EL COSTO en la base de datos para que las próximas OCs lo usen?\n\n• OK → actualiza el costo del SKU en Productos (afecta a futuras OCs)\n• Cancelar → solo aplica el nuevo costo a esta OC`)
    }
    setSaving(true)
    ...
    // ⭐ Si el usuario confirmó, actualizar costo_unitario de los productos modificados en la base
    if(actualizarCostosBD&&costosModificados.length>0){
      let okCount=0
      for(const item of costosModificados){
        const r=await tryDb(()=>supabase.from('productos').update({costo_unitario:item.costo_unitario}).eq('sku',item.sku),`No se pudo actualizar costo de ${item.sku}`)
        if(!r.error){okCount++;await auditLog(cu,'productos','UPDATE_COSTO',item.sku,{old:item.costo_original,new:item.costo_unitario,oc_origen:id})}
      }
      if(okCount>0)toast.success(`${okCount} costo(s) actualizado(s) en Productos`,"Las próximas OCs usarán los nuevos valores")
    }
              ...
              <tr style={{borderBottom:expanded?"none":"1px solid #F2F2F7"}}>
                ...
                <td style={{padding:"8px",textAlign:"right"}}>
                  <input
                    ...
                    min={0}
                    step="any"
                    value={i.costo_unitario||0}
                    onChange={e=>setCosto(i.sku,e.target.value)}
                    style={{...css.input,padding:"5px 8px",fontSize:12,width:90,textAlign:"right",borderColor:i.costo_modificado?"#FF9500":((i.costo_unitario||0)===0?"#FF3B3060":"#E5E5EA"),background:i.costo_modificado?"#FFF8EC":((i.costo_unitario||0)===0?"#FFEEEE":"#fff")}}
                    title={i.costo_modificado?`Costo modificado (original: ${fmt(i.costo_original)})`:((i.costo_unitario||0)===0?"⚠ Costo en $0 — editá para asignar":"Costo unitario CLP")}
                  />
...
function OCDetView({oc,firmas,pagos,updOC,addFirma,setPagos,close,cu,h,provs=[],users=[],sucursales=[]}){
  ...
  // Permitido en todos los estados ANTERIORES a recepción/cierre/rechazo
  const estadosBloqueados=["Recibida OK","Recibida parcial","Cerrada","Rechazada"]
  // Permitido: roles con cualquier permiso de aprobación (admin, dir_negocios, dir_finanzas, analista, dir_general)
  ...
  const[editingItem,setEditingItem]=useState(null) // id del item en edición
  const[editDraft,setEditDraft]=useState({}) // borrador de cambios
  ...
  const startEditItem=(it)=>{
    ...
    setEditDraft({producto:it.producto||"",cantidad_pedida:it.cantidad_pedida||0,costo_unitario:it.costo_unitario||0})
  }
  ...
  const saveItemEdit=async(it)=>{
    ...
    const r=await tryDb(()=>supabase.from('oc_items').update(changes).eq('id',it.id),"No se pudo actualizar el item")
    if(r.error){setSavingEdit(false);return}

    // Actualizar estado local
    const newItems=items.map(x=>x.id===it.id?{...x,...changes}:x)
    setItems(newItems)

    // Audit log
    await auditLog(cu,'oc_items','UPDATE',it.id,{sku:it.sku,changes,before:{producto:it.producto,cantidad_pedida:it.cantidad_pedida,costo_unitario:it.costo_unitario}},oc.id)

    // Recalcular total_clp de la OC
    const nuevoTotal=newItems.reduce((s,x)=>s+((x.cantidad_pedida||0)*(x.costo_unitario||0)),0)
    await updOC(oc.id,{total_clp:nuevoTotal,total_usd:isI?Math.round(nuevoTotal/950):(oc.total_usd||0)})

    // Firma del cambio
    const resumen=Object.entries(changes).map(([k,v])=>`${k}:${typeof v==='number'?fN(v):v}`).join(", ")
    await addFirma(oc.id,`Item editado: ${it.sku} (${resumen})`)

    // Notificar a aprobadores originales
    await notifyEditToAprobadores(`Item modificado en ${oc.id}`,`Se modificó el producto <strong>${it.producto}</strong> (SKU: ${it.sku}).<br/>Cambios: ${resumen}`)

    toast.success("Item actualizado","Total de la OC recalculado")
    setEditingItem(null);setEditDraft({});setSavingEdit(false)
  }

  const deleteItem=async(it)=>{
    if(!window.confirm(`¿Eliminar "${it.producto}" (${it.sku}) de la OC?\n\nEsta acción se puede auditar pero no es reversible desde la interfaz.`))return
    setSavingEdit(true)
    // Borrar también asignaciones por sucursal si existen
    await supabase.from('oc_items_sucursal').delete().eq('oc_item_id',it.id)
    const r=await tryDb(()=>supabase.from('oc_items').delete().eq('id',it.id),"No se pudo eliminar el item")
    if(r.error){setSavingEdit(false);return}

    const newItems=items.filter(x=>x.id!==it.id)
    setItems(newItems)
    setItemsSuc(itemsSuc.filter(a=>a.oc_item_id!==it.id))

    await auditLog(cu,'oc_items','DELETE',it.id,{sku:it.sku,producto:it.producto,cantidad_pedida:it.cantidad_pedida,costo_unitario:it.costo_unitario},oc.id)

    const nuevoTotal=newItems.reduce((s,x)=>s+((x.cantidad_pedida||0)*(x.costo_unitario||0)),0)
    await updOC(oc.id,{total_clp:nuevoTotal,total_usd:isI?Math.round(nuevoTotal/950):(oc.total_usd||0)})

    await addFirma(oc.id,`Item eliminado: ${it.sku} — ${it.producto}`)
    await notifyEditToAprobadores(`Item eliminado de ${oc.id}`,`Se eliminó el producto <strong>${it.producto}</strong> (SKU: ${it.sku}) de la OC.`)
  ...
  const saveNewItem=async()=>{
    if(!newItem.producto||!newItem.sku){toast.error("Faltan datos","Producto y SKU son obligatorios");return}
    if((newItem.cantidad_pedida||0)<=0){toast.error("Cantidad inválida","Debe ser mayor a 0");return}
    setSavingEdit(true)
    const id=uid()
    const row={id,oc_id:oc.id,sku:newItem.sku,producto:newItem.producto,cantidad_sugerida:0,cantidad_pedida:Number(newItem.cantidad_pedida),costo_unitario:Number(newItem.costo_unitario||0)}
    const r=await tryDb(()=>supabase.from('oc_items').insert(row),"No se pudo agregar el item")
    if(r.error){setSavingEdit(false);return}

    const newItems=[...items,row]
    setItems(newItems)

    await auditLog(cu,'oc_items','INSERT',id,{...row},oc.id)

    const nuevoTotal=newItems.reduce((s,x)=>s+((x.cantidad_pedida||0)*(x.costo_unitario||0)),0)
    await updOC(oc.id,{total_clp:nuevoTotal,total_usd:isI?Math.round(nuevoTotal/950):(oc.total_usd||0)})

    await addFirma(oc.id,`Item agregado: ${newItem.sku} — ${newItem.producto} (${fN(newItem.cantidad_pedida)} × ${fmt(newItem.costo_unitario)})`)
    await notifyEditToAprobadores(`Item agregado a ${oc.id}`,`Se agregó el producto <strong>${newItem.producto}</strong> (SKU: ${newItem.sku}).<br/>Cantidad: ${fN(newItem.cantidad_pedida)} · Costo: ${fmt(newItem.costo_unitario)}`)

    toast.success("Item agregado")
    setNewItem({producto:"",sku:"",cantidad_pedida:0,costo_unitario:0})
    setAddingItem(false);setSavingEdit(false)
  }

  // Notificar a aprobadores (dir_negocios + dir_finanzas + admin) sobre cambios
  const notifyEditToAprobadores=async(titulo,mensaje)=>{
    const rolesAprobadores=["dir_negocios","dir_finanzas","admin","dir_general"]
    const emails=new Set()
    for(const r of rolesAprobadores){
    ...
    const prov=provs?.find(p=>p.id===oc.proveedor_id)
    await sendEmail({to,titulo,mensaje,ocId:oc.id,tipoOc:oc.tipo_oc,proveedor:prov?.nombre,usuarioOrigen:`${cu.nombre} (${rl(cu).l})`,cta:"Ver OC actualizada"})
  }
  ...
  return<div>
    ...
    {(items.length>0||addingItem)&&<><div style={{fontSize:15,fontWeight:700,marginBottom:6,display:"flex",justifyContent:"space-between",alignItems:"center"}}>
      ...
      <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,marginBottom:14}}>
        <thead><tr style={{background:"#F2F2F7"}}>
          ...
          {puedeEditarItems&&<th style={{width:90,padding:"6px",fontSize:10,fontWeight:600,color:"#8E8E93"}}></th>}
        </tr></thead>
        <tbody>{items.map((i,idx)=>{
          const asigs=itemsSuc.filter(a=>a.oc_item_id===i.id)
          const isEditing=editingItem===i.id
          if(isEditing){
            return<tr key={idx} style={{borderBottom:"1px solid #F2F2F7",background:"#FFF9E8"}}>
              <td style={{padding:"6px 8px"}}>
                <input value={editDraft.producto||""} onChange={e=>setEditDraft({...editDraft,producto:e.target.value})} style={{...css.input,padding:"5px 8px",fontSize:12,width:"100%"}} placeholder="Nombre del producto"/>
                <div style={{fontSize:10,color:"#AEAEB2",marginTop:3}}>{i.sku}</div>
              </td>
              <td style={{padding:"6px",textAlign:"right",color:"#8E8E93"}}>{fN(i.cantidad_sugerida)}</td>
              <td style={{padding:"6px"}}>
                <input type="number" min={0} value={editDraft.cantidad_pedida||0} onChange={e=>setEditDraft({...editDraft,cantidad_pedida:e.target.value})} style={{...css.input,padding:"5px 8px",fontSize:12,width:80,textAlign:"right"}}/>
              </td>
              <td style={{padding:"6px"}}>
                <input type="number" min={0} step="any" value={editDraft.costo_unitario||0} onChange={e=>setEditDraft({...editDraft,costo_unitario:e.target.value})} style={{...css.input,padding:"5px 8px",fontSize:12,width:90,textAlign:"right"}}/>
              </td>
              ...
              <td style={{padding:"6px",textAlign:"center"}}>
                <div style={{display:"flex",gap:4,justifyContent:"center"}}>
                  <button onClick={()=>saveItemEdit(i)} disabled={savingEdit} style={{padding:"4px 8px",fontSize:11,background:"#34C759",color:"#fff",border:"none",borderRadius:4,cursor:"pointer",fontWeight:600}}>{savingEdit?"...":"✓"}</button>
                  <button onClick={cancelEdit} disabled={savingEdit} style={{padding:"4px 8px",fontSize:11,background:"#8E8E93",color:"#fff",border:"none",borderRadius:4,cursor:"pointer"}}>✕</button>
                </div>
              </td>
            </tr>
          }
          return<tr key={idx} style={{borderBottom:"1px solid #F2F2F7"}}>
            ...
            {puedeEditarItems&&<td style={{padding:"6px",textAlign:"center"}}>
              <div style={{display:"flex",gap:4,justifyContent:"center"}}>
                <button onClick={()=>startEditItem(i)} disabled={!!editingItem||addingItem} style={{padding:"4px 8px",fontSize:11,background:"#007AFF15",color:"#007AFF",border:"1px solid #007AFF30",borderRadius:4,cursor:editingItem||addingItem?"not-allowed":"pointer",opacity:editingItem||addingItem?0.4:1,fontWeight:600}} title="Editar item">✏</button>
                <button onClick={()=>deleteItem(i)} disabled={!!editingItem||addingItem} style={{padding:"4px 8px",fontSize:11,background:"#FF3B3015",color:"#FF3B30",border:"1px solid #FF3B3030",borderRadius:4,cursor:editingItem||addingItem?"not-allowed":"pointer",opacity:editingItem||addingItem?0.4:1}} title="Eliminar item">🗑</button>
              </div>
            </td>}
          </tr>
        })}
        ...
        {addingItem&&<tr style={{borderBottom:"1px solid #F2F2F7",background:"#E8F5E9"}}>
          <td style={{padding:"6px 8px"}}>
            <input value={newItem.producto} onChange={e=>setNewItem({...newItem,producto:e.target.value})} placeholder="Nombre del producto" style={{...css.input,padding:"5px 8px",fontSize:12,width:"100%",marginBottom:3}}/>
            <input value={newItem.sku} onChange={e=>setNewItem({...newItem,sku:e.target.value})} placeholder="SKU" style={{...css.input,padding:"5px 8px",fontSize:11,width:"100%"}}/>
          </td>
          <td style={{padding:"6px",textAlign:"right",color:"#AEAEB2"}}>—</td>
          <td style={{padding:"6px"}}>
            <input type="number" min={0} value={newItem.cantidad_pedida||0} onChange={e=>setNewItem({...newItem,cantidad_pedida:Number(e.target.value)})} style={{...css.input,padding:"5px 8px",fontSize:12,width:80,textAlign:"right"}}/>
          </td>
          <td style={{padding:"6px"}}>
            <input type="number" min={0} step="any" value={newItem.costo_unitario||0} onChange={e=>setNewItem({...newItem,costo_unitario:Number(e.target.value)})} style={{...css.input,padding:"5px 8px",fontSize:12,width:90,textAlign:"right"}}/>
          </td>
          <td style={{padding:"6px 8px",textAlign:"right",fontWeight:600}}>{fmt((newItem.cantidad_pedida||0)*(newItem.costo_unitario||0))}</td>
          <td style={{padding:"6px",textAlign:"center"}}>
            <div style={{display:"flex",gap:4,justifyContent:"center"}}>
              <button onClick={saveNewItem} disabled={savingEdit} style={{padding:"4px 8px",fontSize:11,background:"#34C759",color:"#fff",border:"none",borderRadius:4,cursor:"pointer",fontWeight:600}}>{savingEdit?"...":"✓"}</button>
              <button onClick={()=>{setAddingItem(false);setNewItem({producto:"",sku:"",cantidad_pedida:0,costo_unitario:0})}} disabled={savingEdit} style={{padding:"4px 8px",fontSize:11,background:"#8E8E93",color:"#fff",border:"none",borderRadius:4,cursor:"pointer"}}>✕</button>
            </div>
          </td>
        </tr>}
        </tbody>
      </table>
      {puedeEditarItems&&!estadosBloqueados.includes(oc.estado)&&<div style={{fontSize:10,color:"#8E8E93",marginBottom:14,display:"flex",gap:6,alignItems:"center"}}>
        <span>💡</span><span>Podés editar/agregar/eliminar items en esta etapa. Cada cambio se audita y notifica a los aprobadores (Dir. Negocios, Dir. Finanzas, Admin).</span>
      </div>}
      {!puedeEditarItems&&tienePermisoEdicion&&<div style={{fontSize:10,color:"#FF9500",marginBottom:14,display:"flex",gap:6,alignItems:"center"}}>
        <span>🔒</span><span>La OC está en estado <strong>{oc.estado}</strong>. Ya no se pueden editar items.</span>
      </div>}</>}
...
function CalendarioView({prods,ocs,sucActiva="all",sucursales=[]}){
  ...
  useEffect(()=>{
    const load=async()=>{if(ocsActivas.length===0){setOcItems([]);return};const ids=ocsActivas.map(o=>o.id);const{data}=await fetchAll(supabase.from('oc_items').select('*').in('oc_id',ids));setOcItems(data||[])}
    load()
  },[ocs])
...
function TransitoView({ocs,provs}){
  ...
  useEffect(()=>{
    const load=async()=>{if(activas.length===0){setItems([]);return};const ids=activas.map(o=>o.id);const{data}=await fetchAll(supabase.from('oc_items').select('*').in('oc_id',ids));setItems(data||[])}
    load()
  },[ocs])
...
function ConfigView({config,saveConfig,params,setParams,paramsABCD,setParamsABCD,provs,setProvs,users,setUsers,h,configTab,setConfigTab,loadAll,cu}){
  ...
  const recalcular=async()=>{
    ...
    try{
      const{data:prodList,error}=await fetchAll(supabase.from('productos').select('*'))
