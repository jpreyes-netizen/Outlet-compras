...
/* ═══ COBERTURA — Calendario Gantt de cobertura de stock (v48) ═══ */
function CoberturaView({prods=[],ocs=[],config={}}){
  const[nivel,setNivel]=useState("abcd") // abcd | tipo | sku
  const[metodo,setMetodo]=useState("compensada") // compensada | simple
  const[horizonte,setHorizonte]=useState(6) // meses
  const[abcdSel,setAbcdSel]=useState({A:true,B:true,C:true,D:false})
  const[tipoSel,setTipoSel]=useState("")
  const[qSku,setQSku]=useState("")
  const[expanded,setExpanded]=useState({})

  // Filtro base: solo productos con stock o venta
  const prodsBase=useMemo(()=>{
  ...
  // Calcular venta diaria según método elegido
  const ventaDiaria=(p)=>{
    if(metodo==="compensada")return p.vta_prom_diaria||0
    // simple: promedio mensual simple / 30
    const meses=[p.venta_mes_1,p.venta_mes_2,p.venta_mes_3,p.venta_mes_4].filter(v=>v!==null&&v!==undefined)
    const prom=meses.length>0?meses.reduce((s,v)=>s+Number(v||0),0)/meses.length:0
    return prom/30
  }

  // Calcular días de cobertura proyectados por producto
  const calcDias=(p)=>{
    const disponible=(p.stock_actual||0)+(p.stock_transito||0)
    const vd=ventaDiaria(p)
    if(vd<=0)return 999 // sin venta
    return Math.floor(disponible/vd)
  }

  // Generar array de meses futuros (empezando desde el mes actual)
  ...
  for(let i=0;i<horizonte;i++){
    ...
    meses.push({
      ...
      year:d.getFullYear(),
      month:d.getMonth(),
      startDay:Math.floor((d-new Date(hoyDate.getFullYear(),hoyDate.getMonth(),1))/(86400000))
    })
  }
  const diasTotales=horizonte*30

  // Agregar filas según nivel
  const filas=useMemo(()=>{
    if(nivel==="sku"){
      return prodsBase.map(p=>({
        key:p.sku||p.producto,
        label:p.producto,
        sublabel:`${p.sku||""} · ${p.tipo_producto||""} · Clase ${p.clasif_abcd||"-"}`,
        abcd:p.clasif_abcd,
        dias:calcDias(p),
        stock:p.stock_actual||0,
        transito:p.stock_transito||0,
        ventaDia:ventaDiaria(p),
        esAgrupado:false
      })).sort((a,b)=>a.dias-b.dias)
    }
    // Agrupamiento (abcd o tipo)
    const key=nivel==="abcd"?"clasif_abcd":"tipo_producto"
    const groups={}
    prodsBase.forEach(p=>{
    ...
    return Object.entries(groups).map(([k,g])=>{
      // Agregado: suma de stock, suma de venta diaria
      const stockTotal=g.prods.reduce((s,p)=>s+(p.stock_actual||0),0)
      const transitoTotal=g.prods.reduce((s,p)=>s+(p.stock_transito||0),0)
      const vdTotal=g.prods.reduce((s,p)=>s+ventaDiaria(p),0)
      const diasAgg=vdTotal>0?Math.floor((stockTotal+transitoTotal)/vdTotal):999
      return{
        key:k,
        label:nivel==="abcd"?`Clase ${k}`:k,
        sublabel:`${g.prods.length} SKU · Stock ${fN(stockTotal)}${transitoTotal>0?` · Tránsito ${fN(transitoTotal)}`:""}`,
        abcd:nivel==="abcd"?k:null,
        dias:diasAgg,
        stock:stockTotal,
      ...
      // Orden por ABCD luego por días cobertura
      if(nivel==="abcd"){const o={A:0,B:1,C:2,D:3};return(o[a.key]??9)-(o[b.key]??9)}
      return a.dias-b.dias
    })
  },[prodsBase,nivel,metodo])

  // Tipos únicos para el dropdown
  const tiposUnicos=[...new Set(prods.map(p=>p.tipo_producto).filter(Boolean))].sort()

  // Convertir días de cobertura a ancho de barra en %
  const dayToPct=(d)=>Math.min(100,Math.max(0,(d/diasTotales)*100))

  // Color según ABCD y urgencia
  const colorBarra=(fila)=>{
    if(fila.dias>=diasTotales)return{bg:"#34C759",lbl:"#fff"} // verde: cobertura total
    if(fila.dias>=60)return{bg:"#30B0C7",lbl:"#fff"}
    if(fila.dias>=30)return{bg:"#FF9500",lbl:"#fff"}
    if(fila.dias>=7)return{bg:"#FF3B30",lbl:"#fff"}
    return{bg:"#8E8E93",lbl:"#fff"}
  }

  // Ancho de la parte en tránsito dentro de la barra
  const pctTransito=(fila)=>{
    if(fila.ventaDia<=0||fila.transito<=0)return 0
    const diasTrans=fila.transito/fila.ventaDia
    return Math.min(100,(diasTrans/diasTotales)*100)
  }
  ...
  return<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"baseline",marginBottom:12,flexWrap:"wrap",gap:10}}>
      <div>
        ...
        <div style={{fontSize:12,color:"#8E8E93"}}>Vista tipo Gantt de hasta cuándo alcanza el stock de cada {nivel==="abcd"?"clase ABCD":nivel==="tipo"?"tipo de producto":"SKU"}. Proyección en base a venta promedio {metodo==="compensada"?"compensada":"simple"} y stock actual + tránsito.</div>
      </div>
    </div>
    ...
    <Cd s={{marginBottom:12}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(180px,1fr))",gap:10}}>
        ...
        <div>
          <div style={{fontSize:10,fontWeight:700,color:"#8E8E93",textTransform:"uppercase",marginBottom:4}}>Método de cálculo</div>
          <select value={metodo} onChange={e=>setMetodo(e.target.value)} style={css.select}>
            <option value="compensada">Venta compensada (excluye quiebres)</option>
            <option value="simple">Venta simple (promedio 4 meses)</option>
          </select>
        </div>
        <div>
        ...
        <div>
          <div style={{fontSize:10,fontWeight:700,color:"#8E8E93",textTransform:"uppercase",marginBottom:4}}>Tipo (opcional)</div>
    ...
    {/* LEYENDA */}
    <div style={{display:"flex",gap:10,flexWrap:"wrap",fontSize:11,marginBottom:10,padding:"8px 12px",background:"#fff",borderRadius:8}}>
      <span style={{display:"inline-flex",alignItems:"center",gap:4}}><span style={{width:12,height:12,background:"#34C759",borderRadius:3}}/>Cobertura total</span>
      <span style={{display:"inline-flex",alignItems:"center",gap:4}}><span style={{width:12,height:12,background:"#30B0C7",borderRadius:3}}/>≥60d</span>
      <span style={{display:"inline-flex",alignItems:"center",gap:4}}><span style={{width:12,height:12,background:"#FF9500",borderRadius:3}}/>30-60d</span>
      <span style={{display:"inline-flex",alignItems:"center",gap:4}}><span style={{width:12,height:12,background:"#FF3B30",borderRadius:3}}/>7-30d (crítico)</span>
      <span style={{display:"inline-flex",alignItems:"center",gap:4}}><span style={{width:12,height:12,background:"#8E8E93",borderRadius:3}}/>&lt;7d (quiebre)</span>
      <span style={{display:"inline-flex",alignItems:"center",gap:4,marginLeft:"auto"}}><span style={{width:12,height:12,background:"repeating-linear-gradient(45deg,#007AFF,#007AFF 3px,#007AFF80 3px,#007AFF80 6px)",borderRadius:3}}/>Porción en tránsito</span>
    </div>

    {/* GANTT */}
    <Cd>
      {filas.length===0?<div style={{padding:30,textAlign:"center",color:"#8E8E93"}}>No hay productos que cumplan los filtros seleccionados.</div>:
      <div style={{overflow:"auto"}}>
        <div style={{minWidth:800}}>
          {/* Header de meses */}
          <div style={{display:"grid",gridTemplateColumns:"260px 1fr 70px",alignItems:"center",borderBottom:"2px solid #E5E5EA",paddingBottom:6,marginBottom:6}}>
            <div style={{fontSize:10,fontWeight:700,color:"#8E8E93",textTransform:"uppercase"}}>Ítem</div>
            <div style={{display:"grid",gridTemplateColumns:`repeat(${horizonte},1fr)`,gap:1}}>
              {meses.map((m,i)=><div key={i} style={{fontSize:10,fontWeight:700,color:"#8E8E93",textAlign:"center",borderLeft:i>0?"1px dashed #E5E5EA":"none"}}>{m.label} {String(m.year).slice(2)}</div>)}
            </div>
            <div style={{fontSize:10,fontWeight:700,color:"#8E8E93",textAlign:"right"}}>Días</div>
          </div>

          {/* Filas */}
          {filas.map(fila=>{
            const col=colorBarra(fila)
            const anchoBarra=dayToPct(fila.dias)
            const anchoTransito=pctTransito(fila)
            const abcdColor=fila.abcd?CL[fila.abcd]:null
            const estaExpandido=!!expanded[fila.key]
            return<div key={fila.key}>
              <div style={{display:"grid",gridTemplateColumns:"260px 1fr 70px",alignItems:"center",padding:"8px 0",borderBottom:"1px solid #F2F2F7",cursor:fila.esAgrupado?"pointer":"default"}} onClick={()=>{if(fila.esAgrupado)setExpanded(p=>({...p,[fila.key]:!p[fila.key]}))}}>
                <div style={{paddingRight:10,minWidth:0}}>
                  <div style={{display:"flex",alignItems:"center",gap:6}}>
                    {fila.esAgrupado&&<span style={{fontSize:10,color:"#8E8E93"}}>{estaExpandido?"▼":"▶"}</span>}
                  ...
                  <div style={{fontSize:10,color:"#AEAEB2",marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{fila.sublabel}</div>
                </div>
                <div style={{position:"relative",height:24,background:"#F2F2F7",borderRadius:4}}>
                  {/* Grid de meses al fondo */}
                  <div style={{position:"absolute",inset:0,display:"grid",gridTemplateColumns:`repeat(${horizonte},1fr)`}}>
                    {meses.map((_,i)=><div key={i} style={{borderLeft:i>0?"1px dashed #E5E5EA":"none"}}/>)}
                  </div>
                  {/* Barra principal (stock propio) */}
                  <div style={{position:"absolute",left:0,top:0,bottom:0,width:`${anchoBarra}%`,background:col.bg,borderRadius:4,transition:"width 0.3s",display:"flex",alignItems:"center",paddingLeft:6}}>
                    {anchoBarra>15&&<span style={{fontSize:10,fontWeight:700,color:col.lbl,whiteSpace:"nowrap"}}>{fila.dias>=999?"Sin venta":`${fila.dias}d`}</span>}
                  </div>
                  {/* Overlay de tránsito (hashtag pattern) */}
                  {anchoTransito>0&&<div style={{position:"absolute",right:`${100-anchoBarra}%`,top:0,bottom:0,width:`${anchoTransito}%`,background:"repeating-linear-gradient(45deg,transparent,transparent 4px,rgba(0,122,255,0.4) 4px,rgba(0,122,255,0.4) 8px)",borderRadius:"0 4px 4px 0",pointerEvents:"none"}}/>}
                </div>
                <div style={{textAlign:"right",fontSize:12,fontWeight:700,color:col.bg,paddingLeft:8}}>{fila.dias>=999?"∞":fila.dias+"d"}</div>
              </div>
              {/* Expansión del grupo: muestra SKUs individuales */}
              {fila.esAgrupado&&estaExpandido&&<div style={{paddingLeft:20,background:"#FAFAFA"}}>
                {fila.prods.slice(0,30).map(p=>{
                  const d=calcDias(p)
                  const cSub=colorBarra({dias:d})
                  const vd=ventaDiaria(p)
                  const anchoSub=dayToPct(d)
                  const anchoTransSub=vd>0&&p.stock_transito>0?Math.min(100,((p.stock_transito/vd)/diasTotales)*100):0
                  return<div key={p.sku||p.producto} style={{display:"grid",gridTemplateColumns:"260px 1fr 70px",alignItems:"center",padding:"5px 0",borderBottom:"1px solid #F2F2F7",fontSize:11}}>
                    <div style={{paddingRight:10,paddingLeft:18,minWidth:0}}>
                      <div style={{fontSize:12,fontWeight:500,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{p.producto}</div>
                      <div style={{fontSize:9,color:"#AEAEB2"}}>{p.sku} · Stock {fN(p.stock_actual||0)}{p.stock_transito>0?` · Tránsito ${fN(p.stock_transito)}`:""}</div>
                    </div>
                    <div style={{position:"relative",height:16,background:"#F2F2F7",borderRadius:3}}>
                      <div style={{position:"absolute",inset:0,display:"grid",gridTemplateColumns:`repeat(${horizonte},1fr)`}}>
                        {meses.map((_,i)=><div key={i} style={{borderLeft:i>0?"1px dashed #E5E5EA":"none"}}/>)}
                      </div>
                      <div style={{position:"absolute",left:0,top:0,bottom:0,width:`${anchoSub}%`,background:cSub.bg,borderRadius:3,opacity:0.85}}/>
                      {anchoTransSub>0&&<div style={{position:"absolute",right:`${100-anchoSub}%`,top:0,bottom:0,width:`${anchoTransSub}%`,background:"repeating-linear-gradient(45deg,transparent,transparent 3px,rgba(0,122,255,0.4) 3px,rgba(0,122,255,0.4) 6px)",borderRadius:"0 3px 3px 0"}}/>}
                    </div>
                    <div style={{textAlign:"right",fontSize:11,fontWeight:600,color:cSub.bg,paddingLeft:8}}>{d>=999?"∞":d+"d"}</div>
                  </div>
                })}
                {fila.prods.length>30&&<div style={{fontSize:10,color:"#8E8E93",textAlign:"center",padding:6}}>+ {fila.prods.length-30} SKU más (filtra por tipo o buscá uno específico)</div>}
              </div>}
            </div>
          })}
        </div>
    ...
    {/* RESUMEN */}
    <Cd s={{marginTop:12}}>
      <div style={{fontSize:13,fontWeight:700,marginBottom:8}}>📊 Resumen del horizonte ({horizonte} meses)</div>
      <div style={{display:"grid",gridTemplateColumns:"repeat(auto-fit,minmax(150px,1fr))",gap:10}}>
        <div style={{padding:10,background:"#F2F2F7",borderRadius:8}}>
        ...
        <div style={{padding:10,background:"#34C75910",borderRadius:8}}>
          <div style={{fontSize:10,color:"#34C759",fontWeight:600,textTransform:"uppercase"}}>Cubren horizonte</div>
          <div style={{fontSize:20,fontWeight:800,color:"#34C759"}}>{filas.filter(f=>f.dias>=diasTotales).length}</div>
        </div>
        <div style={{padding:10,background:"#FF950010",borderRadius:8}}>
          <div style={{fontSize:10,color:"#FF9500",fontWeight:600,textTransform:"uppercase"}}>Parcial (30-{horizonte*30}d)</div>
          <div style={{fontSize:20,fontWeight:800,color:"#FF9500"}}>{filas.filter(f=>f.dias<diasTotales&&f.dias>=30).length}</div>
        </div>
        <div style={{padding:10,background:"#FF3B3010",borderRadius:8}}>
          <div style={{fontSize:10,color:"#FF3B30",fontWeight:600,textTransform:"uppercase"}}>Crítico (&lt;30d)</div>
          <div style={{fontSize:20,fontWeight:800,color:"#FF3B30"}}>{filas.filter(f=>f.dias<30&&f.dias<999).length}</div>
        </div>
        <div style={{padding:10,background:"#8E8E9310",borderRadius:8}}>
          <div style={{fontSize:10,color:"#8E8E93",fontWeight:600,textTransform:"uppercase"}}>Sin venta</div>
          <div style={{fontSize:20,fontWeight:800,color:"#8E8E93"}}>{filas.filter(f=>f.dias>=999).length}</div>
        </div>
      </div>
    </Cd>
  </div>
...
function LlegadasView({ocs=[],provs=[],setDet,config={}}){
  ...
  const[filtroTipo,setFiltroTipo]=useState("todos") // todos | IMP | NAC
  const[filtroEstado,setFiltroEstado]=useState("todos") // todos | pendientes | recibidas
