...
La OC original ${oc.id} quedó en estado "Dividida".`)
  ...
  const guardarRecepcion=async()=>{
    ...
    if(riErr){alert("⚠ La recepción se creó pero falló el detalle de ítems: "+riErr.message)}
    ...
    // ── Puente Compras → Logística: las GUÍAS registradas aquí se espejan
    //    automáticamente en log_recepciones (visibles con su OC y proveedor).
    //    Las facturas no: son documentales y viven solo en Compras. ──
      ...
      try{
        ...
        if(!yaEsp){
          const{data:esp,error:eEsp}=await supabase.from('log_recepciones').insert({
            ...
            responsable:cu?.nombre||"ERP Compras", estado:"completado",
            cantidad_total:totalCant, creado_por:"espejo_compras", compras_recepcion_id:recId,
            notas:"[Origen: ERP Compras] Guía registrada en Compras (regularización); espejada a Logística."
          }).select('id').single()
          if(!eEsp&&esp)await supabase.from('recepciones').update({log_recepcion_id:esp.id}).eq('id',recId)
        }
