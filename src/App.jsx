...
const ROLES=[
  ...
  {k:"analista",l:"Analista",c:"#34C759",p:["crear_oc","ver_dash","cerrar_oc","gest_prov","config","seguim","gest_imp","recibir"]},
  ...
  return<div>
    ...
    {vistaOC==="tabla"&&(()=>{
      const TCOLS=[
        ...
        {k:"recepTxt",l:"Recepción",a:"center"},{k:"facturas",l:"Factura(s)",a:"left"},{k:"docs",l:"Documentos",a:"left"},
      ...
      const rows=fil.map(oc=>{
        ...
        return{...oc,tipo:oc.tipo_oc==="Importación"?"IMP":"NAC",prov_nombre:pv?.nombre||oc.proveedor_id||"",
          fase:`${oc.fase_actual||0}/${fases.length}`,pagoTxt,pgPag,pgTot,recepTxt,
          facs:facsOC.get(oc.id)||[],docsList:docsOC.get(oc.id)||[],firmasN:firmas.filter(fi=>fi.oc_id===oc.id).length}
      })
      const sval=(r,k)=>{
        if(k==="facturas")return r.facs.length
        if(k==="docs")return r.docsList.length
      ...
      const expX=()=>{
        try{
          const data=sorted.map(r=>({OC:r.id,Tipo:r.tipo,Estado:r.estado,Fase:r.fase,Proveedor:r.prov_nombre,Condicion_pago:r.condicion_pago||"",
            ...
            Estado_pago:r.pagoTxt,Recepcion:r.recepTxt,Facturas:r.facs.map(f=>`${f.folio} (${fmt(f.monto_total)})`).join(" · "),
            Documentos:r.docsList.map(dc=>dc.nombre_archivo||dc.tipo_documento).join(" · "),Firmas:r.firmasN}))
          ...
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:12,minWidth:1600}}>
            ...
            <tbody>{sorted.map((r,i)=>{
              return<tr key={r.id} style={{borderBottom:"1px solid #F2F2F7",background:i%2===0?"#fff":"#FAFAFA",cursor:"pointer"}} onClick={()=>{const oc=ocs.find(o=>o.id===r.id);if(oc)setDet(oc)}}>
                ...
                <td style={{padding:"6px 10px",fontSize:11,maxWidth:210}} onClick={e=>e.stopPropagation()}>{r.docsList.length>0?r.docsList.map((dc,di)=><div key={di} style={{whiteSpace:"nowrap",overflow:"hidden",textOverflow:"ellipsis",maxWidth:200}}><a onClick={(e)=>{e.preventDefault();abrirDoc(dc)}} href="#" style={{color:"#007AFF",textDecoration:"none"}} title={dc.nombre_archivo}>📎 {(dc.nombre_archivo||dc.tipo_documento||"doc").slice(0,26)}</a></div>):<span style={{color:"#C7C7CC"}}>—</span>}</td>
