...
function RepoView({prods,prodsSuc=[],cart,setCart,go,config,params,paramsABCD,sucursales=[]}){
  ...
  return<div>
    ...
    {sucursales.length>0&&<div style={{background:"#fff",borderRadius:12,padding:"4px",marginBottom:10,display:"flex",gap:2,boxShadow:"0 1px 2px rgba(0,0,0,0.03)",border:"1px solid #E5E5EA"}}>
      <button onClick={()=>{setFsuc("T");setFe("T")}} style={{flex:1,padding:"10px 14px",borderRadius:8,fontSize:12,fontWeight:700,border:"none",cursor:"pointer",background:fsuc==="T"?"#1C1C1E":"transparent",color:fsuc==="T"?"#fff":"#8E8E93",transition:"all 0.2s"}}>
        🏢 Todas las sucursales
      </button>
      {sucursales.filter(s=>s.activo!==false).map(s=>{
        const active=fsuc===s.id
        const color=s.es_cd?"#5AC8FA":"#007AFF"
        return<button key={s.id} onClick={()=>{setFsuc(s.id);setFe("T")}} style={{flex:1,padding:"10px 14px",borderRadius:8,fontSize:12,fontWeight:700,border:"none",cursor:"pointer",background:active?color:"transparent",color:active?"#fff":"#8E8E93",transition:"all 0.2s",display:"flex",alignItems:"center",justifyContent:"center",gap:4}}>
          <span>{s.es_cd?"🏭":"🏬"}</span>
          <span>{s.nombre}</span>
        </button>
      })}
