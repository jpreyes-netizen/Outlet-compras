...
/* ═══ OC DETAIL — With parametric reception per item ═══ */
function OCDetView({oc,firmas,pagos,updOC,addFirma,setPagos,close,cu,h}){
  ...
  const[items,setItems]=useState([]);const[recQty,setRecQty]=useState({})
  useEffect(()=>{supabase.from('oc_items').select('*').eq('oc_id',oc.id).then(r=>{const d=r.data||[];setItems(d);const q={};d.forEach(i=>{q[i.id]=i.cantidad_pedida||0});setRecQty(q)})},[oc.id])
