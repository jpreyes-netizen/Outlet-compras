...
const sendEmail=async(payload)=>{
  try{
    if(!payload.to||(Array.isArray(payload.to)&&payload.to.length===0)){
      console.warn("[email] sin destinatarios, se omite envío");return{error:"no recipients"}
    }
    const{data,error}=await supabase.functions.invoke('send-notification-email',{body:payload})
    if(error){console.warn("[email] error invocando Edge Function:",error.message);toast.warn("No se pudo enviar email de notificación",error.message);return{error}}
    console.log("[email] enviado OK:",data)
    return{data}
  }catch(e){
    console.warn("[email] excepción:",e.message)
    toast.warn("Error enviando email",e.message||String(e))
    return{error:e}
  }
}
...
const generarPDFOC=async({oc,items,prov,firmas=[],cu})=>{
  ...
  const bodyRows=items.map((it,idx)=>{
    ...
    return[idx+1,it.sku||'—',it.producto||'—',fN(it.cantidad_pedida||0),moneda+' '+fN(it.costo_unitario||0),moneda+' '+fN(sub)]
  })
  doc.autoTable({
    startY:y,
...
export default function App(){
  ...
  return<div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif",margin:0,padding:"0 20px 100px",background:"#F2F2F7",minHeight:"100vh",fontSize:14}}>
    ...
    <TabPane active={tab==="monitor"}><MonitorView ocs={ocs} prods={prodsWithTransit} h={h}/></TabPane>
    <TabPane active={tab==="repo"}><RepoView prods={prodsWithTransit} cart={cart} setCart={setCart} go={()=>setTab("nueva")} config={config} params={params} paramsABCD={paramsABCD}/></TabPane>
    <TabPane active={tab==="calendario"}><CalendarioView prods={prodsWithTransit} ocs={ocs}/></TabPane>
    <TabPane active={tab==="costeo"}><CosteoImpView config={config} saveConfig={saveConfig} ocs={ocs}/></TabPane>
    <TabPane active={tab==="forecast"}><ForecastView prods={prodsWithTransit} ocs={ocs} config={config} saveConfig={saveConfig}/></TabPane>
    <TabPane active={tab==="transito"}><TransitoView ocs={ocs} provs={provs}/></TabPane>
    {h("crear_oc")&&<TabPane active={tab==="nueva"}><SolView cart={cart} setCart={setCart} provs={provs} users={users} cu={cu} setOcs={setOcs} addFirma={addFirma} goOC={()=>setTab("ordenes")} ocs={ocs}/></TabPane>}
    ...
    <Sheet show={!!det} onClose={()=>setDet(null)} title={det?.id||""}>{det&&<OCDetView oc={det} firmas={firmas.filter(f=>f.oc_id===det.id)} pagos={pagos.filter(p=>p.oc_id===det.id)} updOC={updOC} addFirma={addFirma} setPagos={setPagos} close={()=>{setDet(null);loadAll()}} cu={cu} h={h} provs={provs} users={users}/>}</Sheet>
...
function SolView({cart,setCart,provs,users,cu,setOcs,addFirma,goOC,ocs}){
  ...
  const submit=async()=>{
    ...
    setSaving(true)
    const isI=tipo==="Importación";const id=nextNum()
...
function OCDetView({oc,firmas,pagos,updOC,addFirma,setPagos,close,cu,h,provs=[],users=[]}){
  ...
  const descargarPDF=async()=>{
    ...
    try{
      const prov=provs.find(p=>p.id===oc.proveedor_id)
      await generarPDFOC({oc,items,prov,firmas,cu})
    }catch(e){
...
function CalendarioView({prods,ocs}){
  ...
  const prodsFiltrados=useMemo(()=>{
    return prods.filter(p=>{
      if(filTipo!=="all"&&p.tipo_producto!==filTipo)return false
      if(filABCD!=="all"&&p.clasif_abcd!==filABCD)return false
  ...
  return<div>
    ...
    <div style={{background:"#fff",borderRadius:12,padding:14,marginBottom:14,boxShadow:"0 1px 3px rgba(0,0,0,0.04)"}}>
      <div style={{display:"grid",gridTemplateColumns:"repeat(6,1fr)",gap:10,alignItems:"end"}}>
        ...
        <div><div style={{fontSize:11,fontWeight:600,color:"#8E8E93",marginBottom:4}}>Tipo</div><select value={filTipo} onChange={e=>setFilTipo(e.target.value)} style={css.select}><option value="all">Todos</option>{tipos.map(t=><option key={t} value={t}>{t}</option>)}</select></div>
        <div><div style={{fontSize:11,fontWeight:600,color:"#8E8E93",marginBottom:4}}>Clasif. ABCD</div><select value={filABCD} onChange={e=>setFilABCD(e.target.value)} style={css.select}><option value="all">Todos</option><option value="A">A · Crítico</option><option value="B">B · Importante</option><option value="C">C · Regular</option><option value="D">D · Bajo</option></select></div>
...
function BsaleConfig({config,saveConfig,bsaleToken,setBsaleToken,saveBsale,loadAll}){
  ...
  const processExcel=async(file)=>{
    ...
    try{
      ...
      let reposCount=0
      const excluirActual=config.excluir_mes_actual!=="false" // default true
      const minNormales=Number(config.min_meses_normales)||2
      const cobSegExtra=Number(config.factor_cob_seguridad)||1 // multiplicador sobre periodo_cubrir
      for(const p of prodList){
        const tp=(pTypes||[]).find(t=>t.tipo_producto===p.tipo_producto)||{dias_fabricacion:30,periodo_cobertura:90}
        const abcd=(pABCD||[]).find(a=>a.clasificacion===p.clasif_abcd)||{dias_emergencia:5}
        ...
        // ⭐ EXCLUSION MES EN CURSO: el último mes es parcial, no lo usamos para el promedio
        const allSales=[p.venta_mes_1,p.venta_mes_2,p.venta_mes_3,p.venta_mes_4]
        const mesesDisp=Number(config.meses_analisis)||4
        const rangeMax=Math.min(mesesDisp,allSales.length)
        // Los primeros N-1 meses si excluyeMesActual, si no, todos
        const sales=excluirActual?allSales.slice(0,rangeMax-1):allSales.slice(0,rangeMax)
        p.mes_actual_excluido=excluirActual?1:0
      ...
      addLog(`⚠️ ${reposCount} productos requieren reposición${excluirActual?" (excluyendo mes en curso)":""}`,"warn")
