  ...
  const guardar=async()=>{
    ...
    try{
      ...
      for(let i=0;i<editing.length;i++){
        ...
        if(c.moneda==="USD"){c.monto_clp=Math.round(c.monto*TC_USD);c.tipo_cambio=TC_USD}
        else{c.monto_clp=c.monto;c.tipo_cambio=1}

        if(idsOriginales.includes(c.id)){
          // update existente
          const upd={concepto:c.concepto,monto:Number(c.monto),moneda:c.moneda,fecha_programada:c.fecha_programada||null,monto_clp:c.monto_clp,tipo_cambio:c.tipo_cambio,pct:Number(c.pct)||0,orden:c.orden}
          ...
          // insert nueva cuota
          const ins={id:c.id,oc_id:oc.id,concepto:c.concepto,monto:Number(c.monto),moneda:c.moneda,fecha_programada:c.fecha_programada||null,estado:"Pendiente",etapa:c.concepto.toLowerCase().replace(/\s+/g,"_"),orden:c.orden,pct:Number(c.pct)||0,monto_clp:c.monto_clp,tipo_cambio:c.tipo_cambio}
