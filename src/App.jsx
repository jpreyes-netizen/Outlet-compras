...
function TooltipFormula({it,mNames}){
  ...
  return<div style={{position:"relative",display:"inline-block"}}
    ...
    {show&&<div style={{position:"absolute",bottom:"calc(100% + 8px)",right:0,zIndex:1000,background:"linear-gradient(135deg,#1a1a2e,#16213e)",color:"#fff",padding:"14px 16px",borderRadius:12,fontSize:11,lineHeight:1.55,minWidth:340,maxWidth:380,boxShadow:"0 8px 24px rgba(0,0,0,0.25)",fontWeight:400,textAlign:"left",letterSpacing:0.1}}>
      ...
      {(it.stock_transito||0)>0&&<div style={{marginTop:8,padding:"6px 8px",background:"rgba(255,159,10,0.15)",borderRadius:6,fontSize:10,color:"#FF9F0A"}}>
        ⚠️ Hay {fN(it.stock_transito)}u en tránsito que no se descontaron del cálculo
      </div>}
    </div>}
  </div>
}
...
function RepoView({prods,cart,setCart,go,config,params,paramsABCD,sucursales=[]}){
  ...
  const calcPreview=(fab,cob)=>{
    ...
    const preview=afectados.map(p=>{
      ...
      const repo=Math.max(0,Math.round((p.vta_prom_diaria*cob)+ptoRe-p.stock_actual))
  ...
  const aplicarParams=async()=>{
    ...
    try{
      ...
      for(const p of afectados){
        ...
        const repo=Math.max(0,Math.round((p.vta_prom_diaria*paramCob)+ptoRe-p.stock_actual))
