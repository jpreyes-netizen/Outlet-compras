...
export default function App(){
  ...
  return<div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif",maxWidth:1600,margin:"0 auto",padding:"0 24px 100px",background:"#F2F2F7",minHeight:"100vh",fontSize:14}}>
    <style>{`@keyframes slideUp{from{transform:translateY(100%)}to{transform:translateY(0)}}*{box-sizing:border-box;margin:0;padding:0}body{background:#F2F2F7}input:focus,select:focus,textarea:focus{border-color:#007AFF!important;box-shadow:0 0 0 3px rgba(0,122,255,0.1)}::selection{background:#007AFF;color:#fff}::-webkit-scrollbar{width:10px;height:10px}::-webkit-scrollbar-track{background:#F2F2F7;border-radius:5px}::-webkit-scrollbar-thumb{background:#C7C7CC;border-radius:5px;border:2px solid #F2F2F7}::-webkit-scrollbar-thumb:hover{background:#8E8E93}table{font-size:13px}`}</style>
...
function RepoView({prods,cart,setCart,go,config,params,paramsABCD}){
  ...
  return<div>
    ...
    <div style={{overflowX:"auto",overflowY:"auto",maxHeight:"calc(100vh - 280px)",borderRadius:10,border:"1px solid #D1D1D6",scrollbarWidth:"auto"}}>
    <table style={{width:"100%",borderCollapse:"collapse",fontSize:13,minWidth:1500}}>
      <thead style={{position:"sticky",top:0,zIndex:5}}><tr style={{background:"#E5E5EA"}}>
        {["","Producto","Tipo","Clasif.","Estado",mNames[0]?.replace("Unidades Vendida ","").replace("Venta ","")||"M1",mNames[1]?.replace("Unidades Vendida ","").replace("Venta ","")||"M2",mNames[2]?.replace("Unidades Vendida ","").replace("Venta ","")||"M3",mNames[3]?.replace("Unidades Vendida ","").replace("Venta ","")||"M4","Vta Total","Quiebres","Vta Comp.","Vta Diaria","Stock","Tránsito","Cobertura","Pto Reorden","Reponer","Costo U.","Inversión"].map((h,i)=>
          <th key={i} style={{padding:"10px 6px",textAlign:i<3?"left":"right",fontSize:11,fontWeight:700,color:"#636366",borderBottom:"2px solid #C7C7CC",textTransform:"uppercase",letterSpacing:"0.04em",whiteSpace:"nowrap",background:"#E5E5EA"}}>{h}</th>
        )}
      </tr></thead>
      <tbody>{fil.map((it,idx)=>{const inC=!!cart[it.sku];const crit=(it.dias_cobertura||999)<15&&["A","B"].includes(it.clasif_abcd);const isR=it.estado==="Reposición";const ec=estColors[it.estado]||estColors["Revisar"]
        return<tr key={it.sku} onClick={()=>tog(it)} style={{background:inC?"#007AFF06":idx%2?"#fafafa":"#fff",cursor:"pointer",borderBottom:"1px solid #E5E5EA",transition:"background 0.1s"}} onMouseOver={e=>{if(!inC)e.currentTarget.style.background="#F2F2F7"}} onMouseOut={e=>{if(!inC)e.currentTarget.style.background=idx%2?"#fafafa":"#fff"}}>
          <td style={{padding:"8px 6px",width:32}}><input type="checkbox" checked={inC} onChange={()=>{}} style={{width:18,height:18}}/></td>
          <td style={{padding:"8px 6px",maxWidth:280}}><div style={{fontSize:14,fontWeight:600,color:"#1C1C1E",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{it.producto}</div><div style={{fontSize:11,color:"#AEAEB2"}}>{it.sku}</div></td>
          <td style={{padding:"8px 6px",fontSize:12,color:"#636366",whiteSpace:"nowrap"}}>{it.tipo_producto}</td>
          <td style={{padding:"8px 6px",textAlign:"center"}}><span style={{display:"inline-block",padding:"3px 8px",borderRadius:5,fontSize:12,fontWeight:700,color:CL[it.clasif_abcd]?.c,background:CL[it.clasif_abcd]?.bg}}>{it.clasif_abcd}</span></td>
          <td style={{padding:"8px 6px",textAlign:"right"}}><span style={{display:"inline-block",padding:"3px 8px",borderRadius:5,fontSize:11,fontWeight:600,color:ec.c,background:ec.bg,whiteSpace:"nowrap"}}>{it.estado}</span></td>
          <td style={{padding:"8px 6px",textAlign:"right",color:it.venta_mes_1?"#1C1C1E":"#D1D1D6",fontVariantNumeric:"tabular-nums"}}>{fN(it.venta_mes_1||0)}</td>
          <td style={{padding:"8px 6px",textAlign:"right",color:it.venta_mes_2?"#1C1C1E":"#D1D1D6",fontVariantNumeric:"tabular-nums"}}>{fN(it.venta_mes_2||0)}</td>
          <td style={{padding:"8px 6px",textAlign:"right",color:it.venta_mes_3?"#1C1C1E":"#D1D1D6",fontVariantNumeric:"tabular-nums"}}>{fN(it.venta_mes_3||0)}</td>
          <td style={{padding:"8px 6px",textAlign:"right",color:it.venta_mes_4?"#1C1C1E":"#D1D1D6",fontVariantNumeric:"tabular-nums"}}>{fN(it.venta_mes_4||0)}</td>
          <td style={{padding:"8px 6px",textAlign:"right",fontWeight:600,color:it.venta_total?"#1C1C1E":"#D1D1D6"}}>{fN(it.venta_total||0)}</td>
          <td style={{padding:"8px 6px",textAlign:"right"}}>{it.meses_quiebre>0?<span style={{color:"#FF9500",fontWeight:700,fontSize:14}}>{it.meses_quiebre}</span>:<span style={{color:"#D1D1D6"}}>0</span>}</td>
          <td style={{padding:"8px 6px",textAlign:"right",fontWeight:600,color:it.vta_prom_compensada?"#007AFF":"#D1D1D6"}}>{fN(it.vta_prom_compensada||0)}</td>
          <td style={{padding:"8px 6px",textAlign:"right",color:"#636366"}}>{it.vta_prom_diaria?Math.round(it.vta_prom_diaria*10)/10:"—"}</td>
          <td style={{padding:"8px 6px",textAlign:"right",fontWeight:600,color:it.stock_actual<=0?"#FF3B30":"#1C1C1E",fontSize:14}}>{fN(it.stock_actual)}</td>
          <td style={{padding:"8px 6px",textAlign:"right",color:it.stock_transito?"#007AFF":"#D1D1D6"}}>{it.stock_transito?fN(it.stock_transito):"—"}</td>
          <td style={{padding:"8px 6px",textAlign:"right"}}><span style={{fontWeight:700,fontSize:13,color:crit?"#FF3B30":(it.dias_cobertura||0)<30?"#FF9500":"#34C759"}}>{it.dias_cobertura&&it.dias_cobertura<999?Math.round(it.dias_cobertura)+"d":"—"}</span></td>
          <td style={{padding:"8px 6px",textAlign:"right",color:"#636366"}}>{it.punto_reorden?fN(it.punto_reorden):"—"}</td>
          <td style={{padding:"8px 6px",textAlign:"right"}}>{inC?<input type="number" value={cart[it.sku]?.cp||0} onChange={e=>{e.stopPropagation();setQty(it.sku,e.target.value)}} onClick={e=>e.stopPropagation()} style={{width:70,textAlign:"center",fontSize:14,fontWeight:700,padding:"4px",border:"2px solid #007AFF",borderRadius:8,color:"#007AFF"}}/>:<strong style={{color:isR?"#FF3B30":"#1C1C1E",fontSize:14}}>{it.reposicion_necesaria?fN(it.reposicion_necesaria):"—"}</strong>}</td>
          <td style={{padding:"8px 6px",textAlign:"right",color:"#636366"}}>{it.costo_unitario?fmt(it.costo_unitario):"—"}</td>
