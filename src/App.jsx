...
La OC original ${oc.id} quedó en estado "Dividida".`)
  ...
  <div>
    ...
    {(()=>{
      ...
      if(!h("act_estado")||!OPS.includes(oc.estado))return null
      const destinos=OPS.filter(e=>e!==oc.estado)
      return<div style={{display:"flex",alignItems:"center",gap:8,flexWrap:"wrap",background:"#F0F9FF",border:"1px solid #007AFF30",borderRadius:10,padding:"8px 12px",marginBottom:12}}>
        ...
        <select defaultValue="" onChange={async e=>{
          ...
          try{
            const up={estado:nuevo,fase_actual:(isI?FASE_IMP:FASE_NAC)[nuevo]||oc.fase_actual}
            if(nuevo==="Recibida OK"&&oc.recepcion_cuadrada==null)up.recepcion_cuadrada=pctRecibidoAcum>=100
            await updOC(oc.id,up)
            await addFirma(oc.id,`Estado actualizado: ${oc.estado} → ${nuevo} — ${motivo.trim()}`)
