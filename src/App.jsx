  ...
  return<div>
    ...
    <table style={{width:"100%",borderCollapse:"collapse",fontSize:11,minWidth:1400}}>
      <thead><tr style={{background:"#F2F2F7"}}>
        {["","Producto","Tipo","Clasif.","Estado",mNames[0]?.replace("Unidades Vendida ","").replace("Venta ","")||"M1",mNames[1]?.replace("Unidades Vendida ","").replace("Venta ","")||"M2",mNames[2]?.replace("Unidades Vendida ","").replace("Venta ","")||"M3",mNames[3]?.replace("Unidades Vendida ","").replace("Venta ","")||"M4","Vta Total","Quiebres","Vta Comp.","Vta Diaria","Stock","Tránsito","Cobertura","Pto Reorden","Reponer","Costo U.","Inversión"].map((h,i)=>
          <th key={i} style={{padding:"7px 5px",textAlign:i<3?"left":"right",fontSize:9,fontWeight:700,color:"#8E8E93",borderBottom:"2px solid #D1D1D6",textTransform:"uppercase",letterSpacing:"0.03em",whiteSpace:"nowrap",position:i===0?"sticky":"static",left:i===0?0:undefined,background:"#F2F2F7",zIndex:i===0?2:1}}>{h}</th>
        )}
      </tr></thead>
      <tbody>{fil.map((it,idx)=>{const inC=!!cart[it.sku];const crit=(it.dias_cobertura||999)<15&&["A","B"].includes(it.clasif_abcd);const isR=it.estado==="Reposición";const ec=estColors[it.estado]||estColors["Revisar"]
        return<tr key={it.sku} onClick={()=>tog(it)} style={{background:inC?"#007AFF06":idx%2?"#fafafa":"#fff",cursor:"pointer",borderBottom:"1px solid #F2F2F7",transition:"background 0.1s"}} onMouseOver={e=>{if(!inC)e.currentTarget.style.background="#F2F2F7"}} onMouseOut={e=>{if(!inC)e.currentTarget.style.background=idx%2?"#fafafa":"#fff"}}>
          <td style={{padding:"6px 5px",width:28,position:"sticky",left:0,background:inC?"#007AFF06":idx%2?"#fafafa":"#fff",zIndex:1}}><input type="checkbox" checked={inC} onChange={()=>{}}/></td>
          <td style={{padding:"6px 5px",maxWidth:220}}><div style={{fontSize:12,fontWeight:600,color:"#1C1C1E",overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{it.producto}</div><div style={{fontSize:9,color:"#AEAEB2"}}>{it.sku}</div></td>
          <td style={{padding:"6px 5px",fontSize:10,color:"#8E8E93",whiteSpace:"nowrap"}}>{it.tipo_producto}</td>
          <td style={{padding:"6px 5px",textAlign:"center"}}><span style={{display:"inline-block",padding:"2px 6px",borderRadius:4,fontSize:10,fontWeight:700,color:CL[it.clasif_abcd]?.c,background:CL[it.clasif_abcd]?.bg}}>{it.clasif_abcd}</span></td>
          <td style={{padding:"6px 5px",textAlign:"right"}}><span style={{display:"inline-block",padding:"2px 6px",borderRadius:4,fontSize:9,fontWeight:600,color:ec.c,background:ec.bg,whiteSpace:"nowrap"}}>{it.estado}</span></td>
          <td style={{padding:"6px 5px",textAlign:"right",color:it.venta_mes_1?"#1C1C1E":"#D1D1D6",fontVariantNumeric:"tabular-nums"}}>{fN(it.venta_mes_1||0)}</td>
          <td style={{padding:"6px 5px",textAlign:"right",color:it.venta_mes_2?"#1C1C1E":"#D1D1D6",fontVariantNumeric:"tabular-nums"}}>{fN(it.venta_mes_2||0)}</td>
          <td style={{padding:"6px 5px",textAlign:"right",color:it.venta_mes_3?"#1C1C1E":"#D1D1D6",fontVariantNumeric:"tabular-nums"}}>{fN(it.venta_mes_3||0)}</td>
          <td style={{padding:"6px 5px",textAlign:"right",color:it.venta_mes_4?"#1C1C1E":"#D1D1D6",fontVariantNumeric:"tabular-nums"}}>{fN(it.venta_mes_4||0)}</td>
          <td style={{padding:"6px 5px",textAlign:"right",fontWeight:600,color:it.venta_total?"#1C1C1E":"#D1D1D6"}}>{fN(it.venta_total||0)}</td>
          <td style={{padding:"6px 5px",textAlign:"right"}}>{it.meses_quiebre>0?<span style={{color:"#FF9500",fontWeight:600}}>{it.meses_quiebre}</span>:<span style={{color:"#D1D1D6"}}>0</span>}</td>
          <td style={{padding:"6px 5px",textAlign:"right",fontWeight:600,color:it.vta_prom_compensada?"#007AFF":"#D1D1D6"}}>{fN(it.vta_prom_compensada||0)}</td>
          <td style={{padding:"6px 5px",textAlign:"right",color:"#8E8E93"}}>{it.vta_prom_diaria?Math.round(it.vta_prom_diaria*10)/10:"—"}</td>
          <td style={{padding:"6px 5px",textAlign:"right",fontWeight:600,color:it.stock_actual<=0?"#FF3B30":"#1C1C1E"}}>{fN(it.stock_actual)}</td>
          <td style={{padding:"6px 5px",textAlign:"right",color:it.stock_transito?"#007AFF":"#D1D1D6"}}>{it.stock_transito?fN(it.stock_transito):"—"}</td>
          <td style={{padding:"6px 5px",textAlign:"right"}}><span style={{fontWeight:600,color:crit?"#FF3B30":(it.dias_cobertura||0)<30?"#FF9500":"#34C759"}}>{it.dias_cobertura&&it.dias_cobertura<999?Math.round(it.dias_cobertura)+"d":"—"}</span></td>
          <td style={{padding:"6px 5px",textAlign:"right",color:"#8E8E93"}}>{it.punto_reorden?fN(it.punto_reorden):"—"}</td>
          <td style={{padding:"6px 5px",textAlign:"right"}}>{inC?<input type="number" value={cart[it.sku]?.cp||0} onChange={e=>{e.stopPropagation();setQty(it.sku,e.target.value)}} onClick={e=>e.stopPropagation()} style={{width:60,textAlign:"center",fontSize:11,fontWeight:700,padding:"2px",border:"2px solid #007AFF",borderRadius:6,color:"#007AFF"}}/>:<strong style={{color:isR?"#FF3B30":"#1C1C1E"}}>{it.reposicion_necesaria?fN(it.reposicion_necesaria):"—"}</strong>}</td>
          <td style={{padding:"6px 5px",textAlign:"right",color:"#8E8E93",fontSize:10}}>{it.costo_unitario?fmt(it.costo_unitario):"—"}</td>
          <td style={{padding:"6px 5px",textAlign:"right",fontWeight:600,fontSize:10}}>{it.costo_reposicion?fmt(it.costo_reposicion):"—"}</td>
        </tr>})}</tbody>
    </table></div>
