...
La OC original ${oc.id} quedó en estado "Dividida".`)
  ...
  const guardarRecepcion=async()=>{
    // Canal único (SOP): la recepción FÍSICA (guías) se registra en Logística.
    // Las FACTURAS son documentales y se registran aquí con normalidad.
    if(cu?.rol!=="admin"&&recForm.tipo!=="factura"){alert("Las guías (recepción física) se registran en el módulo Logística y se reflejan aquí automáticamente.\nEn Compras puedes registrar facturas; el registro manual de guías quedó reservado a administración para regularizaciones.");return}
    ...
    {showRecModal&&<div style={css.modal} onClick={()=>setShowRecModal(false)}>
      <div onClick={e=>e.stopPropagation()} style={{background:"#fff",borderRadius:"20px 20px 0 0",padding:"8px 24px 32px",width:"100%",maxWidth:760,maxHeight:"92vh",overflow:"auto",animation:"slideUp 0.3s ease"}}>
        ...
        <div style={{padding:"9px 12px",borderRadius:10,marginBottom:14,
          ...
          <div style={{fontSize:11.5,fontWeight:800,color:cu?.rol==="admin"?"#C93400":"#007AFF",marginBottom:2}}>
            {cu?.rol==="admin"?"⚠ VÍA DE REGULARIZACIÓN":"📍 LAS RECEPCIONES SE REGISTRAN EN LOGÍSTICA"}
          </div>
          <div style={{fontSize:11,color:"#6D6D72",lineHeight:1.4}}>
            {cu?.rol==="admin"
              ?"Guías (recepción física): el canal oficial es Logística, con inspección y respaldos — el registro manual aquí es sólo regularización. Facturas: se registran aquí con normalidad."
              :"Las guías se recepcionan en el módulo Logística — con inspección de bultos y respaldos — y se reflejan automáticamente en esta OC. Las facturas sí se registran aquí. Para regularizar una guía antigua, pídelo a administración."}
          </div>
        </div>
        ...
        <div style={{display:"flex",gap:10,marginTop:14}}>
          <Bt v="pri" full onClick={guardarRecepcion} dis={guardandoRec||(cu?.rol!=="admin"&&recForm.tipo!=="factura")} ic={guardandoRec?"⏳":(cu?.rol!=="admin"&&recForm.tipo!=="factura")?"🔒":"✓"}>{guardandoRec?"Guardando...":(cu?.rol!=="admin"&&recForm.tipo!=="factura")?"Guías: registro en Logística":`Registrar ${recForm.tipo==="guia"?"guía":recForm.tipo==="factura"?"factura":"documento"}${cu?.rol==="admin"&&recForm.tipo!=="factura"?" (regularización)":""}`}</Bt>
