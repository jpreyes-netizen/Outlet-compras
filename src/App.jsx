...
export default function App(){
  ...
  return<div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif",margin:0,padding:"0 20px 100px",background:"#F2F2F7",minHeight:"100vh",fontSize:14}}>
    ...
    <TabPane active={tab==="ordenes"}><OCListView ocs={ocs} firmas={firmas} pagos={pagos} updOC={updOC} addFirma={addFirma} setDet={setDet} cu={cu} h={h} provs={provs} setOcs={setOcs}/></TabPane>
    ...
    <Sheet show={!!det} onClose={()=>setDet(null)} title={det?.id||""}>{det&&<OCDetView oc={det} firmas={firmas.filter(f=>f.oc_id===det.id)} pagos={pagos.filter(p=>p.oc_id===det.id)} updOC={updOC} addFirma={addFirma} setPagos={setPagos} close={()=>{setDet(null);loadAll()}} cu={cu} h={h} provs={provs}/>}</Sheet>
...
function OCDetView({oc,firmas,pagos,updOC,addFirma,setPagos,close,cu,h,provs=[]}){
  ...
  const recalcular=async()=>{
    ...
    try{
      ...
      for(let i=0;i<prodList.length;i+=200){
        ...
        const{error:e2}=await supabase.from('productos').upsert(batch,{onConflict:'sku'})
        if(!e2)saved+=batch.length
      }
