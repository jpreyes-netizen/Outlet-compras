...
function RepoView({prods,prodsSuc=[],cart,setCart,go,config,params,paramsABCD,sucursales=[]}){
  ...
  const prodsActivos=useMemo(()=>{
    if(fsuc==="T"||!prodsSuc.length)return prods
    const suc=prodsSuc.filter(p=>p.sucursal===fsuc||p.sucursal_id===fsuc)
    if(!suc.length)return prods // fallback a consolidado si no hay datos
    // Merge con prods para tener campos como costo_unitario, tipo, etc.
    return suc.map(ps=>({...prods.find(p=>p.sku===ps.sku)||{},
      ...ps,
      // asegurar que los campos del algoritmo vienen de productos_sucursal
      ...
      meses_quiebre:ps.meses_quiebre,stock_transito:0 // tránsito es CD central
    }))
  },[prods,prodsSuc,fsuc])
