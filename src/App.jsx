...
/* ═══ MAIN APP ═══ */
const TABS=[{k:"monitor",l:"Monitor",ic:"📊"},{k:"repo",l:"Reposición",ic:"📦"},{k:"costeo",l:"Costeo IMP",ic:"🧮"},{k:"transito",l:"Tránsito",ic:"🚚"},{k:"nueva",l:"Nueva OC",ic:"➕"},{k:"ordenes",l:"Órdenes",ic:"📋"},{k:"config",l:"Config",ic:"⚙️"}]
...
export default function App(){
  ...
  return<div style={{fontFamily:"-apple-system,BlinkMacSystemFont,'SF Pro Display',system-ui,sans-serif",margin:0,padding:"0 20px 100px",background:"#F2F2F7",minHeight:"100vh",fontSize:14}}>
    ...
    {tab==="config"&&<ConfigView config={config} saveConfig={saveConfig} params={params} setParams={setParams} paramsABCD={paramsABCD} setParamsABCD={setParamsABCD} provs={provs} setProvs={setProvs} users={users} setUsers={setUsers} h={h} configTab={configTab} setConfigTab={setConfigTab} loadAll={loadAll}/>}
...
function SolView({cart,setCart,provs,users,cu,setOcs,addFirma,goOC,ocs}){
  ...
  if(done)return<div style={{textAlign:"center",padding:"60px 20px"}}><div style={{width:64,height:64,borderRadius:32,background:"#34C75920",margin:"0 auto 16px",display:"flex",alignItems:"center",justifyContent:"center",fontSize:28}}>✓</div><div style={{fontSize:24,fontWeight:800,color:"#1C1C1E",fontFamily:"'SF Mono',monospace"}}>{done.id}</div><div style={{fontSize:14,color:"#8E8E93",marginBottom:20}}>Solicitud enviada a Dir. Negocios para aprobación</div><Bt v="pri" onClick={()=>{setDone(null);goOC()}} ic="📋">Ver órdenes</Bt></div>
  if(sel.length===0)return<div style={{textAlign:"center",padding:"60px 20px"}}><div style={{fontSize:40,marginBottom:12}}>📋</div><div style={{fontSize:18,fontWeight:700,color:"#1C1C1E"}}>Sin productos seleccionados</div><div style={{fontSize:14,color:"#8E8E93",marginTop:4}}>Ve a Reposición y selecciona los SKUs a reponer</div></div>
  ...
  return<div style={{maxWidth:900,margin:"0 auto"}}>
    ...
    <div style={{background:"#fff",borderRadius:12,overflow:"hidden",boxShadow:"0 2px 12px rgba(0,0,0,0.06)"}}>
      ...
      <div style={{background:"linear-gradient(135deg,#1a1a2e 0%,#16213e 100%)",color:"#fff",padding:"24px 32px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
        <div><div style={{fontSize:11,fontWeight:600,color:"rgba(255,255,255,0.5)",letterSpacing:"0.15em",textTransform:"uppercase",marginBottom:4}}>Outlet de Puertas SpA</div><div style={{fontSize:22,fontWeight:800,letterSpacing:"-0.02em"}}>SOLICITUD DE ORDEN DE COMPRA</div><div style={{fontSize:12,color:"rgba(255,255,255,0.6)",marginTop:4}}>SOP P07 · Módulo de Abastecimiento</div></div>
        <div style={{textAlign:"right"}}><div style={{fontSize:10,color:"rgba(255,255,255,0.4)",textTransform:"uppercase",letterSpacing:"0.1em"}}>Fecha emisión</div><div style={{fontSize:18,fontWeight:700}}>{hoy()}</div><div style={{fontSize:12,color:"rgba(255,255,255,0.5)",marginTop:2}}>{hora()}</div></div>
      </div>
      ...
      <div style={{padding:"24px 32px"}}>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:16,marginBottom:20}}>
          <div>
            <Fl l="Solicitante"><div style={{display:"flex",alignItems:"center",gap:10}}><Av n={cu?.avatar} c={rl(cu).c} sz={40}/><div><div style={{fontSize:15,fontWeight:600}}>{cu?.nombre}</div><div style={{fontSize:12,color:rl(cu).c,fontWeight:600}}>{rl(cu).l}</div></div></div></Fl>
          ...
          <div>
            ...
            {sp&&<div style={{background:"#F2F2F7",borderRadius:10,padding:12,fontSize:13}}>
              <div style={{display:"flex",gap:8,alignItems:"center",marginBottom:4}}><Bd c={tipo==="Importación"?"#FF3B30":"#007AFF"} bg={tipo==="Importación"?"#FF3B3015":"#007AFF15"} lg>{tipo}</Bd><span style={{color:"#8E8E93"}}>·</span><strong>{sp.condicion_pago}</strong></div>
              {sp.pct_fabricacion>0&&<div style={{fontSize:12,color:"#FF9500"}}>Abonos: {sp.pct_fabricacion}% fab · {sp.pct_embarque}% emb · {sp.pct_puerto}% pto</div>}
            </div>}
            <Fl l="Observaciones" sub="Opcional"><textarea value={notas} onChange={e=>setNotas(e.target.value)} rows={2} style={{...css.input,resize:"vertical",fontSize:13}}/></Fl>
          </div>
        </div>

        {/* Items table */}
        <div style={{fontSize:12,fontWeight:700,color:"#636366",textTransform:"uppercase",letterSpacing:"0.05em",marginBottom:8}}>Detalle de productos ({sel.length} ítems)</div>
        <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
          <thead><tr style={{background:"#F8F8FA"}}><th style={{padding:"10px 12px",textAlign:"left",fontWeight:700,color:"#636366",fontSize:10,textTransform:"uppercase",borderBottom:"2px solid #E5E5EA"}}>Producto</th><th style={{padding:"10px 8px",textAlign:"right",fontWeight:700,color:"#636366",fontSize:10,textTransform:"uppercase",borderBottom:"2px solid #E5E5EA"}}>SKU</th><th style={{padding:"10px 8px",textAlign:"right",fontWeight:700,color:"#636366",fontSize:10,textTransform:"uppercase",borderBottom:"2px solid #E5E5EA"}}>Sugerido</th><th style={{padding:"10px 8px",textAlign:"right",fontWeight:700,color:"#007AFF",fontSize:10,textTransform:"uppercase",borderBottom:"2px solid #E5E5EA"}}>Cantidad</th><th style={{padding:"10px 8px",textAlign:"right",fontWeight:700,color:"#636366",fontSize:10,textTransform:"uppercase",borderBottom:"2px solid #E5E5EA"}}>Costo Unit.</th><th style={{padding:"10px 12px",textAlign:"right",fontWeight:700,color:"#636366",fontSize:10,textTransform:"uppercase",borderBottom:"2px solid #E5E5EA"}}>Subtotal</th></tr></thead>
          <tbody>{sel.map((i,idx)=><tr key={idx} style={{borderBottom:"1px solid #F2F2F7"}}><td style={{padding:"10px 12px"}}><div style={{fontWeight:600}}>{i.producto}</div><div style={{fontSize:11,color:"#AEAEB2"}}>{i.tipo_producto}</div></td><td style={{padding:"10px 8px",textAlign:"right",fontSize:11,color:"#8E8E93",fontFamily:"monospace"}}>{i.sku}</td><td style={{padding:"10px 8px",textAlign:"right",color:"#8E8E93"}}>{fN(i.reposicion_necesaria)}</td><td style={{padding:"10px 8px",textAlign:"right",color:"#007AFF",fontWeight:700,fontSize:15}}>{fN(i.cp)}</td><td style={{padding:"10px 8px",textAlign:"right",color:"#636366"}}>{fmt(i.costo_unitario)}</td><td style={{padding:"10px 12px",textAlign:"right",fontWeight:600,fontSize:14}}>{fmt(i.cp*(i.costo_unitario||0))}</td></tr>)}</tbody>
          <tfoot><tr style={{borderTop:"2px solid #1C1C1E"}}><td colSpan={5} style={{padding:"14px 12px",textAlign:"right",fontSize:16,fontWeight:700}}>TOTAL ORDEN DE COMPRA</td><td style={{padding:"14px 12px",textAlign:"right",fontSize:22,fontWeight:800,color:"#1C1C1E"}}>{fmt(total)}</td></tr></tfoot>
        </table>
      </div>

      {/* Signature block */}
      <div style={{borderTop:"1px solid #E5E5EA",padding:"20px 32px",background:"#FAFAFA"}}>
        <div style={{fontSize:10,fontWeight:700,color:"#AEAEB2",textTransform:"uppercase",letterSpacing:"0.1em",marginBottom:8}}>Firma del solicitante</div>
        <div style={{display:"flex",alignItems:"center",gap:12}}>
          <Av n={cu?.avatar} c={rl(cu).c} sz={44}/>
          <div><div style={{fontSize:18,fontStyle:"italic",fontWeight:700,color:rl(cu).c}}>{cu?.firma_digital||cu?.nombre}</div><div style={{fontSize:12,color:"#AEAEB2"}}>{rl(cu).l} — {hoy()} {hora()}</div></div>
        </div>
      </div>
    </div>

    <div style={{marginTop:16}}><Bt v="pri" full dis={!prov||!fEst||saving} onClick={submit} ic="✍️">{saving?"Procesando solicitud...":"Firmar y enviar solicitud de compra"}</Bt></div>
  </div>
}
...
/* ═══ COSTEO IMPORTACIÓN — 7-stage import cost simulator ═══ */
function CosteoImpView({config,saveConfig,ocs}){
  const ocsImp=ocs.filter(o=>o.tipo_oc==="Importación"&&!["Cerrada","Rechazada"].includes(o.estado))
  const[selOC,setSelOC]=useState("")
  // Default params (editable)
  const[p,setP]=useState({
    // Etapa 1: FOB
    moneda:"USD",tc:Number(config.tc_usd)||950,cantUds:1000,precioUnitFOB:5.00,
    // Etapa 2: Flete + Seguro
    tipoContenedor:"40HC",costoFlete:Number(config.costo_flete_40hc)||3800,pctSeguro:Number(config.pct_seguro_int)||0.5,
    // Etapa 3: Aduana
    pctArancel:Number(config.pct_arancel)||6,tieneILC:false,pctAforo:1,pctIVA:19,
    // Etapa 4: Puerto
    handling:Number(config.costo_handling)||350,almacenajeDia:Number(config.almacenaje_dia)||45,diasAlmacenaje:Number(config.dias_almacenaje)||5,porteo:Number(config.costo_porteo)||120,
    // Etapa 5: Agente aduana
    honorarioAgente:Number(config.honorario_agente)||450,gastosOpAgente:Number(config.gastos_op_agente)||180,
    // Etapa 6: Transporte terrestre
    transportePuertoBodega:Number(config.transporte_puerto_bodega)||800,seguroTerrestre:Number(config.seguro_terrestre)||50,
    // Etapa 7: Financiero
    pctCostoFinanciero:Number(config.pct_costo_financiero)||1.5,diasTransito:Number(config.dias_transito_imp)||60,costoTransferencia:Number(config.costo_transferencia)||35,
    // Margen
    precioVentaCLP:0,
  })
  ...
  const TC=p.tc
  // E1: FOB
  const fobUSD=p.cantUds*p.precioUnitFOB
  const fobCLP=Math.round(fobUSD*TC)
  // E2: CIF
  const fleteUSD=p.costoFlete
  const seguroUSD=Math.round(fobUSD*p.pctSeguro/100*100)/100
  const cifUSD=fobUSD+fleteUSD+seguroUSD
  const cifCLP=Math.round(cifUSD*TC)
  // E3: Aduana
  const arancelCLP=p.tieneILC?0:Math.round(cifCLP*p.pctArancel/100)
  const aforoCLP=Math.round(cifCLP*p.pctAforo/100)
  const baseIVA=cifCLP+arancelCLP
  const ivaCLP=Math.round(baseIVA*p.pctIVA/100)
  const totalAduana=arancelCLP+aforoCLP+ivaCLP
  // E4: Puerto
  const almacenajeCLP=Math.round(p.almacenajeDia*p.diasAlmacenaje*TC)
  const handlingCLP=Math.round(p.handling*TC)
  const porteoCLP=Math.round(p.porteo*TC)
  const totalPuerto=handlingCLP+almacenajeCLP+porteoCLP
  // E5: Agente
  ...
  // E7: Financiero
  const capitalInmov=cifCLP
  const costoFinCLP=Math.round(capitalInmov*p.pctCostoFinanciero/100*(p.diasTransito/30))
  const costoTransfCLP=Math.round(p.costoTransferencia*TC)
  const totalFinanciero=costoFinCLP+costoTransfCLP
  ...
  const costoUnitUSD=TC>0?Math.round(costoUnitBodega/TC*100)/100:0
  const margenPct=p.precioVentaCLP>0?Math.round((p.precioVentaCLP-costoUnitBodega)/p.precioVentaCLP*100):0
  const margenCLP=p.precioVentaCLP-costoUnitBodega

  // Stage breakdown for chart
  const stages=[
    {n:"FOB Origen",v:fobCLP,c:"#007AFF",ic:"🏭"},
    {n:"Flete + Seguro",v:Math.round((fleteUSD+seguroUSD)*TC),c:"#5856D6",ic:"🚢"},
    {n:"Aduana",v:totalAduana,c:"#FF3B30",ic:"🏛"},
    {n:"Puerto",v:totalPuerto,c:"#FF9500",ic:"⚓"},
    {n:"Agente",v:agenteCLP,c:"#AF52DE",ic:"📋"},
    {n:"Transporte",v:transporteCLP,c:"#34C759",ic:"🚛"},
    {n:"Financiero",v:totalFinanciero,c:"#FF2D55",ic:"💰"},
  ]
  ...
  const saveParams=async()=>{
    const keys={tc_usd:p.tc,costo_flete_40hc:p.costoFlete,pct_seguro_int:p.pctSeguro,pct_arancel:p.pctArancel,costo_handling:p.handling,almacenaje_dia:p.almacenajeDia,dias_almacenaje:p.diasAlmacenaje,costo_porteo:p.porteo,honorario_agente:p.honorarioAgente,gastos_op_agente:p.gastosOpAgente,transporte_puerto_bodega:p.transportePuertoBodega,seguro_terrestre:p.seguroTerrestre,pct_costo_financiero:p.pctCostoFinanciero,dias_transito_imp:p.diasTransito,costo_transferencia:p.costoTransferencia}
    for(const[k,v]of Object.entries(keys))await saveConfig(k,String(v))
    alert("Parámetros guardados como valores por defecto")
  }

  return<div>
    <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:14}}>
      <div><div style={{fontSize:26,fontWeight:800,color:"#1C1C1E",letterSpacing:"-0.02em"}}>Costeo de importación</div><div style={{fontSize:14,color:"#8E8E93"}}>Simulador de costo de internación CIF + gastos locales</div></div>
      <div style={{display:"flex",gap:6}}><Bt v="gry" sm onClick={saveParams} ic="💾">Guardar params</Bt></div>
    </div>
    ...
    <div style={{display:"grid",gridTemplateColumns:"380px 1fr",gap:16}}>
      ...
      <div style={{maxHeight:"calc(100vh - 200px)",overflowY:"auto",paddingRight:8}}>
        ...
        {/* OC selector */}
        {ocsImp.length>0&&<Cd><div style={{fontSize:13,fontWeight:700,marginBottom:6}}>Vincular a OC de importación</div>
          <select value={selOC} onChange={e=>setSelOC(e.target.value)} style={{...css.select,marginBottom:4}}><option value="">Simulación libre</option>{ocsImp.map(o=><option key={o.id} value={o.id}>{o.id} — {fmt(o.total_clp)}</option>)}</select>
        </Cd>}

        {/* E1: FOB */}
        <Cd><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><span style={{fontSize:18}}>🏭</span><div><div style={{fontSize:14,fontWeight:700,color:"#007AFF"}}>Etapa 1 — Costo FOB</div><div style={{fontSize:10,color:"#8E8E93"}}>Precio en origen (Free on Board)</div></div></div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <In l="Cantidad (uds)" k="cantUds" w={100}/>
            <In l="Precio unit. FOB" k="precioUnitFOB" pre="USD" w={90}/>
            <In l="Tipo de cambio" k="tc" pre="$" suf="/USD" w={80}/>
          </div>
          <div style={{background:"#007AFF08",borderRadius:8,padding:8,marginTop:4,fontSize:13}}><strong style={{color:"#007AFF"}}>FOB = {fU(fobUSD)}</strong> → <strong>{fmt(fobCLP)}</strong></div>
        </Cd>

        {/* E2: Flete + Seguro */}
        <Cd><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><span style={{fontSize:18}}>🚢</span><div><div style={{fontSize:14,fontWeight:700,color:"#5856D6"}}>Etapa 2 — Flete + Seguro</div><div style={{fontSize:10,color:"#8E8E93"}}>Transporte marítimo internacional</div></div></div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <div style={{marginBottom:8}}><div style={{fontSize:11,color:"#636366",fontWeight:600,marginBottom:2}}>Contenedor</div><select value={p.tipoContenedor} onChange={e=>us("tipoContenedor",e.target.value)} style={{...css.select,width:100}}><option>20FT</option><option>40FT</option><option>40HC</option></select></div>
            <In l="Costo flete" k="costoFlete" pre="USD" w={90}/>
            <In l="Seguro int." k="pctSeguro" suf="% FOB" w={70}/>
          </div>
          <div style={{background:"#5856D608",borderRadius:8,padding:8,marginTop:4,fontSize:12}}>Flete: <strong>{fU(fleteUSD)}</strong> · Seguro: <strong>{fU(seguroUSD)}</strong> → <strong style={{color:"#5856D6"}}>CIF = {fU(cifUSD)}</strong> ({fmt(cifCLP)})</div>
        </Cd>

        {/* E3: Aduana */}
        <Cd><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><span style={{fontSize:18}}>🏛</span><div><div style={{fontSize:14,fontWeight:700,color:"#FF3B30"}}>Etapa 3 — Derechos aduaneros</div><div style={{fontSize:10,color:"#8E8E93"}}>Aranceles, aforo, IVA importación</div></div></div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap",alignItems:"end"}}>
            <In l="Arancel" k="pctArancel" suf="% CIF" w={70}/>
            <div style={{marginBottom:8}}><div style={{fontSize:11,color:"#636366",fontWeight:600,marginBottom:2}}>TLC vigente</div><label style={{display:"flex",alignItems:"center",gap:4,cursor:"pointer"}}><input type="checkbox" checked={p.tieneILC} onChange={e=>us("tieneILC",e.target.checked)}/><span style={{fontSize:12}}>Arancel 0% por TLC</span></label></div>
            <In l="Aforo" k="pctAforo" suf="% CIF" w={60}/>
            <In l="IVA" k="pctIVA" suf="%" w={60}/>
          </div>
          <div style={{background:"#FF3B3008",borderRadius:8,padding:8,marginTop:4,fontSize:12}}>Arancel: <strong>{fmt(arancelCLP)}</strong>{p.tieneILC&&<span style={{color:"#34C759"}}> (TLC)</span>} · Aforo: <strong>{fmt(aforoCLP)}</strong> · IVA: <strong>{fmt(ivaCLP)}</strong> → <strong style={{color:"#FF3B30"}}>{fmt(totalAduana)}</strong></div>
        </Cd>

        {/* E4: Puerto */}
        <Cd><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><span style={{fontSize:18}}>⚓</span><div><div style={{fontSize:14,fontWeight:700,color:"#FF9500"}}>Etapa 4 — Gastos portuarios</div><div style={{fontSize:10,color:"#8E8E93"}}>Descarga, almacenaje, porteo</div></div></div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <In l="Handling" k="handling" pre="USD" w={80}/>
            <In l="Almacenaje/día" k="almacenajeDia" pre="USD" w={70}/>
            <In l="Días" k="diasAlmacenaje" w={50}/>
            <In l="Porteo" k="porteo" pre="USD" w={70}/>
          </div>
          <div style={{background:"#FF950008",borderRadius:8,padding:8,marginTop:4,fontSize:12}}>Total puerto: <strong style={{color:"#FF9500"}}>{fmt(totalPuerto)}</strong></div>
        </Cd>

        {/* E5: Agente */}
        <Cd><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><span style={{fontSize:18}}>📋</span><div><div style={{fontSize:14,fontWeight:700,color:"#AF52DE"}}>Etapa 5 — Agente de aduana</div></div></div>
          <div style={{display:"flex",gap:8}}><In l="Honorarios" k="honorarioAgente" pre="USD" w={90}/><In l="Gastos op." k="gastosOpAgente" pre="USD" w={90}/></div>
          <div style={{background:"#AF52DE08",borderRadius:8,padding:8,marginTop:4,fontSize:12}}>Total agente: <strong style={{color:"#AF52DE"}}>{fmt(agenteCLP)}</strong></div>
        </Cd>

        {/* E6: Transporte */}
        <Cd><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><span style={{fontSize:18}}>🚛</span><div><div style={{fontSize:14,fontWeight:700,color:"#34C759"}}>Etapa 6 — Transporte terrestre</div><div style={{fontSize:10,color:"#8E8E93"}}>Puerto → Bodega destino</div></div></div>
          <div style={{display:"flex",gap:8}}><In l="Transporte" k="transportePuertoBodega" pre="USD" w={90}/><In l="Seguro terr." k="seguroTerrestre" pre="USD" w={80}/></div>
          <div style={{background:"#34C75908",borderRadius:8,padding:8,marginTop:4,fontSize:12}}>Total transporte: <strong style={{color:"#34C759"}}>{fmt(transporteCLP)}</strong></div>
        </Cd>

        {/* E7: Financiero */}
        <Cd><div style={{display:"flex",alignItems:"center",gap:6,marginBottom:8}}><span style={{fontSize:18}}>💰</span><div><div style={{fontSize:14,fontWeight:700,color:"#FF2D55"}}>Etapa 7 — Costos financieros</div><div style={{fontSize:10,color:"#8E8E93"}}>Capital inmovilizado + comisiones</div></div></div>
          <div style={{display:"flex",gap:8,flexWrap:"wrap"}}>
            <In l="Costo fin. mensual" k="pctCostoFinanciero" suf="%" w={70}/>
            <In l="Días tránsito" k="diasTransito" suf="días" w={70}/>
            <In l="Com. transferencia" k="costoTransferencia" pre="USD" w={80}/>
          </div>
          <div style={{background:"#FF2D5508",borderRadius:8,padding:8,marginTop:4,fontSize:12}}>Costo financiero: <strong>{fmt(costoFinCLP)}</strong> + Transf: <strong>{fmt(costoTransfCLP)}</strong> → <strong style={{color:"#FF2D55"}}>{fmt(totalFinanciero)}</strong></div>
        </Cd>
      </div>
      ...
      <div>
        ...
        <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:10,marginBottom:14}}>
          <div style={{background:"linear-gradient(135deg,#1a1a2e,#16213e)",borderRadius:12,padding:18,color:"#fff"}}><div style={{fontSize:10,color:"rgba(255,255,255,0.5)",textTransform:"uppercase",letterSpacing:"0.1em"}}>Costo total internado</div><div style={{fontSize:28,fontWeight:800,marginTop:4}}>{fmt(costoTotalCLP)}</div><div style={{fontSize:12,color:"rgba(255,255,255,0.6)",marginTop:2}}>{fU(TC>0?costoTotalCLP/TC:0)}</div></div>
          <div style={{background:"#fff",borderRadius:12,padding:18,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",borderLeft:"4px solid #007AFF"}}><div style={{fontSize:10,color:"#8E8E93",textTransform:"uppercase",letterSpacing:"0.1em"}}>Costo unitario bodega</div><div style={{fontSize:28,fontWeight:800,color:"#007AFF",marginTop:4}}>{fmt(costoUnitBodega)}</div><div style={{fontSize:12,color:"#8E8E93",marginTop:2}}>USD {costoUnitUSD} · {fN(p.cantUds)} uds</div></div>
          <div style={{background:"#fff",borderRadius:12,padding:18,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",borderLeft:`4px solid ${margenPct>30?"#34C759":margenPct>15?"#FF9500":"#FF3B30"}`}}>
            <div style={{fontSize:10,color:"#8E8E93",textTransform:"uppercase",letterSpacing:"0.1em"}}>Margen proyectado</div>
            <div style={{fontSize:28,fontWeight:800,color:margenPct>30?"#34C759":margenPct>15?"#FF9500":"#FF3B30",marginTop:4}}>{margenPct}%</div>
            <div style={{marginTop:4}}><In l="" k="precioVentaCLP" pre="Venta CLP" w={100}/></div>
          </div>
        </div>

        {/* Waterfall chart */}
        <div style={{background:"#fff",borderRadius:12,padding:20,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",marginBottom:14}}>
          <div style={{fontSize:15,fontWeight:700,marginBottom:12}}>Composición del costo — Cascada por etapa</div>
          {stages.map((s,i)=>{const pct=costoTotalCLP>0?Math.round(s.v/costoTotalCLP*100):0;return<div key={i} style={{marginBottom:8}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:3}}>
              <div style={{display:"flex",alignItems:"center",gap:6}}><span>{s.ic}</span><span style={{fontSize:13,fontWeight:600}}>{s.n}</span></div>
              <div style={{display:"flex",gap:8,alignItems:"baseline"}}><span style={{fontSize:14,fontWeight:700,color:s.c}}>{fmt(s.v)}</span><span style={{fontSize:11,color:"#8E8E93"}}>{pct}%</span></div>
            </div>
            <div style={{width:"100%",background:"#F2F2F7",borderRadius:4,height:12,overflow:"hidden"}}><div style={{width:Math.max(s.v/maxStage*100,2)+"%",height:"100%",background:s.c,borderRadius:4,transition:"width 0.4s ease"}}/></div>
          </div>})}
          <div style={{borderTop:"2px solid #1C1C1E",paddingTop:8,marginTop:8,display:"flex",justifyContent:"space-between"}}><span style={{fontSize:15,fontWeight:700}}>COSTO TOTAL</span><span style={{fontSize:20,fontWeight:800}}>{fmt(costoTotalCLP)}</span></div>
        </div>

        {/* Cost structure pie-like breakdown */}
        <div style={{background:"#fff",borderRadius:12,padding:20,boxShadow:"0 1px 3px rgba(0,0,0,0.06)",marginBottom:14}}>
          <div style={{fontSize:15,fontWeight:700,marginBottom:12}}>Estructura de costos</div>
          <div style={{display:"flex",borderRadius:8,overflow:"hidden",height:32,marginBottom:12}}>
            {stages.map((s,i)=>{const pct=costoTotalCLP>0?s.v/costoTotalCLP*100:0;return pct>0?<div key={i} title={`${s.n}: ${Math.round(pct)}%`} style={{width:pct+"%",background:s.c,minWidth:pct>3?"auto":2,transition:"width 0.4s"}}/>:null})}
          </div>
          <div style={{display:"flex",flexWrap:"wrap",gap:12}}>{stages.map((s,i)=>{const pct=costoTotalCLP>0?Math.round(s.v/costoTotalCLP*100):0;return<div key={i} style={{display:"flex",alignItems:"center",gap:4}}><div style={{width:10,height:10,borderRadius:2,background:s.c}}/><span style={{fontSize:12}}>{s.n}: <strong>{pct}%</strong></span></div>})}</div>
        </div>

        {/* Comparison table */}
        <div style={{background:"#fff",borderRadius:12,padding:20,boxShadow:"0 1px 3px rgba(0,0,0,0.06)"}}>
          <div style={{fontSize:15,fontWeight:700,marginBottom:12}}>Resumen por etapa</div>
          <table style={{width:"100%",borderCollapse:"collapse",fontSize:13}}>
            <thead><tr style={{background:"#F8F8FA"}}><th style={{padding:"8px 10px",textAlign:"left",fontSize:10,fontWeight:700,color:"#636366",textTransform:"uppercase",borderBottom:"2px solid #E5E5EA"}}>Etapa</th><th style={{padding:"8px 10px",textAlign:"right",fontSize:10,fontWeight:700,color:"#636366",textTransform:"uppercase",borderBottom:"2px solid #E5E5EA"}}>USD</th><th style={{padding:"8px 10px",textAlign:"right",fontSize:10,fontWeight:700,color:"#636366",textTransform:"uppercase",borderBottom:"2px solid #E5E5EA"}}>CLP</th><th style={{padding:"8px 10px",textAlign:"right",fontSize:10,fontWeight:700,color:"#636366",textTransform:"uppercase",borderBottom:"2px solid #E5E5EA"}}>%</th><th style={{padding:"8px 10px",textAlign:"right",fontSize:10,fontWeight:700,color:"#636366",textTransform:"uppercase",borderBottom:"2px solid #E5E5EA"}}>Unit. CLP</th></tr></thead>
            <tbody>{stages.map((s,i)=>{const pct=costoTotalCLP>0?Math.round(s.v/costoTotalCLP*100):0;return<tr key={i} style={{borderBottom:"1px solid #F2F2F7"}}><td style={{padding:"8px 10px"}}><div style={{display:"flex",alignItems:"center",gap:6}}><div style={{width:10,height:10,borderRadius:2,background:s.c}}/><span style={{fontWeight:600}}>{s.ic} {s.n}</span></div></td><td style={{padding:"8px 10px",textAlign:"right",color:"#636366"}}>{fU(TC>0?s.v/TC:0)}</td><td style={{padding:"8px 10px",textAlign:"right",fontWeight:600}}>{fmt(s.v)}</td><td style={{padding:"8px 10px",textAlign:"right",color:s.c,fontWeight:700}}>{pct}%</td><td style={{padding:"8px 10px",textAlign:"right",color:"#8E8E93"}}>{p.cantUds>0?fmt(Math.round(s.v/p.cantUds)):"—"}</td></tr>})}</tbody>
            <tfoot><tr style={{borderTop:"2px solid #1C1C1E",background:"#F8F8FA"}}><td style={{padding:"10px",fontWeight:700}}>TOTAL INTERNADO</td><td style={{padding:"10px",textAlign:"right",fontWeight:700}}>{fU(TC>0?costoTotalCLP/TC:0)}</td><td style={{padding:"10px",textAlign:"right",fontSize:16,fontWeight:800}}>{fmt(costoTotalCLP)}</td><td style={{padding:"10px",textAlign:"right",fontWeight:700}}>100%</td><td style={{padding:"10px",textAlign:"right",fontWeight:700,color:"#007AFF"}}>{fmt(costoUnitBodega)}</td></tr></tfoot>
          </table>
        </div>
      </div>
    </div>
...
/* ═══ CONFIG ═══ */
function ConfigView({config,saveConfig,params,setParams,paramsABCD,setParamsABCD,provs,setProvs,users,setUsers,h,configTab,setConfigTab,loadAll}){
